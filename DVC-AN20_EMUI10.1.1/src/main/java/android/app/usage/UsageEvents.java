...
public final class UsageEvents implements Parcelable {
    ...
    private static final int MAX_EVENT_BUFF_LEN = 33554432;
        ...
        try {
            ...
            if (this.mEventCount > 0) {
                ...
                if (this.mEventsToWrite != null) {
                    ...
                    try {
                        p.setDataPosition(0);
                        for (int i = 0; i < this.mEventCount && p.dataPosition() < 33554432; i++) {
                            writeEventToParcel(this.mEventsToWrite.get(i), p, flags);
                        }
                        if (p.dataPosition() >= 33554432) {
                            Slog.w(TAG, "writeToParcel too long, event count: " + this.mEventCount + " limit to " + p.dataPosition());
                            SparseArray<Integer> counts = new SparseArray<>();
                            for (int i2 = 0; i2 < this.mEventCount; i2++) {
                                Event event = this.mEventsToWrite.get(i2);
                            ...
                            for (int i3 = 0; i3 < counts.size(); i3++) {
                                Slog.w(TAG, "event type: " + counts.keyAt(i3) + " count: " + counts.valueAt(i3));
                            }
                        }

...
public class HwResource extends AbsResources {
    ...
    public int rebuildSpecialDimens(int id, TypedValue value, int res) {
        ...
        if ((id == HwPartResourceUtils.getResourceId("status_bar_height") || id == HwPartResourceUtils.getResourceId("status_bar_height_portrait") || id == HwPartResourceUtils.getResourceId("status_bar_height_landscape")) && !isLockDpi() && !isTypedValueInMillimeters(value.data) && (dpi = getDpi()) > 0 && dpi != (i = sRealLcdDpi) && sIsSupportLockDpi) {

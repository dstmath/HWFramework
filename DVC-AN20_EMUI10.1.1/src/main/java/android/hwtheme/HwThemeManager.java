...
public class HwThemeManager {
    ...
    public static Bundle getPreMultidpiInfo(String packageName) {
        ...
        try {
            ...
            String str = TAG;
            Log.w(str, "getApplicationInfo for " + packageName + ", Exception:" + e);
            tmpInfo = null;
        }
        if (tmpInfo != null) {
    ...
    public static ArrayList<String> getDataSkinThemePackages() {
        ...
        if (themePath.exists()) {
            ...
            for (File f : files) {
                if (f.getName() == null || (!f.getName().contains("launcher") && !f.getName().contains("deskclock") && !f.getName().contains("aod"))) {

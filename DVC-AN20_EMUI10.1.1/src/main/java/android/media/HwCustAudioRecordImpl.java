...
public class HwCustAudioRecordImpl extends HwCustAudioRecord {
    ...
    private boolean isEcMicOpen = IS_OPEN_EC;
    private String padECName = "";
    ...
    public void preStartEC() {
        if (isSupportEc() && !this.isEcMicOpen) {
            ...
            AudioSystem.setParameters("ASR_VENDOR=" + this.padECName);
    ...
    public boolean isSupportEc() {
        if (IS_OPEN_EC) {
            this.padECName = getPADECName(PAD_EC_KEY);
            if (!TextUtils.isEmpty(this.padECName)) {

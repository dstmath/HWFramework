...
public class MediaScanner implements AutoCloseable {
    ...
    private void prescan(String filePath, boolean prescanFiles) throws RemoteException {
        ...
        String path2 = "";
        if (filePath != null) {
            selectionArgs = new String[]{path2, filePath};
            where = "_id>? AND _data=?";
        } else {
                                    ...
                                    try {
                                        ...
                                        if (path3 == null || !path3.startsWith("/")) {
                                            ...
                                            try {
                                                ...
                                                exists = Os.access(path3, OsConstants.F_OK);
                                            ...
                                            if (exists || MtpConstants.isAbstractObject(format)) {
                                                ...
                                                rowId2 = rowId3;
                                                rowId = accessTime;
                                            } else if (!MediaFile.isPlayListMimeType(MediaFile.getMimeTypeForFile(path3))) {
                                                builder2 = builder;
                                                where2 = where3;
                                                rowId2 = rowId3;
                                                try {
                                                    ...
                                                    rowId = accessTime;
                                                    if (path3.toLowerCase(Locale.US).endsWith("/.nomedia")) {
                                                        deleter.flush();
                                                        c = c2;
                                                        mediaScanner.mMediaProvider.call(MediaStore.UNHIDE_CALL, new File(path3).getParent(), null);
                                                    } else {
                                                        c = c2;
                                                    }
                                                } catch (Throwable th4) {
                                            ...
                                            if (!mediaScanner.mNeedFilter || !HwFrameworkFactory.getHwMediaScannerManager().isAudioFilterFile(path3)) {
                                                if (count3 < mediaScanner.mMaxFileCacheEntrySize) {
                                                    mediaScanner.mFileCache.put(path3, new FileEntry(rowId2, path3, lastModified, format, mediaType));
                                                }
                                                count3++;
                                                accessTime = rowId;
                                            } else {

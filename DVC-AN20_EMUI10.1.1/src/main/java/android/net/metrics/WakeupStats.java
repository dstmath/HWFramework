...
public class WakeupStats {
    ...
    public String toString() {
        ...
        for (int i = 0; i < this.ethertypes.size(); i++) {
            j.add(String.format("ethertype 0x%x: %d", Integer.valueOf(this.ethertypes.keyAt(i)), Integer.valueOf(this.ethertypes.valueAt(i))));
        }
        for (int i2 = 0; i2 < this.ipNextHeaders.size(); i2++) {
            j.add(String.format("ipNxtHdr %d: %d", Integer.valueOf(this.ipNextHeaders.keyAt(i2)), Integer.valueOf(this.ipNextHeaders.valueAt(i2))));
        }
        return j.toString();
    }

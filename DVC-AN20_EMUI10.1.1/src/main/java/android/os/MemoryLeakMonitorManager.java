...
public class MemoryLeakMonitorManager {
    ...
    private static final boolean isDebuggable = "1".equals(SystemProperties.get(SYSTEM_DEBUGGABLE, WifiEnterpriseConfig.ENGINE_DISABLE));
    ...
    public static void installMemoryLeakMonitor(Application application) {
        ...
        if (mloadedJar) {
            ...
            try {
                mClassLoader.loadClass(MONITOR_CLASS_NAME).getMethod("installMemoryLeakMonitor", Application.class).invoke(null, application);
            } catch (ClassNotFoundException ex) {
    ...
    public static void watchMemoryLeak(Object objReference) {
        if (mloadedJar) {
            ...
            try {
                mClassLoader.loadClass(MONITOR_CLASS_NAME).getMethod("watchMemoryLeak", Object.class).invoke(null, objReference);
            } catch (ClassNotFoundException ex) {

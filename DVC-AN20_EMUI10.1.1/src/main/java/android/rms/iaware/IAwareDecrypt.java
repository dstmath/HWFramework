...
public class IAwareDecrypt {
    ...
    public static InputStream decryptInputStream(Context context, InputStream ins) {
        ...
        InputStream ret = decryptInputStreamInner(context, inputStream);
        AwareLog.d(TAG, "decryptInputStream decrypt spend " + (System.currentTimeMillis() - start) + "ms!");
        return ret;
    }

    private static InputStream decryptInputStreamInner(Context context, InputStream inputStream) {
    ...
    private static boolean isNormalXml(InputStream inputStream) {
        ...
        try {
            ...
            inputStream.reset();
            if (r != 5 || !XML_HEAD.equals(new String(head, "utf-8"))) {
                return false;
            }
            return true;
        } catch (IOException e) {

...
public class HwInnerTelephonyManagerImpl extends DefaultHwInnerTelephonyManager {
    ...
    public boolean setDualCardMode(int nMode) {
        SystemPropertiesEx.set(HwTelephonyProperties.PROPERTY_NATIONAL_MODE, Integer.toString(nMode));
        return true;
    }

    public int getDualCardMode() {
        return SystemPropertiesEx.getInt(HwTelephonyProperties.PROPERTY_NATIONAL_MODE, 0);
    }
    ...
    public void validateInput(SignalStrength newSignalStrength) {
        ...
        if (newSignalStrength != null) {
            ...
            SignalStrengthEx.setGsmSignalStrength(newSignalStrength, i);
            if (SignalStrengthEx.getWcdmaRscp(newSignalStrength) > 0) {
                i2 = SignalStrengthEx.getWcdmaRscp(newSignalStrength) * -1;
            } else {
                i2 = -1;
            }
            SignalStrengthEx.setWcdmaRscp(newSignalStrength, i2);
            int i8 = 255;
            if (SignalStrengthEx.getWcdmaEcio(newSignalStrength) >= 0) {
                i3 = SignalStrengthEx.getWcdmaEcio(newSignalStrength) * -1;
            } else {
                i3 = 255;
            }
            SignalStrengthEx.setWcdmaEcio(newSignalStrength, i3);
            ...
            if (newSignalStrength.getEvdoDbm() <= 0 || newSignalStrength.getEvdoDbm() >= 125) {
                ...
                i4 = newSignalStrength.getEvdoDbm() * -1;
            }
            SignalStrengthEx.setEvdoDbm(newSignalStrength, i4);
            ...
            if (newSignalStrength.getEvdoSnr() > 0 && newSignalStrength.getEvdoSnr() <= 8) {
                i8 = newSignalStrength.getEvdoSnr();
            }
            SignalStrengthEx.setEvdoSnr(newSignalStrength, i8);
            if (SignalStrengthEx.getLteSignalStrength(newSignalStrength) >= 0) {
                i5 = SignalStrengthEx.getLteSignalStrength(newSignalStrength);
            } else {
                i5 = 99;
            }
            SignalStrengthEx.setLteSignalStrength(newSignalStrength, i5);
            int lteRsrp = SignalStrengthEx.getLteRsrp(newSignalStrength);
            int i9 = HwSignalStrength.WCDMA_STRENGTH_INVALID;
            if (lteRsrp < 44 || SignalStrengthEx.getLteRsrp(newSignalStrength) > 140) {
                i6 = Integer.MAX_VALUE;
            } else {
                i6 = SignalStrengthEx.getLteRsrp(newSignalStrength) * -1;
            }
            SignalStrengthEx.setLteRsrp(newSignalStrength, i6);
            if (SignalStrengthEx.getLteRsrq(newSignalStrength) < 3 || SignalStrengthEx.getLteRsrq(newSignalStrength) > 20) {
                i7 = Integer.MAX_VALUE;
            } else {
                i7 = SignalStrengthEx.getLteRsrq(newSignalStrength) * -1;
            }
            SignalStrengthEx.setLteRsrq(newSignalStrength, i7);
            if (SignalStrengthEx.getLteRssnr(newSignalStrength) >= -200 && SignalStrengthEx.getLteRssnr(newSignalStrength) <= 300) {
                i9 = SignalStrengthEx.getLteRssnr(newSignalStrength);
            }
            SignalStrengthEx.setLteRssnr(newSignalStrength, i9);
    ...
    private void validateInputNr(CellSignalStrengthNr strength, int... parms) {
        ...
        int ssRsrq = parms[4];
        int ssSinr = parms[5];
        CellSignalStrengthNrEx.setNrSignalStrength(strength, 0, (csiRsrp < 44 || csiRsrp > 140) ? HwSignalStrength.WCDMA_STRENGTH_INVALID : csiRsrp * -1);
        CellSignalStrengthNrEx.setNrSignalStrength(strength, 1, (csiRsrq < 3 || csiRsrq > 20) ? HwSignalStrength.WCDMA_STRENGTH_INVALID : csiRsrq * -1);
        CellSignalStrengthNrEx.setNrSignalStrength(strength, 2, (csiSinr < -23 || csiSinr > 23) ? HwSignalStrength.WCDMA_STRENGTH_INVALID : csiSinr * -1);
        CellSignalStrengthNrEx.setNrSignalStrength(strength, 3, (ssRsrp < 44 || ssRsrp > 140) ? HwSignalStrength.WCDMA_STRENGTH_INVALID : ssRsrp * -1);
        CellSignalStrengthNrEx.setNrSignalStrength(strength, 4, (ssRsrq < 3 || ssRsrq > 20) ? HwSignalStrength.WCDMA_STRENGTH_INVALID : ssRsrq * -1);
        CellSignalStrengthNrEx.setNrSignalStrength(strength, 5, (ssSinr < -40 || ssSinr > 23) ? HwSignalStrength.WCDMA_STRENGTH_INVALID : ssSinr * -1);
    }
    ...
    private void validateInputLte(CellSignalStrengthLte strength, int... parms) {
        ...
        int i = HwSignalStrength.WCDMA_STRENGTH_INVALID;
    ...
    private void validateInputCdma(CellSignalStrengthCdma strength, int... parms) {
        ...
        int evdoSnr = parms[4];
        int i2 = HwSignalStrength.WCDMA_STRENGTH_INVALID;
        CellSignalStrengthCdmaEx.setCdmaDbm(strength, (cdmaDbm <= 0 || cdmaDbm == Integer.MAX_VALUE) ? Integer.MAX_VALUE : cdmaDbm * -1);
        CellSignalStrengthCdmaEx.setCdmaEcio(strength, (cdmaEcio <= 0 || cdmaDbm == Integer.MAX_VALUE) ? Integer.MAX_VALUE : cdmaDbm * -1);
        if (evdoDbm <= 0 || evdoDbm >= 125 || evdoDbm == Integer.MAX_VALUE) {
            i = Integer.MAX_VALUE;
        } else {
            i = evdoDbm * -1;
        }
        CellSignalStrengthCdmaEx.setEvdoDbm(strength, i);
        ...
        CellSignalStrengthCdmaEx.setEvdoSnr(strength, i2);
    }
    ...
    private boolean needGetLevelByRule(int phoneId) {
        if (HwModemCapability.isCapabilitySupport(29)) {
            return HwTelephonyManager.getDefault().isNsaState(phoneId);
        }
        return false;
    }
    ...
    public int getLevelHw(SignalStrength signalStrength) {
        if (needGetLevelByRule(SignalStrengthEx.getPhoneId(signalStrength))) {
            return getLevelByRule(signalStrength);
        }
        int nrLevel = SignalStrengthEx.getNrLevel(signalStrength);
        if (nrLevel == 0) {
            return SignalStrengthEx.getPrimaryLevelHw(signalStrength);
        }
        return nrLevel;

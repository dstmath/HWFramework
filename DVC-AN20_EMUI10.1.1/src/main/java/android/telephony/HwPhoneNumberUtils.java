...
public class HwPhoneNumberUtils {
    ...
    public static String custExtraNumbers(long subId, String numbers) {
        ...
        if (TelephonyManagerEx.isMultiSimEnabled()) {
            ...
            custNumbers = SystemPropertiesEx.get(HwTelephonyProperties.PROPERTY_GLOBAL_CUST_ECCLIST + slotId, "");
        } else {
            custNumbers = SystemPropertiesEx.get(HwTelephonyProperties.PROPERTY_GLOBAL_CUST_ECCLIST, "");
        }
    ...
    public static boolean isRemoveSeparateOnSK() {
        return SystemPropertiesEx.get("ro.config.noFormateCountry", "").contains(Locale.getDefault().getCountry());
    }

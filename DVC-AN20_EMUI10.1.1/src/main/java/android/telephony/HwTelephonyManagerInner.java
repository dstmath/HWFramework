...
public class HwTelephonyManagerInner {
    ...
    public boolean isCTSimCard(int slotId) {
        ...
        if (result) {
            String preIccid = SystemPropertiesEx.get("gsm.sim.preiccid_" + slotId, "");
    ...
    private String getCplmn(int slotId) {
        String result = "";
        if (slotId == 0) {
            result = SystemPropertiesEx.get(PROP_VALUE_C_CARD0_PLMN, "");
        } else if (slotId == 1) {
            result = SystemPropertiesEx.get(PROP_VALUE_C_CARD1_PLMN, "");
        }
        if (!TextUtils.isEmpty(result)) {
            RlogEx.e(TAG, "result not null getCplmn for Slot : " + slotId + " result is : " + result);
            return result;
        }
        String value = SystemPropertiesEx.get(PROP_VALUE_C_CARD_PLMN, "");
    ...
    public boolean isSetDefault4GSlotIdEnabled() {
        RlogEx.i(TAG, "In isSetDefault4GSlotIdEnabled");
        try {
    ...
    public static /* synthetic */ class AnonymousClass1 {
        ...
        static {
            try {
                $SwitchMap$android$telephony$HwTelephonyManagerInner$DataSettingModeType[DataSettingModeType.MODE_LTETDD_ONLY.ordinal()] = 1;
            } catch (NoSuchFieldError e) {
            }
            try {
                $SwitchMap$android$telephony$HwTelephonyManagerInner$DataSettingModeType[DataSettingModeType.MODE_LTE_AND_AUTO.ordinal()] = 2;
    ...
    public boolean isAppInWhiteList(String appName) {
        if ("com.android.phone".equals(appName) || "system".equals(appName) || "com.android.systemui".equals(appName) || "com.android.settings".equals(appName) || "com.huawei.systemmanager".equals(appName) || "com.huawei.vassistant".equals(appName) || "com.huawei.systemmanager:service".equals(appName) || "com.huawei.hiassistantoversea".equals(appName)) {
            return true;
        }
        return false;
    }
    ...
    public String getRegPlmn(int slotId) {
        return HwVSimManager.getDefault().getRegPlmn(slotId);
    }
    ...
    public int getCpserr(int slotId) {
        return HwVSimManager.getDefault().getCpserr(slotId);
    }
    ...
    public boolean setUserReservedSubId(int slotId) {
        return HwVSimManager.getDefault().setUserReservedSubId(slotId);
    }
    ...
    public int getLteLevel(SignalStrength strength) {
        if (SignalStrengthEx.isCdma(strength)) {
            if (IS_USE_RSRQ) {
                return getLevel(6, SignalStrengthEx.getLteRsrp(strength), SignalStrengthEx.getLteRsrq(strength), SignalStrengthEx.getPhoneId(strength));
            }
            return getLevel(6, SignalStrengthEx.getLteRsrp(strength), SignalStrengthEx.getLteRssnr(strength), SignalStrengthEx.getPhoneId(strength));
        } else if (IS_USE_RSRQ) {
            return getLevel(5, SignalStrengthEx.getLteRsrp(strength), SignalStrengthEx.getLteRsrq(strength), SignalStrengthEx.getPhoneId(strength));
        } else {
            return getLevel(5, SignalStrengthEx.getLteRsrp(strength), SignalStrengthEx.getLteRssnr(strength), SignalStrengthEx.getPhoneId(strength));
        }
    }
    ...
    public int getCdmaLevel(SignalStrength strength) {
        ...
        return getLevel(3, strength.getCdmaDbm(), strength.getCdmaEcio(), SignalStrengthEx.getPhoneId(strength));
    }
    ...
    public int getEvdoLevel(SignalStrength strength) {
        ...
        return getLevel(4, strength.getEvdoDbm(), strength.getEvdoSnr(), SignalStrengthEx.getPhoneId(strength));
    }
    ...
    public int getGsmLevel(SignalStrength strength) {
        ...
        int wcdmaLevel = getLevel(2, SignalStrengthEx.getWcdmaRscp(strength), SignalStrengthEx.getWcdmaEcio(strength), SignalStrengthEx.getPhoneId(strength));
        return wcdmaLevel == 0 ? getLevel(1, strength.getGsmSignalStrength(), 255, SignalStrengthEx.getPhoneId(strength)) : wcdmaLevel;
    }
    ...
    public boolean isAisCustomDisable() {
        if (INVALID_MCCMNC.equals(SystemPropertiesEx.get(HW_CUST_SW_SIMLOCK, ""))) {
            return true;
        }
        return false;
    }

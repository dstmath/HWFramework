...
public class HwViewRootImpl extends DefaultHwViewRootImpl {
    ...
    public void onBatchedInputConsumed(long starttime) {
        this.mJankDetector.onBatchedInputConsumed(starttime);
    }
    ...
    public void setLastFrameDoneTime(long time) {
        this.mChgState.mLastFrameDoneTime = time;
    }
    ...
    public void setFrameDelayTime(long time) {
        this.mChgState.mFrameDelayTimeOnSync = time;
    }
    ...
    public void checkOldestInputTime() {
        if (!this.mChgState.mInDoframe && this.mChgState.mCallBackCounts <= 0) {
            this.mChgState.mOldestInputTime = Long.MAX_VALUE;
        }
    }
    ...
    public final class JankDetector {
        ...
        private final int CONTINUOUS_REF = SystemPropertiesEx.getInt("ro.config.hw_jank_continuousref", (int) Vibetonz.HAPTIC_EVENT_BULK_MOVE_ICON_GATHER);
        private final int ONTIME_REF = SystemPropertiesEx.getInt("ro.config.hw_jank_ontimeref", 1);
        private ChoreographerState mChgState = null;
        private long mDeliverInputTime = 0;
        final ArrayList<Pair<Long, Long>> mJankList = new ArrayList<>();
        private int mLastFrameDefer = 0;
        private long mLatestSkips = 0;
        private long mSoftDrawTime = 0;
        ...
        public void onBatchedInputConsumed(long starttime) {
            this.mDeliverInputTime = System.nanoTime() - starttime;
        }
        ...
        private void newCheckSkippedFrame(long nowtime, long frameVsynctime, String windowtitle) {
            ...
            if (bLastTraversal) {
                ...
                if (skippedFrames >= 5) {
                    StringBuilder traceMark = new StringBuilder();
                    time = 0;
                    traceMark.append("jank_event_sync: start_ts=");
                    traceMark.append(frameVsynctime);
                    traceMark.append(",end_ts=");
                    traceMark.append(nowtime);
                    traceMark.append(", appid=");
                    traceMark.append(Process.myPid());
                    TraceEx.traceBegin(TraceEx.getTraceTagView(), traceMark.toString());
                ...
                if (skippedFrames >= 15) {
                    ...
                    long totalJankNum = (long) getTotalJankNum();
                    if (totalJankNum >= 90) {
                        ...
                        seqFrameSkipMsg.append(totalJankNum);
        ...
        public void processAfterTraversal(boolean scroll, long[] jankdrawData, String windowtitle) {
            ...
            long delaytime = this.mChgState.mFrameDelayTimeOnSync;
            long lastFrameDoneTime = this.mChgState.mLastFrameDoneTime;
            long frameTime = this.mChgState.mRealFrameTime;
            boolean bOntime = delaytime < ((long) this.ONTIME_REF);
            newCheckSkippedFrame(now, frameTime, windowtitle);
            if (bOntime) {
            ...
            if (!bOntime || now - frameTime >= VSYNC_SPAN - 2500000) {
                if (scroll && frameTime - lastFrameDoneTime < ((long) this.CONTINUOUS_REF)) {
                ...
                if (bContinuousUpdate) {
                    ...
                    if (skip == 0) {
                        this.mLastFrameDefer = 1;
                    } else if (skip != 1) {
                        if (skip > 1) {
                            this.mLastFrameDefer = 0;
                            skip = (skip - 1) - this.mLastFrameDefer;
                        }
                        if (skip >= 5) {
                            StringBuilder oldFrameSkipMsg = new StringBuilder();
                            oldFrameSkipMsg.append("#P:");
                            oldFrameSkipMsg.append(windowtitle);
                            oldFrameSkipMsg.append("#SK:");
                            oldFrameSkipMsg.append(skip);
                            if (jankdrawData != null) {
                                oldFrameSkipMsg.append("#IP:");
                                oldFrameSkipMsg.append(this.mDeliverInputTime / 10000);
                                oldFrameSkipMsg.append("#DR:");
                                oldFrameSkipMsg.append(jankdrawData[0] / 10000);
                                oldFrameSkipMsg.append("#PRO:");
                                oldFrameSkipMsg.append(jankdrawData[1] / 10000);
                                oldFrameSkipMsg.append("#EX:");
                                oldFrameSkipMsg.append(jankdrawData[2] / 10000);
                                oldFrameSkipMsg.append("#TRA:");
                                oldFrameSkipMsg.append(jankdrawData[3] / 10000);
                            } else if (jankdrawData == null) {
                                oldFrameSkipMsg.append("#IP:");
                                oldFrameSkipMsg.append(this.mDeliverInputTime / 10000);
                                oldFrameSkipMsg.append("#DR:");
                                oldFrameSkipMsg.append(0);
                                oldFrameSkipMsg.append("#PRO:");
                                oldFrameSkipMsg.append(0);
                                oldFrameSkipMsg.append("#EX:");
                                oldFrameSkipMsg.append(0);
                                oldFrameSkipMsg.append("#TRA:");
                                oldFrameSkipMsg.append(this.mSoftDrawTime / 10000);
                            }
                            oldFrameSkipMsg.append("#FRT:");
                            oldFrameSkipMsg.append(frameTime / 10000);
                            oldFrameSkipMsg.append("#DNT:");
                            oldFrameSkipMsg.append(now / 10000);
                            oldFrameSkipMsg.append("#LFT:");
                            oldFrameSkipMsg.append(lastFrameDoneTime / 10000);
                            Jlog.d(67, oldFrameSkipMsg.toString());
                        }
                    }
                }
            } else {

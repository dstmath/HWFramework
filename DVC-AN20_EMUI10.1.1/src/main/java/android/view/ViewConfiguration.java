...
public class ViewConfiguration {
    ...
    private ViewConfiguration(Context context) {
        ...
        if (!HwMwUtils.ENABLED || thread == null || !thread.isEnableMagic) {
    ...
    public static ViewConfiguration get(Context context) {
        ...
        ViewConfiguration configuration2;
        int density = (int) (context.getResources().getDisplayMetrics().density * 100.0f);
        if (HwFoldScreenManager.isFoldable()) {
            DisplayMetrics realMetrics = new DisplayMetrics();
            ((WindowManager) context.getSystemService("window")).getDefaultDisplay().getRealMetrics(realMetrics);
            int key = (realMetrics.heightPixels * realMetrics.widthPixels) + density;
            synchronized (sConfigurations) {
                configuration2 = sConfigurations.get(key);
            }
            if (configuration2 != null) {
                return configuration2;
            }
            ViewConfiguration configuration3 = new ViewConfiguration(context);
            synchronized (sConfigurations) {
                sConfigurations.put(key, configuration3);
            }
        ...
        ViewConfiguration configuration4 = new ViewConfiguration(context);
        synchronized (sConfigurations) {
            sConfigurations.put(density, configuration4);
        }

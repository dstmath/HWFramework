...
public class OverScroller {
    ...
    public static class SplineOverScroller {
        ...
        public boolean update() {
            ...
            if (i2 != 0) {
                if (i2 == 1) {
                    float t = ((float) currentTime) / ((float) i);
                    float t2 = t * t;
                    float sign = Math.signum((float) this.mVelocity);
                    int i3 = this.mOver;
                    this.mCurrVelocity = ((float) i3) * sign * 6.0f * ((-t) + t2);
                    distance = this.mIHwSplineOverScrollerInner.getCubicDistance((double) (((float) i3) * sign * ((3.0f * t2) - ((2.0f * t) * t2))), this.mStart, this.mFinal, (long) this.mDuration, currentTime);
                } else if (i2 == 2) {
                    float t3 = ((float) currentTime) / 1000.0f;
                    int i4 = this.mVelocity;
                    float f = this.mDeceleration;
                    this.mCurrVelocity = ((float) i4) + (f * t3);
                    distance = this.mIHwSplineOverScrollerInner.getBallisticDistance((double) ((((float) i4) * t3) + (((f * t3) * t3) / 2.0f)), this.mStart, this.mFinal, (long) i, currentTime);
                }
            } else if (OverScroller.mHwSmartSlideOptimize.isOptimizeEnable()) {
                this.mCurrVelocity = OverScroller.mHwSmartSlideOptimize.getUpdateVelocity(currentTime, this.mSplineDuration, this.mVelocityStart);
                distance = OverScroller.mHwSmartSlideOptimize.getUpdateDistance(currentTime, this.mSplineDuration, this.mSplineDistance);
                ...
                float t4 = ((float) currentTime) / ((float) this.mSplineDuration);
                int index = (int) (t4 * 100.0f);
                ...
                if (index < 100) {
                    ...
                    distanceCoef = d_inf + ((t4 - t_inf) * velocityCoef);
                }
                int i5 = this.mSplineDistance;
                distance = (double) (((float) i5) * distanceCoef);
                this.mCurrVelocity = ((((float) i5) * velocityCoef) / ((float) this.mSplineDuration)) * 1000.0f;
            }
            this.mCurrentPosition = this.mStart + ((int) Math.round(distance));
            return true;
        }

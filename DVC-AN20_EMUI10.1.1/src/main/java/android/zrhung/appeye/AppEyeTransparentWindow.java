...
public final class AppEyeTransparentWindow extends ZrHungImpl {
    ...
    public int init(ZrHungData zrHungData) {
        if (this.mHungConfig == null || this.mHungConfigStatus == 1) {
            this.mHungConfig = ZRHung.getHungConfig(ZRHung.APPEYE_TWIN);
    ...
    public boolean sendEvent(ZrHungData zrHungData) {
        ...
        try {
            Object layoutParam = zrHungData.get("layoutParams");
            ...
            data.putString(DataContract.BaseProperty.PACKAGE_NAME, this.mTransPackage);
            data.putInt("pid", pid);
            sendAppEyeEvent(ZRHung.APPEYE_TWIN, data, null, eventBuffer.toString());
            return true;
        } catch (Exception e) {
            Slog.e(TAG, "send event exception");
            return false;
        }
    }
    ...
    private boolean isZeroAlphaWindow(WindowManager.LayoutParams layoutParams) {
        return layoutParams.alpha < 0.01f;
    }

    private boolean isZeroPixelWindow(WindowManager.LayoutParams layoutParams) {
        return layoutParams.width <= 1 && layoutParams.width >= 0 && layoutParams.height <= 1 && layoutParams.height >= 0;
    }

    private boolean isTranslucentThemeWithoutActionBar(WindowManager.LayoutParams layoutParams) {
        return ((layoutParams.hwFlags & 268435456) == 0 || (layoutParams.hwFlags & AppTypeInfo.APP_ATTRIBUTE_FORCE_DARK) == 0) ? false : true;
    }
}

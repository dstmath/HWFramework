...
public class HwImsManagerInner {
    ...
    public static void factoryReset(Context context, int slotId) {
        ...
        if (hasFiledInDbByFiledName(currentSubId, context, "wfc_ims_roaming_enabled")) {
            if (getBooleanCarrierConfig(context, "carrier_default_wfc_ims_roaming_enabled_bool", currentSlotId)) {
                wfcroaming = 1;
            }
            SubscriptionManagerEx.setSubscriptionProperty(currentSubId, "wfc_ims_roaming_enabled", Integer.toString(wfcroaming));
        }
    ...
    private static int getVolteDefaultValue(Context context, int currentSlotId) {
        if (!HwTelephonyManagerInner.getDefault().isCTSimCard(currentSlotId)) {
            return getBooleanCarrierConfig(context, KEY_CARRIER_DEFAULT_VOLTE_SWITCH_ON_BOOL, currentSlotId) ? 1 : 0;
        }
        if (CL_VOLTE_SWITCH_ON) {
            return 1;
        }
        return 0;
    }

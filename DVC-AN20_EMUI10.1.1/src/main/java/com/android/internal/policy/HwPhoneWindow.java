...
public class HwPhoneWindow extends PhoneWindow {
    static final boolean DEBUG = (SystemProperties.getInt("ro.logsystem.usertype", 1) == 3);
    private static final int FLOATING_MASK = Integer.MIN_VALUE;
    protected static final boolean IS_NOTCH_PROP = (true ^ SystemProperties.get("ro.config.hw_notch_size", BuildConfig.FLAVOR).equals(BuildConfig.FLAVOR));
    ...
    public int getHeightMeasureSpec(int fixh, int heightSize, int defaultHeightMeasureSpec) {
        ...
        if (fixh < metrics.heightPixels) {
            fixh -= (int) (((float) getContext().getResources().getDimensionPixelSize(17105443)) * ((((float) fixh) + 0.0f) / ((float) metrics.heightPixels)));
        }
    ...
    public void updateLayoutParamsColor() {
        ...
        int emuiValue = -1;
        if ((attrs.privateFlags & AppTypeInfo.APP_ATTRIBUTE_FORCE_DARK) == 0) {
    ...
    public void initSplitMode() {
        ...
        if (c != null && (c instanceof Activity) && (intent = ((Activity) c).getIntent()) != null) {
    ...
    public void setHwFlagForNotch(boolean isTranslucent) {
        if (IS_NOTCH_PROP && isTranslucent) {
            WindowManager.LayoutParams attrs = getAttributes();
            attrs.hwFlags = (attrs.hwFlags & -32769) | TpCommandConstant.TSA_EVENT_POWER_DOUBLE_CLICK;
        }
    }
    ...
    public void setHwFlagForInvisibleWindowDetection(boolean isTranslucent, boolean noActionBar) {
        ...
        if (noActionBar) {
            attrs.hwFlags = (attrs.hwFlags & -536870913) | AppTypeInfo.APP_ATTRIBUTE_FORCE_DARK;
        }
    }

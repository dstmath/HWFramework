...
public class HwDualCardSwitcher extends Handler {
    ...
    private void checkIfNeedToSwitchSlot() {
        boolean isVSimWorking;
        boolean isMDMCarrierWorking;
        ...
        if (HwTelephonyManagerInner.getDefault().getDefaultMainSlotCarrier() != 0) {
            isMDMCarrierWorking = true;
        } else {
            isMDMCarrierWorking = false;
        }
        if (isMDMCarrierWorking) {
    ...
    private boolean shouldSwitch() {
        ...
        if (otherIsInService && !otherIsRoaming) {
            ...
            if (hwTelephonyManagerInner != null && hwTelephonyManagerInner.isCTSimCard(this.mSecondarySlot) && !TextUtils.isEmpty(regPlmn) && !regPlmn.startsWith(MCC_OF_CN)) {
                logd("CT card not roaming, register " + (LogEx.getLogHWInfo() ? regPlmn : "***"));
            ...
            if (primaryIsInService && primaryIsRoaming) {
                ...
                logd("secondary card register " + (LogEx.getLogHWInfo() ? regPlmn : "***") + ", primary card's mcc is " + (LogEx.getLogHWInfo() ? mccNoService : "***"));
        ...
        if (msg != null) {
            switch (msg.what) {
                ...
                case 2:
                    ...
                    return;
                case HwFullNetworkConstantsInner.CARD_TYPE_DUAL_MODE /*{ENCODED_INT: 3}*/:

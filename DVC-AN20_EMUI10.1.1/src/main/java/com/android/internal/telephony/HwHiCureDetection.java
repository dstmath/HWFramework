...
public class HwHiCureDetection {
    ...
    public void put(int phoneId, PhoneExt phoneExt) {
        ...
        registerNetRejectInfo(phoneId, phone);
        this.mPhoneMap.put(phoneId, phone);
        RlogEx.i(TAG, "put: phoneId = " + phoneId + " phone = " + phone);
    }

    private void registerNetRejectInfo(int phoneId, Phone phone) {
        Phone cachePhone = this.mPhoneMap.get(phoneId, null);
        if (cachePhone != null) {
            cachePhone.mCi.unSetOnNetReject(this.mDetectionHandler);
        }
        if (phone != null) {
            phone.mCi.setOnNetReject(this.mDetectionHandler, 1, Integer.valueOf(phoneId));
        }
    }
    ...
    private boolean canPerformReattach(int subId) {
        return isPhoneTypeGsm(subId) && isReattachAllowed(subId);
    }

    private boolean isPhoneTypeGsm(int subId) {
        Phone phone = this.mPhoneMap.get(SubscriptionManager.getPhoneId(subId), null);
    ...
    private boolean isReattachAllowed(int subId) {
        RejectInfo rejectInfo = this.mRejctCauses.get(SubscriptionManager.getPhoneId(subId), null);
        if (rejectInfo == null) {
            return true;
        }
    ...
    private boolean canPerformReconnect(int subId) {
        ...
        DcTracker dcTracker = getDcTracker(SubscriptionManager.getPhoneId(subId));
        if (dcTracker == null) {
            RlogEx.e(TAG, "canPerformReconnect: get dct error. subId = " + subId);
            return false;
        }
        Map<String, ApnContext> mApnContexts = dcTracker.getMApnContextsHw();
        if (mApnContexts == null || (apnContext = mApnContexts.get(DEFAULT_APN_TYPE)) == null || apnContext.getState() != DctConstants.State.RETRYING) {
            return false;
        }
        return true;
    }
    ...
    private GsmCdmaCallTracker getGsmCdmaCallTracker(int phoneId) {
        Phone phone = this.mPhoneMap.get(phoneId, null);
        if (phone == null) {
            RlogEx.e(TAG, "getGsmCdmaCallTracker: phone is null");
            return null;
    ...
    private ServiceStateTracker getServiceStateTracker(int phoneId) {
        Phone phone = this.mPhoneMap.get(phoneId, null);
        if (phone == null) {
            RlogEx.e(TAG, "getServiceStateTracker: phone is null");
            return null;
        } else if (phone instanceof GsmCdmaPhone) {
    ...
    private DcTracker getDcTracker(int phoneId) {
        Phone phone = this.mPhoneMap.get(phoneId, null);
        if (phone != null) {
            return phone.getDcTracker(1);
        }
    ...
    private void reConnectData(int subId) {
        ...
        dcTracker.setupDataOnConnectableApnsHw("pdpReset");
    }

    private void reAttach(int subId) {
        ServiceStateTracker sst = getServiceStateTracker(SubscriptionManager.getPhoneId(subId));
        if (sst == null) {
            RlogEx.e(TAG, "reAttach: sst is null");
            return;
        }
        RlogEx.i(TAG, "reAttach");
        sst.reRegisterNetwork((Message) null);
    }
    ...
    private String getTelecomOperatorNumericSelection(int subId) {
        ...
        return "" + (telecomSelection + ") or visible is null)");
    }
    ...
    private boolean isOverSeaUser() {
        if (SystemProperties.getInt("ro.config.hw_optb", (int) INVALID_MCC) != 156) {
            return true;
        }
        return false;
    }

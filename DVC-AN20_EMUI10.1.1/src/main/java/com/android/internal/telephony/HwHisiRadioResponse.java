...
public class HwHisiRadioResponse extends IHisiRadioResponse.Stub {
    ...
    public void getDeviceVersionResponse(RadioResponseInfo responseInfo, RILDeviceVersionResponse deviceVersion) {
        ...
        String[] response = new String[DEVICE_VERSION_LENGTH];
        response[0] = deviceVersion.buildTime;
        response[1] = deviceVersion.externalSwVersion;
        response[2] = deviceVersion.internalSwVersion;
        response[3] = deviceVersion.externalDbVersion;
        response[4] = deviceVersion.internalDbVersion;
        response[5] = deviceVersion.externalHwVersion;
        response[6] = deviceVersion.internalHwVersion;
        response[7] = deviceVersion.externalDutName;
        response[8] = deviceVersion.internalDutName;
        response[9] = deviceVersion.configurateVersion;
        response[10] = deviceVersion.prlVersion;
    ...
    private DataCallResponse convertToRadioSetupDataCallResponse(SetupDataCallResult result) {
        ...
        int cause = result.status;
        int suggestedRetryTime = result.suggestedRetryTime;
        int cid = result.cid;
        int active = result.active;
        int protocolType = ApnSetting.getProtocolIntFromString(result.type);
        int mtu = result.mtu;
        String ifname = result.ifname;
        ...
        return new DataCallResponse(cause, suggestedRetryTime, cid, active, protocolType, ifname, parseLinkAddressResponse(addresses), parseDataCallResultResponse(dnses), parseDataCallResultResponse(gateways), parseDataCallResultResponse(pcscfs), mtu);
    }
            ...
            if (!address2.isEmpty()) {
                try {
                    if (address2.split("/").length == 2) {
                        ...
                        linkAddress = new LinkAddress(inetAddress, inetAddress instanceof Inet4Address ? 32 : 64);
                    }

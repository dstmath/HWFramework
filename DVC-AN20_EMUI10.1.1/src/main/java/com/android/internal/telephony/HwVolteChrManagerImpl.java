...
public class HwVolteChrManagerImpl implements HwVolteChrManager {
    ...
    public void updateMtCallLog(int event) {
        ...
        if (2 == event || 9 == event) {
            ...
            sb.append(callCountEvent.toString());
            sb.append(CALL_TYPE_AUDIO);
    ...
    private CallEndReason getCallEndReason(int causeCode, int remoteCauseCode) {
        int SIP_CODE_MIN = HwImsCallFailCause.IMS_ERROR_BASE + 300;
        int SIP_CODE_MAX = HwImsCallFailCause.IMS_ERROR_BASE + 606;
    ...
    private Connection getFirstHandoverConnection() {
        ...
        if (phone != null && (connections = ((ImsPhone) phone).getHandoverConnection()) != null) {

...
public class WapPushOverSms implements ServiceConnection, IWapPushOverSmsInner {
    ...
    /* JADX INFO: Multiple debug info for r0v29 int: [D('dataIndex' int), D('intentData' byte[])] */
    ...
    public int dispatchWapPdu(byte[] pdu, BroadcastReceiver receiver, InboundSmsHandler handler, String address) {
        ...
        if (HwTelephonyFactory.getHwInnerSmsManager().handleWapPushExtraMimeType(result.mimeType)) {
            ...
            if (this.mSmsTracker != null && HwTelephonyFactory.getHwInnerSmsManager().newSmsShouldBeIntercepted(this.mContext, intent2, handler, this.mSmsTracker.getDeleteWhere(), this.mSmsTracker.getDeleteWhereArgs(), true)) {
                return -1;
            }

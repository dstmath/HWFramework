...
public class DcTracker extends Handler {
    ...
    private static final boolean HW_RADIO_DATA_STALL_ENABLE = SystemProperties.getBoolean("ro.config.hw.data.stall.enabled", true);
                    ...
                    if ("android.intent.action.SCREEN_ON".equals(action)) {
                        ...
                        if (!TextUtils.isEmpty(action_tag) && action_tag.equals("action_disable_data_4G") && DcTracker.this.mPhone != null && DcTracker.this.mPhone.getPhoneId() == 1) {
                            if (intent.getBooleanExtra("dataState", false)) {
                                DcTracker.this.cleanUpAllConnections(PhoneInternalInterface.REASON_DATA_DISABLED_INTERNAL);
                            } else {
                                DcTracker.this.setupDataOnAllConnectableApns(PhoneInternalInterface.REASON_DATA_ENABLED, RetryFailures.ALWAYS);
                            }
                        }
                    } else {
    ...
    public class ProvisionNotificationBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            DcTracker.this.mProvisioningSpinner.setMessage(context.getText(17040486));
    ...
    private void initApnContexts() {
        ...
        for (String networkConfigString : this.mPhone.getContext().getResources().getStringArray(17236091)) {
    ...
    public boolean isDataAllowed(ApnContext apnContext, int requestType, DataConnectionReasons dataConnectionReasons) {
        ...
        int slotId = SubscriptionController.getInstance().getSlotIndex(dataSub);
        ...
        boolean isMeteredApnType2 = apnContext == null || ApnSettingUtils.isMeteredApnType(ApnSetting.getApnTypesBitmaskFromString(apnContext.getApnType()), this.mPhone);
        ...
        if (!attachedState) {
            if (!shouldAutoAttach() || this.mPhone.getSubId() != dataSub) {
                if (requestType != 2 && !this.mHwDcTrackerEx.isNeedForceSetup(apnContextEx)) {
        ...
        if (slotId == 1) {
            if (this.mHwDcTrackerEx.isDataConnectivityDisabled(1, "disable-data")) {
                reasons.add(DataConnectionReasons.DataDisallowedReasonType.INTERNAL_DATA_DISABLED);
                cleanUpAllConnections(PhoneInternalInterface.REASON_DATA_DISABLED_INTERNAL);
            }
        }
        HwCustDcTracker hwCustDcTracker3 = this.mHwCustDcTracker;
        ...
        if (!reasons.containsHardDisallowedReasons()) {
            if (!reasons.allowed()) {
                ...
                if ((this.mHwCustDcTracker != null && apnContext != null && reasons.allowed() && this.mHwCustDcTracker.isRoamDisallowedByCustomization(apnContext)) || (apnContext != null && "mms".equals(apnContext.getApnType()) && this.mPhone.getServiceState().getDataRoaming() && this.mHwDcTrackerEx.isRoamingPushDisabled())) {
                    reasons.add(DataConnectionReasons.DataDisallowedReasonType.ROAMING_DISABLED);
                }
            } else {
                z = false;
    ...
    private void setupDataOnConnectableApn(ApnContext apnContext, String reason, RetryFailures retryFailures) {
        ...
        if (isDefaultDataSubscription() && !apnContext.isEnabled() && PhoneInternalInterface.REASON_SIM_LOADED.equals(reason) && TransportManager.IWLAN_OPERATION_MODE_DEFAULT.equals(apnContext.getApnType()) && this.mHwDcTrackerEx.isDataNeededWithWifiAndBt()) {
    ...
    private boolean cleanUpAllConnectionsInternal(boolean detach, String reason) {
        ...
        for (ApnContext apnContext : this.mApnContexts.values()) {
            if (reason == null || !reason.equals(PhoneInternalInterface.REASON_SINGLE_PDN_ARBITRATION) || !apnContext.getApnType().equals("ims")) {
    ...
    private void setInitialAttachApn() {
        ...
        firstNonEmergencyApnSetting = null;
        firstNonEmergencyApnSetting = null;
        if (this.mHwDcTrackerEx.getPrimarySlot() != this.mPhone.getPhoneId() && !HwModemCapability.isCapabilitySupport(21)) {
            ...
            if (esmFlag5 == 0) {
                ...
                if (apnSetting != null) {
                    ...
                    if (!this.mHwDcTrackerEx.isApnPreset(apnSetting)) {
                        esmFlag6 = esmFlag5;
                        if (this.mPreferredApn.canHandleType(256)) {
            ...
            if (IS_PDN_REJ_CURE_ENABLE) {
                this.mHwDcTrackerEx.setEsmFlag(esmFlag6);
            ...
            if (esmFlag7 != 0) {
                ...
                if (apnSetting2 != null && apnSetting2.canHandleType(256)) {
    ...
    private void onApnChanged() {
        ...
        this.mHwDcTrackerEx.resetDataCureInfo(PhoneInternalInterface.REASON_APN_CHANGED);
        createAllApnList();
    ...
    private void startAlarmForReconnect(long delay, ApnContext apnContext) {
        ...
        int requestCode = this.mTransportType == 1 ? phoneId * 2 : (phoneId * 2) + 1;
        log("startAlarmForReconnect requestCode is: " + requestCode);
        PendingIntent alarmIntent = PendingIntent.getBroadcast(this.mPhone.getContext(), requestCode, intent, 134217728);
    ...
    private void onRecordsLoadedOrSubIdChanged() {
        ...
        this.mHwDcTrackerEx.resetDataCureInfo(PhoneInternalInterface.REASON_SIM_LOADED);
    ...
    private void onSimNotReady() {
        ...
        this.mHwDcTrackerEx.resetDataCureInfo(PhoneInternalInterface.REASON_SIM_NOT_READY);
        createAllApnList();
        setDataProfilesAsNeeded();
    }
    ...
    private DataConnection checkForCompatibleConnectedApnContext(ApnContext apnContext) {
        ...
        for (ApnContext curApnCtx : this.mApnContexts.values()) {
            ...
            if (curDc != null) {
                ...
                } else if (apnSetting != null && ((apnContext.getWaitingApns() == null && apnSetting.canHandleType(apnType)) || ((apnContext.getWaitingApns() != null && apnContext.getWaitingApns().contains(apnSetting)) || ((hwCustDcTracker = this.mHwCustDcTracker) != null && hwCustDcTracker.isDocomoTetheringApn(apnSetting, apnContext.getApnType()))))) {
    ...
    /* JADX DEBUG: Multi-variable search result rejected for r3v18, resolved type: android.net.ConnectivityManager */
    /* JADX WARN: Multi-variable type inference failed */
    ...
    /* JADX WARNING: Removed duplicated region for block: B:92:0x024e  */
    ...
    private void onDataSetupComplete(ApnContext apnContext, boolean success, int cause, int requestType) {
        ...
        if (apnContext != null) {
            ...
            if (success) {
                ...
                log(sb.toString());
                if (isDefault) {
                    SystemProperties.set("gsm.default.apn", apn == null ? PhoneConfigurationManager.SSSS : apn.getApnName());
                ...
                if (this.mHwDcTrackerEx.needSetCTProxy(apn)) {
                    ...
                    try {
                        ...
                        cm.setProvisioningNotificationVisible(r8, r8, this.mProvisionActionName);
                ...
                cm2.setProvisioningNotificationVisible(r8, r8, this.mProvisionActionName);
                completeConnection(apnContext, requestType);
                log("onDataSetupComplete: SETUP complete type=" + apnContext.getApnType());
                if (Build.IS_DEBUGGABLE) {
                }
                ...
                log("onDataSetupComplete: error apn=" + apn2.getApnName() + ", cause=" + cause + ", requestType=" + requestTypeToString(requestType));
            ...
            if (!this.mAttached.get() || !apnContext.isReady() || !retryAfterDisconnected(apnContext)) {
                ...
                if (isOnlySingleDcAllowed(getDataRat())) {
                    ...
                    if (AbstractPhoneInternalInterface.REASON_NO_RETRY_AFTER_DISCONNECT.equals(apnContext.getReason())) {
                        this.mHwDcTrackerEx.setupDataOnConnectableApns(PhoneInternalInterface.REASON_SINGLE_PDN_ARBITRATION, apnContext.getApnType());
        ...
        if (!TextUtils.isEmpty(operator)) {
            ...
            if (cursor != null) {
                while (cursor.moveToNext()) {
                    ...
                    apn2.setIsPreset(cursor.getInt(cursor.getColumnIndexOrThrow(IHwDcTrackerEx.DB_PRESET)) == 1);
            ...
            if (this.mAllApnSettings.isEmpty() && !VSimUtilsInner.isVSimSub(this.mPhone.getPhoneId()) && this.mHwDcTrackerEx.getPrimarySlot() == this.mPhone.getPhoneId() && r != null && true == r.getRecordsLoaded() && operator.length() != 0) {
                ...
                switch (i) {
                    ...
                    default:
                        ...
                        switch (i) {
                            case 270347:
                                if (msg.what == 270347) {
                                    this.mHwDcTrackerEx.resetDataCureInfo(PhoneInternalInterface.REASON_ROAMING_ON);
                                }
                                onDataRoamingOnOrSettingsChanged(msg.what);
                                break;
                            case 270348:
                                this.mHwDcTrackerEx.resetDataCureInfo(PhoneInternalInterface.REASON_ROAMING_OFF);
                                onDataRoamingOff();
                                break;
                            case 270349:
        ...
        public void doRecovery() {
            if (DcTracker.this.getOverallState() == DctConstants.State.CONNECTED) {
                ...
                if (recoveryAction == 0) {
                    ...
                    if (ret != 0) {
                        DcTracker.this.mHwDcTrackerEx.updateLastRadioResetTimestamp();
                        reset();
                    }
                } else {

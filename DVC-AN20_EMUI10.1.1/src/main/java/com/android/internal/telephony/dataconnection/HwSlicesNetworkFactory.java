...
public class HwSlicesNetworkFactory extends NetworkFactoryEx {
    ...
    private static HwSlicesNetworkFactory[] sInstances;
    ...
    private final SubscriptionController mSubscriptionController = SubscriptionController.getInstance();
    ...
    public HwSlicesNetworkFactory(DcTrackerEx dcTracker, Looper looper, Context context, int phoneId) {
        ...
        setCapabilityFilter(new NetworkCapabilitiesEx(makeNetworkFilter(this.mSubscriptionController.getSubIdUsingPhoneId(phoneId))));
        setScoreFilter(SLICES_NETWORK_SCORE);
        this.mInternalHandler = new InternalHandler(looper);
        this.mDcTracker = dcTracker;
        this.mContext = context;
        ((SubscriptionManager) context.getSystemService("telephony_subscription_service")).addOnSubscriptionsChangedListener(this.mOnSubscriptionsChangeListener);
        this.LOG_TAG = "HwSlicesNetworkFactory[" + phoneId + "]";
        register();
        if (sInstances == null) {
            sInstances = new HwSlicesNetworkFactory[TelephonyManagerEx.getDefault().getPhoneCount()];
        }
        if (phoneId >= 0 && phoneId < TelephonyManagerEx.getDefault().getPhoneCount()) {
            sInstances[phoneId] = this;
        }
    }
    ...
    private void onSubIdChange() {
        ...
        if (this.mSubscriptionId != newSubscriptionId) {
            ...
            setCapabilityFilter(new NetworkCapabilitiesEx(makeNetworkFilter(this.mSubscriptionId)));
        }
    }
    ...
    private void onNeedNetworkFor(Message msg) {
        ...
        if (this.mSubscriptionId == this.mSubscriptionController.getDefaultDataSubId()) {
            this.mDcTracker.requestNetwork(networkRequest, 1, (Message) null, (LocalLogEx) null);
        }
    }
    ...
    private void onReleaseNetworkFor(Message msg) {
        ...
        this.mDcTracker.releaseNetwork(networkRequest, 1, (LocalLogEx) null);
    }
}

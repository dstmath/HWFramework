...
public abstract class HwFullNetworkCheckStateHisiBase extends HwFullNetworkCheckStateBase {
    ...
    public void handleMessage(Message msg) {
        switch (msg.what) {
            case HwFullNetworkConstantsInner.EVENT_SET_DATA_ALLOW_DONE /*{ENCODED_INT: 302}*/:
                logd("Received EVENT_SET_DATA_ALLOW_DONE curSetDataAllowCount = " + this.mChipHisi.curSetDataAllowCount);
                this.mChipHisi.curSetDataAllowCount++;
                if (this.mChipHisi.needSetDataAllowCount == this.mChipHisi.curSetDataAllowCount) {
                    ...
                    this.mCheckStateHandler.obtainMessage(HwFullNetworkConstantsInner.EVENT_SET_MAIN_SLOT, this.defaultMainSlot, 0).sendToTarget();
                    return;
                }
                return;
            case HwFullNetworkConstantsInner.EVENT_CMCC_SET_NETWOR_DONE /*{ENCODED_INT: 1011}*/:
    ...
    public void disposeCardStatusWhenAllTrayOut() {
        boolean isSingleCardTrayOut;
        boolean isBothCardTrayOut;
        boolean bDSDA;
        if (!HwFullNetworkConfigInner.IS_SINGLE_CARD_TRAY || !HwCardTrayInfo.getInstance().isCardTrayOut(0)) {
            isSingleCardTrayOut = false;
        } else {
            isSingleCardTrayOut = true;
        }
        if (HwFullNetworkConfigInner.IS_SINGLE_CARD_TRAY || !HwCardTrayInfo.getInstance().isCardTrayOut(0) || !HwCardTrayInfo.getInstance().isCardTrayOut(1)) {
            isBothCardTrayOut = false;
        } else {
            isBothCardTrayOut = true;
        }
    ...
    public boolean switchDualCardsSlotIfNeeded() {
        boolean isCompleted;
        boolean isVSimOn;
        if (!SystemPropertiesEx.getBoolean("persist.sys.dualcards", false) || this.mChipHisi.mAutoSwitchDualCardsSlotDone) {
            isCompleted = true;
        } else {
            isCompleted = false;
        }
        if (!isCompleted || this.mChipHisi.needFixMainSlotPosition) {
            ...
            if (isCard1Present || isCard2Present) {
                ...
                if (!HwTelephonyManager.getDefault().isPlatformSupportVsim() || (!HwVSimUtils.isVSimEnabled() && !HwVSimUtils.isVSimCauseCardReload() && !HwVSimUtils.isSubActivationUpdate())) {
                    isVSimOn = false;
                } else {
                    isVSimOn = true;
                }

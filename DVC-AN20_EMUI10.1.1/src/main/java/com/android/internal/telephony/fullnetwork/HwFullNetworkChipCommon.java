...
public class HwFullNetworkChipCommon {
    ...
    public boolean isSwitchSlotEnabledForCMCC() {
        int otherSlotId;
        boolean isCmccInService;
        boolean isOtherInService;
        ...
        int otherSubId = SubscriptionManagerEx.getSubIdUsingSlotId(otherSlotId);
        ...
        if (cmccState.getState() == 0) {
            isCmccInService = true;
        } else {
            isCmccInService = false;
        }
        if (otherState.getState() == 0) {
            isOtherInService = true;
        } else {
            isOtherInService = false;
        }
    ...
    public void initUiccCard(UiccCardExt uiccCard, IccCardStatusExt status, Integer index) {
        if (sHotPlugController != null) {
            sHotPlugController.initHotPlugCardState(status, index);
        }
    }

    public void updateUiccCard(UiccCardExt uiccCard, IccCardStatusExt status, Integer index) {
        if (sHotPlugController != null) {
            sHotPlugController.updateHotPlugCardState(status, index);
        }
    }
    ...
    public boolean isCardPresent(int slotId) {
        if (this.mUiccController == null || this.mUiccController.getUiccCard(slotId) == null || this.mUiccController.getUiccCard(slotId).getCardState() == IccCardStatusExt.CardStateEx.CARDSTATE_ABSENT) {
            return false;
        }
        return true;
    }
    ...
    public boolean isCMCCCardBySlotId(int slotId) {
        if (!isValidIndex(slotId)) {
            ...
            return false;
        } else if (this.subCarrierTypeArray[slotId] != null && this.subCarrierTypeArray[slotId] != HwFullNetworkConstantsInner.SubCarrierType.ABSENT) {
    ...
    public boolean isCmccHybirdAndNoRoaming() {
        ...
        if (!HwFullNetworkConfigInner.isCMCCDsdxDisable() || !isCmccHybirdBySubCarrierType()) {
            isCmccHybird = false;
        } else {
            isCmccHybird = true;
        }
        return isCmccHybird && !TelephonyManagerEx.isNetworkRoaming(SubscriptionManagerEx.getSubIdUsingSlotId(getCMCCCardSlotId()));
    }
    ...
    private boolean setUnbindFlagOfOeminfo(String unbindFlag) {
        ...
        if (!TextUtils.isEmpty(unbindFlag)) {
            isSuccess = HwProtectAreaManager.getInstance().writeProtectArea(KEY_UNBIND_FILG, unbindFlag.length(), unbindFlag, new int[1]) == 0;
    ...
    public boolean isCMCCUnbindBySNDate() {
        ...
        try {
            ...
            StringBuffer snDateStringBuffer = new StringBuffer().append(configDateString.substring(0, 2)).append(snDateString.substring(0, 2));

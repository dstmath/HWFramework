...
public class HwFullNetworkChipHisi implements HwFullNetworkChipCommon.HwFullNetworkChipInterface {
    ...
    private boolean isSwitchDualEnabledNotChinaTelecom() {
        boolean isSub0Inserted;
        boolean isSub1Inserted;
        if (this.mSwitchTypes[0] > 0 || mChipCommon.isSimInsertedArray[0]) {
            isSub0Inserted = true;
        } else {
            isSub0Inserted = false;
        }
        if (this.mSwitchTypes[1] > 0 || mChipCommon.isSimInsertedArray[1]) {
            isSub1Inserted = true;
        } else {
        ...
        return isSub0Inserted && isSub1Inserted;
    }
    ...
    private int getSlaveSlotNetworkMode(int i) {
        ...
        if (mChipCommon.isCMCCHybird() && !mChipCommon.isCMCCCardBySlotId(i) && !TelephonyManagerEx.isNetworkRoaming(cmccSubId)) {
            return mChipCommon.isCDMASimCard(i) ? 4 : 3;
        }
        int networkMode = mChipCommon.isCDMASimCard(i) ? 4 : 3;
    ...
    public boolean isEuiccInSlot2() {
        ...
        if (uiccCards == null || uiccCards[1] == null || !uiccCards[1].isEuiccCard()) {
            z = false;
        }
        return z;
    }
    ...
    public boolean isSimPresentBySubState(int slotId) {
        boolean z;
        boolean isSimPresent = true;
        boolean isSimPresent2 = isSimPresentByCardState(slotId);
        if (!mChipCommon.isSupportEuicc()) {
            return isSimPresent2;
        }
        StringBuilder append = new StringBuilder().append("isSimPresentBySubState, slotId = ").append(slotId).append(", isSimPresent = ").append(isSimPresent2).append(", subState = ");
        if (HwTelephonyManager.getDefault().getSubState((long) slotId) == 1) {
            z = true;
        } else {
            z = false;
        }
        logd(append.append(z).toString());
        if (!isSimPresent2 || HwTelephonyManager.getDefault().getSubState((long) slotId) != 1) {
            isSimPresent = false;
        }
        return isSimPresent;
    }

...
public class HwFullNetworkDefaultStateQcomMtkBase extends HwFullNetworkDefaultStateBase {
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            case HwFullNetworkConstantsInner.EVENT_SET_LTE_SERVICE_ABILITY:
                setNetworkType(ar, msg.arg1);
                return;
            case HwFullNetworkConstantsInner.EVENT_SET_PRIMARY_STACK_LTE_SWITCH_DONE:
                logd("Received EVENT_SET_PRIMARY_STACK_LTE_SWITCH_DONE");
                handleSetPrimaryStackLteSwitchDone(ar, msg.arg1);
                return;
            case HwFullNetworkConstantsInner.EVENT_SET_SECONDARY_STACK_LTE_SWITCH_DONE:
                logd("Received EVENT_SET_SECONDARY_STACK_LTE_SWITCH_DONE");
                handleSetSecondaryStackLteSwitchDone(ar, msg.arg1);
                return;
            case HwFullNetworkConstantsInner.EVENT_SET_PRIMARY_STACK_ROLL_BACK_DONE:
                logd("Received EVENT_SET_PRIMARY_STACK_ROLL_BACK_DONE");
                handleRollbackDone(ar);
                return;
            case HwFullNetworkConstantsInner.EVENT_RESET_OOS_FLAG:
    ...
    private int getPrimaryAndSecondaryStackAbility(int subId) {
        return HwTelephonyManager.getDefault().isNrSupported() ? subId == this.mChipCommon.getUserSwitchDualCardSlots() ? 1 : 0 : (this.mChipCommon.isDualImsSwitchOpened() || subId == this.mChipCommon.getUserSwitchDualCardSlots()) ? 1 : 0;
    }
    ...
    public void setMainSlot(int slotId, Message responseMsg) {
        if (!SystemPropertiesEx.getBoolean("persist.sys.dualcards", false)) {
            ...
            if (slotId == this.mChipCommon.getUserSwitchDualCardSlots()) {
                ...
                this.mStateHandler.obtainMessage(HwFullNetworkConstantsInner.EVENT_SET_MAIN_SLOT, slotId, 0, responseMsg).sendToTarget();
            }
        }
    }
    ...
    public void setServiceAbilityForQCOM(int sotId, int type, int ability, int serviceOnMappingMode) {
        boolean isNeedSetSecondStackNr;
        getStackPhoneId();
        if (!HwTelephonyManager.getDefault().isNrSupported() || !this.mChipOther.mNeedSetSecondStack) {
            isNeedSetSecondStackNr = false;
        } else {
            isNeedSetSecondStackNr = true;
        }
    ...
    private void recordPrimaryAndSecondaryStackNetworkType(int type, int ability, int serviceOnMappingMode) {
        ...
        if (this.mChipCommon.isCmccHybirdAndNoRoaming()) {
            ...
            int otherSub = this.mChipCommon.default4GSlot == 0 ? 1 : 0;
    ...
    private void handleSetPrimaryStackLteSwitchDone(AsyncResultEx ar, int networkType) {
        ...
        if (!this.mChipCommon.isDualImsSwitchOpened() || (this.mChipOther.mNeedSetSecondStack && (HwFullNetworkConfigInner.isCMCCDsdxDisable() || HwTelephonyManager.getDefault().isNrSupported()))) {
            isNeedSetSecondStack = true;
        } else {
            isNeedSetSecondStack = false;
        }
    ...
    private void handleSetSecondaryStackLteSwitchDone(AsyncResultEx ar, int networkType) {
        ...
        if (!this.mChipCommon.isDualImsSwitchOpened() || (this.mChipOther.mNeedSetSecondStack && (this.mChipCommon.isCmccHybirdAndNoRoaming() || HwTelephonyManager.getDefault().isNrSupported()))) {
            isChipDualAndNeedSave = true;
        } else {
            isChipDualAndNeedSave = false;
        }

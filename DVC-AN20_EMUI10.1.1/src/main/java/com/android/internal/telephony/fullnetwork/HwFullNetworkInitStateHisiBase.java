...
public abstract class HwFullNetworkInitStateHisiBase extends HwFullNetworkInitStateBase {
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case HwFullNetworkConstantsInner.EVENT_ICC_GET_ATR_DONE:
            ...
            default:
                ...
                return;
            case HwFullNetworkConstantsInner.EVENT_QUERY_CARD_TYPE_DONE:
                ...
                return;
            case HwFullNetworkConstantsInner.EVENT_GET_BALONG_SIM_DONE:
                logd("Received EVENT_GET_BALONG_SIM_DONE on index " + index);
                onGetBalongSimDone(ar, index);
                return;
            case HwFullNetworkConstantsInner.EVENT_SIM_HOTPLUG:
            ...
            case HwFullNetworkConstantsInner.EVENT_GET_ICCID_DONE:
                ...
                return;
            case HwFullNetworkConstantsInner.EVENT_GET_ICC_STATUS_DONE:
    ...
    public synchronized void onQueryCardTypeDone(AsyncResultEx ar, Integer index) {
        ...
        if (ar != null) {
            if (ar.getResult() != null) {
                ...
                for (i = 0; i < HwFullNetworkConstantsInner.SIM_NUM; i++) {
                ...
                if (countGetCardTypeDone == HwFullNetworkConstantsInner.SIM_NUM) {
                    ...
                    this.mStateHandler.obtainMessage(HwFullNetworkConstantsInner.EVENT_CHECK_MAIN_SLOT, index).sendToTarget();
                }
            }
        }
    ...
    public void onGetBalongSimDone(AsyncResultEx ar, Integer index) {
        if (ar != null && ar.getResult() != null && ((int[]) ar.getResult()).length == 3) {
            ...
            if (((int[]) ar.getResult())[1] + ((int[]) ar.getResult())[0] > 1) {
        ...
        if (HwFullNetworkConfigInner.isCMCCDsdxEnable() || HwTelephonyManagerInner.getDefault().getDefaultMainSlotCarrier() == 1) {
            ...
            if (mChipCommon.isValidIndex(slotId) && !TextUtils.isEmpty(simState)) {
                ...
                switch (c) {
                    ...
                    case 6:
                    case HwFullNetworkConstantsInner.ICCID_LEN_MINIMUM:
                        onLocked(slotId);
                        return;
                    case '\b':
                    case '\t':
    ...
    private void onImsiReady(int slotId) {
        ...
        if (!mChipCommon.isSet4GSlotInProgress) {
            ...
            this.mStateHandler.obtainMessage(HwFullNetworkConstantsInner.EVENT_CHECK_MAIN_SLOT, 0).sendToTarget();
        }
    }

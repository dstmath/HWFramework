...
public class HwFullNetworkSetStateHisi2_0 extends HwFullNetworkSetStateBase {
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            case HwFullNetworkConstantsInner.EVENT_SET_MAIN_SLOT_DONE:
                ...
                return;
            case HwFullNetworkConstantsInner.EVENT_SET_MAIN_SLOT_TIMEOUT:
                logd("Received EVENT_SET_MAIN_SLOT_TIMEOUT on index " + index);
                setMainSlotTimeOut(msg, index.intValue());
                return;
            case HwFullNetworkConstantsInner.EVENT_GET_CDMA_MODE_SIDE_DONE:
                ...
                return;
            case HwFullNetworkConstantsInner.EVENT_CMCC_SET_NETWOR_DONE:
                logd("EVENT_CMCC_SET_NETWOR_DONE reveived for slot: " + msg.arg1);
                this.mChipHisi.handleSetCmccPrefNetwork(msg);
                return;
            case HwFullNetworkConstantsInner.EVENT_RESTART_RILD_FOR_NV:
                logd("Received EVENT_RESTART_RILD_FOR_NV on index " + index);
                restartRildForNvcfg();
                return;
            case HwFullNetworkConstantsInner.EVENT_SWITCH_SLOT_TYPE_DONE:
                logd("EVENT_SWITCH_SLOT_TYPE_DONE");
                return;
            case HwFullNetworkConstantsInner.EVENT_UNSOL_RESTART_RILD_STATUS:
    ...
    public void setMainSlot(int slotId, Message responseMsg) {
        ...
        if (this.mCis[slotId] == null || 2 != this.mCis[slotId].getRadioState()) {
    ...
    private void fastSwitchDualCardsSlot(int expectedMainSlotId) {
        ...
        if (!isNeedSetRadioCapability(expectedMainSlotId, cdmaSimSlotId)) {
            ...
            this.mChipCommon.sendResponseToTarget(obtainMessage(HwFullNetworkConstantsInner.EVENT_SET_MAIN_SLOT_DONE, Integer.valueOf(expectedMainSlotId)), 0);
            return;
        }
        for (int i = 0; i < HwFullNetworkConstantsInner.SIM_NUM; i++) {
            if (HwFullNetworkConfigInner.isCMCCDsdxDisable() && HwFullNetworkConfigInner.IS_VICE_WCDMA && HwFullNetworkManager.getInstance().isCMCCHybird() && (phoneExt = PhoneFactoryExt.getPhone(i)) != null) {
        ...
        if (!proxyController.setRadioCapability(expectedMainSlotId, cdmaSimSlotId)) {
            ...
            this.mChipCommon.sendResponseToTarget(obtainMessage(HwFullNetworkConstantsInner.EVENT_SET_MAIN_SLOT_DONE, Integer.valueOf(expectedMainSlotId)), 2);
        }
    ...
    private boolean isHandleVsim() {
        ...
        if (HwVSimUtils.isVSimCauseCardReload()) {
            return true;
        }
        if (HwVSimUtils.isSubActivationUpdate()) {
            return true;
        }
        return !HwVSimUtils.isAllowALSwitch();
    }
    ...
    private boolean isNeedSetRadioCapability(int expectedMainSlotId, int cdmaSimSlotId) {
        ...
        if (SubscriptionManagerEx.isValidSlotIndex(expectedMainSlotId) && mPhones[expectedMainSlotId] != null) {
            ...
            if (expectedMainSlotRC == null || ProxyControllerEx.MODEM_0.equals(expectedMainSlotRC.getLogicalModemUuid())) {
    ...
    public void setRadioCapabilityDone(Intent intent) {
        ...
        if (intent != null) {
            this.mChipCommon.sendResponseToTarget(obtainMessage(HwFullNetworkConstantsInner.EVENT_SET_MAIN_SLOT_DONE, Integer.valueOf(intent.getIntExtra("intContent", 0))), 0);
        }
    }
    ...
    public void setMainSlotDone(Message response, int index) {
        if (hasMessages(HwFullNetworkConstantsInner.EVENT_SET_MAIN_SLOT_TIMEOUT)) {
            removeMessages(HwFullNetworkConstantsInner.EVENT_SET_MAIN_SLOT_TIMEOUT);
        }

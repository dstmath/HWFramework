...
public class HwCustGsmServiceStateTrackerImpl extends HwCustGsmServiceStateTracker {
    ...
    private BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent != null && HwCustGsmServiceStateTrackerImpl.this.ACTION_ENFORCE_LTE_NETWORKTYPE.equals(intent.getAction())) {
    ...
    DialogInterface.OnCancelListener mShowRejMsgOnCancelListener = new DialogInterface.OnCancelListener() {
    ...
    DialogInterface.OnKeyListener mShowRejMsgOnKeyListener = new DialogInterface.OnKeyListener() {
        ...
        public void handleMessage(Message msg) {
            if (msg.what == 1) {
                if (HwCustGsmServiceStateTrackerImpl.this.dialogCanceled > 0) {
                    HwCustGsmServiceStateTrackerImpl.access$710(HwCustGsmServiceStateTrackerImpl.this);
    ...
    static /* synthetic */ int access$710(HwCustGsmServiceStateTrackerImpl x0) {
    ...
    public void processEnforceLTENetworkTypePending() {
        ...
        } else if (true == this.mEnforceLTEPending) {
    ...
    private int deleteLTENetworkType() {
        ...
        switch (this.mSetPreferredNetworkType) {
            ...
            case 10:
                ...
                break;
            case PLMN_NOT_ALLOWED /*{ENCODED_INT: 11}*/:
                pseudoNetworkType = 2;
                break;
            case LA_NOT_ALLOWED /*{ENCODED_INT: 12}*/:
                pseudoNetworkType = 2;
                break;
        }
    ...
    private void setMobileDataEnable(int state) {
        if (state == UNKNOWN_STATE) {
            try {
                ...
                this.mGsmPhone.mCi.setMobileDataEnable(state2, (Message) null);
            } catch (Exception e) {
    ...
    private void setRoamingDataEnable(int state) {
        if (state == UNKNOWN_STATE) {
            try {
                ...
                this.mGsmPhone.mCi.setRoamingDataEnable(state2, (Message) null);
            } catch (Exception e) {
    ...
    public String setEmergencyToNoService(ServiceState mSS, String plmn, boolean mEmergencyOnly) {
        if (IS_EMERGENCY_SHOWS_NOSERVICE && mSS.getRadioTechnology() == GPRS_SERVICES_NOT_ALLOWED_IN_THIS_PLMN && mEmergencyOnly) {
            return Resources.getSystem().getText(17040418).toString();
        }
    ...
    public void setPsCell(ServiceState mSS, GsmCellLocation mNewCellLoc, String[] states) {
        if (mSS != null && mNewCellLoc != null && states != null && IS_DATAONLY_LOCATION_ENABLED) {
            ...
            boolean mCsOutOfservice = DBG;
            if (voiceRegState != 1) {
                mCsOutOfservice = false;
            }
    ...
    private void handleNetworkRejection(int rejCode) {
        ...
        String plmn = r.getText(17040029).toString();
        if (rejCode == 2) {
    ...
    public boolean isStopUpdateName(boolean SimCardLoaded) {
        ...
        if (!SimCardLoaded && IS_DELAY_UPDATENAME) {
            return DBG;
        }
                    ...
                    if (rejcode == ILLEGAL_MS) {
                        ...
                        if (rejcode != 8) {
                            if (rejcode != ESM_FAILURE && rejcode != CONGESTTION) {
                                switch (rejcode) {
                                    case NO_SUITABLE_CELLS_IN_LA /*{ENCODED_INT: 15}*/:
                                        break;
                                    case MSC_TEMPORARILY_NOT_REACHABLE /*{ENCODED_INT: 16}*/:
                                    case NETWORK_FAILURE /*{ENCODED_INT: 17}*/:
                                        if (rejrat == 2 && IS_KT) {
                                            msg = r.getString(33685995);
                                            break;
                                        } else {
    ...
    private boolean isLteEmmCauseRecorded(int phoneId) {
        boolean[] zArr = this.lteEmmCauseRecorded;

...
public class HwCustHwGSMPhoneReferenceImpl extends HwCustHwGSMPhoneReference {
    ...
    private void getCustResults(ArrayList<OperatorInfo> searchResult, ArrayList<OperatorInfo> custResults) {
        int list_size = searchResult.size();
        for (int j = 0; j < list_size; j++) {
            OperatorInfo operatorInfo = searchResult.get(j);
            Rlog.d(LOG_TAG, "filterActAndRepeatedItems: operatorInfo = " + operatorInfo);
            OperatorInfo rsltInfo = filterRepeatedItem(custResults, operatorInfo, judgeWhetherContainXg(operatorInfo));
    ...
    private OperatorInfo filterRepeatedItem(ArrayList<OperatorInfo> custResults, OperatorInfo operatorInfo, String longNameWithoutAct) {
        ArrayList<OperatorInfo> arrayList = custResults;
        ...
        while (true) {
            ...
            OperatorInfo info = arrayList.get(i);
        ...
        if (!isFound) {
            return new OperatorInfo(longNameWithoutAct, operatorInfo.getOperatorAlphaShort(), operatorInfo.getOperatorNumeric(), operatorInfo.getState());
        }
        return rsltInfo;
    }

    private String judgeWhetherContainXg(OperatorInfo operatorInfo) {

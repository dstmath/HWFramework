...
public class HwGsmOnsDisplayParamsManager extends HwBaseOnsDisplayParamsManager {
    ...
    private static final String EMERGENCY_PLMN = Resources.getSystem().getText(17040029).toString();
    ...
    private static final String NO_SERVICE_PLMN = Resources.getSystem().getText(17040418).toString();
    ...
    public HwGsmOnsDisplayParamsManager(IServiceStateTrackerInner serviceStateTracker, PhoneExt phoneExt, HwServiceStateTrackerEx hwServiceStateTrackerEx) {
        ...
        this.mHwCustGsmServiceStateManager = (HwCustGsmServiceStateManager) HwCustUtils.createObj(HwCustGsmServiceStateManager.class, new Object[]{serviceStateTracker, this.mPhone.getPhone()});
    }
    ...
    private OnsDisplayParams getOnsDisplayParamsBySpnOnly(boolean isShowSpn, boolean isShowPlmn, int rule, String plmn, String spn) {
        ...
        if (!TextUtils.isEmpty(spnSim) && isMccForSpn(iccRecords.getOperatorNumeric())) {
            if (!TextUtils.isEmpty(homePlmn)) {
    ...
    private OnsDisplayParams getGsmOnsDisplayParamsForChinaOperator(boolean isShowSpn, boolean isShowPlmn, int rule, String plmn, String spn) {
        ...
        if (!isChinaMobileMccMnc()) {
            ...
            return new OnsDisplayParams(false, true, rule, plmn, spnTemp);
        } else if (spnTemp == null || ONE_SPACE_STRING.equals(spnTemp) || "CMCC".equals(spnTemp) || "China Mobile".equals(spnTemp)) {
    ...
    private void setOperatorNameByPlmnOrSpn(OnsDisplayParams odp) {
        ...
        if (!TextUtils.isEmpty(networkNameShow)) {
            if ((getSs() != null && (getSs().getDataRegState() == 0 || getSs().getVoiceRegState() == 0)) && (hwCustGsmServiceStateManager = this.mHwCustGsmServiceStateManager) != null && !hwCustGsmServiceStateManager.skipPlmnUpdateFromCust()) {
    ...
    /* JADX WARNING: Removed duplicated region for block: B:39:0x0150  */
        ...
        if (!isUseVirtualName) {
            if (!isMatchRoamingRule) {
                ...
                if (!TextUtils.isEmpty(plmnValue2)) {
                    ...
                    getSs().setOperatorAlphaLong(plmnValue2);
                }
    ...
    public boolean getRoamingStateHw(boolean roaming) {
        ...
        boolean isRegGsmOrWcdma = ServiceState.isGsm(getNewSS().getRilVoiceRadioTechnology()) && !ServiceState.isLte(getNewSS().getRilVoiceRadioTechnology()) && getNewSS().getRilVoiceRadioTechnology() != 20;
        if (HwTelephonyManagerInner.getDefault().isCTSimCard(this.mPhone.getPhoneId()) && getNewSS().getState() == 0 && isRegGsmOrWcdma) {
            isCTCardRegGSM = true;
        }
        if (isCTCardRegGSM) {
            roamingTemp = true;
            logd("When CT card register in GSM/UMTS, it always should be roamingTemp" + true);
        }
        HwCustGsmServiceStateManager hwCustGsmServiceStateManager2 = this.mHwCustGsmServiceStateManager;
        if (hwCustGsmServiceStateManager2 != null) {
    ...
    private boolean getGsmRoamingCustByIMSIStart(boolean roaming) {
        ...
        if (this.mPhone.getIccRecords() != null) {
            ...
            String regplmn = getNewSS().getOperatorNumeric();
            int netType2 = getNewSS().getVoiceNetworkType();
            TelephonyManager.getDefault();
            int netClass = TelephonyManager.getNetworkClass(netType2);

...
public class HwUsimPhoneBookManagerEmailAnr extends UsimPhoneBookManager {
    ...
    private void readEmailFileAndWait(int recNum) {
        ...
        if (fileIds != null && fileIds.containsKey(Integer.valueOf((int) USIM_EFEMAIL_TAG))) {
            if (this.mEmailPresentInIap) {
                ...
                this.mFh.loadEFLinearFixedAllExcludeEmpty(fileIds.get(Integer.valueOf((int) USIM_EFEMAIL_TAG)).intValue(), obtainMessage(4, Integer.valueOf(recNum)));
                ...
                while (it.hasNext()) {
                    ...
                    this.mFh.loadEFLinearFixedAllExcludeEmpty(efid, obtainMessage(4, Integer.valueOf(recNum)));
    ...
    private void readAnrFileAndWait(int recNum) {
        ...
        if (!(fileIds == null || fileIds.isEmpty()) && fileIds.containsKey(Integer.valueOf((int) USIM_EFANR_TAG))) {
            if (this.mAnrPresentInIap) {
                ...
                this.mFh.loadEFLinearFixedAllExcludeEmpty(fileIds.get(Integer.valueOf((int) USIM_EFANR_TAG)).intValue(), obtainMessage(5, Integer.valueOf(recNum)));
                ...
                while (it.hasNext()) {
                    int efid = it.next().intValue();
                    this.mFh.loadEFLinearFixedAllExcludeEmpty(efid, obtainMessage(5, Integer.valueOf(recNum)));
    ...
    private void readIapFileAndWait(int efid, int recNum) {
        ...
        this.mFh.loadEFLinearFixedAllExcludeEmpty(efid, obtainMessage(3, Integer.valueOf(recNum)));
        try {
            this.mLock.wait();
        } catch (InterruptedException e) {
    ...
    private void updatePhoneAdnRecordWithEmail(int pbrIndex) {
        if (!hasRecordIn(this.mEmailFileRecord, pbrIndex)) {
            ...
            for (int index = 0; index < emailRecsSize; index++) {
                if (1 != this.mEmailFlags.get(Integer.valueOf(pbrIndex)).get(index).intValue() && !"".equals(readEmailRecord(index, pbrIndex, 0))) {
        ...
        if (!hasRecordIn(this.mAnrFileRecord, pbrIndex)) {
            ...
            if (this.mAnrPresentInIap && hasRecordIn(this.mIapFileRecord, pbrIndex)) {
                ...
                for (int index = 0; index < anrRecsSize; index++) {
                    if (1 != this.mAnrFlags.get(Integer.valueOf(pbrIndex)).get(index).intValue() && !"".equals(readAnrRecord(index, pbrIndex, 0))) {
    ...
    private int getEmailRecNumber(int adnRecIndex, int numRecs, String oldEmail) {
        ...
        if (!hasRecordIn(this.mEmailFileRecord, pbrIndex)) {
            ...
            if (record == null || (i = this.mEmailTagNumberInIap) >= record.length || record[i] == -1 || (record[i] & 255) <= 0 || (record[i] & 255) > this.mEmailFileRecord.get(Integer.valueOf(pbrIndex)).size()) {
            ...
            int recordNumber = record[this.mEmailTagNumberInIap] & 255;
    ...
    private int getAnrRecNumber(int adnRecIndex, int numRecs, String oldAnr) {
        ...
        if (record == null || (i = this.mAnrTagNumberInIap) >= record.length || record[i] == -1 || (record[i] & 255) <= 0 || (record[i] & 255) > this.mAnrFileRecord.get(Integer.valueOf(pbrIndex)).size()) {
        ...
        int recordNumber = record[this.mAnrTagNumberInIap] & 255;
    ...
    private void handleEfEmailRecordSizeDone(Message msg) {
        ...
        if (recordSize.length == 3 && recordNumber <= recordSize[2]) {
            if (recordNumber > 0) {
                ...
                this.mFh.updateEFLinearFixed(efid, recordNumber, data, (String) null, obtainMessage(8, actualRecNumber, adnRecIndex, data));
                return;
            }
        }
        this.mSuccess = false;
    ...
    private void handleEfAnrRecordSizeDone(Message msg) {
        ...
        if (recordSize.length == 3 && recordNumber <= recordSize[2]) {
            if (recordNumber > 0) {
                ...
                this.mFh.updateEFLinearFixed(efid, recordNumber, data, (String) null, obtainMessage(9, actualRecNumber, adnRecIndex, data));
                return;
            }
        }
        this.mSuccess = false;
    ...
    private void handleEfIapRecordSizeDone(Message msg) {
        ...
        if (isIapRecordParamInvalid(recordSize, recordIndex, recordNumber)) {
            ...
            this.mFh.updateEFLinearFixed(efid, recordIndex + 1, recordData, (String) null, obtainMessage(11, adnRecIndex, recordNumber, recordData));
        }
    }
    ...
    private void handleEfExt1RecordSizeDone(Message msg) {
        ...
        if (mNumber.length() > ADN_RECORD_LENGTH_DEFAULT) {
            ...
            newExt1 = "";
            newAdnRecord.setExtRecord((int) HwSubscriptionManager.SUB_INIT_STATE);
        }
        byte[] data = buildExt1Data(recordSize[0], adnRecIndex, newExt1);
        this.mFh.updateEFLinearFixed(efid, recordNumber, data, (String) null, obtainMessage(14, recordNumber, adnRecIndex, data));
    }
    ...
    private void readExt1FileAndWait(int recNum) {
        ...
        if (fileIds == null || fileIds.isEmpty()) {
            ...
            this.mFh.loadEFLinearFixedAllExcludeEmpty(fileIds.get(Integer.valueOf((int) USIM_EFEXT1_TAG)).intValue(), obtainMessage(12, recNum, fileIds.get(Integer.valueOf((int) USIM_EFEXT1_TAG)).intValue()));
    ...
    private String readExt1Record(int pbrIndex, int recNum, int offset) {
        ...
        try {
            ...
            if (extRec != null && extRec.length == 13 && (extRec[0] & 255) != 0 && (numberLength = extRec[1] & 255) <= 10) {

...
public class HwCustImsPhoneCallTrackerImpl extends HwCustImsPhoneCallTracker {
    ...
    public void addSipErrorPopup(ImsReasonInfo reasonInfo, Context context) {
        ...
        if (getToastReasonInfo(reasonInfo) && context != null) {
            Toast.makeText(context, context.getResources().getString(17040403), 1).show();
        }
    }
    ...
    private void showCallDropNotification(String notifiTitle, String notifiText, int iconId) {
        ...
        if (context != null && notifiTitle != null && notifiText != null) {
            ((NotificationManager) this.mContext.getSystemService("notification")).notify(1, new Notification.Builder(context).setSmallIcon(iconId).setContentTitle(notifiTitle).setContentText(notifiText).setShowWhen(DBG).setStyle(new Notification.BigTextStyle().bigText(notifiText)).build());
        }
    }

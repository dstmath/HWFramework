...
public class HwVSimController extends StateMachineEx {
    ...
    static /* synthetic */ int access$3408(HwVSimController x0) {
    ...
    public final class ServiceStateHandler extends Handler {
        ...
        private void notifyServiceStateChanged(ServiceState newSS, ServiceState oldSS, PhoneExt phone) {
            boolean registeredStatuChanged;
            boolean plmnStatuChanged;
            ...
            if (ServiceStateEx.getVoiceRegState(newSS) != ServiceStateEx.getVoiceRegState(oldSS)) {
                registeredStatuChanged = true;
            } else {
                registeredStatuChanged = false;
            }
            if (!equalsHandlesNulls(ServiceStateEx.getVoiceOperatorNumeric(newSS), ServiceStateEx.getVoiceOperatorNumeric(oldSS))) {
                plmnStatuChanged = true;
            } else {
                plmnStatuChanged = false;
            }
    ...
    public class HwVSimEventHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case EVENT_HOTPLUG_SWITCHMODE /*{ENCODED_INT: 1000}*/:
                    ...
                    return;
                case EVENT_VSIM_DISABLE_RETRY /*{ENCODED_INT: 1001}*/:
                    ...
                    HwVSimController.access$3408(HwVSimController.this);
    ...
    public void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        if (fd != null && pw != null && args != null) {
            ...
            StringBuilder append = new StringBuilder().append(" mSimSlotsTable=");
            if (this.mSimSlotsTable == null) {
                arrays = "[]";
            } else {
                arrays = Arrays.toString(this.mSimSlotsTable);
            }
            pw.println(append.append(arrays).toString());
        }
    }
    ...
    public int enableVSim(int operation, Bundle bundle) {
        ...
        switch (operation) {
            ...
            case 7:
                ...
                result = enableVSim(imsi, cardType, apnType, acqorder, tapath, vsimloc, challenge, operation, cardInModem1, supportVsimCa);
                break;
            case 2:
                result = setApn(imsi, cardType, apnType, tapath, challenge, false, supportVsimCa);
                break;
            case 3:
            case 4:
            case 5:
    ...
    public boolean dsFlowCfg(int repFlag, int threshold, int totalThreshold, int oper) {
        ...
        String callingAppName = getCallingAppName();
        if (callingAppName.startsWith(VSIM_PKG_NAME) || callingAppName.equals(VSIM_PKG_NAME)) {
    ...
    public void setLastRilFailCause(int cause) {
        ...
        if (DcFailCauseEx.fromInt(cause).isPermanentFailure(this.mContext, this.mVSimPhone.getSubId())) {
    ...
    public int dialupForVSim() {
        ...
        dcTracker.sendMessage(dcTracker.obtainMessage(270339, PhoneExt.REASON_USER_DATA_ENABLED));
        return 0;
    }
    ...
    private int setApn(String imsi, int cardType, int apnType, String tapath, String challenge, boolean isForHash, int supportVsimCa) {
        ...
        this.mVSimPhone.setInternalDataEnabled(true);
    ...
    private boolean needSwitchModeHotplug(int[] cardTypes) {
        int slaveSlot;
        boolean singleCardOnM2AndNoSimOnM1;
        int noReservedSub;
        ...
        if (!HwVSimUtilsInner.isFullNetworkSupported()) {
            ...
            if (cardTypes[mainSlot] == 0 || cardTypes[slaveSlot] != 0) {
                singleCardOnM2AndNoSimOnM1 = false;
            } else {
                singleCardOnM2AndNoSimOnM1 = true;
            }
    ...
    private boolean needSwitchModeHotplugForTelecom(int[] cardTypes) {
        int slaveSlot;
        boolean singleGsmOnM1OrCdmaOnM2;
        int noReservedSub;
        int mainSlot = getMainSlot(this.mSimSlotsTable);
        if (mainSlot == 0) {
            slaveSlot = 1;
        ...
        if ((cardTypes[slaveSlot] != 1 || cardTypes[mainSlot] == 1) && (cardTypes[slaveSlot] == 2 || cardTypes[slaveSlot] == 3 || !(cardTypes[mainSlot] == 2 || cardTypes[mainSlot] == 3))) {
            singleGsmOnM1OrCdmaOnM2 = false;
        } else {
            singleGsmOnM1OrCdmaOnM2 = true;
        }
        if (HwVSimUtilsInner.isPlatformTwoModemsActual()) {
            int reservedSub = getUserReservedSubId();
    ...
    private int getSwitchActivatedSubId(int oldActivatedSubId) {
        ...
        if (HwFullNetworkConfig.IS_HISI_DSDS_AUTO_SWITCH_4G_SLOT) {
            ...
            for (int i = 0; i < this.mCis.length; i++) {
                if (subController.getSubState(i) == 1) {
            ...
            if (activatedSubCount == 1 && !isSubActivated[oldActivatedSubId]) {
                activatedSubId = oldActivatedSubId == 0 ? 1 : 0;
            }
        }

...
public class HwVSimDualModem extends HwVSimModemAdapter {
    ...
    public static HwVSimDualModem create(HwVSimController vsimController, Context context, CommandsInterfaceEx vsimCi, CommandsInterfaceEx[] cis) {
    ...
    private HwVSimDualModem(HwVSimController vsimController, Context context, CommandsInterfaceEx vsimCi, CommandsInterfaceEx[] cis) {
    ...
    public void checkDisableSimCondition(HwVSimProcessor processor, HwVSimRequest request) {
        ...
        if (request != null && processor != null && (cardTypes = request.getCardTypes()) != null && cardTypes.length != 0) {
            logd("Disable: inserted card count = " + HwVSimUtilsInner.getInsertedCardCount(cardTypes));
            int savedMainSlot = getVSimSavedMainSlot();
    ...
    public void switchSimSlot(HwVSimProcessor processor, HwVSimRequest request) {
        ...
        boolean needSwich = (oldSlots.length == 3 && oldSlots[0] == slots[0] && oldSlots[1] == slots[1] && oldSlots[2] == slots[2]) ? false : true;
    ...
    @Override // com.android.internal.telephony.vsim.HwVSimModemAdapter
    public void onGetModemSupportVSimVersionDone(HwVSimProcessor processor, AsyncResultEx ar) {
    ...
    public void getModemSupportVSimVersionInner(HwVSimProcessor processor, HwVSimRequest request) {
        if (processor != null && request != null) {
            ...
            AsyncResultEx.forMessage(onCompleted).setException(new CommandExceptionEx(CommandExceptionEx.Error.REQUEST_NOT_SUPPORTED));
            onCompleted.sendToTarget();
        }
    }

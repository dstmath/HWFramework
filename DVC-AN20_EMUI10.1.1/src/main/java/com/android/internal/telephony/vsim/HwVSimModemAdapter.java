...
public abstract class HwVSimModemAdapter {
    ...
    private static final int SUB_COUNT_CROSS = (MAX_SUB_COUNT > 3 ? 3 : MAX_SUB_COUNT);
    ...
    public abstract void onGetModemSupportVSimVersionDone(HwVSimProcessor hwVSimProcessor, AsyncResultEx asyncResultEx);
    ...
    protected HwVSimModemAdapter(HwVSimController vsimController, Context context, CommandsInterfaceEx vsimCi, CommandsInterfaceEx[] cis) {
    ...
    public int parseModemSupportVSimVersionResult(HwVSimProcessor processor, AsyncResultEx ar) {
        if (processor == null || ar == null) {
            logd("parseModemSupportVSimVersionResult, param is null !");
            return -1;
        } else if (ar.getException() != null) {
            if (processor.isRequestNotSupport(ar.getException())) {
                logd("parse modem vsim version failed for request not support");
                return -2;
            }
            logd("parse modem vsim version failed, exception: " + ar.getException());
            return -1;
    ...
    public void setHwVSimPowerOnOff(int subId, boolean bPowerOn) {
        ...
        CommandsInterfaceEx ci = getCiBySub(subId);
        if (ci != null) {
            ci.setHwVSimPower(power, (Message) null);
        }
    }
    ...
    public boolean calcIsNeedSwitchCommrilMode(HwVSimSlotSwitchController.CommrilMode expect, HwVSimSlotSwitchController.CommrilMode current) {
        if (expect == HwVSimSlotSwitchController.CommrilMode.NON_MODE || expect == current) {
            return false;
        }
        return true;
    }
    ...
    public int getExpectSlotForDisable(int[] cardTypes, int mainSlot, int savedMainSlot) {
        ...
        logd("getExpectSlotForDisable: inserted card count = " + insertedCardCount);

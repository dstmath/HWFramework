...
public class HwVSimUtilsImpl extends DefaultHwVSimUtils {
    ...
    public IccCardConstantsEx.StateEx modifySimStateForVsim(int phoneId, IccCardConstantsEx.StateEx s) {
        return (sIsPlatformSupportVSim && HwVSimController.isInstantiated()) ? HwVSimController.getInstance().modifySimStateForVsim(phoneId, s) : s;
    }
    ...
    public void setDefaultMobileEnableForVSim(boolean enabled) {
        if (sIsPlatformSupportVSim && HwVSimController.isInstantiated()) {
            HwVSimUtilsInner.setDefaultMobileEnableForVSim(enabled);
        }
    }
}

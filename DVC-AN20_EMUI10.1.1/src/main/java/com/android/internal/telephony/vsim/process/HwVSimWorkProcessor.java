...
public abstract class HwVSimWorkProcessor extends HwVSimProcessor {
    ...
    public void onGetIccCardStatusDone(Message msg) {
        ...
        if (!(ar == null || (request = (HwVSimRequest) ar.getUserObj()) == null)) {
            ...
            logd("onGetIccCardStatusDone:mCardState[" + subId + "]=" + (status != null ? status.getCardState() : " state is null"));
    ...
    private int calcNetworkModeByAcqorder(String acqorder) {
        if (HwTelephonyManager.getDefault().isNrSupported() && acqorder.startsWith("08")) {
            return 65;
        }
        ...
        logd("calcNetworkModeByAcqorder, not support for " + 9);
        return 9;
    }

...
public class UserIcons {
    ...
    private static Drawable createRoundPhotoForHonor(Resources resources, int userId) {
        ...
        icon.setColorFilter(resources.getColor(com.android.hwext.internal.R.color.user_icon_head_default_honor, null), PorterDuff.Mode.SRC_IN);
        Bitmap srcBitmap = convertToBitmap(icon);
        if (srcBitmap == null) {
            return null;
        }
        ...
        matrix.setRotate(HALF_CIRCLE_DEGREE, ((float) width) / RADIUS_PARAMETER, ((float) height) / RADIUS_PARAMETER);

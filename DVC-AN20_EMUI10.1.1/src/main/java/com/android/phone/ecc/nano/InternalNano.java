    ...
    public static final <K, V> Map<K, V> mergeMapEntry(CodedInputByteBufferNano input, Map<K, V> map, MapFactories.MapFactory mapFactory, int keyType, int valueType, V value, int keyTag, int valueTag) throws IOException {
        ...
        while (true) {
            ...
            if (tag == 0) {
                ...
                obj2 = input.readPrimitiveField(keyType);
            } else if (tag == valueTag) {
                if (valueType == 11) {
                    input.readMessage((MessageNano) obj);
                } else {
                    obj = input.readPrimitiveField(valueType);
                }
        ...
        if (obj2 == null) {
            obj2 = primitiveDefaultValue(keyType);
        }
        if (obj == null) {
            obj = (V) primitiveDefaultValue(valueType);
        }
        map2.put(obj2, value2);
        return map2;
    }

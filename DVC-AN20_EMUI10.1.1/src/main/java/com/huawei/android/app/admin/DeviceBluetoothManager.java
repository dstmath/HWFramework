...
public class DeviceBluetoothManager {
    ...
    public boolean isBlackListedDevice(ComponentName admin, String device) {
        ...
        if (device == null || device.length() == 0 || (bundle = this.mDpm.getPolicy(admin, BLUETOOTH_BLACK_LIST)) == null || (lists = bundle.getStringArrayList("value")) == null) {
            return false;
        }
    ...
    public boolean isWhiteListedDevice(ComponentName admin, String device) {
        Bundle bundle;
        ArrayList<String> lists;
        if (device == null || device.length() == 0 || (bundle = this.mDpm.getPolicy(admin, BLUETOOTH_WHITE_LIST)) == null || (lists = bundle.getStringArrayList("value")) == null) {
            return false;
        }
        return lists.contains(device);
    }
    ...
    private ArrayList<String> checkBluetoothListConflict(String checkType, ArrayList<String> filteredDevices) {
        ...
        if (checkType.equals(BLUETOOTH_BLACK_LIST)) {
            bundle = this.mDpm.getPolicy((ComponentName) null, BLUETOOTH_WHITE_LIST);
        } else if (checkType.equals(BLUETOOTH_WHITE_LIST)) {
            bundle = this.mDpm.getPolicy((ComponentName) null, BLUETOOTH_BLACK_LIST);
        }
        if (bundle == null) {
            return devices;
        }

...
public class DeviceEmailManager {
    ...
    public boolean isAppDataClearanceDisabled() {
        Bundle bundle = this.mDpm.getPolicy((ComponentName) null, RESTRICT_DISABLE_CLEAR_DATA);
        if (bundle == null) {
    ...
    public void configEmailAccount(ComponentName admin, EmailAccount emailAccount) {
        ...
        if (emailAccount != null) {
            ...
            String str = "1";
            emailAccountConfig.putString(EMAIL_ACCOUNT_IS_DEFAULT, emailAccount.mIsDefault ? str : ProxyControllerEx.MODEM_0);
            ...
            emailAccountConfig.putString(EMAIL_ACCOUNT_INCOMING_SERVER_ACCEPT_ALL_CERTIFICATES, emailAccount.mInComingServerAcceptAllCertificates ? str : ProxyControllerEx.MODEM_0);
            emailAccountConfig.putString(EMAIL_ACCOUNT_INCOMING_SERVER_USE_SSL, emailAccount.mInComingServerUseSSL ? str : ProxyControllerEx.MODEM_0);
            emailAccountConfig.putString(EMAIL_ACCOUNT_INCOMING_SERVER_USE_TLS, emailAccount.mInComingServerUseTLS ? str : ProxyControllerEx.MODEM_0);
            ...
            emailAccountConfig.putString(EMAIL_ACCOUNT_OUTGOING_SERVER_ACCEPT_ALL_CERTIFICATES, emailAccount.mOutGoingServerAcceptAllCertificates ? str : ProxyControllerEx.MODEM_0);
            emailAccountConfig.putString(EMAIL_ACCOUNT_OUTGOING_SERVER_USE_SSL, emailAccount.mOutGoingServerUseSSL ? str : ProxyControllerEx.MODEM_0);
            if (!emailAccount.mOutGoingServerUseTLS) {
                str = ProxyControllerEx.MODEM_0;
            }
            emailAccountConfig.putString(EMAIL_ACCOUNT_OUTGOING_SERVER_USE_TLS, str);
    ...
    public void configExchangeAccount(ComponentName admin, ExchangeAccount exchangeAccount) {
        ...
        if (exchangeAccount != null) {
            String str = "1";
            exchangeAccountConfig.putString(EXCHANGE_ACCOUNT_ACCEPT_ALL_CERTIFICATES, exchangeAccount.mAcceptAllCertificates ? str : ProxyControllerEx.MODEM_0);
            ...
            exchangeAccountConfig.putString(EXCHANGE_ACCOUNT_IS_DEFAULT, exchangeAccount.mIsDefault ? str : ProxyControllerEx.MODEM_0);
            ...
            exchangeAccountConfig.putString(EXCHANGE_ACCOUNT_USE_SSL, exchangeAccount.mUseSSL ? str : ProxyControllerEx.MODEM_0);
            exchangeAccountConfig.putString(EXCHANGE_ACCOUNT_USE_TSL, exchangeAccount.mUseTLS ? str : ProxyControllerEx.MODEM_0);
            exchangeAccountConfig.putString(EXCHANGE_ACCOUNT_SCREENSHOT_DISABLED, exchangeAccount.mScreenShotDisabled ? str : ProxyControllerEx.MODEM_0);
            exchangeAccountConfig.putString(EXCHANGE_ACCOUNT_DISPLAY_HTML_DISABLED, exchangeAccount.mDisplayHtmlDisabled ? str : ProxyControllerEx.MODEM_0);
            if (!exchangeAccount.mForwardEmailDisabled) {
                str = ProxyControllerEx.MODEM_0;
            }
            exchangeAccountConfig.putString(EXCHANGE_ACCOUNT_FORWARD_EMAIL_DISABLED, str);
    ...
    public void configExchangeMailProvider(ComponentName admin, HwMailProvider para) {
        ...
        paraex.putString(Constant.JSONKEY_ID, para.getId());
        paraex.putString(ContactsContractEx.StreamItemsColumns.RES_LABEL, para.getLabel());
    ...
    public HwMailProvider getMailProviderForDomain(ComponentName admin, String domain) {
        ...
        if (paraex != null) {
            return new HwMailProvider(paraex.getString(Constant.JSONKEY_ID), paraex.getString(ContactsContractEx.StreamItemsColumns.RES_LABEL), paraex.getString("domain"), paraex.getString("incominguri"), paraex.getString("incomingusername"), paraex.getString("incomingfield"), paraex.getString("outgoinguri"), paraex.getString("outgoingusername"));
        }
        return null;
    }
    ...
    public boolean isAccountAdditionDisabled(ComponentName admin, String emailAddress) {
        ...
        boolean matched2 = isInRuleList(emailAddress, getAccountsBlackListAllowingAddition(admin));
        Log.d(TAG, "MDM6->isAccountAdditionDisabled->matched in black: " + matched2);
        return matched2;
    }
    ...
    public ArrayList<String> getAccountsWhiteListDisablingDeletion(ComponentName admin) {
        return getAccountsWhiteListDisablingDeletion(admin, UserHandle.myUserId());
    }
    ...
    public ArrayList<String> getAccountsBlackListAllowingDeletion(ComponentName admin) {
        return getAccountsBlackListAllowingDeletion(admin, UserHandle.myUserId());
    }
    ...
    public boolean isAccountDeletionDisabled(ComponentName admin, String emailAddress, int userId) {
        ...
        if (isAccountDeletionDisabled(admin, userId)) {
            boolean matched = isInRuleList(emailAddress, getAccountsWhiteListDisablingDeletion(admin, userId));
            Log.d(TAG, "MDM6->isAccountDeletionAllowed->matched in white: " + matched);
            return !matched;
        }
        boolean matched2 = isInRuleList(emailAddress, getAccountsBlackListAllowingDeletion(admin, userId));
        Log.d(TAG, "MDM6->isAccountDeletionAllowed->matched in black: " + matched2);
        return matched2;
    }
    ...
    private boolean wildCardMatch(String text, String pattern) {
        ...
        if (!(pattern2.endsWith(PhoneConstantsEx.APN_TYPE_ALL) || cards.length <= 1 || text2.endsWith(cards[cards.length - 1]))) {
            return false;
        }
    ...
    public boolean isForceSMIMECertificateAlias(Context context, String emailAddress, String alias, int type) {
        ...
        Uri uri = getUri(ALIAS_PATH, type);
        Cursor cursor = null;
        try {
            cursor = context.getContentResolver().query(uri, null, ACCOUNT_WHERE, new String[]{emailAddress}, null);
            if (cursor == null || !cursor.moveToNext()) {
                return false;
            }
    ...
    public boolean setForceSMIMEMessages(ComponentName admin, String emailAddress, boolean force, int type) {
        return setForceSMIMEPolicy(admin, emailAddress, "", type, force ? 1 : 0);
    }
    ...
    public boolean isForceSMIMEMessages(Context context, String emailAddress, int type) {
        ...
        Uri uri = getUri(SMIME_PATH, type);
        Cursor cursor = null;
        try {
            cursor = context.getContentResolver().query(uri, null, ACCOUNT_WHERE, new String[]{emailAddress}, null);
            if (cursor == null || !cursor.moveToNext()) {

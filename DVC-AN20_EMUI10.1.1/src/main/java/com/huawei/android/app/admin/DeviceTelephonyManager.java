...
public class DeviceTelephonyManager {
    ...
    public boolean isRoamingSyncDisabled(ComponentName admin) {
        ...
        if (bundle != null) {
            ...
            Log.d(TAG, "isRoamingSyncDisabled: " + allow2);
    ...
    public boolean isRoamingPushDisabled(ComponentName admin) {
        ...
        if (bundle != null) {
            Boolean allow2 = Boolean.valueOf(bundle.getBoolean("value"));
            Log.d(TAG, "isRoamingPushDisabled: " + allow2);
            return allow2.booleanValue();
        }
        Log.d(TAG, "has not set the allow, return default false");
        return allow.booleanValue();
    ...
    private boolean removeMonthMode(ComponentName who, boolean isOutgoing) {
        ...
        if (!TextUtils.isEmpty(policyName)) {
            setResult = this.mDpm.removePolicy(who, policyName, (Bundle) null);
        }
    ...
    private boolean removePattern(ComponentName admin, String pattern) {
        return this.mDpm.removePolicy(admin, pattern, (Bundle) null);
    }
    ...
    public boolean setDefaultMainSlotCarrier(ComponentName admin, String carrier) {
        ...
        if (carrier == null || carrier.isEmpty() || carrier.length() > 10) {
    ...
    public boolean removeDefaultMainSlotCarrier(ComponentName admin) {
        return this.mDpm.removePolicy(admin, DEFAULT_MAIN_SLOT_CARRIER, (Bundle) null);
    }
}

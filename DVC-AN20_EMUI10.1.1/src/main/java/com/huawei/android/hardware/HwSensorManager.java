...
public class HwSensorManager implements SensorEventListener {
    ...
    /* JADX INFO: Multiple debug info for r0v2 float[]: [D('gyro_wx' float), D('values' float[])] */
    public void onSensorChanged(SensorEvent event) {
        ...
        if (type == 1) {
            ...
            if (Math.abs(gyro_wy) >= 0.1f || Math.abs(gyro_wx) >= 0.1f || Math.abs(gyro_wz) >= 0.1f || (sensorEventDetector3 = this.mSensorEventDetector) == null) {
                ...
                if (handler2 != null && !handler2.hasMessages(1) && getDirectionChanged(gyro_wy, gyro_wx, angle_y, this.mAngleX)) {

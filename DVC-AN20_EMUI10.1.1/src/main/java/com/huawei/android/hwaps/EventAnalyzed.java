...
public class EventAnalyzed implements IEventAnalyzed {
    ...
    private static final String VERSION = "10.1.1.18";
    ...
    private boolean mRegisteCallbackSucceed = false;
    ...
    public void initAPS(Context context, int myPid) {
        Log.i(TAG, "APS: EventAnalyzed: initAPS: version is 10.1.1.18");
        ...
        this.mDefaultMaxFps = SystemProperties.getInt("sys.aps.defaultmaxfps", (int) DEFAULT_MAX_FPS);
        this.mPowerModeFps = SystemProperties.getInt("sys.aps.powermodefps", -1);
        this.mResumeFpsByTouch = new ResumeFpsByTouch();
        HwApsInterface.nativeSetGamePid(myPid);
    }

    private boolean isSupportPowerModeFps() {
        String str;
        String enablePackage = SystemProperties.get("sys.aps.game.pkgname", "");
    ...
    private void processBaseFps(int mode) {
        ...
        int i = this.mBaseFps;
        if (i >= MIN_APS_ENABLE_FPS && i <= this.mDefaultMaxFps) {
    ...
    private void processIdleFps(int action) {
        int i = this.mMaxFps;
        if (i >= MIN_APS_ENABLE_FPS && i > this.mBaseFps) {
    ...
    public void processiAwareFps(int fps) {
        if (fps >= MIN_APS_ENABLE_FPS && fps < this.mDefaultMaxFps) {
    ...
    public void processAnalyze(int action) {
        ...
        if (action == 1 || action == 0 || action == 4) {
            ...
            int mode = SystemProperties.getInt("sys.aps.power_mode", (int) ACTION_DOWN);
    ...
    public class ApsManagerServiceCallback extends IApsManagerServiceCallback.Stub {
        ...
        public void doCallback(int apsCallbackCode, final int data) {
            try {
                ...
                new Thread(new Runnable() {
                    /* class com.huawei.android.hwaps.EventAnalyzed.ApsManagerServiceCallback.AnonymousClass1 */
                ...
                Log.e(EventAnalyzed.TAG, "APS: EventAnalyzed: doCallback, exception:" + e);
            }
        }
    }
    ...
    private void registerCallbackInApsManagerService() {
        try {
            if (!this.mRegisteCallbackSucceed) {
                ...
                if (!(this.mApsManagerServiceCallback == null || this.mPkgName == null)) {
                    this.mRegisteCallbackSucceed = this.mApsManager.registerCallback(this.mPkgName, this.mApsManagerServiceCallback);
                }
                Log.d(TAG, "APS: EventAnalyzed: registerCallbackInApsManagerService, mPkgName:" + this.mPkgName + "; result = " + this.mRegisteCallbackSucceed);
            }
        } catch (Exception e) {
    ...
    private void unregisterCallbackInApsManagerService() {
        try {
            if (this.mRegisteCallbackSucceed) {
                ...
                this.mRegisteCallbackSucceed = false;
            }
        } catch (Exception e) {

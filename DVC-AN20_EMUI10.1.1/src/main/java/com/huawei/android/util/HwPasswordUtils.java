...
public class HwPasswordUtils {
    ...
    public static void loadSimplePasswordTable(Context context) {
        String simplePasswordSequence;
        for (String str : SIMPLE_PASSWORD_SET) {
            simplePasswordTable.put(str, null);
        }
        if (!(context == null || (simplePasswordSequence = SettingsEx.Systemex.getString(context.getContentResolver(), "simple_password_table")) == null || simplePasswordSequence.isEmpty())) {

...
public class HiPlayAdapter {
    ...
    private void handleServiceStop(DMSDPDeviceService deviceService) {
        ...
        if (tmpDmsdpDevice != null) {
            ...
            this.mDMSDPAdapter.disconnectDevice(4, getDeviceChannelType(tmpDmsdpDevice), tmpDmsdpDevice);
        }
        ConcurrentHashMap<String, Integer> concurrentHashMap = this.deviceServiceState;
        concurrentHashMap.remove(deviceService.getDeviceId() + ":" + deviceService.getServiceType());
        ConcurrentHashMap<String, DMSDPDeviceService> concurrentHashMap2 = this.usedServices;
    ...
    private void checkAndUpdateDeviceConnState() {
        Iterator<String> devIt = this.hiplayNeedDeviceList.iterator();
        while (devIt.hasNext()) {
            String devId = devIt.next();
            Iterator<HiPlayHelper.DevSvrId> devSvrIt = this.speakerServiceList.iterator();
            while (devSvrIt.hasNext()) {
                HiPlayHelper.DevSvrId devSvrId = devSvrIt.next();
    ...
    private synchronized void handleDmsdpDisconnect(DMSDPDevice device) {
        ...
        if (this.allHiPlayDevices.get(device.getDeviceId()) != null) {
            ...
            String str = TAG;
            HwLog.d(str, "serviceTypes is: " + serviceTypes);
    ...
    private boolean validEnableParams(HiPlayDevice device, EnumSet<Capability> capability, String connJson) {
        ...
        if (connJson == null || BuildConfig.FLAVOR.equals(connJson)) {
            HwLog.e(TAG, "enableVirtualDevice connJson is null");
            return false;
        }
        try {
            JSONObject jsonObject = new JSONObject(connJson);
            if (jsonObject.has(HiPlayHelper.AUTH_MODE) && !BuildConfig.FLAVOR.equals(jsonObject.getString(HiPlayHelper.AUTH_MODE)) && jsonObject.has(HiPlayHelper.DISCOVER_MODE)) {
                if (!BuildConfig.FLAVOR.equals(jsonObject.getString(HiPlayHelper.DISCOVER_MODE))) {
                    return true;
                }
            }
            String str2 = TAG;
            HwLog.e(str2, "enable tv connJson invalid" + connJson);
            return false;
    ...
    public synchronized void handleNewDevice(HiPlayHelper.DeviceOperationParams params) {
        ...
        tmpDmsdpDevice.addProperties(DeviceParameterConst.AUTH_MODE_JSON_STRING, params.getJsonString());
        ...
        int channelType = getDeviceChannelType(tmpDmsdpDevice);

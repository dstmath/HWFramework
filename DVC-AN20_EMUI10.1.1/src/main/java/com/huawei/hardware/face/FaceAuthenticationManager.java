...
public class FaceAuthenticationManager {
                ...
                if (type == 2) {
                    synchronized (FaceAuthenticationManager.this.mAuthenticationLock) {
                        if (FaceAuthenticationManager.this.mAuthenticationCallback != null) {
                            if (1 == code) {
                                if (errorCode == 0) {
                                    FaceAuthenticationManager.this.mHandler.obtainMessage(FaceAuthenticationManager.MSG_AUTHENTICATION_SUCCEEDED, UserHandleEx.myUserId(), 0, null).sendToTarget();
                                } else if (3 == errorCode) {
                                    ...
                                    FaceAuthenticationManager.this.mHandler.obtainMessage(FaceAuthenticationManager.MSG_ERROR, error, errorCode, 0L).sendToTarget();
                                }
                            } else if (3 == code) {
                                int acquireInfo = 13;
                                ...
                                FaceAuthenticationManager.this.mHandler.obtainMessage(101, acquireInfo, errorCode, 0L).sendToTarget();
                            }
                        }
                    }
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:21:0x0047, code lost:
        android.util.Log.w(com.huawei.hardware.face.FaceAuthenticationManager.TAG, "Authentication initialization failed.");
    ...
    public boolean isOpenApiSupported(AuthenticationCallback authenticationCallback) {
        ...
        if (authenticationCallback != null) {
            Log.e(TAG, "Face recognize is not supported.");
    ...
    public boolean hasEnrolledFace() {
        FaceRecognizeManagerImpl faceRecognizeManagerImpl = this.mFaceManagerImpl;
        if (faceRecognizeManagerImpl == null || faceRecognizeManagerImpl.getEnrolledFaceIDs().length <= 0) {
            return false;
        }
        return true;
    }
    ...
    public int getEnrolledFaceID() {
        FaceRecognizeManagerImpl faceRecognizeManagerImpl = this.mFaceManagerImpl;
        if (faceRecognizeManagerImpl == null) {
            return 0;
        }
        int[] faceIds = faceRecognizeManagerImpl.getEnrolledFaceIDs();
        if (faceIds.length == 0) {
            return 0;
        }
        return faceIds[0];
    ...
    public boolean isHardwareDetected() {
        FaceRecognizeManagerImpl faceRecognizeManagerImpl = this.mFaceManagerImpl;
        if (faceRecognizeManagerImpl == null || (faceRecognizeManagerImpl.getHardwareSupportType() & 1) == 0) {
            return false;
        }
        return true;
    }
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case 101:
                    ...
                    return;
                case FaceAuthenticationManager.MSG_AUTHENTICATION_SUCCEEDED /*{ENCODED_INT: 102}*/:
                    sendAuthenticatedSucceeded((Face) msg.obj, msg.arg1);
                    return;
                case FaceAuthenticationManager.MSG_AUTHENTICATION_FAILED /*{ENCODED_INT: 103}*/:
                    sendAuthenticatedFailed();
                    return;
                case FaceAuthenticationManager.MSG_ERROR /*{ENCODED_INT: 104}*/:

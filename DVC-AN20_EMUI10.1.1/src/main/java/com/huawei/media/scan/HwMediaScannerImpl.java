...
public class HwMediaScannerImpl extends DefaultHwMediaScanner {
    ...
    private synchronized void init(Context context) {
        ...
        if (this.mContext != null && !this.mIsInit) {
            ...
            if (this.mBlackListUtils != null) {
                this.mBlackListUtils.init();
            }
    ...
    private void handleMediaMounted(Intent intent) {
        ...
        try {
            String volumeName2 = MediaStoreEx.getVolumeName(new File(intent.getData().getPath()).getCanonicalFile());
        ...
        if (uri != null) {
            this.mSkipExternalQuery = isSkipExtSdcard(this.mMediaProvider, this.mExtStoragePath, MediaStoreEx.setIncludeTrashed(MediaStoreEx.setIncludePending(uri.buildUpon().appendQueryParameter("nonotify", "1").build())));
        }
                ...
                while (i2 < length) {
                    ...
                    if (cursor != null) {
                        if (cursor.moveToFirst()) {
                            ...
                            Log.i(TAG, "ensureDefaultRingtones2 ringtoneUri " + ringtoneUri + ", type " + type);

...
public class Utils {
    ...
    public static boolean isValidPath(Context context, String path) {
        ...
        if (MediaStoreEx.IS_SUPPORT_CLONE_APP) {
            for (StorageVolume storageVolume2 : MediaStoreEx.getAppClonedUserVolumes(context)) {
                if (path2.startsWith(appendFileSeparator(StorageVolumeEx.getPath(storageVolume2)))) {
                    return true;
                }
            }
        }

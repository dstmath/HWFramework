        ...
        try {
            ...
            for (int j = 0; j < maTypeListSize; j++) {
                for (int i = 0; i < listenerSize; i++) {
                    this.mMDListenerList.get(i).notifyMotionRecoResult(getRecoResult(maTypeList.get(j).intValue(), rRes, rDirect, rExtras));
                }
            }
        } catch (Exception e) {
            MRLog.w(TAG, "notifyMotionRecoResult Exception");
        }
    }

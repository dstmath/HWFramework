...
public final class ConnectionPool {
    ...
    public ConnectionPool(int maxIdleConnections2, long keepAliveDuration, TimeUnit timeUnit) {
        this.cleanupRunnable = new Runnable() {
            /* class com.huawei.okhttp3.ConnectionPool.AnonymousClass1 */
    ...
    public Socket deduplicate(Address address, StreamAllocation streamAllocation, int connectionReserved) {
        int index = 0;
        for (RealConnection connection : this.connections) {
            if (connection.isEligible(address, null) && connection.isMultiplexed() && connection != streamAllocation.connection() && (index = index + 1) == connectionReserved) {
    ...
    private void removeHttp2ConnectionHost(RealConnection connection) {
        Http2Host http2Host;
        if (connection != null && connection.isMultiplexed() && (http2Host = getHttp2Host(connection.route().address())) != null) {
    ...
    private void onHttp2ConnectionEvictedEvent(Address address) {
        ...
        while (itWr.hasNext()) {
            Http2ConnectionEventListener listenerInList = itWr.next().get();

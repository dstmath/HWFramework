...
public class HwKeystoreManager {
    ...
    public static HwKeystoreManager getInstance() {
        IHwKeystoreService binder = IHwKeystoreService.Stub.asInterface(ServiceManagerEx.getService("com.huawei.security.IHwKeystoreService"));
        if (binder == null) {
    ...
    public static HwUniversalKeyStoreException getKeyStoreException(int errorCode) {
        if (errorCode > 0) {
            ...
            switch (errorCode) {
                case AUTH_TYPE_UNSUPPORT /*{ENCODED_INT: 100000}*/:
                    return new HwUniversalKeyStoreException(errorCode, "Auth type unsupport");
                case AUTH_TEMPLATE_ID_BIND_REPEAT /*{ENCODED_INT: 100001}*/:
                    return new HwUniversalKeyStoreException(errorCode, "Bound auth template ID repeat");
                case AUTH_TEMPLATE_ID_BIND_OVERFLOW /*{ENCODED_INT: 100002}*/:
    ...
    public InvalidKeyException getInvalidKeyException(String keystoreKeyAlias, int uid, HwUniversalKeyStoreException e) {
        ...
        if (errorCode != 15) {
            switch (errorCode) {
                case HwKeymasterDefs.KM_ERROR_KEY_USER_NOT_AUTHENTICATED /*{ENCODED_INT: -26}*/:
                    break;
                case HwKeymasterDefs.KM_ERROR_KEY_EXPIRED /*{ENCODED_INT: -25}*/:
                    return new KeyExpiredException("Key is expired");
                case HwKeymasterDefs.KM_ERROR_KEY_NOT_YET_VALID /*{ENCODED_INT: -24}*/:

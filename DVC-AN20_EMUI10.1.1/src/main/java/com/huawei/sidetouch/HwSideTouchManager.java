...
public class HwSideTouchManager {
    ...
    private static final String SIDE_CONFIG_BOTHHAND = "5";
    private static final String SIDE_CONFIG_LEFTHAND = "3";
    private static final String SIDE_CONFIG_RIGHTHAND = "4";
    ...
    private static HwSideTouchManager mInstance = null;
    private static HwSideVibrationManager mSideViberateManager = null;
    private int currentSideTouchMode = -1;
    ...
    public static synchronized HwSideTouchManager getInstance(Context context) {
        ...
        synchronized (HwSideTouchManager.class) {
            if (mInstance == null) {
                mInstance = new HwSideTouchManager(context);
            }
            hwSideTouchManager = mInstance;
        }
    ...
    private int getRegion(String region, boolean isStart) {
        String[] splitResult;
        int defaultRegion = isStart ? REGION_START_DEFAULT : 1010;
    ...
    private int[] getSideTouchStatus(Bundle bundle) {
        if (bundle != null) {
            Log.d(TAG, " mTPStatus is " + this.mHandStatus);
            return new int[]{this.mHandStatus};
        }
        String volumeMode = null;
        try {
    ...
    private int getModeFromTP(String sMode) {
        String[] splitResult;
        if (sMode == null || (splitResult = sMode.split(TpCommandConstant.SEPARATE)) == null || !STATUS_OK.equals(splitResult[0])) {
            return -1;
        }
        try {
            return Integer.parseInt(splitResult[1]);
    ...
    private void ajustSideTouchRegion(Bundle bundle, boolean isEnable, boolean isMapping) {
        ...
        try {
            ...
            } else if ((rect.bottom + rect.top) / 2 > 1010) {
    ...
    private void notifySideConfig(int sidePosition) {
        if (sidePosition == 1) {
            this.mSideConfig = SIDE_CONFIG_LEFTHAND;
        ...
        if (iHwSideTouchCallback != null) {
            iHwSideTouchCallback.notifySideConfig(this.mSideConfig);
        }
    }
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void updateSidePosition(int position) {
        if (position > 3 || position < 0) {
            Log.w(TAG, "side position error " + position);
            return;
        }
        notifySideConfig(position);
    }
    ...
    private void updateSideTouchVolumeMode() {
        ...
        this.currentSideTouchMode = volumeModes[0];
        if (this.currentSideTouchMode == 0) {
            Log.i(TAG, "currentSideTouchMode is disable, do nothing");
            return;
        }
        int newTouchMode = 2248;
        boolean isSingleHandMode = getSingleHandEnableState();
        boolean isTalkBackEnabled = getTalkBackEnableState();
        Log.i(TAG, "isSingleHandMode is " + isSingleHandMode + " isTalkBackEnabled is " + isTalkBackEnabled + " currentSideTouchMode is " + this.currentSideTouchMode);
        if (isSingleHandMode || isTalkBackEnabled) {
            newTouchMode = getSideTouchVolumeStepMode(this.currentSideTouchMode);
        }
        if (newTouchMode == -1) {
    ...
    public boolean getTalkBackEnableState() {
        if (Settings.Secure.getIntForUser(this.mContext.getContentResolver(), ACCESSIBILITY_SCREENREADER_ENABLED, 0, ActivityManager.getCurrentUser()) == 1) {
            return true;
        }
        return false;
    }
    ...
    private void initSidePositionObserver() {
        ...
        this.mSidePositionObserver = new ContentObserver(null) {
            ...
            public void onChange(boolean isSelfChange) {
                int sidePosition = Settings.Secure.getIntForUser(HwSideTouchManager.this.mContext.getContentResolver(), TpCommandConstant.KEY_SIDE_POSITION, 3, ActivityManager.getCurrentUser());
                Log.i(HwSideTouchManager.TAG, "side position onChange: " + sidePosition);
                HwSideTouchManager.this.updateSidePosition(sidePosition);
            }
        };
        ...
        this.mSidePositionObserver.onChange(true);
    }
    ...
    private HwSideVibrationManager getVibrateManager() {
        Context context;
        if (mSideViberateManager == null && (context = this.mContext) != null) {
            mSideViberateManager = HwSideVibrationManager.getInstance(context);
        }
        return mSideViberateManager;
    }
    ...
    public void notifySideTouchManager(int event) {
        ...
        int leftTrigger = event & 1024;
        int rightTrigger = event & TpCommandConstant.TSA_EVENT_RIGHT_VOLUME_TRIGGER;
        if (leftTrigger != 0 || rightTrigger != 0) {
            mSideViberateManager = getVibrateManager();
            HwSideVibrationManager hwSideVibrationManager = mSideViberateManager;
            if (hwSideVibrationManager != null) {
                hwSideVibrationManager.notifyTriggerTimer();
            }
        }
    }
    ...
    public void notifySendVolumeKeyToSystem(KeyEvent event) {
        if (event == null) {
            ...
            Log.e(TAG, "keycode is not volume down");
        } else {
            boolean isDown = false;
            boolean isTriggered = (event.getFlags() & 4096) != 0;

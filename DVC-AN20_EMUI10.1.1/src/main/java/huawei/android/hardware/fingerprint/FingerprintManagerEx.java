...
public class FingerprintManagerEx {
    ...
    private static final int FINGERPRINT_SPOT_BOTTOM_RIGHT_FIRST = 2;
    private static final int FINGERPRINT_SPOT_BOTTOM_RIGHT_SECOND = 3;
    private static final int FINGERPRINT_SPOT_TOP_LEFT_FIRST = 0;
    private static final int FINGERPRINT_SPOT_TOP_LEFT_SECOND = 1;
    ...
    public List<Fingerprint> getEnrolledFingerprints(int targetDevice, int userId) {
        ...
        if (b != null) {
            try {
                ...
                _reply.readException();
                _reply.readTypedList(arrayList, Fingerprint.CREATOR);
            } catch (RemoteException e) {
    ...
    private static int[] getHardwarePositionFromHal() {
        ...
        if (b != null) {
            try {
                ...
                Log.e(TAG, "getHardwarePositionFromHal with RemoteException happened");
            } catch (Throwable th) {
                _reply.recycle();
                _data.recycle();
                throw th;
    ...
    private static int getInformationFromHal(int halCommand) {
        ...
        if (binder != null) {
            try {
                ...
                binder.transact(halCommand, spotData, spotReply, 0);
    ...
    public static int[] getSpotPosition() {
        ...
        if (binder != null) {
            try {
                spotData.writeInterfaceToken(DESCRIPTOR_FINGERPRINT_SERVICE);
                binder.transact(CODE_GET_HARDWARE_POSITION, spotData, spotReply, 0);
                spotReply.readException();
                positions[0] = spotReply.readInt();
                positions[1] = spotReply.readInt();
                positions[2] = spotReply.readInt();
                positions[3] = spotReply.readInt();
            } catch (RemoteException e) {
                Log.e(TAG, "getSpotPosition");
            } catch (Throwable th) {
                spotReply.recycle();
                spotData.recycle();
                throw th;

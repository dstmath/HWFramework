    ...
    public static byte[] getPolicyBlock(String apkFile) throws PolicyNotFoundException {
        if (apkFile != null) {
            try {
                ...
                ByteBuffer policyBlockBuffer = ApkSigningBlockUtils.findHwSignature(apk).signatureBlock;
                byte[] policyBlock = new byte[policyBlockBuffer.remaining()];
                policyBlockBuffer.get(policyBlock);
                apk.close();
                return policyBlock;
            } catch (SignatureNotFoundException e) {
                throw new PolicyNotFoundException("getPolicy err for SignatureNotFoundException", e);

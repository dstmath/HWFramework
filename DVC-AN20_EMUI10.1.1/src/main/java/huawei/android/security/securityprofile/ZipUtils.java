...
abstract class ZipUtils {
    ...
    private static Pair<ByteBuffer, Long> findZipEndOfCentralDirectoryRecord(RandomAccessFile zip, int maxCommentSize) throws IOException {
        ...
        int realMaxCommentSize = maxCommentSize;
        if (((long) maxCommentSize) > fileSize - 22) {
            realMaxCommentSize = (int) (fileSize - 22);
        }
        ByteBuffer buf = ByteBuffer.allocate(realMaxCommentSize + 22);

...
public class CallerInfoHW implements TelephonyInterfacesHW {
    ...
    public int getCallerIndex(Cursor cursor, String compNum, String columnName) {
        return getCallerIndex(cursor, compNum, columnName, SystemPropertiesEx.get(HwTelephonyProperties.PROPERTY_NETWORK_COUNTRY_ISO, ""));
    }

    /* JADX INFO: Multiple debug info for r1v1 int: [D('NUM_LONG' int), D('compNumLong' java.lang.String)] */
    /* JADX INFO: Multiple debug info for r2v1 int: [D('NUM_SHORT' int), D('tmpNum' java.lang.String)] */
    ...
    public int getCallerIndex(Cursor cursor, String compNum, String columnName, String countryIso) {
        ...
        if (cursor != null) {
            if (cursor.getCount() > 0) {
                ...
                String compNum6 = formatedForDualNumber(PhoneNumberUtils.stripSeparators(compNum));
                compNum6.length();
                ...
                sb.append(countryIso);
                logd(sb.toString());
                if (this.IS_CHINA_TELECOM && compNum6.startsWith("**133") && compNum6.endsWith("#")) {
                    compNum6 = compNum6.substring(0, compNum6.length() - 1);
                    logd("compNum startsWith **133 && endsWith #");
                }
                this.mNetworkOperator = SystemPropertiesEx.get(HwTelephonyProperties.PROPERTY_NETWORK_OPERATOR, "");
                if (!TextUtils.isEmpty(countryIso)) {
                    String formattedCompNum = PhoneNumberUtils.formatNumberToE164(compNum6, countryIso.toUpperCase(Locale.US));
                    if (formattedCompNum != null) {
                        logd("formattedCompNum: " + formattedCompNum + ", with countryIso: " + countryIso);
                        compNum6 = formattedCompNum;
                ...
                int countryCodeLen4 = getIntlPrefixAndCCLen(compNum6);
                if (countryCodeLen4 > 0) {
                    compNumPrefix = compNum6.substring(0, countryCodeLen4);
                    compNum6 = compNum6.substring(countryCodeLen4);
                    StringBuilder sb2 = new StringBuilder();
                    countryCodeLen = countryCodeLen4;
                    sb2.append("compNum after remove prefix: ");
                    sb2.append(compNum6);
                    sb2.append(", compNumLen: ");
                    sb2.append(compNum6.length());
                ...
                String tmpCompNum3 = !TextUtils.isEmpty(compNum2) ? compNum2 : compNum6;
                ...
                if (isCnNumber) {
                    String compNum7 = deleteIPHead(compNum6);
                    boolean isCompNumCnMPN2 = isChineseMobilePhoneNumber(compNum7);
                    if (!isCompNumCnMPN2) {
                        int areaCodeLen = getChineseFixNumberAreaCodeLength(compNum7);
                        if (areaCodeLen > 0) {
                            isCompNumCnMPN = isCompNumCnMPN2;
                            compNumAreaCode4 = compNum7.substring(0, areaCodeLen);
                            compNum7 = compNum7.substring(areaCodeLen);
                            logd("CN compNum after remove area code: " + compNum7 + ", compNumLen: " + compNum7.length() + ", compNumAreaCode: " + compNumAreaCode4);
                        } else {
                            isCompNumCnMPN = isCompNumCnMPN2;
                        }
                        origTmpNum = null;
                    ...
                    if ("PE".equalsIgnoreCase(countryIso)) {
                        ...
                        isCompNumCnMPN = false;
                        if (compNum6.length() >= 7) {
                            origTmpNum = null;
                            if (CHINA_AREACODE.equals(compNum6.substring(0, 1)) && !CHINA_AREACODE.equals(compNum6.substring(1, 2))) {
                                origTmpNum2 = compNum6.substring(1);
                                compNumAreaCode = null;
                                NUM_LONG = NUM_LONG5;
                ...
                if (compNumLen >= NUM_LONG) {
                    ...
                    if (cursor.moveToFirst()) {
                        ...
                        int formatColumnIndex3 = cursor.getColumnIndex("normalized_number");
                        int data4ColumnIndex2 = cursor.getColumnIndex("data4");
                        logd("11: columnIndex: " + columnIndex + ", formatColumnIndex: " + formatColumnIndex3 + ", data4ColumnIndex: " + data4ColumnIndex2);
                        if (columnIndex != -1) {
                            int numShortID2 = -1;
                            int numLongID2 = -1;
                            while (true) {
                                String tmpNum11 = cursor.getString(columnIndex);
                                if (tmpNum11 == null) {
                                    return -1;
                                }
                                if (tmpNum11.indexOf(64) >= 0) {
                                    return -1;
                                }
                                origTmpNum6 = PhoneNumberUtils.stripSeparators(tmpNum11);
                                logd("origTmpNum: " + origTmpNum6);
                                if (-1 != formatColumnIndex3) {
                                    tmpNumFormat5 = cursor.getString(formatColumnIndex3);
                                    tmpNum9 = isValidData4Number(origTmpNum6, tmpNumFormat5) ? tmpNumFormat5 : origTmpNum6;
                                } else if (-1 != data4ColumnIndex2) {
                                    tmpNumFormat5 = cursor.getString(data4ColumnIndex2);
                                    tmpNum9 = isValidData4Number(origTmpNum6, tmpNumFormat5) ? tmpNumFormat5 : origTmpNum6;
                                } else {
                                    tmpNum9 = origTmpNum6;
                                    tmpNumFormat5 = tmpNumFormat6;
                                }
                                logd("11: tmpNumFormat: " + tmpNumFormat5);
                                tmpNum9.length();
                                logd("11: tmpNum = " + tmpNum9 + ", tmpNum.length11: " + tmpNum9.length() + ",ID = " + cursor.getPosition());
                                if (tmpNum9.equals(tmpCompNum3)) {
                                    numLongID = cursor.getPosition();
                                    logd("11: > NUM_LONG numLongID = " + numLongID + ", formattedNum full match!");
                                    tmpNumFormat6 = tmpNumFormat5;
                                    break;
                                }
                                int countryCodeLen5 = getIntlPrefixAndCCLen(tmpNum9);
                                if (countryCodeLen5 > 0) {
                                    tmpNumAreaCode8 = null;
                                    tmpNumFormat6 = tmpNumFormat5;
                                    tmpNumPrefix8 = tmpNum9.substring(0, countryCodeLen5);
                                    tmpNum9 = tmpNum9.substring(countryCodeLen5);
                                    StringBuilder sb3 = new StringBuilder();
                                    countryCodeLen3 = countryCodeLen5;
                                    sb3.append("11: tmpNum after remove prefix: ");
                                    sb3.append(tmpNum9);
                                    sb3.append(", tmpNum.length11: ");
                                    sb3.append(tmpNum9.length());
                                ...
                                if (isEqualCountryCodePrefix(compNumPrefix, countryIso, tmpNumPrefix8, null)) {
                                    if (isCnNumber) {
                                        String tmpNum12 = deleteIPHead(tmpNum9);
                                        boolean isTmpNumCnMPN = isChineseMobilePhoneNumber(tmpNum12);
                                        if ((!isCompNumCnMPN || isTmpNumCnMPN) && (isCompNumCnMPN || !isTmpNumCnMPN)) {
                                            if (!isCompNumCnMPN || !isTmpNumCnMPN) {
                                                int areaCodeLen2 = getChineseFixNumberAreaCodeLength(tmpNum12);
                                                if (areaCodeLen2 > 0) {
                                                    tmpNumPrefix9 = tmpNumPrefix8;
                                                    String tmpNumAreaCode12 = tmpNum12.substring(0, areaCodeLen2);
                                                    tmpNum12 = tmpNum12.substring(areaCodeLen2);
                                                    logd("11: CN tmpNum after remove area code: " + tmpNum12 + ", tmpNum.length11: " + tmpNum12.length() + ", tmpNumAreaCode: " + tmpNumAreaCode12);
                                        ...
                                        if (tmpNum9.length() >= 7) {
                                            tmpNumAreaCode9 = str2;
                                            if (tmpNumAreaCode9.equals(tmpNum9.substring(0, 1))) {
                                                compNumAreaCode5 = compNumAreaCode5;
                                                if (!tmpNumAreaCode9.equals(tmpNum9.substring(1, 2))) {
                                                    tmpNum9 = tmpNum9.substring(1);
                                    ...
                                    int tmpNumLen = tmpNum9.length();
                                    if (tmpNumLen >= NUM_LONG7) {
                                        String tmpNumLong = tmpNum9.substring(tmpNumLen - NUM_LONG7);
                                        NUM_LONG7 = NUM_LONG7;
                                        ...
                                        if (tmpNumLen >= NUM_SHORT2) {
                                            String tmpNumShort4 = tmpNum9.substring(tmpNumLen - NUM_SHORT2);
                                            if (-1 == numShortID2 && compNumShort4.compareTo(tmpNumShort4) == 0) {
                                                numShortID2 = cursor.getPosition();
                                            }
                                            logd("11: >=NUM_SHORT, tmpNumShort = " + tmpNumShort4 + ", numShortID:" + numShortID2);
                                ...
                                formatColumnIndex3 = formatColumnIndex3;
                        ...
                        tmpCompNum = tmpCompNum3;
                        compNum3 = origTmpNum2;
                        formatColumnIndex = compNumLen;
                    ...
                    if (compNumLen >= NUM_SHORT2) {
                        String compNumShort5 = compNum8.substring(compNumLen - NUM_SHORT2);
                        ...
                        if (cursor.moveToFirst()) {
                            ...
                            int formatColumnIndex4 = cursor.getColumnIndex("normalized_number");
                            ...
                            if (columnIndex2 != -1) {
                                ...
                                int numLongID4 = -1;
                                while (true) {
                                    String tmpNum13 = cursor.getString(columnIndex2);
                                    if (tmpNum13 == null) {
                                        return -1;
                                    }
                                    if (tmpNum13.indexOf(64) >= 0) {
                                        return -1;
                                    }
                                    origTmpNum4 = PhoneNumberUtils.stripSeparators(tmpNum13);
                                    logd("origTmpNum: " + origTmpNum4);
                                    if (-1 != formatColumnIndex4) {
                                        tmpNumFormat2 = cursor.getString(formatColumnIndex4);
                                        tmpNum6 = isValidData4Number(origTmpNum4, tmpNumFormat2) ? tmpNumFormat2 : origTmpNum4;
                                    } else if (-1 != data4ColumnIndex3) {
                                        tmpNumFormat2 = cursor.getString(data4ColumnIndex3);
                                        tmpNum6 = isValidData4Number(origTmpNum4, tmpNumFormat2) ? tmpNumFormat2 : origTmpNum4;
                                    } else {
                                        tmpNum6 = origTmpNum4;
                                        tmpNumFormat2 = tmpNumFormat6;
                                    }
                                    logd("7: tmpNumFormat: " + tmpNumFormat2);
                                    tmpNum6.length();
                                    logd("7: tmpNum = " + tmpNum6 + ", tmpNum.length7: " + tmpNum6.length() + ",ID = " + cursor.getPosition());
                                    if (tmpNum6.equals(tmpCompNum3)) {
                                        numShortID = cursor.getPosition();
                                        logd("7: >= NUM_SHORT numShortID = " + numShortID + ", formattedNum full match!");
                                        tmpNumFormat6 = tmpNumFormat2;
                                        compNumShort3 = compNumShort6;
                                        NUM_LONG2 = NUM_SHORT3;
                                        break;
                                    }
                                    int countryCodeLen6 = getIntlPrefixAndCCLen(tmpNum6);
                                    if (countryCodeLen6 > 0) {
                                        tmpNumAreaCode5 = null;
                                        tmpNumFormat3 = tmpNumFormat2;
                                        tmpNumPrefix5 = tmpNum6.substring(0, countryCodeLen6);
                                        tmpNum6 = tmpNum6.substring(countryCodeLen6);
                                        logd("7: tmpNum after remove prefix: " + tmpNum6 + ", tmpNum.length7: " + tmpNum6.length() + ", tmpNumPrefix: " + tmpNumPrefix5);
                                    } else {
                                        tmpNumAreaCode5 = null;
                                        tmpNumFormat3 = tmpNumFormat2;
                                        tmpNumPrefix5 = null;
                                    ...
                                    if (isEqualCountryCodePrefix(compNumPrefix, str, tmpNumPrefix5, null)) {
                                        if (isCnNumber) {
                                            String tmpNum14 = deleteIPHead(tmpNum6);
                                            boolean isTmpNumCnMPN2 = isChineseMobilePhoneNumber(tmpNum14);
                                            if ((!isCompNumCnMPN || isTmpNumCnMPN2) && (isCompNumCnMPN || !isTmpNumCnMPN2)) {
                                                if (!isCompNumCnMPN || !isTmpNumCnMPN2) {
                                                    int areaCodeLen3 = getChineseFixNumberAreaCodeLength(tmpNum14);
                                                    if (areaCodeLen3 > 0) {
                                                        tmpNumPrefix6 = tmpNumPrefix5;
                                                        String tmpNumAreaCode13 = tmpNum14.substring(0, areaCodeLen3);
                                                        tmpNum14 = tmpNum14.substring(areaCodeLen3);
                                                        logd("7: CN tmpNum after remove area code: " + tmpNum14 + ", tmpNum.length7: " + tmpNum14.length() + ", tmpNumAreaCode: " + tmpNumAreaCode13);
                                            ...
                                            tmpNumPrefix6 = tmpNumPrefix5;
                                            compNumAreaCode2 = compNumAreaCode5;
                                            if (tmpNum6.length() >= 7 && str3.equals(tmpNum6.substring(0, 1)) && !str3.equals(tmpNum6.substring(1, 2))) {
                                                tmpNum6 = tmpNum6.substring(1);
                                                logd("7: tmpNum remove 0 at beginning");
                                            }
                                        }
                                        int tmpNumLen2 = tmpNum6.length();
                                        if (tmpNumLen2 >= NUM_SHORT6) {
                                            String tmpNumShort5 = tmpNum6.substring(tmpNumLen2 - NUM_SHORT3);
                                            if (-1 == numLongID4) {
                                                compNumShort3 = compNumShort6;
                                                if (compNumShort3.compareTo(tmpNumShort5) == 0) {
                                            ...
                                            sb7.append(tmpNumShort5);
                                            sb7.append(", numLongID:");
                                            sb7.append(numLongID4);
                                            logd(sb7.toString());
                                            tmpNumShort3 = tmpNumShort5;
                                            tmpNum7 = tmpNum6;
                                            NUM_LONG2 = NUM_SHORT3;
                                        } else {
                                            compNumAreaCode5 = compNumAreaCode2;
                                            ...
                                            if (tmpNumLen2 >= NUM_LONG2) {
                                                String tmpNumShort6 = tmpNum6.substring(tmpNumLen2 - NUM_LONG2);
                                                if (-1 == numShortID3 && compNumShort3.compareTo(tmpNumShort6) == 0) {
                                                    int numShortID4 = cursor.getPosition();
                                                    logd("7: >= NUM_SHORT numShortID = " + numShortID4);
                                                    tmpNumShort3 = tmpNumShort6;
                                                    numShortID3 = numShortID4;
                                                    tmpNum7 = tmpNum6;
                                                } else {
                                                    logd("7: >=NUM_SHORT, and !=, tmpNumShort = " + tmpNumShort6 + ", numShortID:" + numShortID3);
                                                    tmpNumShort3 = tmpNumShort6;
                                                    tmpNum7 = tmpNum6;
                                                }
                                            } else {
                                    ...
                                    formatColumnIndex4 = formatColumnIndex4;
                        ...
                        if (cursor.moveToFirst()) {
                            ...
                            if (columnIndex3 != -1) {
                                ...
                                while (true) {
                                    String tmpNum15 = cursor.getString(columnIndex3);
                                    if (tmpNum15 != null && tmpNum15.indexOf(64) < 0) {
                                        String origTmpNum7 = PhoneNumberUtils.stripSeparators(tmpNum15);
                                        ...
                                        if (isEqualCountryCodePrefix(compNumPrefix, str, tmpNumPrefix2, null)) {
                                            if (isCnNumber) {
                                                String tmpNum16 = deleteIPHead(tmpNum);
                                                boolean isTmpNumCnMPN3 = isChineseMobilePhoneNumber(tmpNum16);
                                                if ((!isCompNumCnMPN || isTmpNumCnMPN3) && (isCompNumCnMPN || !isTmpNumCnMPN3)) {
                                                    if (!isCompNumCnMPN || !isTmpNumCnMPN3) {
                                                        int areaCodeLen4 = getChineseFixNumberAreaCodeLength(tmpNum16);
                                                        if (areaCodeLen4 > 0) {
                                                            tmpNumPrefix3 = tmpNumPrefix2;
                                                            String tmpNumAreaCode14 = tmpNum16.substring(0, areaCodeLen4);
                                                            tmpNum16 = tmpNum16.substring(areaCodeLen4);
                                                            StringBuilder sb8 = new StringBuilder();
                                                            tmpCompNum = tmpCompNum3;
                                                            sb8.append("5: CN tmpNum after remove area code: ");
                                                            sb8.append(tmpNum16);
                                                            sb8.append(", tmpNum.length5: ");
                                                            sb8.append(tmpNum16.length());
                                            ...
                                            formatColumnIndex = compNumLen2;
                                            if (tmpNum.length() != formatColumnIndex) {
                                                compNum3 = compNum8;
                                                compNum5 = str3;
                                                logd("5: continue");
                                            } else if (-1 == fixedIndex5) {
                                                compNum3 = compNum8;
                                                if (compNum3.compareTo(tmpNum) == 0) {
                                        ...
                                        compNumLen2 = formatColumnIndex;
                                        tmpNumFormat6 = tmpNumFormat;
                                        compNumLen3 = compNumLen3;
                                        columnIndex3 = columnIndex3;
                                        str3 = compNum5;
                                        tmpCompNum3 = tmpCompNum;
                                        compNum8 = compNum3;
                                ...
                                tmpCompNum2 = tmpCompNum3;
                                compNum4 = compNum8;
                                formatColumnIndex2 = compNumLen2;
                            }
                        } else {
                            tmpCompNum2 = tmpCompNum3;
                            NUM_SHORT3 = NUM_SHORT2;
                ...
                if (shouldDoNumberMatchAgainBySimMccmnc(compNum6, str)) {
                    return getCallerIndexInternal(cursor, compNum6, columnName, this.mSimNumLong, this.mSimNumShort);
                }
                return NUM_SHORT4;
            }
        }
    ...
    private int getCallerIndexInternal(Cursor cursor, String compNum, String columnName, int numLong, int numShort) {
        ...
        String tmpNumLong4 = null;
        String tmpNumLong5 = null;
        int numShortID = -1;
        int numShortID2 = -1;
        int numLongID = -1;
        int numLongID2 = -1;
        ...
        if (TextUtils.isEmpty(compNum)) {
            ...
            Log.e(TAG, "getCallerIndexInternal(),null == compNum! fixedIndex = " + fixedIndex3);
            return fixedIndex3;
        }
        ...
        if (cursor != null) {
                ...
                if (cursor.moveToFirst()) {
                    ...
                    if (columnIndex != -1) {
                        while (true) {
                            ...
                            tmpNumLong2 = tmpNumLong5;
                            logd("11: tmpNum = " + tmpNum + ", tmpNum.length11: " + tmpNum.length() + ", ID = " + cursor.getPosition());
                            if (compNum.equals(tmpNum)) {
                                int numLongID3 = cursor.getPosition();
                                logd("exact match: break! numLongID = " + numLongID3);
                                numLongID2 = numLongID3;
                                break;
                            }
                            if (tmpNumLen >= NUM_LONG) {
                                tmpNumLong5 = tmpNum.substring(tmpNumLen - NUM_LONG);
                                if (-1 == numLongID2 && compNumLong2.compareTo(tmpNumLong5) == 0) {
                                    numLongID2 = cursor.getPosition();
                                    StringBuilder sb2 = new StringBuilder();
                                    compNumLong = compNumLong2;
                                    sb2.append("11: > NUM_LONG numLongID = ");
                                    sb2.append(numLongID2);
                                    logd(sb2.toString());
                                } else {
                                    compNumLong = compNumLong2;
                                }
                                logd("11: >= NUM_LONG, and !=,  tmpNumLong = " + tmpNumLong5 + ", numLongID: " + numLongID2);
                            } else {
                                compNumLong = compNumLong2;
                                if (tmpNumLen >= NUM_SHORT) {
                                    String tmpNumShort3 = tmpNum.substring(tmpNumLen - NUM_SHORT);
                                    if (-1 == numShortID2 && compNumShort2.compareTo(tmpNumShort3) == 0) {
                                        numShortID2 = cursor.getPosition();
                                    }
                                    logd("11: >= NUM_SHORT, tmpNumShort = " + tmpNumShort3 + ", numShortID:" + numShortID2);
                                    tmpNumShort2 = tmpNumShort3;
                                    tmpNumLong5 = tmpNumLong2;
                                } else {
                                    logd("tmpNum11, continue");
                                    tmpNumLong5 = tmpNumLong2;
                                }
                            ...
                            columnIndex = columnIndex;
                            fixedIndex4 = fixedIndex4;
                            compNumLong2 = compNumLong;
                        }
                        logd("11: numLongID = " + numLongID2 + ", numShortID = " + numShortID2);
                        if (-1 != numLongID2) {
                            fixedIndex5 = numLongID2;
                            tmpNumLong4 = tmpNumLong2;
                            tmpNumShort = tmpNumShort2;
                        } else if (-1 != numShortID2) {
                            fixedIndex5 = numShortID2;
                            tmpNumLong4 = tmpNumLong2;
                            tmpNumShort = tmpNumShort2;
                        } else {
                            fixedIndex5 = -1;
                            ...
                            while (true) {
                                ...
                                if (tmpNum3 != null && tmpNum3.indexOf(64) < 0) {
                                    ...
                                    if (compNum.equals(tmpNum4)) {
                                        int numShortID3 = cursor.getPosition();
                                        logd("exact match numShortID = " + numShortID3);
                                        numShortID = numShortID3;
                                        break;
                                    }
                        ...
                        while (true) {
                            ...
                            if (tmpNum7 != null && tmpNum7.indexOf(64) < 0) {
                                ...
                                if (tmpNumLen3 == compNumLen) {
                                    fixedIndex = fixedIndex6;
                                    if (-1 == fixedIndex && compNum.compareTo(tmpNum8) == 0) {
                                        int fixedIndex8 = cursor.getPosition();
                                        logd("5: break! numLongID = " + fixedIndex8);
                                        fixedIndex = fixedIndex8;
                                        break;
                                    }
                                } else {
                                    fixedIndex = fixedIndex6;
                                ...
                                fixedIndex6 = fixedIndex;
                            }
                        }
                        logd("5: fixedIndex = " + fixedIndex5);
                    } else {
                        fixedIndex2 = -1;
                        compNumShort = str;
                    }
            ...
            logd("getCallerIndexInternal, fixedIndex: " + fixedIndex5);
            return fixedIndex5;
        }
        compNumShort = null;
        logd("getCallerIndexInternal, fixedIndex: " + fixedIndex5);

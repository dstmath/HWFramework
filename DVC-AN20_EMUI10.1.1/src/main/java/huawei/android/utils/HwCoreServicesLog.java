...
public class HwCoreServicesLog {
    ...
    public static boolean handleLogRequest(String[] args) {
        ...
        if (args.length < 1 || (debug_flow = scanArg(args[0], DEBUG_FLOW.values())) == null) {
            return false;
        }
        int i = AnonymousClass1.$SwitchMap$huawei$android$utils$HwCoreServicesLog$DEBUG_FLOW[debug_flow.ordinal()];
        if (i == 1) {

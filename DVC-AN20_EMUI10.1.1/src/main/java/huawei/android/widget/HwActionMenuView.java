...
public class HwActionMenuView extends ActionMenuView implements HwSmartColorListener {
    ...
    private void fillItemSpec(ItemSpec[] items, int cellSize, int[] standardIconPositions) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            items[i].mIsTooLong = items[i].mMeasuredSize > cellSize;
        }
    }
    ...
    private void adjustItemSize(ItemSpec[] items) {
        ...
        for (int i = 0; i < childCount; i++) {
            if (getChildAt(i) instanceof TextView) {
                ...
                if (child.getVisibility() == 8) {
                    gone++;
                } else if (items[index].mIsTooLong) {
    ...
    private void adjustSizeFromSiblings(TextView child, ItemSpec[] items, int index) {
        if (index == 0 && items.length > 1) {
            ...
            if (!items[index - 1].mIsTooLong && items[index + 1].mIsTooLong) {
                adjustSize(child, items, index, -1);
    ...
    private void adjustSize(TextView child, ItemSpec[] items, int index, int diff) {
        ...
        if (!items[brother].mIsTooLong) {
            int needMeasuredSize = items[index].mMeasuredSize;
            boolean isStillTooLong = false;
            ...
            if (needDiff < overflowDiff) {
                ...
                boolean z = true;
                if (items[index].mUsingSmallFont == -1) {
                    child.setTypeface(this.mCondensed);
    ...
    private void adjustSize(TextView child, ItemSpec[] items, int index, int diff1, int diff2) {
        int brother1 = index + diff1;
        int brother2 = index + diff2;
        int overflowDiff1 = items[brother1].mItemSize - items[brother1].mMeasuredSize;
        int overflowDiff2 = items[brother2].mItemSize - items[brother2].mMeasuredSize;
        int min = overflowDiff1 > overflowDiff2 ? brother2 : brother1;
        int minOverflowDiff = overflowDiff1 > overflowDiff2 ? overflowDiff2 : overflowDiff1;
        int max = overflowDiff1 > overflowDiff2 ? brother2 : brother1;
        int maxOverflowDiff = overflowDiff1 > overflowDiff2 ? overflowDiff1 : overflowDiff2;
        int needMeasuredSize = items[index].mMeasuredSize;
        boolean isStillTooLong = false;
        if (items[index].mMeasuredSize > items[index].mMaxSize && items[index].mMaxSize > 0) {
            needMeasuredSize = items[index].mMaxSize;
    ...
    private int getItemMaxSize(int index, int visibleItemCount) {
        if (visibleItemCount == 3) {
            return this.mToolBarMenuItemMaxSize3Arrays[index];
        }
        if (visibleItemCount == 4) {
            return this.mToolBarMenuItemMaxSize4Arrays[index];
        }
        if (visibleItemCount == 5) {
            return this.mToolBarMenuItemMaxSize5Arrays[index];
        }
        return 0;
    }

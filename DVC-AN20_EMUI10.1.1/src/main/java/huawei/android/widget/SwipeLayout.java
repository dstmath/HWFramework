...
public class SwipeLayout extends FrameLayout {
    ...
    public SwipeLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        this.mDragHelperCallback = new ViewDragHelper.Callback() {
            ...
            public int clampViewPositionHorizontal(View child, int left, int dx) {
                if (child == SwipeLayout.this.getSurfaceView()) {
                    ...
                    if (i2 != 3) {
                        if (i2 == 4 && SwipeLayout.this.mShowMode == ShowMode.PullOut && left < SwipeLayout.this.getMeasuredWidth() - SwipeLayout.this.mDragDistance) {
        ...
        this.mEdgeSwipesOffset[DragEdge.Left.getId()] = a.getDimension(1, 0.0f);
        this.mEdgeSwipesOffset[DragEdge.Right.getId()] = a.getDimension(2, 0.0f);
        this.mEdgeSwipesOffset[DragEdge.Top.getId()] = a.getDimension(3, 0.0f);
        this.mEdgeSwipesOffset[DragEdge.Bottom.getId()] = a.getDimension(4, 0.0f);
        setClickToClose(a.getBoolean(6, this.mIsClickToClose));
        setDragThenClose(a.getBoolean(7, this.mIsDragThenClose));
        ...
        this.mShowMode = ShowMode.values()[a.getInt(5, ShowMode.PullOut.getId())];
        a.recycle();
    }
    ...
    /* JADX WARNING: Removed duplicated region for block: B:47:0x0092 A[RETURN] */
        ...
        if (getShowMode() == ShowMode.LayDown) {
            ...
            if (i != 1) {
                if (i != 2) {
                    ...
                    if (i == 4 && surfaceRight > childLeft && surfaceRight <= childRight) {
                        return true;
                    }
                } else if (surfaceBottom > childTop && surfaceBottom <= childBottom) {
                    return true;
                }
            } else if (surfaceTop >= childTop && surfaceTop < childBottom) {
                return true;
            }
        } else if (getShowMode() != ShowMode.PullOut) {
            return false;
            ...
            if (i2 != 1) {
                if (i2 != 2) {
                    ...
                    if (i2 == 4 && childLeft <= getWidth() && childRight > getWidth()) {
                        return true;
                    }
                } else if (childTop < getHeight() && childTop >= getPaddingTop()) {
                    return true;
                }
            } else if (childTop < getPaddingTop() && childBottom >= getPaddingTop()) {
                return true;
            }
        }
    }
    ...
    public Rect getRelativePosition(View child) {
        ...
        while (curView.getParent() != null && curView != getRootView() && (curView = (View) curView.getParent()) != this) {
            ...
            for (Map.Entry<View, ArrayList<OnRevealListener>> entry : this.mRevealListeners.entrySet()) {
                ...
                if (isViewShowing(child, rect, this.mCurrentDragEdge, surfaceLeft, surfaceTop, surfaceRight, surfaceBottom)) {
                    ...
                    if (getShowMode() == ShowMode.LayDown) {
                        ...
                        int i2 = AnonymousClass2.$SwitchMap$huawei$android$widget$SwipeLayout$DragEdge[this.mCurrentDragEdge.ordinal()];
                        if (i2 == 1) {
                ...
                if (isViewTotallyFirstShowed(child, rect, this.mCurrentDragEdge, surfaceLeft, surfaceTop, surfaceRight, surfaceBottom)) {
                    ...
                    while (it2.hasNext()) {
                        OnRevealListener listener = it2.next();
    ...
    private void checkCanDrag(MotionEvent event) {
        ...
        if (!this.mIsBeingDragged) {
            ...
            checkDrag(status, distanceX, distanceY, angle);
        }
    }
    ...
    public void setClickToClose(boolean isClicktoclose) {
        this.mIsClickToClose = isClicktoclose;
    }

    public void setDragThenClose(boolean isDragThenClose) {
        this.mIsDragThenClose = isDragThenClose;
    }

    public void setSwipeEnabled(boolean isEnable) {
        this.mIsSwipeEnabled = isEnable;
    }
    ...
    private boolean performAdapterViewItemLongClick() {
        ...
        try {
            ...
            return ((Boolean) performLongPress.invoke(view, this, Integer.valueOf(position), Long.valueOf(itemIdAtPosition))).booleanValue();

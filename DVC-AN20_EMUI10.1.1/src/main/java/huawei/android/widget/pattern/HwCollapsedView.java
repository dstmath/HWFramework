...
public class HwCollapsedView extends TextView {
    ...
    private void formatCollapsedText(TextView.BufferType type, CharSequence text) {
        ...
        int expandedTextWidth = (int) ((((paint.measureText(ELLIPSE) + ((float) (getExpandState() ? this.mExpandedDrawable : this.mCollapsedDrawable).getIntrinsicWidth())) + 0.5f) + lastLineWidth) - textWidth);
        if (lastLineWidth + ((float) expandedTextWidth) > ((float) this.mShowWidth)) {
            float[] measureWidths = new float[1];
            charSequence = ELLIPSE;
            int cutCount = paint.breakText(this.mOriginalText, lastLineStart, lastLineEnd, false, (float) expandedTextWidth, measureWidths);

...
public class HwWidgetUtils {
    ...
    public static ClickEffectEntry getCleckEffectEntry(Context context, int defStyleAttr) {
        ...
        if (theme != null) {
            TypedArray a = theme.obtainStyledAttributes(null, identifiers, defStyleAttr, 0);
            clickEffectEntry.setColor(a.getColor(resLoader.getIdentifier(context, RESOURCE_TYPE_STYLEABLE, "Theme_clickEffectColor"), clickEffectEntry.getColor()));
            clickEffectEntry.setAlpha(a.getFloat(resLoader.getIdentifier(context, RESOURCE_TYPE_STYLEABLE, "Theme_clickEffectAlpha"), clickEffectEntry.getAlpha()));
            clickEffectEntry.setMinRecScale(a.getFloat(resLoader.getIdentifier(context, RESOURCE_TYPE_STYLEABLE, "Theme_clickEffectMinRecScale"), clickEffectEntry.getMinRecScale()));
            clickEffectEntry.setMaxRecScale(a.getFloat(resLoader.getIdentifier(context, RESOURCE_TYPE_STYLEABLE, "Theme_clickEffectMaxRecScale"), clickEffectEntry.getMaxRecScale()));
            clickEffectEntry.setCornerRadius(a.getFloat(resLoader.getIdentifier(context, RESOURCE_TYPE_STYLEABLE, "Theme_clickEffectCornerRadius"), clickEffectEntry.getCornerRadius()));
            clickEffectEntry.setIsForceDoScaleAnim(a.getBoolean(resLoader.getIdentifier(context, RESOURCE_TYPE_STYLEABLE, "Theme_clickEffectForceDoScaleAnim"), clickEffectEntry.isForceDoScaleAnim()));
            a.recycle();
        }

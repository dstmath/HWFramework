...
public class PressGestureDetector implements IPressGestureDetector {
    ...
    public void onAttached(final int windowType) {
        new Thread() {
            /* class huawei.com.android.internal.policy.PressGestureDetector.AnonymousClass1 */

            public void run() {
                PressGestureDetector pressGestureDetector = PressGestureDetector.this;
                pressGestureDetector.mHiTouchRestricted = pressGestureDetector.mSensor.isUnsupportScence(windowType);
    ...
    public boolean dispatchTouchEvent(MotionEvent ev, boolean isHandling) {
        ...
        int actionMasked = ev.getAction() & 255;
        if (actionMasked == 0) {
                ...
                if (ev.getPointerCount() > 2) {
                    ...
                    if (intervalTwoFingers > TOUCH_TWO_FINGERS_TIME_OUT_LIMIT) {
                        ...
                        if (!checkOnePointPosition(this.mLongPressDownX, this.mLongPressDownY) || !checkOnePointPosition(this.mLongPressPointerDownX, this.mLongPressPointerDownY)) {
                            this.mGestureInterrupted = true;
                        } else if (checkPointersTooClose(this.mLongPressDownX, this.mLongPressDownY, this.mLongPressPointerDownX, this.mLongPressPointerDownY)) {
                            this.mGestureInterrupted = true;
                        } else if (!checkIfPointsOutOfDecorView(this.mLongPressDownX, this.mLongPressDownY, this.mLongPressPointerDownX, this.mLongPressPointerDownY)) {
                            startCountingTime();
                        }
                    }
                }
        ...
        } else if (!this.mHasTriggered && !this.isMoveTooMuch) {
    ...
    private void startCountingTime() {
        ...
        this.mHandler.postDelayed(new Runnable() {
            /* class huawei.com.android.internal.policy.PressGestureDetector.AnonymousClass2 */

            public void run() {
                PressGestureDetector pressGestureDetector = PressGestureDetector.this;
                pressGestureDetector.triggerLaunchHiTouch(pressGestureDetector.mLongPressDownX, PressGestureDetector.this.mLongPressDownY, PressGestureDetector.this.mLongPressPointerDownX, PressGestureDetector.this.mLongPressPointerDownY);
            }
        }, actualTriggerTime);
    }
    ...
    private boolean triggerLaunchHiTouch(float x1, float y1, float x2, float y2) {
    ...
    private void updateDisplayParameters() {
        ...
        if (!SystemProperties.get("ro.config.hw_notch_size", BuildConfig.FLAVOR).trim().isEmpty()) {
            int notchHeight = this.mContextActivity.getResources().getDimensionPixelSize(17105443);

...
public class HiEvent {
    ...
    public static final int MAX_KEY_LEN = 32;
    public static final int MAX_PAIR_NUM = 256;
    ...
    public HiEvent putString(String key, String value) {
        if (checkValid(key, 32) && checkValid(value, 128)) {
            this.mPayload.put(key, value);
        }
        return this;
    }
    ...
    public String flatten() {
        if (this.mStatus != Status.Flattened || this.mFlattened == null) {
            ...
            this.mStatus = Status.Flattened;
            Log.i(TAG, "Flatten done:" + this.mFlattened);
    ...
    public boolean setTraceInfo() {
        ...
        if (traceId == null || !traceId.isValid()) {
            Log.i(TAG, "trace is not open or not valid");
            return false;
        }

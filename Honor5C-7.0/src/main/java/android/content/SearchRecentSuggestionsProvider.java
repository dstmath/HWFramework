...
public class SearchRecentSuggestionsProvider extends ContentProvider {
    ...
    protected void setupSuggestions(String authority, int mode) {
        ...
        boolean z;
        if ((mode & DATABASE_MODE_2LINES) != 0) {
            z = true;
        } else {
            z = false;
        }
        ...
        this.mUriMatcher.addURI(this.mAuthority, SearchManager.SUGGEST_URI_PATH_QUERY, URI_MATCH_SUGGEST);
        if (this.mTwoLineDisplay) {
            this.mSuggestSuggestionClause = "display1 LIKE ? OR display2 LIKE ?";
            this.mSuggestionProjection = new String[]{"0 AS suggest_format", "'android.resource://system/17301578' AS suggest_icon_1", "display1 AS suggest_text_1", "display2 AS suggest_text_2", "query AS suggest_intent_query", HwUserData._ID};
            return;
        }
        this.mSuggestSuggestionClause = "display1 LIKE ?";
        this.mSuggestionProjection = new String[]{"0 AS suggest_format", "'android.resource://system/17301578' AS suggest_icon_1", "display1 AS suggest_text_1", "query AS suggest_intent_query", HwUserData._ID};
    }
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
        ...
        if (this.mUriMatcher.match(uri) == URI_MATCH_SUGGEST) {
            ...
            if (TextUtils.isEmpty(selectionArgs[0])) {
                ...
                if (this.mTwoLineDisplay) {
                    strArr = new String[DATABASE_MODE_2LINES];
                    strArr[0] = like;
                    strArr[URI_MATCH_SUGGEST] = like;
                } else {
                    strArr = new String[URI_MATCH_SUGGEST];
                    strArr[0] = like;
                }
        ...
        if (length == URI_MATCH_SUGGEST || length == DATABASE_MODE_2LINES) {
            ...
            if (base.equals(sSuggestions)) {
                ...
                if (projection != null && projection.length > 0) {
                    strArr2 = new String[(projection.length + URI_MATCH_SUGGEST)];
                    System.arraycopy(projection, 0, strArr2, 0, projection.length);
                    strArr2[projection.length] = "_id AS _id";
                }
                StringBuilder stringBuilder = new StringBuilder(TriangleMeshBuilder.TEXTURE_0);
                if (length == DATABASE_MODE_2LINES) {

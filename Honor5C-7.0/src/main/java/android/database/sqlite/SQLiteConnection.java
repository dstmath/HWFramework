...
public final class SQLiteConnection implements OnCancelListener {
    ...
    private static final class OperationLog {
        ...
        private boolean endOperationDeferLogLocked(int cookie) {
            boolean z = SQLiteConnection.DEBUG;
            Operation operation = getOperationLocked(cookie);
            if (operation == null) {
...
/*
        Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: android.database.sqlite.SQLiteConnection.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: android.database.sqlite.SQLiteConnection.<clinit>():void
...
*/
    ...
    private SQLiteConnection(SQLiteConnectionPool pool, SQLiteDatabaseConfiguration configuration, int connectionId, boolean primaryConnection) {
        this(pool, configuration, connectionId, primaryConnection, DEBUG);
    }
    ...
    void reconfigure(SQLiteDatabaseConfiguration configuration) {
        ...
        boolean foreignKeyModeChanged = configuration.foreignKeyConstraintsEnabled != this.mConfiguration.foreignKeyConstraintsEnabled ? true : DEBUG;
        boolean walModeChanged = ((configuration.openFlags ^ this.mConfiguration.openFlags) & KeymasterDefs.KM_ENUM_REP) != 0 ? true : DEBUG;
        boolean localeChanged = configuration.locale.equals(this.mConfiguration.locale) ? DEBUG : true;
    ...
    private void detachCancellationSignal(CancellationSignal cancellationSignal) {
        if (cancellationSignal != null) {
            if (!-assertionsDisabled) {
                if (!(this.mCancellationSignalAttachCount > 0 ? true : DEBUG)) {
    ...
    static SQLiteConnection openExclusive(SQLiteConnectionPool pool, SQLiteDatabaseConfiguration configuration, int connectionId) {
        SQLiteConnection connection = new SQLiteConnection(pool, configuration, connectionId, DEBUG, true);
        try {
            connection.open();
            return connection;
        } catch (SQLiteException ex) {

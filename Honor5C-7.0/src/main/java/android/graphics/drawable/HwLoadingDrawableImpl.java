...
public class HwLoadingDrawableImpl extends HwLoadingDrawable implements Animatable {
    ...
    private void init(Resources res, int color) {
        ...
        this.mBallDrawable = (BitmapDrawable) res.getDrawable(33751409, null);
        this.mBallBitmap = this.mBallDrawable.getBitmap();
        this.mRingRadius = ((float) Math.min(this.mWidth, this.mHeight)) * HwFragmentLayout.DISPLAY_RATE_FORTY_PERCENT;
        this.mAnimator = ObjectAnimator.ofFloat(this, "progress", new float[]{0.0f, HwFragmentMenuItemView.ALPHA_NORMAL});
        this.mAnimator.setDuration(1300);
        this.mAnimator.setRepeatCount(-1);
        this.mAnimator.setInterpolator(new LinearInterpolator());
        this.mBalls = new Ball[NUM_OF_POINTS];
        for (int i = 0; i < NUM_OF_POINTS; i++) {
            this.mBalls[i] = new Ball(0, 0, (int) (((float) (((5 - i) + 1) * this.mWidth)) * BALL_SIZE_FACTOR));
        }
        this.mPaint = new Paint(1);
        this.mFilter = new ColorMatrixColorFilter(createThresholdMatrix(THRESHOLD, color));
        setProgress(0.0f);
    }
    ...
    private void updatePointsPosition(float progress) {
        ...
        float step = 0.05f * (3.0f * (HwFragmentMenuItemView.ALPHA_PRESSED - Math.abs(HwFragmentMenuItemView.ALPHA_PRESSED - progress)));
        int centerX = this.mWidth / 2;
        int centerY = this.mHeight / 2;
        for (int i = 0; i < NUM_OF_POINTS; i++) {
            if (percent < 0.0f) {
                percent += HwFragmentMenuItemView.ALPHA_NORMAL;
            }
    ...
    public static ColorMatrix createThresholdMatrix(int threshold, int color) {
        float gradient;
        if (threshold == PduHeaders.STORE_STATUS_ERROR_END || threshold <= 0) {
            gradient = 255.0f;
        } else {

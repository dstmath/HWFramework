...
public class NinePatchDrawable extends Drawable {
    ...
    public void draw(Canvas canvas) {
        ...
        if (state.mBaseAlpha != Engine.DEFAULT_VOLUME) {
            ...
            this.mPaint.setAlpha((int) ((((float) restoreAlpha) * state.mBaseAlpha) + NetworkHistoryUtils.RECOVERY_PERCENTAGE));
        } else {
            restoreAlpha = -1;
        }
        ...
        if (needsMirroring()) {
            ...
            canvas.scale(ScaledLayoutParams.SCALE_UNSPECIFIED, Engine.DEFAULT_VOLUME, ((float) (bounds.left + bounds.right)) / 2.0f, ((float) (bounds.top + bounds.bottom)) / 2.0f);
        }
    ...
    public void setAlpha(int alpha) {
        if (this.mPaint != null || alpha != Process.PROC_TERM_MASK) {
    ...
    public int getOpacity() {
        return (this.mNinePatchState.mNinePatch.hasAlpha() || (this.mPaint != null && this.mPaint.getAlpha() < Process.PROC_TERM_MASK)) ? -3 : -1;
    }

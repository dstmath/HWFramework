...
public abstract class CameraMetadata<TKey> {
    ...
    static <TKey> ArrayList<TKey> getKeysStatic(Class<?> type, Class<TKey> keyClass, CameraMetadata<TKey> instance, int[] filterTags) {
        ...
        for (int i = TONEMAP_MODE_CONTRAST_CURVE; i < length; i += TONEMAP_PRESET_CURVE_REC709) {
            Field field = fields[i];
            if (field.getType().isAssignableFrom(keyClass) && (field.getModifiers() & TONEMAP_PRESET_CURVE_REC709) != 0) {
                try {

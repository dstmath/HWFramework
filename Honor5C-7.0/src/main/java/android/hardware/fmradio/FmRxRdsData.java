...
public class FmRxRdsData {
    ...
    public int rdsOn(boolean on) {
        ...
        if (on) {
            return FmReceiverJNI.setControlNative(this.mFd, V4L2_CID_PRIVATE_TAVARUA_RDSON, RDS_GROUP_RT);
        }
    ...
    public int rdsGrpOptions(int grpMask, int buffSize, boolean rdsFilter) {
        int rdsFilt;
        byte rds_group_mask = (byte) (((byte) FmReceiverJNI.getControlNative(this.mFd, V4L2_CID_PRIVATE_TAVARUA_RDSGROUP_PROC)) & UsbConstants.USB_CLASS_APP_SPEC);
        if (rdsFilter) {
            rdsFilt = RDS_GROUP_RT;
        } else {
            rdsFilt = 0;
        }
    ...
    public int rdsOptions(int rdsMask) {
        byte rds_group_mask = (byte) FmReceiverJNI.getControlNative(this.mFd, V4L2_CID_PRIVATE_TAVARUA_RDSGROUP_PROC);
        int psAllVal = rdsMask & RDS_PS_ALL;
        Log.d(LOGTAG, "In rdsOptions: rdsMask: " + rdsMask);
        int re = FmReceiverJNI.setControlNative(this.mFd, V4L2_CID_PRIVATE_TAVARUA_RDSGROUP_PROC, (byte) (((rdsMask & 7) << 3) | ((byte) (rds_group_mask & Impl.STATUS_DEVICE_NOT_FOUND_ERROR))));
        return FmReceiverJNI.setControlNative(this.mFd, V4L2_CID_PRIVATE_TAVARUA_PSALL, psAllVal >> RDS_GROUP_AF);
    }
    ...
    public int enableAFjump(boolean AFenable) {
        ...
        if (AFenable) {
            FmReceiverJNI.setControlNative(this.mFd, V4L2_CID_PRIVATE_TAVARUA_RDSGROUP_PROC, rds_group_mask | RDS_AF_AUTO);
        } else {
            FmReceiverJNI.setControlNative(this.mFd, V4L2_CID_PRIVATE_TAVARUA_RDSGROUP_PROC, rds_group_mask & -65);
        }
        return RDS_GROUP_RT;
    }

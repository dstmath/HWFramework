...
/*
        Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: android.icu.impl.locale.InternalLocaleBuilder.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: android.icu.impl.locale.InternalLocaleBuilder.<clinit>():void
        ...
        while (!itr.isDone()) {
            if (key != null) {
                if (UnicodeLocaleExtension.isKey(itr.current())) {
                    ...
                    String type = typeStart == -1 ? XmlPullParser.NO_NAMESPACE : subtags.substring(typeStart, typeEnd);
                    if (this._ukeywords == null) {
                        this._ukeywords = new HashMap(4);
                    }
            ...
            if (itr.hasNext()) {
                ...
                type = typeStart == -1 ? XmlPullParser.NO_NAMESPACE : subtags.substring(typeStart, typeEnd);
                if (this._ukeywords == null) {
                    this._ukeywords = new HashMap(4);
                }
                this._ukeywords.put(key, type);

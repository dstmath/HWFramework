...
class TransliterationRule {
    ...
    public TransliterationRule(String input, int anteContextPos, int postContextPos, String output, int cursorPos, int cursorOffset, UnicodeMatcher[] segs, boolean anchorStart, boolean anchorEnd, Data theData) {
        ...
        this.flags = (byte) 0;
        if (anchorStart) {
            this.flags = (byte) (this.flags | ANCHOR_START);
        }
        if (anchorEnd) {
            this.flags = (byte) (this.flags | ANCHOR_END);
        }
    ...
    final int getIndexValue() {
        ...
        if (this.data.lookupMatcher(c) == null) {
            i = c & Opcodes.OP_CONST_CLASS_JUMBO;
        }
        return i;
    }
    ...
    public boolean masks(TransliterationRule r2) {
        ...
        if (left == left2 && right == right2 && this.keyLength <= r2.keyLength && r2.pattern.regionMatches(0, this.pattern, 0, len)) {
            if (this.flags != r2.flags && (!((this.flags & ANCHOR_START) == 0 && (this.flags & ANCHOR_END) == 0) && ((r2.flags & ANCHOR_START) == 0 || (r2.flags & ANCHOR_END) == 0))) {
                z = false;
            }
            return z;
        }
    ...
    public int matchAndReplace(Replaceable text, Position pos, boolean incremental) {
        if (this.segments != null) {
            for (int i = 0; i < this.segments.length; i += ANCHOR_START) {
                ((StringMatcher) this.segments[i]).resetMatch();
            }
        }
        int[] intRef = new int[ANCHOR_START];
        int anteLimit = posBefore(text, pos.contextStart);
        intRef[0] = posBefore(text, pos.start);
        if (this.anteContext != null && this.anteContext.matches(text, intRef, anteLimit, false) != ANCHOR_END) {
            return 0;
        }
        int oText = intRef[0];
        int minOText = posAfter(text, oText);
        if ((this.flags & ANCHOR_START) != 0 && oText != anteLimit) {
            return 0;
        }
        int match;
        intRef[0] = pos.start;
    ...
    public String toRule(boolean escapeUnprintable) {
        ...
        if (emitBraces) {
            Utility.appendToRule(rule, (int) Opcodes.OP_NEG_INT, true, escapeUnprintable, quoteBuf);
        }
        Utility.appendToRule(rule, this.key, escapeUnprintable, quoteBuf);
        if (emitBraces) {
            Utility.appendToRule(rule, (int) Opcodes.OP_NEG_LONG, true, escapeUnprintable, quoteBuf);
        }

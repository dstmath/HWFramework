...
/*
        Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: android.opengl.GLES10.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: android.opengl.GLES10.<clinit>():void
...
*/
    ...
    public static void glNormalPointer(int type, int stride, Buffer pointer) {
        ...
        if (!(type == GL_FLOAT || type == GL_BYTE || type == GL_SHORT)) {
            if (type != GL_FIXED) {
                return;
            }
        }
    ...
    public static void glTexCoordPointer(int size, int type, int stride, Buffer pointer) {
        ...
        if (!(size == GL_LINE_LOOP || size == GL_LINE_STRIP)) {
            if (size != GL_TRIANGLES) {
                return;
            }
        }

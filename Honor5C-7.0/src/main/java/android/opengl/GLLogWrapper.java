...
class GLLogWrapper extends GLWrapperBase {
    ...
    private void argPointer(int size, int type, int stride, Buffer pointer) {
        arg("size", size);
        arg(PhoneLookupColumns.TYPE, getPointerTypeName(type));
    ...
    private String getClearBufferMask(int mask) {
        ...
        if ((mask & TriangleMeshBuilder.TEXTURE_0) != 0) {
            b.append("GL_DEPTH_BUFFER_BIT");
            mask &= -257;
        }
        if ((mask & Document.FLAG_SUPPORTS_REMOVE) != 0) {
            if (b.length() > 0) {
                b.append(" | ");
            }
        ...
        if ((mask & Process.PROC_OUT_FLOAT) != 0) {
            if (b.length() > 0) {
                b.append(" | ");
            }
    ...
    private String getClientState(int clientState) {
        switch (clientState) {
            case GLES32.GL_VERTEX_ARRAY /*32884*/:
    ...
    private String getCap(int cap) {
        switch (cap) {
            ...
            case HistoryItem.EVENT_PROC_FINISH /*16385*/:
                return "GL_LIGHT1";
            case HistoryItem.EVENT_FOREGROUND_FINISH /*16386*/:
                return "GL_LIGHT2";
    ...
    private String getLightName(int light) {
        if (light < Process.PROC_OUT_FLOAT || light > HistoryItem.EVENT_USER_RUNNING_FINISH) {
            return getHex(light);
        }
        ...
        switch (type) {
            case GLES20.GL_UNSIGNED_BYTE /*5121*/:
                ...
                for (int i = 0; i < count; i += FORMAT_FLOAT) {
                    result[i] = (char) (array[offset + i] & Process.PROC_TERM_MASK);
                }
                break;
            case GLES20.GL_UNSIGNED_SHORT /*5123*/:
                ...
                for (int i = 0; i < size; i += FORMAT_FLOAT) {
                    ...
                    switch (type) {
                        case GLES20.GL_BYTE /*5120*/:
                            ...
                            break;
                        case GLES20.GL_UNSIGNED_BYTE /*5121*/:
                            builder.append(Integer.toString(byteBuffer.get(byteOffset) & Process.PROC_TERM_MASK));
                            break;
                        case GLES20.GL_SHORT /*5122*/:
    ...
    public void glCompressedTexImage2D(int target, int level, int internalformat, int width, int height, int border, int imageSize, Buffer data) {
        ...
        arg(Thumbnails.WIDTH, width);
        arg(Thumbnails.HEIGHT, height);
        arg("border", border);
        arg("imageSize", imageSize);
        arg(VoiceInteractionSession.KEY_DATA, data.toString());
        end();
    ...
    public void glDrawArrays(int mode, int first, int count) {
        ...
        arg(Intents.EXTRA_MODE, mode);
        arg("first", first);
        arg(ConnectivityMetricsLogger.DATA_KEY_EVENTS_COUNT, count);
        startLogIndices();
        for (int i = 0; i < count; i += FORMAT_FLOAT) {
    ...
    public void glDrawElements(int mode, int count, int type, Buffer indices) {
        ...
        arg(Intents.EXTRA_MODE, getBeginMode(mode));
        arg(ConnectivityMetricsLogger.DATA_KEY_EVENTS_COUNT, count);
        arg(PhoneLookupColumns.TYPE, getIndexType(type));
    ...
    public void glHint(int target, int mode) {
        ...
        arg(Intents.EXTRA_MODE, getHintMode(mode));
        end();
    ...
    public void glMatrixMode(int mode) {
        begin("glMatrixMode");
        arg(Intents.EXTRA_MODE, getMatrixMode(mode));
        end();
    ...
    public void glNormalPointer(int type, int stride, Buffer pointer) {
        ...
        arg(PhoneLookupColumns.TYPE, type);
        arg(MediaFormat.KEY_STRIDE, stride);
        arg("pointer", pointer.toString());
        end();
    ...
    public void glSampleCoverage(float value, boolean invert) {
        ...
        arg(NameValueTable.VALUE, value);
        arg("invert", invert);
        end();
    ...
    public void glShadeModel(int mode) {
        begin("glShadeModel");
        arg(Intents.EXTRA_MODE, getShadeModel(mode));
        end();
    ...
    public void glDrawTexsOES(short x, short y, short z, short width, short height) {
        ...
        arg("y", (int) y);
        arg("z", (int) z);
        arg(Thumbnails.WIDTH, (int) width);
        arg(Thumbnails.HEIGHT, (int) height);
        end();

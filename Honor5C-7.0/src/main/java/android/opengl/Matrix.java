...
/*
        Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: android.opengl.Matrix.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: android.opengl.Matrix.<clinit>():void
...
*/
    ...
    public static boolean invertM(float[] mInv, int mInvOffset, float[] m, int mOffset) {
        ...
        float invdet = Engine.DEFAULT_VOLUME / det;
    ...
    public static void orthoM(float[] m, int mOffset, float left, float right, float bottom, float top, float near, float far) {
        if (left == right) {
            ...
            float r_width = Engine.DEFAULT_VOLUME / (right - left);
            float r_height = Engine.DEFAULT_VOLUME / (top - bottom);
            float r_depth = Engine.DEFAULT_VOLUME / (far - near);
            ...
            m[mOffset + 13] = ty;
            m[mOffset + 14] = tz;
            m[mOffset + 15] = Engine.DEFAULT_VOLUME;
            m[mOffset + 1] = 0.0f;
            m[mOffset + 2] = 0.0f;
            m[mOffset + 3] = 0.0f;
            m[mOffset + 4] = 0.0f;
    ...
    public static void frustumM(float[] m, int offset, float left, float right, float bottom, float top, float near, float far) {
        if (left == right) {
            ...
            float r_width = Engine.DEFAULT_VOLUME / (right - left);
            float r_height = Engine.DEFAULT_VOLUME / (top - bottom);
            float r_depth = Engine.DEFAULT_VOLUME / (near - far);
            ...
            m[offset + 10] = C;
            m[offset + 14] = D;
            m[offset + 11] = ScaledLayoutParams.SCALE_UNSPECIFIED;
            m[offset + 1] = 0.0f;
            m[offset + 2] = 0.0f;
            m[offset + 3] = 0.0f;
            m[offset + 4] = 0.0f;
    ...
    public static void perspectiveM(float[] m, int offset, float fovy, float aspect, float zNear, float zFar) {
        float f = Engine.DEFAULT_VOLUME / ((float) Math.tan(((double) fovy) * 0.008726646259971648d));
        float rangeReciprocal = Engine.DEFAULT_VOLUME / (zNear - zFar);
        m[offset + 0] = f / aspect;
        m[offset + 1] = 0.0f;
        m[offset + 2] = 0.0f;
        m[offset + 3] = 0.0f;
    ...
    public static void setIdentityM(float[] sm, int smOffset) {
        ...
        for (i = 0; i < 16; i += 5) {
            sm[smOffset + i] = Engine.DEFAULT_VOLUME;
        }
    }
    ...
    public static void setRotateM(float[] rm, int rmOffset, float a, float x, float y, float z) {
        ...
        rm[rmOffset + 13] = 0.0f;
        rm[rmOffset + 14] = 0.0f;
        rm[rmOffset + 15] = Engine.DEFAULT_VOLUME;
        a *= 0.017453292f;
        float s = (float) Math.sin((double) a);
        float c = (float) Math.cos((double) a);
        if (Engine.DEFAULT_VOLUME == x && 0.0f == y && 0.0f == z) {
            rm[rmOffset + 5] = c;
            rm[rmOffset + 10] = c;
            rm[rmOffset + 6] = s;
            rm[rmOffset + 9] = -s;
            ...
            rm[rmOffset + 4] = 0.0f;
            rm[rmOffset + 8] = 0.0f;
            rm[rmOffset + 0] = Engine.DEFAULT_VOLUME;
        } else if (0.0f == x && Engine.DEFAULT_VOLUME == y && 0.0f == z) {
            rm[rmOffset + 0] = c;
            rm[rmOffset + 10] = c;
            rm[rmOffset + 8] = s;
            rm[rmOffset + 2] = -s;
            ...
            rm[rmOffset + 6] = 0.0f;
            rm[rmOffset + 9] = 0.0f;
            rm[rmOffset + 5] = Engine.DEFAULT_VOLUME;
        } else if (0.0f == x && 0.0f == y && Engine.DEFAULT_VOLUME == z) {
            rm[rmOffset + 0] = c;
            rm[rmOffset + 5] = c;
            rm[rmOffset + 1] = s;
            rm[rmOffset + 4] = -s;
            ...
            rm[rmOffset + 8] = 0.0f;
            rm[rmOffset + 9] = 0.0f;
            rm[rmOffset + 10] = Engine.DEFAULT_VOLUME;
        } else {
            float len = length(x, y, z);
            if (Engine.DEFAULT_VOLUME != len) {
                float recipLen = Engine.DEFAULT_VOLUME / len;
                x *= recipLen;
                y *= recipLen;
                z *= recipLen;
            }
            float nc = Engine.DEFAULT_VOLUME - c;
            float xy = x * y;
            float yz = y * z;
            float zx = z * x;
            float xs = x * s;
    ...
    public static void setLookAtM(float[] rm, int rmOffset, float eyeX, float eyeY, float eyeZ, float centerX, float centerY, float centerZ, float upX, float upY, float upZ) {
        ...
        float rlf = Engine.DEFAULT_VOLUME / length(fx, fy, fz);
        fx *= rlf;
        fy *= rlf;
        fz *= rlf;
        ...
        float rls = Engine.DEFAULT_VOLUME / length(sx, sy, sz);
        sx *= rls;
        sy *= rls;
        sz *= rls;

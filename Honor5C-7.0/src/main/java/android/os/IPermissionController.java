        ...
        private static class Proxy implements IPermissionController {
            ...
            public boolean isRuntimePermission(String permission) throws RemoteException {
                ...
                try {
                    _data.writeInterfaceToken(Stub.DESCRIPTOR);
                    _data.writeString(permission);
                    this.mRemote.transact(Stub.TRANSACTION_isRuntimePermission, _data, _reply, 0);
                    _reply.readException();
                    boolean _result = _reply.readInt() != 0;
                    _reply.recycle();
                    _data.recycle();
        ...
        public boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
            ...
            switch (code) {
                case TRANSACTION_checkPermission /*1*/:
                    ...
                    if (_result) {
                        i = TRANSACTION_checkPermission;
                    }
                    reply.writeInt(i);
                    return true;
                case TRANSACTION_getPackagesForUid /*2*/:
                    data.enforceInterface(DESCRIPTOR);
                    String[] _result2 = getPackagesForUid(data.readInt());
                    reply.writeNoException();
                    reply.writeStringArray(_result2);
                    return true;
                case TRANSACTION_isRuntimePermission /*3*/:
                    data.enforceInterface(DESCRIPTOR);

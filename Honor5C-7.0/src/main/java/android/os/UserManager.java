...
public class UserManager {
    ...
    private static final int[] SYSTEM_RESERVED_USERIDS = null;
...
/*
        Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: android.os.UserManager.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: android.os.UserManager.<clinit>():void
...
*/
    ...
    public static boolean supportsMultipleUsers() {
        if (getMaxSupportedUsers() > USER_CREATION_FAILED_NOT_PERMITTED) {
            return SystemProperties.getBoolean("fw.show_multiuserui", Resources.getSystem().getBoolean(17956983));
        }
        return false;
    }
    ...
    public boolean canSwitchUsers() {
        boolean allowUserSwitchingWhenSystemUserLocked = Global.getInt(this.mContext.getContentResolver(), Global.ALLOW_USER_SWITCHING_WHEN_SYSTEM_USER_LOCKED, RESTRICTION_NOT_SET) != 0;
    ...
    public boolean isUserRunning(int userId) {
        try {
            return ActivityManagerNative.getDefault().isUserRunning(userId, RESTRICTION_NOT_SET);
        } catch (RemoteException re) {
            throw re.rethrowFromSystemServer();
        }
    }
    ...
    public boolean isUserRunningOrStopping(UserHandle user) {
        try {
            return ActivityManagerNative.getDefault().isUserRunning(user.getIdentifier(), USER_CREATION_FAILED_NOT_PERMITTED);
        } catch (RemoteException re) {
            throw re.rethrowFromSystemServer();
        }
    }
    ...
    public boolean isUserRunningAndLocked(UserHandle user) {
        try {
            return ActivityManagerNative.getDefault().isUserRunning(user.getIdentifier(), USER_CREATION_FAILED_NO_MORE_USERS);
        } catch (RemoteException re) {
            throw re.rethrowFromSystemServer();
        }
    }
    ...
    public boolean isUserRunningAndUnlocked(UserHandle user) {
        try {
            return ActivityManagerNative.getDefault().isUserRunning(user.getIdentifier(), RESTRICTION_SOURCE_PROFILE_OWNER);
        } catch (RemoteException re) {
            throw re.rethrowFromSystemServer();
        }
    }
    ...
    public boolean isUserUnlocked(int userId) {
        try {
            return ActivityManagerNative.getDefault().isUserRunning(userId, RESTRICTION_SOURCE_PROFILE_OWNER);
        } catch (RemoteException re) {
            throw re.rethrowFromSystemServer();
        }
    }
    ...
    public UserInfo createUser(String name, int flags) {
        try {
            ...
            if (!(user == null || user.isAdmin() || user.isHwHiddenSpace())) {
    ...
    public UserInfo createGuest(Context context, String name) {
        try {
            UserInfo guest = this.mService.createUser(name, RESTRICTION_SOURCE_PROFILE_OWNER);
            if (guest != null) {
    ...
    public int getUserCount() {
        ...
        return users != null ? users.size() : USER_CREATION_FAILED_NOT_PERMITTED;
    }
    ...
    public long[] getSerialNumbersOfUsers(boolean excludeDying) {
        try {
            ...
            for (int i = RESTRICTION_NOT_SET; i < result.length; i += USER_CREATION_FAILED_NOT_PERMITTED) {
    ...
    public boolean canAddMoreUsers() {
        ...
        int aliveUserCount = RESTRICTION_NOT_SET;
        for (int i = RESTRICTION_NOT_SET; i < totalUserCount; i += USER_CREATION_FAILED_NOT_PERMITTED) {
            if (!((UserInfo) users.get(i)).isGuest()) {
                aliveUserCount += USER_CREATION_FAILED_NOT_PERMITTED;
            }
        }
    ...
    public List<UserHandle> getUserProfiles() {
        ...
        for (int i = RESTRICTION_NOT_SET; i < length; i += USER_CREATION_FAILED_NOT_PERMITTED) {
            result.add(UserHandle.of(userIds[i]));
        }
        return result;
    }
    ...
    public static int getMaxSupportedUsers() {
        if (Build.ID.startsWith("JVP") || ActivityManager.isLowRamDeviceStatic()) {
            return USER_CREATION_FAILED_NOT_PERMITTED;
        }
        return SystemProperties.getInt("fw.max_users", Resources.getSystem().getInteger(17694862));
    }
    ...
    public boolean isUserSwitcherEnabled() {
        ...
        int switchableUserCount = RESTRICTION_NOT_SET;
        for (UserInfo user : users) {
            if (user.supportsSwitchToByUser()) {
                switchableUserCount += USER_CREATION_FAILED_NOT_PERMITTED;
            }
        }
        ...
        if (switchableUserCount > USER_CREATION_FAILED_NOT_PERMITTED) {
            guestEnabled = true;
        }
        return guestEnabled;
    }
    ...
    public static boolean isSystemReservedUserId(int userId) {
        int index = PIN_VERIFICATION_SUCCESS;
        for (int i = RESTRICTION_NOT_SET; i < SYSTEM_RESERVED_USERIDS.length; i += USER_CREATION_FAILED_NOT_PERMITTED) {
            if (userId == SYSTEM_RESERVED_USERIDS[i]) {
                index = i;
                break;
            }
        }
        if (index != PIN_VERIFICATION_SUCCESS) {
            return true;
        }
        return false;
    }

    public void createUserDir(int userId) throws RemoteException {
        if (isSystemReservedUserId(userId)) {
            Parcel data = Parcel.obtain();
            data.writeInt(userId);
            Parcel reply = Parcel.obtain();
            data.writeInterfaceToken("android.os.IUserManager");
            this.mService.asBinder().transact(MKDIR_FOR_USER_TRANSACTION, data, reply, RESTRICTION_NOT_SET);
            reply.readException();
            data.recycle();
            reply.recycle();
        }

...
public final class PrinterDiscoverySession {
    ...
    public static final class PrinterDiscoveryObserver extends Stub {
        ...
        public void onPrintersAdded(ParceledListSlice printers) {
            ...
            if (session != null) {
                session.mHandler.obtainMessage(PrinterDiscoverySession.MSG_PRINTERS_ADDED, printers.getList()).sendToTarget();
            }
        }
        ...
        public void onPrintersRemoved(ParceledListSlice printerIds) {
            PrinterDiscoverySession session = (PrinterDiscoverySession) this.mWeakSession.get();
            if (session != null) {
                session.mHandler.obtainMessage(PrinterDiscoverySession.MSG_PRINTERS_REMOVED, printerIds.getList()).sendToTarget();
            }
        }
    }
    ...
    private final class SessionHandler extends Handler {
        ...
        public void handleMessage(Message message) {
            switch (message.what) {
                case PrinterDiscoverySession.MSG_PRINTERS_ADDED /*1*/:
                    PrinterDiscoverySession.this.handlePrintersAdded(message.obj);
                case PrinterDiscoverySession.MSG_PRINTERS_REMOVED /*2*/:
    ...
    private void handlePrintersAdded(List<PrinterInfo> addedPrinters) {
        if (!isDestroyed()) {
            ...
            if (this.mPrinters.isEmpty()) {
                ...
                for (i = 0; i < printerCount; i += MSG_PRINTERS_ADDED) {
    ...
    private void handlePrintersRemoved(List<PrinterId> printerIds) {
        if (!isDestroyed()) {
            ...
            for (int i = 0; i < removedPrinterIdCount; i += MSG_PRINTERS_ADDED) {

...
public class Map {
    private final MapBiller biller;
    private int centerPixelX;
    private int centerPixelY;
    private int completeTilesInViewport;
    private int cornerToCenterDist;
    private int displayHeight;
    private int displayWidth;
    private long earliestTileNeededTime;
    private int estimatedCountOfRenderedImagesInMapCache;
    private int halfDisplayHeight;
    ...
    private int width;
    private int xDisplayTiles;
    private int xTiles;
    private int yDisplayTiles;
    private int yTiles;
    ...
    public Map(int maxCacheDataSize, int targetCacheDataSize, int maxFlashSize, MapPoint defaultCenter, Zoom defaultZoom, int maxRecordStores) {
        this.height = 0;
        this.width = 0;
        this.cornerToCenterDist = 0;
        this.halfWidth = 0;
        this.halfHeight = 0;
        this.displayHeight = 0;
        this.displayWidth = 0;
        this.halfDisplayWidth = 0;
        this.halfDisplayHeight = 0;
        this.xTiles = 0;
        this.yTiles = 0;
        this.xDisplayTiles = 0;
        this.yDisplayTiles = 0;
        this.completeTilesInViewport = 0;
        this.biller = new MapBiller();
        this.pixelMapper = null;
        this.hardwareAcceleration = false;
        this.estimatedCountOfRenderedImagesInMapCache = 0;
    ...
    Tile getTopLeftTile(int numXTiles, int numYTiles) {
        boolean xTilesIsEven;
        boolean yTilesIsEven;
        ...
        if (xTilesIsEven && centerXOffsetFromCenterTile > 128) {
            xIndex++;
        }
        if (numYTiles % 2 != 0) {
            yTilesIsEven = false;
    ...
    private void logPerceivedTileLatency(int tilesDrawn) {
        ...
        if (this.completeTilesInViewport < tilesDrawn && tilesDrawn == this.xTiles * this.yTiles) {
            String eventType;
            if (this.isViewportAllNew) {
                eventType = "tc";
            } else {
                eventType = "tp";
            }
            Log.addEvent((short) 22, eventType + tileType, "" + latency);
        }
        this.completeTilesInViewport = tilesDrawn;
    }
    ...
    private boolean drawTile(Tile tile, int i, int j, GoogleGraphics g, int xOffset, int yOffset, boolean fetch, boolean drawIncompleteTiles, boolean drawOptionalFeatures, long paintStartTime, long accessTime) {
        ...
        if (!tile.notValid()) {
            ...
            if (drawOptionalFeatures) {
                i2 = 2;
            } else {
                i2 = 1;
            }
            MapTile mapTile = mapService.getTile(tile, priority, fetch, i2, accessTime);

...
class MapFlashService implements MapTileStorage {
    private int catalogRecordBytes;
    private boolean catalogUpdatedSinceLastWrite;
    private final Vector flashRecords;
    private int highestRecordId;
    ...
    private final Hashtable tileToRecordMap;
    ...
    MapFlashService(MapService mapService, String recordStoreBaseName, int maxFlashSize, int maxRecordStores) {
        ...
        this.tileEdition = -1;
        this.textSize = -1;
        this.flashRecords = new Vector();
        this.tileToRecordMap = new Hashtable();
    ...
    private void handlePersistentStoreWriteException(PersistentStoreException e, boolean catalog) {
        ...
        StringBuilder append = new StringBuilder().append("FLASH ").append(curFlashSize).append("B ").append(curNumRecordBlocks).append("R");
        if (catalog) {
            str = " catalog";
        } else {
            str = "";
        }
        Log.logQuietThrowable(append.append(str).toString(), e);
        if (e.getType() == -2) {
    ...
    public boolean writeCache() throws IOException {
        ...
        if (this.nextPersistTime >= startTime) {
            obj = 1;
        } else {
            obj = null;
        }
        if (obj == null) {
            if ((this.lastChangedTime + 1500 >= startTime ? 1 : null) == null) {
                try {
                    boolean cachingStillActive;
                    int status = doPersist(mapCache);
                    if (status == 3 || status == 4) {
                        cachingStillActive = true;
                    } else {
                        cachingStillActive = false;
                    }

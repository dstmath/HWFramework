...
public class AESUtils {
    ...
    public static String encrypt4AES(String source, String key) {
        try {
            ...
            String encryptResultStr = Base64.encode(cipher.doFinal(source.getBytes("UTF-8")));
            String ivString = Base64.encode(ivBytes);
            return ivString + encryptResultStr;
    ...
    public static String decrypt4AES(String content, String key) {
        try {
            ...
            IvParameterSpec ivParameterSpec = new IvParameterSpec(Base64.decode(ivEncodeStr));
            SecretKeySpec secretKeySpec = new SecretKeySpec(key.getBytes("UTF-8"), ALGORITHM_AES);
            Cipher cipher = Cipher.getInstance(CIPHER_FORM);

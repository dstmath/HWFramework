...
public abstract class Updater implements IUpdate {
    ...
    public int getItem() {
        return this.mPref.getInt(this.mAutoItemKey, ITEM_DEFAULT);
    }
    ...
    private void doFinishNotification(String title) {
        ...
        intent.setPackage(this.mContext.getPackageName());
        CommonUtilMethods.constructAndSendSummaryNotification(this.mContext, title);
    ...
    private static Notification constructFinishNotification(Context context, String title, Intent intent) {
        Builder groupSummary = new Builder(context).setAutoCancel(true).setSmallIcon(CommonUtilMethods.getBitampIcon(context, 2130837910)).setContentTitle(title).setTicker(title).setGroup("group_key_contacts").setGroupSummary(false);
        if (intent == null) {
            intent = new Intent();
        }

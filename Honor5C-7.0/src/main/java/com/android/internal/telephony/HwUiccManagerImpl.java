...
import com.android.internal.telephony.msim.HwMSimIccProviderUtils;
...
public class HwUiccManagerImpl implements HwUiccManager {
    ...
    private static HwUiccManager instance;

    static {
        /* JADX: method processing error */
/*
        Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: com.android.internal.telephony.HwUiccManagerImpl.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: com.android.internal.telephony.HwUiccManagerImpl.<clinit>():void
...
*/
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: com.android.internal.telephony.HwUiccManagerImpl.<clinit>():void");
    }
    ...
    public Cursor simContactsQueryMSim(Context context, Uri url, String[] projection, String selection, String[] selectionArgs, String sort) {
        return HwMSimIccProviderUtils.getDefault(context).query(url, projection, selection, selectionArgs, sort);
    }

    public String simContactsGetTypeMsim(Context context, Uri url) {
        return HwMSimIccProviderUtils.getDefault(context).getType(url);
    }

    public Uri simContactsInsertMSim(Context context, Uri url, ContentValues initialValues) {
        return HwMSimIccProviderUtils.getDefault(context).insert(url, initialValues);
    }

    public int simContactsDeleteMSim(Context context, Uri url, String where, String[] whereArgs) {
        return HwMSimIccProviderUtils.getDefault(context).delete(url, where, whereArgs);
    }

    public int simContactsUpdateMSim(Context context, Uri url, ContentValues values, String where, String[] whereArgs) {
        return HwMSimIccProviderUtils.getDefault(context).update(url, values, where, whereArgs);
    }
    ...
    public RadioState powerUpRadioIfhasCard(Context c, int mSlotId, RadioState radioState, RadioState mLastRadioState, CardState mCardState) {
        ...
        if (radioState == RadioState.RADIO_OFF && mLastRadioState == RadioState.RADIO_ON) {
            ...
            if (Global.getInt(c.getContentResolver(), "airplane_mode_on", 0) != 0) {
                isAirplaneModeOn = true;
            } else {
                isAirplaneModeOn = false;
            }
            if (!(isAirplaneModeOn || mCardState == CardState.CARDSTATE_ABSENT)) {
                if (HWFLOW) {
    ...
    public boolean isFullNetworkSupported() {
        if (SystemProperties.getBoolean("ro.config.full_network_support", false) && !HwModemCapability.isCapabilitySupport(9) && SystemProperties.getBoolean("persist.hisi.fullnetwork", true)) {

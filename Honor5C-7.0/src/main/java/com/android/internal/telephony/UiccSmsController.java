...
public class UiccSmsController extends AbstractUiccSmsController {
    ...
    public void sendTextForSubscriberWithSelfPermissions(int subId, String callingPackage, String destAddr, String scAddr, String text, PendingIntent sentIntent, PendingIntent deliveryIntent) {
        IccSmsInterfaceManager iccSmsIntMgr = getIccSmsInterfaceManager(subId);
        if (iccSmsIntMgr != null) {
            iccSmsIntMgr.sendTextWithSelfPermissions(callingPackage, destAddr, scAddr, text, sentIntent, deliveryIntent);
        } else {
            Rlog.e(LOG_TAG, "sendText iccSmsIntMgr is null for Subscription: " + subId);
        }
    }
    ...
    public void setEnabledSingleShiftTables(int[] tables) {
        Application mApplication = ActivityThread.currentApplication();
        if (mApplication != null) {
            mApplication.getApplicationContext().enforceCallingPermission("android.permission.SEND_SMS", "Requires send sms permission for setEnabledSingleShiftTables");
    ...
    public void setSmsCodingNationalCode(String code) {
        Application mApplication = ActivityThread.currentApplication();
        if (mApplication != null) {
            mApplication.getApplicationContext().enforceCallingPermission("android.permission.SEND_SMS", "Requires send sms permission for setSmsCodingNationalCode");

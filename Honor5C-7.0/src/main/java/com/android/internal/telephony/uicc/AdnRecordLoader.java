...
public class AdnRecordLoader extends Handler {
    ...
    public void loadFromEF(int ef, int extensionEF, int recordNumber, Message response) {
        ...
        this.mFh.loadEFLinearFixed(ef, getEFPath(ef), recordNumber, obtainMessage(EVENT_ADN_LOAD_DONE));
    }
    ...
    public void loadAllFromEF(int ef, int extensionEF, Message response) {
        ...
        this.mExtensionEF = extensionEF;
        this.mUserResponse = response;
        this.mFh.loadEFLinearFixedAll(ef, getEFPath(ef), obtainMessage(EVENT_ADN_LOAD_ALL_DONE));
    }
    ...
    public void updateEF(AdnRecord adn, int ef, int extensionEF, int recordNumber, String pin2, Message response) {
        ...
        this.mUserResponse = response;
        this.mPin2 = pin2;
        this.mFh.getEFLinearRecordSize(ef, getEFPath(ef), obtainMessage(EVENT_EF_LINEAR_RECORD_SIZE_DONE, adn));
    }
                ...
                case EVENT_EF_LINEAR_RECORD_SIZE_DONE /*4*/:
                    ...
                    if (ar.exception == null) {
                        ...
                        if (recordSize.length == EVENT_ADN_LOAD_ALL_DONE && this.mRecordNumber <= recordSize[EVENT_EXT_RECORD_LOAD_DONE]) {
                            ...
                            if (adn.mExtRecord != PduHeaders.STORE_STATUS_ERROR_END) {
                                data[data.length - 1] = (byte) adn.mExtRecord;
                            }
                            if (data != null) {
                                this.mFh.updateEFLinearFixed(this.mEf, getEFPath(this.mEf), this.mRecordNumber, data, this.mPin2, obtainMessage(EVENT_UPDATE_RECORD_DONE));
                                this.mPendingExtLoads = EVENT_ADN_LOAD_DONE;
                                break;
                            }

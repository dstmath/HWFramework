...
public class HwVSimDReadyProcessor extends HwVSimReadyProcessor {
    ...
    protected void onRadioPowerOnDone(Message msg) {
        ...
        if (isAsyncResultValidForRequestNotSupport(msg.obj)) {
            processDReady();
        }
    }

    private void processDReady() {
        if (this.mRequest != null && this.mRequest.getIsNeedSwitchCommrilMode() && this.mVSimController.isPinNeedBlock(this.mRequest.getMainSlot())) {

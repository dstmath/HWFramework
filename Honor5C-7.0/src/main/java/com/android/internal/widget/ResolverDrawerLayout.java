...
/*
            Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: com.android.internal.widget.ResolverDrawerLayout.SavedState.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: com.android.internal.widget.ResolverDrawerLayout.SavedState.<clinit>():void
...
*/
    ...
    private void smoothScrollTo(int yOffset, float velocity) {
        ...
        if (dy != 0) {
            ...
            float distance = ((float) halfHeight) + (((float) halfHeight) * distanceInfluenceForSnapDuration(Math.min(android.view.WindowManager.LayoutParams.BRIGHTNESS_OVERRIDE_FULL, (((float) Math.abs(dy)) * android.view.WindowManager.LayoutParams.BRIGHTNESS_OVERRIDE_FULL) / ((float) height))));
            velocity = Math.abs(velocity);
            if (velocity > 0.0f) {
                duration = Math.round(Math.abs(distance / velocity) * 1000.0f) * 4;
            } else {
                duration = (int) (((((float) Math.abs(dy)) / ((float) height)) + android.view.WindowManager.LayoutParams.BRIGHTNESS_OVERRIDE_FULL) * 100.0f);
            }
            this.mScroller.startScroll(0, sy, 0, dy, Math.min(duration, StatisticalConstant.TYPE_WIFIP2P_INVITATION_ALERT));
            postInvalidateOnAnimation();
        }
    }
    ...
    public boolean onNestedFling(View target, float velocityX, float velocityY, boolean consumed) {
        ...
        if (this.mOnDismissedListener == null || velocityY >= 0.0f || this.mCollapseOffset <= this.mCollapsibleHeight) {
            if (velocityY <= 0.0f) {
                i = this.mCollapsibleHeight;
            }
    ...
    public boolean onNestedPrePerformAccessibilityAction(View target, int action, Bundle args) {
        ...
        if (action != HwPerformance.PERF_EVENT_RAW_REQ || this.mCollapseOffset == 0) {
            return false;
        }
        smoothScrollTo(0, 0.0f);
        return true;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int widthSpec = MeasureSpec.makeMeasureSpec(widthSize, EditorInfo.IME_FLAG_NO_ENTER_ACTION);
        int heightSpec = MeasureSpec.makeMeasureSpec(heightSize, EditorInfo.IME_FLAG_NO_ENTER_ACTION);

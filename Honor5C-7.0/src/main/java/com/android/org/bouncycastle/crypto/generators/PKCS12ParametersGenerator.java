...
public class PKCS12ParametersGenerator extends PBEParametersGenerator {
    ...
    private byte[] generateDerivedKey(int idByte, int n) {
        ...
        for (i = 0; i != D.length; i += KEY_MATERIAL) {
            D[i] = (byte) idByte;
        }
        if (this.salt == null || this.salt.length == 0) {
            ...
            for (i = 0; i != S.length; i += KEY_MATERIAL) {
        ...
        if (this.password == null || this.password.length == 0) {
            ...
            for (i = 0; i != P.length; i += KEY_MATERIAL) {
        ...
        for (i = KEY_MATERIAL; i <= c; i += KEY_MATERIAL) {
            int j;
            this.digest.update(D, 0, D.length);
            this.digest.update(I, 0, I.length);
            this.digest.doFinal(A, 0);
            for (j = KEY_MATERIAL; j < this.iterationCount; j += KEY_MATERIAL) {
                this.digest.update(A, 0, A.length);
                this.digest.doFinal(A, 0);
            }
            for (j = 0; j != B.length; j += KEY_MATERIAL) {
                B[j] = A[j % A.length];
            }
            for (j = 0; j != I.length / this.v; j += KEY_MATERIAL) {
                adjust(I, this.v * j, B);
            }
            if (i == c) {

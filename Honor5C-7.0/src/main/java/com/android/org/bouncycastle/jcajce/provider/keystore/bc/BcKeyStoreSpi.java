...
public class BcKeyStoreSpi extends KeyStoreSpi implements BCKeyStore {
    ...
    public static class BouncyCastleStore extends BcKeyStoreSpi {
        ...
        public void engineLoad(InputStream stream, char[] password) throws IOException {
            ...
            if (stream != null) {
                ...
                if (version == BcKeyStoreSpi.STORE_VERSION || version == 0 || version == BcKeyStoreSpi.KEY_PUBLIC) {
                    byte[] salt = new byte[dIn.readInt()];
                    if (salt.length != BcKeyStoreSpi.STORE_SALT_SIZE) {
                    ...
                    CipherInputStream cIn = new CipherInputStream(dIn, makePBECipher(cipherAlg, BcKeyStoreSpi.STORE_VERSION, password, salt, iterationCount));
                    Digest dig = new SHA1Digest();
                    loadStore(new DigestInputStream(cIn, dig));
                    byte[] hash = new byte[dig.getDigestSize()];
                    dig.doFinal(hash, BcKeyStoreSpi.NULL);
        ...
        public void engineStore(OutputStream stream, char[] password) throws IOException {
            ...
            byte[] salt = new byte[BcKeyStoreSpi.STORE_SALT_SIZE];
            ...
            CipherOutputStream cOut = new CipherOutputStream(dOut, makePBECipher(BcKeyStoreSpi.STORE_CIPHER, BcKeyStoreSpi.KEY_PUBLIC, password, salt, iterationCount));
    ...
    private class StoreEntry {
        ...
        StoreEntry(String alias, Key key, char[] password, Certificate[] certChain) throws Exception {
            ...
            dOut.write(salt);
            dOut.writeInt(iterationCount);
            dOut = new DataOutputStream(new CipherOutputStream(dOut, BcKeyStoreSpi.this.makePBECipher(BcKeyStoreSpi.KEY_CIPHER, BcKeyStoreSpi.KEY_PUBLIC, password, salt, iterationCount)));
        ...
        Object getObject(char[] password) throws NoSuchAlgorithmException, UnrecoverableKeyException {
            ...
            if (this.type == BcKeyStoreSpi.SEALED) {
                ...
                try {
                    ...
                    dIn2.readFully(salt);
                    return BcKeyStoreSpi.this.decodeKey(new DataInputStream(new CipherInputStream(dIn2, BcKeyStoreSpi.this.makePBECipher(BcKeyStoreSpi.KEY_CIPHER, BcKeyStoreSpi.STORE_VERSION, password, salt, dIn2.readInt()))));
                } catch (Exception e) {
                    ...
                    try {
                        ...
                        int iterationCount = dIn.readInt();
                        try {
                            k = BcKeyStoreSpi.this.decodeKey(new DataInputStream(new CipherInputStream(dIn, BcKeyStoreSpi.this.makePBECipher("BrokenPBEWithSHAAnd3-KeyTripleDES-CBC", BcKeyStoreSpi.STORE_VERSION, password, salt, iterationCount))));
                            dIn2 = dIn;
                            byteArrayInputStream = bIn;
                        } catch (Exception e3) {
                            try {
                                ...
                                k = BcKeyStoreSpi.this.decodeKey(new DataInputStream(new CipherInputStream(dIn2, BcKeyStoreSpi.this.makePBECipher("OldPBEWithSHAAnd3-KeyTripleDES-CBC", BcKeyStoreSpi.STORE_VERSION, password, salt, iterationCount))));
                            } catch (Exception e4) {
                                dIn2 = dIn;
                                throw new UnrecoverableKeyException("no match");
                            }
    ...
    protected void loadStore(InputStream in) throws IOException {
        ...
        for (int type = dataInputStream.read(); type > 0; type = dataInputStream.read()) {
            ...
            switch (type) {
                case KEY_PUBLIC /*1*/:
                    this.table.put(alias, new StoreEntry(alias, date, (int) KEY_PUBLIC, decodeCertificate(dataInputStream)));
                    break;
                case STORE_VERSION /*2*/:
                    this.table.put(alias, new StoreEntry(alias, date, STORE_VERSION, decodeKey(dataInputStream), certificateArr));
                    break;
                case SECRET /*3*/:
                case SEALED /*4*/:

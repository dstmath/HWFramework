...
public class PKCS12KeyStoreSpi extends KeyStoreSpi implements PKCSObjectIdentifiers, X509ObjectIdentifiers, BCKeyStore {
    ...
    private static class DefaultSecretKeyProvider {
        ...
        DefaultSecretKeyProvider() {
            ...
            keySizes.put(new ASN1ObjectIdentifier("1.2.840.113533.7.66.10"), Integers.valueOf(ReasonFlags.unused));
...
/*
        Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: com.android.org.bouncycastle.jcajce.provider.keystore.pkcs12.PKCS12KeyStoreSpi.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: com.android.org.bouncycastle.jcajce.provider.keystore.pkcs12.PKCS12KeyStoreSpi.<clinit>():void
...
*/
    ...
    private static byte[] getDigest(SubjectPublicKeyInfo spki) {
        ...
        digest.update(bytes, NULL, bytes.length);
        digest.doFinal(resBuf, NULL);
        return resBuf;
    }
    ...
    public void engineSetKeyEntry(String alias, Key key, char[] password, Certificate[] chain) throws KeyStoreException {
        if (!(key instanceof PrivateKey)) {
            ...
            if (chain != null) {
                this.certs.put(alias, chain[NULL]);
    ...
    protected PrivateKey unwrapKey(AlgorithmIdentifier algId, byte[] data, char[] password, boolean wrongPKCS12Zero) throws IOException {
        ...
        try {
            if (algorithm.on(PKCSObjectIdentifiers.pkcs_12PbeIds)) {
                ...
                cipher.init(SEALED, new PKCS12Key(password, wrongPKCS12Zero), defParams);
                return (PrivateKey) cipher.unwrap(data, "", KEY_SECRET);
            } else if (algorithm.equals(PKCSObjectIdentifiers.id_PBES2)) {
                return (PrivateKey) createCipher(SEALED, password, algId).unwrap(data, "", KEY_SECRET);
            } else {
    ...
    protected byte[] wrapKey(String algorithm, Key key, PKCS12PBEParams pbeParams, char[] password) throws IOException {
        ...
        try {
            ...
            cipher.init(SECRET, keyFact.generateSecret(pbeSpec), defParams);
    ...
    protected byte[] cryptData(boolean forEncryption, AlgorithmIdentifier algId, char[] password, boolean wrongPKCS12Zero, byte[] data) throws IOException {
        ASN1ObjectIdentifier algorithm = algId.getAlgorithm();
        int mode = forEncryption ? KEY_PUBLIC : KEY_SECRET;
        if (algorithm.on(PKCSObjectIdentifiers.pkcs_12PbeIds)) {
            PKCS12PBEParams pbeParams = PKCS12PBEParams.getInstance(algId.getParameters());
            PBEKeySpec pbeSpec = new PBEKeySpec(password);
            try {
    ...
    public void engineLoad(InputStream stream, char[] password) throws IOException {
        if (stream != null) {
            ...
            if (info.getContentType().equals(data)) {
                ...
                for (i = NULL; i != c.length; i += KEY_PUBLIC) {
                    ASN1Sequence seq;
                    int j;
                    ...
                    if (c[i].getContentType().equals(data)) {
                        ...
                        for (j = NULL; j != seq.size(); j += KEY_PUBLIC) {
                            ...
                            if (b.getBagId().equals(pkcs8ShroudedKeyBag)) {
                                ...
                                if (b.getBagAttributes() != null) {
                                    ...
                                    while (e.hasMoreElements()) {
                                        ...
                                        aOid = (ASN1ObjectIdentifier) sq.getObjectAt(NULL);
                                        attrSet = (ASN1Set) sq.getObjectAt(KEY_PUBLIC);
                                        aSN1Primitive = null;
                                        if (attrSet.size() > 0) {
                                            aSN1Primitive = (ASN1Primitive) attrSet.getObjectAt(NULL);
                        ...
                        for (j = NULL; j != seq.size(); j += KEY_PUBLIC) {
                            ...
                            if (b.getBagId().equals(certBag)) {
                                ...
                                while (e.hasMoreElements()) {
                                    ...
                                    aOid = ASN1ObjectIdentifier.getInstance(sq.getObjectAt(NULL));
                                    attrSet = ASN1Set.getInstance(sq.getObjectAt(KEY_PUBLIC));
                                    if (attrSet.size() > 0) {
                                        aSN1Primitive = (ASN1Primitive) attrSet.getObjectAt(NULL);
                                        existing = bagAttr.getBagAttribute(aOid);
                                        if (existing == null) {
                ...
                if (cb.getCertId().equals(x509Certificate)) {
                    try {
                        ...
                        if (b.getBagAttributes() != null) {
                            ...
                            while (e.hasMoreElements()) {
                                sq = ASN1Sequence.getInstance(e.nextElement());
                                ASN1ObjectIdentifier oid = ASN1ObjectIdentifier.getInstance(sq.getObjectAt(NULL));
                                attrSet = ASN1Set.getInstance(sq.getObjectAt(KEY_PUBLIC));
                                if (attrSet.size() > 0) {
                                    aSN1Primitive = (ASN1Primitive) attrSet.getObjectAt(NULL);
    ...
    private void doStore(OutputStream stream, char[] password, boolean useDEREncoding) throws IOException {
        ...
        while (ks.hasMoreElements()) {
            ...
            byte[] kBytes = wrapKey(this.keyAlgorithm.getId(), privKey, pKCS12PBEParams, password);
            EncryptedPrivateKeyInfo encryptedPrivateKeyInfo = new EncryptedPrivateKeyInfo(new AlgorithmIdentifier(this.keyAlgorithm, pKCS12PBEParams.toASN1Primitive()), kBytes);
        ...
        ContentInfo[] info = new ContentInfo[KEY_SECRET];
        info[NULL] = new ContentInfo(data, bEROctetString);
        info[KEY_PUBLIC] = new ContentInfo(encryptedData, encryptedData.toASN1Primitive());
        AuthenticatedSafe authenticatedSafe = new AuthenticatedSafe(info);
        ...
        try {
            byte[] res = calculatePbeMac(id_SHA1, mSalt, MIN_ITERATIONS, password, false, ((ASN1OctetString) contentInfo.getContent()).getOctets());
            Pfx pfx = new Pfx(contentInfo, new MacData(new DigestInfo(new AlgorithmIdentifier(id_SHA1, DERNull.INSTANCE), res), mSalt, MIN_ITERATIONS));
            if (useDEREncoding) {
    ...
    private Set getUsedCertificateSet() {
        ...
        while (en.hasMoreElements()) {
            ...
            for (int i = NULL; i != certs.length; i += KEY_PUBLIC) {
                usedSet.add(certs[i]);
            }
        }
        en = this.certs.keys();

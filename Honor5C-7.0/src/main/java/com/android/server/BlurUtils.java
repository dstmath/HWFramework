...
public class BlurUtils {
    public static Bitmap stackBlur(Bitmap input, int radius) {
        ...
        int divsum = (div + 1) >> 1;
        divsum *= divsum;
        int[] dv = new int[(divsum * HwGlobalActionsData.FLAG_SILENTMODE_SILENT)];
        for (i = 0; i < divsum * HwGlobalActionsData.FLAG_SILENTMODE_SILENT; i++) {
            dv[i] = i / divsum;
        }
        int yi = 0;
        int yw = 0;
        ...
        for (y = 0; y < h; y++) {
            ...
            for (i = -radius; i <= radius; i++) {
                ...
                sir[0] = (16711680 & pixel) >> 16;
                sir[1] = (65280 & pixel) >> 8;
                sir[2] = pixel & Utils.MAXINUM_TEMPERATURE;

...
public class GestureLauncherService extends SystemService {
    ...
    /* renamed from: com.android.server.GestureLauncherService.2 */
    ...
    private final class GestureEventListener implements SensorEventListener {
        ...
        public void onSensorChanged(SensorEvent event) {
            if (GestureLauncherService.this.mRegistered && event.sensor == GestureLauncherService.this.mCameraLaunchSensor && GestureLauncherService.this.handleCameraLaunchGesture(true, 0)) {
                MetricsLogger.action(GestureLauncherService.this.mContext, DumpState.DUMP_SHARED_USERS);
    ...
    public GestureLauncherService(Context context) {
        ...
        this.mSettingObserver = new AnonymousClass2(new Handler());
    ...
    public void onBootPhase(int phase) {
        if (phase == NetdResponseCode.InterfaceChange && isGestureLauncherEnabled(this.mContext.getResources())) {
    ...
    private void registerContentObservers() {
        this.mContext.getContentResolver().registerContentObserver(Secure.getUriFor("camera_gesture_disabled"), DBG, this.mSettingObserver, this.mUserId);
        this.mContext.getContentResolver().registerContentObserver(Secure.getUriFor("camera_double_tap_power_gesture_disabled"), DBG, this.mSettingObserver, this.mUserId);
    }
    ...
    private void registerCameraLaunchGesture(Resources resources) {
        if (!this.mRegistered) {
            ...
            if (cameraLaunchGestureId != -1) {
                this.mRegistered = DBG;
                String sensorName = resources.getString(17039466);
    ...
    public static boolean isCameraLaunchEnabled(Resources resources) {
        ...
        if (!configSet || SystemProperties.getBoolean("gesture.disable_camera_launch", DBG)) {
            return DBG;
        }
        return true;
    }
    ...
    public static boolean isCameraDoubleTapPowerEnabled(Resources resources) {
        return resources.getBoolean(17957035);
    }
    ...
    public boolean interceptPowerKeyDown(KeyEvent event, boolean interactive, MutableBoolean outLaunched) {
        ...
        boolean launched = DBG;
        boolean intercept = DBG;
        synchronized (this) {
        ...
        if (launched) {
            ...
            launched = handleCameraLaunchGesture(DBG, 1);
            if (launched) {
                MetricsLogger.action(this.mContext, RampAnimator.DEFAULT_MAX_BRIGHTNESS, (int) doubleTapInterval);
            }
        }
        MetricsLogger.histogram(this.mContext, "power_double_tap_interval", (int) doubleTapInterval);
        outLaunched.value = launched;
        return intercept ? launched : DBG;
    }

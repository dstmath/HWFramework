...
public class HwInputMethodManagerService extends InputMethodManagerService {
    ...
    private static final boolean isSupportedSecIme;
    private boolean isSecMethodUsing;
    private boolean isSecureIMEEnabled;
    private boolean isSecureIMEExist;
    ...
    private class SecureSettingsObserver extends ContentObserver {
        ...
        public SecureSettingsObserver() {
            super(new Handler());
            this.mRegistered = HwInputMethodManagerService.IS_CHINA_AREA;
        }
        ...
        public void registerContentObserverInner(int userId) {
            ...
            if (!this.mRegistered || this.mUserId != userId) {
                ...
                this.mUserId = userId;
                resolver.registerContentObserver(Secure.getUriFor(HwInputMethodManagerService.SETTINGS_SECURE_KEYBOARD_CONTROL), HwInputMethodManagerService.IS_CHINA_AREA, this, userId);
        ...
        public void onChange(boolean selfChange, Uri uri) {
            if (uri != null) {
                ...
                if (Secure.getUriFor(HwInputMethodManagerService.SETTINGS_SECURE_KEYBOARD_CONTROL).equals(uri)) {
    ...
    static {
        ...
        isSupportedSecIme = IS_CHINA_AREA;
        ENGLISH_LOCALE = new Locale("en");
    }

    public HwInputMethodManagerService(Context context) {
        super(context);
        this.isSecureIMEExist = IS_CHINA_AREA;
        this.isSecureIMEEnabled = IS_CHINA_AREA;
        this.isSecMethodUsing = IS_CHINA_AREA;
    }
    ...
    protected void createFlagIfNecessary(int newUserId) {
        ...
        boolean existFlag = IS_CHINA_AREA;
        try {
            if (flagFile.exists()) {
                existFlag = true;
            } else {
    ...
    protected void ensureEnableSystemIME(String id, InputMethodInfo p, Context context, int userId) {
        if (!isFlagExists(userId)) {
            if (isValidSystemDefaultIme(p, context) || isSystemImeThatHasEnglishSubtype(p)) {
    ...
    private static boolean isSystemIme(InputMethodInfo inputMethod) {
        return (inputMethod.getServiceInfo().applicationInfo.flags & 1) != 0 ? true : IS_CHINA_AREA;
    }
    ...
    public boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
        switch (code) {
            case TRANSACTION_isUseSecureIME /*1001*/:
    ...
    private boolean isSecureIMEEnable() {
        return Secure.getIntForUser(this.mContext.getContentResolver(), SETTINGS_SECURE_KEYBOARD_CONTROL, 1, this.mSettings.getCurrentUserId()) == 1 ? true : IS_CHINA_AREA;
    }
    ...
    public void setImeWindowStatus(IBinder token, int vis, int backDisposition) {
        ...
        try {
            this.mSecurityInputMethodService.setImeWindowStatus(token, vis, backDisposition);
        } catch (RemoteException e) {
        }
    }
    ...
    public InputBindResult startInputOrWindowGainedFocus(int startInputReason, IInputMethodClient client, IBinder windowToken, int controlFlags, int softInputMode, int windowFlags, EditorInfo attribute, IInputContext inputContext, int missingMethods) {
        if (windowToken == null) {
            ...
            if (isPasswordType(attribute)) {
                ...
                this.isSecMethodUsing = true;
                super.startInputOrWindowGainedFocus(startInputReason, client, windowToken, controlFlags, softInputMode, windowFlags, attribute, inputContext, missingMethods);
                ...
                try {
                    ...
                    this.mSecurityInputMethodService.startInputOrWindowGainedFocus(UNBIND_SECIME_IF_SHOULD, client, windowToken, controlFlags, softInputMode, windowFlags, attribute, inputContext, missingMethods);
                    return inputBindRes;
    ...
    public boolean showSoftInput(IInputMethodClient client, int flags, ResultReceiver resultReceiver) {
        if (isUseSecureIME() && this.isSecMethodUsing) {
            try {
                hideSoftInput(client, SECURE_IME_NO_HIDE_FLAG, resultReceiver);
    ...
    public boolean hideSoftInput(IInputMethodClient client, int flags, ResultReceiver resultReceiver) {
        if (isUseSecureIME() && this.isSecMethodUsing) {
            if (SECURE_IME_NO_HIDE_FLAG != flags) {
                try {
    ...
    protected int getNaviBarEnabledDefValue() {
        int defValue;
        boolean FRONT_FINGERPRINT_NAVIGATION = SystemProperties.getBoolean("ro.config.hw_front_fp_navi", IS_CHINA_AREA);
        ...
        if (FRONT_FINGERPRINT_NAVIGATION) {
            ...
            if (FRONT_FINGERPRINT_NAVIGATION_TRIKEY == 1 && isTrikeyExist) {
                defValue = 0;
            } else if (SystemProperties.get("ro.config.hw_optb", PPPOEStateMachine.PHASE_DEAD).equals("156")) {
                defValue = 0;
            } else {
                defValue = 1;
            }

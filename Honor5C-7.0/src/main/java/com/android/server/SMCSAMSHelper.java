...
public final class SMCSAMSHelper {
    ...
    private static final boolean DBG_PERFORMANCE = false;
    ...
    private static final int STP_EVENT_MAX_NUM = 0;
    private static final String TAG = "SMCSAMSHelper";
    private static SMCSAMSHelper mSelf;
    HwActivityManagerService mAms;
    private ArrayList<SmartTrimProcessEvent> mSTPEvent;
    private long mSelfAutoTrimId;
    private HashSet<String> mSelfAutoTrimedProcs;

    static {
        /* JADX: method processing error */
/*
        Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: com.android.server.SMCSAMSHelper.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: com.android.server.SMCSAMSHelper.<clinit>():void
...
*/
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: com.android.server.SMCSAMSHelper.<clinit>():void");
    }

    private SMCSAMSHelper() {
        this.mSelfAutoTrimedProcs = null;
        this.mSelfAutoTrimId = 0;
        this.mSTPEvent = new ArrayList();
    ...
    public boolean handleTransact(Parcel data, Parcel reply, int flag) {
        ...
        switch (iEvent) {
            case SMCS_AMS_GET_RUNNING_PROCESS /*1*/:
                handleGetRunningProcesses(data, reply, flag);
                break;
            case SMCS_AMS_MOVE_ST_EVENTS /*2*/:
                handleMoveSTEvents(data, reply, flag);
                break;
            case SMCS_AMS_TRIM_PROCESSES /*3*/:
                handleTrimProcesses(data, reply, flag);
                break;
            case SMCS_AMS_TRIM_PKGS /*4*/:
                handleTrimPkgs(data, reply, flag);
                break;
            case SMCS_AMS_MOVE_SELF_AUTO_TRIMED_PROCS /*5*/:
                handleMoveSelfAutoTrimedProcs(data, reply);
                break;
            case SMCS_AMS_GET_MAX_OOM_MIN_FREE /*6*/:
    ...
    public void smartTrimAddProcessRelation(String clientProc, int clientCurAdj, ArrayMap<String, ProcessStateHolder> clientPkgMap, String serverProc, int serverCurAdj, ArrayMap<String, ProcessStateHolder> serverPkgMap) {
        try {
            ...
            for (i = STP_EVENT_MAX_NUM; i < clientPkgMap.size(); i += SMCS_AMS_GET_RUNNING_PROCESS) {
                clientPkgList.add((String) clientPkgMap.keyAt(i));
            }
            for (i = STP_EVENT_MAX_NUM; i < serverPkgMap.size(); i += SMCS_AMS_GET_RUNNING_PROCESS) {
            ...
            synchronized (this.mSTPEvent) {
                if (e != null) {
                    if (this.mSTPEvent.size() >= STP_EVENT_MAX_NUM) {
                        ...
                        this.mSTPEvent.remove(STP_EVENT_MAX_NUM);
                    }
                    if (DBG) {
    ...
    private void dumpSmartTrimProcessEvent(ArrayList<SmartTrimProcessEvent> events, String sLog) {
        ...
        for (int i = STP_EVENT_MAX_NUM; i < size; i += SMCS_AMS_GET_RUNNING_PROCESS) {

...
public final class ContentService extends Stub {
    ...
    private static HwSysResource mContentObserverResource;
    private static PackageManager mPackageManager;
    @GuardedBy("mCache")
    private final SparseArray<ArrayMap<String, ArrayMap<Pair<String, Uri>, Bundle>>> mCache;
    private BroadcastReceiver mCacheReceiver;
    ...
    /* renamed from: com.android.server.content.ContentService.2 */
    class AnonymousClass2 implements Comparator<Integer> {
        final /* synthetic */ SparseIntArray val$pidCounts;

        AnonymousClass2(SparseIntArray val$pidCounts) {
            this.val$pidCounts = val$pidCounts;
        }
    ...
    public static class Lifecycle extends SystemService {
        ...
        public void onStart() {
            this.mService = new ContentService(getContext(), FactoryTest.getMode() == 1 ? true : ContentService.DEBUG);
    ...
    public static final class ObserverNode {
        ...
        private void addObserverLocked(Uri uri, int index, IContentObserver observer, boolean notifyForDescendants, Object observersLock, int uid, int pid, int userHandle) {
            if (index == countUriSegments(uri)) {
                ...
                if (ContentService.mContentObserverResource == null) {
                    ContentService.mContentObserverResource = HwFrameworkFactory.getHwResource(35);
                }
        ...
        private void collectMyObserversLocked(boolean leaf, IContentObserver observer, boolean observerWantsSelfNotifications, int flags, int targetUserHandle, ArrayList<ObserverCall> calls) {
            ...
            for (int i = INSERT_TYPE; i < N; i += UPDATE_TYPE) {
                ObserverEntry entry = (ObserverEntry) this.mObservers.get(i);
                boolean selfChange = entry.observer.asBinder() == asBinder ? true : ContentService.DEBUG;
                if ((!selfChange || observerWantsSelfNotifications) && !HwServiceFactory.getHwNLPManager().shouldSkipGoogleNlp(entry.pid)) {
                    if (!(targetUserHandle == -1 || entry.userHandle == -1)) {
                        if (targetUserHandle == entry.userHandle) {
                        }
                    }
                    if (leaf) {
                        if ((flags & DELETE_TYPE) != 0 && entry.notifyForDescendants) {
                        }
        ...
        public void collectObserversLocked(Uri uri, int index, IContentObserver observer, boolean observerWantsSelfNotifications, int flags, int targetUserHandle, ArrayList<ObserverCall> calls) {
            ...
            if (index >= segmentCount) {
                ...
                collectMyObserversLocked(ContentService.DEBUG, observer, observerWantsSelfNotifications, flags, targetUserHandle, calls);
            }
            int N = this.mChildren.size();
            for (int i = INSERT_TYPE; i < N; i += UPDATE_TYPE) {
                ObserverNode node = (ObserverNode) this.mChildren.get(i);
                if (segment == null || node.mName.equals(segment)) {
                    node.collectObserversLocked(uri, index + UPDATE_TYPE, observer, observerWantsSelfNotifications, flags, targetUserHandle, calls);
                    if (segment != null) {
                        return;
                    }
                }
...
/*
        Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: com.android.server.content.ContentService.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: com.android.server.content.ContentService.<clinit>():void
...
*/
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: com.android.server.content.ContentService.<clinit>():void");
    }

    private SyncManager getSyncManager() {
        if (SystemProperties.getBoolean("config.disable_network", DEBUG)) {
            return null;
        }
    ...
    protected synchronized void dump(FileDescriptor fd, PrintWriter pw_, String[] args) {
        ...
        try {
            ...
            synchronized (this.mRootNode) {
                ...
                Collections.sort(sorted, new AnonymousClass2(pidCounts));
    ...
    ContentService(Context context, boolean factoryTest) {
        ...
        this.mCacheReceiver = new BroadcastReceiver() {
    ...
    public void requestSync(Account account, String authority, Bundle extras) {
        ...
        try {
            SyncManager syncManager = getSyncManager();
            if (syncManager != null) {
                syncManager.scheduleSync(account, userId, uId, authority, extras, 0, 0, DEBUG);
            }
            restoreCallingIdentity(identityToken);
        } catch (Throwable th) {
            restoreCallingIdentity(identityToken);
        ...
        try {
            ...
            if (syncManager != null) {
                ...
                if (request.isPeriodic()) {
                    ...
                    syncManager.scheduleSync(request.getAccount(), userId, callerUid, request.getProvider(), extras, flextime * 1000, runAtTime * 1000, DEBUG);
                }
                restoreCallingIdentity(identityToken);
            }
        } finally {
    ...
    public List<SyncInfo> getCurrentSyncsAsUser(int userId) {
        enforceCrossUserPermission(userId, "no permission to read the sync settings for user " + userId);
        this.mContext.enforceCallingOrSelfPermission("android.permission.READ_SYNC_STATS", "no permission to read the sync stats");
        boolean canAccessAccounts = this.mContext.checkCallingOrSelfPermission("android.permission.GET_ACCOUNTS") == 0 ? true : DEBUG;
        long identityToken = clearCallingIdentity();
        try {

...
public class NetworkStatsObservers {
    ...
    private final SparseArray<RequestInfo> mDataUsageRequests;
    private Handler mHandler;
    private Callback mHandlerCallback;
    private final AtomicInteger mNextDataUsageRequestId;
    ...
    private static abstract class RequestInfo implements DeathRecipient {
        ...
        public void binderDied() {
            this.mStatsObserver.unregister(this.mRequest, ProcessList.PSS_SAFE_TIME_FROM_STATE_CHANGE);
            callCallback(NetworkStatsObservers.MSG_REGISTER);
        }
        ...
        private String callbackTypeToName(int callbackType) {
            switch (callbackType) {
                ...
                case NetworkStatsObservers.MSG_REGISTER /*1*/:
                    return "RELEASED";
                default:
                    return "UNKNOWN";
            }
    ...
    private static class NetworkUsageRequestInfo extends RequestInfo {
        ...
        protected boolean checkStats() {
            ...
            return NetworkStatsObservers.LOGV;
        }
    ...
    private static class UserUsageRequestInfo extends RequestInfo {
        ...
        protected boolean checkStats() {
            ...
            for (int i = 0; i < uidsToMonitor.length; i += NetworkStatsObservers.MSG_REGISTER) {
                if (getTotalBytesForNetworkUid(this.mRequest.template, uidsToMonitor[i]) > this.mRequest.thresholdInBytes) {
                    return true;
                }
            }
            return NetworkStatsObservers.LOGV;
    ...
    public NetworkStatsObservers() {
        this.mDataUsageRequests = new SparseArray();
        this.mNextDataUsageRequestId = new AtomicInteger();
            ...
            public boolean handleMessage(Message msg) {
                switch (msg.what) {
                    case NetworkStatsObservers.MSG_REGISTER /*1*/:
                        ...
                        return true;
                    case NetworkStatsObservers.MSG_UNREGISTER /*2*/:
                        NetworkStatsObservers.this.handleUnregister((DataUsageRequest) msg.obj, msg.arg1);
                        return true;
                    case NetworkStatsObservers.MSG_UPDATE_STATS /*3*/:
    ...
    public DataUsageRequest register(DataUsageRequest inputRequest, Messenger messenger, IBinder binder, int callingUid, int accessLevel) {
        ...
        getHandler().sendMessage(this.mHandler.obtainMessage(MSG_REGISTER, buildRequestInfo(request, messenger, binder, callingUid, accessLevel)));
        return request;
    }

    public void unregister(DataUsageRequest request, int callingUid) {
        getHandler().sendMessage(this.mHandler.obtainMessage(MSG_UNREGISTER, callingUid, 0, request));
    }
    ...
    public void updateStats(NetworkStats xtSnapshot, NetworkStats uidSnapshot, ArrayMap<String, NetworkIdentitySet> activeIfaces, ArrayMap<String, NetworkIdentitySet> activeUidIfaces, VpnInfo[] vpnArray, long currentTime) {
        getHandler().sendMessage(this.mHandler.obtainMessage(MSG_UPDATE_STATS, new StatsContext(xtSnapshot, uidSnapshot, activeIfaces, activeUidIfaces, vpnArray, currentTime)));
    }
    ...
    private void handleUnregister(DataUsageRequest request, int callingUid) {
        ...
        if (requestInfo != null) {
            if (ProcessList.PSS_SAFE_TIME_FROM_STATE_CHANGE == callingUid || requestInfo.mCallingUid == callingUid) {
                this.mDataUsageRequests.remove(request.requestId);
                requestInfo.unlinkDeathRecipient();
                requestInfo.callCallback(MSG_REGISTER);
                return;
            }
    ...
    private void handleUpdateStats(StatsContext statsContext) {
        if (this.mDataUsageRequests.size() != 0) {
            for (int i = 0; i < this.mDataUsageRequests.size(); i += MSG_REGISTER) {

...
public class PermissionProcessor extends BaseProcessor {
    ...
    public boolean readCert(String line, CertificationData rawCert) {
        ...
        if (key == null || key.isEmpty()) {
            HwAuthLogger.e(Utils.TAG, "readCert:permission is empty");
            return false;
        }
    ...
    public boolean verifyCert(Package pkg, HwCertification cert) {
        if (HwAuthLogger.getHWFLOW()) {
            HwAuthLogger.i(Utils.TAG, "--Verify Permission--");
        }
        if (HwAuthLogger.getHWFLOW()) {
            HwAuthLogger.i(Utils.TAG, "verifyCert:permission line ok");
        }
        return true;
    }
    ...
    public boolean parseXmlTag(String tag, XmlPullParser parser, HwCertification cert) {
        ...
        cert.mCertificationData.mPermissionsString = parser.getAttributeValue(null, HwCertXmlHandler.TAG_VALUE);
        return true;
    }
}

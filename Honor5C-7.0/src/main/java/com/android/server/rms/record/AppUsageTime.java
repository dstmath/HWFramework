...
public class AppUsageTime implements Reader, Rewriter {
    ...
    private final HashMap<String, UsageInfo> mUsageTimeMap;
    ...
    static final class UsageInfo {
        ...
        private long getTotalBytesNum() {
            ...
            return (long) (pkgByteSize + ENTRY_BYTE_SIZE);
        }
    }

    public AppUsageTime(Context context, int time) {
        this.mUsageTimeMap = new HashMap();
        ...
        for (Entry entry : this.mUsageTimeMap.entrySet()) {
            ...
            if (info != null) {
                if (historyTimeDump) {
                    ...
                    Log.i(TAG, "dumpInfo: pkg " + info.mPkg + ", usageTime is " + ((info.mUsageTime - info.mHistoryTime) / 1000) + "s");
                }
            }
        }
    }
        ...
        if (!this.mUsageTimeMap.isEmpty()) {
            ...
            for (Entry entry : this.mUsageTimeMap.entrySet()) {
                ...
                if (info != null && info.mUsageTime > 0) {
                    dataOutStream.writeUTF(info.mPkg != null ? info.mPkg : AppHibernateCst.INVALID_PKG);

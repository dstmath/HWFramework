...
public abstract class TLVInvoker<T> implements ICommand {
    ...
    private boolean parseTLVInternal(ByteBuffer buffer) {
        if (buffer == null || buffer.remaining() < 4) {
            LogHelper.e(ICommand.TAG, "error_tlv: remaining tlv is too short:" + buffer.remaining());
            return false;
        }
        ...
        if (isTypeExists(tag)) {
            ...
            LogHelper.d(ICommand.TAG, "Vlength:" + claimedValueLength + " type:" + (clazz == null ? HwCertification.SIGNATURE_DEFAULT : clazz.getSimpleName()) + "\nvalue: " + ByteUtil.type2HexString(this.mType));
            return true;
        }

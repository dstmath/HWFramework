...
class SipSessionGroup implements SipListener {
    ...
    static class KeepAliveProcessCallbackProxy implements KeepAliveProcessCallback {
        ...
        class AnonymousClass1 implements Runnable {
            ...
            AnonymousClass1(boolean val$portChanged) {
                this.val$portChanged = val$portChanged;
            }

            public void run() {
                try {
                    KeepAliveProcessCallbackProxy.this.mCallback.onResponse(this.val$portChanged);
                } catch (Throwable t) {
        ...
        class AnonymousClass2 implements Runnable {
            ...
            AnonymousClass2(int val$errorCode, String val$description) {
            ...
            public void run() {
                try {
                    KeepAliveProcessCallbackProxy.this.mCallback.onError(this.val$errorCode, this.val$description);
                } catch (Throwable t) {
        ...
        public void onResponse(boolean portChanged) {
            if (this.mCallback != null) {
                proxy(new AnonymousClass1(portChanged));
            }
        }

        public void onError(int errorCode, String description) {
            if (this.mCallback != null) {
                proxy(new AnonymousClass2(errorCode, description));
            }
        }
    ...
    class SipSessionImpl extends Stub {
        ...
        class AnonymousClass1 implements Runnable {
            ...
            AnonymousClass1(EventObject val$command) {
                this.val$command = val$command;
            }

            public void run() {
                try {
                    SipSessionImpl.this.processCommand(this.val$command);
                } catch (Throwable e) {
                    SipSessionGroup.this.loge("command error: " + this.val$command + ": " + SipSessionGroup.this.mLocalProfile.getUriString(), SipSessionGroup.this.getRootCause(e));
        ...
        class SessionTimer {
            ...
            class AnonymousClass1 implements Runnable {
                ...
                AnonymousClass1(int val$timeout) {
                    this.val$timeout = val$timeout;
                }

                public void run() {
                    SessionTimer.this.sleep(this.val$timeout);
            ...
            SessionTimer() {
                this.mRunning = SipSessionImpl.SSI_DBG;
            }

            void start(int timeout) {
                new Thread(new AnonymousClass1(timeout), "SipSessionTimerThread").start();
            }
        ...
        class SipKeepAlive extends SipSessionAdapter implements Runnable {
            ...
            SipKeepAlive() {
                this.mRunning = SipSessionGroup.DBG_PING;
                this.mPortChanged = SipSessionGroup.DBG_PING;
                this.mRPort = 0;
            }
            ...
            private void sendKeepAlive() throws SipException {
                synchronized (SipSessionGroup.this) {
                    ...
                    SipSessionImpl.this.startSessionTimer(SipSessionGroup.KEEPALIVE_TIMEOUT);
                }
            }
        ...
        private void doCommandAsync(EventObject command) {
            new Thread(new AnonymousClass1(command), "SipSessionAsyncCmdThread").start();
        }
                ...
                if (SipSessionGroup.this.isClosed()) {
                    ...
                    switch (this.mState) {
                        case State.READY_TO_CALL /*0*/:
                            ...
                            break;
                        case State.REGISTERING /*1*/:
                        case State.DEREGISTERING /*2*/:
                            processed = registeringToReady(evt);
                            break;
                        case SipSessionGroup.END_CALL_TIMER /*3*/:
                            processed = incomingCall(evt);
                            break;
                        case State.INCOMING_CALL_ANSWERING /*4*/:
                            processed = incomingCallToInCall(evt);
                            break;
                        case SipSessionGroup.KEEPALIVE_TIMEOUT /*5*/:
                        ...
                        case State.IN_CALL /*8*/:
                            ...
                            break;
                        case SipSessionGroup.INCALL_KEEPALIVE_INTERVAL /*10*/:
        ...
        private boolean readyForCall(EventObject evt) throws SipException {
            if (evt instanceof MakeCallCommand) {
                this.mState = SipSessionGroup.KEEPALIVE_TIMEOUT;
        ...
        private boolean outgoingCall(EventObject evt) throws SipException {
            if (SipSessionGroup.expectResponse("INVITE", evt)) {
                ...
                switch (statusCode) {
                    ...
                    case 182:
                    case 183:
                        if (this.mState == SipSessionGroup.KEEPALIVE_TIMEOUT) {
                            this.mState = 6;
                            cancelSessionTimer();
                    ...
                    default:
                        ...
                        if (statusCode < 400) {
                            return statusCode >= 300 ? SipSessionGroup.DBG_PING : SSI_DBG;
                        } else {
                            onError(response);
                            return SSI_DBG;
                        }
                ...
                startSessionTimer(SipSessionGroup.END_CALL_TIMER);
                return SSI_DBG;
        ...
        private boolean inCall(EventObject evt) throws SipException {
            if (SipSessionGroup.END_CALL == evt) {
                this.mState = SipSessionGroup.INCALL_KEEPALIVE_INTERVAL;
                SipSessionGroup.this.mSipHelper.sendBye(this.mDialog);
                this.mProxy.onCallEnded(this);
                startSessionTimer(SipSessionGroup.END_CALL_TIMER);
                return SSI_DBG;
            } else if (SipSessionGroup.isRequestEvent("INVITE", evt)) {
                this.mState = SipSessionGroup.END_CALL_TIMER;
        ...
        private void enableKeepAlive() {
            ...
            try {
                this.mSipSessionImpl.startKeepAliveProcess(SipSessionGroup.INCALL_KEEPALIVE_INTERVAL, this.mPeerProfile, null);
    ...
    private class SipSessionCallReceiverImpl extends SipSessionImpl {
        ...
        private void processNewInviteRequest(RequestEvent event) throws SipException {
            ...
            if (replaces != null) {
                ...
                if (response == 200) {
                    newSession = SipSessionGroup.this.createNewSession(event, ((SipSessionImpl) SipSessionGroup.this.mSessionMap.get(replaces.getCallId())).mProxy.getListener(), SipSessionGroup.this.mSipHelper.getServerTransaction(event), SipSessionGroup.END_CALL_TIMER);
                ...
                newSession = SipSessionGroup.this.createNewSession(event, this.mProxy, SipSessionGroup.this.mSipHelper.sendRinging(event, generateTag()), SipSessionGroup.END_CALL_TIMER);
...
/*
        Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: com.android.server.sip.SipSessionGroup.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: com.android.server.sip.SipSessionGroup.<clinit>():void
...
*/
    ...
    synchronized void reset() throws SipException {
        ...
        if (str == null) {
            ...
            properties.setProperty("gov.nist.javax.sip.THREAD_POOL_SIZE", THREAD_POOL_SIZE);
            this.mSipStack = SipFactory.getInstance().createSipStack(properties);
            SipProvider provider2 = this.mSipStack.createSipProvider(this.mSipStack.createListeningPoint(str, port, protocol));
            provider2.addSipListener(this);
            this.mSipHelper = new SipHelper(this.mSipStack, provider2);
            this.mSipStack.start();
        }
    }
    ...
    public synchronized boolean isClosed() {
        return this.mSipStack == null ? true : DBG_PING;
    }
    ...
    private synchronized void process(EventObject event) {
        ...
        try {
            ...
            boolean process = session != null ? session.process(event) : DBG_PING;
    ...
    private static boolean isLoggable(SipSessionImpl s, EventObject evt) {
        if (isLoggable(s) && evt != null) {
            return evt instanceof ResponseEvent ? "OPTIONS".equals(((ResponseEvent) evt).getResponse().getHeader("CSeq")) ? DBG_PING : DBG_PING : ((evt instanceof RequestEvent) && isRequestEvent("OPTIONS", evt)) ? DBG_PING : DBG_PING;
        } else {
            return DBG_PING;
        }
    }

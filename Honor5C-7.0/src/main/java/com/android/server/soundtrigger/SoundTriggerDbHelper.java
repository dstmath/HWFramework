...
public class SoundTriggerDbHelper extends SQLiteOpenHelper {
    ...
    public boolean updateGenericSoundModel(GenericSoundModel soundModel) {
        ...
        synchronized (this) {
            ...
            try {
                z = db.insertWithOnConflict(GenericSoundModelContract.TABLE, null, values, 5) != -1 ? true : DBG;
                db.close();
            } catch (Throwable th) {
                db.close();
            }
    ...
    public GenericSoundModel getGenericSoundModel(UUID model_uuid) {
        synchronized (this) {
            ...
            try {
                if (c.moveToFirst()) {
                    GenericSoundModel genericSoundModel = new GenericSoundModel(model_uuid, UUID.fromString(c.getString(c.getColumnIndex(GenericSoundModelContract.KEY_VENDOR_UUID))), c.getBlob(c.getColumnIndex(SoundModelContract.KEY_DATA)));
                    c.close();
                    db.close();

...
public class AndroidMaskFilter extends Filter {
    ...
    public void process(FilterContext context) {
        ...
        for (int i = 0; i < num_face; i++) {
            ...
            float s1 = ((float) androidFormat.getWidth()) / ((float) androidFormat.getHeight());
            ...
            Quad region = new Quad(p0, new Point(s, 0.0f).minus(c).rotated(angle).mult(1.0f, s2), p1, p2).translated(center);
            this.mOverlayProgram.setTargetRegion(region);

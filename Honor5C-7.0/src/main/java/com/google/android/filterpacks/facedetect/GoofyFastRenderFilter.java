...
public class GoofyFastRenderFilter extends Filter {
    ...
    public void process(FilterContext context) {
        ...
        if (this.mBigEyesProgram == null) {
            ...
            if (aspectRatio[SQUEEZE] == this.mAspect[SQUEEZE]) {
            }
            this.mAspect = aspectRatio;
            createMesh(context);
        }
        ...
        if (face_count > 0) {
            ...
            if (this.mEnableAnimation) {
                if (this.mAnimateCurrent < this.mDistortionAmount) {
                    if (this.mCurrentTimeStamp > 0) {
                        if (this.mAnimateCurrent == 0.0f) {
                            this.mAnimationStartTimeStamp = this.mCurrentTimeStamp;
                            this.mAnimateCurrent = 0.001f;
                        } else {
                            float f = (float) ((this.mCurrentTimeStamp - this.mAnimationStartTimeStamp) / 1000000);
                            this.mAnimateCurrent = (r0 / 2000.0f) * this.mDistortionAmount;
                        }
                    } else {

...
public class PageTransition extends BaseTransition {
    ...
    public boolean transform(int part, boolean isOverScrollFirst, boolean isOverScrollLast, float scrollProgress, View child) {
        ...
        if (this.mOrientation == 0) {
            this.mTransformationInfo.mBounds.set(rect.left + ((rect.width() / 2) * part), rect.top, rect.left + ((rect.width() / 2) * (part + PAGE_RIGHT)), rect.bottom);
        } else {
            this.mTransformationInfo.mBounds.set(PAGE_LEFT, (rect.height() / 2) * part, rect.width(), (rect.height() / 2) * (part + PAGE_RIGHT));
        }

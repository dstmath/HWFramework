...
public abstract class AbsAdRequest extends AbsRequest {
    ...
    public AbsAdRequest() {
        this.mConnectTimeout = READ_TIME_OUT;
        this.mReadTimeout = READ_TIME_OUT;
    }
    ...
    protected void doRequest(Context ctx) {
        int retryCount = 0;
        Log.d(TAG, "doRequest");
        while (retryCount < SYSTEM_32) {
            if (!innerConnectRequest(ctx)) {
                retryCount += SYSTEM_32;
            } else {
                return;
            }
        }
    ...
    private String getPostRequestParam(Context context) {
        try {
            ...
            if (isNeedDefaultParam()) {
                ...
                map.put(PARAM_VERSION, getSelfVersion(context));

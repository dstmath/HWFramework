    ...
    private static void thresholdBlock(byte[] luminances, int xoffset, int yoffset, int threshold, int stride, BitMatrix matrix) {
        ...
        while (y < BLOCK_SIZE) {
            for (int x = 0; x < BLOCK_SIZE; x++) {
                if ((luminances[offset + x] & IccConstantsEx.RUIM_SMS_BEARER_DATA_LEN) <= threshold) {
    ...
    private static int[][] calculateBlackPoints(byte[] luminances, int subWidth, int subHeight, int width, int height) {
        ...
        for (int y = 0; y < subHeight; y++) {
            ...
            while (x < subWidth) {
                ...
                int sum = 0;
                int min = IccConstantsEx.RUIM_SMS_BEARER_DATA_LEN;
                int max = 0;
                int yy = 0;
                ...
                while (yy < BLOCK_SIZE) {
                    ...
                    for (xx = 0; xx < BLOCK_SIZE; xx++) {
                        int pixel = luminances[offset + xx] & IccConstantsEx.RUIM_SMS_BEARER_DATA_LEN;
                        sum += pixel;
                        if (pixel < min) {
                            min = pixel;
                        }
                    ...
                    if (max - min > MIN_DYNAMIC_RANGE) {
                        ...
                        while (yy < BLOCK_SIZE) {
                            for (xx = 0; xx < BLOCK_SIZE; xx++) {
                                sum += luminances[offset + xx] & IccConstantsEx.RUIM_SMS_BEARER_DATA_LEN;
                            }
                            yy++;
                            offset += width;
                        }

...
public final class MonochromeRectangleDetector {
    ...
    public ResultPoint[] detect() throws NotFoundException {
        ...
        int deltaY = Math.max(1, height / MetricConstant.METRIC_ID_MAX);
        int deltaX = Math.max(1, width / MetricConstant.METRIC_ID_MAX);
        int bottom = height;
        int right = width;
            ...
            if (horizontal ? this.image.get(start, fixedDimension) : this.image.get(fixedDimension, start)) {
                ...
                do {
                    ...
                    if (start < minDim) {
                        ...
                        z = this.image.get(start, fixedDimension);
                        continue;
                    } else {
                        z = this.image.get(fixedDimension, start);
                        continue;
                    }
                } while (!z);
            ...
            if (horizontal ? this.image.get(end, fixedDimension) : this.image.get(fixedDimension, end)) {
                ...
                do {
                    ...
                    if (end >= maxDim) {
                        ...
                        z = this.image.get(end, fixedDimension);
                        continue;
                    } else {
                        z = this.image.get(fixedDimension, end);
                        continue;
                    }
                } while (!z);

...
final class DetectionResult {
    ...
    DetectionResult(BarcodeMetadata barcodeMetadata, BoundingBox boundingBox) {
        ...
        this.detectionResultColumns = new DetectionResultColumn[(this.barcodeColumnCount + ADJUST_ROW_NUMBER_SKIP)];
    }
    ...
    private int adjustRowNumbersFromRRI() {
        ...
        for (int codewordsRow = 0; codewordsRow < codewords.length; codewordsRow++) {
            if (codewords[codewordsRow] != null) {
                ...
                for (int barcodeColumn = this.barcodeColumnCount + 1; barcodeColumn > 0 && invalidRowCounts < ADJUST_ROW_NUMBER_SKIP; barcodeColumn--) {
                    Codeword codeword = this.detectionResultColumns[barcodeColumn].getCodewords()[codewordsRow];
                    if (codeword != null) {
    ...
    private int adjustRowNumbersFromLRI() {
        ...
        for (int codewordsRow = 0; codewordsRow < codewords.length; codewordsRow++) {
            if (codewords[codewordsRow] != null) {
                int rowIndicatorRowNumber = codewords[codewordsRow].getRowNumber();
                int invalidRowCounts = 0;
                for (int barcodeColumn = 1; barcodeColumn < this.barcodeColumnCount + 1 && invalidRowCounts < ADJUST_ROW_NUMBER_SKIP; barcodeColumn++) {
                    Codeword codeword = this.detectionResultColumns[barcodeColumn].getCodewords()[codewordsRow];
                    if (codeword != null) {
                        invalidRowCounts = adjustRowNumberIfValid(rowIndicatorRowNumber, invalidRowCounts, codeword);
                        if (!codeword.hasValidRowNumber()) {
    ...
    private void adjustRowNumbers(int barcodeColumn, int codewordsRow, Codeword[] codewords) {
        ...
        otherCodewords[ADJUST_ROW_NUMBER_SKIP] = previousColumnCodewords[codewordsRow];
        ...
        if (codewordsRow < codewords.length - 2) {
            otherCodewords[9] = codewords[codewordsRow + ADJUST_ROW_NUMBER_SKIP];
            otherCodewords[12] = previousColumnCodewords[codewordsRow + ADJUST_ROW_NUMBER_SKIP];
            otherCodewords[13] = nextColumnCodewords[codewordsRow + ADJUST_ROW_NUMBER_SKIP];
        }
    ...
    public String toString() {
        ...
        for (int codewordsRow = 0; codewordsRow < rowIndicatorColumn.getCodewords().length; codewordsRow++) {
            ...
            for (int barcodeColumn = 0; barcodeColumn < this.barcodeColumnCount + ADJUST_ROW_NUMBER_SKIP; barcodeColumn++) {
                if (this.detectionResultColumns[barcodeColumn] == null) {
                    ...
                    if (codeword == null) {
                        ...
                        Object[] objArr = new Object[ADJUST_ROW_NUMBER_SKIP];
                        objArr[0] = Integer.valueOf(codeword.getRowNumber());
                        objArr[1] = Integer.valueOf(codeword.getValue());

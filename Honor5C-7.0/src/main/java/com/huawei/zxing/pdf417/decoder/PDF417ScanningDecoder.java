...
/*
        Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: com.huawei.zxing.pdf417.decoder.PDF417ScanningDecoder.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: com.huawei.zxing.pdf417.decoder.PDF417ScanningDecoder.<clinit>():void
...
*/
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: com.huawei.zxing.pdf417.decoder.PDF417ScanningDecoder.<clinit>():void");
    }
    ...
    private static BarcodeValue[][] createBarcodeMatrix(DetectionResult detectionResult) {
        int column;
        int[] iArr = new int[CODEWORD_SKEW_SIZE];
        iArr[0] = detectionResult.getBarcodeRowCount();
        iArr[1] = detectionResult.getBarcodeColumnCount() + CODEWORD_SKEW_SIZE;
    ...
    private static int[] getModuleBitCount(BitMatrix image, int minColumn, int maxColumn, boolean leftToRight, int startColumn, int imageRow) {
        ...
        while (true) {
            if (((leftToRight && imageColumn < maxColumn) || (!leftToRight && imageColumn >= minColumn)) && moduleNumber < moduleBitCount.length) {
                if (image.get(imageColumn, imageRow) == z) {
    ...
    private static int adjustCodewordStartColumn(BitMatrix image, int minColumn, int maxColumn, boolean leftToRight, int codewordStartColumn, int imageRow) {
        ...
        int increment = leftToRight ? -1 : 1;
        for (int i = 0; i < CODEWORD_SKEW_SIZE; i++) {
            while (true) {
                if (((!leftToRight || correctedStartColumn < minColumn) && (leftToRight || correctedStartColumn >= maxColumn)) || leftToRight != image.get(correctedStartColumn, imageRow)) {
                    increment = -increment;
                } else if (Math.abs(codewordStartColumn - correctedStartColumn) > CODEWORD_SKEW_SIZE) {
    ...
    private static boolean checkCodewordSkew(int codewordSize, int minCodewordWidth, int maxCodewordWidth) {
        if (minCodewordWidth - 2 > codewordSize || codewordSize > maxCodewordWidth + CODEWORD_SKEW_SIZE) {
            return false;
        }
        return true;
    }

...
public abstract class Parser extends ParserCore implements TokenTypes {
    ...
    protected String sipVersion() throws ParseException {
        ...
        try {
            ...
            this.lexer.match(47);
            if (!this.lexer.match(TokenTypes.ID).getTokenValue().equals("2.0")) {
    ...
    protected String method() throws ParseException {
        try {
            ...
            if (!(token.getTokenType() == TokenTypes.INVITE || token.getTokenType() == TokenTypes.ACK)) {
                if (!(token.getTokenType() == TokenTypes.OPTIONS || token.getTokenType() == TokenTypes.BYE || token.getTokenType() == TokenTypes.REGISTER || token.getTokenType() == TokenTypes.CANCEL || token.getTokenType() == TokenTypes.SUBSCRIBE || token.getTokenType() == TokenTypes.NOTIFY || token.getTokenType() == TokenTypes.PUBLISH || token.getTokenType() == TokenTypes.MESSAGE || token.getTokenType() == TokenTypes.ID)) {

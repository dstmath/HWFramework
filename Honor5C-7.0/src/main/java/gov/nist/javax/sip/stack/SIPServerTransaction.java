...
public class SIPServerTransaction extends SIPTransaction implements ServerRequestInterface, ServerTransaction, ServerTransactionExt {
    ...
    public boolean isMessagePartOfTransaction(SIPMessage messageToTest) {
        ...
        if (!method.equals(TokenNames.INVITE) && isTerminated()) {
            return false;
        }
    ...
    public void processRequest(SIPRequest transactionRequest, MessageChannel sourceChannel) {
        ...
        try {
            if (getRealState() == null) {
            ...
            } else if (isInviteTransaction() && TransactionState.COMPLETED == getRealState() && transactionRequest.getMethod().equals(TokenNames.ACK)) {
                ...
                } else if (transactionRequest.getMethod().equals(TokenNames.ACK)) {
            ...
            if (TransactionState.COMPLETED == getRealState() || TransactionState.TERMINATED == getRealState() || this.requestOf == null) {
                getSIPStack();
                if (SIPTransactionStack.isDialogCreated(getOriginalRequest().getMethod()) && getRealState() == TransactionState.TERMINATED && transactionRequest.getMethod().equals(TokenNames.ACK) && this.requestOf != null) {
    ...
    public void sendMessage(SIPMessage messageToSend) throws IOException {
        ...
        try {
            ...
            if (getOriginalRequest().getTopmostVia().getBranch() != null) {
                ...
                transactionResponse.getTopmostVia().removeParameter(Via.BRANCH);
            }
    ...
    protected void fireTimeoutTimer() {
        ...
        } else if (getOriginalRequest().getMethod().equals(TokenNames.BYE) && dialog != null && dialog.isTerminatedOnBye()) {
            dialog.setState(SIPDialog.TERMINATED_STATE);
        }
    ...
    public void sendResponse(Response response) throws SipException {
        ...
        try {
            ...
            if (sipResponse.getCSeq().getMethod().equals(getMethod())) {
                if (getMethod().equals(TokenNames.SUBSCRIBE) && response.getStatusCode() / 100 == 2) {
                ...
                if (sipResponse.getStatusCode() == Response.OK && sipResponse.getCSeq().getMethod().equals(TokenNames.INVITE)) {
                    if (sipResponse.getHeader(ContactHeader.NAME) == null) {
                ...
                if (isMessagePartOfTransaction((SIPMessage) response)) {
                    try {
                        ...
                        String fromTag;
                        String dialogId;
                        if (!(this.pendingReliableResponse == null || getDialog() == null || getState() == TransactionState.TERMINATED)) {
                            if (((SIPResponse) response).getContentTypeHeader() != null && response.getStatusCode() / 100 == 2) {
                                if (((SIPResponse) response).getContentTypeHeader().getContentType().equalsIgnoreCase("application")) {
                                    if (((SIPResponse) response).getContentTypeHeader().getContentSubType().equalsIgnoreCase("sdp")) {
                                        try {
                                        ...
                                        fromTag = ((SIPRequest) getRequest()).getFrom().getTag();
                                        if (fromTag != null || sipResponse.getFromTag() == null || sipResponse.getFromTag().equals(fromTag)) {
                                            ...
                                            } else if (dialog == null && getMethod().equals(TokenNames.INVITE) && this.retransmissionAlertEnabled && this.retransmissionAlertTimerTask == null && response.getStatusCode() / 100 == 2) {
    ...
    public void enableRetransmissionAlerts() throws SipException {
        ...
        } else if (getMethod().equals(TokenNames.INVITE)) {

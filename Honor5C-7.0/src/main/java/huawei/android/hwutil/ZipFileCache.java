...
public final class ZipFileCache {
    ...
    private static final ConcurrentHashMap<String, ZipFileCache> sZipFileCacheMaps = null;
    private String HWT_PATH_SKIN;
    private String HWT_PATH_TEMP_SKIN;
    private boolean mFileNotExist;
    private boolean mInited;
    private String mPath;
    private String mZip;
    private ZipFile mZipFile;
    private ZipResDir[] mZipResDir;
...
/*
        Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: huawei.android.hwutil.ZipFileCache.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: huawei.android.hwutil.ZipFileCache.<clinit>():void
...
*/
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: huawei.android.hwutil.ZipFileCache.<clinit>():void");
    }

    private ZipFileCache(String path, String zip) {
        this.mFileNotExist = DEBUG;
        this.HWT_PATH_SKIN = "/data/skin";
        this.HWT_PATH_TEMP_SKIN = "/data/skin.tmp";
        this.mInited = DEBUG;
        this.mZipResDir = new ZipResDir[]{new ZipResDir(-1, null), new ZipResDir(-1, null), new ZipResDir(-1, null), new ZipResDir(-1, null), new ZipResDir(-1, null), new ZipResDir(-1, null)};
        this.mPath = path;
        this.mZip = zip;
                ...
                if (zipFileCache == null) {
                    ...
                    try {
                        ...
                        if (zipFileCache2.mZipFile == null) {
                            if (zipFileCache2.mFileNotExist) {
    ...
    private synchronized boolean openZipFile() {
        try {
            ...
            if (file.exists() || !themeFile.exists() || tempThemeFile.exists()) {
                this.mZipFile = new ZipFile(file, RES_INDEX_LAND);
                this.mInited = DEBUG;
                return DEBUG_ICON;
            }
            this.mFileNotExist = DEBUG_ICON;
            return DEBUG;
        } catch (IOException e) {
            Log.w(TAG, "Open zip file:" + this.mZip + " failed!");
            closeZipFile();
            setEmpty();
            return DEBUG;
        }
        ...
        while (reTryCount > 0) {
            ...
            try {
                ...
                if (entry != null) {
                    ...
                    if (bitmap != null) {
                        bitmap.setDensity(impl.getDisplayMetrics().densityDpi);
                        if (bitmap.getWidth() != bitmap.getHeight()) {
                            Log.i(TAG, "getBitmapEntry bmp width = " + bitmap.getWidth() + "bmp height = " + bitmap.getHeight() + " fileName = " + fileName);
                        }
                    }
                }
                closeInputStream(is);

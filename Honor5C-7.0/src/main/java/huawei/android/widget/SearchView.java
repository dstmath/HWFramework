...
public class SearchView extends android.widget.SearchView {
    ...
    private void initialBarcodeButton(Context context, AttributeSet attrs) {
        this.mBarcodeButton = findViewById(34603184);
    ...
    public void setActionModeEnabled(boolean enabled) {
        ...
        autotv.setClickable(z);
        if (enabled) {
            z = false;
        } else {
            z = true;
        }
        autotv.setFocusable(z);
        if (enabled) {
            z = false;
        } else {
            z = true;
        }
        autotv.setShowSoftInputOnFocus(z);
        if (!enabled) {
            z2 = true;
        }
        setClickable(z2);
    ...
    public Drawable getIcon(Context context) {
        return context.getResources().getDrawable(33751345);
    }

...
public class DataOutputStream extends FilterOutputStream implements DataOutput {
    ...
    static int writeUTF(String str, DataOutput out) throws IOException {
        ...
        for (i = 0; i < strlen; i++) {
            int c = str.charAt(i);
            if (c >= 1 && c <= FloatConsts.MAX_EXPONENT) {
                utflen++;
            } else if (c > 2047) {
                utflen += 3;
            } else {
        ...
        while (i < strlen) {
            ...
            if (c >= 1 && c <= FloatConsts.MAX_EXPONENT) {
                ...
                count = count2 + 1;
                bytearr[count2] = (byte) (((c >> 6) & 63) | Pattern.CANON_EQ);
                count2 = count + 1;
                bytearr[count] = (byte) (((c >> 0) & 63) | Pattern.CANON_EQ);
            } else {
                count2 = count + 1;
                bytearr[count] = (byte) (((c >> 6) & 31) | 192);
                count = count2 + 1;
                bytearr[count2] = (byte) (((c >> 0) & 63) | Pattern.CANON_EQ);
                count2 = count;
            }
            i++;
            count = count2;

...
public class ChoiceFormat extends NumberFormat {
    ...
    public void applyPattern(String newPattern) {
        ...
        double oldStartValue = DoubleConsts.NaN;
            ...
            if (ch == '\'') {
                ...
                try {
                    ...
                    if (tempBuffer.equals("\u221e")) {
                        startValue = DoubleConsts.POSITIVE_INFINITY;
                    } else {
                        if (tempBuffer.equals("-\u221e")) {
                            startValue = DoubleConsts.NEGATIVE_INFINITY;
                        } else {
                            startValue = Double.valueOf(segments[0].toString()).doubleValue();
                        }
                    }
                    if (!(ch != '<' || startValue == DoubleConsts.POSITIVE_INFINITY || startValue == DoubleConsts.NEGATIVE_INFINITY)) {
    ...
    public String toPattern() {
        ...
        for (int i = 0; i < this.choiceLimits.length; i++) {
            ...
            if (Math.abs(Math.IEEEremainder(this.choiceLimits[i], 1.0d)) < Math.abs(Math.IEEEremainder(less, 1.0d))) {
                ...
                if (this.choiceLimits[i] == DoubleConsts.POSITIVE_INFINITY) {
                    result.append("\u221e");
                } else if (this.choiceLimits[i] == DoubleConsts.NEGATIVE_INFINITY) {
                    result.append("-\u221e");
                } else {
                    result.append("" + less);
                }
    ...
    public Number parse(String text, ParsePosition status) {
        ...
        int furthest = start;
        double bestNumber = DoubleConsts.NaN;
    ...
    public static double nextDouble(double d, boolean positive) {
        ...
        if ((bits > 0) != positive) {
            magnitude--;
        } else if (magnitude != POSITIVEINFINITY) {
            magnitude++;
        }
        return Double.longBitsToDouble(magnitude | (bits & SIGN));
    }

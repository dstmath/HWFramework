    ...
    static final class ForEachOrderedTask<S, T> extends CountedCompleter<Void> {
        ...
        private static <S, T> void doCompute(ForEachOrderedTask<S, T> task) {
            ...
            if (task2.getPendingCount() > 0) {
                task2.node = ((Builder) task2.helper.wrapAndCopyInto(task2.helper.makeNodeBuilder(task2.helper.exactOutputSizeIfKnown(rightSplit), new ForEachOps$ForEachOrderedTask$-void_doCompute_java_util_stream_ForEachOps$ForEachOrderedTask_task_LambdaImpl0()), rightSplit)).build();

...
public final class ClientOperation implements Operation, BaseStream {
    ...
    public synchronized void abort() throws IOException {
        ...
        if (!this.mOperationDone || this.mReplyHeader.responseCode == ResponseCodes.OBEX_HTTP_CONTINUE) {
            ...
            if (!this.mOperationDone && this.mReplyHeader.responseCode == ResponseCodes.OBEX_HTTP_CONTINUE) {
                ...
                this.mParent.sendRequest(ObexHelper.OBEX_OPCODE_ABORT, null, this.mReplyHeader, null, false);
    ...
    public long getLength() {
        try {
            Long temp = (Long) this.mReplyHeader.getHeader(ResponseCodes.OBEX_HTTP_FORBIDDEN);
            if (temp == null) {
                return -1;
            }
    ...
    private boolean sendRequest(int opCode) throws IOException {
        ...
        if (bodyLength > 0) {
            ...
            if (!this.mPrivateOutput.isClosed() || returnValue || this.mEndOfBodySent || (opCode & ObexHelper.OBEX_OPCODE_FINAL_BIT_MASK) == 0) {
                out.write(72);
            } else {
                out.write(73);

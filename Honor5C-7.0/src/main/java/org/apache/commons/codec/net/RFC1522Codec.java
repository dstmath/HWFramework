...
abstract class RFC1522Codec {
    ...
    protected String encodeText(String text, String charset) throws EncoderException, UnsupportedEncodingException {
        ...
        buffer.append('?');
        buffer.append(new String(doEncoding(text.getBytes(charset)), HTTP.US_ASCII));
        buffer.append("?=");
    ...
    protected String decodeText(String text) throws DecoderException, UnsupportedEncodingException {
        ...
        if (text.startsWith("=?") && text.endsWith("?=")) {
            ...
            if (getEncoding().equalsIgnoreCase(encoding)) {
                from = to + 1;
                return new String(doDecoding(text.substring(from, text.indexOf("?", from)).getBytes(HTTP.US_ASCII)), charset);
            }

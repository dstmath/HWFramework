...
public class StylesheetHandler extends DefaultHandler implements TemplatesHandler, PrefixResolver, NodeConsumer {
    ...
    public StylesheetHandler(TransformerFactoryImpl processor) throws TransformerConfigurationException {
        ...
        this.m_elementID = 0;
        this.m_fragmentID = 0;
        this.m_stylesheetType = STYPE_ROOT;
        ...
        this.m_source_location = ((Boolean) processor.getAttribute(XalanProperties.SOURCE_LOCATION)).booleanValue();
        init(processor);
    }
    ...
    XPath createMatchPatternXPath(String str, ElemTemplateElement owningTemplate) throws TransformerException {
        XPath xpath = new XPath(str, owningTemplate, this, STYPE_ROOT, this.m_stylesheetProcessor.getErrorListener(), this.m_funcTable);
        xpath.callVisitors(xpath, new ExpressionVisitor(getStylesheetRoot()));
        return xpath;
    }
    ...
    XSLTElementProcessor getProcessorFor(String uri, String localName, String rawName) throws SAXException {
        ...
        if (elemProcessor == null && !(currentProcessor instanceof ProcessorStylesheetDoc)) {
            if (getStylesheet() != null && Double.valueOf(getStylesheet().getVersion()).doubleValue() <= Constants.XSLTVERSUPPORTED && (uri.equals(Constants.S_XSLNAMESPACEURL) || !(currentProcessor instanceof ProcessorStylesheetElement))) {
                if (getElemVersion() > Constants.XSLTVERSUPPORTED) {
                }
            }
            elemProcessor = def.getProcessorForUnknown(uri, localName);
        }
        if (elemProcessor == null) {
            String str = XSLTErrorResources.ER_NOT_ALLOWED_IN_POSITION;
            Object[] objArr = new Object[STYPE_ROOT];
            objArr[0] = rawName;
            error(XSLMessages.createMessage(str, objArr), null);
        }
    ...
    public void startDocument() throws SAXException {
        this.m_stylesheetLevel += STYPE_ROOT;
    ...
    public void endElement(String uri, String localName, String rawName) throws SAXException {
        this.m_elementID--;
        if (this.m_shouldProcess) {
            if (this.m_elementID + STYPE_ROOT == this.m_fragmentID) {
                this.m_shouldProcess = false;
            }
            flushCharacters();
            popSpaceHandling();
    ...
    public void processingInstruction(String target, String data) throws SAXException {
        if (this.m_shouldProcess) {
            String prefix = SerializerConstants.EMPTYSTRING;
            String ns = SerializerConstants.EMPTYSTRING;
            String localName = target;
            int colon = target.indexOf(58);
            if (colon >= 0) {
                ns = getNamespaceForPrefix(target.substring(0, colon));
                localName = target.substring(colon + STYPE_ROOT);
            }
            try {
    ...
    private void checkForFragmentID(Attributes attributes) {
        if (!this.m_shouldProcess && attributes != null && this.m_fragmentIDString != null) {
            ...
            int i = 0;
            while (i < n) {
                if (attributes.getQName(i).equals(org.apache.xalan.templates.Constants.ATTRVAL_ID) && attributes.getValue(i).equalsIgnoreCase(this.m_fragmentIDString)) {
                    this.m_shouldProcess = true;
    ...
    int nextUid() {
        ...
        this.m_docOrderCount = i + STYPE_ROOT;
        return i;
    }
    ...
    void pushBaseIndentifier(String baseID) {
        if (baseID != null) {
            ...
            if (posOfHash > -1) {
                this.m_fragmentIDString = baseID.substring(posOfHash + STYPE_ROOT);
                this.m_shouldProcess = false;
            } else {
                this.m_shouldProcess = true;
            }
    ...
    public String getBaseIdentifier() {
        ...
        return locator == null ? SerializerConstants.EMPTYSTRING : locator.getSystemId();
    }
    ...
    private double getElemVersion() {
        ...
        double version = -1.0d;
        while (true) {
            if ((version == -1.0d || version == Constants.XSLTVERSUPPORTED) && elem != null) {
                try {
        ...
        return version == -1.0d ? Constants.XSLTVERSUPPORTED : version;
    }

...
public class XSLTElementProcessor extends ElemTemplateElement {
    ...
    Attributes setPropertiesFromAttributes(StylesheetHandler handler, String rawName, Attributes attributes, ElemTemplateElement target, boolean throwError) throws SAXException {
        ...
        boolean isCompatibleMode = (handler.getStylesheet() == null || !handler.getStylesheet().getCompatibleMode()) ? !throwError : true;
        if (isCompatibleMode) {
        ...
        while (i < nAttrs) {
            ...
            if (attrUri != null && attrUri.length() == 0 && (attributes.getQName(i).startsWith(Constants.ATTRNAME_XMLNS) || attributes.getQName(i).equals(SerializerConstants.XMLNS_PREFIX))) {
                attrUri = JAXPPrefixResolver.S_XMLNAMESPACEURI;
            }
            ...
            if (attrDef != null) {
                ...
                if (attrDef.getName().compareTo(PsuedoNames.PSEUDONAME_OTHER) == 0 && handler.getStylesheetProcessor().isSecureProcessing()) {

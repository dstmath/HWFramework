...
public abstract class DTMDefaultBase implements DTM {
    ...
    public DTMDefaultBase(DTMManager mgr, Source source, int dtmIdentity, DTMWSFilter whiteSpaceFilter, XMLStringFactory xstringfactory, boolean doIndexing, int blocksize, boolean usePrevsib, boolean newNameTable) {
        ...
        this.m_shouldStripWS = JJK_DEBUG;
        this.m_namespaceLists = null;
        if (blocksize <= 64) {
            numblocks = DEFAULT_NUMBLOCKS_SMALL;
            this.m_dtmIdent = new SuballocatedIntVector(DEFAULT_NUMBLOCKS_SMALL, 1);
        } else {
            numblocks = DEFAULT_NUMBLOCKS;
            this.m_dtmIdent = new SuballocatedIntVector(DEFAULT_NUMBLOCKS);
        }
        ...
        if (whiteSpaceFilter != null) {
            ...
            pushShouldStripWhitespace(JJK_DEBUG);
        }
    }
    ...
    protected int _firstch(int identity) {
        int info = identity >= this.m_size ? NOTPROCESSED : this.m_firstch.elementAt(identity);
    ...
    protected int _nextsib(int identity) {
        int info = identity >= this.m_size ? NOTPROCESSED : this.m_nextsib.elementAt(identity);
        while (info == NOTPROCESSED) {
            boolean isMore = nextNode();
            if (identity >= this.m_size && !isMore) {
                return -1;
        ...
        for (int index = 0; index < nRecords; index++) {
            ...
            switch (_type(index)) {
                ...
                case OpCodes.OP_AND /*3*/:
                    ...
                    break;
                case DEFAULT_NUMBLOCKS_SMALL /*4*/:
    ...
    public boolean hasChildNodes(int nodeHandle) {
        return _firstch(makeNodeIdentity(nodeHandle)) != -1 ? true : JJK_DEBUG;
    }
    ...
    protected void declareNamespaceInContext(int elementNodeIndex, int namespaceNodeIndex) {
        ...
        if (this.m_namespaceDeclSets == null) {
            this.m_namespaceDeclSetElements = new SuballocatedIntVector(DEFAULT_NUMBLOCKS);
            this.m_namespaceDeclSetElements.addElement(elementNodeIndex);
            this.m_namespaceDeclSets = new Vector();
            suballocatedIntVector = new SuballocatedIntVector(DEFAULT_NUMBLOCKS);
        ...
        if (suballocatedIntVector == null) {
            ...
            if (inherited != null) {
                ...
                suballocatedIntVector = new SuballocatedIntVector(Math.max(Math.min(isize + 16, DTMFilter.SHOW_NOTATION), DEFAULT_NUMBLOCKS));
                for (i = 0; i < isize; i++) {

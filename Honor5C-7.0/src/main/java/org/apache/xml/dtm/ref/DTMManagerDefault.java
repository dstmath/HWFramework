...
public class DTMManagerDefault extends DTMManager {
    ...
    public synchronized void addDTM(DTM dtm, int id, int offset) {
        ...
        if (oldlen <= id) {
            int newlen = Math.min(id + DTMFilter.SHOW_DOCUMENT, WalkerFactory.BIT_CHILD);
    ...
    public DTMManagerDefault() {
        this.m_dtms = new DTM[DTMFilter.SHOW_DOCUMENT];
        this.m_dtm_offsets = new int[DTMFilter.SHOW_DOCUMENT];
    ...
    public synchronized DTM getDTM(Source source, boolean unique, DTMWSFilter whiteSpaceFilter, boolean incremental, boolean doIndexing) {
        ...
        if (source == null || !(source instanceof DOMSource)) {
            ...
            if (z || z2) {
                ...
                addDTM(dtm, dtmPos, 0);
                if (xMLReader != null) {
                    haveXercesParser = xMLReader.getClass().getName().equals("org.apache.xerces.parsers.SAXParser");
                } else {

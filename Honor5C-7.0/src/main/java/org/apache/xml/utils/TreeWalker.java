        ...
        switch (node.getNodeType()) {
            case OpCodes.OP_XPATH /*1*/:
                ...
                for (int i = 0; i < nAttrs; i++) {
                    ...
                    if (!attrName.equals(SerializerConstants.XMLNS_PREFIX)) {
                        if (!attrName.startsWith(Constants.ATTRNAME_XMLNS)) {
                        }
                    }
                    int index = attrName.indexOf(":");
                    this.m_contentHandler.startPrefixMapping(index < 0 ? SerializerConstants.EMPTYSTRING : attrName.substring(index + 1), attr.getNodeValue());
                }
        ...
        switch (node.getNodeType()) {
            case OpCodes.OP_XPATH /*1*/:
                ...
                for (int i = 0; i < nAttrs; i++) {
                    ...
                    if (attrName.equals(SerializerConstants.XMLNS_PREFIX) || attrName.startsWith(Constants.ATTRNAME_XMLNS)) {
                        int index = attrName.indexOf(":");
                        this.m_contentHandler.endPrefixMapping(index < 0 ? SerializerConstants.EMPTYSTRING : attrName.substring(index + 1));
                    }
                }
            case OpCodes.OP_EQUALS /*5*/:
                EntityReference eref = (EntityReference) node;

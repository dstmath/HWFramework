...
public class DescendantIterator extends LocPathIterator {
    ...
    DescendantIterator(Compiler compiler, int opPos, int analysis) throws TransformerException {
        ...
        if ((WalkerFactory.BIT_CHILD & analysis) != 0) {
            orSelf = false;
        }
        if (fromRoot) {
            if (orSelf) {
    ...
    public void setRoot(int context, Object environment) {
        ...
        if (-1 == what || PsuedoNames.PSEUDONAME_OTHER.equals(localName) || PsuedoNames.PSEUDONAME_OTHER.equals(namespace)) {
    ...
    public int asNode(XPathContext xctxt) throws TransformerException {
        ...
        String namespace = getNamespace();
        int what = this.m_whatToShow;
        if (-1 == what || localName == PsuedoNames.PSEUDONAME_OTHER || namespace == PsuedoNames.PSEUDONAME_OTHER) {

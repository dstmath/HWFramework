...
public class X509CRLImpl extends X509CRL implements DerEncoder {
    ...
    public void encodeInfo(OutputStream out) throws CRLException {
        try {
            ...
            if (!this.revokedList.isEmpty()) {
                ...
                tmp.write((byte) DerValue.tag_SequenceOf, rCerts);
            }
            if (this.extensions != null) {
                this.extensions.encode(tmp, isExplicit);
            }
            seq.write((byte) DerValue.tag_SequenceOf, tmp);
    ...
    public BigInteger getCRLNumber() throws IOException {
        ...
        if (numExt != null) {
            return (BigInteger) numExt.get(CertificateX509Key.KEY);
        }
        return null;
    }
    ...
    public BigInteger getBaseCRLNumber() throws IOException {
        ...
        if (dciExt != null) {
            return (BigInteger) dciExt.get(CertificateX509Key.KEY);
        }
        return null;
    }
                    ...
                    if (tmpId.equals(this.sigAlgId)) {
                        ...
                        if (derStrm.available() != 0) {
                            ...
                            if (derStrm.available() != 0) {
                                ...
                                if (nextByte2 == 48 && (nextByte2 & 192) != Pattern.CANON_EQ) {
    ...
    private X500Principal getCertIssuer(X509CRLEntryImpl entry, X500Principal prevCertIssuer) throws IOException {
        ...
        if (ciExt != null) {
            return ((X500Name) ((GeneralNames) ciExt.get(X509CertInfo.ISSUER)).get(0).getName()).asX500Principal();
        }

...
/*
        Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: sun.util.calendar.BaseCalendar.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: sun.util.calendar.BaseCalendar.<clinit>():void
...
*/
    ...
    public boolean validate(CalendarDate date) {
        ...
        if (month < SUNDAY || month > DECEMBER) {
        ...
        if ((dow != PlatformLogger.ALL && dow != getDayOfWeek(bdate)) || !validateTime(date)) {
            return -assertionsDisabled;
        }
    ...
    public boolean normalize(CalendarDate date) {
        ...
        if (d > 0 && d <= ((long) ml)) {
            ...
            getCalendarDateFromFixedDate(bdate, (getFixedDate(y, m, SUNDAY, bdate) + d) - 1);
        } else {
            m += SUNDAY;
            bdate.setDayOfMonth((int) (d - ((long) ml)));
            if (m > DECEMBER) {
                bdate.setNormalizedYear(y + SUNDAY);
                m = SUNDAY;
            }
            bdate.setMonth(m);
        }
    ...
    private final int getMonthLength(int year, int month) {
        ...
        if (month == MONDAY && isLeapYear(year)) {
            return days + SUNDAY;
        }
        return days;
    }
    ...
    public long getFixedDate(int year, int month, int dayOfMonth, Date cache) {
        boolean isJan1 = (month == SUNDAY && dayOfMonth == SUNDAY) ? true : -assertionsDisabled;
        if (cache == null || !cache.hit(year)) {
            ...
            if (n < 0 || n >= FIXED_DATES.length) {
                ...
                if (prevyear >= 0) {
                    days += ((((365 * prevyear) + (prevyear / 4)) - (prevyear / 100)) + (prevyear / 400)) + ((long) (((month * 367) - 362) / DECEMBER));
                } else {
                    days += ((((365 * prevyear) + CalendarUtils.floorDivide(prevyear, 4)) - CalendarUtils.floorDivide(prevyear, 100)) + CalendarUtils.floorDivide(prevyear, 400)) + ((long) CalendarUtils.floorDivide((month * 367) - 362, (int) DECEMBER));
                }
                if (month > MONDAY) {
                    days -= (long) (isLeapYear(year) ? SUNDAY : MONDAY);
                }
    ...
    public void getCalendarDateFromFixedDate(CalendarDate date, long fixedDate) {
        ...
        if (gdate.hit(fixedDate)) {
            ...
            jan1 = getFixedDate(year, SUNDAY, SUNDAY, null);
            isLeap = isLeapYear(year);
        ...
        if (fixedDate >= mar1) {
            priorDays += isLeap ? SUNDAY : MONDAY;
        }
        int month = (priorDays * DECEMBER) + 373;
        if (month > 0) {
            month /= 367;
        } else {
        ...
        if (isLeap && month >= TUESDAY) {
            month1++;
        }
        int dayOfMonth = ((int) (fixedDate - month1)) + SUNDAY;
    ...
    final int getGregorianYearFromFixedDate(long fixedDate) {
        ...
        if (fixedDate > 0) {
            ...
            d4 = CalendarUtils.mod(d3, 365) + SUNDAY;
        }
        int year = (((n400 * PlatformLogger.FINER) + (n100 * 100)) + (n4 * WEDNESDAY)) + n1;
        if (n100 == WEDNESDAY || n1 == WEDNESDAY) {
            return year;
        }
        return year + SUNDAY;
    }

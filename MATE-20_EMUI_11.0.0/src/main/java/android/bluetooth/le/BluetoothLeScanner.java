...
public final class BluetoothLeScanner {
    ...
    public void updateScanParams(ScanCallback callback, int scanMode) {
        BluetoothLeUtils.checkAdapterStateOn(this.mBluetoothAdapter);
        synchronized (this.mLeScanClients) {
            BleScanCallbackWrapper wrapper = this.mLeScanClients.get(callback);
            if (wrapper != null) {
                wrapper.updateLeScanParams(scanMode);
            }
        }
    }
    ...
    public class BleScanCallbackWrapper extends IScannerCallback.Stub {
        ...
        public void updateLeScanParams(int scanMode) {
            Log.e(BluetoothLeScanner.TAG, "updateLeScanParams scanMode: " + scanMode);
            synchronized (this) {
                if (this.mScannerId <= 0) {
                    Log.e(BluetoothLeScanner.TAG, "Error state, mLeHandle: " + this.mScannerId);
                    return;
                ...
                try {
                    this.mBluetoothGatt.updateScanParams(this.mScannerId, false, scanMode);
                } catch (RemoteException e) {
                    Log.e(BluetoothLeScanner.TAG, "Failed to stop scan and unregister", e);
                }
            }

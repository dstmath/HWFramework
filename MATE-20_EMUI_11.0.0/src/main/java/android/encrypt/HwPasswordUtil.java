...
public class HwPasswordUtil implements PasswordUtil {
    ...
    private static String hexStr2Str(String hexStr) {
        String hexStr2 = hexStr.trim().replace(HanziToPinyin.Token.SEPARATOR, StorageManagerExt.INVALID_KEY_DESC).toUpperCase(Locale.US);
        char[] hexs = hexStr2.toCharArray();
        byte[] bytes = new byte[(hexStr2.length() / 2)];
        for (int i = 0; i < bytes.length; i++) {
            bytes[i] = (byte) (((mHexStr.indexOf(hexs[i * 2]) << 4) | mHexStr.indexOf(hexs[(i * 2) + 1])) & ConstantValues.MAX_CHANNEL_VALUE);
        }

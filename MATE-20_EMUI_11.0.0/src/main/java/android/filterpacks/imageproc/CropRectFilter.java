...
public class CropRectFilter extends Filter {
    ...
    public void updateSourceRect(int width, int height) {
        ...
        int i = this.mWidth;
        int i2 = this.mHeight;
        ((ShaderProgram) this.mProgram).setSourceRect(((float) this.mXorigin) / ((float) i), ((float) this.mYorigin) / ((float) i2), ((float) this.mOutputWidth) / ((float) i), ((float) this.mOutputHeight) / ((float) i2));
    }
}

...
public class RedEyeFilter extends Filter {
    ...
    private void createRedEyeFrame(FilterContext context) {
        ...
        this.mRadius = Math.max((float) MIN_RADIUS, ((float) Math.min(bitmapWidth, bitmapHeight)) * RADIUS_RATIO);
        int i = 0;
        while (true) {
            float[] fArr = this.mCenters;
            if (i < fArr.length) {
                this.mCanvas.drawCircle(fArr[i] * ((float) bitmapWidth), fArr[i + 1] * ((float) bitmapHeight), this.mRadius, this.mPaint);
                i += 2;
            } else {

...
public class RtgSched extends DefaultRtgSchedImpl {
    ...
    private void markFrameSchedStartMoveEvent() {
        if (RME_ENABLE != 0) {
            TraceEx.traceBegin(TraceEx.getTraceTagView(), "onMove");
            resetClickState();
            if (!sFrameSchedFling) {
    ...
    private void markFrameSchedStartSlideEvent() {
        if (RME_ENABLE != 0) {
            TraceEx.traceBegin(TraceEx.getTraceTagView(), "onMotionupSlide");
            resetClickState();
            if (sFrameSchedFling) {
                this.mRtgSchedHandler.removeMessages(3);
            }
    ...
    private void markFrameSchedStartFlingStopEvent() {
        if (RME_ENABLE != 0) {
            TraceEx.traceBegin(TraceEx.getTraceTagView(), "flingstopevent");
            resetClickState();
            if (sFrameSchedFling) {
                this.mRtgSchedHandler.removeMessages(3);
            }
    ...
    public void doDeliverInput() {
        TraceEx.traceBegin(TraceEx.getTraceTagView(), "DeliverInput");
        TraceEx.traceEnd(TraceEx.getTraceTagView());
    }
    ...
    public void doFlingStart() {
        TraceEx.traceBegin(TraceEx.getTraceTagView(), "FlingStart");
        TraceEx.traceEnd(TraceEx.getTraceTagView());
    }

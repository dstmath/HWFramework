    ...
    private void notifiyInstallEvent(int eventId) {
        ...
        synchronized (this.mLock) {
            if (this.mInstallerInfo != null) {
                installerPkgName = this.mInstallerInfo.installerPkgName;
                pkgName = this.mInstallerInfo.pkgName;
                installerUid = this.mInstallerInfo.installerUid;
                AwareLog.d(TAG, "notifiyInstallEvent eventId = " + eventId + " pkgName = " + pkgName);
            } else {
                return;
            }
        }
        if (installerPkgName != null && pkgName != null) {
            Bundle bundle = new Bundle();
            bundle.putInt(KEY_BUNDLE_EVENT_ID, eventId);
            bundle.putString(KEY_BUNDLE_INSTALLER_NAME, installerPkgName);
            bundle.putString("package_name", pkgName);
            bundle.putInt(KEY_BUNDLE_INSTALLER_UID, installerUid);
            notifiyEvent(bundle);
        }
    }

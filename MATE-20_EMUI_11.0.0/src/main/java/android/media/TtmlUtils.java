...
final class TtmlUtils {
    ...
    public static long parseTimeExpression(String time, int frameRate, int subframeRate, int tickRate) throws NumberFormatException {
        ...
        if (matcher.matches()) {
            ...
            double durationSeconds3 = durationSeconds2 + (frames != null ? ((double) Long.parseLong(frames)) / ((double) frameRate) : 0.0d);
            String subframes = matcher.group(6);
            if (subframes != null) {
                d = (((double) Long.parseLong(subframes)) / ((double) subframeRate)) / ((double) frameRate);
            } else {
                d = 0.0d;
            }

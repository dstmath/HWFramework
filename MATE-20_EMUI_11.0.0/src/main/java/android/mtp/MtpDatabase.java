...
public class MtpDatabase implements AutoCloseable {
    ...
    private int renameFile(int handle, String newName) {
        ...
        try {
            this.mMediaProvider.update(MediaStore.Files.getMtpObjectsUri(obj.getVolumeName()), values, PATH_WHERE, new String[]{oldPath.toString()});
        } catch (RemoteException e2) {
    ...
    private int findInMedia(MtpStorageManager.MtpObject obj, Path path) {
        ...
        Cursor c = null;
        try {
            c = this.mMediaProvider.query(MediaStore.Files.getMtpObjectsUri(obj.getVolumeName()), ID_PROJECTION, PATH_WHERE, new String[]{path.toString()}, null, null);
            if (c != null && c.moveToNext()) {
                ret = c.getInt(0);
            }
        } catch (RemoteException e) {

...
public class HwArpPacket {
    ...
    public boolean doArp(int timeoutMillis, Inet4Address requestedAddress, boolean fillSenderIp) {
        ...
        buf.putShort(1);
        buf.put(this.mMyMac);
        buf.put((!fillSenderIp || (inetAddress = this.mMyAddr) == null) ? new byte[4] : inetAddress.getAddress());
        buf.put(new byte[6]);
        buf.put(desiredIp);
        buf.flip();
        ...
        while (SystemClock.elapsedRealtime() < timeout) {
            if (this.mSocket.read(recvBuf, 0, recvBuf.length, -1, (int) (timeout - SystemClock.elapsedRealtime())) >= 28 && recvBuf[0] == 0 && recvBuf[b] == b) {
                if (recvBuf[2] != 8 || recvBuf[3] != 0 || recvBuf[b2] != 6 || recvBuf[5] != b2 || recvBuf[6] != 0 || recvBuf[7] != 2 || recvBuf[14] != desiredIp[0] || recvBuf[15] != desiredIp[1] || recvBuf[16] != desiredIp[2] || recvBuf[17] != desiredIp[3]) {
                    b = 1;
                } else if (!isMyMac(this.mMyMac, recvBuf)) {

...
public final class Settings {
    ...
    private static final String TEMP_TRACE_CONFIG = "temp_trace_config";
    ...
    public static class NameValueCache {
        ...
        private static final String PKG_WATCH_AOD = "com.huawei.watch.aod";
        private static final String PKG_WATCH_SETTINGS = "com.huawei.watch.settings";
        private static final String[] SELECT_VALUE_PROJECTION = {"value"};
        private String[] allowAdjustBrightnessApps = {"com.android.systemui", "com.android.settings", "com.huawei.desktop.systemui", PKG_MMITEST, PKG_RETAILDEMO, PKG_WATCH_SETTINGS, PKG_WATCH_AOD};
            ...
            try {
                if (System.SCREEN_BRIGHTNESS_MODE.equals(name)) {
                    ...
                    if (allowBrightnessAdjust(packageName)) {
                        Log.i(mode, "nosystemapp to change adjust brightness mode,packageName:" + packageName + " , mode:" + mode2 + ",value=" + value);
                        if (mode2 == null || mode2.equals(value)) {
                            str2 = mode;
                        } else if (mode2.equals(WifiEnterpriseConfig.ENGINE_DISABLE)) {
                            str2 = mode;
                            try {
                                ...
                                str = name;
                                mode = str2;
                                Log.w(mode, "Can't set key " + str + " in " + this.mUri, e);
                                return false;
                            }
                        } else {
                            str2 = mode;
                ...
                if (System.SCREEN_BRIGHTNESS.equals(name)) {
                    String mode3 = getStringForUser(cr, System.SCREEN_BRIGHTNESS_MODE, userHandle);
                    if (allowBrightnessAdjust(cr.getPackageName()) && mode3 != null && mode3.equals(WifiEnterpriseConfig.ENGINE_DISABLE)) {
                ...
                try {
                    ...
                    if (Settings.isRootMode()) {
                        str = name;
                        mode = str2;
                        try {
                            showPutStringTraceLog(cr, name, value, this.mCallSetCommand, userHandle);
        ...
        private void showPutStringTraceLog(ContentResolver cr, String name, String value, String command, int userHandle) {
            String table;
            String tempTraceConfigValue = Secure.getStringForUser(cr, Settings.TEMP_TRACE_CONFIG, userHandle);
            String[] values = TextUtils.isEmpty(tempTraceConfigValue) ? null : tempTraceConfigValue.split("\\.");
            if (values != null && values.length > 1) {
                String traceTable = values[0];
                String traceKey = values[1];
                if (this.mUri.getPathSegments().size() > 0 && (table = this.mUri.getPathSegments().get(0)) != null && table.equals(traceTable) && name.equals(traceKey)) {
                    Log.w(Settings.TAG, "put settings calling package: " + cr.getPackageName() + ", operation: " + command + ", table: " + table + ", field: " + name + ", value: " + value);
                    Log.w(Settings.TAG, Log.getStackTraceString(new Exception()));
                }
            }
        }
        ...
        public String getStringForUser(ContentResolver cr, String name, int userHandle) {
            ...
            if (this.mCallGetCommand != null) {
                ...
                if (!Settings.isInSystemServer() || Binder.getCallingUid() == Process.myUid()) {
                    b = cp.call(cr.getPackageName(), this.mProviderHolder.mUri.getAuthority(), this.mCallGetCommand, name, args2);
                } else {
                    long token = Binder.clearCallingIdentity();
                    try {
                        Bundle b2 = cp.call(cr.getPackageName(), this.mProviderHolder.mUri.getAuthority(), this.mCallGetCommand, name, args2);
                            ...
                            if (needsGenerationTracker) {
                                ...
                                if (array != null && index >= 0) {
                                    ...
                                    this.mGenerationTracker = new GenerationTracker(array, index, generation, new Runnable() {
                                        /* class android.provider.$$Lambda$Settings$NameValueCache$qSyMM6rUAHCa5rsPatfAqR3sA */
    ...
    public static final class Secure extends NameValueTable {
        ...
        public static final String CUSTOM_TIME_DARK_THEME_ACTIVATED = "custom_time_dark_theme_activated";
        ...
        private static final SettingsValidators.Validator DARK_THEME_CUSTOM_END_TIME_VALIDATOR = SettingsValidators.NON_NEGATIVE_INTEGER_VALIDATOR;
        public static final String DARK_THEME_CUSTOM_START_TIME = "dark_theme_custom_start_time";
        private static final SettingsValidators.Validator DARK_THEME_CUSTOM_START_TIME_VALIDATOR = SettingsValidators.NON_NEGATIVE_INTEGER_VALIDATOR;
        public static final String DARK_THEME_LAST_ACTIVATED_TIME = "dark_theme_last_activated_time";
        @Deprecated
        public static final String DATA_ROAMING = "data_roaming";
        ...
        public static final String DRAG_BAR_POPUP_FIRST_APPEAR = "drag_bar_popup_first_appear";
        ...
        public static final String FLOATING_WIN_COUNT = "start_floatwin_boots_count";
        ...
        static {
            ...
            VALIDATORS.put(DARK_THEME_CUSTOM_START_TIME, DARK_THEME_CUSTOM_START_TIME_VALIDATOR);
            VALIDATORS.put(DARK_THEME_CUSTOM_END_TIME, DARK_THEME_CUSTOM_END_TIME_VALIDATOR);
            ...
            CLONE_TO_MANAGED_PROFILE.add(DRAG_BAR_POPUP_FIRST_APPEAR);
        ...
        public static void resetToDefaultsAsUser(ContentResolver resolver, String tag, int mode, int userHandle) {
            try {
                ...
                if (Settings.isRootMode()) {
                    Settings.showResetTraceLog(resolver, sProviderHolder.mUri, Settings.CALL_METHOD_RESET_SECURE, userHandle);
                }
            } catch (RemoteException e) {
    ...
    public static final class Global extends NameValueTable {
        ...
        public static void resetToDefaultsAsUser(ContentResolver resolver, String tag, int mode, int userHandle) {
            try {
                ...
                if (Settings.isRootMode()) {
                    Settings.showResetTraceLog(resolver, sProviderHolder.mUri, Settings.CALL_METHOD_RESET_GLOBAL, userHandle);
                }
            } catch (RemoteException e) {
                Log.w(Settings.TAG, "Can't reset do defaults for " + CONTENT_URI, e);
            }
    ...
    /* access modifiers changed from: private */
    public static void showResetTraceLog(ContentResolver resolver, Uri uri, String command, int userHandle) {
        String traceTable = Secure.getStringForUser(resolver, TEMP_TRACE_CONFIG, userHandle);
        if (!TextUtils.isEmpty(traceTable) && uri.getPathSegments().size() > 0) {
            String table = uri.getPathSegments().get(0);
            if (traceTable.equals(table)) {
                Log.w(TAG, "reset settings calling package: " + resolver.getPackageName() + ", operation: " + command + ", table: " + table);
    ...
    public static boolean isRootMode() {
        return SystemProperties.getInt("ro.secure", -1) == 0;
    }
}

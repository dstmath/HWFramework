...
public class HwSysResImpl extends DefaultHwSysResource {
    private static final boolean IS_ENABLE_IAWARE = SystemPropertiesEx.getBoolean("persist.sys.enable_iaware", false);
    private static final boolean IS_ENABLE_RMS = SystemPropertiesEx.getBoolean("ro.config.enable_rms", false);
    ...
    private Map<Integer, ArrayList<String>> mWhiteListMap = new HashMap((int) LIST_INITIAL_CAPCITY);
    ...
    public boolean registerResourceCallback(IUpdateWhiteListCallback updateCallback) {
        return this.mResourceManger.registerResourceCallback(updateCallback);
    }

    /* access modifiers changed from: protected */
    public ArrayList<String> getResWhiteList(int type) {
        if (this.mWhiteListMap.get(Integer.valueOf(type)) == null) {
            this.mWhiteListMap.put(Integer.valueOf(type), new ArrayList<>((int) LIST_INITIAL_CAPCITY));
        }
    ...
    private ArrayList<String> initResWhiteList(int type) {
        ...
        ArrayList<String> mList = new ArrayList<>((int) LIST_INITIAL_CAPCITY);
        String configWhiteList = this.mResourceManger.getWhiteList(this.mResourceType, type);
        if (Utils.DEBUG) {

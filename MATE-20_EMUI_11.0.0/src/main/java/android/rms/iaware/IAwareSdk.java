...
public class IAwareSdk {
    ...
    public static void asyncSendData(String tag, int cmdId, int event) {
        if (tag != null) {
            asyncReportData(AwareNRTConstant.FIRST_UNIPERF_CTRL_EVENT_ID, tag + ',' + cmdId + ',' + event, 0);
        }
    }

    public static void asyncSendData(String tag, int cmdId) {
        if (tag != null) {
            asyncReportData(AwareNRTConstant.FIRST_UNIPERF_CTRL_EVENT_ID, tag + ',' + cmdId, 0);
        }
    }

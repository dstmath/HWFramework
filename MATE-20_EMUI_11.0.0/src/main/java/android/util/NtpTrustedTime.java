...
public class NtpTrustedTime implements TrustedTime {
    ...
    public static synchronized NtpTrustedTime getInstance(Context context) {
        ...
        synchronized (NtpTrustedTime.class) {
            if (sSingleton == null) {
                ...
                sSingleton = new NtpTrustedTime(secureServer != null ? secureServer : "", Settings.Global.getLong(resolver, Settings.Global.NTP_TIMEOUT, (long) res.getInteger(R.integer.config_ntpTimeout)));
                sContext = context;
            }
    ...
    public boolean forceRefresh(Network network) {
        if (TextUtils.isEmpty(this.mServer) && TextUtils.isEmpty(this.mbackupNtpServer)) {
            return false;
        }
        synchronized (this) {
            if (this.mCM == null) {

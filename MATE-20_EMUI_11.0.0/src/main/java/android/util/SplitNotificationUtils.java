...
public class SplitNotificationUtils {
    ...
    public String getNotificationType(String imsPkgName, int caller) {
        ...
        if (((TextUtils.isEmpty(imsPkgName) || !isSupportSplitScreen(imsPkgName)) && caller == 1) || isKeyguardLockedOrFreeformVisible() || (runningTaskInfo = getRunningTaskInfo()) == null || (runningTaskInfo.configuration.windowConfiguration.getWindowingMode() != 1 && runningTaskInfo.configuration.windowConfiguration.getWindowingMode() != 105)) {
            return "";
        }
        ...
        if (!HwFreeFormUtils.isFreeFormEnable() || caller != 1 || !floatingWindowImsListPkgNames.contains(imsPkgName) || !isTopFullscreen()) {
            if (shouldSkipTriggerFloatingWindow(imsPkgName, dockableTopPkgName, caller) || runningTaskInfo.supportsSplitScreenMultiWindow) {
                ...
                Flog.bdReport((int) StatisticalConstant.TYPE_FREEFROM_TRIPLE_FINGERS_COUNT, "pkg", dockableTopPkgName);

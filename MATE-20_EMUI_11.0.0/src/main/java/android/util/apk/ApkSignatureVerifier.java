...
public class ApkSignatureVerifier {
                ...
                if (minSignatureSchemeVersion < 3) {
                    ...
                    if (minSignatureSchemeVersion <= 2) {
                        ...
                        try {
                            ...
                            throw new PackageParser.PackageParserException((int) PackageManager.INSTALL_PARSE_FAILED_NO_CERTIFICATES, "No APK Signature Scheme v2 signature in package " + apkPath, e2);
                        } catch (Exception e3) {
                            throw new PackageParser.PackageParserException((int) PackageManager.INSTALL_PARSE_FAILED_NO_CERTIFICATES, "Failed to collect certificates from " + apkPath + " using APK Signature Scheme v2", e3);
                        } catch (Throwable th) {
                            Trace.traceEnd(262144);
                            throw th;
                        }
                    ...
                    throw new PackageParser.PackageParserException((int) PackageManager.INSTALL_PARSE_FAILED_NO_CERTIFICATES, "No APK Signature Scheme v3 signature in package " + apkPath, e);
                }
            } catch (Exception e4) {
                throw new PackageParser.PackageParserException((int) PackageManager.INSTALL_PARSE_FAILED_NO_CERTIFICATES, "Failed to collect certificates from " + apkPath + " using APK Signature Scheme v3", e4);
            } catch (Throwable th2) {
                Trace.traceEnd(262144);
                throw th2;
            }
    ...
    private static PackageParser.SigningDetails verifyV1Signature(String apkPath, boolean verifyFull) throws PackageParser.PackageParserException {
        ...
        try {
            ...
            throw new PackageParser.PackageParserException((int) PackageManager.INSTALL_PARSE_FAILED_CERTIFICATE_ENCODING, "Failed to collect certificates from " + apkPath, e);
        } catch (IOException | RuntimeException e2) {
            throw new PackageParser.PackageParserException((int) PackageManager.INSTALL_PARSE_FAILED_NO_CERTIFICATES, "Failed to collect certificates from " + apkPath, e2);
        } catch (Throwable th) {
            Trace.traceEnd(262144);
            closeQuietly(jarFile);
            throw th;
    ...
    private static Certificate[][] loadCertificates(StrictJarFile jarFile, ZipEntry entry) throws PackageParser.PackageParserException {
        ...
        try {
            ...
            throw new PackageParser.PackageParserException((int) PackageManager.INSTALL_PARSE_FAILED_UNEXPECTED_EXCEPTION, "Failed reading " + entry.getName() + " in " + jarFile, e);
        } catch (Throwable th) {
            IoUtils.closeQuietly(is);
            throw th;
        }

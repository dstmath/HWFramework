...
public class HorizontalScrollView extends FrameLayout {
    ...
    public HorizontalScrollView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        this.mOnGenericMotionScrollListener = new HwOnScrollListener() {
            /* class android.widget.HorizontalScrollView.AnonymousClass1 */

            @Override // huawei.android.widget.HwOnScrollListener
            public boolean onScrollBy(float deltaX, float deltaY, MotionEvent event) {
                return HorizontalScrollView.this.scrollInDetector(deltaX, deltaY);
            }
        };
        initScrollView();
    ...
    private boolean scrollInDetector(float deltaX, float deltaY) {
        int oldX = getScrollX();
        int oldY = getScrollY();
        if (Float.compare(deltaX, 0.0f) != 0) {
            scrollTo(((int) deltaX) + oldX, oldY);
        } else {
            scrollTo(oldX + ((int) deltaY), oldY);
        }
        return true;
    }
}

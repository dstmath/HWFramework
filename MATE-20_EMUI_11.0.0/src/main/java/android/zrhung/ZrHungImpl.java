...
import android.zrhung.watchpoint.SysHungVmWTG;
import com.huawei.dfr.zrhung.DefaultZrHungImpl;

public class ZrHungImpl extends DefaultZrHungImpl {
    ...
    public static IZrHung getZrHung(String wpName) {
        ...
        switch (c) {
            case AppEyeBinderBlock.PROCESS_NOT_NATIVE /* 0 */:
    ...
    public boolean sendAppEyeEvent(short wpId, ZrHungData zrHungData, String cmdBuf, String buffer) {
        try {
            ...
            if (zrHungData != null) {
                int uid = zrHungData.getInt(ZRHUNG_UID_PARAM);
                if (uid > 0) {
                    hungEventInfo.append("uid = ");
                    hungEventInfo.append(uid);
                    hungEventInfo.append(System.lineSeparator());
                }
                int pid = zrHungData.getInt(ZRHUNG_PID_PARAM);
                if (pid > 0) {
                    hungEventInfo.append("pid = ");
                    hungEventInfo.append(pid);
                    hungEventInfo.append(System.lineSeparator());
                }
                String pkgName = zrHungData.getString(ZRHUNG_PKGNAME_PARAM);
                if (pkgName != null) {
                    hungEventInfo.append("packageName = ");
                    hungEventInfo.append(pkgName);
                    hungEventInfo.append(System.lineSeparator());
                }
                String procName = zrHungData.getString(ZRHUNG_PROCNAME_PARAM);
                if (procName != null) {

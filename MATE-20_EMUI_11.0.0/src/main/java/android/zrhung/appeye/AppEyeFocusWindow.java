...
import com.android.internal.os.ExitCatchEx;
...
public final class AppEyeFocusWindow extends ZrHungImpl {
    ...
    private static final int EXIT_CATCH_FORBID = -2;
    private static final int EXIT_CATCH_INITIAL = -1;
    private static final int EXIT_CATCH_MIN_INTERVAL = 30000;
    ...
    private static final int MESSAGE_APP_ACTIVITY_SWITCH = 2;
    ...
    private int mExitCatchCheckInterval;
    ...
    public AppEyeFocusWindow(String wpName) {
        ...
        this.mLastPid = 0;
        this.mLastCatchTime = 0;
        this.mExitCatchCheckInterval = -1;
        this.mAppEyeFocusWindowHandler = new AppEyeFocusWindowHandler(BackgroundThreadEx.getHandler().getLooper());
    }
    ...
    public class AppEyeFocusWindowHandler extends HandlerEx {
        ...
        public void handleMessage(Message msg) {
            ...
            } else if (i == AppEyeFocusWindow.MESSAGE_APP_ACTIVITY_SWITCH) {
                if (msg.arg1 > 0) {
                    ExitCatchEx.disable(msg.arg1);
                }
                if (msg.arg2 > 0) {
                    ExitCatchEx.enable(msg.arg2, 8);
                }
            }
        }
    }

    private int getExitCheckInterval() {
        ZRHung.HungConfig cfg = ZRHung.getHungConfig(293);
        if (cfg == null || cfg.status != 0) {
            return -1;
        }
        if (cfg.value == null) {
            return EXIT_CATCH_FORBID;
        }
        try {
            int mInterval = Integer.parseInt(cfg.value);
            if (mInterval >= EXIT_CATCH_MIN_INTERVAL || mInterval <= 0) {
                return mInterval;
            }
            return EXIT_CATCH_MIN_INTERVAL;
    ...
    public int init(ZrHungData zrHungData) {
        ...
        this.mExitCatchCheckInterval = getExitCheckInterval();
        return 0;
    }
    ...
    private void updateExitCatch(int newPid) {
        if (newPid != this.mLastPid && newPid >= 0) {
            if (this.mExitCatchCheckInterval == -1) {
                this.mExitCatchCheckInterval = getExitCheckInterval();
            }
            if (this.mExitCatchCheckInterval >= 0) {
                long curUptime = SystemClock.uptimeMillis();
                int enablePid = 0;
                long j = this.mLastCatchTime;
                int i = this.mExitCatchCheckInterval;
                if (curUptime - j >= ((long) i) || j <= 0 || i == 0) {
                    enablePid = newPid;
                }
                if (enablePid > 0 || this.mLastPid > 0) {
                    AppEyeFocusWindowHandler appEyeFocusWindowHandler = this.mAppEyeFocusWindowHandler;
                    appEyeFocusWindowHandler.sendMessage(appEyeFocusWindowHandler.obtainMessage(MESSAGE_APP_ACTIVITY_SWITCH, this.mLastPid, enablePid));
                    Log.d(TAG, "mLastPid =" + this.mLastPid + " newPid =" + newPid);
                }
                if (enablePid > 0) {
                    this.mLastPid = enablePid;
                    this.mLastCatchTime = SystemClock.uptimeMillis();
                    return;
                }
                this.mLastPid = 0;
            }
    ...
    public boolean cancelCheck(ZrHungData zrHungData) {
        ...
        try {
            ...
            this.mFocusedPackage = (String) focusedPackage;
            this.mFocusedActivity = (String) focusedActivity;
            Object pObj = zrHungData.get("pid");
            if (pObj != null && IS_BETA_VERSION) {
                int newPid = Integer.parseInt(String.valueOf(pObj));
                if (HW_LAUNCHER_PKGNAME.equals(focusedPackage) || Process.myPid() == newPid) {
                    newPid = 0;
                }
                updateExitCatch(newPid);
            }
            ...
            SlogEx.e(TAG, "cancel check exception");
            return false;
        }
    }
    ...
    public boolean sendEvent(ZrHungData zrHungData) {
        if (NULL_STRING.equals(this.mFocusedWindow)) {
            ...
            if (this.mHungConfig != null && this.mHungConfigStatus == 0 && "1".equals(this.mHungConfigEnable)) {
                ...
                sendAppEyeEvent(272, data, null, eventBuffer.toString());
            }
        }
    ...
    private void updateUploadInfo(ZrHungData data, String pkgName) {
        if (!"false".equals(SystemPropertiesEx.get("ro.feature.dfr.appeye")) || IS_BETA_VERSION) {
            ...
            if (topBundle.getInt("pid") > 0) {
                data.putInt("pid", topBundle.getInt("pid"));
            }
            if (info != null && info.applicationInfo != null) {
                ApplicationInfo appInfo = info.applicationInfo;
                if (!pkgName.equals(appInfo.packageName)) {
                    data.remove("packageName");
                    data.putString("packageName", appInfo.packageName);
                }
                data.putInt("uid", appInfo.uid);

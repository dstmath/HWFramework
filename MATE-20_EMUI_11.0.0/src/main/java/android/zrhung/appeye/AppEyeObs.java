...
public final class AppEyeObs extends ZrHungImpl {
    ...
    public boolean sendEvent(ZrHungData zrHungData) {
        ...
        if (zrHungData == null || zrHungData.getString("packageName") == null || zrHungData.getInt("count", -1) == -1 || !isEnabled() || (count = zrHungData.getInt("count", -1)) < threshold) {
            return false;
        }
        String packageName = zrHungData.getString("packageName");
        return sendAppEyeEvent(277, zrHungData, null, EVENT_TAG + packageName + ", count=" + count);
    }
}

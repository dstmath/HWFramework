...
public class HwBaseInnerSmsManagerImpl extends DefaultHwBaseInnerSmsManager {
    ...
    public boolean shouldSetDefaultApplicationForPackage(String packageName, Context context) {
        ...
        if (!SystemPropertiesEx.getBoolean("ro.config.hw_privacymode", false) || 1 != Settings.Secure.getInt(context.getContentResolver(), "privacy_mode_on", 0) || 1 != Settings.Secure.getInt(context.getContentResolver(), "privacy_mode_state", 0)) {
            return true;
        }
        if (packageName.equals(hwMmsPackageName) || packageName.equals(hwMmsPackageNameNew)) {
            return true;
        }
        return false;
    }
    ...
    public boolean allowToSetSmsWritePermission(String packageName) {
        return CONTACT_PACKAGE_NAME.equalsIgnoreCase(packageName) || NEW_CONTACT_PACKAGE_NAME.equalsIgnoreCase(packageName) || MMS_PACKAGE_NAME.equalsIgnoreCase(packageName);
    }
    ...
    public boolean checkSmsBlacklistFlag(SmsMessageEx.PduParserEx p) {
        ...
        RlogEx.i(LOG_TAG, "parsePdu blacklistFlag: truep.mCur: " + p.getCurHw());
        return true;
    }

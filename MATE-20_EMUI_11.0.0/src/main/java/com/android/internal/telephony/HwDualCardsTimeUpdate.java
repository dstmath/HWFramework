...
public class HwDualCardsTimeUpdate {
    ...
    private boolean dualCardNitzTimeStrategy(PhoneExt phone, long nitzTime) {
        if (getCardPresentNum() != 1) {
            ...
            HwReportManagerImpl.getDefaultImpl().reportInvalidNitzTime(phone.getPhoneId(), timeStamptoDate(nitzTime), (int) COMPARE_ANOTHER_NITZ_TIME, timeStamptoDate(saveNitzTime[otherPhoneId]));
            return false;
        } else if (isCardPresent(phone.getPhoneId())) {
            return true;
        } else {
            HwReportManagerImpl.getDefaultImpl().reportInvalidNitzTime(phone.getPhoneId(), timeStamptoDate(nitzTime), 1, (String) null);
            return false;
        }
    }

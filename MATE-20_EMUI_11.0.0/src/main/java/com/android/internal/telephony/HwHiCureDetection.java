...
public class HwHiCureDetection {
    ...
    private boolean canPerformReconnect(int subId) {
        ...
        if (dcTracker == null) {
            ...
            return false;
        } else if (dcTracker.getState(DEFAULT_APN_TYPE) == DcTrackerEx.State.RETRYING) {
            return true;
        } else {
            return false;
        }
    ...
    private boolean isCallIdle() {
        ...
        for (int i = 0; i < phoneCount; i++) {
            PhoneExt phone = this.mPhoneMap.get(i, null);
            if (!(phone == null || phone.isCallTrackerStateIdle())) {
                return false;
            }
        }
        return true;

...
import com.huawei.internal.telephony.HwDataServiceChrManagerEx;
...
public class HwSubscriptionControllerReference implements IHwSubscriptionControllerEx {
    ...
    public void setNwMode(int subId, int nwMode) {
        ...
        this.mContext.getContentResolver().update(SubscriptionManagerEx.CONTENT_URI, value, "sim_id= ?", new String[]{Integer.toString(slotId)});
    ...
    public int setSubState(int slotId, int subStatus) {
        ...
        int result = this.mContext.getContentResolver().update(SubscriptionManagerEx.CONTENT_URI, value, "sim_id= ?", new String[]{Integer.toString(slotId)});
        this.mSubscriptionController.refreshCachedActiveSubscriptionInfoList();
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void updateDataSubId(AsyncResultEx asyncResultEx) {
        Integer subId = (Integer) asyncResultEx.getResult();
        logd(" updateDataSubId,  subId=" + subId);
        if (asyncResultEx.getException() == null) {
            setDataSubId(subId.intValue());
        } else {
            HwDataServiceChrManagerEx.sendIntentWhenSetDataSubFail(subId.intValue());
        }
    ...
    public int updateClatForMobile(int subId) {
        ...
        try {
            String mccMnc = TelephonyManagerEx.getSimOperatorNumericForPhone(phoneId);
            ...
            loge("Exception for SystemPropertiesEx.set");
            return 4;
        } catch (Exception e2) {
    ...
    public void getQcRilHook() {
        ...
        if (this.qcRilHook == null) {
            try {
                this.qcRilHook = new PathClassLoader("system/framework/qcrilhook.jar", ClassLoader.getSystemClassLoader()).loadClass("com.qualcomm.qcrilhook.QcRilHook").getConstructor(Context.class).newInstance(this.mContext);
    ...
    public void checkNeedSetMainSlotByPid(int slotId, int pid) {
        ...
        logd("checkNeedSetMainSlotByPid, subId = " + slotId + ", pid = " + pid);
    ...
    private class DataConnectionHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == 1 && (asyncResultEx = AsyncResultEx.from(msg.obj)) != null) {
                HwSubscriptionControllerReference hwSubscriptionControllerReference = HwSubscriptionControllerReference.this;
                hwSubscriptionControllerReference.logd("EVENT_SET_DEFAULT_DATA_DONE subId:" + ((Integer) asyncResultEx.getResult()));
                HwSubscriptionControllerReference.this.updateDataSubId(asyncResultEx);
            }
        }
    }

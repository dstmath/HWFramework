...
public class HwSubscriptionManagerImpl extends DefaultHwSubscriptionManager {
            ...
            if (equals) {
                ...
                if (result == 1) {
                    ...
                    if (!HwSubscriptionManagerImpl.isRetryed && slotId == 1 && HwTelephonyManagerInner.getDefault().isDataConnectivityDisabled(1, "disable-sub")) {
                        boolean targetState = intent.getIntExtra("newSubState", -1) == 0 ? false : HwSubscriptionManagerImpl.DBG;
    ...
    private boolean checkNeedSetDefault4GSlot(int slotId, int otherSlotId) {
        boolean isTLHybirdActiveCMCC = ((HwTelephonyManagerInner.getDefault().getDefaultMainSlotCarrier() == 1 || HwFullNetworkManager.getInstance().isCMCCDsdxDisable()) && HwFullNetworkManager.getInstance().isCMCCCardBySlotId(slotId) && HwFullNetworkManager.getInstance().isCMCCHybird()) ? DBG : false;
        boolean isCTHybirdActive = ((HwTelephonyManagerInner.getDefault().getDefaultMainSlotCarrier() == 2 || HwFullNetworkConfig.IS_CT_4GSWITCH_DISABLE) && HwFullNetworkManager.getInstance().isCTCardBySlotId(slotId) && HwFullNetworkManager.getInstance().isCTHybird()) ? DBG : false;
    ...
    private boolean deactiveSubscription(int slotId) {
        ...
        int otherSlotId = slotId == 0 ? 1 : 0;
        Message response = null;
        if ((HwTelephonyManagerInner.getDefault().getDefault4GSlotId() == slotId && this.mSubscriptionController.getSubState(otherSlotId) == 1 && SystemPropertiesEx.getBoolean("persist.sys.dualcards", false)) ? DBG : false) {
            boolean isVSim = (!HwTelephonyManager.getDefault().isPlatformSupportVsim() || (!HwVSimUtils.isVSimEnabled() && !HwVSimUtils.isVSimCauseCardReload())) ? false : DBG;
            if (HwFullNetworkConfig.IS_HISI_DSDX && !isVSim) {
                if (HwFullNetworkConfig.IS_FAST_SWITCH_SIMSLOT) {
                    response = this.mHandler.obtainMessage(EVENT_FAST_SWITCH_SIM_SLOT_RESULT);
                    this.mSavedCompleteMsg = this.mCompleteMsg;
    ...
    private boolean setSubscriptionCheck(int slotId) {
        if (!isValidSlotId(slotId)) {
            ...
            if (!HwFullNetworkManager.getInstance().isCMCCDsdxDisable() || !HwFullNetworkManager.getInstance().isCMCCCardBySlotId(slotId) || this.mSubscriptionController.getSubState(slotId) != 0 || HwFullNetworkManager.getInstance().isCMCCCardBySlotId(otherSlotId) || TelephonyManagerEx.getCallState(TelephonyManagerEx.getDefault(), getSubidFromSlotId(otherSlotId)) == 0) {
                boolean isMDMCMCCHybird = (HwTelephonyManagerInner.getDefault().getDefaultMainSlotCarrier() != 1 || !HwFullNetworkManager.getInstance().isCMCCCardBySlotId(slotId) || HwFullNetworkManager.getInstance().isCMCCCardBySlotId(otherSlotId)) ? false : DBG;
                boolean isMDMCTHybird = (HwTelephonyManagerInner.getDefault().getDefaultMainSlotCarrier() != 2 || !HwFullNetworkManager.getInstance().isCTCardBySlotId(slotId) || HwFullNetworkManager.getInstance().isCTCardBySlotId(otherSlotId)) ? false : DBG;
                boolean isCallIdleState = (this.mSubscriptionController.getSubState(slotId) != 0 || TelephonyManagerEx.getCallState(TelephonyManagerEx.getDefault(), getSubidFromSlotId(otherSlotId)) == 0) ? false : DBG;
    ...
    private void sendCompleteMsg(Exception e) {
        ...
        if (this.mCompleteMsg != null) {
            AsyncResultEx.forMessage(this.mCompleteMsg).setException(e);
    ...
    private void registerListener() {
        ...
        if (!SubscriptionManager.isValidSubscriptionId(newSubId)) {
            ...
            if (this.mPhoneStateListener != null) {
                this.mPhoneStateListener.cancelListen();
                this.mPhoneStateListener = null;
            }
        } else if (this.mPhoneStateListener == null || this.mPhoneStateListener.mSubId != newSubId) {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 11:
                    ...
                    return;
                case HwSubscriptionManagerImpl.EVENT_FAST_SWITCH_SIM_SLOT_RESULT /* 12 */:

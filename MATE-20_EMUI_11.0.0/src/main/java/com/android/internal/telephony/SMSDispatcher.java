...
public abstract class SMSDispatcher extends Handler implements ISMSDispatcherInner {
    ...
    private static int sConcatenatedRef = new Random().nextInt(ApnSettingHelper.TYPE_IA);
    @UnsupportedAppUsage
    protected final ArrayList<SmsTracker> deliveryPendingList = new ArrayList<>();
    private String mCallingPackage = PhoneConfigurationManager.SSSS;
    @UnsupportedAppUsage
    protected final CommandsInterface mCi;
    @UnsupportedAppUsage
    protected final Context mContext;
    protected IHwCdmaSMSDispatcherEx mHwCdmaSmsDispatcher;
    private HwCustSMSDispatcher mHwCust;
    protected IHwGsmSMSDispatcherEx mHwGsmSmsDispatcher;
    ...
    protected SMSDispatcher(Phone phone, SmsDispatchersController smsDispatchersController) {
        ...
        this.mHwCdmaSmsDispatcher = HwPartTelephonyFactory.loadFactory(HwPartTelephonyFactory.TELEPHONY_FACTORY_IMPL_NAME).createHwCdmaSMSDispatcherEx(this.mContext, this, phoneExt);
        this.mHwGsmSmsDispatcher = HwPartTelephonyFactory.loadFactory(HwPartTelephonyFactory.TELEPHONY_FACTORY_IMPL_NAME).createHwGsmSMSDispatcherEx(this.mContext, this, phoneExt);
    }
    ...
    public void handleMessage(Message msg) {
        ...
        if (i == 2) {
            ...
            if (msg.obj instanceof SmsTracker) {
                SMSDispatcherEx.SmsTrackerEx smsTrackerEx = getSmsTrackerEx((SmsTracker) msg.obj);
                handleSendSmsReTry(smsTrackerEx);
                sendSmsImmediately(smsTrackerEx);
            }
        } else if (i == 4) {
            ...
            if (i == 17) {
                ...
                this.mToast = Toast.makeText(this.mContext, 33686259, 1);
                this.mToast.show();
            } else if (i != 1000) {
                switch (i) {
                    case 7:
    ...
    public void handleSendComplete(AsyncResult ar) {
        ...
        sendSmsSendingTimeOutMessageDelayed(getSmsTrackerEx(tracker));
    }
    ...
    public void handleReachSentLimit(SmsTracker tracker) {
        if (!denyIfQueueLimitReached(tracker)) {
            ...
            Spanned messageText = Html.fromHtml(r.getString(17041250, appLabel));
            ConfirmDialogListener listener = new ConfirmDialogListener(tracker, null, 1);
            AlertDialog d = new AlertDialog.Builder(this.mContext, 33947691).setTitle(17041252).setIcon(17301642).setMessage(messageText).setPositiveButton(r.getString(17041253), listener).setNegativeButton(r.getString(17041251), listener).setOnCancelListener(listener).create();
    ...
    public void handleConfirmShortCode(boolean isPremium, SmsTracker tracker) {
        ...
        if (!denyIfQueueLimitReached(tracker)) {
            ...
            CharSequence appLabel = HwTelephonyFactory.getHwInnerSmsManager().getAppLabel(this.mContext, tracker.getAppPackageName(), tracker.mUserId, getSmsDispatcherEx());
            Resources r = Resources.getSystem();
            Spanned messageText = Html.fromHtml(r.getString(17041259, appLabel, tracker.mDestAddress));
            View layout = ((LayoutInflater) new ContextThemeWrapper(this.mContext, r.getIdentifier("androidhwext:style/Theme.Emui", null, null)).getSystemService("layout_inflater")).inflate(17367305, (ViewGroup) null);
            ConfirmDialogListener listener = new ConfirmDialogListener(tracker, (TextView) layout.findViewById(16909422), 0);
            ((TextView) layout.findViewById(16909417)).setText(messageText);
            ((TextView) ((ViewGroup) layout.findViewById(16909418)).findViewById(16909419)).setText(detailsId);
            ((CheckBox) layout.findViewById(16909420)).setOnCheckedChangeListener(listener);
            AlertDialog d = new AlertDialog.Builder(this.mContext, 33947691).setView(layout).setPositiveButton(r.getString(17041256), listener).setNegativeButton(r.getString(17041258), listener).setOnCancelListener(listener).create();
            d.getWindow().setType(TelephonyProto.TelephonyEvent.RilSetupDataCallResponse.RilDataCallFailCause.PDP_FAIL_MIP_FA_MOBILE_NODE_AUTHENTICATION_FAILURE);
            d.show();
    ...
    public static class SmsTracker {
        ...
        private SMSDispatcherEx.SmsTrackerEx mSmsTrackerEx;
        private int mSubId;
        @UnsupportedAppUsage
        ...
        private Uri persistSentMessageIfRequired(Context context, int messageType, int errorCode) {
            ...
            if (!this.mIsText || !this.mPersistMessage || ((packageInfo = this.mAppInfo) != null && !SmsApplication.shouldWriteMessageForPackage(packageInfo.packageName, context))) {
                return null;
            }
    ...
    public final class ConfirmDialogListener implements DialogInterface.OnClickListener, DialogInterface.OnCancelListener, CompoundButton.OnCheckedChangeListener {
        ...
        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
            ...
            if (isChecked) {
                this.mPositiveButton.setText(17041257);
                this.mNegativeButton.setText(17041260);
                TextView textView = this.mRememberUndoInstruction;
                if (textView != null) {
                    textView.setText(33686233);
                    this.mRememberUndoInstruction.setPadding(0, 0, 0, 32);
                    return;
                }
                return;
            }
            this.mPositiveButton.setText(17041256);
            this.mNegativeButton.setText(17041258);
    ...
    @Override // com.android.internal.telephony.ISMSDispatcherInner
    ...
    private boolean sendSmsImmediately(SMSDispatcherEx.SmsTrackerEx tracker) {
        if (isCdmaMo()) {
            IHwCdmaSMSDispatcherEx iHwCdmaSMSDispatcherEx = this.mHwCdmaSmsDispatcher;
            if (iHwCdmaSMSDispatcherEx != null) {
                return iHwCdmaSMSDispatcherEx.sendSmsImmediately(tracker);
            }
            return true;
        }
        IHwGsmSMSDispatcherEx iHwGsmSMSDispatcherEx = this.mHwGsmSmsDispatcher;
        if (iHwGsmSMSDispatcherEx != null) {
            return iHwGsmSMSDispatcherEx.sendSmsImmediately(tracker);
        }
        return true;
    }

    private boolean isViaAndCdma() {
        IHwCdmaSMSDispatcherEx iHwCdmaSMSDispatcherEx;
        if (!isCdmaMo() || (iHwCdmaSMSDispatcherEx = this.mHwCdmaSmsDispatcher) == null) {
            return false;
        }
        return iHwCdmaSMSDispatcherEx.isViaAndCdma();
    }

    private void sendSmsSendingTimeOutMessageDelayed(SMSDispatcherEx.SmsTrackerEx tracker) {
        if (isCdmaMo()) {
            IHwCdmaSMSDispatcherEx iHwCdmaSMSDispatcherEx = this.mHwCdmaSmsDispatcher;
            if (iHwCdmaSMSDispatcherEx != null) {
                iHwCdmaSMSDispatcherEx.sendSmsSendingTimeOutMessageDelayed(tracker);
                return;
            }
            return;
        }
        IHwGsmSMSDispatcherEx iHwGsmSMSDispatcherEx = this.mHwGsmSmsDispatcher;
        if (iHwGsmSMSDispatcherEx != null) {
            iHwGsmSMSDispatcherEx.sendSmsSendingTimeOutMessageDelayed(tracker);
        }
    }

    private void handleSmsSendingTimeout(Message msg) {
        if (isCdmaMo()) {
            IHwCdmaSMSDispatcherEx iHwCdmaSMSDispatcherEx = this.mHwCdmaSmsDispatcher;
            if (iHwCdmaSMSDispatcherEx != null) {
                iHwCdmaSMSDispatcherEx.handleSmsSendingTimeout(msg);
                return;
            }
            return;
        }
        IHwGsmSMSDispatcherEx iHwGsmSMSDispatcherEx = this.mHwGsmSmsDispatcher;
        if (iHwGsmSMSDispatcherEx != null) {
            iHwGsmSMSDispatcherEx.handleSmsSendingTimeout(msg);
        }
    }
    ...
    private void removeTimeoutEvent() {
        if (isCdmaMo()) {
            IHwCdmaSMSDispatcherEx iHwCdmaSMSDispatcherEx = this.mHwCdmaSmsDispatcher;
            if (iHwCdmaSMSDispatcherEx != null) {
                iHwCdmaSMSDispatcherEx.removeTimeoutEvent();
                return;
            }
            return;
        }
        IHwGsmSMSDispatcherEx iHwGsmSMSDispatcherEx = this.mHwGsmSmsDispatcher;
        if (iHwGsmSMSDispatcherEx != null) {
            iHwGsmSMSDispatcherEx.removeTimeoutEvent();
        }
    }

    private void handleSendSmsReTry(SMSDispatcherEx.SmsTrackerEx tracker) {
        if (isCdmaMo()) {
            IHwCdmaSMSDispatcherEx iHwCdmaSMSDispatcherEx = this.mHwCdmaSmsDispatcher;
            if (iHwCdmaSMSDispatcherEx != null) {
                iHwCdmaSMSDispatcherEx.handleSendSmsReTry(tracker);
                return;
            }
            return;
        }
        IHwGsmSMSDispatcherEx iHwGsmSMSDispatcherEx = this.mHwGsmSmsDispatcher;
        if (iHwGsmSMSDispatcherEx != null) {
            iHwGsmSMSDispatcherEx.handleSendSmsReTry(tracker);
        }
    }

    private boolean checkCustIgnoreShortCodeTips() {
        IHwGsmSMSDispatcherEx iHwGsmSMSDispatcherEx;
        if (isCdmaMo() || (iHwGsmSMSDispatcherEx = this.mHwGsmSmsDispatcher) == null) {
            return false;
        }
        return iHwGsmSMSDispatcherEx.checkCustIgnoreShortCodeTips();
    }

    @Override // com.android.internal.telephony.ISMSDispatcherInner
    public void sendSms(SMSDispatcherEx.SmsTrackerEx trackerEx) {
        sendSms(trackerEx.getSmsTracker());
    }

    @Override // com.android.internal.telephony.ISMSDispatcherInner
    public SMSDispatcherEx getSmsDispatcherEx() {
        SMSDispatcherEx smsDispatcherEx = new SMSDispatcherEx();
        smsDispatcherEx.setSmsDispatcher(this);
        return smsDispatcherEx;
    }

    /* access modifiers changed from: protected */
    public SMSDispatcherEx.SmsTrackerEx getSmsTrackerEx(SmsTracker tracker) {
        if (tracker.mSmsTrackerEx != null) {
            return tracker.mSmsTrackerEx;
        }
        SMSDispatcherEx.SmsTrackerEx trackerEx = new SMSDispatcherEx.SmsTrackerEx();
        trackerEx.setSmsTracker(tracker);
        tracker.mSmsTrackerEx = trackerEx;
        return tracker.mSmsTrackerEx;
    }
}

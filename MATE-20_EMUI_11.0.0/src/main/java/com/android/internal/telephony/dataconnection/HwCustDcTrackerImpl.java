...
public class HwCustDcTrackerImpl extends HwCustDcTracker {
    ...
    private static boolean IS_PDN_REJ_CURE_ENABLE = DBG;
    ...
    public HwCustDcTrackerImpl(DcTracker dcTracker) {
        ...
        if (this.mDcTracker.getHwDcTrackerEx() != null) {
            IS_PDN_REJ_CURE_ENABLE = this.mDcTracker.getHwDcTrackerEx().isPdnRejCureEnable();
        } else {
            log("getHwDcTrackerEx fail, IS_PDN_REJ_CURE_ENABLE will use the default value:" + IS_PDN_REJ_CURE_ENABLE);
        }
            ...
            for (int i = 0; i < roamingApnList.length; i++) {
                if (!TextUtils.isEmpty(roamingApnList[i])) {
                    ...
                    if (4 == roamingApn.length) {
                        ...
                        if (operator.equals(mccmnc) && apnSetting.getEntryName().equals(carrier) && apnSetting.getApnName().equals(apn)) {
                            Rlog.d(this.TAG, "apn setting ");
                        }
                    } else {
    ...
    public boolean getmIsPseudoImsi() {
        ...
        try {
            ...
            loge("getmIsPseudoImsi format exception");
        }
    ...
    public ApnSetting getDocomoApn(ApnSetting preferredApn) {
        ApnSetting apn = preferredApn;
        if (apn != null && !apn.canHandleType(8)) {
            int apnTypesBitmask = apn.getApnTypeBitmask();
            apn = ApnSetting.makeApnSetting(apn.getId(), apn.getOperatorNumeric(), apn.getEntryName(), apn.getApnName(), apn.getProxyAddressAsString(), apn.getProxyPort(), apn.getMmsc(), apn.getMmsProxyAddressAsString(), apn.getMmsProxyPort(), apn.getUser(), apn.getPassword(), apn.getAuthType(), ApnSetting.getApnTypesBitmaskFromString(ApnSetting.getApnTypesStringFromBitmask(apnTypesBitmask) + ",dun"), apn.getProtocol(), apn.getRoamingProtocol(), apn.isEnabled(), apn.getNetworkTypeBitmask(), apn.getProfileId(), apn.isPersistent(), apn.getMaxConns(), apn.getWaitTime(), apn.getMaxConnsTime(), apn.getMtu(), apn.getMvnoType(), apn.getMvnoMatchData(), apn.getApnSetId(), apn.getCarrierId(), apn.getSkip464Xlat());
            apn.setIsPreset(preferredApn.isPreset());
        }
        log("getDocomoApn end, preferredApn:" + apn);
        return apn;
    }
    ...
    private String[] getPcoValueFromSetting() {
        ...
        return (pcoValues == null || pcoValues.length != 2) ? new String[]{String.valueOf((int) DEFAULT_PCO_VALUE), String.valueOf((int) DEFAULT_PCO_VALUE)} : pcoValues;
    }

...
public class HwEmergencyNumberTrackerMgrImpl implements HwEmergencyNumberTrackerMgr {
    ...
    private static final String STRING_PROP_LOCAL_REGION = SystemPropertiesEx.get("ro.product.locale.region", BuildConfig.FLAVOR);
    ...
    public String splitEmergencyNum(String number) {
        ...
        return number.substring(number.indexOf(SYMBOL_PLUS) + 1);
    }
    ...
    public EmergencyNumber getCustLabeledEmergencyNumberForEcclist(List<EmergencyNumber> emergencyNumberListFromDatabase, String number, String countryIso) {
        ...
        if (number != null && number.contains(SYMBOL_PLUS)) {
            ...
            tmpNumber = number.substring(number.indexOf(SYMBOL_PLUS) + 1);
        }
        for (EmergencyNumber num : emergencyNumberListFromDatabase) {
            if (num != null && num.getNumber().equals(tmpNumber)) {
                return EmergencyNumberEx.getEmergencyNumber(tmpNumber, countryIso.toLowerCase(Locale.ENGLISH), BuildConfig.FLAVOR, EmergencyNumberEx.getEmergencyServiceCategoryBitmask(num), new ArrayList(), 16, 0);
            }
        }
        return EmergencyNumberEx.getEmergencyNumber(tmpNumber, countryIso.toLowerCase(Locale.ENGLISH), BuildConfig.FLAVOR, category, new ArrayList(), 0, 0);
    }

    public boolean isCustEmergencyNum(String number, String emergencyNum) {
        if (number == null || emergencyNum == null) {
            return false;
        }

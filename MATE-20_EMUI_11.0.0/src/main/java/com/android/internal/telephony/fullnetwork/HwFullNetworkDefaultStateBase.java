...
public abstract class HwFullNetworkDefaultStateBase extends Handler {
    ...
    protected BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent == null) {
                ...
                HwFullNetworkDefaultStateBase hwFullNetworkDefaultStateBase = HwFullNetworkDefaultStateBase.this;
                hwFullNetworkDefaultStateBase.logd("received ACTION_HW_SWITCH_SLOT_DONE, mNeedToSetMainSlotByBroadcast:" + HwFullNetworkDefaultStateBase.this.mNeedToSetMainSlotByBroadcast);
    ...
    public void handleMessage(Message msg) {
        ...
        if (i != 1010) {
            switch (i) {
                case HwFullNetworkConstantsInner.EVENT_RADIO_UNAVAILABLE /* 1002 */:
                    logd("Received EVENT_RADIO_UNAVAILABLE on index " + index);
                    onRadioUnavailable(index);
                    return;
                case HwFullNetworkConstantsInner.EVENT_RADIO_AVAILABLE /* 1003 */:
    ...
    public void onVoiceCallEnded(Integer index) {
        ...
        if (!(HwTelephonyManager.getDefault().getSubState((long) (index.intValue() == 0 ? 1 : 0)) == 1 || HwTelephonyManager.getDefault().getSubState((long) index.intValue()) != 1 || index.intValue() == this.mChipCommon.getUserSwitchDualCardSlots())) {
            this.mStateHandler.sendMessage(this.mStateHandler.obtainMessage(202, index.intValue(), -1));
        }
        int cmccSlotId = this.mChipCommon.getCMCCCardSlotId();
        if (((HwFullNetworkConfigInner.isCMCCDsdxDisable() || HwTelephonyManagerInner.getDefault().getDefaultMainSlotCarrier() == 1) && this.mChipCommon.isCMCCHybird()) && HwTelephonyManager.getDefault().getSubState((long) cmccSlotId) == 1 && cmccSlotId != this.mChipCommon.getUserSwitchDualCardSlots()) {
            this.mStateHandler.sendMessage(this.mStateHandler.obtainMessage(202, cmccSlotId, -1));
        }
        int ctSlotId = this.mChipCommon.getCTCardSlotId();
        if ((HwTelephonyManagerInner.getDefault().getDefaultMainSlotCarrier() == 2 && this.mChipCommon.isCTHybird()) && HwTelephonyManager.getDefault().getSubState((long) ctSlotId) == 1 && ctSlotId != this.mChipCommon.getUserSwitchDualCardSlots()) {
            this.mStateHandler.sendMessage(this.mStateHandler.obtainMessage(202, ctSlotId, -1));
        }
    ...
    public void processMdmPolicyChanged(Intent intent) {
        if (intent != null) {
            ...
            if (!TextUtils.isEmpty(actionTag) && actionTag.equals("action_disable_data_4G")) {
                ...
                boolean isBothCardsPresent = false;
                ...
                if (isBothCardsPresent && dataState && isSub0Active) {
                    Message msg = this.mStateHandler.obtainMessage(202);
                    msg.arg1 = targetId;

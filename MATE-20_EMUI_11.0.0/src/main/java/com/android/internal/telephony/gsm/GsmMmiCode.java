...
public final class GsmMmiCode extends Handler implements MmiCode {
    ...
    public static GsmMmiCode newFromDialString(String dialString, GsmCdmaPhone phone, UiccCardApplication app, ResultReceiver wrappedCallback) {
        ...
        if (HwTelephonyFactory.getHwPhoneManager().isStringHuaweiIgnoreCode(phone.getGsmCdmaPhone(), dialString)) {
            Rlog.d("GsmMmiCode[SUB" + phone.getPhoneId() + "]", "newFromDialString, a huawei ignore code found, return null.");
            return null;
        }
    ...
    private static boolean isShortCode(String dialString, GsmCdmaPhone phone) {
        ...
        IHwGsmCdmaPhoneEx hwGsmCdmaPhoneEx = phone.getHwGsmCdmaPhoneEx();
        PhoneExt phoneExt = phone.getGsmCdmaPhone();
        if ((hwGsmCdmaPhoneEx == null || !hwGsmCdmaPhoneEx.isShortCodeHw(dialString, phoneExt)) && !PhoneNumberUtils.isLocalEmergencyNumber(phone.getContext(), dialString)) {
    ...
    public void processCode() throws CallStateException {
        ...
        try {
            ...
            } else if (HwTelephonyFactory.getHwPhoneManager().changeMMItoUSSD(getPhoneExt(), this.mPoundString)) {
                Rlog.d(this.LOG_TAG, "changeMMItoUSSD");
                sendUssd(this.mPoundString);
            } else if (HwTelephonyFactory.getHwPhoneManager().processImsPhoneMmiCode(getMmiCodeExt(), getImsPhoneExt(this.mImsPhone))) {
            ...
            this.mMessage = this.mContext.getText(17040618);
    ...
    public void onUssdFinished(String ussdMessage, boolean isUssdRequest) {
        if (this.mState == MmiCode.State.PENDING) {
            if (TextUtils.isEmpty(ussdMessage)) {
                this.mMessage = this.mContext.getText(HwTelephonyFactory.getHwPhoneManager().showMmiError(17040617, SubscriptionManager.getSlotIndex(this.mPhone.getSubId())));
            ...
            if (!isUssdRequest) {
                ...
                int custUssdState = -1;
                IHwGsmCdmaPhoneEx hwGsmCdmaPhoneEx = this.mPhone.getHwGsmCdmaPhoneEx();
                if (hwGsmCdmaPhoneEx != null) {
                    custUssdState = hwGsmCdmaPhoneEx.removeUssdCust(getPhoneExt());
                }
    ...
    public void onUssdFinishedError() {
        if (this.mState == MmiCode.State.PENDING) {
            ...
            if (hwGsmCdmaPhoneEx != null) {
                custremoveUssdState = hwGsmCdmaPhoneEx.removeUssdCust(getPhoneExt());
            }
    ...
    public void sendUssd(String ussdMessage) {
        if (HwTelephonyFactory.getHwPhoneManager().processSendUssdInImsCall(getMmiCodeExt(), getImsPhoneExt(this.mImsPhone))) {
    ...
    public void handleMessage(Message msg) {
        switch (msg.what) {
            ...
            case 7:
                ...
                return;
            default:
                HwTelephonyFactory.getHwPhoneManager().handleMessageGsmMmiCode(getMmiCodeExt(), msg);
                return;
        }
    }
    ...
    private CharSequence getErrorMessage(AsyncResult ar) {
        if (ar.exception instanceof CommandException) {
            ...
            if (err == CommandException.Error.FDN_CHECK_FAILURE) {
                ...
                return this.mContext.getText(17041335);
            }
        }
        CharSequence result = this.mContext.getText(17040618);
            ...
            if (ar.exception instanceof CommandException) {
                ...
                if (err == CommandException.Error.PASSWORD_INCORRECT) {
                    if (isPinPukCommand()) {
                        ...
                        sb.append(this.mContext.getText(17040719));
                    }
                    ...
                    if (this.mSc.equals(SC_PIN)) {
                        sb.append(this.mContext.getText(17040033));
                    }
    ...
    /* JADX DEBUG: Can't convert new array creation: APUT found in different block: 0x0063: APUT  
      ...
      (r9v0 'this' com.android.internal.telephony.gsm.GsmMmiCode A[IMMUTABLE_TYPE, THIS])
     ...
     type: DIRECT call: com.android.internal.telephony.gsm.GsmMmiCode.serviceClassToCFString(int):java.lang.CharSequence)
     */
    ...
    private CharSequence createQueryCallWaitingResultMessage(int serviceClass) {
        StringBuilder sb = new StringBuilder(this.mContext.getText(17041203));
    ...
    public static GsmMmiCode newFromDialString(String dialString, IHwGsmCdmaPhoneInner phone, UiccCardApplication app) {
        return newFromDialString(dialString, (GsmCdmaPhone) phone.getGsmCdmaPhone().getPhone(), app);
    }
    ...
    public IccRecords getIccRcords() {
        return this.mIccRecords;
    }

    public void setState(MmiCode.State state) {
        this.mState = state;
    }
    ...
    public boolean isDeactivateHw() {
        return isDeactivate();
    }

    public boolean isInterrogateHw() {
        return isInterrogate();
    }

    public boolean isRegisterHw() {
        return isRegister();
    }

    public boolean isErasureHw() {
        return isErasure();
    }

    public static String scToBarringFacilityHw(String sc) {
        return scToBarringFacility(sc);
    }

    public static boolean isServiceCodeCallBarringHw(String sc) {
        return isServiceCodeCallBarring(sc);
    }

    public PhoneExt getPhoneExt() {
        PhoneExt phoneExt = new PhoneExt();
        phoneExt.setPhone(getPhone());
        return phoneExt;
    }

    private PhoneExt getImsPhoneExt(Phone imsPhone) {
        PhoneExt phoneExt = new PhoneExt();
        phoneExt.setPhone(imsPhone);
        return phoneExt;
    }

    private MmiCodeExt getMmiCodeExt() {
        MmiCodeExt mmiCodeExt = new MmiCodeExt();
        mmiCodeExt.setGsmMmiCode(this);
        return mmiCodeExt;
    }

    public static GsmMmiCode newFromDialString(String dialString, IHwGsmCdmaPhoneInner phone, UiccCardApplicationEx app) {
        return newFromDialString(dialString, (GsmCdmaPhone) phone.getGsmCdmaPhone().getPhone(), app.getUiccCardApplication());
    }
}

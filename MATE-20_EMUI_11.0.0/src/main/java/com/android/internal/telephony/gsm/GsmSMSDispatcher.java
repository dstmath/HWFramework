...
public final class GsmSMSDispatcher extends SMSDispatcher {
    ...
    public void sendSms(SMSDispatcher.SmsTracker tracker) {
        ...
        if (this.mHwGsmSmsDispatcher.isNeedToSendSms(smsTrackerEx)) {
            HashMap<String, Object> map = tracker.getData();
            byte[] pdu = (byte[]) map.get("pdu");
            if (tracker.mRetryCount > 0) {
                Rlog.i(TAG, "sendSms:  mRetryCount=" + tracker.mRetryCount + " mMessageRef=" + tracker.mMessageRef + " SS=" + this.mPhone.getServiceState().getState());
                if ((pdu[0] & 1) == 1) {
                    pdu[0] = (byte) (pdu[0] | 4);
            ...
            if (isIms() || ss == 0) {
                ...
                this.mHwGsmSmsDispatcher.handleSendSmsAfter(smsTrackerEx);
                return;
            }
    ...
    private void onUpdateIccAvailability() {
        ...
        if (this.mUiccController != null && (app = this.mUiccApplication.get()) != (newUiccApplication = getUiccCardApplication())) {
            if (app != null) {
                Rlog.i(TAG, "Removing stale icc objects.");
            ...
            if (newUiccApplication != null) {
                Rlog.i(TAG, "New Uicc application found");

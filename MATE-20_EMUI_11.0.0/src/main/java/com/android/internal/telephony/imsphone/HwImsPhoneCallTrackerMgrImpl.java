...
public class HwImsPhoneCallTrackerMgrImpl implements HwImsPhoneCallTrackerMgr {
    ...
    private static final int NO_CONNECTION_LEFT_IN_CALL = 0;
    private static final int ONE_CALL_LEFT_IN_IMS_CONF = 1;
    private static final int ONE_CONNECTION_LEFT_IN_CALL = 1;
    ...
    public void hangupHisiImsConnection(ImsPhoneConnection conn, IHwImsPhoneCallTrackerInner tracker) throws CallStateException {
        if (conn == null || tracker == null) {
            RlogEx.e(LOG_TAG, "hangupHisiImsConnection: conn or tracker is null");
            return;
        }
        ...
        if (call != null && call.getConnections() != null) {
            ...
            } else if (tracker.getImsPhoneCallTracker() == null || call != tracker.getImsPhoneCallTracker().mForegroundCall) {
                hangupImsConnection(conn, tracker);
            } else {
    ...
    public void hangupHisiImsCall(ImsPhoneCall call, IHwImsPhoneCallTrackerInner tracker) throws CallStateException {
        if (call == null || tracker == null) {
            RlogEx.e(LOG_TAG, "hangupHisiImsCall: call or tracker is null");
        } else if (!call.isMultiparty() || call != tracker.getImsPhoneCallTracker().mForegroundCall) {
    ...
    public void hangupConnectionByIndex(ImsPhoneCall call, int index, IHwImsPhoneCallTrackerInner tracker) throws CallStateException {
        if (call == null || call.mConnections == null) {
            RlogEx.e(LOG_TAG, "hangupConnectionByIndex: call or connections is null");
            return;
        }
        int count = call.mConnections.size();
        for (int i = 0; i < count; i++) {
            Connection connection = (Connection) call.mConnections.get(i);
            if (connection instanceof ImsPhoneConnection) {
                ImsPhoneConnection cn = (ImsPhoneConnection) connection;
    ...
    private void hangupImsConnection(ImsPhoneConnection conn, IHwImsPhoneCallTrackerInner tracker) throws CallStateException {
        ...
        ImsPhoneCall call = conn.getCall();
        boolean isRejectCall = preHangupImsConnection(conn, call, tracker);
        ImsCall imsCall = conn.getImsCall();
        log("hangupImsConnection imsCall :: " + imsCall + "isRejectCall: " + isRejectCall);
        try {
    ...
    private boolean preHangupImsConnection(ImsPhoneConnection conn, ImsPhoneCall call, IHwImsPhoneCallTrackerInner tracker) throws CallStateException {

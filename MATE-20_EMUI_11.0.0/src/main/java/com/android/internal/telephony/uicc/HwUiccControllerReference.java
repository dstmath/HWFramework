...
public class HwUiccControllerReference extends Handler implements IHwUiccControllerEx {
    ...
    private static final long DELAY_SET_RADIOPOWERDOWN_IFNOCARD_MILLIS = ((long) SystemPropertiesEx.getInt("persist.sys.time_sim2airplane", 10000));
    ...
    private RegistrantListEx mIccFdnStatusChangedRegistrants = new RegistrantListEx();
    private PowerManager.WakeLock mWakeLock = null;

    static {
        boolean z = false;
        if (SystemPropertiesEx.getBoolean("ro.config.hw_sim2airplane", false) && SystemPropertiesEx.getBoolean("persist.sys.hw_sim2airplane", true) && TelephonyManagerEx.isMultiSimEnabled()) {
            z = true;
        }
    ...
    private void powerDownIfNoCardimmediately() {
        if (SystemPropertiesEx.getBoolean("gsm.dualcards.switch", false)) {
        ...
        if (uiccCardExts != null) {
            ...
            int[] sub1Ids = SubscriptionManagerEx.getSubId(0);
            int phone1Id = SubscriptionManagerEx.getPhoneId(sub1Ids == null ? 0 : sub1Ids[0]);
            int powerOffSub = 1;
            int[] sub2Ids = SubscriptionManagerEx.getSubId(1);
            int phone2Id = SubscriptionManagerEx.getPhoneId(sub2Ids == null ? 1 : sub2Ids[0]);
            ...
            boolean isCard2Present = isCard2Present(uiccCardExts[1]);
            boolean isRadio1Off = PhoneFactoryExt.getPhone(phone1Id).getServiceState().getState() == 3;
            boolean isRadio2Off = PhoneFactoryExt.getPhone(phone2Id).getServiceState().getState() == 3;
            ...
            if (!isRadio1Off2) {
                if (!isRadio2Off2) {
                    if (isCard1Present && isCard2Present) {
                        ...
                        if (PhoneFactoryExt.getPhone(0).getState() != PhoneConstantsEx.StateEx.IDLE.ordinal()) {
                            log("should try to power off the 1st modem later");
                            setRadioPowerDownIfNoCard();
                            return;
                        } else if (!isRadio1Off2) {
                            log("try to power off the 1st modem immediately");
                            PhoneFactoryExt.getPhone(0).setRadioPower(false);
                            releaseWakeLock();
                            return;
                        } else {
                        ...
                        if (PhoneFactoryExt.getPhone(powerOffSub).getState() != PhoneConstantsEx.StateEx.IDLE.ordinal()) {
                            log("should try to power off the 2st modem later");
                            setRadioPowerDownIfNoCard();
                            return;
                        } else if (!isSlaveModemOff) {
                            log("try to power off the 2st modem immediately");
                            PhoneFactoryExt.getPhone(powerOffSub).setRadioPower(false);
                            releaseWakeLock();
                            return;
                        } else {
    ...
    private boolean isCard2Present(UiccCardExt uiccCard2) {
        if (!uiccCard2.isEuiccCard()) {
            return HwTelephonyManagerInner.getDefault().getCardType(1) != -1 || uiccCard2.getCardState() == IccCardStatusExt.CardStateEx.CARDSTATE_PRESENT;
        }
        int numApps = uiccCard2.getNumApplications();
        log("isCard2Present, numApps " + numApps);
        return numApps != 0;
    }
    ...
    private void setRadioPowerDownIfNoCard() {
        ...
        PowerManager pm = (PowerManager) PhoneFactoryExt.getPhone(0).getContext().getSystemService("power");
    ...
    public void registerForFdnStatusChange(Handler h, int what, Object obj) {
        if (HuaweiTelephonyConfigs.isPsRestrictedByFdn()) {
            synchronized (mLock) {
                ...
                this.mIccFdnStatusChangedRegistrants.add(registrantEx);
    ...
    private boolean powerUpRadioIfhasCard(int phoneId, boolean isRadioOff, boolean isCardPresent) {
        ...
        if (TelephonyManagerEx.MultiSimVariantsExt.DSDS != TelephonyManagerEx.getMultiSimConfiguration()) {
            ...
            if (!SystemPropertiesEx.getBoolean("ro.hwpp.set_uicc_by_radiopower", false)) {
                if (!HwTelephonyManager.getDefault().isPlatformSupportVsim() || !HwVSimUtils.isVSimInProcess()) {
                    Context context = PhoneFactoryExt.getPhone(phoneId).getContext();
                    if (context == null) {
                    ...
                    PhoneFactoryExt.getPhone(phoneId).setRadioPower(true);
                    return false;
                }

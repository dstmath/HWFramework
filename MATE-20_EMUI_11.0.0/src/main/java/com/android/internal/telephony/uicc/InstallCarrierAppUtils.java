...
public class InstallCarrierAppUtils {
    ...
    static void showNotification(Context context, String pkgName) {
        ...
        String title = res.getString(17040301);
        String appName = getAppNameFromPackageName(context, pkgName);
        boolean persistent = true;
        if (TextUtils.isEmpty(appName)) {
            message = res.getString(17040299);
        } else {
            message = res.getString(17040300, appName);
        }
        String downloadButtonText = res.getString(17040298);

...
public class HwVSimDualModem extends HwVSimModemAdapter {
    ...
    /* JADX DEBUG: Can't convert new array creation: APUT found in different block: 0x006c: APUT  
      (r6v1 'responseSlots' int[] A[D('responseSlots' int[])])
      (0 ??[int, short, byte, char])
      (wrap: int : 0x006a: AGET  (r9v2 int) = (r4v8 'slots' int[] A[D('slots' int[])]), (0 ??[int, short, byte, char]))
     */
    ...
    public void switchSimSlot(HwVSimProcessor processor, HwVSimRequest request) {
        ...
        if (processor.isEnableProcess()) {
            ...
            HwSubscriptionManager.getInstance().setDefaultDataSubIdToDbBySlotId(modem02);
            modem2 = 2;
            modem1 = modem02;
            modem0 = modem12;
        } else {

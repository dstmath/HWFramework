...
public class HwVSimTriModem extends HwVSimModemAdapter {
    ...
    private HwVSimTriModem(HwVSimController vsimController, Context context, CommandsInterfaceEx vsimCi, CommandsInterfaceEx[] cis) {
        super(vsimController, context, vsimCi, cis);
    }

    public static HwVSimTriModem create(HwVSimController vsimController, Context context, CommandsInterfaceEx vsimCi, CommandsInterfaceEx[] cis) {
    ...
    public void switchSimSlot(HwVSimProcessor processor, HwVSimRequest request) {
        ...
        if (processor != null && request != null) {
            ...
            if (processor.isEnableProcess() || processor.isSwitchModeProcess()) {
                ...
                HwSubscriptionManager.getInstance().setDefaultDataSubIdToDbBySlotId(modem03);
                modem02 = modem03;
                modem0 = modem1;
            } else {
                processor.doProcessException(null, request);
            ...
            int[] slots = createSimSlotsTable(modem02, modem0, modem2);
            ...
            if (needSwich) {
                logd("switchSimSlot subId " + subId + " modem0 = " + modem02 + " modem1 = " + modem0 + " modem2 = " + modem2);
                CommandsInterfaceEx ci = getCiBySub(subId);
                if (ci != null) {
                    ci.hotSwitchSimSlot(modem02, modem0, modem2, onCompleted);
                    return;
                }
                return;
            }

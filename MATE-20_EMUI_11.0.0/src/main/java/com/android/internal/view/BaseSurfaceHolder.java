...
public abstract class BaseSurfaceHolder implements SurfaceHolder {
    ...
    public Canvas lockHardwareWideColorGamutCanvas() {
        return internalLockCanvasInner(null, true, true);
    }

    private final Canvas internalLockCanvas(Rect dirty, boolean hardware) {
        return internalLockCanvasInner(dirty, hardware, false);
    }

    private final Canvas internalLockCanvasInner(Rect dirty, boolean hardware, boolean isWideColorGamut) {
        if (this.mType != 3) {
            ...
            if (onAllowLockCanvas()) {
                ...
                } else if (isWideColorGamut) {
                    try {
                        c = this.mSurface.lockHardwareWideColorGamutCanvas();
                    } catch (Exception e) {

...
public final class HwBatteryService extends BatteryServiceEx {
    ...
    static /* synthetic */ int access$908(HwBatteryService x0) {
    ...
    private boolean isTablet() {
        return GestureNavConst.DEVICE_TYPE_TABLET.equals(SystemPropertiesEx.get("ro.build.characteristics", ""));
    }
    ...
    private void handleWirelessTxStatus() {
        ...
        SlogEx.i(TAG, "handleWirelessTxStatus current status:" + wirelessTxStatus);
    ...
    private void sendWirelessTxStatusChangeBroadcast(int status) {
        ...
        intent.putExtra(AwarenessConstants.TvConstants.TV_REPORT_STATUS_NAME, status);
    ...
    public final class HwBatteryHandler extends Handler {
        ...
        private void handleBatteryPropertiesRegister() {
            ...
            if (binder == null || !binder.isBinderAlive()) {
                HwBatteryService.access$908(HwBatteryService.this);
        ...
        private void sendNonStandardChargeMsg(int loopTimes) {
            SlogEx.d(HwBatteryService.TAG, "Receive msg MSG_CHECK_NON_STANDARD_CABLE with loopTimes: " + loopTimes);
            if (loopTimes <= 0) {
                return;
            }
    ...
    public void handleNonStandardChargeLine(int plugInType) {
        if (getOldPlugInType() != 0) {
            SlogEx.e(TAG, "same plug in type as the last time");
            return;
        }
        setOldPlugInType(plugInType);
        HwBatteryHandler hwBatteryHandler = this.mHwBatteryHandler;
        if (hwBatteryHandler != null) {
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private Boolean isChargeLineStandard() {
    ...
    private int getSuperChargeLineStatus() {
        ...
        try {
            lineNode = FileUtilsEx.readTextFile(new File(SUPER_CHARGE_LINE_NODE), 0, (String) null).trim();
            lineStatus = Integer.parseInt(lineNode);
        } catch (IOException e) {
            SlogEx.e(TAG, "Error occurs when read charge line status file");
        } catch (NumberFormatException e2) {
            SlogEx.e(TAG, "Error occurs when translate lineNode: " + lineNode);
        }
        SlogEx.i(TAG, "get super charge line status: " + lineStatus + " lineNode: " + lineNode);
        return lineStatus;
    }

...
public class HwConnectivityService extends ConnectivityService {
    ...
    private static final boolean IS_CHINA_TELECOM = (SystemPropertiesEx.get("ro.config.hw_opta", "0").equals("92") && SystemPropertiesEx.get("ro.config.hw_optb", "0").equals("156"));
    ...
    private BroadcastReceiver mVerizonWifiDisconnectReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (netInfo.getState() == NetworkInfo.State.CONNECTED) {
                ...
                Toast.makeText(context, 33686284, 1).show();
        ...
        if (str != null) {
            ...
            if (WrapperFactory.getMSimTelephonyManagerWrapper().isMultiSimEnabled()) {
                ...
                if (str.equals(FEATURE_ENABLE_MMS_SUB2)) {
                    str = "enableMMS";
                    subId = 1;
                } else if (str.equals(FEATURE_ENABLE_MMS_SUB1)) {
                    str = "enableMMS";
                    subId = 0;
                }
            }
    ...
    public class WifiDisconnectManager {
        ...
        private void registerReceiver() {
            this.REMIND_WIFI_TO_PDP = AppActConstant.VALUE_TRUE.equals(Settings.Global.getString(HwConnectivityService.this.mContext.getContentResolver(), "hw_RemindWifiToPdp"));
    ...
    private AlertDialog createWarningRoamingToPdp() {
        ...
        buider.setIcon(17301543);
        buider.setPositiveButton(17040224, new DialogInterface.OnClickListener() {
        ...
        buider.setNegativeButton(17040223, new DialogInterface.OnClickListener() {
    ...
    private boolean shouldDisablePortalCheck(String ssid) {
        ...
        if (2 == Settings.Secure.getInt(this.mContext.getContentResolver(), WifiProCommonUtils.PORTAL_NETWORK_FLAG, 0)) {
            ...
            return true;
        } else if (Settings.Global.getInt(this.mContext.getContentResolver(), "device_provisioned", 0) == 0 && AppActConstant.VALUE_TRUE.equals(SettingsEx.Systemex.getString(this.mContext.getContentResolver(), "wifi.challenge.required"))) {
        ...
        if (isWifiMmsUtOn && isMapconOn && isDomainPreferRequest(nri.request)) {
            ...
            if (prefer == null || !rebuildNetworkRequestByPrefer(nri, prefer)) {
                ...
                if (AbstractConnectivityService.DomainPreferType.DOMAIN_PREFER_WIFI == prefer || AbstractConnectivityService.DomainPreferType.DOMAIN_PREFER_CELLULAR == prefer || AbstractConnectivityService.DomainPreferType.DOMAIN_PREFER_VOLTE == prefer) {
                    this.mDomainPreferHandler.sendMessageDelayed(this.mDomainPreferHandler.obtainMessage(2, prefer.value(), 0, nri.request), HwArbitrationDEFS.TIMEOUT_FOR_QUERY_QOE_WM);
                }
            }
        }
    ...
    public void startCaptivePortalAppInternal(Network network, Bundle appExtras) {
        ...
        if (SystemProperties.getBoolean("runtime.hwwifi.portal_webview_support", false) || !IS_CHINA) {

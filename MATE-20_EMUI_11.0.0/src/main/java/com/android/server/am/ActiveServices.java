...
public class ActiveServices {
        ...
        if (fgRequired) {
            ...
            if (mode != 0) {
                ...
                if (mode == 1) {
                    ...
                    if (str == null || ((str.equals(PackageManagerService.PLATFORM_PACKAGE_NAME) && ACTION_HWPUSH_MSG_EVENT.equals(service.getAction())) || this.mAm.getUidState(callingUid) == 2 || (pgm = (PGManagerInternal) LocalServices.getService(PGManagerInternal.class)) == null || !pgm.isServiceProxy(r.name, null) || (str.equals(r.name.getPackageName()) && !pgm.isServiceProxySelf(r.name.getPackageName())))) {
            ...
            if ((flags & DumpState.DUMP_FROZEN) != 0 && !isCallerSystem) {
                ...
                if (callingPackage != null) {
                    if (!callingPackage.equals(PackageManagerService.PLATFORM_PACKAGE_NAME) || !ACTION_HWPUSH_MSG_EVENT.equals(service2.getAction())) {
                        if (this.mAm.getUidState(callerApp.info.uid) != 2 && (pgm = (PGManagerInternal) LocalServices.getService(PGManagerInternal.class)) != null && pgm.isServiceProxy(s.name, null) && (!callingPackage.equals(s.name.getPackageName()) || pgm.isServiceProxySelf(s.name.getPackageName()))) {
                            if (!isCallerSystem || !callingPackage.equals("com.huawei.harmonyos.foundation")) {
    ...
    public boolean bringDownDisabledPackageServicesLocked(String packageName, Set<String> filterByClasses, int userId, boolean evenPersistent, boolean doit) {
        ...
        try {
            ...
            Slog.e(ActivityManagerService.TAG, "IndexOutOfBoundsException in bringDownDisabledPackageServicesLocked", e);
        }
        return didSomething;
    }

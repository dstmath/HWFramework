                    ...
                    if (intent.getAction() != null) {
                        if (intent.getAction().equals(UiModeManager.ACTION_ENTER_CAR_MODE) || intent.getAction().equals(UiModeManager.ACTION_EXIT_CAR_MODE)) {
                            if (IUiModeManager.Stub.asInterface(ServiceManager.getService("uimode")).getCurrentModeType() == 3) {
                                HwApsManagerService.this.mIsInCarMode = true;
                                HwApsManagerService.this.mHwApsManagerServiceConfig.stopAllAppsInLowResolution();
                            } else {
                                HwApsManagerService.this.mIsInCarMode = false;
                                HwApsManagerService.this.mHwApsManagerServiceConfig.stopAllAppsInLowResolution();
                            }
                            Slog.i(HwApsManagerService.TAG, "APS: HwApsManagerService.BroadcastReceiver.onRecive, now is in car mode? = " + HwApsManagerService.this.mIsInCarMode);
                        }
                    }
                } catch (RemoteException e) {
                    Slog.e(HwApsManagerService.TAG, "APS: HwApsManagerService.BroadcastReceiver.onRecive, remote exception is thrown. e = " + e.getMessage());
                }
            }
        }
    };

...
public class AudioService extends AbsAudioService implements AccessibilityManager.TouchExplorationStateChangeListener, AccessibilityManager.AccessibilityServicesStateChangeListener, IHwAudioServiceInner {
    ...
    private void setExtVolumeController(IAudioPolicyCallback apc) {
        if (!this.mContext.getResources().getBoolean(17891464)) {
    ...
    public class AudioPolicyProxy extends AudioPolicyConfig implements IBinder.DeathRecipient {
        ...
        public void release() {
            ...
            long identity = Binder.clearCallingIdentity();
            AudioSystem.registerPolicyMixes(this.mMixes, false, Binder.getCallingUid());
            Binder.restoreCallingIdentity(identity);
        }
        ...
        public int addMixes(ArrayList<AudioMix> mixes) {
            ...
            synchronized (this.mMixes) {
                int uid = Binder.getCallingUid();
                AudioSystem.registerPolicyMixes(this.mMixes, false, uid);
                add(mixes);
                registerPolicyMixes = AudioSystem.registerPolicyMixes(this.mMixes, true, uid);
            }
        ...
        public int connectMixes() {
            long identity = Binder.clearCallingIdentity();
            int status = AudioSystem.registerPolicyMixes(this.mMixes, true, Binder.getCallingUid());
            Binder.restoreCallingIdentity(identity);
            return status;
        }

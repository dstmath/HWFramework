...
public class DeviceApplicationPlugin extends DevicePolicyPlugin {
        ...
        if (struct.containsPolicyName("policy-single-app") || struct.containsPolicyName("task-lock-app-list")) {
            ...
            this.mContext.registerReceiver(new BroadcastReceiver() {
                ...
                public void onReceive(Context context, Intent intent) {
                    HwLog.i(DeviceApplicationPlugin.TAG, "Mdm singleApp or taskLockAppList onReceive");
                    ...
                    if (!TextUtils.isEmpty(packageName)) {
                        DeviceApplicationPlugin.this.startMdmActivity(packageName);
                        return;
                    }
                    List<String> taskLockAppList = HwDeviceManager.getList(55);
                    if (taskLockAppList != null && !taskLockAppList.isEmpty()) {
                        String packageName2 = taskLockAppList.get(0);
                        if (!TextUtils.isEmpty(packageName2)) {
                            DeviceApplicationPlugin.this.startMdmActivity(packageName2);
                        }
                    }
                }
            }, filter);
    ...
    public boolean onRemovePolicy(ComponentName who, String policyName, Bundle policyData, boolean isEffective) {
        ...
        try {
            ...
            HwLog.e(TAG, "onRemovePolicy FIX_APP_RUNTIME_PERMISSION_LIST exception");
            return false;
        }
    }
    ...
    public boolean isNotEffective(ComponentName who, String policyName, Bundle policyData, boolean isEffective) {
        if (policyData != null && (INSTALL_APKS_BLACK_LIST.equals(policyName) || "ignore-frequent-relaunch-app".equals(policyName))) {

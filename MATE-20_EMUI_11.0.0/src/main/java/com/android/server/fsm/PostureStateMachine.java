...
public class PostureStateMachine extends StateMachine {
    ...
    private static final int SET_DISPLAY_MODE_DELAY_DEFAULT = (HwFoldScreenState.isOutFoldDevice() ? WMStateCons.MSG_POWER_CONNECTED : 0);
    ...
    private State mAgingTestInnerRefreshState = new AgingTestInnerRefreshState();
    private State mAgingTestOuterRefreshState = new AgingTestOuterRefreshState();
    ...
    private int mSetDisplayModeFlag = SystemProperties.getInt("persist.sys.testDisplayMode", 1);
    ...
    public void init(HwFoldScreenManagerService service) {
        ...
        this.mHwFoldPolicy = HwServiceFactory.getHwFoldPolicy(this.mService.getContext());
        addStates();
    ...
    private void setDisplayModeInner(int sourceDisplayMode, boolean isNeedClear) {
        ...
        if (displayMode == this.mDisplayMode) {
            ...
            if (this.mService.isPausedDispModeChange()) {
                this.mService.setDeferredDispMode(displayMode);
                Slog.i("Fsm_PostureStateMachine", "setDisplayModeInner replace the deferredMode to " + displayMode);
            }
    ...
    public void onPostHandleMessage(Message msg) {
        ...
        if (i == 0) {
            ...
            if (currentFoldState != this.mFoldState) {
                DefaultGestureNavManager mGestureNavPolicy = (DefaultGestureNavManager) LocalServices.getService(DefaultGestureNavManager.class);
                ...
                if (this.mLastScreenState != 3) {
                    if (!this.mIsInPostureChanging.get()) {
                        ...
                        Slog.i("Fsm_PostureStateMachine", "eventMsg:" + eventMsg2);
            ...
            Slog.i("Fsm_PostureStateMachine", "setDisplayModeInnerDelay " + SET_DISPLAY_MODE_DELAY_DEFAULT + "ms");
            int i3 = msg.arg1;
            if (msg.arg2 != 1) {
                z = false;
            }
    ...
    public void setPosture(int sourcePosture, boolean isUseLast) {
        this.mSetDisplayModeFlag = SystemProperties.getInt("persist.sys.testDisplayMode", 1);
        if (!isUseLast) {
            this.mLastDisplayMode = 0;
        }
    ...
    /* access modifiers changed from: private */
    public abstract class PostureState extends State {
        /* access modifiers changed from: protected */

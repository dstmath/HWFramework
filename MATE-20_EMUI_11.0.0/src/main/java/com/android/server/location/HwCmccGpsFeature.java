...
public class HwCmccGpsFeature implements IHwCmccGpsFeature {
    ...
    private boolean mIsAgpsSwitchOn = SystemProperties.getBoolean("ro.config.agps_server_setting", false);
    private boolean mIsRoaming = false;
    private boolean mIsSyncedTimeFlag = true;
    ...
    public int setPostionModeAndAgpsServer(int oldPositionMode, boolean isAgpsEnabled) {
        if (!this.mIsAgpsSwitchOn || !isAgpsEnabled) {
    ...
    public void syncTime(long timestamp) {
        if (this.mNeedSyncTime == 1 && this.mIsSyncedTimeFlag && this.mIsAgpsSwitchOn) {
            LBSLog.i(TAG, false, "syncing gps time", new Object[0]);
            this.mIsSyncedTimeFlag = false;

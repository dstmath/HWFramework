...
public class NavigationCallOut {
    ...
    private boolean touchDownIsValid(float pointX, float pointY) {
        ...
        if ((statusBarManager != null && statusBarManager.isNotificationsPanelExpand()) || canDisableGesture()) {
            return false;
        }
    ...
    private void updateSettings() {
        synchronized (this.mLock) {
            ...
            reset();
            Slog.e(TAG, "mIsControlCenterOn" + this.mIsControlCenterOn + this.mHwHasNaviBar + this.mIsGestureOn);
        }
    }
    ...
    private void changeArea() {
        if (this.mIsCtrlCenterInstalled) {
            updateRealSize();
            this.mSlideStartThreshold = this.mContext.getResources().getDimensionPixelSize(34472818);
            this.mLandAreaHeight = this.mContext.getResources().getDimensionPixelSize(17105187);
            this.mAreaHeight = this.mContext.getResources().getDimensionPixelSize(17105187);
    ...
    private boolean shouldCheckAftForThisGesture() {
        ...
        return HwBasicPlatformFactory.loadFactory(HwBasicPlatformFactory.HW_PART_BASIC_PLATFORM_SERVICES_FACTORY_IMPL).getHwGestureNavWhiteConfig().isEnable(focusWinEx, rotation, policyEx);
    }
    ...
    private void showReTryToast() {
        this.mHandler.post(new Runnable() {
            ...
            @Override // java.lang.Runnable
            public void run() {
                Toast toast = Toast.makeText(new ContextThemeWrapper(NavigationCallOut.this.mContext, 33947656), 33686255, 0);

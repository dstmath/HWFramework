...
public class DataAppHandle extends AbsDataHandle {
        ...
        try {
            ...
            try {
                ...
                if (Swap.getInstance().isSwapEnabled()) {
                    Class.forName("com.huawei.server.rme.hyperhold.SceneProcessing").getMethod("notifyAppReqMem", Long.TYPE).invoke(SceneProcessing.getInstance(), Long.valueOf(reqMemKb));
                }
                return 0;
                ...
                AwareLog.e(TAG, "Notify hyperhold fail.");
                return -1;
            }
    ...
    private int handleActivityFinish(long timeStamp, int event, ArrayMap<String, String> appInfo) {
        ...
        if (MemoryFeatureEx.IS_UP_MEMORY_FEATURE.get() && MemoryConstant.getConfigGmcSwitch() != 0) {
            int pid = 0;
            try {
                pid = Integer.parseInt(appInfo.get(SceneRecogFeature.DATA_PID));
    ...
    private static boolean isInCallActivity(String pkgName) {
        return MemoryConstant.IN_CALL_UI_PACKAGE_NAME_PHONE.equals(pkgName) || "com.huawei.harmonyos.call".equals(pkgName);
    }

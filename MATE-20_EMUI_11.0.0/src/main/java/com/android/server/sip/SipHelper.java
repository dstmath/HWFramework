...
public class SipHelper {
    ...
    public ClientTransaction handleChallenge(ResponseEvent responseEvent, AccountManager accountManager) throws SipException {
        ClientTransaction ct = this.mSipStack.getAuthenticationHelper(accountManager, this.mHeaderFactory).handleChallenge(responseEvent.getResponse(), responseEvent.getClientTransaction(), this.mSipProvider, 5);
        ct.sendRequest();
        return ct;
    }
    ...
    public void sendInviteAck(ResponseEvent event, Dialog dialog) throws SipException {
        dialog.sendAck(dialog.createAck(event.getResponse().getHeader("CSeq").getSeqNumber()));
    }
    ...
    public static String getCallId(EventObject event) {
        ...
        if (event instanceof RequestEvent) {
            return getCallId((Message) ((RequestEvent) event).getRequest());
        }
        if (event instanceof ResponseEvent) {
            return getCallId((Message) ((ResponseEvent) event).getResponse());
        }
        if (event instanceof DialogTerminatedEvent) {
            ...
            return getCallId((Transaction) serverTransaction);
        } else {
    ...
    public static String getCallId(Transaction transaction) {
        if (transaction != null) {
            return getCallId((Message) transaction.getRequest());
        }
        return "";
    }

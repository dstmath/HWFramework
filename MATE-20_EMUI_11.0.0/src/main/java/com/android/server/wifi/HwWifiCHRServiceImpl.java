...
public class HwWifiCHRServiceImpl implements HwWifiCHRService {
    ...
    private void putRxListenState(Bundle msgData) {
        if (!HwWifiNearFindUtils.isHi1105Chip() || !HwWifiNearFindUtils.isWifiNearFindSwitchOn() || HwWifiNearFindUtils.getWifiNearFindLin() == 0 || !HwWifiNearFindUtils.isHiLinkInstalled(this.mContext)) {
    ...
    public void uploadDisconnectException(int code) {
        ...
        Message msg = this.mNCDFTExceptionHandler.obtainMessage(0, HwWifi2Manager.CLOSE_WIFI2_CONNECT_TIMEOUT, 0);
        msg.setData(data);
        msg.sendToTarget();
    }
    ...
    public void uploadDhcpException(String strDhcpError) {
        Message msg = this.mNCDFTExceptionHandler.obtainMessage(0, HwWifi2Manager.CLOSE_WIFI2_WIFIDIRECT_ACTIVITY, 0);
        Bundle data = new Bundle();
    ...
    private void collectRouterInfoByScanResult() {
        ...
        for (ScanResult result : scanResults) {
            ...
            if (!TextUtils.isEmpty(ssid) && !ssidList.contains(ssid) && !TextUtils.isEmpty(result.capabilities)) {
                if (result.capabilities.contains(KEY_MGMT_OWE) || result.capabilities.contains(KEY_MGMT_EAP_SUITE_B_192) || result.capabilities.contains(KEY_MGMT_SAE) || isParse80211axEid(result.informationElements)) {
                    ssidList.add(ssid);
    ...
    private Bundle parseBeaconIeInfo(ScanResult.InformationElement[] ies) {
        ...
        for (ScanResult.InformationElement ie : ies) {
            if (ie != null && (ie.id == EID_80211AX || ie.id == 70 || ie.id == 107 || ie.id == 45 || ie.id == 191 || ie.id == 61 || ie.id == 192)) {

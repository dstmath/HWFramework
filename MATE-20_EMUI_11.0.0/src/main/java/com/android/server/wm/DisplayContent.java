...
public class DisplayContent extends AbsDisplayContent implements WindowManagerPolicy.DisplayContentInfo {
    ...
    public Task findTaskForResizePoint(int x, int y) {
        ...
        while (true) {
            ...
            if (stack != null && (stack.getWindowConfiguration().canReceiveKeys() || stack.taskIdFromPoint(x, y) >= 0)) {
                if (stack.getWindowConfiguration().canResizeTask() || stack.getWindowConfiguration().inHwFreeFormWindowingMode() || (HwFreeFormUtils.isFreeFormEnable() && isStackVisible(5))) {
        ...
        if (this.mTmpTaskForResizePointSearchResult.searchDone && this.mTmpTaskForResizePointSearchResult.taskForResize != null && (this.mTmpTaskForResizePointSearchResult.taskForResize.inHwFreeFormWindowingMode() || this.mTmpTaskForResizePointSearchResult.taskForResize.inFreeformWindowingMode())) {
            for (int tokenNdx = this.mAboveAppWindowsContainers.getChildCount() - 1; tokenNdx >= 0; tokenNdx--) {
                ...
                if (token != null) {
                    for (int winNdx = token.getChildCount() - 1; winNdx >= 0; winNdx--) {
                        ...
                        if (window != null && window.isVisible() && (window.mAttrs.flags & 16) == 0 && window.mAttrs.type != 2029) {
                            Region region = new Region();
                            window.getTouchableRegion(region);
    ...
    public final class TaskStackContainers extends DisplayChildWindowContainer<TaskStack> {
        ...
        /* JADX INFO: Multiple debug info for r2v15 'layer'  int: [D('layer' int), D('layerForAnimationLayer' int)] */
        /* access modifiers changed from: package-private */

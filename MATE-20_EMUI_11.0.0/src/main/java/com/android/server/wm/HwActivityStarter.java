...
public final class HwActivityStarter extends ActivityStarter {
    ...
    private int showToastDisallowedByMdmApk(Intent intent) {
        ...
        UiThread.getHandler().post(new Runnable() {
            ...
            public void run() {
                ...
                if (context != null) {
                    Toast toast = Toast.makeText(context, context.getString(33686026), 0);
    ...
    private void standardizeIntentUriForClone(Intent intent, int userId, StorageManager storageManager) {
        ...
        if (clipData != null) {
            ...
            while (i < itemCount) {
                ...
                Uri uri = item.getUri();
                if (uri != null && FILE_STR.equals(uri.getScheme()) && uri.getPath().startsWith(sdcard)) {
                    if (volume == null) {
        ...
        if (uri2 != null && FILE_STR.equals(uri2.getScheme()) && uri2.getPath().startsWith(sdcard)) {
        ...
        if (stream != null && FILE_STR.equals(stream.getScheme()) && stream.getPath().startsWith(sdcard)) {
    ...
    private void showErrorDialogToRemoveUser(final Context context, final int userId) {
        UiThread.getHandler().post(new Runnable() {
            ...
            @Override // java.lang.Runnable
            public void run() {
                AlertDialog errorDialog = new AlertDialog.Builder(context, 33947691).setPositiveButton(33685652, new DialogInterface.OnClickListener() {
                ...
                }).setTitle(context.getString(33685654)).setMessage(context.getString(33685653)).create();

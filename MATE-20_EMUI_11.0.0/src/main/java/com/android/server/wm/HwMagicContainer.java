...
public abstract class HwMagicContainer implements DeviceAttribute {
    private static final String TAG = "HwMagicContainer";
            ...
            public void onPointerEvent(MotionEvent motionEvent) {
                if (motionEvent.getActionMasked() == 0) {
                    synchronized (HwMagicContainer.this.mParameters.getWms().getGlobalLock()) {
                        ActivityRecordEx focus = HwMagicContainer.this.mParameters.getMwWinManager().getAmsPolicy().getTopActivity(HwMagicContainer.this);
                        if (focus != null) {
                            focus.setMagicWindowPageType(1);
                        }
                    }
                }
            }

...
class HwScreenRotationAnimation extends ScreenRotationAnimationBridgeEx {
    ...
    public void setAnimationTypeInfo(Bundle animaitonTypeInfo) {
        if (animaitonTypeInfo == null) {
            Slog.w(TAG, "setAnimationTypeInfo: animaitonTypeInfo is null!");
            return;
        }
        ...
        if (infoObj instanceof Bundle) {
            ...
            Slog.w(TAG, "setAnimationTypeInfo: info is not type of Bundle!");
        }
        ...
        Slog.i(TAG, "setAnimationTypeInfo: animaitonTypeInfo = " + animaitonTypeInfo + ", mAnimationTypeInfo = " + getAospAnimationTypeInfo() + ", mUsingFoldAnim = " + isUsingFoldAnim());
    }
    ...
    public void stepAnimationToEnd(Animation animation, long nowTime) {
        if (!this.mIsExitFoldAnimEnd) {
            ...
            if (this.mIsExitFoldAnimEnd) {
                Slog.d(TAG, "ExitFoldScreenAnimationListener endFraction = " + fraction);
    ...
    public Animation createScreenFoldAnimation(boolean isEnter, int fromFoldMode, int toFoldMode) {
        ...
        Slog.i(TAG, "createScreenFoldAnimation: isEnter = " + isEnter + ", fromFoldMode = " + fromFoldMode + ", toFoldMode = " + toFoldMode);
    ...
    private float getFractionByNow(Animation animation, long nowTime) {
        if (animation == null) {
            Slog.w(TAG, "getFractionByNow failed! animation is null");
        ...
        Slog.w(TAG, "getFractionByNow failed! duration little than zero");
        return GestureNavConst.BOTTOM_WINDOW_SINGLE_HAND_RATIO;
    }
    ...
    public Animation createFoldProjectionAnimation(int fromFoldMode, int toFoldMode) {
        ...
        if (fromFoldMode == 1) {
            if (toFoldMode == 2) {
                ...
                Slog.i(TAG, String.format(Locale.ENGLISH, "class : %s, method : %s, mFullDisplayRect : %s, mMainDisplayRect : %s, rotation : %d, displayMode : %s, fromXDelta : %f, toXDelta : %f, fromYDelta : %f, toYDelta : %f", TAG, "createFoldProjectionAnimation", this.mFullDisplayRect, this.mMainDisplayRect, Integer.valueOf(rotation), Integer.valueOf(this.mService.getFoldDisplayMode()), Float.valueOf(fromXDelta), Float.valueOf(toXDelta), Float.valueOf(fromYDelta), Float.valueOf(toYDelta)));
                return animationSet;
            }
        }
        isFullToMain = false;
    ...
    public void updateFoldProjection() {
        if (!isStarted() || isFoldProjectionAnimationNull() || this.mFoldProjectionTransaction == null) {
            Slog.i(TAG, String.format(Locale.ENGLISH, "class : %s, method : %s, updateFoldProjection return mIsExitFoldAnimEnd : %b", TAG, "updateFoldProjection", Boolean.valueOf(this.mIsExitFoldAnimEnd)));
            return;
        }
        ...
        this.mFoldProjectionTransaction.setDisplayProjection(DisplayManagerInternalEx.getInstance().getDisplayToken(getDisplayId()), rotation2, this.mLayerStackRect, this.mDisplayRect);
        Slog.i(TAG, String.format(Locale.ENGLISH, "class : %s, method : %s, displayId : %d, rotation : %d, mLayerStackRect : %s, mDisplayRect : %s, mIsExitFoldAnimEnd : %b", "HwScreenRotationAnimationImpl", "updateFoldProjection", Integer.valueOf(getDisplayId()), Integer.valueOf(rotation2), this.mLayerStackRect, this.mDisplayRect, Boolean.valueOf(this.mIsExitFoldAnimEnd)));
    ...
    private void setEndProjection() {
        ...
        int rotation2 = (rotation + 3) % 4;
        this.mFoldProjectionTransaction.setDisplayProjection(this.mService.getDisplayToken(getDisplayId()), rotation2, this.mLayerStackRect, this.mDisplayRect);
        Slog.i(TAG, String.format(Locale.ENGLISH, "class : %s, method : %s, display mode : %d, rect : %s, rotation : %d, mDisplayRect : %s, mLayerStackRect : %s", TAG, "setEndProjection", Integer.valueOf(displayMode), foldDisplayModeRect, Integer.valueOf(rotation2), this.mDisplayRect, this.mLayerStackRect));
        SurfaceControl.mergeToGlobalTransaction(this.mFoldProjectionTransaction);
    }
}

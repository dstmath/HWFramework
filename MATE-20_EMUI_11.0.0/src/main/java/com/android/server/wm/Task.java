...
public class Task extends WindowContainer<AppWindowToken> implements ConfigurationContainerListener {
    ...
    public void prepareSurfaces() {
        ...
        if (this.mDimmer.updateDims(getPendingTransaction(), this.mTmpDimBoundsRect)) {
            if (this.mStack.mActivityStack.inHwFreeFormWindowingMode() && !this.mStack.mActivityStack.inHwPCMultiStackWindowingMode()) {
                this.mDimmer.updateDimsCornerRadius(getPendingTransaction(), (float) this.mStack.mActivityStack.mService.mContext.getResources().getDimensionPixelSize(34472569));
            }
            if (this.mStack.mActivityStack.inHwSplitScreenWindowingMode()) {
                this.mDimmer.updateDimsCornerRadius(getPendingTransaction(), HwActivityManager.IS_PHONE ? 0.0f : (float) this.mStack.mActivityStack.mService.mContext.getResources().getDimensionPixelSize(34472572));
            }
            scheduleAnimation();
        }
    }
    ...
    /* access modifiers changed from: package-private */
    public void startReboundScaleAnimation(final Rect startBounds, Rect stopBounds, final int ctrlType, final float mCurScale, final float mInitialScale) {
        ...
        if (valueAnimator != null && valueAnimator.isRunning()) {
            ...
            this.widthAnimator.setDuration(300L);
            final int deltaWidth = (stopBounds.right - stopBounds.left) - (startBounds.right - startBounds.left);
            int i = startBounds.left;
            int i2 = startBounds.right;
                        ...
                        try {
                            ...
                            if (i != 1) {
                                if (i != 2) {
                                    switch (i) {
                                        case 8:
                                            Task.this.mHwFreeFormTransaction.setPosition(Task.this.mHwFreeFormScaleLeash, (float) (startBounds.left - ((int) ((((float) deltaWidth) * animTempScale) / 2.0f))), (float) startBounds.top);
                                            break;
                                    }
                                    Task.this.setReboundState(mCurScale, mInitialScale, Task.this.mMaxScale, Task.this.mMinScale, animTempScale);
                                    WindowManagerService.resetPriorityAfterLockedSection();
                                }
                                Task.this.mHwFreeFormTransaction.setPosition(Task.this.mHwFreeFormScaleLeash, (float) startBounds.left, (float) startBounds.top);
                                Task.this.setReboundState(mCurScale, mInitialScale, Task.this.mMaxScale, Task.this.mMinScale, animTempScale);
                                WindowManagerService.resetPriorityAfterLockedSection();
                            }
                            Task.this.mHwFreeFormTransaction.setPosition(Task.this.mHwFreeFormScaleLeash, (float) (startBounds.left - ((int) (((float) deltaWidth) * animTempScale))), (float) startBounds.top);
                            Task.this.setReboundState(mCurScale, mInitialScale, Task.this.mMaxScale, Task.this.mMinScale, animTempScale);
                            WindowManagerService.resetPriorityAfterLockedSection();
                        } finally {
                            WindowManagerService.resetPriorityAfterLockedSection();
            ...
            this.widthAnimator.addListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationEnd(Animator animation) {
                    synchronized (Task.this.mWmService.getGlobalLock()) {
                        try {
                            WindowManagerService.boostPriorityForLockedSection();
                            if (Task.this.isNarrowAnim) {
                                Task.this.destroyHwFreeFormScaleLeash(Task.this.mMinScale);
                            } else {
                                Task.this.destroyHwFreeFormScaleLeash(Task.this.mMaxScale);
                            }
    ...
    /* access modifiers changed from: package-private */
    public void setReboundState(float curScale, float initialScale, float maxScale, float minScale, float animScale) {

    ...
    public void prepareSurfaces() {
        ...
        if (this.mDimmer.updateDims(getPendingTransaction(), this.mTmpDimBoundsRect)) {
            if (this.mActivityStack.inHwFreeFormWindowingMode() && !this.mActivityStack.inHwPCMultiStackWindowingMode()) {
                this.mDimmer.updateDimsCornerRadius(getPendingTransaction(), (float) this.mActivityStack.mService.mContext.getResources().getDimensionPixelSize(34472569));
            }
            if (this.mActivityStack.inHwSplitScreenWindowingMode()) {
                this.mDimmer.updateDimsCornerRadius(getPendingTransaction(), HwActivityManager.IS_PHONE ? 0.0f : (float) this.mActivityStack.mService.mContext.getResources().getDimensionPixelSize(34472572));
            }
            scheduleAnimation();
        }
    }

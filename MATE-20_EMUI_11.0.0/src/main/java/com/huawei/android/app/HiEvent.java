package com.huawei.android.app;

import android.content.Context;
import android.content.pm.PackageInfo;
import android.content.pm.PackageManager;
import android.util.Log;
import android.util.Singleton;
import java.util.Date;

public class HiEvent {
    private static final String APP_NAME = "PNAMEID";
    private static final String APP_VERSION = "PVERSIONID";
    private static final Singleton<Boolean> SINGLETON_HIEVENT = new Singleton<Boolean>() {
        /* class com.huawei.android.app.HiEvent.AnonymousClass1 */

        /* access modifiers changed from: protected */
        public Boolean create() {
            boolean hiEventDefined = true;
            try {
                Class.forName("huawei.hiview.HiEvent");
            } catch (ClassNotFoundException e) {
                hiEventDefined = false;
                Log.e(HiEvent.TAG, "ClassNotFoundException: huawei.hiview.HiEvent not exsist.");
            }
            return new Boolean(hiEventDefined);
        }
    };
    private static final String TAG = "HiEvent";
    private static Context lastContext;
    private static String name;
    private static String version;
    private huawei.hiview.HiEvent event;

    public HiEvent(int id) {
        if (isHiEventSdkExist()) {
            this.event = new huawei.hiview.HiEvent(id);
        }
    }

    private static boolean isHiEventSdkExist() {
        return ((Boolean) SINGLETON_HIEVENT.get()).booleanValue();
    }

    /* access modifiers changed from: package-private */
    public huawei.hiview.HiEvent getHiEvent() {
        return this.event;
    }

    /* access modifiers changed from: package-private */
    public void setHiEvent(huawei.hiview.HiEvent value) {
        this.event = value;
    }

    public HiEvent setTime(Date date) {
        if (!isHiEventSdkExist()) {
            return this;
        }
        this.event.setTime(date);
        return this;
    }

    public HiEvent putBool(String key, boolean value) {
        if (!isHiEventSdkExist()) {
            return this;
        }
        this.event.putBool(key, value);
        return this;
    }

    public HiEvent putBoolArray(String key, boolean[] value) {
        if (!isHiEventSdkExist()) {
            return this;
        }
        this.event.putBoolArray(key, value);
        return this;
    }

    public HiEvent putByte(String key, byte value) {
        if (!isHiEventSdkExist()) {
            return this;
        }
        this.event.putByte(key, value);
        return this;
    }

    public HiEvent putByteArray(String key, byte[] value) {
        if (!isHiEventSdkExist()) {
            return this;
        }
        this.event.putByteArray(key, value);
        return this;
    }

    public HiEvent putShort(String key, short value) {
        if (!isHiEventSdkExist()) {
            return this;
        }
        this.event.putShort(key, value);
        return this;
    }

    public HiEvent putShortArray(String key, short[] value) {
        if (!isHiEventSdkExist()) {
            return this;
        }
        this.event.putShortArray(key, value);
        return this;
    }

    public HiEvent putInt(String key, int value) {
        if (!isHiEventSdkExist()) {
            return this;
        }
        this.event.putInt(key, value);
        return this;
    }

    public HiEvent putIntArray(String key, int[] value) {
        if (!isHiEventSdkExist()) {
            return this;
        }
        this.event.putIntArray(key, value);
        return this;
    }

    public HiEvent putLong(String key, long value) {
        if (!isHiEventSdkExist()) {
            return this;
        }
        this.event.putLong(key, value);
        return this;
    }

    public HiEvent putLongArray(String key, long[] value) {
        if (!isHiEventSdkExist()) {
            return this;
        }
        this.event.putLongArray(key, value);
        return this;
    }

    public HiEvent putFloat(String key, float value) {
        if (!isHiEventSdkExist()) {
            return this;
        }
        this.event.putFloat(key, value);
        return this;
    }

    public HiEvent putFloatArray(String key, float[] value) {
        if (!isHiEventSdkExist()) {
            return this;
        }
        this.event.putFloatArray(key, value);
        return this;
    }

    public HiEvent putString(String key, String value) {
        if (!isHiEventSdkExist()) {
            return this;
        }
        this.event.putString(key, value);
        return this;
    }

    public HiEvent putStringArray(String key, String[] value) {
        if (!isHiEventSdkExist()) {
            return this;
        }
        this.event.putStringArray(key, value);
        return this;
    }

    public HiEvent putHiEvent(String key, HiEvent value) {
        if (isHiEventSdkExist() && value != null) {
            this.event.putHiEvent(key, value.getHiEvent());
        }
        return this;
    }

    public HiEvent putHiEventArray(String key, HiEvent[] value) {
        if (isHiEventSdkExist() && value != null && value.length > 0) {
            huawei.hiview.HiEvent[] eventValue = new huawei.hiview.HiEvent[value.length];
            int i = 0;
            while (i < value.length && value[i] != null) {
                eventValue[i] = value[i].getHiEvent();
                i++;
            }
            this.event.putHiEventArray(key, eventValue);
        }
        return this;
    }

    public HiEvent addFilePath(String path) {
        if (!isHiEventSdkExist()) {
            return this;
        }
        this.event.addFilePath(path);
        return this;
    }

    public HiEvent reset() {
        if (!isHiEventSdkExist()) {
            return this;
        }
        this.event.reset();
        return this;
    }

    public HiEvent putAppInfo(Context context) {
        if (!isHiEventSdkExist() || context == null) {
            return this;
        }
        Context context2 = lastContext;
        if (context2 == null || !context2.equals(context)) {
            lastContext = context;
            try {
                name = context.getPackageName();
                PackageInfo packageInfo = context.getPackageManager().getPackageInfo(name, 0);
                if (packageInfo != null) {
                    version = packageInfo.versionName;
                }
            } catch (PackageManager.NameNotFoundException e) {
                Log.e(TAG, "HiEvent putAppInfo NameNotFoundException");
            }
        }
        this.event.putString(APP_NAME, name);
        this.event.putString(APP_VERSION, version);
        return this;
    }

    public int getId() {
        if (!isHiEventSdkExist()) {
            return -1;
        }
        return this.event.getId();
    }
}

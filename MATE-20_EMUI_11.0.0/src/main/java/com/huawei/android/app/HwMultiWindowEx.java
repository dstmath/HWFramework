...
public class HwMultiWindowEx {
    ...
    public static boolean isMultiWin(int taskId) {
        Method method;
        if (isInMultiWindowMode() && (method = sIsMultiWinMethod) != null) {
            try {
                return ((Boolean) method.invoke(sMultiWinService, Integer.valueOf(taskId))).booleanValue();
    ...
    public static Rect getMultiWinFrame(int taskId) {
        Rect frame = new Rect();
        Method method = sGetMultiWinFrameMethod;
        if (method != null) {
            try {
                method.invoke(sMultiWinService, Integer.valueOf(taskId), frame);
            } catch (IllegalAccessException e) {
                String str = TAG;
                Log.d(str, "call method " + method.getName() + " failed !!!");
            } catch (IllegalArgumentException e2) {
    ...
    public static boolean setStateChangeListener(StateChangeListener stateChangeListener) {
        if (BuildEx.VERSION.EMUI_SDK_INT >= 11) {
            return setStateChangeListener_N(stateChangeListener);
        }
        ...
        if (isMultiWindowSupported()) {
            synchronized (sLock) {
                ...
                if (!sStateChangeListeners.contains(stateChangeListener)) {
                    isListenerAdded = sStateChangeListeners.add(stateChangeListener);
                }
            }
            if (!sRegistered) {
    ...
    public static boolean unregisterStateChangeListener(StateChangeListener stateChangeListener) {
        ...
        if (BuildEx.VERSION.EMUI_SDK_INT >= 11) {
            return unregisterStateChangeListener_N(stateChangeListener);
        }
        boolean isListenerRemoved = false;
        List<StateChangeListener> list = sStateChangeListeners;
        if (list != null) {
            isListenerRemoved = list.remove(stateChangeListener);
    ...
    private static boolean registerThirdPartyCallBack(IMWThirdpartyCallback callBackReference) {
        Method method = sRegisterThirdPartyCallBackMethod;
        if (method != null) {
            try {
                return ((Boolean) method.invoke(sMultiWinService, callBackReference)).booleanValue();
            } catch (IllegalAccessException e) {
                String str = TAG;
                Log.d(str, "call method " + method.getName() + " failed !!!");
            } catch (IllegalArgumentException e2) {
    ...
    private static boolean unregisterThirdPartyCallBack(IMWThirdpartyCallback callBackReference) {
    ...
    private static boolean setStateChangeListener_N(StateChangeListener stateChangeListener) {
        boolean isListenerAdded = false;
        synchronized (sLock) {
            if (sStateChangeListeners == null) {
                sStateChangeListeners = new ArrayList();
    ...
    private static boolean unregisterStateChangeListener_N(StateChangeListener stateChangeListener) {
        ThirdpartyCallBackHandler thirdpartyCallBackHandler;
        boolean isListenerRemoved = false;
        List<StateChangeListener> list = sStateChangeListeners;
        if (list != null) {
    ...
    private static boolean registerThirdPartyCallBack_N(IMWThirdpartyCallback callBackReference) {
        return HwActivityTaskManager.registerThirdPartyCallBack(callBackReference);
    }

    private static boolean unregisterThirdPartyCallBack_N(IMWThirdpartyCallback callBackReference) {
        return HwActivityTaskManager.unregisterThirdPartyCallBack(callBackReference);
    }
    ...
    public static class ThirdpartyCallBackHandler extends IMWThirdpartyCallback.Stub {
        ...
        public void onModeChanged(boolean isChanged) {
            for (StateChangeListener listener : HwMultiWindowEx.sStateChangeListeners) {
                listener.onModeChanged(isChanged);
            }
        }

...
public class DevicePasswordManager {
    ...
    public boolean isKeyguardDisabled(ComponentName admin, int keyguardType) {
        Bundle bundle;
        if (keyguardType == 0 && (bundle = this.mDpm.getPolicy(admin, POLICY_KEYGUARD_DISABLED)) != null && bundle.getString(KEY_KEYGUARD_TYPE, DeviceSettingsManager.EMPTY_STRING).equals(String.valueOf(keyguardType))) {
            return Boolean.valueOf(bundle.getString(KEY_KEYGUARD_DISABLED, Boolean.FALSE.toString())).booleanValue();
        }
        return false;
    }

    public boolean setQuickToolsDisabled(ComponentName admin, boolean disabled) {
        Bundle bundle = new Bundle();
        bundle.putBoolean(KEY_QUICK_TOOLS, disabled);

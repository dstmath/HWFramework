...
class BrightnessTrainingAlgoImpl {
    ...
    private static String mAlgoXmlPath = StorageManagerExt.INVALID_KEY_DESC;
    ...
    private int readDragInfo() {
        ...
        algoParam.mDragInfo = StorageManagerExt.INVALID_KEY_DESC;
        int i = -1;
        if (items != null) {
            int i2 = 0;
            while (i2 < items.size()) {
    ...
    private int processAlgo() {
        ...
        DeLog.i(TAG, "ESCW " + ((int) outbuffer[0]) + ", " + ((int) outbuffer[1]) + ", " + ((int) outbuffer[2]) + ", " + ((int) outbuffer[3]) + ", eswValue = " + ((int) outbuffer[4]));
        writeBrightnessCurve(outbuffer);
        Bundle data = new Bundle();
        data.putInt("CurveUpdateFlag", 1);
        HwPowerManager.setHwBrightnessData("PersonalizedBrightness", data);
        synchronized (this.mLockJNI) {
            destroyAlgo();
        }
        return 0;
    ...
    public int processTraining() {
        ...
        if (this.mAlgoInfo.mLowLumaCount == 35 && this.mAlgoInfo.mMedialLumaCount == 35 && this.mAlgoInfo.mHighLumaCount == 35 && this.mAlgoInfo.mDefaultLumaCount == 35 && this.mAlgoInfo.mESCWCount == 1) {
            ...
            if (processAlgo() != 0) {
                DeLog.i(TAG, " processAlgo failed! ");
            }
            synchronized (this.mLockJNI) {
                int ret2 = DisplayEngineLibraries.nativeDeinitAlgorithm(1, this.mHandle);
                this.mHandle = -1;
                if (ret2 != 0) {
                    DeLog.e(TAG, " nativeDeinitAlgorithm failed! ");
                    return -1;
                }
                DeLog.i(TAG, "processTraining  stop ");
                return 0;
            }
        }

...
public class SensorAgent extends VirtualManager {
    ...
    private static final int FAST_MILLISECOND = 60;
    private static final int FIVE_SECONDS = 5000;
    private static final int NORMAL_MILLISECOND = 200;
    ...
    private static final int SLOW_MILLISECOND = 5000;
            ...
            if (listener != null) {
                if (sensor != null) {
                    if (sensor.getDeviceId() != null) {
                        if (isSensorMatchOldInterface(sensor)) {
                            ...
                            int ret = this.mDMSDPAdapter.subscribeSensorDataListener(kitlistener, new VirtualSensor(sensor.getDeviceId(), sensor.getSensorId(), sensor.getSensorType()), 5000);
                            if (ret != 0) {
    ...
    private boolean isSensorMatchOldInterface(VirtualSensor sensor) {
        return sensor.getSensorType() == 1 || sensor.getSensorType() == 2;
    }

    public int subscribeSensorDataListener(IVirtualSensorDataListener listener, VirtualSensor sensor, int rate) {
        synchronized (SENSOR_LOCK) {
            if (this.mDMSDPAdapter == null) {
                return -11;
            }
            ...
            if (!(listener == null || sensor == null)) {
                if (isRateValid(rate)) {
                    ...
                    int ret = this.mDMSDPAdapter.subscribeSensorDataListener(kitlistener, new VirtualSensor(sensor.getDeviceId(), sensor.getSensorId(), sensor.getSensorType()), convertRateTime(rate));
                    if (ret != 0) {
                        HwLog.i("VirtualSensorService", "subscribeSensorDataListener err:" + ret);
                        return ret;
                    }
    ...
    private boolean isRateValid(int rateType) {
    ...
    private int convertRateTime(int rateType) {
        if (rateType == 1) {
            return FAST_MILLISECOND;
        }
        if (rateType != 2) {
            return 5000;
        }

...
public class VirtualDeviceManager extends VirtualManager {
    ...
    private DMSDPListener dmsdpListener = new DMSDPListener() {
        ...
        public void onDeviceChange(DMSDPDevice device, int state, Map<String, Object> map) {
            ...
            if (state == 101) {
                ...
                VirtualDeviceManager.this.reportDeviceEvent(deviceId, state);
            } else if (state == 103 || state == 108 || state == -5) {
                VirtualDeviceManager.this.allReqeustServiceTypes.remove(deviceId);
                VirtualDeviceManager.this.reportDeviceEvent(deviceId, state);
            }
        }
    ...
    public DMSDPDeviceService getDmsdpDeviceService() {
        synchronized (DEVICE_MANAGER_LOCK) {
                ...
                if (!deviceId.isEmpty()) {
                    if (capability == null) {
                        ...
                        if (dmsdpDevice == null) {
                            ...
                            int serviceType = transformCapability(capability);
                            int serviceTypes = 0;
    ...
    private int dealDeviceType(DMSDPDevice dmsdpDevice, String deviceId, EnumSet<Capability> capability, Map<String, String> info) {
        ...
        if (deviceType == 2) {
            ...
            this.mDMSDPAdapter.requestDeviceService(3, dmsdpDevice, transformCapability(capability));
            setServiceFlag(deviceId, capability, info);
            return 0;
        } else if (deviceType == 4) {
            HwLog.d(TAG, "the deviceType is HICAR");
            this.mDMSDPAdapter.requestDeviceService(1, dmsdpDevice, transformCapability(capability));
            setServiceFlag(deviceId, capability, info);
            return 0;
            ...
            this.mDMSDPAdapter.requestDeviceService(2, dmsdpDevice, transformCapability(capability));
            setServiceFlag(deviceId, capability, info);
            return 0;
        }
    }
    ...
    public int getDeviceCapabilityInfo(String deviceId, EnumSet<Capability> capability, List<DMSDPDeviceService> services) {
        ...
        synchronized (DEVICE_MANAGER_LOCK) {
            for (Map.Entry<String, DMSDPDeviceService> entry : this.allRunningServices.entrySet()) {
                if (entry.getKey().startsWith(deviceId)) {
                    DMSDPDeviceService service = entry.getValue();
                    if ((service.getServiceType() & serviceType) != 0) {
    ...
    private void clearRequestServiceType(String deviceId, EnumSet<Capability> capabilities) {
        if (this.allReqeustServiceTypes.containsKey(deviceId)) {
            int newServiceType = this.allReqeustServiceTypes.get(deviceId).intValue() & (~transformCapability(capabilities));
            this.allReqeustServiceTypes.remove(deviceId);
            if (newServiceType != 0) {
                this.allReqeustServiceTypes.put(deviceId, Integer.valueOf(newServiceType));
            }
        }
    }

    private void clearNotConnectedDevices() {
        if (!this.allDevices.isEmpty()) {
            if (this.allConnectedDevice.isEmpty()) {
                HwLog.d(TAG, "not exist connected devices");
                this.allDevices.clear();
                return;
            }
            Iterator<Map.Entry<String, DMSDPDevice>> iter = this.allDevices.entrySet().iterator();
            while (iter.hasNext()) {
                if (!this.allConnectedDevice.containsKey(iter.next().getKey())) {
                    iter.remove();
                }
            }
        }
    ...
    public int updateDeviceService(DMSDPDeviceService service, int action, Map<String, Object> params) {
        ...
        synchronized (VIRTUALIZATION_LOCK) {
            updateDeviceService = this.mDMSDPAdapter.updateDeviceService(4, service, action, params);
        }
    ...
    private void enableDeviceService(DMSDPDeviceService service) {
        ...
        synchronized (VIRTUALIZATION_LOCK) {
            if (this.mDMSDPAdapter == null) {
                ...
                Map<String, DMSDPDeviceService> map = this.allRunningServices;
                map.put(service.getDeviceId() + DMSDPConfig.SPLIT + service.getServiceId(), service);
                if (service.getServiceType() == 1) {
    ...
    private void reportDeviceEvent(String deviceId, int event) {
        ...
        if (this.allVirtualDevices.get(deviceId) == null) {
            ...
            reportDeviceBusy(deviceId);
        } else {
            String str = TAG;
            HwLog.i(str, "reportDeviceEvent, in else,event is" + event);
        }
    }
    ...
    private void reportDeviceBusy(String deviceId) {
        if (this.virtualDeviceObserver != null) {
            HwLog.i(TAG, "report device busy");
            this.virtualDeviceObserver.onDeviceStateChange(this.allVirtualDevices.get(deviceId), 111);
        }
    }
    ...
    private int transformCapability(EnumSet<Capability> capability) {
        int res = 0;
    ...
    public int keepChannelActive(String deviceId, int duration) {
        if (this.mDMSDPAdapter == null) {
            HwLog.e(TAG, "keep channel active: mDMSDPAdapter is null");
            return -11;
        } else if (this.allConnectedDevice.containsKey(deviceId)) {
            return ReturnCodeConverter.convertReturnCode(this.mDMSDPAdapter.keepChannelActive(deviceId, duration));
        } else {
            HwLog.e(TAG, "keep channel active: device disconnected");
            return -9;
        }
    }
}

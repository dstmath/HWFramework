...
public class HwForceRotationManager implements IForceRotationManager {
    ...
    private boolean checkForceRotationService() {
        synchronized (this) {
            if (this.mService == null) {
                this.mService = IHwForceRotationManager.Stub.asInterface(ServiceManager.getService("forceRotationService"));
            }
        }
        if (this.mService != null) {
            return true;
    ...
    public boolean isForceRotationSwitchOpen() {
        ...
        try {
            return this.mService.isForceRotationSwitchOpen();
    ...
    public boolean isForceRotationSwitchOpen(Context context) {
        if (context != null && context.getContentResolver() != null && Settings.System.getInt(context.getContentResolver(), "force_rotation_mode", 0) == 1 && (!HwPCUtils.enabledInPad() || !HwPCUtils.isPcCastModeInServer())) {
            return true;
        }
        return false;
    }
    ...
    public boolean isAppInForceRotationWhiteList(String packageName) {
        ...
        try {
            return this.mService.isAppInForceRotationWhiteList(packageName);
        } catch (RemoteException ex) {
    ...
    public boolean isAppForceLandRotatable(String packageName, IBinder aToken) {
        ...
        try {
            return this.mService.isAppForceLandRotatable(packageName, aToken);
        } catch (RemoteException ex) {
    ...
    public boolean saveOrUpdateForceRotationAppInfo(String packageName, String componentName, IBinder aToken, int reqOrientation) {
        ...
        try {
            return this.mService.saveOrUpdateForceRotationAppInfo(packageName, componentName, aToken, reqOrientation);
        } catch (RemoteException ex) {
    ...
    public int recalculateWidthForForceRotation(int width, int height, int logicalHeight) {
        ...
        try {
            return this.mService.recalculateWidthForForceRotation(width, height, logicalHeight, ActivityThread.currentPackageName());
        } catch (RemoteException ex) {
            Slog.w(TAG, "applyForceRotationLayout,ex:" + ex);
            return width;
        }

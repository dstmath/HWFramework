...
public class CubicBezierInterpolator extends BaseInterpolator implements Interpolator {
    ...
    public CubicBezierInterpolator(Resources res, Resources.Theme theme, AttributeSet attrs) {
        ...
        this.mControlPoint2y = 0.0f;
        if (theme != null) {
            typedArray = theme.obtainStyledAttributes(attrs, R.styleable.TranslateAnimation, 0, 0);
        } else if (res != null) {
            typedArray = res.obtainAttributes(attrs, R.styleable.TranslateAnimation);
        } else {
            Log.e(TAG, " Cubic Bezier Interpolator input parameter is null");
            return;
        }
        this.mControlPoint1x = parseValue(typedArray.peekValue(0));
        this.mControlPoint1y = parseValue(typedArray.peekValue(2));
        this.mControlPoint2x = parseValue(typedArray.peekValue(1));
        this.mControlPoint2y = parseValue(typedArray.peekValue(3));
        typedArray.recycle();
    }
    ...
    private float parseValue(TypedValue value) {
        ...
        Log.e(TAG, "Wrong value type: " + value.type);
        return 1.0f;
    }

...
public final class SQLiteSession {
    ...
    public String[] getSQLTables(String sql, int connectionFlags, int rwFlags, CancellationSignal cancellationSignal) {
        if (sql != null) {
            if (cancellationSignal != null) {
                cancellationSignal.throwIfCanceled();
            }
            acquireConnection(sql, connectionFlags, cancellationSignal);
            try {
                return this.mConnection.getSQLTables(sql, rwFlags);
            } finally {
                releaseConnection();
            }
        } else {
    ...
    /* access modifiers changed from: package-private */
    public SQLiteConnection.PreparedStatement beginStepQuery(String sql, int connectionFlags, CancellationSignal cancellationSignal) {
        acquireConnection(sql, connectionFlags, cancellationSignal);

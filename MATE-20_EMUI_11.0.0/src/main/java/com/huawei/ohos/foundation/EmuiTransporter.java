...
public final class EmuiTransporter {
        ...
        if (iRemoteObject != null && elementName != null && iBinder != null) {
            ...
            if (convertToHarymonyRemote.isPresent()) {
                ...
                try {
                    ...
                    HiLog.debug(DMS_LABEL, "transactConnectToRemoteService result %{public}b", new Object[]{Boolean.valueOf(iRemoteObject.sendRequest(1, obtain, obtain2, new MessageOption()))});
                    obtain.reclaim();
                    obtain2.reclaim();
    ...
    static void transactDisconnectToRemoteService(IRemoteObject iRemoteObject, ElementName elementName, int i) {
        ...
        if (iRemoteObject != null && elementName != null) {
            ...
            try {
                ...
                HiLog.debug(DMS_LABEL, "EmuiTransporter: transactDisconnectToRemoteService result %{public}b", new Object[]{Boolean.valueOf(iRemoteObject.sendRequest(2, obtain, obtain2, new MessageOption()))});
                obtain.reclaim();
                obtain2.reclaim();
            } catch (RemoteException unused) {
    ...
    private static Optional<IRemoteObject> convertToHarymonyRemote(IBinder iBinder) {
        return IPCAdapter.translateToIRemoteObject(iBinder);
    }

    private static boolean writeConnectParcel(MessageParcel messageParcel, ElementName elementName, IRemoteObject iRemoteObject, int i) {
        if (messageParcel.writeInterfaceToken(INTERFACE_TOKEN) && new ElementNameAdapter(elementName).marshalling(messageParcel) && messageParcel.writeRemoteObject(iRemoteObject) && messageParcel.writeInt(i)) {
            return true;
        }
        return false;
    }

    private static boolean writeDisConnectParcel(MessageParcel messageParcel, ElementName elementName, int i) {
        if (messageParcel.writeInterfaceToken(INTERFACE_TOKEN) && new ElementNameAdapter(elementName).marshalling(messageParcel) && messageParcel.writeInt(i)) {
            return true;
        }
        return false;
    }

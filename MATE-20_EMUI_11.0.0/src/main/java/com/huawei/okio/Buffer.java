...
public final class Buffer implements BufferedSource, BufferedSink, Cloneable, ByteChannel {
    ...
    /* JADX INFO: Multiple debug info for r11v1 int: [D('possiblePrefixIndex' int), D('triePos' int)] */
    ...
    public int selectPrefix(Options options, boolean selectTruncated) {
        ...
        if (head2 != null) {
            ...
            while (true) {
                ...
                int triePos5 = trie[triePos2];
                if (triePos5 != -1) {
                    prefixIndex = triePos5;
                }
                if (s == null) {
                    break;
                }
                if (triePos3 < 0) {
                    ...
                    while (true) {
                        int pos2 = pos + 1;
                        int triePos6 = triePos4 + 1;
                        if ((data[pos] & 255) != trie[triePos4]) {
                            return prefixIndex;
                        }
                        boolean scanComplete = triePos6 == trieLimit;
                        if (pos2 == limit) {
                            s = s.next;
                            pos2 = s.pos;
                            data = s.data;
                    ...
                    while (triePos4 != selectLimit) {
                        if (b == trie[triePos4]) {
                            int nextStep2 = trie[triePos4 + triePos3];
                            if (pos3 == limit) {
                                s = s.next;
                                int pos4 = s.pos;
                                data = s.data;

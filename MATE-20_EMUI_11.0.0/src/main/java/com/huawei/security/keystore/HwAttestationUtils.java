...
public abstract class HwAttestationUtils {
    ...
    private static X509Certificate[] convertToX509CertChain(Collection<byte[]> rawChain) throws IOException, CertificateException {
        X509Certificate[] retCerts = new X509Certificate[rawChain.size()];
        CertificateFactory certFactory = CertificateFactory.getInstance("X.509");
        int index = 0;
        for (byte[] cert : rawChain) {
            retCerts[index] = (X509Certificate) certFactory.generateCertificate(new ByteArrayInputStream(cert));
            index++;
        }
        Log.i(TAG, "retCerts length is: " + retCerts.length);
        return retCerts;
    }

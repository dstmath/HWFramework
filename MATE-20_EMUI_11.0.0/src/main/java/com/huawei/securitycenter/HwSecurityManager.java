...
public class HwSecurityManager {
    ...
    public Bundle checkUninstalledApk(@NonNull String filePath, String sourcePkg, long timeOutMillis) {
        if ("tv".equals(SystemPropertiesEx.get("ro.build.characteristics", "default"))) {
            Log.i(TAG, "is tv mode");
            return HwTvSecurityManager.getInstance().checkUninstalledApk(filePath, sourcePkg, timeOutMillis);
        }
        try {

...
public final class KernelInterface {
    ...
    public boolean changeSwapOutSwitch(boolean isOpenSwapOut) {
        ...
        if (!writeDevMemcg("/sys/block/zram0/hyperhold_enable", content)) {
            Slog.e(TAG, "nandlife change swapout switch failed");
            return false;
        }
        ...
        try {
            if (logFile.createNewFile()) {
                ...
                if (FileUtils.setPermissions(logFile.getPath(), PRIVILEGE, -1, -1) != 0) {
                    Slog.e(TAG, "log_priv: ak_kill_app.log permission grant failed.");
                }
            }
            if (isOverWrite) {

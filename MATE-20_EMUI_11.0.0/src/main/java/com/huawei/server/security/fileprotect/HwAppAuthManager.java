...
public class HwAppAuthManager extends DefaultHwAppAuthManager {
    ...
    private static class SingletonHolder {
        private static final HwAppAuthManager INSTANCE = (IS_INIT_SUCCESS ? new HwAppAuthManager() : new HwaaDisabledManager());
    ...
    public void notifyPMSReady(Context context) {
        ...
        List<HwaaPackageInfo> protectedPkgInfos = new ArrayList<>(syncPkgs.size());
        ...
        HwAppAuth.syncInstalledPackages((HwaaPackageInfo[]) protectedPkgInfos.toArray(new HwaaPackageInfo[0]));
    ...
    private HwaaPackageInfo getHwaaPackageInfoByPkg(String packageName) {
        ...
        HwaaPackageInfo.Builder builder = new HwaaPackageInfo.Builder();
        builder.setAppId(appId).setPackageName(packageName).setSharedUserId(pkgInfo.sharedUserId).addDefaultPolicy(ApplicationInfoEx.getCredentialProtectedDataDir(appInfo));

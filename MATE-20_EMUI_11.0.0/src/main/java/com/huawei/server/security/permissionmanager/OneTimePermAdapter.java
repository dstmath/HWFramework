...
public class OneTimePermAdapter {
    ...
    public Bundle getOneTimePermissionInfo() {
        ...
        synchronized (LOCK) {
            if (!this.isInitialized) {
                SlogEx.w(LOG_TAG, "query before initialize or update success.");
            }
                ...
                if (updatePackageNames.size() <= 10000) {
                    ...
                    synchronized (LOCK) {
                        if (isInfoTypeValid(infoType)) {
                            ...
                            updateAllInDb(infoType, columnPackageName, updatePackageNames);
    ...
    public void initInfoForInstalledPkg() {
        ...
        synchronized (LOCK) {
            if (this.isInitialized) {
                SlogEx.i(LOG_TAG, "get initialized in update, give up this initialize.");
                return;
            }
            Cursor cursor = null;
            try {
    ...
    private void updateAllInDb(String infoType, String columnPackageName, List<String> updatePackageNames) {
        ...
        this.mHwPermDbHelper.removeAndUpdateAllForOneTimePerm(infoType, contentValuesList);
    }
}

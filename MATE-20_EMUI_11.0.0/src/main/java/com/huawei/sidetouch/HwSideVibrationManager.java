...
public class HwSideVibrationManager {
    ...
    public void notifyThpEvent(int event) {
        ...
        boolean isTouchEvent = (196608 & event) != 0;
        if ((event & 12288) != 0) {
            isVolumeEvent = true;
        }
    ...
    private void vibrateForSideVolumeEvent(KeyEvent event, boolean isAudioPlaying, boolean isScreenOn, boolean isKeyguardShowing) {
            ...
            if (isScreenOn || isAudioPlaying) {
                ...
                if (streamType2 < 0) {
                    ...
                    if (currentlevel == minVolumeLevel) {
                        ...
                        processVibrate(event, true, 4, curTime);
                    } else {
                        scheduleVibrate(1);
                    }
                }
    ...
    public void onSideVolumeEvent(KeyEvent event, boolean isAudioPlaying, boolean isScreenOn, boolean isKeyguardShowing) {
        vibrateForSideVolumeEvent(event, isAudioPlaying, isScreenOn, isKeyguardShowing);
    }
    ...
    public void onNormalVolumeEvent(KeyEvent event, boolean isInjected, boolean isScreenOn) {
        ...
        if (!isInjected && isScreenOn) {
            this.mShouldVibrateWhenVolumeChanged = false;
        }
    }

...
import huawei.android.filemonitor.IFileMonitorManager;
import java.util.List;

public class HwFileMonitorManager {
    private static final String TAG = "HwFileMonitorManager";
    private static volatile HwFileMonitorManager sInstance = null;
    ...
    public static synchronized HwFileMonitorManager getInstance() {
        ...
        synchronized (HwFileMonitorManager.class) {
            if (sInstance == null) {
                sInstance = new HwFileMonitorManager();
            }
            hwFileMonitorManager = sInstance;
        }
    ...
    private IFileMonitorManager getService() {
        return IFileMonitorManager.Stub.asInterface(ServiceManager.getService("HwFileMonitorService"));
    }

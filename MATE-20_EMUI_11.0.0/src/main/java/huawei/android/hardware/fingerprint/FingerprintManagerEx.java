...
public class FingerprintManagerEx {
    ...
    private static int[] getPositionFromHal(int position_type) {
        ...
        if (binder != null) {
            try {
                ...
                binder.transact(position_type, data, reply, 0);
                reply.readException();
                ...
                Log.e(TAG, "getPositionFromHal with RemoteException happened: " + position_type);
            } catch (Throwable th) {
                reply.recycle();
                data.recycle();
                throw th;

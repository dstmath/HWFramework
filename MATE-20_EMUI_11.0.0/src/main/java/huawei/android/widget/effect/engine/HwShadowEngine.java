...
public class HwShadowEngine {
    ...
    private static final boolean IS_EMUI_LITE = SystemUtil.getSystemProperty("ro.build.hw_emui_lite.enable", (boolean) IS_EMUI_LITE);
    ...
    private Bitmap getBitmapForBlur(View targetView) {
        ...
        if (bitmap == null || this.mViewCanvas == null || bitmap.getWidth() != ((int) viewBitmapW) || this.mViewBitmap.getHeight() != ((int) viewBitmapH)) {
            if (viewBitmapW <= 0.0f || viewBitmapH <= 0.0f) {
        ...
        this.mViewCanvas.clipRect(0.0f, 0.0f, (float) targetW, bottomH);
        this.mViewCanvas.translate(0.0f, bottomH - ((float) targetH));

    ...
    public LocalePicker.LocaleInfo[] addModifyLocaleInfos(Context context, LocalePicker.LocaleInfo[] localeInfos) {
        ...
        for (int i = 0; i < finalSize; i++) {
            if (localeInfos[i].getLabel().equals("FYROM") || localeInfos[i].getLabel().equals(apStr2)) {
                if (!StorageManagerExt.INVALID_KEY_DESC.equals(simOptr) && "294".equals(simOptr)) {
                    newlabel = apStr2;
                } else if (!StorageManagerExt.INVALID_KEY_DESC.equals(simOptr) || !SystemProperties.getBoolean("ro.config.MkLanguageCust", false)) {
                    newlabel = "FYROM";
                } else {

...
public final class AbilitySliceScheduler {
    ...
    private boolean checkLegalForAdd(AbilitySlice abilitySlice, AbilitySlice abilitySlice2, Intent intent, int i) {
        if (this.topAbilitySlice == null) {
            ...
            return false;
        } else if (this.abilitySliceManager.getAbilityState() != 2) {
    ...
    private boolean checkLegalForAddWithResult(AbilitySlice abilitySlice) {
        if (this.topAbilitySlice == null) {
            Log.error(LABEL, "topAbilitySlice is null, not allow jump.", new Object[0]);
            return false;
        } else if (!this.abilitySliceStack.exist(abilitySlice) && this.topAbilitySlice != abilitySlice) {
            return true;
        } else {
    ...
    private boolean checkLegalForRemove(AbilitySlice abilitySlice) {
        if (this.topAbilitySlice == null) {
            Log.error(LABEL, "topAbilitySlice is null, not allow jump.", new Object[0]);
            return false;
        } else if (this.abilitySliceManager.getAbilityState() == 0 || this.abilitySliceManager.getAbilityState() == -1) {
            ...
            Log.error(LABEL, "Slice is not in the stack and not on top, can't terminate.", new Object[0]);
            return false;
        } else if (abilitySlice.getState() == AbilitySliceLifecycleExecutor.LifecycleState.INITIAL) {
    ...
    private void addAbilitySliceSchedule(AbilitySlice abilitySlice, AbilitySlice abilitySlice2, Intent intent) {
        ...
        if (abilitySlice2.getState() == AbilitySliceLifecycleExecutor.LifecycleState.INITIAL) {
            ...
            if (transitionController == null || !transitionController.isTransitionEnabled()) {
                abilitySlice2.viewEnterAnimator();
            } else {
    ...
    /* access modifiers changed from: package-private */
    public void addAbilitySlice(final AbilitySlice abilitySlice, final AbilitySlice abilitySlice2, final Intent intent) {
            ...
            this.taskDispatcher.asyncDispatch(new Runnable() {
                ...
                public void run() {
                    synchronized (AbilitySliceScheduler.this.syncLock) {
                        Bytrace.finishAsyncTrace(2147483648L, "present", hashCode);
                        if (!AbilitySliceScheduler.this.checkLegalForAdd(abilitySlice, abilitySlice2, intent, -1)) {
                            if (!(intent == null || (intent.getFlags() & 67108864) == 0)) {
                        ...
                        if (!(intent == null || (intent.getFlags() & 67108864) == 0)) {
                            if (Log.isDebuggable()) {
                                Log.debug(AbilitySliceScheduler.LABEL, "add AbilitySlice with forward flag. caller: %{public}s, target: %{public}s.", abilitySlice.getClass().getName(), abilitySlice2.getClass().getName());
                            }
                            AbilitySliceScheduler.this.abilitySliceJumpRecordsSet.updateForwardCaller(abilitySlice, abilitySlice2);
                        }
    ...
    private void removeAllAbilitySlice() {
        for (AbilitySlice abilitySlice : this.abilitySliceStack.getAllSlices()) {
            ...
            removeAbilitySlice(abilitySlice, null);
        }
    ...
    public class RemoveAbilitySliceTask implements Runnable {
        ...
        private void scheduleSliceWhenAbilityBackground() {
            ...
            pop.setLatestUIAttachedFlag(false);
            if (AbilitySliceScheduler.this.abilitySliceManager.getAbilityState() == 1) {
                pop.foreground(this.intent);
            }
            AbilitySliceScheduler.this.topAbilitySlice.background();
            AbilitySliceScheduler.this.stopAbilitySlice(this.abilitySlice);
        ...
        public void run() {
            synchronized (AbilitySliceScheduler.this.syncLock) {
                ...
                if (AbilitySliceScheduler.this.checkLegalForRemove(this.abilitySlice)) {
                    ...
                    if (this.abilitySlice != AbilitySliceScheduler.this.topAbilitySlice) {
                        AbilitySliceScheduler.this.stopAbilitySlice(this.abilitySlice);
                    ...
                    } else if (AbilitySliceScheduler.this.abilitySliceManager.getAbilityState() == 2) {
    ...
    /* access modifiers changed from: package-private */
    public void removeAbilitySlice(AbilitySlice abilitySlice, Intent intent) {
    ...
    public void handleActiveAbilitySlice(Intent intent) {
        ...
        if (this.topAbilitySlice != null) {
            if (intent == null || this.topAbilitySlice == (targetAbilitySlice = getTargetAbilitySlice(intent))) {
                try {
                ...
                this.topAbilitySlice.present(targetAbilitySlice, intent);
            }
        }
    }
    ...
    public void handleMoveAbilitySliceToForeground(Intent intent) {
        if (this.topAbilitySlice != null) {
            AbilitySlice targetAbilitySlice = getTargetAbilitySlice(intent);
            if (targetAbilitySlice != null) {
                try {
                    if (this.topAbilitySlice.equals(targetAbilitySlice)) {
                        this.topAbilitySlice.foreground(intent);
                    } else if (this.abilitySliceStack.exist(targetAbilitySlice)) {
                        targetAbilitySlice.foreground(intent);
                        while (this.abilitySliceStack.top() != targetAbilitySlice) {
                            stopAbilitySlice(this.abilitySliceStack.pop());
                        }
                        this.abilitySliceStack.pop();
                    } else {
                        targetAbilitySlice.start(intent);
                        this.abilitySliceStack.push(this.topAbilitySlice);
                    }
                    this.topAbilitySlice = targetAbilitySlice;
                } catch (LifecycleException unused) {
                    throw new IllegalStateException("Unable handle transaction when:" + this.topAbilitySlice.getState());
                }
            } else {
                throw new IllegalStateException("Get target AbilitySlice instance failed.");
            }
        }
    }
    ...
    public void dumpSliceStack(String str, PrintWriter printWriter, String[] strArr) {
        ...
        printWriter.println("[Ability slice name: " + this.topAbilitySlice.getClass().getName() + "]");
        AbilitySlice abilitySlice = this.topAbilitySlice;
        abilitySlice.dumpAbilitySlice(Ability.PREFIX + str, printWriter, strArr[1]);
        synchronized (this.syncLock) {
            for (AbilitySlice abilitySlice2 : this.abilitySliceStack.getAllSlices()) {
                printWriter.print(str);
                printWriter.println("[Ability slice name: " + abilitySlice2.getClass().getName() + "]");
                StringBuilder sb = new StringBuilder();
                sb.append(Ability.PREFIX);
                sb.append(str);
                abilitySlice2.dumpAbilitySlice(sb.toString(), printWriter, "");
            }
        }
    }

...
public class AbilityWindow {
    ...
    public void setUIContent(ComponentContainer componentContainer) {
        if (this.window == null) {
            reportHiviewEvent(1);
            Log.error(LABEL, "window is null, setUIContent with viewGroup failed", new Object[0]);
            return;
        }
        if (Log.isDebuggable()) {
            KeyLog.debugBound("[%{public}s][%{public}s][%{public}s]: attach layout viewGroup to window: [%{public}s]", LABEL.getTag(), KeyLog.SET_UI_CONTENT, KeyLog.LogState.START, componentContainer);
        }
    ...
    public void setUIContent(String str) {
        if (this.window == null) {
            Log.error(LABEL, "window is null, setUIContent with xmlPath failed", new Object[0]);
            return;
        }
        if (Log.isDebuggable()) {
            KeyLog.debugBound("[%{public}s][%{public}s][%{public}s]: attach layout xml path to window: [%{public}s]", LABEL.getTag(), KeyLog.SET_UI_CONTENT, KeyLog.LogState.START, str);
        }
        this.window.setContentLayout(str);
    }
    ...
    public void setWindowAttribute(Intent intent) {
        ...
        if (updateLayoutParamInt(intent, "PageLayoutGravity", attributes.gravity, new Consumer() {
    ...
    public Component findComponentById(int i) {
        ...
        Log.error(LABEL, "window is null, find view failed", new Object[0]);
        return null;
    }
    ...
    public WindowManager.LayoutConfig getLayoutParams() {
        ...
        layoutConfig.gravity = attributes.gravity;
    ...
    public void setLayoutParams(WindowManager.LayoutConfig layoutConfig) {
        if (layoutConfig == null) {
            ...
            layoutParams.gravity = layoutConfig.gravity;

...
public class ViewsStatus implements Sequenceable {
    ...
    public static final Sequenceable.Producer<ViewsStatus> PRODUCER = $$Lambda$ViewsStatus$M7OnU2PGqqTrEY9OTKfSaYsWhQ.INSTANCE;
    ...
    static /* synthetic */ ViewsStatus lambda$static$0(Parcel parcel) {
        ViewsStatus viewsStatus = new ViewsStatus();
        viewsStatus.unmarshalling(parcel);
        return viewsStatus;
    }

    static ViewsStatus buildViewsStatus(ComponentProvider componentProvider, Set<Integer> set, Intent intent) {
        if (componentProvider == null) {
            HiLog.error(LABEL, "views is null when build views status", new Object[0]);
            return null;
        } else if (set == null) {
            HiLog.error(LABEL, "viewIds Listen is null when build views status", new Object[0]);
            return null;
        } else {
            ViewsStatus viewsStatus = new ViewsStatus();
            ComponentContainer allComponents = componentProvider.getAllComponents();
            if (allComponents == null) {
                HiLog.error(LABEL, "view group is null when build views status", new Object[0]);
                return null;
            }
            for (Integer num : set) {
                ...
                if (findComponentById == null) {
                    HiLog.error(LABEL, "unavailable view id when build views status. view id: %{public}d", new Object[]{Integer.valueOf(intValue)});
                } else {
    ...
    private static Intent getViewStatus(Component component) {
        ...
        if (component == null) {
            HiLog.error(LABEL, "view is null when get view status, return empty intent.", new Object[0]);
            return intent;
        }
        if (component instanceof Button) {
            getStatusFromButton(intent, (Button) component);
        } else if (component instanceof Text) {
            getStatusFromTextView(intent, (Text) component);
        } else {
            HiLog.error(LABEL, "do not support view. view: %{public}s", new Object[]{component.getClass()});
        }
        return intent;
    }

    private static void getStatusFromTextView(Intent intent, Text text) {
        intent.setParam("text", text.getText());
    }

    private static void getStatusFromButton(Intent intent, Button button) {
        getStatusFromTextView(intent, button);
    }
    ...
    public Intent getStatusByViewId(int i) {

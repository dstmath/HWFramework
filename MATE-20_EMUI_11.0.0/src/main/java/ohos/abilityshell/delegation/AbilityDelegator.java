...
public class AbilityDelegator implements IAbilityDelegator {
    ...
    @Override // ohos.aafwk.ability.delegation.IAbilityDelegator
    public IAbilityMonitor addAbilityMonitor(IntentFilter intentFilter) {
        AbilityMonitor abilityMonitor = new AbilityMonitor(intentFilter);
        synchronized (LOCK) {
            this.abilityMonitors.add(abilityMonitor);
        }
        return abilityMonitor;
    ...
    private Optional<Object> getAbilityShell(Ability ability) {
        ...
        try {
            Method declaredMethod = AbilityContext.class.getDeclaredMethod("getAbilityShell", new Class[0]);
    ...
    public boolean triggerClickEvent(Ability ability, Component component) {
        ...
        if (ability == null) {
            ...
            AppLog.e(LABEL, "view is null", new Object[0]);
            return false;
        } else {
            FutureTask futureTask = new FutureTask(new Callable() {
                /* class ohos.abilityshell.delegation.$$Lambda$ZB9BGGuLUcoMe5kTaGvSCMv6c */

                @Override // java.util.concurrent.Callable
                public final Object call() {
                    return Boolean.valueOf(Component.this.performClick());
                }
            });
            if (runOnUIThreadSync(futureTask)) {
                try {
    ...
    public boolean doAbilitySliceStop(Ability ability) {
        ...
        try {
            AbilityHelper.reflectAbilitySliceLifecycle(ability, new Intent(), AbilityHelper.LifecycleAction.STOP);
            return true;
        } catch (AbilityDelegatorException e) {
            AppLog.e(LABEL, "stopAbility failed %{public}s", e.getMessage());
            return false;
        }
    }
    ...
    private boolean parseCasesAndRunner(Intent intent) {
        ...
        if (split.length == 1) {
            ...
            AppLog.e(LABEL, "unittest parameter is invalid, there is no unittest found in %{public}s", new Object[0]);
            return false;
        }
        ...
        AppLog.w(LABEL, "unittest parameter is invalid, there is no unittest found in %{public}s", new Object[0]);
        return false;
    }
        ...
        static int reflectGetAbilitySliceState(AbilitySlice abilitySlice) {
            try {
                ...
                if (invoke instanceof AbilitySliceLifecycleExecutor.LifecycleState) {
                    return ((AbilitySliceLifecycleExecutor.LifecycleState) invoke).toValue();
                }
                return -1;

...
public abstract class AccessibleAbility extends Ability {
    ...
    public interface AccessibilityCallbacks {
        ...
        void onDisplyResizeChanged(int i, Rect rect, float f, float f2, float f3);
    ...
    public void onPerformGestureResult(int i, boolean z) {
        ...
        if (!this.mGestureListenerInfos.isEmpty()) {
            ...
            if (gestureResultListenerInfo != null && gestureResultListenerInfo.getmGestureResultListener() != null) {
                if (z) {
                    gestureResultListenerInfo.getmGestureResultListener().onCompleted();
                } else {
                    gestureResultListenerInfo.getmGestureResultListener().onCancelled();
                }
            }
        }
    }
    ...
    private AccessibilityCallbacks getAccessibilityCallbackImpl() {
        return new AccessibilityCallbacks() {
            ...
            @Override // ohos.accessibility.ability.AccessibleAbility.AccessibilityCallbacks
            public void onDisplyResizeChanged(int i, Rect rect, float f, float f2, float f3) {

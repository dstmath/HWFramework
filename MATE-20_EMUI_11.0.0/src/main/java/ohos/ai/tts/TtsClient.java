...
public class TtsClient {
    ...
    public static final int METHOD_SPEAK_BATCH_TEXT = 1002;
    ...
    public synchronized void release() {
        ...
        try {
            ...
            sendCallback(100, METHOD_RELEASE);
        } catch (RemoteException unused) {
            HiAILog.error(TAG, "release RemoteException");
            sendCallback(101, METHOD_RELEASE);
        }
    }
    ...
    public synchronized void setIsSaveTtsData(boolean z) {
        ...
        try {
            ...
            sendCallback(100, METHOD_SET_IS_SAVE_TTS_DATA);
        } catch (RemoteException unused) {
            HiAILog.error(TAG, "setIsSaveTtsData RemoteException");
            sendCallback(101, METHOD_SET_IS_SAVE_TTS_DATA);
        }
    }
    ...
    public synchronized void speakBatchText(List<String> list, String str) {
        String str2;
        HiAILog.info(TAG, "speakBatchText");
        if (!isCreated()) {
            HiAILog.error(TAG, "Tts client was not created, speakBatchText failed");
            return;
        }
        if (str != null) {
            try {
                ...
                HiAILog.error(TAG, "speakBatchText RemoteException");
                sendCallback(101, METHOD_SPEAK_BATCH_TEXT);
            }
        } else {
            str2 = null;
        }
        this.ttsService.speakBatchText(list, str2, this.ttsCallback);
        sendCallback(100, METHOD_SPEAK_BATCH_TEXT);
    }
    ...
    public synchronized void speakLongText(String str, String str2) {
        ...
        if (str2 != null) {
            try {
                ...
                sendCallback(101, METHOD_SPEAK_LONG_TEXT);
            }
        } else {
            str3 = null;
        }
        this.ttsService.speakLongText(str, str3, this.ttsCallback);
        sendCallback(100, METHOD_SPEAK_LONG_TEXT);
    }
    ...
    public synchronized void stopSpeak() {
        ...
        try {
            ...
            sendCallback(100, METHOD_STOP_SPEAK);
        } catch (RemoteException unused) {
            HiAILog.error(TAG, "stopSpeak RemoteException");
            sendCallback(101, METHOD_STOP_SPEAK);
        }
    }

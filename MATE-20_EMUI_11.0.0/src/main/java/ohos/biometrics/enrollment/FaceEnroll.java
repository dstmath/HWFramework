...
class FaceEnroll {
    ...
    private FaceEnroll(Ability ability) {
        this.mProxy = new FaceEnrollProxy(ability);
    }

    static FaceEnroll getInstance(Ability ability) {
        if (sInstance == null) {
            synchronized (FaceEnroll.class) {
                if (sInstance == null) {
                    sInstance = new FaceEnroll(ability);
                }
            }
        }
        return sInstance;

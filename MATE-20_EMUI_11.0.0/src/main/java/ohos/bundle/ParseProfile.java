...
public class ParseProfile {
    ...
    /* JADX WARNING: Removed duplicated region for block: B:27:0x0063 A[ExcHandler: all (th java.lang.Throwable), Splitter:B:17:0x0041] */
    public static BundleInfo parse(Context context, String str, int i) {
        ...
        if (str == null || str.isEmpty()) {
            AppLog.e("ParseProfile::parse srcPath, fileName is empty!", new Object[0]);
            return null;
        }
        try {
            zipFile = new ZipFile(new File(getFormattedPath(context, str)));
            try {
                ZipEntry entry = zipFile.getEntry(HARMONY_PROFILE);
                if (entry == null) {

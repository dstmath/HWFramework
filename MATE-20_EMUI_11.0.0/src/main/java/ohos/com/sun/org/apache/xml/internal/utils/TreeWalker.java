...
public class TreeWalker {
    ...
    private final void dispatachChars(Node node) throws SAXException {
        DOM2DTM.CharacterNodeHandler characterNodeHandler = this.m_contentHandler;
        if (characterNodeHandler instanceof DOM2DTM.CharacterNodeHandler) {
            characterNodeHandler.characters(node);
            return;
        }
    ...
    public void startNode(Node node) throws SAXException {
        String str;
        NodeConsumer nodeConsumer = this.m_contentHandler;
        if (nodeConsumer instanceof NodeConsumer) {
            nodeConsumer.setOriginatingNode(node);
        }

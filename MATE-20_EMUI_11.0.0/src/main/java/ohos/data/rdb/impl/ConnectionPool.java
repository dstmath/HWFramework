...
public class ConnectionPool {
    ...
    private static void checkConnectionPoolConfig(ConnectionPool connectionPool, SqliteDatabaseConfig sqliteDatabaseConfig) {
        if (!connectionPool.config.getJournalMode().equalsIgnoreCase(sqliteDatabaseConfig.getJournalMode())) {
            throw new IllegalArgumentException("Journal mode cannot be set after opening the database");
        } else if (!connectionPool.config.getEncryptKeyLoader().equals(sqliteDatabaseConfig.getEncryptKeyLoader())) {
            throw new IllegalArgumentException("Encrypt cannot be set after opening the database");
        }
    }

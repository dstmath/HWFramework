...
public class MeasureFormat extends UFormat {
    ...
    private MeasureFormat(ULocale uLocale, FormatWidth formatWidth2, NumberFormat numberFormat2, PluralRules pluralRules, NumericFormatters numericFormatters2) {
        ...
        if (numericFormatters2 == null && formatWidth2 == FormatWidth.NUMERIC && (numericFormatters2 = localeToNumericDurationFormatters.get(uLocale)) == null) {
    ...
    private synchronized LocalizedNumberFormatter getUnitFormatterFromCache(int i, MeasureUnit measureUnit, MeasureUnit measureUnit2) {
        ...
        if (i == 1) {
            localizedNumberFormatter = (LocalizedNumberFormatter) ((LocalizedNumberFormatter) ((LocalizedNumberFormatter) getNumberFormatter().unit(measureUnit)).perUnit(measureUnit2)).unitWidth(this.formatWidth.unitWidth);
        } else if (i == 2) {
            localizedNumberFormatter = (LocalizedNumberFormatter) ((LocalizedNumberFormatter) ((LocalizedNumberFormatter) NumberFormatter.withLocale(getLocale()).unit(measureUnit)).perUnit(measureUnit2)).unitWidth(this.formatWidth.currencyWidth);
        } else {
            localizedNumberFormatter = (LocalizedNumberFormatter) ((LocalizedNumberFormatter) ((LocalizedNumberFormatter) ((LocalizedNumberFormatter) getNumberFormatter().unit(measureUnit)).perUnit(measureUnit2)).unitWidth(this.formatWidth.unitWidth)).precision(Precision.integer().withMode(RoundingUtils.mathContextUnlimited(RoundingMode.DOWN)));
        }
    ...
    private static Number[] toHMS(Measure[] measureArr) {
        ...
        while (i2 < length) {
            ...
            if (measure.getNumber().doubleValue() < XPath.MATCH_SCORE_QNAME || (num = hmsTo012.get(measure.getUnit())) == null || (intValue = num.intValue()) <= i) {
                return null;
            }
            ...
            if (charAt == '\'') {
                ...
                if (z) {
                    ...
                    if (i >= str.length() || str.charAt(i) != charAt) {
                        ...
                        formattedStringBuilder.append(localizedNumberFormatter.format(num), (Format.Field) null);
                    }
                }
                i2++;
                c = 0;

...
public class LightManager {
    ...
    public List<Integer> getLightIdList() {
        ...
        HiLog.error(TAG, "the lightId list is empty", new Object[0]);
        return Collections.emptyList();
    }
    ...
    public boolean isEffectSupport(int i, String str) {
        ...
        HiLog.error(TAG, "the light effect is not support", new Object[0]);
        return false;
    }

    public boolean turnOn(int i, LightEffectUtil lightEffectUtil) {
        HiLog.info(TAG, "light turnOn with lightEffect start", new Object[0]);
        if (!isNativeInitSuccess() || !isLightIdValid(i)) {
            return false;
        }
        if (lightEffectUtil == null) {
            HiLog.error(TAG, "the light lightEffect is invalid", new Object[0]);
            return false;
        }
        ...
        HiLog.info(TAG, "light turnOn end", new Object[0]);
        return true;
    }

    public boolean turnOn(int i, String str) {
        HiLog.info(TAG, "light turnOn with effectId start", new Object[0]);
        if (!isNativeInitSuccess() || !isLightIdValid(i) || !isLightEffectIdValid(str)) {
            return false;
        }
    ...
    public boolean turnOff(int i) {
        HiLog.info(TAG, "light turnOff start", new Object[0]);
        if (!isNativeInitSuccess() || !isLightIdValid(i)) {
            return false;
        }
        if (nativeDisableLight(i) != 0) {
            HiLog.error(TAG, "the light turnOff is failed", new Object[0]);
            return false;
        }
        HiLog.info(TAG, "light turnOff end", new Object[0]);
        return true;
    }
    ...
    private static void getAllLight() {
        if (!hasGetLightIdList) {
            ...
            HiLog.debug(TAG, "get light id list is success", new Object[0]);
        }
    }
    ...
    private boolean isLightIdValid(int i) {
        ...
        HiLog.error(TAG, "the light Id is not exist", new Object[0]);
        return false;
    }

    private boolean isLightEffectIdValid(String str) {
        if (str == null || str.isEmpty()) {
            HiLog.error(TAG, "the light effectId is isEmpty", new Object[0]);
            return false;
        } else if (LightConstant.LIGHT_EFFECT_ID_LIST.contains(str)) {
            return true;
        } else {
            HiLog.error(TAG, "the light effectId is not exist", new Object[0]);
            return false;
        }
    }
}

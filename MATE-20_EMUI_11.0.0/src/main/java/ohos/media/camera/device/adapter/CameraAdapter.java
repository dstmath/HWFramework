...
public class CameraAdapter implements ICamera, IBinder.DeathRecipient {
    ...
    public int createOutput(StreamConfiguration streamConfiguration) throws AccessException {
        int i = -1;
        try {
            OutputConfiguration outputConfiguration = new OutputConfiguration(Converter.convert2ASurface(streamConfiguration.getSurface()));
            i = this.cameraDeviceUser.createStream(outputConfiguration);
            this.streamId2OutputConfigMap.put(i, outputConfiguration);
            return i;
    ...
    @Override // ohos.media.camera.zidl.ICamera
    public void endConfig() throws AccessException {
        try {
            this.cameraDeviceUser.endConfigure(0, new CameraMetadataNative());
        } catch (RemoteException | ServiceSpecificException e) {
            ExceptionTransfer.trans2AccessException(e);
        }
    }

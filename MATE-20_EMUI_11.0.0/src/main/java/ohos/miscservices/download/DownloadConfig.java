...
public class DownloadConfig implements Sequenceable {
    ...
    private DownloadConfig(Builder builder) {
        ...
        this.httpHeaders = builder.httpHeaders.getAll();
    ...
    public boolean marshalling(Parcel parcel) {
        ...
        String[] strArr = new String[this.httpHeaders.size()];
        String[] strArr2 = new String[this.httpHeaders.size()];
        int i = 0;
        for (Map.Entry<String, Object> entry : this.httpHeaders.entrySet()) {
            strArr[i] = entry.getKey();
            strArr2[i] = entry.getValue() == null ? "" : entry.getValue().toString();
            i++;
        }
        if (!parcel.writeStringArray(strArr)) {
            HiLog.error(TAG, "write http head key wrong", new Object[0]);
            return false;
        } else if (!parcel.writeStringArray(strArr2)) {
            HiLog.error(TAG, "write http head value wrong", new Object[0]);
            return false;
        } else if (!parcel.writeString(this.title)) {

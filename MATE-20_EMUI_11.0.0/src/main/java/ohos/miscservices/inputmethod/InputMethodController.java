...
public class InputMethodController {
    ...
    public boolean startInput(Context context, int i, boolean z) {
        return startInput(i, z);
    }
    ...
    public void notifyCaretCoordinateChanged(float f, float f2, float[] fArr) {
        try {
            HiLog.info(TAG, "notifyCaretCoordinateChanged begin.", new Object[0]);
            this.mInputMethodAgent.notifyCaretCoordinateChanged(f, f2, fArr);
        } catch (RemoteException e) {
            HiLog.error(TAG, "notifyCaretCoordinateChanged failed,Exception = %s", e.toString());
        }
    }

    public void setCaretCoordinateNotifyMode(int i) {
        try {
            HiLog.info(TAG, "setCaretCoordinateNotifyMode: mode = %d", Integer.valueOf(i));
            this.mSystemAbility.setCaretCoordinateNotifyMode(i);
        } catch (RemoteException e) {
            HiLog.error(TAG, "setCaretCoordinateNotifyMode failed,Exception = %s", e.toString());
        }
    }

    public boolean isCaretCoordinateSubscribed() {
        try {
            HiLog.info(TAG, "isCaretCoordinateSubscribed begin.", new Object[0]);
            return this.mSystemAbility.isCaretCoordinateSubscribed();
        } catch (RemoteException e) {
            HiLog.error(TAG, "isCaretCoordinateSubscribed failed,Exception = %s", e.toString());
            return false;
        }
    }

    class InputDataChannelLocalImpl extends RemoteInputDataChannelSkeleton {
    ...
    public void setSelection(int i, int i2) {
        try {
            HiLog.debug(TAG, "InputMethodController: setSelection: start character index = %d, end character index = %d", Integer.valueOf(i), Integer.valueOf(i2));
            ...
            HiLog.error(TAG, "setSelection failed, Exception = %s", e.toString());
        }
    }
    ...
    public int getInputMethodDisplayHeight() {
        try {
            HiLog.debug(TAG, "getInputMethodDisplayHeight start", new Object[0]);
            return this.mSystemAbility.getKeyboardWindowHeight();
        } catch (RemoteException e) {
            HiLog.error(TAG, "getInputMethodDisplayHeight failed, Exception = %s", e.toString());
            return 0;
        }
    }

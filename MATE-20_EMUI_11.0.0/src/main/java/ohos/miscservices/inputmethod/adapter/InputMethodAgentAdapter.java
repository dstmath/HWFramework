...
public class InputMethodAgentAdapter {
    ...
    private float mCaretHorizontal = ConstantValue.MIN_ZOOM_VALUE;
    ...
    public float getmCaretHorizontal() {
        return this.mCaretHorizontal;
    }

    public void notifyCaretCoordinateChanged(float f, float f2, float[] fArr) throws RemoteException {
        Optional<InputMethodManager> inputMethodManager = AdaptUtil.getInputMethodManager();
        if (!inputMethodManager.isPresent()) {
            HiLog.error(TAG, "notifyCaretCoordinateChanged failed: current InputMethodManager instance is null.", new Object[0]);
            return;
        }
        Optional<View> surfaceView = ReflectUtil.getSurfaceView();
        if (!surfaceView.isPresent()) {
            HiLog.error(TAG, "notifyCaretCoordinateChanged failed because current surfaceView is null.", new Object[0]);
            return;
        }
        Optional<CursorAnchorInfo> convertToCursorAnchorInfo = CursorContextInfoAdapter.convertToCursorAnchorInfo(f, f2, fArr);
        ...
        convertToCursorAnchorInfo.ifPresent(new Consumer() {
            /* class ohos.miscservices.inputmethod.adapter.$$Lambda$InputMethodAgentAdapter$RCvV83T7wud3Hf6EduQQ40CCy4Y */

            @Override // java.util.function.Consumer
            public final void accept(Object obj) {
                InputMethodAgentAdapter.this.lambda$notifyCaretCoordinateChanged$0$InputMethodAgentAdapter((CursorAnchorInfo) obj);
            }
        });
    }

    public /* synthetic */ void lambda$notifyCaretCoordinateChanged$0$InputMethodAgentAdapter(CursorAnchorInfo cursorAnchorInfo) {
        this.mCaretHorizontal = cursorAnchorInfo.getInsertionMarkerHorizontal();
    }
}

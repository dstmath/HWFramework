...
public class WallpaperControllerProxyImpl {
    ...
    public Element getDefaultDrawable(int i, int i2, boolean z, float f, float f2, int i3) throws RemoteException {
        if (this.mWallpaperManager != null || tryInit()) {
            try {
                Drawable builtInDrawable = this.mWallpaperManager.getBuiltInDrawable(i, i2, z, f, f2, i3);
                if (builtInDrawable == null) {
                    HiLog.error(TAG, "getDefaultDrawable:drawable is null!", new Object[0]);
                    throw new RemoteException();
                    ...
                    HiLog.error(TAG, "getDefaultDrawable:pixelMap is null!", new Object[0]);
                    throw new RemoteException();
                }
            } catch (IllegalArgumentException unused) {
                HiLog.error(TAG, "getDefaultDrawable:illegal argument!", new Object[0]);
                throw new RemoteException();
            } catch (RuntimeException unused2) {
                HiLog.error(TAG, "getDefaultDrawable:Service is not running!", new Object[0]);
                throw new RemoteException();
            }
        } else {
            HiLog.error(TAG, "getDefaultDrawable:Get wallpaper controller instance failed!", new Object[0]);
            throw new RemoteException();
        }
    }
    ...
    public Element getDrawable() {
        try {
            if (!isOperationAllowed()) {
                HiLog.error(TAG, "getDrawable:don't support getting wallpaper drawable!", new Object[0]);
                return null;
            }
            PixelMap pixelmap = getPixelmap(1);
            if (pixelmap != null) {
                return new PixelMapElement(pixelmap);
            }
            HiLog.error(TAG, "getDrawable:pixelmap is null!", new Object[0]);
            return null;
        } catch (RemoteException unused) {
            HiLog.error(TAG, "getDrawable:call isWallpaperSupported failed!", new Object[0]);
            return null;
        }
    }
}

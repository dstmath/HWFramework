...
class NetManagerProxy implements INetManager {
    ...
    private int getLegacyType(NetCapabilities netCapabilities) {
        ...
        if (netCapabilities.hasCapability(5)) {
            return 12;
        }
        if (netCapabilities.hasCapability(4)) {
            return 11;
        }
        if (netCapabilities.hasCapability(3)) {
            return 10;
        }
        if (netCapabilities.hasCapability(2)) {
            return 4;
        }
        if (netCapabilities.hasCapability(1)) {
            return 3;
        }
        if (netCapabilities.hasCapability(12)) {
            return 5;
        }
        if (netCapabilities.hasCapability(25)) {
            return 38;
        }
        if (netCapabilities.hasCapability(26)) {
            return 39;
        }
        if (netCapabilities.hasCapability(27)) {
            return 40;
        }
        if (netCapabilities.hasCapability(28)) {
            return 41;
        }
        if (netCapabilities.hasCapability(29)) {
            return 42;
        }
        if (netCapabilities.hasCapability(30)) {
            return TYPE_MOBILE_BIP5;
        }
        if (netCapabilities.hasCapability(31)) {
            return TYPE_MOBILE_BIP6;
        }
        if (netCapabilities.hasCapability(9)) {
            return TYPE_MOBILE_XCAP;
        }
        if (netCapabilities.hasCapability(32)) {

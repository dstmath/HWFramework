...
public class RemoteObjectCallbackList<E extends IRemoteBroker> {
    ...
    /* JADX WARN: Type inference failed for: r1v6, types: [E, E extends ohos.rpc.IRemoteBroker] */
    public E getBroadcastInterface(int i) {
        if (i >= 0 && i < this.mBroadcastCount) {
            return ((RemoteObjectCallback) this.mBroadcastBackup[i]).mCallback;
        }
    ...
    public Object getBroadcastCookie(int i) {
        if (i >= 0 && i < this.mBroadcastCount) {
            return ((RemoteObjectCallback) this.mBroadcastBackup[i]).mCookie;
        }

...
public class SysAbilityRegistry implements ISysAbilityRegistry {
    ...
    public String[] listSysAbilities(int i) throws RemoteException {
        MessageParcel obtain;
        MessageParcel obtain2;
        if (this.iRemoteObject != null) {
            ArrayList arrayList = new ArrayList();
            int i2 = 0;
            while (true) {
                obtain = MessageParcel.obtain();
                obtain2 = MessageParcel.obtain();
                try {
                    if (!obtain.writeInterfaceToken(SAMGR_INTERFACE_TOKEN)) {
                        break;
                    } else if (!obtain.writeInt(i2)) {
                        HiLog.error(TAG, "listSysAbilities: write index error!", new Object[0]);
                        break;
                    } else if (!obtain.writeInt(i)) {
                        HiLog.error(TAG, "listSysAbilities: write dumpPriority error!", new Object[0]);
                        break;
                    } else {
                        i2++;
                        if (!this.iRemoteObject.sendRequest(5, obtain, obtain2, new MessageOption())) {
                            break;

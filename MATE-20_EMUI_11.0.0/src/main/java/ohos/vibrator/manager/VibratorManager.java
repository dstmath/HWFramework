...
public class VibratorManager {
    ...
    private static native int nativeEnableVibrator(int i, String str);
    ...
    public static VibratorManager getInstance() {
        if (instance == null) {
            HiLog.info(TAG, "start instance class load", new Object[0]);
            synchronized (VibratorManager.class) {
                if (instance == null) {
                    instance = new VibratorManager();
                }
            }
            HiLog.info(TAG, "end instance class load", new Object[0]);
        }
        return instance;
    }
    ...
    public List<Integer> getVibratorIdList() {
        ...
        HiLog.error(TAG, "the vibratorId list is empty", new Object[0]);
        return Collections.emptyList();
    }
    ...
    public boolean isEffectSupport(int i, String str) {
        ...
        HiLog.error(TAG, "the vibrator effect is not support", new Object[0]);
        return false;
    }
    ...
    public boolean vibrate(int i, String str) {
        ...
        if (nativeEnableVibrator(i, str) == 0) {
            return true;
        }
        HiLog.error(TAG, "the vibrator enable with effect ID failed", new Object[0]);
        return false;
    }
    ...
    public boolean vibrate(int i, int i2) {
        ...
        HiLog.error(TAG, "the vibrator enable with duration failed", new Object[0]);
        return false;
    }
    ...
    public boolean vibrate(int i, VibratorEffectUtil vibratorEffectUtil) {
        ...
        if (vibratorEffectUtil == null) {
            HiLog.error(TAG, "the vibrator define effect is invalid", new Object[0]);
            return false;
        }
        ...
        HiLog.error(TAG, "the vibrator vibrate enable failed", new Object[0]);
        return false;
    }

    public boolean stop(int i, String str) {
        HiLog.info(TAG, "vibrator stop vibrate start", new Object[0]);
        if (!isNativeInitSuccess() || !isVibratorIdValid(i)) {
            return false;
        }
        if (!VibratorConstant.VIBRATOR_STOP_MODE_LIST.contains(str)) {
            HiLog.error(TAG, "the vibrator stop mode is invalid", new Object[0]);
            return false;
        } else if (nativeDisableVibrator(i, str) != 0) {
            HiLog.error(TAG, "the vibrator stop vibrate failed", new Object[0]);
            return false;
        } else {
            HiLog.info(TAG, "vibrator stop vibrate end", new Object[0]);
            return true;
        }
    }

    public boolean setVibratorParameter(int i, String str) {
        HiLog.info(TAG, "vibrator setVibratorParameter start", new Object[0]);
        if (!isNativeInitSuccess() || !isVibratorIdValid(i)) {
            return false;
        }
        if (!VibratorConstant.VIBRATOR_SET_COMMAND_PARAMETER_LIST.contains(str)) {
            HiLog.error(TAG, "the vibrator's set command is not exist", new Object[0]);
            return false;
        } else if (nativeSetVibratorParameter(i, str) != 0) {
            HiLog.error(TAG, "the vibrator set command is failed", new Object[0]);
            return false;
        } else {
            HiLog.info(TAG, "vibrator setVibratorParameter end", new Object[0]);
            return true;
        }
    }

    public String getVibratorParameter(int i, String str) {
        HiLog.info(TAG, "vibrator getVibratorParameter start", new Object[0]);
        if (!isNativeInitSuccess() || !isVibratorIdValid(i)) {
            return "";
        }
        if (!VibratorConstant.VIBRATOR_GET_COMMAND_PARAMETER_LIST.contains(str)) {
            HiLog.error(TAG, "the vibrator's get command is not exist", new Object[0]);
            return "";
        }
        HiLog.info(TAG, "vibrator getVibratorParameter end", new Object[0]);
    ...
    private static void getAllVibrator() {
        if (!hasGetVibratorIdList) {
            ...
            HiLog.debug(TAG, "get vibrator id list is successful", new Object[0]);
        }
        if (vibratorIdVector.isEmpty()) {
            HiLog.error(TAG, "get vibrator id list is failed", new Object[0]);
        }
    }
    ...
    private boolean isVibratorIdValid(int i) {
        ...
        HiLog.error(TAG, "the vibrator Id is not exist", new Object[0]);
        return false;
    }
    ...
    private boolean isEffectIdValid(String str) {
        ...
        HiLog.error(TAG, "the vibrator effect Id is invalid", new Object[0]);
        return false;
    }
}

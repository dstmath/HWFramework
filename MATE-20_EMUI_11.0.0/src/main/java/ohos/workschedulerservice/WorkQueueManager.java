...
public class WorkQueueManager implements IDeviceStateMonitor {
    ...
    public void onUserStateChanged(int i) {
        this.workMgrEventHandler.sendEvent(InnerEvent.get(9, (long) i));
    }
    ...
    public boolean tryStartSignWork(WorkInfo workInfo, int i, int i2) {
        ...
        synchronized (this.lock) {
            ...
            if (workStatusById != null) {
                stopWorkByWork(workStatusById, false);
            }
        ...
        if (WorkConnectionManager.getInstance().hasConnections()) {
            ...
            while (it.hasNext()) {
                ...
                if (!(next.getCurrentState() == 1 || next.getCurrentState() == 3)) {
                    ...
                    if (isForbidden(bundleName) || isHibernate(bundleName)) {
                        HiLog.debug(LOG_LABEL, "%{public}s is forbidden or hibernate", bundleName);
                        arrayList.add(next);
                    } else {
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:25:0x0058, code lost:
        if (r2.equals("currentUserId") != false) goto L_0x005c;
     */
    ...
    private void dumpUserId(PrintWriter printWriter) {
        printWriter.println("dumpUserId:");
        int initUserId = WorkschedulerAdapter.getInitUserId();
        printWriter.println("init userId : " + initUserId);
        printWriter.println("current userId : " + this.currentUserId);
    }
}

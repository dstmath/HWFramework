...
public class GeomagneticField {
    ...
    private static class LegendreTable {
        ...
        static {
            Class<GeomagneticField> cls = GeomagneticField.class;
        }
    ...
    public GeomagneticField(float gdLatitudeDeg, float gdLongitudeDeg, float altitudeMeters, long timeMillis) {
        ...
        while (n < MAX_N) {
            ...
            while (m2 <= n) {
                ...
                gcY3 += relativeRadiusPower[n + 2] * ((float) m2) * ((sinMLon[m2] * g) - (cosMLon[m2] * h)) * legendre.mP[n][m2] * SCHMIDT_QUASI_NORM_FACTORS[n][m2] * inverseCosLatitude;
                gcZ -= (((((float) (n + 1)) * relativeRadiusPower[n + 2]) * ((cosMLon[m2] * g) + (sinMLon[m2] * h))) * legendre.mP[n][m2]) * SCHMIDT_QUASI_NORM_FACTORS[n][m2];
                m2++;
                MAX_N = MAX_N;
                float f = gdLongitudeDeg;
            }
            n++;
            gcY = gcY3;
            gcY2 = gcZ;
            float f2 = gdLongitudeDeg;
        }
        float f3 = gdLatitudeDeg2;
        double latDiffRad = Math.toRadians((double) gdLatitudeDeg2) - ((double) this.mGcLatitudeRad);
        LegendreTable legendreTable = legendre;
        float[] fArr = relativeRadiusPower;
        this.mX = (float) ((((double) gcX) * Math.cos(latDiffRad)) + (((double) gcY2) * Math.sin(latDiffRad)));
        this.mY = gcY;
        this.mZ = (float) ((((double) (-gcX)) * Math.sin(latDiffRad)) + (((double) gcY2) * Math.cos(latDiffRad)));
    }

...
public final class GeofenceHardwareImpl {
    ...
    class Reaper implements IBinder.DeathRecipient {
        ...
        public int hashCode() {
            int i = 0;
            int result = 31 * ((31 * 17) + (this.mCallback != null ? this.mCallback.asBinder().hashCode() : 0));
            if (this.mMonitorCallback != null) {
                i = this.mMonitorCallback.asBinder().hashCode();
            }
            return (31 * (result + i)) + this.mMonitoringType;
        }
        ...
        try {
            if (this.mVersion >= 2) {
                if ((this.mCapabilities & 1) == 0) {
                    ...
                    hasGnnsCapabilities = this.mFusedService == null && this.mFusedService.isSupported() && hasGnnsCapabilities2;
    ...
    public void reportGeofenceTransition(int geofenceId, Location location, int transition, long transitionTimestamp, int monitoringType, int sourcesUsed) {
        ...
        GeofenceTransition geofenceTransition = new GeofenceTransition(geofenceId, i3, j, location2, i2, i);
        acquireWakeLock();

...
public class CalendarAstronomer {
    ...
    private long lstToUT(double lst) {
        ...
        return ((long) (3600000.0d * lt)) + ((86400000 * ((this.time + this.fGmtOffset) / 86400000)) - this.fGmtOffset);
    }
    ...
    public Horizon eclipticToHorizon(double eclipLong) {
        ...
        Equatorial equatorial2 = equatorial;
    ...
    public Equatorial getMoonPosition() {
        if (this.moonPosition == null) {
            ...
            double center = 0.10975677534091541d * Math.sin(meanAnomalyMoon2);
            double d = a3;
            this.moonLongitude = (((meanLongitude + evection) + center) - annual) + (0.0037350045992678655d * Math.sin(2.0d * meanAnomalyMoon2));
            double d2 = sunLong;
            this.moonLongitude += Math.sin((this.moonLongitude - sunLong) * 2.0d) * 0.011489502465878671d;
            double d3 = center;
            double nodeLongitude = norm2PI(moonN0 - (9.242199067718253E-4d * day2)) - (0.0027925268031909274d * Math.sin(this.meanAnomalySun));
            double y = Math.sin(this.moonLongitude - nodeLongitude);
            double d4 = meanLongitude;
            double d5 = meanAnomalyMoon2;
            this.moonEclipLong = Math.atan2(y * Math.cos(moonI), Math.cos(this.moonLongitude - nodeLongitude)) + nodeLongitude;
            double d6 = nodeLongitude;
    ...
    private long timeOfAngle(AngleFunc func, double desired, double periodDays, long epsilon, boolean next) {
        ...
        do {
            ...
            double factor = Math.abs(deltaT / normPI(angle - lastAngle2));
            double d2 = deltaT;
            ...
            if (Math.abs(deltaT) > Math.abs(lastDeltaT)) {
                ...
                if (next) {
                    double d4 = lastDeltaT;
                    j = delta;
                } else {
                    double d5 = lastDeltaT;
                    j = -delta;
                }
                setTime(j + startTime);
                return timeOfAngle(func, desired, periodDays, epsilon, next);
            }
            double d6 = lastDeltaT;
            lastDeltaT = deltaT;
            lastAngle2 = angle;
            setTime(this.time + ((long) deltaT));
    ...
    private long riseOrSet(CoordFunc func, boolean rise, double diameter, double refraction, long epsilon) {
        ...
        Equatorial equatorial = pos;
        double d = tanL;
        double y = Math.asin(Math.sin((diameter / 2.0d) + refraction) / Math.sin(Math.acos(Math.sin(this.fLatitude) / cosD)));
        double d2 = y;

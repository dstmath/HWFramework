...
public final class UBiDiProps {
    ...
    public final int getJoiningGroup(int c) {
        ...
        int limit = this.indexes[5];
        if (start <= c && c < limit) {
            return this.jgArray[c - start] & UCharacterEnums.ECharacterDirection.DIRECTIONALITY_UNDEFINED;
        }
        int start2 = this.indexes[6];
        int limit2 = this.indexes[7];
        if (start2 > c || c >= limit2) {
            return 0;
        }
        return this.jgArray2[c - start2] & UCharacterEnums.ECharacterDirection.DIRECTIONALITY_UNDEFINED;
    }

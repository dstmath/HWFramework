...
public final class UCharacterUtility {
    ...
    static int getNullTermByteSubString(StringBuffer str, byte[] array, int index) {
        ...
        while (b != 0) {
            b = array[index];
            if (b != 0) {
                str.append((char) (b & UCharacterEnums.ECharacterDirection.DIRECTIONALITY_UNDEFINED));
            }
            index++;
        }
        return index;
    ...
    static int compareNullTermByteSubString(String str, byte[] array, int strindex, int aindex) {
        ...
        while (b != 0) {
            ...
            if (b == 0) {
                break;
            } else if (strindex == length || str.charAt(strindex) != ((char) (b & UCharacterEnums.ECharacterDirection.DIRECTIONALITY_UNDEFINED))) {
                return -1;
            } else {
                strindex++;
            }

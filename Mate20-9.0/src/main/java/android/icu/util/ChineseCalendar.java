...
public class ChineseCalendar extends Calendar {
    ...
    public void roll(int field, int amount) {
        if (field != 2) {
            ...
            if (this.isLeapYear) {
                if (get(22) == 1) {
                    m++;
                } else if (isLeapMonthBetween(newMoonNear(moon - ((int) (29.530588853d * (((double) m) - 0.5d))), true), moon) != 0) {
                    m++;
                }
            }
            int n = this.isLeapYear != 0 ? 13 : 12;
            int newM = (m + amount) % n;
            if (newM < 0) {
                newM += n;
            }
    ...
    private void computeChineseFields(int days, int gyear, int gmonth, boolean setAllFields) {
        ...
        if (setAllFields) {
            ...
            int solsticeBefore2 = floorDivide(cycle_year - 1, 60, yearOfCycle);
            int i6 = solsticeBefore2;
            internalSet(0, solsticeBefore2 + 1);
            internalSet(1, yearOfCycle[0] + 1);
            internalSet(5, (i - thisMoon) + 1);
        ...
        int i7 = gmonth;
        int i8 = solsticeAfter;
        int i9 = solsticeBefore;
    }

    ...
    private String _getDisplayName(int style, boolean daylight, ULocale locale) {
        if (locale != null) {
            ...
            if (style == 7 || style == 3 || style == 2) {
                ...
                int offset2 = (!daylight || !useDaylightTime()) ? getRawOffset() : getRawOffset() + getDSTSavings();
                switch (style) {
                    case 4:
                        return tzfmt2.formatOffsetISO8601Basic(offset2, false, false, false);
                    case 5:
                        return tzfmt2.formatOffsetLocalizedGMT(offset2);
                    default:
                        return null;
                }
            } else {
                ...
                String result2 = tznames.getDisplayName(ZoneMeta.getCanonicalCLDRID(this), nameType, date2);
                if (result2 != null) {
                    return result2;
                }
                TimeZoneFormat tzfmt3 = TimeZoneFormat.getInstance(locale);
                int offset3 = (!daylight || !useDaylightTime()) ? getRawOffset() : getRawOffset() + getDSTSavings();
                return style == 1 ? tzfmt3.formatOffsetLocalizedGMT(offset3) : tzfmt3.formatOffsetShortLocalizedGMT(offset3);
            }
        } else {

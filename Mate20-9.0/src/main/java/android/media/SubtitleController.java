    ...
    public SubtitleTrack getDefaultTrack() {
        ...
        synchronized (this.mTracks) {
            ...
            while (it.hasNext()) {
                ...
                boolean forced = format.getInteger(MediaFormat.KEY_IS_FORCED_SUBTITLE, 0) != 0 ? languageMatches2 : false;
                boolean autoselect = format.getInteger(MediaFormat.KEY_IS_AUTOSELECT, languageMatches2) != 0 ? languageMatches2 : false;
                boolean is_default = format.getInteger(MediaFormat.KEY_IS_DEFAULT, 0) != 0 ? languageMatches2 : false;
                if (locale2 != null && !locale2.getLanguage().equals("") && !locale2.getISO3Language().equals(language)) {
                    if (!locale2.getLanguage().equals(language)) {
                        ...
                        score = i + ((selectedLocale == null || !is_default) ? 0 : 4) + (!autoselect ? 0 : 2) + (!languageMatches ? 1 : 0);

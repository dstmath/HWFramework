...
public final class DynamicsProcessing extends AudioEffect {
    ...
    public static final class Channel {
        ...
        public Channel(float inputGain, boolean preEqInUse, int preEqBandCount, boolean mbcInUse, int mbcBandCount, boolean postEqInUse, int postEqBandCount, boolean limiterInUse) {
            ...
            Limiter limiter = new Limiter(limiterInUse, true, 0, 1.0f, 60.0f, 10.0f, DynamicsProcessing.LIMITER_DEFAULT_THRESHOLD, 0.0f);
        ...
        public static final class Builder {
            ...
            public Builder(int variant, int channelCount, boolean preEqInUse, int preEqBandCount, boolean mbcInUse, int mbcBandCount, boolean postEqInUse, int postEqBandCount, boolean limiterInUse) {
                ...
                while (ch < this.mChannelCount) {
                    ...
                    int i3 = i2;
                    boolean z4 = z3;
                    Channel channel = new Channel(0.0f, z, i, z2, i3, z4, this.mPostEqBandCount, this.mLimiterInUse);
                    channelArr[ch] = channel;
                    ch++;
                    int i4 = variant;
                }
            }
            ...
            public Config build() {
                Config config = new Config(this.mVariant, this.mPreferredFrameDuration, this.mChannelCount, this.mPreEqInUse, this.mPreEqBandCount, this.mMbcInUse, this.mMbcBandCount, this.mPostEqInUse, this.mPostEqBandCount, this.mLimiterInUse, this.mChannel);
                return config;
            }
        }
        ...
        public Mbc(boolean inUse, boolean enabled, int bandCount) {
            ...
            if (isInUse()) {
                ...
                for (int b = 0; b < i; b++) {
                    ...
                    MbcBand mbcBand = new MbcBand(true, freq, DynamicsProcessing.MBC_DEFAULT_ATTACK_TIME, DynamicsProcessing.MBC_DEFAULT_RELEASE_TIME, 1.0f, DynamicsProcessing.MBC_DEFAULT_THRESHOLD, 0.0f, DynamicsProcessing.MBC_DEFAULT_NOISE_GATE_THRESHOLD, 1.0f, 0.0f, 0.0f);
                    this.mBands[b] = mbcBand;
                }
                return;
            }
            this.mBands = null;
    ...
    private MbcBand queryEngineMbcBandByChannelIndex(int channelIndex, int bandIndex) {
        ...
        Number[] values = {0, Float.valueOf(0.0f), Float.valueOf(0.0f), Float.valueOf(0.0f), Float.valueOf(0.0f), Float.valueOf(0.0f), Float.valueOf(0.0f), Float.valueOf(0.0f), Float.valueOf(0.0f), Float.valueOf(0.0f), Float.valueOf(0.0f)};
        byte[] paramBytes = numberArrayToByteArray(params);
        byte[] valueBytes = numberArrayToByteArray(values);
        getParameter(paramBytes, valueBytes);
        byteArrayToNumberArray(valueBytes, values);
        MbcBand mbcBand = new MbcBand(values[0].intValue() > 0, values[1].floatValue(), values[2].floatValue(), values[3].floatValue(), values[4].floatValue(), values[5].floatValue(), values[6].floatValue(), values[7].floatValue(), values[8].floatValue(), values[9].floatValue(), values[10].floatValue());
        return mbcBand;
    }
    ...
    private Limiter queryEngineLimiterByChannelIndex(int channelIndex) {
        ...
        Number[] values = {0, 0, 0, Float.valueOf(0.0f), Float.valueOf(0.0f), Float.valueOf(0.0f), Float.valueOf(0.0f), Float.valueOf(0.0f)};
        byte[] paramBytes = numberArrayToByteArray(params);
        byte[] valueBytes = numberArrayToByteArray(values);
        getParameter(paramBytes, valueBytes);
        byteArrayToNumberArray(valueBytes, values);
        Limiter limiter = new Limiter(values[0].intValue() > 0, values[1].intValue() > 0, values[2].intValue(), values[3].floatValue(), values[4].floatValue(), values[5].floatValue(), values[6].floatValue(), values[7].floatValue());
        return limiter;
    }
    ...
    private void byteArrayToNumberArray(byte[] valuesIn, Number[] valuesOut) {
        ...
        int inIndex = 0;
        int outIndex2 = 0;
        while (inIndex < valuesIn.length && outIndex2 < valuesOut.length) {
            if (valuesOut[outIndex2] instanceof Integer) {
                outIndex = outIndex2 + 1;
                valuesOut[outIndex2] = Integer.valueOf(byteArrayToInt(valuesIn, inIndex));
            } else if (valuesOut[outIndex2] instanceof Float) {
                outIndex = outIndex2 + 1;
                valuesOut[outIndex2] = Float.valueOf(byteArrayToFloat(valuesIn, inIndex));
            } else {
                throw new IllegalArgumentException("can't convert " + valuesOut[outIndex2].getClass());
            }
            inIndex += 4;
            outIndex2 = outIndex;
        }
        if (outIndex2 != valuesOut.length) {
            throw new IllegalArgumentException("only converted " + outIndex2 + " values out of " + valuesOut.length + " expected");
        }
    }

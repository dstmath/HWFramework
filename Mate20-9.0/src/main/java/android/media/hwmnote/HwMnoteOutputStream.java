...
class HwMnoteOutputStream {
    ...
    private int calculateOffsetOfIfd(HwMnoteIfdData ifd, int offset) {
        int calculateOff = offset + 2 + (ifd.getTagCount() * 12) + 4;
    ...
    private void createRequiredIfdAndTag() {
        ...
        if (this.mHwMnoteData.getIfdData(1) != null) {
            HwMnoteTag sceneOffsetTag = new HwMnoteTag(HwMnoteInterfaceImpl.getTrueTagKey(HwMnoteInterfaceUtils.HW_MNOTE_TAG_SCENE_IFD), 4, 1, 0, true);
            ifd0.setTag(sceneOffsetTag);
        }
        if (this.mHwMnoteData.getIfdData(2) != null) {
            HwMnoteTag faceOffsetTag = new HwMnoteTag(HwMnoteInterfaceImpl.getTrueTagKey(HwMnoteInterfaceUtils.HW_MNOTE_TAG_FACE_IFD), 4, 1, 0, true);
            ifd0.setTag(faceOffsetTag);
        }
    }

...
public class SimpleSessionDescription {
    ...
    public SimpleSessionDescription(long sessionId, String address) {
        ...
        this.mFields.parse(String.format(Locale.US, "o=- %d %d %s", new Object[]{Long.valueOf(sessionId), Long.valueOf(System.currentTimeMillis()), address2}));
            ...
            try {
                if (line.charAt(1) == '=') {
                    if (line.charAt(0) == 'm') {
                        ...
                        if (ports.length >= 2) {
                            i2 = Integer.parseInt(ports[1]);
                        }
                        Media media = newMedia(str, parseInt, i2, parts[2]);

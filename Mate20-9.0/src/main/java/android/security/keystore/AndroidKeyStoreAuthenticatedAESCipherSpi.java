...
abstract class AndroidKeyStoreAuthenticatedAESCipherSpi extends AndroidKeyStoreCipherSpiBase {
    ...
    private static class AdditionalAuthenticationDataStream implements KeyStoreCryptoOperationChunkedStreamer.Stream {
        ...
        public OperationResult update(byte[] input) {
            ...
            OperationResult result2 = new OperationResult(result.resultCode, result.token, result.operationHandle, input.length, result.output, result.outParams);
            return result2;
        }

        public OperationResult finish(byte[] signature, byte[] additionalEntropy) {
            if (additionalEntropy == null || additionalEntropy.length <= 0) {
                OperationResult operationResult = new OperationResult(1, this.mOperationToken, 0, 0, EmptyArray.BYTE, new KeymasterArguments());

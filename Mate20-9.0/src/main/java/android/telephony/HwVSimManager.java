...
public class HwVSimManager {
    ...
    private IHwVSim getIHwVSim() throws RemoteException {
        IHwVSim iHwVSim = IHwVSim.Stub.asInterface(ServiceManager.getService("ihwvsim"));
        if (iHwVSim != null) {
            return iHwVSim;
        }
    ...
    public int enableVSim(int operation, String imsi, int cardtype, int apntype, String acqorder, String tapath, int vsimloc, String challenge) {
        ...
        int result = 3;
        try {
            result = getIHwVSim().enableVSimV2(i, imsi, i2, i3, str, tapath, i4, challenge);
        } catch (RemoteException ex) {
            printExLogIfSupportVSim(ex);
        }
        Rlog.d(TAG, "enableVSim finish, result is " + result);
    ...
    public int enableVSim(int operation, Bundle bundle) {
        Rlog.d(TAG, "enableVSim, operation = " + operation);
        int result = 3;
        try {

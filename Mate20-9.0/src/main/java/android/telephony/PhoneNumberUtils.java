...
public class PhoneNumberUtils {
        ...
        if (str == null || str2 == null) {
            ...
            if (!okToIgnorePrefix) {
                ...
                while (backwardIndexA >= forwardIndexA) {
                    ...
                    backwardIndexA--;
                    boolean z3 = acceptInvalidCCCPrefix;
                }
    ...
    private static void internalCalledPartyBCDFragmentToString(StringBuilder sb, byte[] bytes, int offset, int length, int bcdExtType) {
        ...
        while (i < length + offset) {
            ...
            if (c != 0) {
                ...
                if (b == 15 && i + 1 == length + offset) {
                    break;
                }
                char c2 = bcdToChar(b, bcdExtType);
                if (c2 != 0) {
                    sb.append(c2);
                    i++;
                } else {
    ...
    private static boolean isEmergencyNumberInternal(int subId, String number, String defaultCountryIso, boolean useExactMatch) {
        ...
        String emergencyNumbers2 = HwFrameworkFactory.getHwInnerTelephonyManager().custExtraEmergencyNumbers((long) subId, emergencyNumbers);
        if (!TextUtils.isEmpty(emergencyNumbers2)) {
            for (String emergencyNum : emergencyNumbers2.split(",")) {
                if (useExactMatch || "BR".equalsIgnoreCase(defaultCountryIso)) {
                    ...
                    if (isChinaRegion && number2.equals(emergencyNum)) {
                        return true;
                    }
                    if (!isChinaRegion && number2.startsWith(emergencyNum)) {
                        return true;
                    }
                } else {
                    continue;
            ...
            if (useExactMatch) {
                return info.isEmergencyNumber(number2, defaultCountryIso);
            }
            return info.connectsToEmergencyNumber(number2, defaultCountryIso);
        }
    }
    ...
    public static String cdmaCheckAndProcessPlusCode(String dialStr) {
        if (!TextUtils.isEmpty(dialStr) && isReallyDialable(dialStr.charAt(0)) && isNonSeparator(dialStr)) {
            ...
            if (TextUtils.isEmpty(currIso) == 0 && !TextUtils.isEmpty(defaultIso)) {
    ...
    public static String cdmaCheckAndProcessPlusCodeForSms(String dialStr) {
        if (!TextUtils.isEmpty(dialStr) && isReallyDialable(dialStr.charAt(0)) && isNonSeparator(dialStr)) {
            ...
            if (TextUtils.isEmpty(defaultIso) == 0) {

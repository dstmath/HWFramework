...
public class HwLogException implements LogException {
    ...
    public int msg(String category, int level, String header, String body) {
        return HwLogExceptionInner.println_exception_native(category, level, "message", header + 10 + body);
    }

    public int msg(String category, int level, int mask, String header, String body) {
        return HwLogExceptionInner.println_exception_native(category, level, "message", "mask=" + mask + SmartTrimProcessEvent.ST_EVENT_INTER_STRING_TOKEN + header + 10 + body);
    }

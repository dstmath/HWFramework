...
public final class HwPCUtils {
    ...
    public static final String APP_APPLOCK_COMPONENT_NAME = "com.huawei.systemmanager/.applock.password.AuthLaunchLockedAppActivity";
    ...
    public static final boolean isValidExtDisplayId(Context context) {
        ...
        while (context instanceof ContextWrapper) {
            ContextImpl baseContext = ((ContextWrapper) context).getBaseContext();
            ContextImpl contextImpl = baseContext;
            if (baseContext != null && context != contextImpl) {
                context = contextImpl;
                count++;
                if (count > 12) {
                    break;
        ...
        if (context instanceof ContextImpl) {
            return isValidExtDisplayId(context.peekHwPCDisplayId());
        }
        return false;
    }
    ...
    public static void ignoreInjectEventForFreeMouse(boolean ignore) {
        try {
            IHwPCManager pcManager = getHwPCManager();
            if (pcManager != null) {
                pcManager.ignoreInjectEventForFreeMouse(ignore);
            }
        } catch (RemoteException e) {
            log(TAG, "call ignoreInjectEventForFreeMouse error");
        }
    }
}

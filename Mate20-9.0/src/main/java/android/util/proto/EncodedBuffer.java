...
public final class EncodedBuffer {
    ...
    public int readRawFixed32() {
        return (readRawByte() & BatteryStats.HistoryItem.CMD_NULL) | ((readRawByte() & BatteryStats.HistoryItem.CMD_NULL) << 8) | ((readRawByte() & BatteryStats.HistoryItem.CMD_NULL) << WifiScanner.PnoSettings.PnoNetwork.FLAG_SAME_NETWORK) | ((readRawByte() & BatteryStats.HistoryItem.CMD_NULL) << 24);
    }
    ...
    public int getRawFixed32At(int pos) {
        return (this.mBuffers.get(pos / this.mChunkSize)[pos % this.mChunkSize] & BatteryStats.HistoryItem.CMD_NULL) | ((this.mBuffers.get((pos + 1) / this.mChunkSize)[(pos + 1) % this.mChunkSize] & BatteryStats.HistoryItem.CMD_NULL) << 8) | ((this.mBuffers.get((pos + 2) / this.mChunkSize)[(pos + 2) % this.mChunkSize] & BatteryStats.HistoryItem.CMD_NULL) << WifiScanner.PnoSettings.PnoNetwork.FLAG_SAME_NETWORK) | ((255 & this.mBuffers.get((pos + 3) / this.mChunkSize)[(pos + 3) % this.mChunkSize]) << 24);
    }

...
public final class PointerIcon implements Parcelable {
    ...
    public static PointerIcon getSystemIcon(Context context, int type) {
        if (context == null) {
            ...
            gSystemIcons.append(type, icon2);
            return icon2;
        }
    }
    ...
    private static void validateHotSpot(Bitmap bitmap, float hotSpotX, float hotSpotY) {
        if (hotSpotX < 0.0f || hotSpotX >= ((float) bitmap.getWidth())) {
            throw new IllegalArgumentException("x hotspot lies outside of the bitmap area hotSpotX = " + hotSpotX + " bitmap.getWidth() = " + bitmap.getWidth());
        } else if (hotSpotY < 0.0f || hotSpotY >= ((float) bitmap.getHeight())) {
            throw new IllegalArgumentException("y hotspot lies outside of the bitmap area hotSpotY = " + hotSpotY + " bitmap.getHeight() = " + bitmap.getHeight());
        }
    }

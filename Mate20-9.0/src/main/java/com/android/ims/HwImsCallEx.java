...
public class HwImsCallEx implements IHwImsCallEx {
    ...
    public void updateQcomConferenceParticipantsList(IHwImsCallManagerInner bgCall) {
        logd("updateQcomConferenceParticipantsList :: bgImsCall=" + bgCall);
        if (bgCall != null) {
            ...
            if (bgCall.getImsCall().isMultiparty()) {
                logd("updateQcomConferenceParticipantsList: BG call is conference");
                parentCall = bgCall;
                childCall = this.mImsCallMgrInner;
            } else if (!this.mImsCallMgrInner.getImsCall().isMultiparty()) {
                logd("updateQcomConferenceParticipantsList: Make this call as conference and add child");
                addToQcomConferenceParticipantList(this.mImsCallMgrInner, this.mImsCallMgrInner);
            }
    ...
    private void addToQcomConferenceParticipantList(IHwImsCallManagerInner parentCall, IHwImsCallManagerInner childCall) {
        if (parentCall != null && childCall != null) {
            ...
            synchronized (hwImsCallEx.getQcomLockObj()) {
                ...
                logd("Adding participant: " + participant + " to list:" + conferenceParticipants);
    ...
    public boolean shouldSaveQcomParticipantList() {
        ...
        logd("isQcomPlatform: " + HuaweiTelephonyConfigs.isQcomPlatform());
        if (HuaweiTelephonyConfigs.isQcomPlatform()) {
            String localCEPMccMnc = Settings.System.getString(this.mContext.getContentResolver(), LOCAL_CEP_MCC_MNC);
    ...
    public void removeQcomParticipantsFromList(String[] participants) {
        if (this.mImsCallMgrInner != null) {
            List<ConferenceParticipant> conferenceParticipants = this.mImsCallMgrInner.getConferenceParticipantsForExt();

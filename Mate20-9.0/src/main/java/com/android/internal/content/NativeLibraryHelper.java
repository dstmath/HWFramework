    ...
    public static class Handle implements Closeable {
        ...
        private static Handle create(List<String> codePaths, boolean multiArch2, boolean extractNativeLibs2, boolean debuggable2) throws IOException {
            ...
            for (int i = 0; i < size; i++) {
                apkHandles2[i] = NativeLibraryHelper.nativeOpenApk(codePaths.get(i));
                if (apkHandles2[i] == 0) {
                    for (int j = 0; j < i; j++) {
        ...
        try {
            if (handle.multiArch) {
                ...
                if (Build.SUPPORTED_32_BIT_ABIS.length > 0) {
                    int copyRet = copyNativeBinariesForSupportedAbi(handle, libraryRoot, Build.SUPPORTED_32_BIT_ABIS, true);
                ...
                if (Build.SUPPORTED_64_BIT_ABIS.length > 0) {
                    int copyRet2 = copyNativeBinariesForSupportedAbi(handle, libraryRoot, Build.SUPPORTED_64_BIT_ABIS, true);
                    if (!(copyRet2 >= 0 || copyRet2 == -114 || copyRet2 == -113)) {

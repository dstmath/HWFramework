...
public class GsmAlphabet {
    ...
    public static String gsm8BitUnpackedToString(byte[] data, int offset, int length, String characterset) {
        ...
        } else if (!characterset.equalsIgnoreCase("us-ascii") && Charset.isSupported(characterset)) {
            isMbcs = true;
        ...
        while (i2 < offset + i) {
            ...
            if (c3 == 27) {
                ...
                if (prevWasEscape) {
                    char shiftChar = c3 < shiftTableToChar.length() ? shiftTableToChar.charAt(c3) : ' ';
                    if (shiftChar != ' ') {
                        ret.append(shiftChar);
                    } else if (c3 < languageTableToChar.length()) {
                        ret.append(languageTableToChar.charAt(c3));
                    } else {
                        ret.append(' ');
                    }
                    c = 2;
                } else if (!isMbcs || c3 < 128 || i2 + 1 >= offset + i) {
                    c = 2;
                    if (c3 < languageTableToChar.length()) {
                        ret.append(languageTableToChar.charAt(c3));
                    } else {
                        ret.append(' ');
                    }
        ...
        for (LanguagePairCount lpc2 : lpcList) {
            ...
            while (shiftTable <= maxSingleShiftCode) {
                ...
                if (septets2 != i2) {
                    ...
                    if (!z || unencodableCount <= minUnencodableCount2) {
                        if (z && unencodableCount < minUnencodableCount2) {

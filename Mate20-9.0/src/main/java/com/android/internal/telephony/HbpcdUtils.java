...
public final class HbpcdUtils {
    ...
    public int getMcc(int sid, int tz, int DSTflag, boolean isNitzTimeZone) {
        ...
        Cursor c2 = contentResolver.query(uri, new String[]{"MCC"}, "SID=" + i, null, null);
        if (c2 != null) {
            if (c2.getCount() == 1) {
                c2.moveToFirst();
        ...
        Cursor c3 = contentResolver2.query(uri2, new String[]{"MCC"}, "SID_Conflict=" + i + " and (((" + HbpcdLookup.MccLookup.GMT_OFFSET_LOW + "<=" + i2 + ") and (" + i2 + "<=" + HbpcdLookup.MccLookup.GMT_OFFSET_HIGH + ") and (0=" + i3 + ")) or ((" + HbpcdLookup.MccLookup.GMT_DST_LOW + "<=" + i2 + ") and (" + i2 + "<=" + HbpcdLookup.MccLookup.GMT_DST_HIGH + ") and (1=" + i3 + ")))", null, null);
        if (c3 != null) {
        ...
        Cursor c5 = contentResolver3.query(uri3, new String[]{"MCC"}, "SID_Range_Low<=" + i + " and " + HbpcdLookup.MccSidRange.RANGE_HIGH + ">=" + i, null, null);
        if (c5 != null) {
            if (c5.getCount() > 0) {
                c5.moveToFirst();

...
public class HwDataServiceChrManagerImpl implements HwDataServiceChrManager {
    ...
    /* access modifiers changed from: private */
    public int WIFI_TO_MOBILE_MONITOR_TIMER_INTERVAL = 20000;
    /* access modifiers changed from: private */
    public Phone[] mActivePhone = new Phone[3];
    private ApnChangeObserver mApnObserver;
    /* access modifiers changed from: private */
    public Timer mCdmaPdpSameStatusTimer;
    private GsmCdmaPhone mCdmaPhone;
    private boolean mCheckApnContextState = false;
    /* access modifiers changed from: private */
    public int mChrCdmaPdpRilFailCause;
    private String mDataNotAllowedReason = null;
    /* access modifiers changed from: private */
    public int mDataSubId;
    /* access modifiers changed from: private */
    public boolean[] mDefaultAPNReported = {false, false, false};
    /* access modifiers changed from: private */
    public ApnContext[] mDefaultApnContext = new ApnContext[3];
    /* access modifiers changed from: private */
    public boolean[] mDunAPNReported = {false, false, false};
            ...
            try {
                switch (msg.what) {
                    case 1:
                        ...
                        Timer unused = HwDataServiceChrManagerImpl.this.mSimloadedTimer = null;
                        HwDataServiceChrManagerImpl.this.stopTimer(HwDataServiceChrManagerImpl.this.mCdmaPdpSameStatusTimer);
                        Timer unused2 = HwDataServiceChrManagerImpl.this.mCdmaPdpSameStatusTimer = null;
                        HwDataServiceChrManagerImpl.this.cleanLastStatus();
                        boolean unused3 = HwDataServiceChrManagerImpl.this.mUserDataEnabled = TelephonyManager.getDefault().getDataEnabled();
                        ...
                        if (SubscriptionController.getInstance().getSubState(HwDataServiceChrManagerImpl.this.mDataSubId) == 0) {
                            ...
                            Timer unused4 = HwDataServiceChrManagerImpl.this.mSimloadedTimer = HwDataServiceChrManagerImpl.this.startTimer(HwDataServiceChrManagerImpl.this.mSimloadedTimer, HwDataServiceChrManagerImpl.TIMER_INTERVAL_CHECK_SIMLOADED_ISRECEIVED);
                        ...
                        return;
                    case 2:
                        HwDataServiceChrManagerImpl.this.sendIntentWhenCdmaPdpActFail((AsyncResult) msg.obj);
                        int unused5 = HwDataServiceChrManagerImpl.this.mChrCdmaPdpRilFailCause = -1;
                        return;
                    case 3:
                ...
                Log.e(HwDataServiceChrManagerImpl.LOG_TAG, "Exception: handleMessage fail!");
            }
        }
    };
            ...
            try {
                ...
                if ("android.intent.action.SIM_STATE_CHANGED".equals(action)) {
                    ...
                    if (simState != null && "READY".equals(simState) && subId == SubscriptionController.getInstance().getDefaultDataSubId()) {
                        ...
                        int unused = HwDataServiceChrManagerImpl.this.mDataSubId = subId;
                        if (HwDataServiceChrManagerImpl.this.mSimloadedTimer == null) {
                            Timer unused2 = HwDataServiceChrManagerImpl.this.mSimloadedTimer = HwDataServiceChrManagerImpl.this.startTimer(HwDataServiceChrManagerImpl.this.mSimloadedTimer, HwDataServiceChrManagerImpl.TIMER_INTERVAL_CHECK_SIMLOADED_ISRECEIVED);
                        }
                    }
                    ...
                    if (true == HwDataServiceChrManagerImpl.this.mIsWifiConnected && intent.getIntExtra("wifi_state", 4) == 1) {
                        ...
                        boolean unused3 = HwDataServiceChrManagerImpl.this.mIsWifiConnected = false;
                    }
                    ...
                    if (networkInfo != null) {
                        if (true == HwDataServiceChrManagerImpl.this.mIsWifiConnected && networkInfo.getState() == NetworkInfo.State.DISCONNECTED) {
                            HwDataServiceChrManagerImpl.this.removeMonitorWifiSwitchToMobileMessage();
                            HwDataServiceChrManagerImpl.this.sendMonitorWifiSwitchToMobileMessage(HwDataServiceChrManagerImpl.this.WIFI_TO_MOBILE_MONITOR_TIMER_INTERVAL);
                        } else if (!HwDataServiceChrManagerImpl.this.mIsWifiConnected && networkInfo.getState() == NetworkInfo.State.CONNECTED) {
                            HwDataServiceChrManagerImpl.this.removeMonitorWifiSwitchToMobileMessage();
                        }
                        boolean unused4 = HwDataServiceChrManagerImpl.this.mIsWifiConnected = networkInfo.isConnected();
                    }
                    Log.d(HwDataServiceChrManagerImpl.LOG_TAG, "mIsWifiConnected = " + HwDataServiceChrManagerImpl.this.mIsWifiConnected);
                }
            } catch (RuntimeException e) {
                Log.e(HwDataServiceChrManagerImpl.LOG_TAG, "RuntimeException: onReceive fail!");
            } catch (Exception e2) {
                Log.e(HwDataServiceChrManagerImpl.LOG_TAG, "Exception: onReceive fail!");
            }
        }
    };
    private boolean mIsBringUp = false;
    /* access modifiers changed from: private */
    public boolean mIsDataConnectionAttached = false;
    ...
    /* access modifiers changed from: private */
    public boolean[] mMmsAPNReported = {false, false, false};
    private String mPdpActiveIpType = null;
    private int mSameStatusTimes = 0;
    /* access modifiers changed from: private */
    public Timer mSimloadedTimer;
    /* access modifiers changed from: private */
    public boolean mUserDataEnabled = false;
    ...
    public void init(Context context) {
        if (context != null) {
            try {
                ...
                Log.e(LOG_TAG, "Exception: init fail!");
            }
        }
    }
    ...
    public void setDataNotAllowedReason(Phone phone, boolean attachedState, boolean autoAttachOnCreation, boolean recordsLoaded, boolean internalDataEnabled, boolean userDataEnabled, boolean isPsRestricted) {
        ...
        if (phone != null) {
            try {
                ...
                Log.e(LOG_TAG, "Exception: setDataNotAllowedReason fail!");
                return;
            }
        }
    ...
    public void setAnyDataEnabledFalseReason(boolean internalDataEnabled, boolean userDataEnabled, boolean sPolicyDataEnabled, boolean checkUserDataEnabled) {
        try {
            ...
            this.mGetAnyDataEnabledFalseReason = internalDataEnabled + "," + userDataEnabled + "," + sPolicyDataEnabled + "," + checkUserDataEnabled;
        } catch (Exception e) {
            Log.e(LOG_TAG, "Exception: setAnyDataEnabledFalseReason fail!");
        }
    }
    ...
    public void setReceivedSimloadedMsg(Phone phone, boolean isReceivedSimloadedMsg, ConcurrentHashMap<String, ApnContext> apnContexts, boolean userDataEnabled) {
        ...
        this.mDefaultApnContext[sub] = apnContexts.get("default");
    ...
    public void getModemParamsWhenCdmaPdpActFail(Phone phone, int rilFailCause) {
        ...
        try {
            ...
            Log.e(LOG_TAG, "Exception: getModemParamsWhenCdmaPdpActFail fail!");
        }
    }
    ...
    public void sendIntentWhenDorecovery(Phone phone, int recoveryAction) {
        ...
        try {
            ...
            Log.e(LOG_TAG, "Exception: sendIntentWhenDorecovery fail!");
        }
    }
    ...
    public void sendIntentDSUseStatistics(Phone phone, int duration) {
        try {
            ...
            Log.e(LOG_TAG, "Exception: sendIntentDSUseStatistics fail!");
        }
    }
    ...
    public void sendIntentApnContextDisabledWhenWifiDisconnected(Phone phone, boolean isWifiConnected, boolean userDataEnabled, ApnContext apnContext) {
        ...
        if (this.mCheckApnContextState) {
            try {
                ...
                Log.e(LOG_TAG, "Exception: sendIntentApnContextDisabledWhenWifiDisconnected fail!");
            }
        }
    }
    ...
    private boolean isLabCard() {
        try {
            ...
            Log.e(LOG_TAG, "Exception: isLabCard fail!");
        }
        return false;
    }
    ...
    private boolean isCardReady(Phone phone) {
        ...
        try {
            ...
            Log.e(LOG_TAG, "Exception: isCardReady fail!");
        }
    }
    ...
    private boolean ignoreReport(Phone phone) {
        try {
            ...
            Log.e(LOG_TAG, "Exception: ignoreReport fail!");
        }
    }

    /* access modifiers changed from: private */
    public void cleanLastStatus() {
    ...
    private boolean isNeedReportChrForSameFailStatus(int[] params) {
        if (params != null) {
            try {
                ...
                Log.e(LOG_TAG, "Exception: isNeedReportChrForSameFailStatus!");
            }
        }
        return true;
    }
    ...
    private boolean isUserModifyApnSetting() {
        String[] selections = {"visible<>1", "visible is null"};
    ...
    /* access modifiers changed from: private */
    public void sendIntentWhenCdmaPdpActFail(AsyncResult cdmaChrAR) {
        Intent intent;
        if (cdmaChrAR == null) {
        ...
        try {
            int[] params = (int[]) cdmaChrAR.result;
            ...
            Log.e(LOG_TAG, "Exception: sendIntentWhenCdmaPdpActFail fail!");
        }
    }

    /* access modifiers changed from: private */
    public void sendIntentWhenSimloadedMsgIsNotReceived(int subId) {
        if (mContext != null) {
    ...
    /* access modifiers changed from: private */
    public void sendIntentWhenPdpActFailBlockAtFw(int subId) {
        if (mContext != null) {
    ...
    /* access modifiers changed from: private */
    public Timer startTimer(Timer timer, int timerInterval) {
        ...
        try {
            ...
            Log.e(LOG_TAG, "Exception: startTimer fail!");
            return null;
        }
    }
    ...
    public void stopTimer(Timer timer) {
        if (timer != null) {
            try {
                ...
                Log.e(LOG_TAG, "Exception: stopTimer fail!");
            }
        }
    }

    /* access modifiers changed from: private */
    public int getSimCardState(int subId) {

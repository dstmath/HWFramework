...
public class HwIccIdUtil {
    ...
    private static File getImsiIccidFile() {
        try {
            File cfg = HwCfgFilePolicy.getCfgFile(String.format("/xml/%s", new Object[]{PREFIX_IMSI_ICCID_CONFIG_FILE}), 0);
            if (cfg != null) {
                return cfg;
            }
            return null;
        } catch (NoClassDefFoundError e) {
            Rlog.e(LOG_TAG, "NoClassDefFoundError : HwCfgFilePolicy ");
            return null;
        } catch (NoSuchMethodError e2) {
            Rlog.e(LOG_TAG, "[ERROR:NoSuchMethodError] HwCfgFilePolicy.getImsiIccidFile.");
            return null;
        } catch (Exception e3) {
            Rlog.e(LOG_TAG, "Exception get imsi_iccid_config file.");
            return null;
        }
    }
    ...
    private static void loadImsiIccidTable() {
        ...
        if (imsiIccidFile == null) {
            Rlog.e(LOG_TAG, "load imsi iccid prefix file failed ! Load local data!");
            loadImsiIccidLocal();
            return;
        }
        Rlog.d(LOG_TAG, "load imsi iccid from file.");
        FileInputStream fin = null;
        try {
            FileInputStream fin2 = new FileInputStream(imsiIccidFile);
            XmlPullParser parser = Xml.newPullParser();
            parser.setInput(fin2, "UTF-8");
            XmlUtils.beginDocument(parser, TABLE_IMSI_ICCID_ROOT);
            while (true) {
                ...
                if (TABLE_IMSI_ICCID_ENTRY.equalsIgnoreCase(parser.getName())) {
                    ...
                    if (!TextUtils.isEmpty(operator)) {
                        ...
                        if (operator.equalsIgnoreCase(TABLE_OPERATOR_NAME_CMCC)) {
                        ...
                        } else if (operator.equalsIgnoreCase(TABLE_OPERATOR_NAME_CT)) {
                    ...
                    try {
                        ...
                        Rlog.e(LOG_TAG, "IOException happened when close mcciccidtable parser.");
                    }
                }
            }
            fin2.close();
        } catch (XmlPullParserException e2) {
            Rlog.e(LOG_TAG, "XmlPullParserException in mcciccidtable parser.");
            if (fin != null) {
                fin.close();
            }
        } catch (IOException e3) {
            Rlog.e(LOG_TAG, "IOException in mcciccidtable parser.");
            if (fin != null) {
                fin.close();
            }
        } catch (Exception e4) {
            Rlog.e(LOG_TAG, "Exception in mcciccidtable parser.");
            if (fin != null) {
                fin.close();
            }
        } catch (Throwable th) {
            ...
            if (imsiArray != null && imsiArray.length > 0) {
                for (String item : imsiArray) {
                    try {
                        ...
                        Rlog.e(LOG_TAG, "Invalid imsi in  imsiIccidTable.");
                    }
                }
            }
            if (iccidArray != null && iccidArray.length > 0) {
                for (String item2 : iccidArray) {
                    try {
                        ...
                        Rlog.e(LOG_TAG, "Invalid iccid in  imsiIccidTable.");
                    }
                }
            }
        }

...
public class HwVSimPhoneSwitcher extends Handler {
    ...
    public int getTopPrioritySubscriptionId() {
        if (this.mPrioritizedDcRequests.size() > 0) {
            DcRequest request = this.mPrioritizedDcRequests.get(0);
            if (request != null) {
    ...
    private int phoneIdForRequest(NetworkRequest netRequest) {
        int subId;
        StringNetworkSpecifier specifier = netRequest.networkCapabilities.getNetworkSpecifier();

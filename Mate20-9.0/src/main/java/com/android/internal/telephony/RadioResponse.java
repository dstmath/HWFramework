...
public class RadioResponse extends IRadioResponse.Stub {
            ...
            for (int i = 0; i < num; i++) {
                ...
                if (calls.get(i).uusInfo.size() == 1) {
                    ...
                    if (!TextUtils.isEmpty(calls.get(i).uusInfo.get(0).uusData)) {
                        dc.uusInfo.setUserData(calls.get(i).uusInfo.get(0).uusData.getBytes());
                    } else {
            ...
            for (int i = 0; i < num; i++) {
                ...
                if (calls.get(i).base.uusInfo.size() == 1) {
                    ...
                    if (!TextUtils.isEmpty(calls.get(i).base.uusInfo.get(0).uusData)) {
                        dc.uusInfo.setUserData(calls.get(i).base.uusInfo.get(0).uusData.getBytes());
                    } else {
                        this.mRil.riljLog("responseCurrentCalls: uusInfo data is null or empty");
                    }
                    this.mRil.riljLogv(String.format("Incoming UUS : type=%d, dcs=%d, length=%d", new Object[]{Integer.valueOf(dc.uusInfo.getType()), Integer.valueOf(dc.uusInfo.getDcs()), Integer.valueOf(dc.uusInfo.getUserData().length)}));
    ...
    private void responseSignalStrength(RadioResponseInfo responseInfo, SignalStrength signalStrength) {
        RILRequest rr = this.mRil.processResponse(responseInfo);
        if (rr != null) {
            android.telephony.SignalStrength ret = RIL.convertHalSignalStrength(signalStrength, this.mRil.mPhoneId.intValue());
            if (responseInfo.error == 0) {
                sendMessageResponse(rr.mResult, ret);
            }
            this.mRil.processResponseDone(rr, responseInfo, ret);
    ...
    private void responseSignalStrength_1_2(RadioResponseInfo responseInfo, android.hardware.radio.V1_2.SignalStrength signalStrength) {
        RILRequest rr = this.mRil.processResponse(responseInfo);
        if (rr != null) {
            android.telephony.SignalStrength ret = RIL.convertHalSignalStrength_1_2(signalStrength, this.mRil.mPhoneId.intValue());
            if (responseInfo.error == 0) {
                sendMessageResponse(rr.mResult, ret);
            }
            this.mRil.processResponseDone(rr, responseInfo, ret);
    ...
    private void responseGmsBroadcastConfig(RadioResponseInfo responseInfo, ArrayList<GsmBroadcastSmsConfigInfo> configs) {
        ...
        if (rr != null) {
            ...
            for (int i = 0; i < configs.size(); i++) {
                SmsBroadcastConfigInfo smsBroadcastConfigInfo = new SmsBroadcastConfigInfo(configs.get(i).fromServiceId, configs.get(i).toServiceId, configs.get(i).fromCodeScheme, configs.get(i).toCodeScheme, configs.get(i).selected);
                ret.add(smsBroadcastConfigInfo);
            }
            if (responseInfo.error == 0) {
                sendMessageResponse(rr.mResult, ret);
            }
    ...
    private void responseActivityData(RadioResponseInfo responseInfo, ActivityStatsInfo activityInfo) {
        ...
        if (rr != null) {
            if (radioResponseInfo.error == 0) {
                ...
                int[] iArr = txModeTimeMs;
                int i2 = idleModeTimeMs;
                ModemActivityInfo modemActivityInfo = new ModemActivityInfo(SystemClock.elapsedRealtime(), sleepModeTimeMs, idleModeTimeMs, txModeTimeMs, activityStatsInfo.rxModeTimeMs, 0);
                ret = modemActivityInfo;
            } else {
                ModemActivityInfo modemActivityInfo2 = new ModemActivityInfo(0, 0, 0, new int[5], 0, 0);
                ret = modemActivityInfo2;
    ...
    private void responseCarrierIdentifiers(RadioResponseInfo responseInfo, boolean allAllowed, CarrierRestrictions carriers) {
        ...
        if (rr != null) {
            ...
            while (i < carrierRestrictions.allowedCarriers.size()) {
                String mcc = carrierRestrictions.allowedCarriers.get(i).mcc;
                String mnc = carrierRestrictions.allowedCarriers.get(i).mnc;
                String spn = null;
                String imsi = null;
                String gid1 = null;
                String gid2 = null;
                int matchType = carrierRestrictions.allowedCarriers.get(i).matchType;
                String matchData = carrierRestrictions.allowedCarriers.get(i).matchData;
                if (matchType == 1) {
                    spn = matchData;
                } else if (matchType == 2) {
                    imsi = matchData;

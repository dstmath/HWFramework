...
public class SmsInterceptionService extends ISmsInterception.Stub {
    ...
    /* JADX WARNING: type inference failed for: r1v0, types: [android.os.IBinder, com.android.internal.telephony.SmsInterceptionService] */
    ...
    public boolean dispatchNewSmsToInterceptionProcess(Bundle smsInfo, boolean isWapPush) {
        ...
        synchronized (this.mListener) {
            for (int i = priorityList.size() - 1; i >= 0; i--) {
                if (isWapPush) {
                    try {
                        if (1 == this.mListener.get(priorityList.get(i)).handleWapPushDeliverActionInner(smsInfo)) {
                ...
                } else if (1 == this.mListener.get(priorityList.get(i)).handleSmsDeliverActionInner(smsInfo)) {
    ...
    public boolean sendNumberBlockedRecord(Bundle smsInfo) {
        ...
        synchronized (this.mListener) {
            ...
            while (i >= 0) {
                try {
                    if (this.mListener.get(priorityList.get(i)).sendNumberBlockedRecordInner(smsInfo)) {

...
public class UsimPhoneBookManager extends AbstractUsimPhoneBookManager implements IccConstants {
                ...
                switch (tag) {
                    ...
                    case UsimPhoneBookManager.USIM_EFCCP1_TAG /*203*/:
                        ...
                        if (data.length >= 2 && data.length <= 3) {
                            ...
                            int efid = ((data[0] & UsimPhoneBookManager.INVALID_BYTE) << 8) | (data[1] & 255);
                            SparseArray<File> sparseArray = this.mFileIds;
                            File file = new File(parentTag, efid, sfi, tagNumberWithinParentTag);
                            sparseArray.put(tag, file);
                            break;
                        } else {
            ...
            if (email != null) {
                ...
                for (int i = 0; i < recId; i++) {
                    if (this.mPbrRecords.get(i) != null) {
                        SparseArray<File> previousFileIds = this.mPbrRecords.get(i).mFileIds;
                ...
                try {
                    ...
                    if (email != null && !email.equals("")) {
                        if (sfi == -1 || this.mSfiEfidTable.get(sfi) == 0) {
                            File file = this.mPbrRecords.get(recId).mFileIds.get(192);
                            if (file != null) {
                        ...
                        int index = ((65535 & adnEfid) << 8) | ((adnRecId + INVALID_BYTE) & 255);
    ...
    private boolean buildType2EmailList(int recId) {
        ...
        File adnFile = this.mPbrRecords.get(recId).mFileIds.get(192);
        if (adnFile == null) {
        ...
        for (int i = 0; i < numRecs; i++) {
            try {
                String email = readEmailRecord(this.mIapFileRecord.get(i)[this.mPbrRecords.get(recId).mFileIds.get(USIM_EFEMAIL_TAG).getIndex()] + INVALID_BYTE);
                if (email != null && !email.equals("")) {
    ...
    private void createPbrFile(ArrayList<byte[]> records) {
        ...
        while (it.hasNext()) {
            ...
            File file = record.mFileIds.get(192);
            if (file != null) {
                int sfi = file.getSfi();
                if (sfi != -1) {
                    this.mSfiEfidTable.put(sfi, record.mFileIds.get(192).getEfid());
                }
            }
        }
    }

...
public class AdnRecordCache extends AbstractAdnRecordCache implements IccConstants {
    ...
    private void clearWaiters() {
        ...
        for (int i = 0; i < size; i++) {
            notifyWaiters(this.mAdnLikeWaiters.valueAt(i), new AsyncResult(null, null, new RuntimeException("AdnCache reset")));
        }
    ...
    public void updateAdnBySearch(int efid, AdnRecord oldAdn, AdnRecord newAdn, String pin2, Message response) {
        ...
        int extensionEF = extensionEfForEf(efid);
        if (extensionEF < 0) {
            sendErrorResponse(message, "EF is not known ADN-like EF:0x" + Integer.toHexString(efid).toUpperCase());
            return;
        }
        if (efid2 == 20272) {
        ...
        if (oldAdnList2 == null) {
            sendErrorResponse(message, "Adn list not exist for EF:0x" + Integer.toHexString(efid).toUpperCase());
            return;
        }
        int index = -1;
        ...
        if (efid2 == 20272) {
            AdnRecord foundAdn = oldAdnList2.get(index - 1);
        ...
        if (this.mUserWriteResponse.get(efid2) != null) {
            sendErrorResponse(message, "Have pending update for EF:0x" + Integer.toHexString(efid2).toUpperCase());
            return;
        }
    ...
    public void requestLoadAllAdnLike(int efid, int extensionEf, Message response) {
        ...
        this.mAdnLikeWaiters.put(efid, arrayList);
        if (extensionEf < 0) {
            if (response != null) {
    ...
    public void handleMessage(Message msg) {
        switch (msg.what) {
            ...
            case 2:
                ...
                if (ar2.exception == null) {
                    if (this.mAdnLikeFiles.get(efid2) != null) {
                        if (index < 1 || index > this.mAdnLikeFiles.get(efid2).size()) {
                            Rlog.d("ADN RECORD", "IndexOutOfBounds index = " + index + ",size = " + this.mAdnLikeFiles.get(efid2).size());
                        } else {
                ...
                if (response != null) {
                    AsyncResult.forMessage(response, null, ar2.exception);
                    response.sendToTarget();
                    return;
                }
                return;

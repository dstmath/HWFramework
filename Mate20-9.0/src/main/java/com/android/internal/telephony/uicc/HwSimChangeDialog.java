...
public class HwSimChangeDialog {
    ...
    public AlertDialog getSimAddDialog(Context c, boolean isAdded, int mSlotId) {
        ...
        if (isAdded) {
            title = r.getString(17041113);
        } else {
            title = r.getString(17041116);
        }
        if (TelephonyManager.getDefault().isMultiSimEnabled()) {
            ...
            message = String.format(message2, new Object[]{Integer.valueOf(mSlotId + 1)});
        } else {
            if (isAdded) {
    ...
    private AlertDialog getDialogWithCountDownTimer(Context context, boolean isAdded, int mSlotId) {
        ...
        if (TelephonyManager.getDefault().isMultiSimEnabled()) {
            if (isAdded) {
                message2 = context2.getString(33685757);
            } else {
                message2 = context2.getString(33685758);
            }
            message = String.format(message2, new Object[]{Integer.valueOf(mSlotId + 1)});
        } else {
            message = context2.getString(33685952);
        }
        this.dialog = new AlertDialog.Builder(context2, 33947691).setTitle(title).setMessage(message).create();
        LinearLayout layout = new LinearLayout(context2);
        layout.setOrientation(0);
        ...
        this.dialog.setCanceledOnTouchOutside(false);
        LinearLayout.LayoutParams layoutParams = lpButtonRestart;
        int i = themeID;
    ...
    private void handleCountdown(Context context, Button restart) {
        if (restart != null && context != null && this.dialog != null) {
            ...
            AnonymousClass5 r2 = new CountDownTimer(3000, TIME_UNIT) {
                public void onTick(long millisUntilFinished) {
                    int unused = HwSimChangeDialog.this.mCurrentTime = (int) (millisUntilFinished / HwSimChangeDialog.TIME_UNIT);
    ...
    public void isGoingToshowCountDownTimerDialog(CommandsInterface.RadioState radioState, CommandsInterface.RadioState lastRadioState, IccCardStatus.CardState oldState, IccCardStatus.CardState cardState, Handler handler, int phoneId) {
        if (IS_SHOW_TIME) {
            ...
            if (radioState == CommandsInterface.RadioState.RADIO_ON && lastRadioState == CommandsInterface.RadioState.RADIO_ON && oldState == IccCardStatus.CardState.CARDSTATE_ABSENT && cardState != IccCardStatus.CardState.CARDSTATE_ABSENT) {
                PhoneFactory.getPhone(phoneId).setRadioPower(false);
            }
            if (radioState != CommandsInterface.RadioState.RADIO_UNAVAILABLE && lastRadioState == CommandsInterface.RadioState.RADIO_OFF) {
                if (oldState == IccCardStatus.CardState.CARDSTATE_PRESENT && cardState == IccCardStatus.CardState.CARDSTATE_ABSENT) {
                    handler.sendMessage(handler.obtainMessage(13, null));
                } else if (oldState == IccCardStatus.CardState.CARDSTATE_ABSENT && cardState == IccCardStatus.CardState.CARDSTATE_PRESENT) {

...
public class InstallCarrierAppUtils {
    ...
    static void showNotification(Context context, String pkgName) {
        ...
        if (TextUtils.isEmpty(appName)) {
            ...
            message = res.getString(17040225, new Object[]{appName});
        }
        ...
        getNotificationManager(context).notify(pkgName, 12, new Notification.Builder(context, NotificationChannelController.CHANNEL_ID_SIM).setContentTitle(title).setContentText(message).setSmallIcon(17302752).addAction(new Notification.Action.Builder(null, downloadButtonText, PendingIntent.getActivity(context, 0, getPlayStoreIntent(pkgName), 134217728)).build()).setOngoing(persistent).setVisibility(-1).build());
    }

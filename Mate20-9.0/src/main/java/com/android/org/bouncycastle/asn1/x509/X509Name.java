...
public class X509Name extends ASN1Object {
    ...
    public X509Name(Vector ordering2, Hashtable attributes, X509NameEntryConverter converter2) {
        ...
        int i = 0;
        if (ordering2 != null) {
            for (int i2 = 0; i2 != ordering2.size(); i2++) {
                this.ordering.addElement(ordering2.elementAt(i2));
    ...
    public boolean equals(Object r19) {
        ...
        try {
            X509Name other = getInstance(r19);
            int orderingSize = this.ordering.size();
            if (orderingSize != other.ordering.size()) {
                return false;
            }
            ...
            for (int i2 = start; i2 != end; i2 += delta) {
                boolean found = false;
                ASN1ObjectIdentifier oid = (ASN1ObjectIdentifier) this.ordering.elementAt(i2);
                String value = (String) this.values.elementAt(i2);
                int j = i;
                while (true) {
                    if (j >= orderingSize) {
                        break;

    ...
    static DuckTypedPSKKeyManager getInstance(Object obj) throws NoSuchMethodException {
        ...
        for (Method targetMethod : PSKKeyManager.class.getMethods()) {
            if (!targetMethod.isSynthetic()) {
                Class<?> sourceReturnType = sourceClass.getMethod(targetMethod.getName(), targetMethod.getParameterTypes()).getReturnType();
                if (!targetMethod.getReturnType().isAssignableFrom(sourceReturnType)) {

...
final class BroadcastRecord extends Binder {
    ...
    private boolean isFromEmailMDM(Intent intent2) {
        if (intent2 == null) {
            return false;
        }
        return "com.huawei.devicepolicy.action.POLICY_CHANGED".equals(intent2.getAction());
    }
    ...
    public boolean cleanupDisabledPackageReceiversLocked(String packageName, Set<String> filterByClasses, int userId2, boolean doit) {
        if ((userId2 != -1 && this.userId != userId2) || this.receivers == null) {
            return false;
        }
        ...
        for (int i = this.receivers.size() - 1; i >= 0; i--) {
            ...
            if (o instanceof ResolveInfo) {
                ...
                if (!(packageName == null || (info.applicationInfo.packageName.equals(packageName) && (filterByClasses == null || filterByClasses.contains(info.name))))) {
                    continue;
                } else if (!doit) {
                    return true;
                } else {

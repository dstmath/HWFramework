...
public final class ContentProviderRecord implements ComponentName.WithComponentName {
    ...
    public ContentProviderRecord(ActivityManagerService _service, ProviderInfo _info, ApplicationInfo ai, ComponentName _name, boolean _singleton) {
        ...
        if (_info.processName != null) {
            Iterator<String> it = NEED_RELEASE_PROVIDERS.iterator();
            while (true) {
                if (!it.hasNext()) {
                    break;
                }
                if (_info.processName.contains(it.next())) {
        ...
        this.noReleaseNeeded = (this.uid == 0 || this.uid == 1000) && !releaseProvider;
    }
    ...
    public void addExternalProcessHandleLocked(IBinder token) {
        ...
        int unused = handle.mAcquisitionCount = handle.mAcquisitionCount + 1;
    }
    ...
    public void removeExternalProcessHandleInternalLocked(IBinder token) {
        this.externalProcessTokenToHandle.get(token).unlinkFromOwnDeathLocked();

...
public class HwPCMultiWindowPolicy {
    ...
    public static void layoutWindowForPadPCMode(WindowManagerPolicy.WindowState win, Rect pf, Rect df, Rect cf, Rect vf, int contentBottom) {
        if (HwPCUtils.enabledInPad() || HwPCUtils.mTouchDeviceID != -1) {
            int windowState = -1;
            ActivityRecord r = ActivityRecord.forToken(win.getAppToken());
            if (r != null) {

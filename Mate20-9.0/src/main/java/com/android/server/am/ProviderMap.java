...
public final class ProviderMap {
    ...
    private boolean collectPackageProvidersLocked(String packageName, Set<String> filterByClasses, boolean doit, boolean evenPersistent, HashMap<ComponentName, ContentProviderRecord> providers, ArrayList<ContentProviderRecord> result) {
        ...
        for (ContentProviderRecord provider : providers.values()) {
            if ((packageName == null || (provider.info.packageName.equals(packageName) && (filterByClasses == null || filterByClasses.contains(provider.name.getClassName())))) && (provider.proc == null || evenPersistent || !provider.proc.persistent)) {
                if (!doit) {
                    return true;
                }
                didSomething = true;
    ...
    public boolean collectPackageProvidersLocked(String packageName, Set<String> filterByClasses, boolean doit, boolean evenPersistent, int userId, ArrayList<ContentProviderRecord> result) {
        ...
        if (i == -1) {
            ...
            while (true) {
                ...
                if (collectPackageProvidersLocked(packageName, filterByClasses, doit, evenPersistent, this.mProvidersByClassPerUser.valueAt(i3), result)) {
                    if (!doit) {
                        return true;
                    }
                    didSomething = true;
    ...
    public boolean dumpProvidersLocked(PrintWriter pw, boolean dumpAll, String dumpPackage) {
        ...
        for (int i = 0; i < this.mProvidersByClassPerUser.size(); i++) {
            needSep2 |= dumpProvidersByClassLocked(pw, dumpAll, dumpPackage, "  Published user " + this.mProvidersByClassPerUser.keyAt(i) + " content providers (by class):", needSep2, this.mProvidersByClassPerUser.valueAt(i));
        }
        if (!dumpAll) {
            return needSep2;
        }

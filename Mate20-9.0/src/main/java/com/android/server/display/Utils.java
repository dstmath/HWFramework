...
public class Utils {
    ...
    public static final int COLOR_TEMPERATURE_MODE_MANUAL = 0;
    ...
    public static Bundle getNotificationThemeData(Bundle bundle, int contIconId, int repIconId, int bgIndex, int repLocation) {
        ...
        if (contIconId > 0) {
            bundle2.putInt(HW_NOTIFICATION_CONTENT_ICON, contIconId);
        }
        if (repIconId > 0) {
            bundle2.putInt(HW_NOTIFICATION_REPLACE_ICONID, repIconId);
        }
        if (bgIndex >= 0) {
            bundle2.putInt(HW_NOTIFICATION_BACKGROUND_INDEX, bgIndex);
        }
        if (repLocation > 0) {
            bundle2.putInt(HW_NOTIFICATION_REPLACE_LOCATION, repLocation);
        }
        return bundle2;
    }

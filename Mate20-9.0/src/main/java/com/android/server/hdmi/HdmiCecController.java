...
final class HdmiCecController {
    ...
    /* access modifiers changed from: private */
    @HdmiAnnotations.ServiceThreadOnly
    public void runDevicePolling(int sourceAddress, List<Integer> candidates, int retryCount, HdmiControlService.DevicePollingCallback callback, List<Integer> allocated) {
        assertRunOnServiceThread();
        ...
        final Integer remove = candidates.remove(0);
        final int i2 = retryCount;
        final List<Integer> list = allocated;
        final List<Integer> list2 = candidates;
        final HdmiControlService.DevicePollingCallback devicePollingCallback = callback;
    ...
    public void sendCommand(final HdmiCecMessage cecMessage, final HdmiControlService.SendMessageCallback callback) {
        ...
        runOnIoThread(new Runnable() {
            public void run() {
                ...
                final int i3 = errorCode;
                if (i3 != 0) {
                    Slog.w(HdmiCecController.TAG, "Failed to send " + cecMessage + " with errorCode=" + i3);
                }
                if (callback != null) {
                    HdmiCecController.this.runOnServiceThread(new Runnable() {
                        public void run() {
                            callback.onSendCompleted(i3);
                        }
                    });
                }
            }

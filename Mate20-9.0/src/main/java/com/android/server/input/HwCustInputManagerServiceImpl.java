...
public class HwCustInputManagerServiceImpl extends HwCustInputManagerService {
    ...
    public void setGloveMode() {
        if (this.mContext != null) {
            int isGloveMode = Settings.System.getIntForUser(this.mContext.getContentResolver(), "glove_file_node", 0, ActivityManager.getCurrentUser());
            if (HWDBG) {
    ...
    public int registerContentObserverForSetGloveMode(Context context) {
        ...
        if (this.mContext != null) {
            this.mContext.getContentResolver().registerContentObserver(Settings.System.getUriFor("glove_file_node"), false, new GloveModeObserver(), -1);

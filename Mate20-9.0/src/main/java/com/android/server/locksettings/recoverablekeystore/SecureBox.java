        ...
        if (encryptedPayload != null) {
            ...
            if (Arrays.equals(readEncryptedPayload(ciphertextBuffer, VERSION.length), VERSION)) {
                if (ourPrivateKey == null) {
                    ...
                    hkdfInfo = dhComputeSecret(ourPrivateKey, decodePublicKey(readEncryptedPayload(ciphertextBuffer, 65)));
                    dhSecret = HKDF_INFO_WITH_PUBLIC_KEY;
                }
                return aesGcmDecrypt(hkdfDeriveKey(concat(hkdfInfo, sharedSecret2), HKDF_SALT, dhSecret), readEncryptedPayload(ciphertextBuffer, 12), readEncryptedPayload(ciphertextBuffer, ciphertextBuffer.remaining()), header2);
            }

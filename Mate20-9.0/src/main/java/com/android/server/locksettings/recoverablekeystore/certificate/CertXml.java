...
public final class CertXml {
    ...
    private CertXml(long serial2, long refreshInterval2, List<X509Certificate> intermediateCerts2, List<X509Certificate> endpointCerts2) {
        this.serial = serial2;
    ...
    public static CertXml parse(byte[] bytes) throws CertParsingException {
        ...
        CertXml certXml = new CertXml(parseSerial(rootNode), parseRefreshInterval(rootNode), parseIntermediateCerts(rootNode), parseEndpointCerts(rootNode));
        return certXml;
    }
    ...
    private static long parseRefreshInterval(Element rootNode) throws CertParsingException {
        return Long.parseLong(CertUtils.getXmlNodeContents(1, rootNode, METADATA_NODE_TAG, METADATA_REFRESH_INTERVAL_NODE_TAG).get(0));
    }

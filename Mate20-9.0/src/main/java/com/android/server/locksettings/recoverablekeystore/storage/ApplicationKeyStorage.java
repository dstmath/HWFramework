...
public class ApplicationKeyStorage {
    ...
    public void setSymmetricKeyEntry(int userId, int uid, String alias, byte[] secretKey) throws KeyStoreException {
        ...
        try {
            this.mKeyStore.setEntry(getInternalAlias(userId, uid, alias), new KeyStore.SecretKeyEntry(new SecretKeySpec(secretKey, "AES")), new KeyProtection.Builder(3).setBlockModes(new String[]{"GCM"}).setEncryptionPaddings(new String[]{"NoPadding"}).build());

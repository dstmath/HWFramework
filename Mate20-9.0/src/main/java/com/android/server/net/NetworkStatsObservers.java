...
public class NetworkStatsObservers {
    ...
    private RequestInfo buildRequestInfo(DataUsageRequest request, Messenger messenger, IBinder binder, int callingUid, int accessLevel) {
        boolean z = true;
        if (accessLevel <= 1) {
            UserUsageRequestInfo userUsageRequestInfo = new UserUsageRequestInfo(this, request, messenger, binder, callingUid, accessLevel);
            return userUsageRequestInfo;
        }
        if (accessLevel < 2) {
            z = false;
        }
        Preconditions.checkArgument(z);
        NetworkUsageRequestInfo networkUsageRequestInfo = new NetworkUsageRequestInfo(this, request, messenger, binder, callingUid, accessLevel);
        return networkUsageRequestInfo;
    }
}

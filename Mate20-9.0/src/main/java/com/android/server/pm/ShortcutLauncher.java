...
class ShortcutLauncher extends ShortcutPackageItem {
    ...
    private ShortcutLauncher(ShortcutUser shortcutUser, int ownerUserId, String packageName, int launcherUserId, ShortcutPackageInfo spi) {
        super(shortcutUser, launcherUserId, packageName, r0);
    ...
    public static ShortcutLauncher loadFromXml(XmlPullParser parser, ShortcutUser shortcutUser, int ownerUserId, boolean fromBackup) throws IOException, XmlPullParserException {
        ...
        int i = ownerUserId;
        boolean z2 = fromBackup;
        ShortcutLauncher ret = new ShortcutLauncher(shortcutUser, i, ShortcutService.parseStringAttribute(xmlPullParser, ATTR_PACKAGE_NAME), z2 ? i : ShortcutService.parseIntAttribute(xmlPullParser, ATTR_LAUNCHER_USER_ID, i));
            ...
            if (type == 2) {
                ...
                if (depth == outerDepth + 1) {
                    ...
                    if (hashCode == -1923478059) {
                        ...
                        switch (z) {
                            case false:
                                ret.getPackageInfo().loadFromXml(xmlPullParser, z2);
                                continue;
                                continue;
                                continue;
                            case true:
                                String packageName = ShortcutService.parseStringAttribute(xmlPullParser, ATTR_PACKAGE_NAME);
                                int packageUserId = z2 ? i : ShortcutService.parseIntAttribute(xmlPullParser, ATTR_PACKAGE_USER_ID, i);
                                ids = new ArraySet<>();

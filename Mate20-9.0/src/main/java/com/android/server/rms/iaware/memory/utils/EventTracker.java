    ...
    public ArrayList<DumpData> getDumpData(int time) {
        ...
        synchronized (this.mRecordData) {
            try {
                ...
                for (int i = this.mRecordData.size() > 5 ? this.mRecordData.size() - 5 : 0; i < this.mRecordData.size(); i++) {
                    DumpData tempDd = this.mRecordData.get(i);
            ...
            synchronized (this.mRecordData) {
                if (this.mRecordData.isEmpty()) {
                    ...
                    while (!this.mRecordData.isEmpty()) {
                        if (!(time - this.mRecordData.get(0).getTime() > AppHibernateCst.DELAY_ONE_MINS)) {
                            break;
                        }
    ...
    public void insertStatisticData(String subtype, int exetime, int effect) {
        ...
        if (str != null) {
            synchronized (this.mStatisticsData) {
                ...
                long j = now;
                StatisticsData data2 = new StatisticsData(MEMORY_FEATURE_ID, 2, str, 1, exetime, effect, now, now);

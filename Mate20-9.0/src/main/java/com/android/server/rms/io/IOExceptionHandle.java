...
public class IOExceptionHandle {
    ...
    public List<AppExceptionData> checkExceptionOnApp() {
        ...
        for (int index = 0; index < historyListSize; index++) {
            AppExceptionData appExceptionData = historyList.get(historyList.keyAt(index)).checkIfAppIOException();
    ...
    public ExceptionData checkIfExistException() {
        ...
        Iterator<CheckHandler> it = checkList.iterator();
        while (true) {
            if (!it.hasNext()) {
                break;
            }
            CheckHandler checkHandler = it.next();

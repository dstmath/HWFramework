...
public class LifeTimeCollection implements IOFileRotator.Reader {
    private static final String TAG = "RMS.IO.LifeTimeCollection";
    private List<IOExceptionHandle.LifeTimeData> mDataList = new ArrayList();

    static class LifeTimeRewriter implements IOFileRotator.Rewriter {
    ...
    public void addAll(List<IOExceptionHandle.LifeTimeData> dataList) {
        if (dataList == null) {
    ...
    public List<IOExceptionHandle.LifeTimeData> getAllDataList() {
    ...
    public void read(InputStream in) throws IOException {
        ...
        List<IOExceptionHandle.LifeTimeData> currentList = new ArrayList<>();
        currentList.addAll(this.mDataList);
        this.mDataList.clear();
        while (inputStream.available() > 0) {
            this.mDataList.add(new IOExceptionHandle.LifeTimeData(inputStream.readLong(), inputStream.readInt()));
        }
    ...
    public void write(OutputStream out) throws IOException {
        ...
        for (IOExceptionHandle.LifeTimeData data : this.mDataList) {

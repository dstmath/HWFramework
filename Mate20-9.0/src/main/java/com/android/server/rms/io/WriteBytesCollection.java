...
public class WriteBytesCollection implements IOFileRotator.Reader {
    private static final String TAG = "RMS.IO.LifeTimeCollection";
    private List<IOExceptionHandle.WriteBytesData> mDataList = new ArrayList();

    static class TotalWrittenRewriter implements IOFileRotator.Rewriter {
    ...
    public void addAll(List<IOExceptionHandle.WriteBytesData> dataList) {
        if (dataList == null) {
    ...
    public List<IOExceptionHandle.WriteBytesData> getAllDataList() {
    ...
    public void read(InputStream in) throws IOException {
        ...
        List<IOExceptionHandle.WriteBytesData> currentList = new ArrayList<>();
        currentList.addAll(this.mDataList);
        this.mDataList.clear();
        while (inputStream.available() > 0) {
            this.mDataList.add(new IOExceptionHandle.WriteBytesData(inputStream.readLong(), inputStream.readLong()));
        }
    ...
    public void write(OutputStream out) throws IOException {
        ...
        for (IOExceptionHandle.WriteBytesData data : this.mDataList) {

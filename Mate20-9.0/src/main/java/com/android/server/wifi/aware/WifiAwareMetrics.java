...
public class WifiAwareMetrics {
    ...
    static {
        MetricsUtils.LogHistParms logHistParms = new MetricsUtils.LogHistParms(0, 1, 10, 9, 8);
        DURATION_LOG_HISTOGRAM = logHistParms;
    }
    ...
    private void recordDiscoverySessionInternal(int uid, SparseArray<WifiAwareClientState> clients, boolean isRangingEnabledSubscriber, int minRange, int maxRange) {
        ...
        while (i3 < clients.size()) {
            ...
            int numSubscribesWithRangingOnUid = numPublishesWithRangingOnUid;
            int numPublishesWithRangingOnUid2 = numSubscribesWithRangingInSystem;
            int numSubscribesWithRangingInSystem2 = numPublishesWithRangingInSystem;
            int numPublishesWithRangingInSystem2 = numSubscribesOnUid;
            int numSubscribesOnUid2 = numPublishesInSystem;
            ...
            while (j < sessions.size()) {
                ...
                if (session.isPublishSession()) {
                    ...
                    if (sameUid) {
                        numPublishesOnUid++;
                        if (isRangingEnabledForThisSession) {
                            numPublishesWithRangingOnUid2++;
                        }
                    }
                } else {
                    numSubscribesInSystem++;
                    if (isRangingEnabledForThisSession) {
                        numSubscribesWithRangingInSystem2++;
                    }
                    if (sameUid) {
                        numSubscribesOnUid2++;
                        if (isRangingEnabledForThisSession) {
            ...
            numPublishesInSystem = numSubscribesOnUid2;
            numSubscribesOnUid = numPublishesWithRangingInSystem2;
            numPublishesWithRangingInSystem = numSubscribesWithRangingInSystem2;
            numSubscribesWithRangingInSystem = numPublishesWithRangingOnUid2;
        ...
        synchronized (this.mLock) {
            ...
            this.mMaxSubscribeInApp = Math.max(this.mMaxSubscribeInApp, numPublishesInSystem);
            this.mMaxDiscoveryInApp = Math.max(this.mMaxDiscoveryInApp, numPublishesOnUid + numPublishesInSystem);
            ...
            this.mMaxPublishWithRangingInApp = Math.max(this.mMaxPublishWithRangingInApp, numSubscribesWithRangingInSystem);
            this.mMaxSubscribeWithRangingInApp = Math.max(this.mMaxSubscribeWithRangingInApp, numPublishesWithRangingOnUid);
            this.mMaxPublishWithRangingInSystem = Math.max(this.mMaxPublishWithRangingInSystem, numSubscribesOnUid);
            this.mMaxSubscribeWithRangingInSystem = Math.max(this.mMaxSubscribeWithRangingInSystem, numPublishesWithRangingInSystem);

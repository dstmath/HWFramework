...
public class HSWanMetricsElement extends ANQPElement {
    ...
    public static HSWanMetricsElement parse(ByteBuffer payload) throws ProtocolException {
        ...
        if (payload.remaining() == 13) {
            ...
            byte b3 = b2;
            byte b4 = b;
            HSWanMetricsElement hSWanMetricsElement = new HSWanMetricsElement(status, symmetric, capped, downlinkSpeed, uplinkSpeed, b, b2, ((int) ByteBufferReader.readInteger(byteBuffer, ByteOrder.LITTLE_ENDIAN, 2)) & Constants.SHORT_MASK);

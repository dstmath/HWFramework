            ...
            if (isBucketMaybeScanned(i, scanData.getBucketsScanned())) {
                ...
                while (true) {
                    ...
                    if (scanSettings.numBssidsPerScan > 0 && filteredResults.size() >= scanSettings.numBssidsPerScan) {
                        keyLogs.append(", result size:" + filteredResults.size() + ", numBssidsPerScan:" + scanSettings.numBssidsPerScan);
                        break;
                    }
                    i2++;
                }
                if (filteredResults.size() == scanData.getResults().length) {
                    ...
                    filteredScanDatas.add(new WifiScanner.ScanData(scanData.getId(), scanData.getFlags(), (ScanResult[]) filteredResults.toArray(new ScanResult[filteredResults.size()])));
                }
            } else {
                ChannelHelper channelHelper3 = channelHelper;
                keyLogs.append(",isBucketMaybeScanned false scheduledBucket:" + i + " scanData.getBucketsScanned() " + scanData.getBucketsScanned());
            }
        }

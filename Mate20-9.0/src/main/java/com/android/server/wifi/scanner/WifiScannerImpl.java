    ...
    public static final WifiScannerImplFactory DEFAULT_FACTORY = new WifiScannerImplFactory() {
        public WifiScannerImpl create(Context context, Looper looper, Clock clock) {
            ...
            if (wifiNative.getBgScanCapabilities(ifaceName, new WifiNative.ScanCapabilities())) {
                HalWifiScannerImpl halWifiScannerImpl = new HalWifiScannerImpl(context, ifaceName, wifiNative, wifiMonitor, looper, clock);
                return halWifiScannerImpl;
            }
            WificondScannerImpl wificondScannerImpl = new WificondScannerImpl(context, ifaceName, wifiNative, wifiMonitor, new WificondChannelHelper(wifiNative), looper, clock);
            return wificondScannerImpl;
        }
    };

...
public class ApConfigUtil {
    ...
    public static int chooseApChannel(int apBand, ArrayList<Integer> allowed2GChannels, int[] allowed5GFreqList) {
        if (apBand != 0 && apBand != 1 && apBand != -1) {
            ...
            return getSelected5GChannel(allowed5GFreqList);
        } else {
    ...
    public static int getSelected5GChannel(int[] allowed5GFreqList) {
        ...
        int j = 0;
        for (int convertFrequencyToChannel : allowed5GFreqList) {
            if (convertFrequencyToChannel(convertFrequencyToChannel) == 149) {
                Log.d(TAG, "5G Channel is 149");
                return DEFAULT_AP_CHANNEL_5G;
            }
        }
        int len = allowed5GFreqList.length;
        for (int convertFrequencyToChannel2 : allowed5GFreqList) {
            if (convertFrequencyToChannel(convertFrequencyToChannel2) == 165) {

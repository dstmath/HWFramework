...
public class HwDualBandQualityEngine {
    ...
    private static final RttScore[] RTT_SCORE_TABLE = {new RttScore(400, 15), new RttScore(800, 10), new RttScore(WifiproBqeUtils.BQE_GOOD_RTT, 5), new RttScore(2000, 0), new RttScore(4000, -5), new RttScore(16000, -10)};
    ...
    public int getScoreByRssi(int rssi) {
        ...
        while (true) {
            if (index > maxIndex) {
                break;
            } else if (rssi <= RSSI_SCORE_TABLE[index].RSSI) {
                index++;
            } else if (index == 0) {
    ...
    public int getScoreByConnectTime(int connectTime) {
        ...
        while (true) {
            if (index > maxIndex) {
                break;
            } else if (connectTime <= AP_CONNECT_TIME_SCORE_TABLE[index].TOTAL_CONNECT_TIME) {
                index++;
            } else if (index == 0) {
    ...
    public int getScoreByLossRate(double lossRate) {
        ...
        while (true) {
            if (index > maxIndex) {
                break;
            } else if (lossRate >= LOSE_RATE_SCORE_TABLE[index].LOSE_RATE) {
                index++;
            } else if (index == 0) {

    ...
    private boolean checkIfRcdExist() {
        ...
        try {
            ...
            while (true) {
                ...
                String lastStatUploadTime = c.getString(c.getColumnIndex("mLastStatUploadTime"));
                logd("checkIfRcdExist read record succ, lastStatUploadTime:" + lastStatUploadTime);
            }
            c.close();
            if (recCnt > 1) {
    ...
    public boolean addOrUpdateChrStatRcd(WifiProStatisticsRecord dbr) {
        synchronized (this.mChrLock) {
            if (this.mDatabase != null && this.mDatabase.isOpen()) {
                if (dbr != null) {
                    if (checkIfRcdExist()) {
                        boolean updateChrStatRcd = updateChrStatRcd(dbr);
                        return updateChrStatRcd;
                    }
                    boolean insertChrStatRcd = insertChrStatRcd(dbr);
                    return insertChrStatRcd;
                }
            }

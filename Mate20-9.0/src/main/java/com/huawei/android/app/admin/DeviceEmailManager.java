...
public class DeviceEmailManager {
    ...
    private Uri getUriBySyncType(int syncType, int userId) {
        switch (syncType) {
            case 0:
                return Uri.parse("content://" + userId + "@com.android.email.huawei.mdm.provider/syncCalendar");
            case 1:
                return Uri.parse("content://" + userId + "@com.android.email.huawei.mdm.provider/syncContacts");
            default:
                return null;
        }
    }
    ...
    public boolean isAccountAddedByMDM(Context context, String emailAddress, int userId) {
        ...
        boolean result = false;
        try {
            cursor = context.getContentResolver().query(Uri.parse("content://" + userId + "@com.android.email.huawei.mdm.provider/accountAdded").buildUpon().appendQueryParameter(KEY_EMAIL_ADDRESS, emailAddress).build(), new String[]{ACCOUNT_ADDED_BY_MDM}, null, null, null);
            if (cursor != null && cursor.moveToNext()) {
    ...
    public boolean isForceSMIMECertificateAlias(Context context, String emailAddress, String alias, int type) {
        ...
        try {
            ...
            if (cursor != null && cursor.moveToNext()) {
                return TextUtils.equals(alias, cursor.getString(0));
            }
            if (cursor != null) {
                cursor.close();
            }
    ...
    public boolean isForceSMIMEMessages(Context context, String emailAddress, int type) {
        ...
        try {
            ...
            return Boolean.TRUE.toString().equals(cursor.getString(0));
        } finally {
            if (cursor != null) {
                cursor.close();
            }
    ...
    private Uri getUri(String path, int type) {
        return Uri.parse("content://com.android.email.huawei.mdm.provider/" + path + "/" + type);
    }
}

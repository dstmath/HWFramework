...
public class BaseFmConfig {
    ...
    protected static boolean fmConfigure(int fd, BaseFmConfig configSettings) {
        Log.d(TAG, "In fmConfigure");
        int controlNative = FmReceiverWrapper.setControlNative(fd, V4L2_CID_PRIVATE_TAVARUA_EMPHASIS, configSettings.getEmphasis());
        int re = FmReceiverWrapper.setControlNative(fd, V4L2_CID_PRIVATE_TAVARUA_RDS_STD, configSettings.getRdsStd());
        int re2 = FmReceiverWrapper.setControlNative(fd, V4L2_CID_PRIVATE_TAVARUA_SPACING, configSettings.getChSpacing());
        int re3 = FmReceiverWrapper.setBandNative(fd, configSettings.getLowerLimit(), configSettings.getUpperLimit());

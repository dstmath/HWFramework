...
class FmRxControls {
    ...
    public void searchStations(int fd, int mode, int dwell, int dir, int pty, int pi) {
        ...
        int re = FmReceiverWrapper.setControlNative(fd, V4L2_CID_PRIVATE_TAVARUA_SRCHMODE, mode);
        int re2 = FmReceiverWrapper.setControlNative(fd, V4L2_CID_PRIVATE_TAVARUA_SCANDWELL, dwell);
        if (pty != 0) {
            int re3 = FmReceiverWrapper.setControlNative(fd, V4L2_CID_PRIVATE_TAVARUA_SRCH_PTY, pty);
        }
        if (pi != 0) {
            int re4 = FmReceiverWrapper.setControlNative(fd, V4L2_CID_PRIVATE_TAVARUA_SRCH_PI, pi);
        }
        int re5 = FmReceiverWrapper.startSearchNative(fd, dir);
    }

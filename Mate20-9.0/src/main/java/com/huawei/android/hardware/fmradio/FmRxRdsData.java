...
public class FmRxRdsData {
    ...
    public int rdsGrpOptions(int grpMask, int buffSize, boolean rdsFilter) {
        int rdsFilt;
        int rds_group_mask = (byte) (((byte) FmReceiverWrapper.getControlNative(this.mFd, V4L2_CID_PRIVATE_TAVARUA_RDSGROUP_PROC)) & 254);
        if (rdsFilter) {
            rdsFilt = 1;
        } else {
            rdsFilt = 0;
    ...
    public int rdsOptions(int rdsMask) {
        ...
        int re = FmReceiverWrapper.setControlNative(this.mFd, V4L2_CID_PRIVATE_TAVARUA_RDSGROUP_PROC, (byte) (((rdsMask & 7) << 3) | ((byte) (((byte) FmReceiverWrapper.getControlNative(this.mFd, V4L2_CID_PRIVATE_TAVARUA_RDSGROUP_PROC)) & 199))));
        return FmReceiverWrapper.setControlNative(this.mFd, V4L2_CID_PRIVATE_TAVARUA_PSALL, (rdsMask & 16) >> 4);
    }

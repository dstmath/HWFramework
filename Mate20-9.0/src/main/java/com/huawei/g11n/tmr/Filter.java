    ...
    private List<Match> filterDate(String content, List<Match> ms) {
        ...
        while (i2 < ms.size()) {
            ...
            if (type == 1 || type == 5 || type == 6 || type == 7) {
                ...
                List<Match> sub = null;
                if (hasNum > 1) {
                    sub = list.subList(i2 + 1, i2 + 3);
                } else if (hasNum == 1) {
                    sub = list.subList(i2 + 1, i2 + 2);
                }
            ...
            if (curren == null) {
                ...
                if ((ctype == 1 && type == i2) || ((ctype == 1 && type == 3 && m.isTimePeriod()) || ((ctype == i2 && type == 1) || (ctype == 3 && curren.isTimePeriod() && type == 1)))) {
                    ...
                    if (flag) {
                        ...
                        if (ctype == i2 && type == 1) {
                            int add = 0;
                            String bStr = str.substring(curren.getEnd());
                            Matcher bcm = Pattern.compile("\\s*\\((.*?)\\s*\\)").matcher(bStr);
                            String d = null;
                            if (bcm.lookingAt()) {
                                d = bcm.group(1);
                        ...
                        if (ctype == 1) {
                            i = 2;
                            if (type == 2) {
                                String bStr2 = str.substring(0, curren.getBegin());
                                String eStr = str.substring(curren.getEnd(), m.getBegin());
                                if (bStr2.trim().endsWith("(") && eStr.trim().equals(")")) {
                                    curren.setBegin(bStr2.lastIndexOf(40));
                                    curren.setEnd(m.getEnd());
                                    curren.setType(0);
                                    it.remove();
                                    change = false;

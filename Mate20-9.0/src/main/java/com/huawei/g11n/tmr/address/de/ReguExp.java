...
public class ReguExp extends Regexs {
    ...
    private static String citykey1 = (String.valueOf(citykey0) + "(-" + citykey0 + ")?");
    ...
    static {
        StringBuilder sb = new StringBuilder(String.valueOf(wordsBig));
        sb.append(keyStr);
        sb.append("(?:(\\s*(?<!\\d)\\d{1,4}(?!\\d))(?:-\\d{1,4})?)?");
        ST = sb.toString();
        StringBuilder sb2 = new StringBuilder("\\s+");
        sb2.append(keyPre);
        sb2.append("\\s+");
        regPrep = sb2.toString();
    }
    ...
    public void init() {
        ...
        put("pRegBlackKeyIndi_withBlank", String.valueOf(blackkey_indi) + "(?:\\s*-\\s*|\\s+)");

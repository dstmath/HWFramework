...
public class ReguExp extends Regexs {
    ...
    private static String buiStr = (String.valueOf(bui) + "(?:(?:\\s+(?:dos?\\s+)?|\\s*\\(\\s*|\\s*,\\s*)" + "(?<street1>" + str + ")" + "(?:\\s*\\))?)?");
    ...
    static {
        StringBuilder sb = new StringBuilder(String.valueOf(keyBui));
        sb.append("\\s+(?:(?:de|dos?|das?)\\s+)?");
        sb.append(words3_Str);
        bui = sb.toString();
        StringBuilder sb2 = new StringBuilder(String.valueOf(buiStr));
        sb2.append("|");
        sb2.append("(?<street2>");
        sb2.append(str);
        sb2.append(")");
        bs = sb2.toString();
        StringBuilder sb3 = new StringBuilder(String.valueOf(con));
        sb3.append(codeCity);
        _cityWithCode = sb3.toString();
        StringBuilder sb4 = new StringBuilder("\\s*\\(.*\\)|");
        sb4.append(con);
        sb4.append("\\(?(?:");
        sb4.append(word);
        sb4.append("(?:\\s+(?:(?:(?i)de|das?|do)\\s+)?|\\s*(?:,|/|-|\\(|\\))\\s*)){0,4}");
        sb4.append(word);
        sb4.append("(?:\\s*\\))?");
        _cityNoCode = sb4.toString();
    }

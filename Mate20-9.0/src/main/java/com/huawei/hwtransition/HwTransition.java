...
public class HwTransition {
    ...
    public boolean animateDispatchDraw(Canvas canvas, int transitonX, boolean isScrolling) {
        ...
        while (i2 < n) {
            ...
            if (right >= left) {
                scrollProgress = TransitionUtil.getScrollProgress(vg, transitonX, child, i2, this.mPageSpacing);
        ...
        if (i2 < n) {
            ...
            viewInfo.fraction = scrollProgress2;
            viewInfo.isOverScrollFirst = i2 == 0 && scrollProgress2 < 0.0f;
            viewInfo.isOverScrollLast = i2 == n + -1;
            viewInfo.index = i2;
            ...
            if (scrollProgress2 > 0.0f && !viewInfo.isOverScrollLast) {
                if (right < left) {
                    i = right;
                } else {
                    i = i2 + 1;
                }
                ViewInfo viewInfo2 = new ViewInfo(vg.getChildAt(i));
                viewInfo2.fraction = -1.0f + scrollProgress2;
                viewInfo2.index = i;
                if (right < left) {
                    viewInfo2.relativeTrans = (float) TransitionUtil.getChildOffset(vg, n, this.mPageSpacing);
                }
                info.views.add(viewInfo2);
            }
            animView(canvas, info);
            for (ViewInfo vi : info.views) {
    ...
    public boolean animateDispatchDraw(Canvas canvas, int transitonX, boolean isScrolling, int leftScreen, int rightScreen) {
        ...
        if (leftScreen < 0 || leftScreen >= n) {
            ...
            if (rightScreen == n - 1) {
                if (info.views.get(info.views.size() - 1).isOverScrollLast && leftScreen < rightScreen) {

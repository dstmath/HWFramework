...
public class GarbageCanDelAnim {
    ...
    public void startDeleteAnim(View currentPage, View nextPage, float x, float y, int duration) {
        ...
        if (currentPage != null) {
            ...
            ObjectAnimator scaleAnim = ObjectAnimator.ofPropertyValuesHolder(currentPage, new PropertyValuesHolder[]{PropertyValuesHolder.ofFloat("scaleX", new float[]{MIN_CURRENT_PAGE_SCLAE}), PropertyValuesHolder.ofFloat("scaleY", new float[]{MIN_CURRENT_PAGE_SCLAE})});
            scaleAnim.setDuration((long) duration);
            scaleAnim.setInterpolator(CURRENT_PAGE_SCALE_IP);
            ObjectAnimator translationXAnim = ObjectAnimator.ofFloat(currentPage, "translationX", new float[]{0.0f, x - (((float) currentPage.getWidth()) / 2.0f)});
            translationXAnim.setDuration((long) duration);
            translationXAnim.setInterpolator(CURRENT_PAGE_TRANSLATIONX_IP);
            ObjectAnimator translationYAnim = ObjectAnimator.ofFloat(currentPage, "translationY", new float[]{0.0f, y - (((float) height) / 2.0f)});
            ...
            AnimatorSet animatorSet = this.mCurrentPageAnim;

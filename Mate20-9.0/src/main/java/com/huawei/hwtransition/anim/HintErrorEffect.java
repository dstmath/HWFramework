    ...
    public void drawErrorEffect(Canvas canvas) {
        if (this.isShowOutline) {
            ...
            if (this.mRect != null && !this.mRect.isEmpty()) {
                int delta = 0;
                for (int i = 0; i < this.mAlphaListFactor.size(); i++) {
                    this.mPaint.setAlpha((int) (this.mAlphaListFactor.get(i).floatValue() * ((float) OUTLINE_START_ALPHA)));
                ...
                for (int i2 = 0; i2 < this.mAlphaListFactor.size(); i2++) {
                    this.mPaint.setAlpha((int) (this.mAlphaListFactor.get(i2).floatValue() * ((float) OUTLINE_START_ALPHA)));
                    ...
                    canvas.restore();
                    delta2 += OUTLINE_SHADOW_STROKE_WIDTH * 2;
                }
            }
        }
    }
        ...
        while (currentAlpha > 0) {
            ...
            for (int j = -range; j < range; j++) {
                if (index + j < 0) {
                    ...
                    alpha = this.mAlphaListFactor.get(index + j).floatValue();
                }
                currentAlphaFactor += alpha;
            }
            float currentAlphaFactor2 = currentAlphaFactor / ((float) OUTLINE_SHADOW_RADIUS);
            this.mAlphaListFactor.add(Float.valueOf(currentAlphaFactor2));
            currentAlpha = (int) (((float) OUTLINE_START_ALPHA) * currentAlphaFactor2);
            index++;
        }
    }

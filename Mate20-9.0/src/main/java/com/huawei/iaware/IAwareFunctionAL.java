...
public class IAwareFunctionAL {
    ...
    public static int getFeatureTypeIAware(int featureId) {
        return AwareConstant.FeatureType.getFeatureType(featureId).ordinal();
    }

    public static int valuesFeatureIAware(int featureId) {
        return AwareConstant.FeatureType.values()[featureId].ordinal();
    }
        ...
        if (obj != null) {
            try {
                if (obj instanceof IHwTelephony) {
                    return ((IHwTelephony) obj).setTemperatureControlToModem(level, type, ((IHwTelephony) obj).getDefault4GSlotId(), null);
                }
            } catch (RemoteException e) {

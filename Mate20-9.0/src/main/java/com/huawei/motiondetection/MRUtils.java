...
public final class MRUtils {
    ...
    public static int getMotionEnableState(Context context, String motionitemkey, boolean isEx) {
        if (!MotionTypeApps.KEY_PROXI_BLUETOOTHSET.equals(motionitemkey) || !NEED_TAILORED) {
            int enabled = -1;
            if (isMotionServiceConfigurable(context)) {
                try {
        ...
        MRLog.i(TAG, "getMotionEnableState bluetoothset always enable ");
        return 1;
    }
    ...
    public static int getMotionEnableStateAsUser(Context context, String motionitemkey, int userId) {
        ...
        if (contextForUser == null) {
            ...
            return -1;
        } else if (!MotionTypeApps.KEY_PROXI_BLUETOOTHSET.equals(motionitemkey) || !NEED_TAILORED) {
            if (isMotionServiceConfigurable(contextForUser)) {
                try {
            ...
            MRLog.i(TAG, "getMotionEnableStateAsUser bluetoothset always enable");
            return 1;
        }
    }
    ...
    public static boolean isServiceRunningAsUser(Context context, String serviceProcess, int userId) {
        ...
        List<ActivityManager.RunningServiceInfo> serviceList = ((ActivityManager) contextForUser.getSystemService("activity")).getRunningServices(Integer.MAX_VALUE);
        if (serviceList == null) {
    ...
    private static boolean isMotionServiceConfigurable(Context context) {
        PackageInfo serviceAppInfo = null;
        try {
            if (context.getPackageManager() != null) {
                serviceAppInfo = context.getPackageManager().getPackageInfo(MotionConfig.MOTION_SERVICE_PACKAGE, 1);
            }

...
public class MotionDetectionManager {
    ...
    public static boolean isMotionRecoApkExist(Context context) {
        Intent intent = new Intent(MOTION_SERVICE_APK_ACTION);
        List<ResolveInfo> packages = null;
        if (context.getApplicationContext().getPackageManager() != null) {
            packages = context.getApplicationContext().getPackageManager().queryIntentActivities(intent, 0);
        }
    ...
    private boolean isMotionStopValid(int motionApps, int motionTypeReco) {
        ...
        while (i < motionAppsSize) {
            int tmpMApps = this.mMotionAppsRegList.get(i).intValue();
    ...
    public void processMotionRecoResult(int relayType, Object mrecoRes) {
        ...
        try {
            ...
            while (j < maTypeListSize) {
                ...
                for (int i = 0; i < listenerSize; i++) {
                    tmpMRes2 = getRecoResult(maTypeList.get(j).intValue(), rRes, rDirect, rExtras);
                    this.mMDListenerList.get(i).notifyMotionRecoResult(tmpMRes2);
                }
                j++;
                tmpMRes = tmpMRes2;
            }
    ...
    private ArrayList<Integer> getMotionsAppsByMotionReco(int mType) {
        ...
        if (appsRegListSize > 0) {
            for (int i = 0; i < appsRegListSize; i++) {
                if (MotionTypeApps.getMotionTypeByMotionApps(this.mMotionAppsRegList.get(i).intValue()) == mType) {

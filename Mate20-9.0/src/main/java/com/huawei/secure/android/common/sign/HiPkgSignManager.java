...
public class HiPkgSignManager {
    ...
    public static byte[] getInstalledAPPSignature(Context context, String packageName) {
        ...
        try {
            ...
            if (mPackageManager != null) {
                PackageInfo foundPkgInfo = mPackageManager.getPackageInfo(packageName, 64);
    ...
    public static byte[] getUnInstalledAPPSignature(Context context, String archiveFilePath) {
        ...
        try {
            PackageManager mPackageManager = context.getPackageManager();
            if (mPackageManager != null) {
                PackageInfo foundPkgInfo = mPackageManager.getPackageArchiveInfo(archiveFilePath, 64);
                if (foundPkgInfo != null) {
                    return foundPkgInfo.signatures[0].toByteArray();
                }
            }
    ...
    private static String encryByteBySHA256(byte[] data) {
        try {
            return byteArrayToHexString(MessageDigest.getInstance(SHA_256.ALGORITHM_SHA256).digest(data));
    ...
    public static boolean doCheckArchiveApk(Context context, String preSign, String apkArchivePath, String pkgName) {
        ...
        if (TextUtils.isEmpty(packageName) || TextUtils.isEmpty(unInstallAppHash) || !preSign.equals(unInstallAppHash.toUpperCase(Locale.getDefault()))) {
            return false;
        }
        return true;
    }

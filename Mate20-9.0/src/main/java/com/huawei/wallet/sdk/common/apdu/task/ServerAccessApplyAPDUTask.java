...
public class ServerAccessApplyAPDUTask extends HttpConnTask<ServerAccessApplyAPDUResponse, ServerAccessApplyAPDURequest> {
    ...
    private static final String TAG = "ServerAccessApplyAPDUTask";
    ...
    public String prepareRequestStr(ServerAccessApplyAPDURequest request) {
        if (request == null || StringUtil.isEmpty(request.getIssuerId(), true) || StringUtil.isEmpty(request.getAppletAid(), true) || StringUtil.isEmpty(request.getCplc(), true) || StringUtil.isEmpty(request.getTransactionId(), true) || request.getApduList() == null || request.getApduList().isEmpty() || request.getApduCount() != request.getApduList().size() || StringUtil.isEmpty(request.getDeviceModel(), true) || StringUtil.isEmpty(request.getSeChipManuFacturer(), true)) {
            LogC.e("ServerAccessApplyAPDUTask prepareRequestStr, invalid param", false);
            return null;
        }
        ...
        LogC.d("ServerAccessApplyAPDUTask prepareRequestStr, commander= get.apdu reportRequestMessageJson= " + reportRequestMessageJson, false);
    ...
    private JSONObject reportRequestMessage(ServerAccessApplyAPDURequest request) {
        try {
            ...
            LogC.e("ServerAccessApplyAPDUTask reportRequestMessage, JSONException", false);
            return null;
        }
    }
    ...
    private JSONObject createDataStr(JSONObject headerObject, ServerAccessApplyAPDURequest request) {
        ...
        if (headerObject == null) {
            LogC.e("ServerAccessApplyAPDUTask createDataStr, invalid param", false);
            return null;
        }
        try {
            jObj = new JSONObject();
            ...
            LogC.e("ServerAccessApplyAPDUTask createDataStr, JSONException", false);
            jObj = null;
        }
        return jObj;
    }
    ...
    private void getSrcTranId(ServerAccessApplyAPDUResponse response, JSONObject dataObject) {
        if (dataObject != null) {
            try {
                ...
                LogC.e("ServerAccessApplyAPDUTask getSrcTransationId, JSONException", false);
    ...
    public void setErrorInfo(JSONObject dataObject, ServerAccessBaseResponse response) {
        if (dataObject != null && dataObject.has("errorInfo")) {
            ...
            try {
                ...
                LogC.e("setErrorInfo, JSONException", false);
                response.returnCode = -99;
            }
    ...
    private void parseAwardsInfo(ServerAccessApplyAPDUResponse response, JSONObject dataObject) {
        LogC.d("ServerAccessApplyAPDUTask parseAwardsInfo dataObject: " + dataObject.toString(), false);
        try {
            ...
            LogC.d("ServerAccessApplyAPDUTask parseAwardsInfo, no responseType, set to 1_lottery", false);
            response.responseType = "1_lottery";
        } catch (JSONException e) {
            LogC.i("ServerAccessApplyAPDUTask parseAwardsInfo,  Exception occured.", false);
        }
    }
    ...
    public ServerAccessApplyAPDUResponse readErrorResponse(int errorCode, String errorMessage) {
        ...
        LogC.d("ServerAccessApplyAPDUTask readErrorResponse, commander= get.apdu errorCode= " + errorCode, false);
        return response;
    }

    public ServerAccessApplyAPDUResponse readSuccessResponse(int returnCode, String returnDesc, JSONObject jsonObject) {
        ServerAccessApplyAPDUResponse response = new ServerAccessApplyAPDUResponse();
        response.returnCode = returnCode;
        response.setResultDesc(returnDesc);
        setErrorInfo(jsonObject, response);
        getSrcTranId(response, jsonObject);
        if (jsonObject != null && jsonObject.has("errorInfo")) {
            ErrorInfo errorInfo = null;
            try {
                errorInfo = ErrorInfo.build(jsonObject.getJSONObject("errorInfo"));
            } catch (JSONException e) {
                LogC.e("ServerAccessApplyAPDUTask readSuccessResponse, JSONException", false);
                response.returnCode = -99;
            }
            response.setErrorInfo(errorInfo);
            StringBuilder sb = this.builder;
        ...
        if (returnCode == 0) {
            ...
            try {
                ...
                if (jsonObject.has(ExecuteApduTask.NEXT_STEP)) {
                    ...
                    StringBuilder sb2 = this.builder;
                    sb2.append(" nextStep=");
                    sb2.append(JSONHelper.getStringValue(jsonObject, ExecuteApduTask.NEXT_STEP));
                }
                if (apduArr != null) {
                ...
                if (jsonObject.has("cutoverInfo")) {
                    ServerAccessCutoverInfo cutoverInfo = ServerAccessCutoverInfo.build(jsonObject.getJSONObject("cutoverInfo"));
            ...
            parseAwardsInfo(response, jsonObject);
        }
        LogC.d("ServerAccessApplyAPDUTask readSuccessResponse, commander= get.apdu returnCode= 0 returnDesc= success dataObject: " + this.builder.toString(), false);
        return response;
    }
}

    ...
    private int adjustRowNumbersFromLRI() {
        ...
        for (int codewordsRow = 0; codewordsRow < codewords.length; codewordsRow++) {
            if (codewords[codewordsRow] != null) {
                int rowIndicatorRowNumber = codewords[codewordsRow].getRowNumber();
                int invalidRowCounts = 0;
                int unadjustedCount = barcodeColumn;
                for (int barcodeColumn2 = 1; barcodeColumn2 < this.barcodeColumnCount + 1 && invalidRowCounts < 2; barcodeColumn2++) {
                    Codeword codeword = this.detectionResultColumns[barcodeColumn2].getCodewords()[codewordsRow];
                    if (codeword != null) {
                        invalidRowCounts = adjustRowNumberIfValid(rowIndicatorRowNumber, invalidRowCounts, codeword);
                        if (!codeword.hasValidRowNumber()) {
                            unadjustedCount++;

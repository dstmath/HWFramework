...
class EventScanner implements Runnable {
    ...
    public void deliverEvent(EventWrapper eventWrapper) {
        ...
        if (sipEvent instanceof RequestEvent) {
            ...
            if (this.sipStack.isLoggingEnabled()) {
                ...
                stackLogger10.logDebug("Done processing Message " + ((SIPRequest) ((RequestEvent) sipEvent).getRequest()).getFirstLine());
            }
            if (eventWrapper.transaction != null && ((SIPServerTransaction) eventWrapper.transaction).passToListener()) {
                ((SIPServerTransaction) eventWrapper.transaction).releaseSem();
            }
            ...
            try {
                ...
                if (this.sipStack.isLoggingEnabled()) {
                    ...
                    stackLogger11.logDebug("Calling listener for " + sipResponse.getFirstLine());
                }
                if (sipListener != null) {
                ...
                if (sipResponse.getCSeq().getMethod().equals("INVITE") && sipDialog != null && sipResponse.getStatusCode() == 200) {
                    if (this.sipStack.isLoggingEnabled()) {
                        ...
                        stackLogger12.logDebug("Warning! unacknowledged dialog. " + sipDialog.getState());
                    }
            ...
            try {
                if (this.sipStack.isLoggingEnabled()) {
                    ...
                    stackLogger13.logDebug("tx = " + ((TransactionTerminatedEvent) sipEvent).getClientTransaction());
                    StackLogger stackLogger14 = this.sipStack.getStackLogger();
                    stackLogger14.logDebug("tx = " + ((TransactionTerminatedEvent) sipEvent).getServerTransaction());
                }
                if (sipListener != null) {
            ...
            StackLogger stackLogger15 = this.sipStack.getStackLogger();
            stackLogger15.logFatalError("bad event" + sipEvent);
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:42:0x00a3, code lost:
        ...
        r4.logDebug("Processing " + r1 + "nevents " + r3.size());
     */

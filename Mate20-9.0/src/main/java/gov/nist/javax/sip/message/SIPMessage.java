...
public abstract class SIPMessage extends MessageObject implements Message, MessageExt {
            ...
            if (hisHeaders instanceof SIPHeaderList) {
                ...
                while (outerIterator.hasNext()) {
                    ...
                    if (!(hisHeader instanceof ContentLength)) {
                        ListIterator<SIPHeader> listIterator = myHeaders.listIterator();
                        boolean found = false;
                        while (true) {
                            if (listIterator.hasNext()) {
                                if (listIterator.next().match(hisHeader)) {
                                    found = true;
                                    break;
                                }
                            } else {
                ...
                while (true) {
                    if (innerIterator.hasNext()) {
                        if (innerIterator.next().match(hisHeader2)) {
                            found2 = true;
                            break;
                        }
                    } else {
    ...
    /* JADX WARNING: type inference failed for: r1v16, types: [javax.sip.header.Header] */
    /* JADX WARNING: Multi-variable type inference failed */
    public String getTransactionId() {
        Via topVia = null;
        if (!getViaHeaders().isEmpty()) {
            topVia = getViaHeaders().getFirst();
        }
    ...
    public void setMessageContent(String type, String subType, String messageContent2) {
        if (messageContent2 != null) {
            setHeader((Header) new ContentType(type, subType));

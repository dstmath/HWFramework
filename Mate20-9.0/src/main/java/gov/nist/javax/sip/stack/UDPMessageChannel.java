        ...
        try {
            ...
            if (sipMessage == null) {
                ...
                if (hashtable.get(packet.getAddress().getHostAddress() + Separators.COLON + packet.getPort()) == null) {
                    byte[] retval = "\r\n\r\n".getBytes();
                    DatagramPacket datagramPacket = new DatagramPacket(retval, 0, retval.length, packet.getAddress(), packet.getPort());
                    ((UDPMessageProcessor) this.messageProcessor).sock.send(datagramPacket);

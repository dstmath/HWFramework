...
public class HwAlphaIndexResourceManager {
    ...
    private HwAlphaIndexResourceManager(Locale locale) {
        ...
        if ("zh_Hant".equals(validLocale)) {
            ...
            this.labelList = getLabelList(lang + "_ALPHA_INDEX");
        }
    ...
    private List<Alpha> getLabelList(String alphaIndex) {
        String alphaIndex2 = languageIndexerMap.get(alphaIndex);
        if (alphaIndex2 == null) {
            return new ArrayList();
        }
        String[] alphaArray = alphaIndex2.split(" ");
    ...
    private Map<String, String> parseLanguageIndexerFile(InputStream file) {
        ...
        try {
            ...
            BufferedReader buffReader2 = new BufferedReader(reader2, 2048);
            int lineNum = 0;
            while (true) {
                String readLine = buffReader2.readLine();
    ...
    public static List<String> populateBulletAlphaIndex(int M, List<String> alphaList) {
        ...
        while ((minBulletCount + 1) * min < alphaList.size() - 1) {
            minBulletCount++;
        }
        int bulletCount = minBulletCount - 1;
        if ((bulletCount + 1) * min == alphaList.size() - 1) {
            bulletCount++;
        }
        int n2 = (N - 1) - ((bulletCount + 1) * min);
        int n1 = min - n2;
        List<String> ret = new ArrayList<>();
        ret.add(alphaList.get(0));
        int pos = 0 + 1;
        while (true) {
            int i = 0;
            if (n2 <= 0) {
            ...
            bulletContent.setLength(bulletContent.length() - 1);
            ret.add(bulletContent.toString());
            ret.add(alphaList.get(pos));
            pos++;
        }
        while (n1 > 0) {
            n1--;
            StringBuilder bulletContent2 = new StringBuilder();
            int pos2 = pos;
            int i2 = 0;
            while (i2 < bulletCount) {
                bulletContent2.append(alphaList.get(pos2));
                bulletContent2.append(' ');
                i2++;
                pos2++;
            }
            bulletContent2.setLength(bulletContent2.length() - 1);
            ret.add(bulletContent2.toString());
            ret.add(alphaList.get(pos2));
            pos = pos2 + 1;
        }
        for (int i3 = pos; i3 < N; i3++) {
            ret.add(alphaList.get(i3));
        }
        return ret;
    }
}

...
public class CustomGestureDetector {
    ...
    private class GesturePerformedRunnable implements Runnable {
        ...
        public void run() {
            ...
            Gesture unused = CustomGestureDetector.this.mCurrentGesture = null;
        }
    }
    ...
    private float calculateDistanceBetweenFirstAndLastPoint() {
        float[] points = this.mCurrentGesture.getStrokes().get(0).points;
        if (points.length < 4) {
    ...
    private boolean notifyIfLineGesture() {
        OrientedBoundingBox box = this.mCurrentGesture.getStrokes().get(0).computeOrientedBoundingBox();
    ...
    private boolean notifyIfLetterGesture() {
        try {
            ...
            if (predMultipleOrient == null || predSingleOrient == null) {
                ...
                Prediction bestPredMultOrient = predMultipleOrient.get(0);
                ...
                Prediction bestPredSingleOrient = predSingleOrient.get(0);
                ...
                if (predSingleOrient.size() <= 1 || !bestPredMultOrient.name.equals(predSingleOrient.get(1).name)) {
                    return false;
                }
                notifyLetterGesture(bestPredMultOrient.name, bestPredMultOrient.score);
                return true;
    ...
    /* access modifiers changed from: private */
    public void fireOnGesturePerformed(GesturePerformedRunnable gesturePerformed) {
        if (this.mCurrentGesture == null || this.mCurrentGesture.getStrokesCount() <= 0) {
            ...
            IMonitor.EventStream eStream = IMonitor.openEventStream(KnuckGestureSetting.KNOCK_GESTURE_DATA_RECORD);
            if (eStream != null && !this.mIsStylus) {
                eStream.setParam(15, 1);
                eStream.setParam(23, this.knuckGestureSetting.getTpVendorName());
                eStream.setParam(24, this.knuckGestureSetting.getAccVendorName());
                eStream.setParam(25, this.knuckGestureSetting.getLcdInfo());
                eStream.setParam(26, this.knuckGestureSetting.getOrientation());

...
public final class NumericShaper implements Serializable {
    ...
    public enum Range {
        ...
        private Range(int base2, int start2, int end2) {
            this.base = base2 - ('0' + getNumericBase());
            this.start = start2;
            this.end = end2;
        }
    ...
    private static int getKeyFromMask(int mask2) {
        int key2 = 0;
        while (key2 < NUM_KEYS && ((1 << key2) & mask2) == 0) {
            key2++;
        }
        if (key2 != NUM_KEYS && ((~(1 << key2)) & mask2) == 0) {
            return key2;
        }
    ...
    private void shapeNonContextually(char[] text, int start, int count) {
        ...
        if (this.shapingRange != null) {
            int access$700 = this.shapingRange.getDigitBase();
                    ...
                    try {
                        ...
                        if (isStrongDirectional(c)) {
                            ...
                            if (newkey != lastkey2) {
                                lastkey2 = newkey;
                                int ctxKey3 = newkey;
                                if ((this.mask & 4) != 0 && (ctxKey3 == 1 || ctxKey3 == 2)) {
                                    ctxKey3 = 2;
                                } else if ((this.mask & 2) != 0 && (ctxKey3 == 1 || ctxKey3 == 2)) {
                                    ctxKey3 = 1;
                                } else if ((this.mask & (1 << ctxKey3)) == 0) {
                                    ctxKey3 = 0;
                                }
                                char base3 = bases[ctxKey3];
                                minDigit = ctxKey3 == 16 ? '1' : '0';
                                base2 = base3;
                            }
                        }
                        i++;
    ...
    private void shapeContextually(char[] text, int start, int count, Range ctxKey) {
        ...
        for (int i = start; i < end; i++) {
            ...
            if (c >= minDigit && c <= '9') {
                ...
                Range ctxKey2 = rangeForCodePoint(c);
                if (ctxKey2 != lastKey) {
                    lastKey = ctxKey2;
                    base2 = ctxKey2.getDigitBase();

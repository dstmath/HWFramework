...
public final class Character implements Serializable, Comparable<Character> {
    ...
    public static int codePointCount(CharSequence seq, int beginIndex, int endIndex) {
        ...
        while (n2 < endIndex) {
            int i = n2 + 1;
            if (isHighSurrogate(seq.charAt(n2)) == 0 || i >= endIndex || !isLowSurrogate(seq.charAt(i))) {
                n2 = i;
            } else {
                n--;
                n2 = i + 1;
    ...
    static int codePointCountImpl(char[] a, int offset, int count) {
        ...
        while (n2 < endIndex) {
            int i = n2 + 1;
            if (isHighSurrogate(a[n2]) == 0 || i >= endIndex || !isLowSurrogate(a[i])) {
                n2 = i;
            } else {
                n--;
                n2 = i + 1;
    ...
    public static int offsetByCodePoints(CharSequence seq, int index, int codePointOffset) {
        ...
        if (codePointOffset >= 0) {
            ...
            while (x < length && i < codePointOffset) {
                int x2 = x + 1;
                if (isHighSurrogate(seq.charAt(x)) != 0 && x2 < length && isLowSurrogate(seq.charAt(x2))) {
                    x2++;
                }
                x = x2;
                i++;
        ...
        while (x3 > 0 && i2 < 0) {
            x3--;
            if (isLowSurrogate(seq.charAt(x3)) && x3 > 0 && isHighSurrogate(seq.charAt(x3 - 1))) {
                x3--;
            }
            i2++;
        }
    ...
    static int offsetByCodePointsImpl(char[] a, int start, int count, int index, int codePointOffset) {
        ...
        if (codePointOffset >= 0) {
            ...
            while (x < limit && i < codePointOffset) {
                int x2 = x + 1;
                if (isHighSurrogate(a[x]) != 0 && x2 < limit && isLowSurrogate(a[x2])) {
                    x2++;
                }
                x = x2;
                i++;
        ...
        while (x3 > start && i2 < 0) {
            x3--;
            if (isLowSurrogate(a[x3]) && x3 > start && isHighSurrogate(a[x3 - 1])) {
                x3--;
            }
            i2++;
        }

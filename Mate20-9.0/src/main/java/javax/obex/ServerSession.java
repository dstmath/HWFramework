...
public final class ServerSession extends ObexSession implements Runnable {
    ...
    private void handlePutRequest(int type) throws IOException {
        int response;
        ServerOperation serverOperation = new ServerOperation(this, this.mInput, type, this.mMaxPacketLength, this.mListener);
        ServerOperation op = serverOperation;
        try {
            ...
            if (length > 5) {
                ...
                if (request.mAuthResp != null) {
                    if (!handleAuthResp(request.mAuthResp)) {
                        code2 = ResponseCodes.OBEX_HTTP_UNAUTHORIZED;
    ...
    private void handleDisconnectRequest() throws IOException {
        int code;
        byte[] replyData;
        int code2 = ResponseCodes.OBEX_HTTP_OK;
        int totalLength = 3;
        byte[] head = null;
        HeaderSet request = new HeaderSet();
        HeaderSet reply = new HeaderSet();

...
public class SipFactory {
    ...
    public synchronized SipStack createSipStack(Properties properties) throws PeerUnavailableException {
        ...
        sipStack = this.mNameSipStackMap.get(name);
        if (sipStack == null) {
            try {
                sipStack = (SipStack) Class.forName("gov.nist." + SipStack.class.getCanonicalName() + "Impl").asSubclass(SipStack.class).getConstructor(new Class[]{Properties.class}).newInstance(new Object[]{properties});

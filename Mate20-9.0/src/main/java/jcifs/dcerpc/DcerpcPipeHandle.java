...
public class DcerpcPipeHandle extends DcerpcHandle {
    ...
    public DcerpcPipeHandle(String url, NtlmPasswordAuthentication auth) throws UnknownHostException, MalformedURLException, DcerpcException {
        ...
        this.pipe = new SmbNamedPipe((server != null ? params + "&address=" + ((String) this.binding.getOption("address")) : params).length() > 0 ? url2 + "?" + (server != null ? params + "&address=" + ((String) this.binding.getOption("address")) : params).substring(1) : url2, 27198979, auth);
    }

...
class ProcessorNamespaceAlias extends XSLTElementProcessor {
    ...
    public void startElement(StylesheetHandler handler, String uri, String localName, String rawName, Attributes attributes) throws SAXException {
        ...
        String prefix2 = na.getResultPrefix();
        if (prefix2.equals("#default")) {
            na.setResultPrefix("");
            resultNS = handler.getNamespaceForPrefix("");
            if (resultNS == null) {
                handler.error(XSLTErrorResources.ER_INVALID_NAMESPACE_URI_VALUE_FOR_RESULT_PREFIX_FOR_DEFAULT, null, null);
            }
        } else {
            resultNS = handler.getNamespaceForPrefix(prefix2);
            if (resultNS == null) {
                handler.error(XSLTErrorResources.ER_INVALID_NAMESPACE_URI_VALUE_FOR_RESULT_PREFIX, new Object[]{prefix2}, null);
            }
        }

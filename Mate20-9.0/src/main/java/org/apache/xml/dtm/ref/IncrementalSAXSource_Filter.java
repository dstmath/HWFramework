...
public class IncrementalSAXSource_Filter implements IncrementalSAXSource, ContentHandler, DTDHandler, LexicalHandler, ErrorHandler, Runnable {
    ...
    public void count_and_yield(boolean moreExpected) throws SAXException {
        if (!moreExpected) {
            this.eventcounter = 0;
        }
        int i = this.eventcounter - 1;
            ...
            try {
                ...
                if (!(inner instanceof StopException)) {
                    if (this.DEBUG) {
                        ...
                        printStream.println("Active IncrementalSAXSource_Filter UNEXPECTED SAX exception: " + inner);

...
public class FastStringBuffer {
    ...
    public FastStringBuffer(int initChunkBits, int maxChunkBits, int rebundleBits) {
        ...
        int maxChunkBits2 = initChunkBits;
        this.m_array = new char[16][];
        initChunkBits = initChunkBits > maxChunkBits2 ? maxChunkBits2 : initChunkBits;
        this.m_chunkBits = initChunkBits;
        this.m_maxChunkBits = maxChunkBits2;
    ...
    public final void append(char value) {
        ...
        if (this.m_firstFree < this.m_chunkSize) {
            ...
            char[][] newarray2 = this.m_array;
            int i2 = this.m_lastChunk + 1;
            this.m_lastChunk = i2;
            char[] chunk2 = newarray2[i2];
            if (chunk2 == null) {
    ...
    public String getOneChunkString(int startChunk, int startColumn, int length) {
        return new String(this.m_array[startChunk], startColumn, length);
    }

    /* access modifiers changed from: package-private */
    public StringBuffer getString(StringBuffer sb, int start, int length) {
        return getString(sb, start >>> this.m_chunkBits, this.m_chunkMask & start, length);
    }

    /* access modifiers changed from: package-private */
    public StringBuffer getString(StringBuffer sb, int startChunk, int startColumn, int length) {
        ...
        for (int i = startChunk; i < stopChunk; i++) {
            if (i != 0 || this.m_innerFSB == null) {
                sb.append(this.m_array[i], startColumn2, this.m_chunkSize - startColumn2);
            } else {
    ...
    public void sendSAXcharacters(ContentHandler ch, int start, int length) throws SAXException {
        ...
        if (startColumn + length >= this.m_chunkMask || this.m_innerFSB != null) {
            ...
            for (int i = startChunk; i < stopChunk; i++) {
                if (i != 0 || this.m_innerFSB == null) {
                    ch.characters(this.m_array[i], startColumn2, this.m_chunkSize - startColumn2);
                } else {
    ...
    static int sendNormalizedSAXcharacters(char[] ch, int start, int length, ContentHandler handler, int edgeTreatmentFlags) throws SAXException {
        ...
        while (currPos < limit) {
            int currPos2 = currPos;
            while (currPos2 < limit && !XMLCharacterRecognizer.isWhiteSpace(ch[currPos2])) {
                currPos2++;
            }
            if (currPos != currPos2) {
                if (seenWhitespace) {
                ...
                handler.characters(ch, currPos, currPos2 - currPos);
            }
            int currPos3 = currPos2;
            while (currPos3 < limit && XMLCharacterRecognizer.isWhiteSpace(ch[currPos3])) {
                currPos3++;
            }
            if (currPos2 != currPos3) {
                seenWhitespace = true;
            }
            currPos = currPos3;
        }

...
public final class CryptoServicesRegistrar {
    ...
    private static DHParameters toDH(DSAParameters dSAParameters) {
        DHParameters dHParameters = new DHParameters(dSAParameters.getP(), dSAParameters.getG(), dSAParameters.getQ(), chooseLowerBound(dSAParameters.getP().bitLength()), 0, null, new DHValidationParameters(dSAParameters.getValidationParameters().getSeed(), dSAParameters.getValidationParameters().getCounter()));
        return dHParameters;
    }
}

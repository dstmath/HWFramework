...
public class DSTU7564Digest implements ExtendedDigest, Memoable {
    ...
    private void subBytes(long[] jArr) {
        for (int i = 0; i < this.columns; i++) {
            ...
            jArr[i] = (((long) b) & BodyPartID.bodyIdMax) | (((long) ((S3[i3 >>> 24] << 24) | (((b2 & 255) | ((b3 & 255) << 8)) | ((b4 & 255) << Tnaf.POW_2_WIDTH)))) << 32);
        }
    }
    ...
    public int doFinal(byte[] bArr, int i) {
        ...
        long j = (((this.inputBlocks & BodyPartID.bodyIdMax) * ((long) this.blockSize)) + ((long) i2)) << 3;

    ...
    private void generateSubKeys(byte[] bArr) {
        if (bArr.length == this.KEY_LENGTH) {
            ...
            for (int i2 = 0; i2 < this.SUB_LENGTH; i2++) {
                ...
                bArr2[i2] = b;
                bArr4[i2] = b;
                byte[] bArr5 = this.subKeys[1];
                byte b2 = bArr[this.SUB_LENGTH + i2];
                bArr3[i2] = b2;
                bArr5[i2] = b2;
            }
            byte[] bArr6 = new byte[this.SUB_LENGTH];
            for (int i3 = 1; i3 < 5; i3++) {
                ...
                int i5 = 2 * i3;
                System.arraycopy(bArr2, 0, this.subKeys[i5], 0, this.SUB_LENGTH);
                System.arraycopy(bArr3, 0, this.subKeys[i5 + 1], 0, this.SUB_LENGTH);
            }
            return;
        }

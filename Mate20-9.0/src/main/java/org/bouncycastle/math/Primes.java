    ...
    private static STOutput implSTRandomPrime(Digest digest, int i, byte[] bArr) {
        ...
        if (i3 < 33) {
            ...
            do {
                ...
                inc(bArr2, 1);
                i5++;
                long extract32 = ((long) (((extract32(bArr3) ^ extract32(bArr4)) & (-1 >>> (32 - i3))) | (1 << (i3 - 1)) | 1)) & BodyPartID.bodyIdMax;
                if (isPrime32(extract32)) {
                    return new STOutput(BigInteger.valueOf(extract32), bArr2, i5);
                }
            } while (i5 <= 4 * i3);
            throw new IllegalStateException("Too many iterations in Shawe-Taylor Random_Prime Routine");
        }
        STOutput implSTRandomPrime = implSTRandomPrime(digest2, (i3 + 3) / 2, bArr2);
        ...
        int i7 = i3 - 1;
        int i8 = (i7 / i6) + 1;
        BigInteger bit = hashGen(digest2, primeSeed, i8).mod(ONE.shiftLeft(i7)).setBit(i7);
        ...
        int i10 = primeGenCounter;
        while (true) {
            if (add.bitLength() > i3) {
                bigInteger = ONE.shiftLeft(i7).subtract(ONE).divide(shiftLeft).add(ONE).shiftLeft(i4);
                add = bigInteger.multiply(prime).add(ONE);
            }
            i10 += i4;
            if (!implHasAnySmallFactors(add)) {
                BigInteger add2 = hashGen(digest2, primeSeed, i8).mod(add.subtract(THREE)).add(TWO);
                BigInteger add3 = bigInteger.add(BigInteger.valueOf((long) i9));
                BigInteger modPow = add2.modPow(add3, add);
                if (add.gcd(modPow.subtract(ONE)).equals(ONE) && modPow.modPow(prime, add).equals(ONE)) {
                    return new STOutput(add, primeSeed, i10);
                }
                bigInteger = add3;
                i2 = 4;
                i9 = 0;

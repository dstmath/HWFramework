    ...
    public static byte[] signConversion(int i, int i2, byte[] bArr) {
        if (i >= i2) {
            ...
            for (int i10 = 0; i10 < i; i10++) {
                binomial = binomial.multiply(new BigInteger(Integer.toString(i9 - i8))).divide(new BigInteger(Integer.toString(i9)));
                i9--;
                if (((byte) (bArr2[i10 >>> 3] & (1 << (i10 & 7)))) != 0) {
                    bigInteger = bigInteger.add(binomial);
                    i8--;
                    binomial = i9 == i8 ? ONE : binomial.multiply(new BigInteger(Integer.toString(i8 + 1))).divide(new BigInteger(Integer.toString(i9 - i8)));
                }
            }

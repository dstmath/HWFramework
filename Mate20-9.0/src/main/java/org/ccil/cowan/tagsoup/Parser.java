...
public class Parser extends DefaultHandler implements ScanHandler, XMLReader, LexicalHandler {
    ...
    private String expandEntities(String src) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            int dstlen = refStart + 1;
            dst[refStart] = ch;
            if (ch == '&' && refStart2 == -1) {
                refStart = dstlen;
                refStart2 = refStart;
            } else {
                if (refStart2 != -1 && !Character.isLetter(ch) && !Character.isDigit(ch) && ch != '#') {
                    if (ch == ';') {
                        int ent2 = lookupEntity(dst, refStart2, (dstlen - refStart2) - 1);
                        if (ent2 > 65535) {
                            int ent3 = ent2 - HTMLModels.M_OPTION;
                            dst[refStart2 - 1] = (char) ((ent3 >> 10) + 55296);
                            dst[refStart2] = (char) ((ent3 & 1023) + 56320);
                            dstlen = refStart2 + 1;
                        } else if (ent2 != 0) {
                            dst[refStart2 - 1] = (char) ent2;
                            dstlen = refStart2;
                        }
                        ent = -1;
                    } else {
        ...
        return new String(dst, 0, refStart);
    }
    ...
    private String makeName(char[] buff, int offset, int length) {
        ...
        while (true) {
            ...
            char length3 = buff[offset];
            if (Character.isLetter(length3) || length3 == '_') {
                start = false;
                dst.append(length3);
            } else if (Character.isDigit(length3) || length3 == '-' || length3 == '.') {
                if (start) {
                    dst.append('_');
                }
                start = false;
                dst.append(length3);
            } else if (length3 == ':' && !seenColon) {
                seenColon = true;
                if (start) {
                    dst.append('_');
                }
        ...
        if (dst.length() == 0 || dst.charAt(dstLength - 1) == ':') {
            dst.append('_');
        }

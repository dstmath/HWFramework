...
public class IFAAManagerV4Impl extends IFAAManagerV4 {
    ...
    public int getSupportBIOTypes(Context context) {
        ...
        try {
            mFaceAbility = this.mFaceManager.getFaceRecognitionAbility();
    ...
    public int[] getIDList(int type) {
        Log.i(LOG_TAG, "getIDList enter");
        if (!isBioTypeOK(type)) {
            Log.e(LOG_TAG, "unsupport type");
            return new int[0];
        }
        int[] result = getBiometricsID(type, 0);
        StringBuilder sb = new StringBuilder();
        sb.append("id list 0 is ");
        sb.append(result.length > 0 ? Integer.valueOf(result[0]) : "not exisrt");
    ...
    private int[] getBiometricsID(int type, int userID) {
        ...
        for (int i = 0; i < fingerprintCount; i++) {
            result[i] = items.get(i).getFingerId();
        }
        return result;
    }

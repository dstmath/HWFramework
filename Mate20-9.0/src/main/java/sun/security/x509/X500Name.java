        ...
        while (true) {
            if (nextComma >= 0 || nextSemiColon >= 0) {
                ...
                if (rdnEnd >= 0 && quoteCount != 1 && !escaped(rdnEnd, searchOffset, dnString)) {
                    dnVector.add(new RDN(dnString.substring(dnOffset, rdnEnd), keywordMap));
                    dnOffset = rdnEnd + 1;
                    quoteCount = 0;
                }
                ...
                dnVector.add(new RDN(dnString.substring(dnOffset), keywordMap));
        ...
        try {
            Object[] result = (Object[]) AccessController.doPrivileged(new PrivilegedExceptionAction<Object[]>() {
                public Object[] run() throws Exception {
                    ...
                    Constructor<X500Principal> cons = pClass.getDeclaredConstructor(X500Name.class);

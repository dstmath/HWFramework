...
public class HwSplitUtils {
    ...
    private static final String TAG = "HwSplitUtils";
    ...
    public static boolean isNeedSplit(Context context) {
        ...
        double[] splitSizeList = activity.getIntent().getDoubleArrayExtra(EXTRAS_HWSPLIT_SIZE);
        if (splitSizeList == null || splitSizeList.length < 2) {
            landSplitLimit = 5.5d;
            portSplitLimit = 8.0d;
        } else {
            landSplitLimit = splitSizeList[0];
            portSplitLimit = splitSizeList[1];
        }
    ...
    private static boolean isFullState() {
        ...
        try {
            ...
            Log.i(TAG, "getDisplayMode : " + displayMode);
        } catch (RuntimeException e) {
            Log.e(TAG, "fail to getDisplayMode : " + e.getMessage());
        }
        if (displayMode != 0) {

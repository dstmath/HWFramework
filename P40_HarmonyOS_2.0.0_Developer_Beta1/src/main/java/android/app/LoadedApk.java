        ...
        if (SystemProperties.getBoolean("dalvik.vm.usejitprofiles", false)) {
            ...
            if (this.mApplicationInfo.uid == Process.myUid()) {
                ...
                if (!codePaths.isEmpty()) {
                    ...
                    if ((this.mApplicationInfo.hwFlags & 1048576) == 1048576) {
                        String baseAppPath = new File(this.mApplicationInfo.sourceDir).getParent();
                        String[] hapPaths = new File(baseAppPath).list($$Lambda$LoadedApk$jGn233eZatkUEuAq0uN9zqwZCY.INSTANCE);
                        if (!(hapPaths == null || hapPaths.length == 0)) {
                            for (String hapPath : hapPaths) {
                                String profileFile = ArtManager.getCurrentHarmonyProfilePath(this.mPackageName, UserHandle.myUserId(), hapPath);
                                if (profileFile != null) {
                                    VMRuntime.registerAppInfo(profileFile, new String[]{baseAppPath + '/' + hapPath});
                                }
                            }
                        }
                    }

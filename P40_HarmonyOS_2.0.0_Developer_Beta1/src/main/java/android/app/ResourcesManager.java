...
public class ResourcesManager {
        ...
        try {
            ...
            synchronized (this) {
                ...
                if (!Objects.equals(activityResources.overrideConfig, overrideConfig) || movedToDifferentDisplay) {
                    ...
                    int refCount = activityResources.activityResources.size();
                    for (int i = 0; i < refCount; i++) {
                        Resources resources = activityResources.activityResources.get(i).get();
                        if (resources != null) {
    ...
    public final boolean applyConfigurationToResourcesLocked(Configuration config, CompatibilityInfo compat) {
        ...
        try {
            ...
            if (ActivityThread.DEBUG_CONFIGURATION) {
                Slog.v(TAG, "Skipping new config: curSeq=" + this.mResConfiguration.seq + ", newSeq=" + config.seq + ", res = " + this.mResConfiguration + ", config = " + config);
            }
            Trace.traceEnd(8192);
            return false;
        } finally {
    ...
    /* access modifiers changed from: package-private */
    @UnsupportedAppUsage
    public final void applyConfigurationToAppResourcesLocked(int displayId, Configuration config) {
        ...
        try {
            ...
            if (!(app == null || config == null || config.windowConfiguration.getAppBounds() == null || (currentAppConfig = app.getResources().getConfiguration()) == null || currentAppConfig.windowConfiguration.getAppBounds() == null)) {
                ...
                if ((currentAppConfig.windowConfiguration.getAppBounds().equals(config.windowConfiguration.getAppBounds()) && app.getDisplayId() == displayId) || !HwActivityTaskManager.IS_HW_MULTIWINDOW_APPCOMPACT_SUPPORTED) {
                    return;
                }
                if (ActivityThread.isAdjustConfig(currentAppConfig) || ActivityThread.isAdjustConfig(config) || ActivityThread.isAdjustDisplays(app.getDisplayId(), displayId)) {
                    Slog.i(TAG, "applyConfigurationToAppResourcesLocked newConfig: " + config + ", currentAppConfig: " + currentAppConfig);
                    this.mAdjustedDisplays.clear();
                    ...
                    if (app.getDisplayId() != displayId) {
                        ...
                        if (displayId == 0) {
                            ...
                            metrics = getDisplayMetrics(displayId, app.getResources().getDisplayAdjustments());
                        }
                    }
                    this.mResConfiguration.setMultiWindowConfigTo(currentAppConfig);
                    ...
                    for (int i = 0; i < refCount; i++) {
                        ...
                        if (resources != null) {
                            resources.updateConfiguration(currentAppConfig, metrics, null);
                        }
                    }
                }
            }
        } catch (IndexOutOfBoundsException e) {
            Slog.i(TAG, "applyConfigurationToAppResourcesLocked get fail");
        } catch (Exception e2) {

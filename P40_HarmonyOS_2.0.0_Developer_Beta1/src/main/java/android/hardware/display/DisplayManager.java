...
public final class DisplayManager {
    ...
    private boolean isSpecialPackageForFilter(String packageName) {
        return "com.bankid.bus".equals(packageName) || "com.playstation.tornemobile".equals(packageName) || "com.fami_geki.video".equals(packageName) || "com.citrix.Receiver".equals(packageName) || "com.android.chrome".equals(packageName);
    }
    ...
    private Display[] filterDisplaysForPCMode(Display[] displays) {
        ...
        boolean isEnabledInPad = HwPCUtils.enabledInPad();
        for (int i = 0; i < size; i++) {
            if (isEnabledInPad) {
                if (!HwPCUtils.PAD_PC_DISPLAY_NAME.equals(displays[i].getName())) {
                    displayList.add(displays[i]);
                }
        ...
        return (Display[]) displayList.toArray(new Display[0]);
    }

    ...
    private KeyphraseMetadata getKeyphraseMetadataFromApplicationInfo(PackageManager pm, ApplicationInfo ai, List<String> parseErrors) {
        ...
        try {
            ...
            try {
                ...
                parseErrors.add(error3 + ": " + e2);
                Slog.w(TAG, error3, e2);
                if (parser != null) {
                    parser.close();
                }
                ...
                try {
                    ...
                    parseErrors.add(error4 + ": " + e3);
                    Slog.w(TAG, error4, e3);
                    if (parser != null) {
                        parser.close();
                    }
                    return keyphraseMetadata;
            ...
            parseErrors.add(error32 + ": " + e2);
            Slog.w(TAG, error32, e2);
            if (parser != null) {
            }
            return keyphraseMetadata;
            ...
            parseErrors.add(error42 + ": " + e3);
            Slog.w(TAG, error42, e3);
            if (parser != null) {
            }
            return keyphraseMetadata;
        } catch (Throwable th3) {

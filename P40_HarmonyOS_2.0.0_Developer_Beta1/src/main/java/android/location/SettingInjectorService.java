...
public abstract class SettingInjectorService extends Service {
    ...
    private void sendStatus(Intent intent, String summary, boolean enabled) {
        ...
        try {
            Object msg = intent.getParcelableExtra("messenger");
            if (msg instanceof Messenger) {
                messenger = (Messenger) msg;
            }
        } catch (BadParcelableException e) {
            Log.e(TAG, this.mName + ": sendStatus BadParcelableException");
        }
        if (messenger != null) {
            ...
            try {
                ...
                Log.e(TAG, this.mName + ": sending dynamic status failed", e2);
            }
        }
    }

...
public class HeifUtils {
            ...
            if (!heifFile.isFile() || !heifFile.exists()) {
                ...
                if (option.outHeight > 65535 || option.outWidth > 65535) {
                    ...
                    try {
                        ...
                        OutputStream jpgStream2 = new ExternalStorageFileOutputStreamImpl(jpgFile);
                        boolean hardDecodingResult = false;
                        if (supportHeifHardDecoding()) {
                            hardDecodingResult = BitmapFactory.convertHeifToJpeg(heifPath, jpgStream2);
                        }
                        if (!hardDecodingResult) {
                            ...
                            bm.compress(Bitmap.CompressFormat.JPEG, 90, jpgStream2);
                        }
                        try {
                            jpgStream2.flush();
                            jpgStream2.close();
    ...
    private static boolean supportHeifHardDecoding() {
        ...
        if (platform != null && (platform.startsWith("kirin980") || platform.startsWith("kirin990"))) {
            return true;
        }
        if (hardware != null && (hardware.startsWith("kirin980") || hardware.startsWith("kirin990"))) {
            return true;
        }
        if (hardware != null && (hardware.startsWith("mt6889") || platform.startsWith("mt6885"))) {
            return true;
        }
        if (hardware == null) {
            return false;
        }
        if (hardware.startsWith("mt6853") || platform.startsWith("mt6853")) {
            return true;
        }
        return false;
    }

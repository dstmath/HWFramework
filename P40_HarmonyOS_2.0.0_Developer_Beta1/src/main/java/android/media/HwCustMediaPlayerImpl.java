...
public class HwCustMediaPlayerImpl extends HwCustMediaPlayer {
    private static final boolean IS_HWFLOW = ((Log.HWINFO || (Log.HWModuleLog && Log.isLoggable(TAG, 4))) ? IS_HWLOGW_E : IS_HWFLOW);
    ...
    public class URLConvertThread implements Runnable {
        ...
        private URLConvertThread() {
            this.ipAddr = "ERROR";
            this.isStop = HwCustMediaPlayerImpl.IS_HWFLOW;
        }
        ...
        public void run() {
            ...
            try {
                ...
                this.isStop = HwCustMediaPlayerImpl.IS_HWLOGW_E;
                throw th;
            }
            this.isStop = HwCustMediaPlayerImpl.IS_HWLOGW_E;
        }
    ...
    public Map<String, String> setStreamingMediaHeaders(Context context, Uri uri, Map<String, String> headers) {
        if (!((!"237".equals(SystemProperties.get("ro.config.hw_opta", "0")) || !"840".equals(SystemProperties.get("ro.config.hw_optb", "0"))) ? IS_HWFLOW : IS_HWLOGW_E)) {
            return headers;
        }
        if (headers == null) {
            headers = new HashMap();

    ...
    public synchronized void add(AudioStream stream) {
        if (!this.mStreams.containsKey(stream)) {
            try {
                ...
                this.mStreams.put(stream, Long.valueOf(nativeAdd(stream.getMode(), stream.getSocket(), stream.getRemoteAddress().getHostAddress(), stream.getRemotePort(), String.format(Locale.US, "%d %s %s", Integer.valueOf(codec.type), codec.rtpmap, codec.fmtp), stream.getDtmfType(), ActivityThread.currentOpPackageName())));

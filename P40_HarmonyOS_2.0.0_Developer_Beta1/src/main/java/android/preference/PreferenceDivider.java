...
public class PreferenceDivider extends PreferenceCategory {
    ...
    public PreferenceDivider(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        if (theme != null) {
            TypedArray array = theme.obtainStyledAttributes(attrs, ResLoader.getInstance().getIdentifierArray(context, ResLoaderUtil.STAYLEABLE, "DividerPreference"), defStyleAttr, defStyleRes);
            this.mDefHeight = array.getResources().getDimensionPixelSize(ResLoader.getInstance().getIdentifier(context, ResLoaderUtil.DIMEN, "emui_dividerPreference_default_height"));
            this.mMore = array.getString(ResLoader.getInstance().getIdentifier(context, ResLoaderUtil.STAYLEABLE, "DividerPreference_more"));
            this.mDividerHeight = array.getDimensionPixelSize(ResLoader.getInstance().getIdentifier(context, ResLoaderUtil.STAYLEABLE, "DividerPreference_dividerHeight"), this.mDefHeight);
            array.recycle();
        }
    }

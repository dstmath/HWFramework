...
public class TrustAgentService extends Service {
    ...
    public final void grantTrustInNewAuth(final CharSequence message, final long durationMs, final int flags, final boolean isNewAuth) {
        synchronized (this.mLock) {
            if (!this.mManagingTrust) {
                throw new IllegalStateException("Cannot grant trust if agent is not managing trust. Call setManagingTrust(true) first.");
            } else if (this.mCallback != null) {
                try {
                    this.mCallback.grantTrustInNewAuth(message.toString(), durationMs, flags, isNewAuth);
                } catch (RemoteException e) {
                    onError("calling grantTrustInNewAuth()");
                }
            } else {
                this.mPendingGrantTrustTask = new Runnable() {
                    /* class android.service.trust.TrustAgentService.AnonymousClass3 */

                    @Override // java.lang.Runnable
                    public void run() {
                        TrustAgentService.this.grantTrustInNewAuth(message, durationMs, flags, isNewAuth);
                    }
                };
            }
        }
    ...
    public final void revokeTrustWithPara(boolean isFeatureAndScreenOff) {
        synchronized (this.mLock) {
            if (this.mPendingGrantTrustTask != null) {
                this.mPendingGrantTrustTask = null;
            }
            if (this.mCallback != null) {
                try {
                    this.mCallback.revokeTrustWithPara(isFeatureAndScreenOff);
                } catch (RemoteException e) {
                    onError("calling revokeTrustWithPara");
                }
            }
        }
    }

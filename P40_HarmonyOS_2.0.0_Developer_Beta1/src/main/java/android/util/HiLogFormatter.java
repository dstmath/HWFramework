...
public final class HiLogFormatter implements Closeable, Flushable {
    ...
    private static final Appendable nonNullAppendable(Appendable appendable2) {
        if (appendable2 == null) {
    ...
    private HiLogFormatter(Locale loc2, Appendable appendable2) {
        this.appendable = appendable2;
        this.loc = loc2;
        this.zero = getZero(loc2);
    }

    private HiLogFormatter(Charset charset, Locale loc2, File file) throws FileNotFoundException {
        this(loc2, new BufferedWriter(new OutputStreamWriter(new FileOutputStream(file), charset)));
    }
    ...
    public HiLogFormatter(Appendable appendable2) {
        this(Locale.getDefault(Locale.Category.FORMAT), nonNullAppendable(appendable2));
    }

    public HiLogFormatter(Locale loc2) {
        this(loc2, new StringBuilder());
    }

    public HiLogFormatter(Appendable appendable2, Locale loc2) {
        this(loc2, nonNullAppendable(appendable2));
    }
    ...
    public HiLogFormatter(String fileName, String csn, Locale loc2) throws FileNotFoundException, UnsupportedEncodingException {
        this(toCharset(csn), loc2, new File(fileName));
    }
    ...
    public HiLogFormatter(File file, String csn, Locale loc2) throws FileNotFoundException, UnsupportedEncodingException {
        this(toCharset(csn), loc2, file);
    }
    ...
    public HiLogFormatter(OutputStream os, String csn, Locale loc2) throws UnsupportedEncodingException {
        this(loc2, new BufferedWriter(new OutputStreamWriter(os, csn)));
    }

    private static char getZero(Locale loc2) {
        if (loc2 == null || loc2.equals(Locale.US)) {
            return '0';
        }
        return DecimalFormatSymbols.getInstance(loc2).getZeroDigit();
    }
    ...
    public HiLogFormatter format(boolean showPrivacy, String format, Object... args) {
        return format(this.loc, showPrivacy, format, args);
    }

    public HiLogFormatter format(Locale loc2, boolean showPrivacy, String format, Object... args) {
        ensureOpen();
        int last = -1;
        int lasto = -1;
        ...
        for (FormatString fs : fsa) {
            ...
            if (index == -2) {
                ...
                if (index != 0) {
                    ...
                    formatSpecifier.print(obj, loc2, showPrivacy);
                } else {
                    lasto++;
                    last = lasto;
                    if (args != null) {
                    ...
                    formatSpecifier2.print(obj, loc2, showPrivacy);
                }
                ...
                formatSpecifier3.print(obj, loc2, showPrivacy);
            }
        }
        return this;
    }
    ...
    public class FixedString implements FormatString {
        ...
        @Override // android.util.HiLogFormatter.FormatString
        public void print(Object arg, Locale loc) throws IOException {
            HiLogFormatter.this.appendable.append(this.s);
        }
    ...
    public class FormatSpecifier implements FormatString {
        ...
        public void print(Object arg, Locale loc, boolean showPrivacy) throws IOException {
            if (showPrivacy || !this.privacy) {
                print(arg, loc);
            } else {
                printString(HiLogString.PRIVATE_STRING, loc);
            }
        }
        ...
        public void print(Object arg, Locale loc) throws IOException {
            ...
            if (c2 != '%') {
                if (c2 != 'C') {
                    if (c2 != 's') {
                        if (c2 != 'x') {
                            if (c2 == 'n') {
                                HiLogFormatter.this.appendable.append(System.lineSeparator());
                                return;
                            } else if (c2 != 'o') {
                                switch (c2) {
                                    case 'a':
            ...
            HiLogFormatter.this.appendable.append('%');
        }

        private void printInteger(Object arg, Locale loc) throws IOException {
            if (arg == null) {
                print("null");
            } else if (arg instanceof Byte) {
                print(((Byte) arg).byteValue(), loc);
            } else if (arg instanceof Short) {
                print(((Short) arg).shortValue(), loc);
            } else if (arg instanceof Integer) {
                print(((Integer) arg).intValue(), loc);
            } else if (arg instanceof Long) {
                print(((Long) arg).longValue(), loc);
            } else if (arg instanceof BigInteger) {
                print((BigInteger) arg, loc);
            } else {
                failConversion(this.c, arg);
            }
        }

        private void printFloat(Object arg, Locale loc) throws IOException {
            if (arg == null) {
                print("null");
            } else if (arg instanceof Float) {
                print(((Float) arg).floatValue(), loc);
            } else if (arg instanceof Double) {
                print(((Double) arg).doubleValue(), loc);
            } else if (arg instanceof BigDecimal) {
                print((BigDecimal) arg, loc);
            } else {
                failConversion(this.c, arg);
            }
        }

        private void printDateTime(Object arg, Locale loc) throws IOException {
            if (arg == null) {
                print("null");
                return;
            }
            ...
            if (arg instanceof Long) {
                cal = Calendar.getInstance(loc == null ? Locale.US : loc);
                ...
                print((TemporalAccessor) arg, this.c, loc);
                return;
            } else {
                failConversion(this.c, arg);
            }
            print(cal, this.c, loc);
        }
        ...
        private void printString(Object arg, Locale loc) throws IOException {
        ...
        private void printHashCode(Object arg) throws IOException {
            print(arg == null ? "null" : Integer.toHexString(arg.hashCode()));
        }
        ...
        private void print(String s) throws IOException {
            ...
            if (this.f.contains(Flags.UPPERCASE)) {
                s = s.toUpperCase(HiLogFormatter.this.loc != null ? HiLogFormatter.this.loc : Locale.getDefault());
            }
            HiLogFormatter.this.appendable.append(justify(s));
        }
        ...
        private void print(byte value, Locale loc) throws IOException {
            char c2;
            long v = (long) value;
        ...
        private void print(short value, Locale loc) throws IOException {
            char c2;
            long v = (long) value;
            if (value < 0 && ((c2 = this.c) == 'o' || c2 == 'x')) {
                v += 65536;
        ...
        private void print(int value, Locale loc) throws IOException {
            char c2;
            long v = (long) value;
            if (value < 0 && ((c2 = this.c) == 'o' || c2 == 'x')) {
                v += 4294967296L;
        ...
        private void print(long value, Locale loc) throws IOException {
            char[] va;
            StringBuilder sb = new StringBuilder();
            char c2 = this.c;
            boolean neg = false;
            if (c2 == 'd') {
                ...
                leadingSign(sb, neg);
                Flags flags = this.f;
                localizedMagnitude(sb, va, flags, adjustWidth(this.width, flags, neg), loc);
                trailingSign(sb, neg);
            } else if (c2 == 'o') {
                ...
                int len2 = this.f.contains(Flags.ALTERNATE) ? s2.length() + 2 : s2.length();
                if (this.f.contains(Flags.ALTERNATE)) {
            ...
            HiLogFormatter.this.appendable.append(justify(sb.toString()));
        }
        ...
        private void print(BigInteger value, Locale loc) throws IOException {
            StringBuilder sb = new StringBuilder();
            boolean z = false;
        ...
        private void print(float value, Locale loc) throws IOException {
            print((double) value, loc);
        }

        private void print(double value, Locale loc) throws IOException {
            StringBuilder sb = new StringBuilder();
            ...
            if (!Double.isNaN(value)) {
                ...
                if (!Double.isInfinite(v)) {
                    print(sb, v, loc, this.f, this.c, this.precision, neg);
                } else {
        ...
        private void print(StringBuilder sb, double value, Locale loc, Flags f2, char c2, int precision2, boolean neg) throws IOException {
        ...
        private String hexDouble(double d, int prec) {
            ...
            long newSign = (Long.MAX_VALUE & doppel) >> shiftDistance;
            long roundingBits = (~(-1 << shiftDistance)) & doppel;
            boolean leastZero = (newSign & 1) == 0;
            boolean round = ((1 << (shiftDistance + -1)) & roundingBits) != 0;
            boolean sticky = shiftDistance > 1 && ((~(1 << (shiftDistance + -1))) & roundingBits) != 0;
            if ((leastZero && round && sticky) || (!leastZero && round)) {
                newSign++;
            }
            double result = Double.longBitsToDouble((Long.MIN_VALUE & doppel) | (newSign << shiftDistance));
        ...
        private void print(BigDecimal value, Locale loc) throws IOException {
            char c2 = this.c;
            if (c2 == 'a') {
        ...
        private void print(StringBuilder sb, BigDecimal value, Locale loc, Flags f2, char c2, int precision2, boolean neg) throws IOException {
            int prec;
            int compPrec;
            int newW;
            ...
            if (c2 == 'e') {
                ...
                localizedMagnitude(sb, mant2, f2, newW, loc);
                ...
                sb.append((CharSequence) localizedMagnitude((StringBuilder) null, tmp, flags, -1, loc));
                return;
            }
            int nzeros2 = 0;
            if (c2 == 'f') {
                ...
                localizedMagnitude(sb, trailingZeros(mant3, nzeros2), f2, adjustWidth(this.width, f2, neg), loc);
            } else if (c2 == 'g') {
                if (precision2 == -1) {
                    prec = 6;
                ...
                if (!value2.equals(BigDecimal.ZERO)) {
                    if (value2.compareTo(tenToTheNegFour) == -1 || value2.compareTo(tenToThePrec) != -1) {
                        print(sb, value, loc, f2, 'e', prec - 1, neg);
                        return;
                    }
                }
                print(sb, value, loc, f2, 'f', (prec - ((-value.scale()) + (value.unscaledValue().toString().length() - 1))) - 1, neg);
            } else {
                if (c2 == 'a') {
                }
            }
                    ...
                    if (len > 1) {
                        ...
                        if (form == BigDecimalLayoutForm.SCIENTIFIC) {
                            ...
                            if (len < 10) {
                                ...
                                sb.append(HwSettings.System.FINGERSENSE_KNUCKLE_GESTURE_OFF);
                                sb.append(len - 1);
                                return;
                            }
        ...
        private void print(Calendar t, char c2, Locale loc) throws IOException {
            StringBuilder sb = new StringBuilder();
            print(sb, t, c2, loc);
            ...
            HiLogFormatter.this.appendable.append(s);
        }

        /* JADX WARNING: Removed duplicated region for block: B:48:0x0183  */
        private Appendable print(StringBuilder sb, Calendar t, char c2, Locale loc) throws IOException {
            StringBuilder sb2;
            int i;
            int size;
            int i2;
            ...
            if (c2 != 'F') {
                if (c2 != 'h') {
                    if (c2 != 'p') {
                        ...
                        if (!(c2 == 'H' || c2 == 'I')) {
                            ...
                            if (c2 != 'Y') {
                                if (c2 == 'Z') {
                                    ...
                                    sb2.append(tz.getDisplayName(neg, 0, loc == null ? Locale.US : loc));
                                } else if (c2 == 'r') {
                                    print(sb2, t, 'I', loc).append(':');
                                    print(sb2, t, 'M', loc).append(':');
                                    print(sb2, t, 'S', loc).append(' ');
                                    StringBuilder tsb = new StringBuilder();
                                    print(tsb, t, 'p', loc);
                                    sb2.append(tsb.toString().toUpperCase(loc != null ? loc : Locale.US));
                                } else if (c2 == 's') {
                                    sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, t.getTimeInMillis() / 1000, Flags.NONE, this.width, loc));
                                } else if (c2 != 'y') {
                                    if (c2 != 'z') {
                                        switch (c2) {
                                            case 'A':
                                                int i5 = t.get(7);
                                                DateFormatSymbols dfs = DateFormatSymbols.getInstance(loc == null ? Locale.US : loc);
                                                if (c2 != 'A') {
                                            ...
                                            case HwLogExceptionInner.LEVEL_D /* 68 */:
                                                print(sb2, t, 'm', loc).append('/');
                                                print(sb2, t, 'd', loc).append('/');
                                                print(sb2, t, 'y', loc);
                                                break;
                                            default:
                                                switch (c2) {
                                                    case 'L':
                                                        sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, (long) t.get(14), Flags.ZERO_PAD, 3, loc));
                                                        break;
                                                    case 'M':
                                                        sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, (long) t.get(12), Flags.ZERO_PAD, 2, loc));
                                                        break;
                                                    case 'N':
                                                        sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, (long) (t.get(14) * 1000000), Flags.ZERO_PAD, 9, loc));
                                                        break;
                                                    default:
                                                        switch (c2) {
                                                            case 'Q':
                                                                sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, t.getTimeInMillis(), Flags.NONE, this.width, loc));
                                                                break;
                                                            case 'R':
                                                            case 'T':
                                                                print(sb2, t, 'H', loc).append(':');
                                                                print(sb2, t, 'M', loc);
                                                                if (c2 == 'T') {
                                                                    sb2.append(':');
                                                                    print(sb2, t, 'S', loc);
                                                                ...
                                                                break;
                                                            case 'S':
                                                                sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, (long) t.get(13), Flags.ZERO_PAD, 2, loc));
                                                                break;
                                                            default:
                                                                switch (c2) {
                                                                    case 'a':
                                                                    ...
                                                                    case 'b':
                                                                        break;
                                                                    case 'c':
                                                                        print(sb2, t, 'a', loc).append(' ');
                                                                        print(sb2, t, 'b', loc).append(' ');
                                                                        print(sb2, t, 'd', loc).append(' ');
                                                                        print(sb2, t, 'T', loc).append(' ');
                                                                        print(sb2, t, 'Z', loc).append(' ');
                                                                        print(sb2, t, 'Y', loc);
                                                                        break;
                                                                    case 'd':
                                                                    case 'e':
                                                                        sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, (long) t.get(5), c2 == 'd' ? Flags.ZERO_PAD : Flags.NONE, 2, loc));
                                                                        break;
                                                                    default:
                                                                        switch (c2) {
                                                                            case 'j':
                                                                                sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, (long) t.get(6), Flags.ZERO_PAD, 3, loc));
                                                                                break;
                                                                            case 'm':
                                                                                sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, (long) (t.get(2) + 1), Flags.ZERO_PAD, 2, loc));
                                                                                break;
                                                                        }
                                                                }
                                                        }
                                        ...
                                        sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, (long) (((min / 60) * 100) + (min % 60)), Flags.ZERO_PAD, 4, loc));
                                    }
                                }
                            }
                            int i7 = t.get(1);
                            ...
                            sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, (long) i2, Flags.ZERO_PAD, size, loc));
                        }
                        int i8 = t.get(11);
                        if (c2 == 'I' || c2 == 'l') {
                            if (!(i8 == 0 || i8 == 12)) {
                        ...
                        sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, (long) i, (c2 == 'H' || c2 == 'I') ? Flags.ZERO_PAD : Flags.NONE, 2, loc));
                    } else {
                        String[] ampm = {"AM", "PM"};
                        if (!(loc == null || loc == Locale.US)) {
                            ampm = DateFormatSymbols.getInstance(loc).getAmPmStrings();
                        }
                        sb2.append(ampm[t.get(9)].toLowerCase(loc != null ? loc : Locale.US));
                    }
                }
                int i9 = t.get(2);
                DateFormatSymbols dfs2 = DateFormatSymbols.getInstance(loc == null ? Locale.US : loc);
                if (c2 == 'B') {
        ...
        private void print(TemporalAccessor t, char c2, Locale loc) throws IOException {
            StringBuilder sb = new StringBuilder();
            print(sb, t, c2, loc);
            String s = justify(sb.toString());
            if (this.f.contains(Flags.UPPERCASE)) {
        ...
        /* JADX WARNING: Removed duplicated region for block: B:51:0x01bb A[Catch:{ DateTimeException -> 0x01e8 }] */
        private Appendable print(StringBuilder sb, TemporalAccessor t, char c2, Locale loc) throws IOException {
            StringBuilder sb2;
            int size;
            if (sb == null) {
                sb2 = new StringBuilder();
            ...
            if (c2 != 'F') {
                ...
                if (c2 != 'h') {
                    if (c2 == 'p') {
                        ...
                        sb2.append(ampm[t.get(ChronoField.AMPM_OF_DAY)].toLowerCase(loc != null ? loc : Locale.US));
                    } else if (c2 == 'H') {
                        sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, (long) t.get(ChronoField.HOUR_OF_DAY), Flags.ZERO_PAD, 2, loc));
                    } else if (c2 != 'I') {
                        if (c2 != 'Y') {
                            if (c2 == 'Z') {
                                ...
                                if (zid == null) {
                                    ...
                                    sb2.append(TimeZone.getTimeZone(zid.getId()).getDisplayName(zid.getRules().isDaylightSavings(Instant.from(t)), 0, loc == null ? Locale.US : loc));
                                }
                            } else if (c2 == 'r') {
                                print(sb2, t, 'I', loc).append(':');
                                print(sb2, t, 'M', loc).append(':');
                                ...
                                sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, t.getLong(ChronoField.INSTANT_SECONDS), Flags.NONE, this.width, loc));
                            } else if (c2 != 'y') {
                                if (c2 != 'z') {
                                    switch (c2) {
                                        case 'A':
                                        ...
                                        default:
                                            switch (c2) {
                                                case 'L':
                                                    sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, (long) t.get(ChronoField.MILLI_OF_SECOND), Flags.ZERO_PAD, 3, loc));
                                                    break;
                                                case 'M':
                                                    sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, (long) t.get(ChronoField.MINUTE_OF_HOUR), Flags.ZERO_PAD, 2, loc));
                                                    break;
                                                case 'N':
                                                    sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, (long) (t.get(ChronoField.MILLI_OF_SECOND) * 1000000), Flags.ZERO_PAD, 9, loc));
                                                    break;
                                                default:
                                                    switch (c2) {
                                                        case 'Q':
                                                            sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, (t.getLong(ChronoField.INSTANT_SECONDS) * 1000) + t.getLong(ChronoField.MILLI_OF_SECOND), Flags.NONE, this.width, loc));
                                                            break;
                                                        case 'R':
                                                        case 'T':
                                                            print(sb2, t, 'H', loc).append(':');
                                                            ...
                                                            break;
                                                        case 'S':
                                                            sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, (long) t.get(ChronoField.SECOND_OF_MINUTE), Flags.ZERO_PAD, 2, loc));
                                                            break;
                                                        default:
                                                            switch (c2) {
                                                                case 'a':
                                                                ...
                                                                case 'd':
                                                                case 'e':
                                                                    sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, (long) t.get(ChronoField.DAY_OF_MONTH), c2 == 'd' ? Flags.ZERO_PAD : Flags.NONE, 2, loc));
                                                                    break;
                                                                default:
                                                                    switch (c2) {
                                                                        case 'j':
                                                                            sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, (long) t.get(ChronoField.DAY_OF_YEAR), Flags.ZERO_PAD, 3, loc));
                                                                            break;
                                                                        case 'k':
                                                                            sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, (long) t.get(ChronoField.HOUR_OF_DAY), Flags.NONE, 2, loc));
                                                                            break;
                                                                        case 'l':
                                                                            sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, (long) t.get(ChronoField.CLOCK_HOUR_OF_AMPM), Flags.NONE, 2, loc));
                                                                            break;
                                                                        case 'm':
                                                                            try {
                                                                                sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, (long) t.get(ChronoField.MONTH_OF_YEAR), Flags.ZERO_PAD, 2, loc));
                                                                                break;
                                    ...
                                    int i2 = t.get(ChronoField.OFFSET_SECONDS);
                                    if (i2 >= 0) {
                                        neg = false;
                                    }
                                    sb2.append(neg ? '-' : '+');
                        ...
                        sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, (long) i3, Flags.ZERO_PAD, size, loc));
                    } else {
                        sb2.append((CharSequence) localizedMagnitude((StringBuilder) null, (long) t.get(ChronoField.CLOCK_HOUR_OF_AMPM), Flags.ZERO_PAD, 2, loc));
                    }
                }
                int i4 = t.get(ChronoField.MONTH_OF_YEAR) - 1;
                DateFormatSymbols dfs2 = DateFormatSymbols.getInstance(loc == null ? Locale.US : loc);
                if (c2 == 'B') {
                    sb2.append(dfs2.getMonths()[i4]);
                } else {
                    sb2.append(dfs2.getShortMonths()[i4]);
                }
            } else {
                print(sb2, t, 'Y', loc).append('-');
                print(sb2, t, 'm', loc).append('-');
        ...
        private char getZero(Locale loc) {
            if (loc == null || loc.equals(HiLogFormatter.this.locale())) {
                return HiLogFormatter.this.zero;
            }
            return DecimalFormatSymbols.getInstance(loc).getZeroDigit();
        }

        private StringBuilder localizedMagnitude(StringBuilder sb, long value, Flags f2, int width2, Locale loc) {
            return localizedMagnitude(sb, Long.toString(value, 10).toCharArray(), f2, width2, loc);
        }

        private StringBuilder localizedMagnitude(StringBuilder sb, char[] value, Flags f2, int width2, Locale loc) {
            StringBuilder sb2;
            char[] cArr = value;
            if (sb == null) {
                sb2 = new StringBuilder();
            ...
            if (dot < len) {
                if (loc == null || loc.equals(Locale.US)) {
                    decSep = '.';
                } else {
                    decSep = DecimalFormatSymbols.getInstance(loc).getDecimalSeparator();
                }
            }
            if (f2.contains(Flags.GROUP)) {
                if (loc == null || loc.equals(Locale.US)) {
                    ...
                    grpSep = DecimalFormatSymbols.getInstance(loc).getGroupingSeparator();
                    DecimalFormat df = (DecimalFormat) NumberFormat.getIntegerInstance(loc);

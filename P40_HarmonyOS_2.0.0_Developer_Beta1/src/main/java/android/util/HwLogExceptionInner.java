...
public class HwLogExceptionInner implements LogException {
    ...
    private boolean mIsPropLogSwitch = false;
    ...
    public int msg(String category, int level, String header, String body) {
        return println_exception_native(category, level, "message", header + System.lineSeparator() + body);
    }

    public int msg(String category, int level, int mask, String header, String body) {
        return println_exception_native(category, level, "message", "mask=" + mask + ";" + header + System.lineSeparator() + body);
    }
    ...
    public void initLogBlackList() {
        this.mIsPropLogSwitch = getPropLogSwitch();
    ...
    public boolean isInLogBlackList(String packageName) {
        ...
        updateAllSwitch();
        if (this.mIsPropLogSwitch || !isInLogBlackListStatic()) {
            return false;
        }
        return true;
    }
    ...
    public void updateLogSwitch() {
        ...
        if (!sPackageNameList.isEmpty()) {
            ...
            if (this.mIsLogDisable != isDisable) {
                setliblogparam(2, isDisable ? "on" : "off");
                this.mIsLogDisable = isDisable;
            }
        }
    }
    ...
    public boolean updateAllSwitch() {
        ...
        boolean isPropswitch = getPropLogSwitch();
        if (this.mIsPropLogSwitch != isPropswitch) {
            isUpdateAllSwitch = true;
            this.mIsPropLogSwitch = isPropswitch;
        }
        if (!isPropswitch && (configFileCode = getConfigFileUpdated()) != 0) {

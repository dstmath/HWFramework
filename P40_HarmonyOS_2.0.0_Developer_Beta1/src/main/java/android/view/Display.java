...
public final class Display {
    ...
    static List<String> mSpecialPackage = new ArrayList();
    ...
    static {
        mSpecialPackage.add("com.huawei.fastapp");
    }
    ...
    public void getSize(Point outSize) {
        synchronized (this) {
            updateDisplayInfoLocked();
            this.mDisplayInfo.getAppMetrics(this.mTempMetrics, getAdjustments());
    ...
    public int getRotation() {
        synchronized (this) {
            updateDisplayInfoLocked();
            DisplayAdjustments adjustments = getAdjustments();
            if (isAdjustResourcesForRotation(adjustments)) {
                return adjustments.getConfiguration().windowConfiguration.getRotation();
            }
    ...
    public Mode getMode() {
        synchronized (this) {
            updateDisplayInfoLocked();
            Mode mode = this.mDisplayInfo.getMode();
            if (HwFrameworkFactory.getHwApsImpl() == null || this.mDisplayId != 0) {
                return mode;
            }
            return HwFrameworkFactory.getHwApsImpl().scaleDisplayModeInRog(mode);
        }
    }
    ...
    public Mode[] getSupportedModes() {
        synchronized (this) {
            ...
            return HwFrameworkFactory.getHwApsImpl().scaleDisplayModesInRog(modes);
        }
    }
    ...
    public void getMetrics(DisplayMetrics outMetrics) {
        synchronized (this) {
            updateDisplayInfoLocked();
            this.mDisplayInfo.getAppMetrics(outMetrics, getAdjustments());
        }
    }

    public void getRealSize(Point outSize) {
        getRealSize(outSize, true);
    }

    public void getRealSize(Point outSize, boolean isNeedAdjust) {
        synchronized (this) {
            updateDisplayInfoLocked();
            Application app = ActivityThread.currentApplication();
            if (!HwFoldScreenManager.isFoldable() || app == null || app.getApplicationInfo() == null || !"com.huawei.camera".equals(app.getApplicationInfo().packageName) || HwFoldScreenManager.getDisplayMode() != 4 || !(this.mDisplayInfo.rotation == 0 || this.mDisplayInfo.rotation == 2)) {
                outSize.x = this.mDisplayInfo.logicalWidth;
                outSize.y = this.mDisplayInfo.logicalHeight;
                DisplayAdjustments adjustments = getAdjustments();
                if (!isNeedAdjust || !isPCAdjustments(adjustments)) {
                    ...
                    if (!ci.supportsScreen()) {
                        ...
                        Log.i(TAG, "getRealSize: before " + outSize + " ratio " + ratio + " outSize " + outSize);
                    }
                    if (!TextUtils.isEmpty(SIDE_PROP)) {
                        ...
                        if (!(app == null || app.mLoadedApk == null)) {
                            ...
                            if (outSize.x < outSize.y) {
                                if (isSystem == 0 || mSpecialPackage.contains(packagename)) {
                                    if (!HwWindowManager.isAppNeedExpand(packagename)) {
                                        outSize.x -= displaySideWidthPixels;
                                    }
                                }
                            } else if (outSize.x > outSize.y && (isSystem == 0 || mSpecialPackage.contains(packagename))) {
                ...
                outSize.x = adjustments.getConfiguration().windowConfiguration.getAppBounds().width();
                outSize.y = adjustments.getConfiguration().windowConfiguration.getAppBounds().height();
                return;
            }
    ...
    public void getRealMetrics(DisplayMetrics outMetrics) {
        synchronized (this) {
            updateDisplayInfoLocked();
            if (isPCAdjustments(getAdjustments())) {
    ...
    private void updateDisplayInfoLocked() {
        ...
        if (HwPCUtils.enabled() && (thread = ActivityThread.currentActivityThread()) != null && (overrideConfig = thread.getOverrideConfig()) != null && !overrideConfig.equals(Configuration.EMPTY) && HwPCUtils.isValidExtDisplayId(thread.getDisplayId())) {
            ...
            if (displayInfo3.appWidth > this.mDisplayInfo.appHeight) {
                i = 1;
            }
            displayInfo3.rotation = i;
        }
    }
    ...
    @UnsupportedAppUsage
    private boolean isAdjustments(DisplayAdjustments adjustments) {
        return adjustments != null && ActivityThread.isAdjustConfig(adjustments.getConfiguration());
    }

    @UnsupportedAppUsage
    private boolean isPCAdjustments(DisplayAdjustments adjustments) {
        return adjustments != null && ActivityThread.isAdjustPCConfig(adjustments.getConfiguration());
    }

    @UnsupportedAppUsage
    private DisplayAdjustments getAdjustments() {
        DisplayAdjustments adjustments = getDisplayAdjustments();
        if (!isAdjustments(adjustments) && isAdjustResources(this.mResources)) {
            return new DisplayAdjustments(this.mResources.getConfiguration());
        }
        return adjustments;
    }

    @UnsupportedAppUsage
    private boolean isAdjustResources(Resources resources) {
        return resources != null && ActivityThread.isAdjustConfig(resources.getConfiguration());
    }

    private boolean isAdjustResourcesForRotation(DisplayAdjustments adjustments) {
        if (adjustments == null || adjustments.getConfiguration() == null || !ActivityThread.isAdjustConfig(adjustments.getConfiguration())) {
            return false;
        }
        if (adjustments.getConfiguration().orientation != 2 || !ActivityThread.isAdjustConfigForMagicWindow(adjustments.getConfiguration())) {
            return true;
        }
        return false;
    }

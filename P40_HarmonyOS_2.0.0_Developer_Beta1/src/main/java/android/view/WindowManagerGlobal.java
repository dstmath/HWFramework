...
public final class WindowManagerGlobal {
        ...
        if (view == null) {
            ...
            if (HwFreeFormUtils.isFreeFormEnable() && parentWindow != null) {
                ...
                if (callback == null || !(callback instanceof Activity)) {
                    ...
                    if ((windowMode == 5 || WindowConfiguration.isHwMultiStackWindowingMode(windowMode)) && wparams.isNeededSetOutline() && !WindowConfiguration.isHwTvSplitWindowingMode(windowMode)) {
                        ...
                        } else if (WindowConfiguration.isHwTvFreeFormWindowingMode(windowMode)) {
                            view.setOutlineProvider(ViewOutlineProvider.HW_TV_FREEFORM_OUTLINE_PROVIDER);
                        } else {
            ...
            if (HwActivityTaskManager.isPCMultiCastMode() && view.getResources() != null) {
                HwActivityTaskManager.adjustParamsForPCCast(view.getResources().getConfiguration(), wparams, false);
            }
    ...
    public void adjustOverlayWindowWhileActivityConfigChanged(Configuration configuration) {
        synchronized (this.mLock) {
            for (int i = 0; i < this.mViews.size(); i++) {
                View view = this.mViews.get(i);
                if (view != null) {
                    WindowManager.LayoutParams wparams = (WindowManager.LayoutParams) view.getLayoutParams();
                    if (HwActivityTaskManager.adjustParamsForPCCast(configuration, wparams, true)) {

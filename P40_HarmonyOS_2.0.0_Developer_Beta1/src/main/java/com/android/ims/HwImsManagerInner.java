...
public class HwImsManagerInner {
    ...
    private static final String KEY_ENHANCED_4G_LTE_ON_BY_DEFAULT_BOOL = "enhanced_4g_lte_on_by_default_bool";
    ...
    public static void factoryReset(Context context, int slotId) {
        ...
        if (Settings.Global.getInt(context.getContentResolver(), "volte_vt_enabled", -1) != -1) {
            Settings.Global.putInt(context.getContentResolver(), "volte_vt_enabled", getVolteDefaultValue(context, currentSlotId));
        }
        ...
        if (hasFiledInDbByFiledName(currentSubId, context, "volte_vt_enabled")) {
            SubscriptionManagerEx.setSubscriptionProperty(currentSubId, "volte_vt_enabled", Integer.toString(getBooleanCarrierConfig(context, KEY_ENHANCED_4G_LTE_ON_BY_DEFAULT_BOOL, currentSlotId) ? 1 : 0));
        }

...
public class ZygoteConnection {
    ...
    /* JADX INFO: Multiple debug info for r0v9 int[]: [D('errnoEx' android.system.ErrnoException), D('fdsToClose' int[])] */
    /* access modifiers changed from: package-private */
    ...
    private void handleParentProc(int pid, FileDescriptor[] descriptors, FileDescriptor pipeFd) {
        ...
        if (pipeFd == null || pid2 <= 0) {
            ...
            int innerPid2 = -1;
            try {
                fds = new StructPollfd[]{new StructPollfd()};
                data = new byte[4];
                remainingSleepTime = 30000;
                dataIndex = 0;

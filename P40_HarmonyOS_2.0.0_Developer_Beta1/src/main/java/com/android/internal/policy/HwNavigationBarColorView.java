...
public class HwNavigationBarColorView extends View {
    private static final boolean IS_SHOW_DCM_NAVBAR_AT_BOTTOM = SystemProperties.getBoolean("ro.config.dcm_landscreen_navbar", false);
    ...
    public void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        if (this.mOrientation == 2 && !isNavBarAtBottom()) {
    ...
    private boolean isNavBarAtBottom() {
        int i = ROTATION;
        return (i == 0 || 2 == i) ? IS_SHOW_DCM_NAVBAR_AT_BOTTOM : this.mOrientation == 2;
    }
}

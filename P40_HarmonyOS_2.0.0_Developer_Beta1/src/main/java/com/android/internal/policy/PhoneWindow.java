...
public class PhoneWindow extends AbsWindow implements MenuBuilder.Callback {
    ...
    private static final boolean IS_WATCH = "watch".equals(SystemProperties.get("ro.build.characteristics", PhoneConstants.APN_TYPE_DEFAULT));
    ...
    public void onFreeFormOutLineChanged(int touchingState) {
        ...
        } else if (touchingState == -1 || touchingState == -2) {
            this.mDecor.onScaleFreeFrom(touchingState);
        }
    }

    @Override // android.view.Window
    public void onFreeFormOutLineChangedForFloating(int touchingState) {
        if (this.mDecor == null) {
            HwFreeFormUtils.log(TAG, "FreeFormOutLineForFloating decorview is null.");
        } else if (touchingState == -1 || touchingState == -2) {
            this.mDecor.onScaleFreeFromIfLoseHighLightNeeded(touchingState);
        }
    }
    ...
    public ViewGroup generateLayout(DecorView decor) {
        ...
        if ((features & 2048) != 0) {
            ...
            if (this.mIsFloating) {
                ...
                layoutResource = 17367273;
            }
            removeFeature(8);
        } else if ((features & 2) == 0) {
            if (this.mIsFloating) {
                TypedValue res3 = new TypedValue();
                getContext().getTheme().resolveAttribute(16844295, res3, true);
    ...
    private void registerSwipeCallbacks(ViewGroup contentParent) {
        ...
        swipeDismiss.setOnSwipeProgressChangedListener(new SwipeDismissLayout.OnSwipeProgressChangedListener() {
            ...
            public void onSwipeProgressChanged(SwipeDismissLayout layout, float alpha, float translate) {
                int flags;
                if (PhoneWindow.IS_WATCH && !this.mHasSnapShot) {
                    Window.WindowControllerCallback callback = PhoneWindow.this.getWindowControllerCallback();
                    if (callback instanceof Activity) {
                        HwActivityTaskManager.takeTaskSnapshot(((Activity) callback).getActivityToken());
    ...
    public static final class PanelFeatureState {
        ...
        public void setStyle(Context context) {
            ...
            this.isCompact = a.getBoolean(315, false);
            this.listPresenterTheme = a.getResourceId(316, R.style.Theme_ExpandedMenu);
            a.recycle();
        }

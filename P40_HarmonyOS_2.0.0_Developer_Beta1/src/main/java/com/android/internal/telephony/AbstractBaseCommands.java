...
public abstract class AbstractBaseCommands {
    ...
    protected RegistrantList mModemDataRetryRegistrants = new RegistrantList();
    ...
    public void registerForVsimIndication(Handler handler, int what, Object obj) {
        this.mVsimIndicationRegistrants.add(new Registrant(handler, what, obj));
    }
    ...
    public void registerForModemDataRetry(Handler handler, int what, Object obj) {
        this.mModemDataRetryRegistrants.add(new Registrant(handler, what, obj));
    }

    public void unregisterForModemDataRetry(Handler handler) {
        this.mModemDataRetryRegistrants.remove(handler);
    }
    ...
    public void recordVoiceBandInfo(int[] type) {
        if (isVoiceBandInfoValid(type)) {
            Settings.System.putString(PhoneFactory.getDefaultPhone().mContext.getContentResolver(), "speech_codec", String.valueOf(type[0]));
        }
    }

    private boolean isVoiceBandInfoValid(int[] type) {
        if (PhoneFactory.getDefaultPhone() == null || PhoneFactory.getDefaultPhone().mContext == null || type == null || type.length == 0) {
            return false;
        }
        return true;
    }

...
public class HwBaseOnsDisplayParamsManager {
    ...
    public OnsDisplayParams getOnsDisplayParamsForVoWifi(OnsDisplayParams ons) {
        ...
        String combineWifi2;
        int voiceIdx = 0;
        String spnConfiged = BuildConfig.FLAVOR;
        boolean isHideWifi = false;
        String wifiConfiged = BuildConfig.FLAVOR;
        ...
        if (!isHideWifi) {
            ...
            String[] wfcSpnFormats = this.mContext.getResources().getStringArray(HwPartResourceUtils.getResourceId("wfcSpnFormats"));
            if (!TextUtils.isEmpty(wifiConfiged)) {
                formatWifi = wifiConfiged;
            } else if (!isUsingGoogleWifiFormat || wfcSpnFormats == null) {
                formatWifi = this.mContext.getResources().getString(HwPartResourceUtils.getResourceId("systemui_vowifi_operator_name"));
            } else {
        ...
        ServiceState serviceState = this.mServiceStateTracker.getmSSHw();
        boolean isInService = this.mHwServiceStateTrackerEx.getCombinedRegState(serviceState) == 0;
        ...
        int combinedRegState = this.mHwServiceStateTrackerEx.getCombinedRegState(serviceState);
        if (combinedRegState == 1 || combinedRegState == 2) {
            if (ServiceStateEx.isEmergencyOnly(serviceState)) {
        ...
        if (!TextUtils.isEmpty(spnConfiged)) {
            ...
            combineWifi = (!isInService || TextUtils.isEmpty(ons.mPlmn)) ? BuildConfig.FLAVOR : ons.mPlmn;
        }
        if (!isAirplaneMode || !isShowAirplane) {
            combineWifi2 = combineWifi;
            if (isNoService && isShowNoService) {
                combineWifi2 = Resources.getSystem().getText(HwPartResourceUtils.getResourceId("lockscreen_carrier_default")).toString();
            } else if (!isEmergencyOnly || !isShowEmergency) {
                loge("getOnsDisplayParamsForVoWifi combine wifi fail");
            } else {
                combineWifi2 = Resources.getSystem().getText(HwPartResourceUtils.getResourceId("emergency_calls_only")).toString();
            }
        } else {
            combineWifi2 = Resources.getSystem().getText(HwPartResourceUtils.getResourceId("global_actions_toggle_airplane_mode")).toString();
        }
        try {
    ...
    public int getRegState(ServiceState ss) {
        ...
        int regState = ServiceStateEx.getVoiceRegState(ss);
    ...
    public ServiceState getSs() {
        IServiceStateTrackerInner iServiceStateTrackerInner = this.mServiceStateTracker;
        if (iServiceStateTrackerInner == null) {
            return null;
        }
        return iServiceStateTrackerInner.getmSSHw();
    }
    ...
    public ServiceState getNewSS() {
        ...
        return iServiceStateTrackerInner.getmNewSSHw();
    }

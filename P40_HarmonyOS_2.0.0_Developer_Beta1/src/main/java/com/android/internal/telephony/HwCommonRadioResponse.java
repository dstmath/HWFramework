...
import vendor.huawei.hardware.radio.V2_2.DsFlowInfo;
import vendor.huawei.hardware.radio.V2_2.IRadioResponse;
import vendor.huawei.hardware.radio.V2_2.LteAttachInfo;
import vendor.huawei.hardware.radio.V2_2.VsimEvent;
...
public class HwCommonRadioResponse extends IRadioResponse.Stub {
    ...
    public void RspMsg(RadioResponseInfo responseInfo, int msgId, RspMsgPayload payload) {
        ...
        switch (msgId) {
            ...
            case 763:
                ...
                break;
            default:
                extendRspMsg(responseInfo, null, msgId, payload, true);
                validMsgId = false;
                break;
        }
        if (validMsgId) {
    ...
    private void extendRspMsg(RadioResponseInfo responseInfo, Object ret, int msgId, RspMsgPayload payload, boolean isValidMsgId) {
        if (msgId != 8) {
            isValidMsgId = false;
        } else {
            ret = processInts(payload);
        ...
        if (isValidMsgId) {
            if (!(responseInfo.error == 0 || msgId == 8)) {
                ret = null;
            }
            processRadioResponse(responseInfo, ret);
        }
    ...
    public void setUeOperationModeResponse(RadioResponseInfo info) {
        logi("setUeOperationModeResponse");
        processRadioResponse(info, null);
    }

    public void getLteAttachInfoResponse(RadioResponseInfo info, LteAttachInfo attachInfo) {
        logi("getLteAttachInfoResponse ");
    ...
    private void responseLteAttachInfo(RadioResponseInfo responseInfo, LteAttachInfo attachInfo) {
        ...
        if (rr != null) {
            if (responseInfo.error == 0) {
                logi("responseLteAttachInfo, sendMessageResponse. apn name is:" + attachInfo.apn);
    ...
    public void getDsFlowInfoResponse(RadioResponseInfo responseInfo, DsFlowInfo dsFlowInfo) {
        if (dsFlowInfo == null) {
    ...
    public void sendVsimNotificationExResponse(RadioResponseInfo responseInfo, VsimEvent event) {
        if (event == null) {
            logi("sendVsimNotificationExResponse: event is null.");
        } else {
            processRadioResponse(responseInfo, Integer.valueOf(event.transactionId));
        }
    }
}

...
public class HwCustPhoneServiceImpl extends HwCustPhoneService {
    ...
    public int getNrOnOffMappingNetworkMode(int slotId, int type, int ability) {
        ...
        if (networkModeByAbility == null || TextUtils.isEmpty(networkModeByAbility) || (networkModes = networkModeByAbility.split(",")) == null || networkModes.length != 2 || (serviceState = TelephonyManager.getDefault().getServiceStateForSubscriber(getSubIdBySlotId(slotId))) == null || serviceState.getRoaming() || type != 1) {
            return INVALID_NETWORK_MODE;
        }
        try {

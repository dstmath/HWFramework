...
public class HwDualCardSwitcher extends Handler {
    ...
    private void checkIfNeedToSwitchSlot() {
        ...
        if (HwTelephonyManagerInner.getDefault().getDefaultMainSlotCarrier() != 0) {
            ...
            HwFullNetworkManager instance = HwFullNetworkManager.getInstance();
            int i = this.mSecondarySlot;
            instance.setMainSlot(i, obtainMessage(2, i, 0));
    ...
    private boolean shouldSwitch() {
        ...
        if (HwFullNetworkConfigInner.isCustomVersion()) {
            logd("shouldSwitch, AIS/SMART/MTN custom verion. can't auto switch card.");
            return false;
        }
        ...
        if (otherIsInService && !otherIsRoaming) {
            ...
            if (hwTelephonyManagerInner != null && hwTelephonyManagerInner.isCTSimCard(this.mSecondarySlot) && !TextUtils.isEmpty(regPlmn) && !regPlmn.startsWith(MCC_OF_CN)) {
                ...
                sb.append(LogEx.getLogHWInfo() ? regPlmn : str);
                logd(sb.toString());
                PhoneExt[] phones = PhoneFactoryExt.getPhones();
                if (phones != null) {
                    int i = this.mPrimarySlot;
                    if (phones[i] != null) {
                        String imsi = phones[i].getSubscriberId();
            ...
            if (primaryIsInService && primaryIsRoaming) {
                ...
                if (phones2 != null) {
                    int i2 = this.mPrimarySlot;
                    if (phones2[i2] != null) {
                        String imsi2 = phones2[i2].getSubscriberId();
                ...
                sb2.append(LogEx.getLogHWInfo() ? regPlmn : str);

...
public class HwIccSmsInterfaceManagerEx extends DefaultHwIccSmsInterfaceManagerEx {
    ...
    private static final int ANTI_CLOSE_REPORT_ID = 990580010;
    ...
    private Handler mHwHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case HwIccSmsInterfaceManagerEx.EVENT_GET_SMSC_DONE /* 101 */:
                    HwIccSmsInterfaceManagerEx.this.handleGetSmscDone(msg);
                    return;
                case HwIccSmsInterfaceManagerEx.EVENT_SET_SMSC_DONE /* 102 */:
                    HwIccSmsInterfaceManagerEx.this.handleSetSmscDone(msg);
                    return;
                case HwIccSmsInterfaceManagerEx.EVENT_GET_MEID_OR_PESN_DONE /* 103 */:
                    HwIccSmsInterfaceManagerEx.this.handleGetMeidOrPesnDone(msg);
                    return;
                case HwIccSmsInterfaceManagerEx.EVENT_SET_MEID_OR_PESN_DONE /* 104 */:
                    HwIccSmsInterfaceManagerEx.this.handleSetMeidOrPesnDone(msg);
                    return;
                case HwIccSmsInterfaceManagerEx.EVENT_GET_UIM_SUPPORT_MEID_DONE /* 105 */:
                    HwIccSmsInterfaceManagerEx.this.handleGetUimSupportMeidDone(msg);
                    return;
                case HwIccSmsInterfaceManagerEx.ANTI_SNIFFING_PROCESS_SERVICE_DONE /* 106 */:
                    HwIccSmsInterfaceManagerEx.this.handleAntiSniffingProcessServiceDone(msg);
                    return;
                case HwIccSmsInterfaceManagerEx.ANTI_SNIFFING_CLOSE_SELF_DONE /* 107 */:
                    HwIccSmsInterfaceManagerEx.this.handleAntiSniffingCloseSelfDone(msg);
                    return;
                case HwIccSmsInterfaceManagerEx.EVENT_ANTI_CLOSE_DELAY_TIMEOUT /* 108 */:
    ...
    public String getMeidOrPesn() {
        if (!TelephonyPermissionsExt.checkCallingOrSelfReadDeviceIdentifiers(this.mContext, this.mPhone.getSubId(), (String) null, "getMeidOrPesn")) {
            log("getMeidOrPesn can't pass checkCallingOrSelfReadDeviceIdentifiers.");
            return null;
        }
    ...
    private void handleAntiSniffingCloseSelfDone(Message msg) {
        ...
        if (ar.getResult() != null && (ar.getResult() instanceof ArrayList)) {
            ...
            if (list.size() < 2) {
                ...
                String ctmPara = Calendar.getInstance().getTime().toString();
                HiEventEx event = new HiEventEx((int) ANTI_CLOSE_REPORT_ID);
                event.putString("ctm", ctmPara).putString("ctc", "screen").putInt("sid", slotId);

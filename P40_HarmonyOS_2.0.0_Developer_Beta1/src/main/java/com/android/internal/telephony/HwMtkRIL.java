...
public class HwMtkRIL extends RIL {
    ...
    public void setLTEReleaseVersion(int state, Message result) {
        ...
        if (radioProxy != null) {
            ...
            addRequestEx(rr);
            mtkRiljLog(rr.serialString() + "> " + requestToString(rr.mRequest) + " state = " + state);
            try {
                radioProxy.setLteReleaseVersion(rr.mSerial, state);
            } catch (RemoteException | RuntimeException e) {
                handleRadioProxyExceptionForRREx(requestToString(rr.mRequest), e, rr);
            }
        }
    ...
    public void registerCsconModeInfo(Handler handler, int what, Object obj) {
        this.mCsconModeInfoRegistrant = new Registrant(handler, what, obj);
    }

    public void unregisterCsconModeInfo(Handler handler) {
    ...
    public void setNrSwitch(boolean on, Message onComplete) {
        ...
        } else if (isShowVolteSwitchInNsa(this.mContext, this.mInstanceId.intValue())) {
            if (onComplete != null) {
                onComplete.sendToTarget();
            }
        } else if (HwImsManagerInner.isVolteEnabledByPlatform(this.mContext, this.mInstanceId.intValue())) {
            if (on) {
    ...
    private void setImsSwitch(boolean on, boolean isSaveDB) {
        ...
        if (isSaveDB) {
            if (HwImsManagerInner.isDualImsAvailable()) {
                dbName = HW_VOLTE_USER_SWITCH_DUALIMS[this.mInstanceId.intValue()];
            } else {
    ...
    public void sendVsimNotification(final int transactionId, final int eventId, final int simType, String challenge, Message message) {
        mtkRiljLog("sendVsimNotification");
        if (challenge == null) {
            invokeIRadio(2207, message, new MtkRILCommand() {
                /* class com.android.internal.telephony.HwMtkRIL.AnonymousClass13 */
        ...
        mtkRiljLog("sendVsimNotification with chanllenge");
        sendVsimNotificationEx(transactionId, eventId, simType, challenge, message);
    }

    public void sendVsimOperation(final int transactionId, final int eventId, final int messageId, final int dataLength, byte[] data, Message message) {
        riljLog("sendVsimOperation");
        final ArrayList<Byte> arrList = new ArrayList<>();
        for (byte b : data) {
            arrList.add(Byte.valueOf(b));
        }
        invokeIRadio(2208, message, new MtkRILCommand() {
            /* class com.android.internal.telephony.HwMtkRIL.AnonymousClass14 */

            @Override // com.android.internal.telephony.HwMtkRIL.MtkRILCommand
            public void excute(IMtkRadioEx radio, int serial) throws RemoteException, RuntimeException {
                radio.sendVsimOperation(serial, transactionId, eventId, messageId, dataLength, arrList);
            }
        });
    }
}

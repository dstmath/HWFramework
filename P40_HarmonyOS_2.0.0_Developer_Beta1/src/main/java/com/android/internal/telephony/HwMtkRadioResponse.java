...
public class HwMtkRadioResponse extends IMtkRadioExResponse.Stub {
    ...
    public void getLteReleaseVersionResponse(RadioResponseInfo responseInfo, int mode) {
        ...
        if (rr != null) {
            if (responseInfo.error == 0) {
                sendMessageResponse(rr.mResult, new int[]{mode});
            }
            this.mMtkRil.processResponseDoneEx(rr, responseInfo, null);
        }
    }
    ...
    public void vsimNotificationResponse(RadioResponseInfo info, VsimEvent event) {
        RadioResponseInfo radioResponseInfo = switchResponseInfo(info);
        RILRequest rilRequest = this.mMtkRil.processResponse(info);
        if (rilRequest != null) {
            Object ret = null;
            if (radioResponseInfo.error == 0) {
                ret = Integer.valueOf(event.transactionId);
                sendMessageResponse(rilRequest.mResult, ret);
            }
            this.mMtkRil.processResponseDone(rilRequest, radioResponseInfo, ret);
            return;
        }
        logi("vsimNotificationResponse, rilRequest is null");
    }

    public void vsimOperationResponse(RadioResponseInfo info) {
        RadioResponseInfo radioResponseInfo = switchResponseInfo(info);
        RILRequest rilRequest = this.mMtkRil.processResponse(radioResponseInfo);
        if (rilRequest != null) {
            if (radioResponseInfo.error == 0) {
                sendMessageResponse(rilRequest.mResult, null);
            }
            this.mMtkRil.processResponseDone(rilRequest, radioResponseInfo, null);
            return;
        }
        logi("vsimOperationResponse, rilRequest is null");
    }

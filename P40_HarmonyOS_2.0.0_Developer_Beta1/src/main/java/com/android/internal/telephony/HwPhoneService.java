...
public class HwPhoneService extends IHwTelephony.Stub implements HwPhoneManager.PhoneServiceInterface {
    ...
    private void initService() {
        ...
        if (HuaweiTelephonyConfigs.isHisiPlatform() || HuaweiTelephonyConfigs.isMTKPlatform()) {
    ...
    public void closeRrc() {
        ...
        int dataSlotId = SubscriptionManagerEx.getSlotIndex(dataSubId);
        logi("closeRrc, dataSubId = " + dataSubId + ", dataSlotId = " + dataSlotId);
        if (dataSlotId >= 0) {
            PhoneExt[] phoneExtArr = this.mPhones;
            if (dataSlotId < phoneExtArr.length) {
                phoneExtArr[dataSlotId].closeRrc();
            }
        }
    }
    ...
    private int calculateNetworkType(int ability, int type) {
        ...
        if ((type == 0 && HwNetworkTypeUtils.isNrServiceOn(mappingNetworkType)) || (type == 1 && !HwNetworkTypeUtils.isNrServiceOn(mappingNetworkType))) {
            mappingNetworkType = -1;
        }
    ...
    private CommandsInterfaceEx getDataSlotCommandsInterface() {
        PhoneExt phoneExt = PhoneFactoryExt.getPhone(getDefault4GSlotId());
        if (phoneExt != null) {
            return phoneExt.getCi();
        }
        return null;
    }
    ...
    public boolean notifyDeviceState(String device, String state, String extra) {
        ...
        try {
            CommandsInterfaceEx commandsInterfaceEx = getDataSlotCommandsInterface();
            if (commandsInterfaceEx == null) {
                return false;
            }
            commandsInterfaceEx.notifyDeviceState(device, state, extra, response);
            return true;
        } catch (RuntimeException e) {
    ...
    public void informModemTetherStatusToChangeGRO(int enable, String faceName) {
        this.mContext.enforceCallingPermission("android.permission.MODIFY_PHONE_STATE", null);
        log("informModemTetherStatusToChangeGRO, enable = " + enable + ", name = " + faceName);
        if (faceName == null) {
            loge("informModemTetherStatusToChangeGRO, illegal para, return");
            return;
        }
    ...
    public byte[] getCardTrayInfo() {
        ...
        if (HuaweiTelephonyConfigs.isQcomPlatform()) {
            log("Qcom not support getCardTrayInfo");
            return new byte[0];
        }
        enforceReadPermission();
        synchronized (this.mCardTrayLock) {
    ...
    public boolean setNrOptionMode(int mode, Message msg) {
        return setNrOptionModeForSlotId(HwTelephonyManagerInner.getDefault().getDefault4GSlotId(), mode, msg);
    }
    ...
    public int getNrOptionMode() {
        return getNrOptionModeForSlotId(HwTelephonyManagerInner.getDefault().getDefault4GSlotId());
    }

    public int getNrOptionModeForSlotId(int slotId) {
        return ((Integer) sendRequest(CMD_GET_NR_OPTION_MODE, null, Integer.valueOf(slotId))).intValue();
    }
    ...
    private void handleGetNrOptionMode(Message msg) {
        MainThreadRequest request = (MainThreadRequest) msg.obj;
        Message onCompleted = this.mMainHandler.obtainMessage(EVENT_GET_NR_OPTION_MODE_DONE, request);
        CommandsInterfaceEx ci = getCommandsInterface(request.subId.intValue());
        if (ci != null) {
    ...
    public class HwInnerPhoneService extends IHwTelephonyInner.Stub {
        ...
        public void setSwitchState(int type, int ability) {
            HwPhoneService.this.mHwPhoneServiceEx.setSwitchState(type, ability);
        }

        public boolean getSwitchState(int type) {
            return HwPhoneService.this.mHwPhoneServiceEx.getSwitchState(type);
        }

        public boolean setNrOptionModeForSlotId(int slotId, int mode, Message msg) {
            return HwPhoneService.this.mHwPhoneServiceEx.setNrOptionModeForSlotId(slotId, mode, msg);
        }

        public int getNrOptionModeForSlotId(int slotId) {
            return HwPhoneService.this.mHwPhoneServiceEx.getNrOptionModeForSlotId(slotId);
        }
    }
}

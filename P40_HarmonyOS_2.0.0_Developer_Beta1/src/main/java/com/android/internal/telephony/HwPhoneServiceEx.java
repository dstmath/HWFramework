...
public class HwPhoneServiceEx {
    ...
    private static final int SWITCH_SERVICE_TYPE_NR = 0;
    ...
    public void setSwitchState(int type, int ability) {
        if (type != 0) {
            RlogEx.e(TAG, "caller is not 5g switch.");
        } else if (!HwTelephonyManager.getDefault().isNrSupported()) {
            RlogEx.e(TAG, "the device is not supported nr.");
        } else {
            HwPhoneService hwPhoneService = HwPhoneService.getInstance();
            if (hwPhoneService == null) {
                RlogEx.e(TAG, "hwPhoneService is null.");
            } else if (!HwTelephonyManagerInnerUtils.getDefault().isDualNrSupported() || ability != 0) {
                hwPhoneService.setServiceAbilityForSlotId(hwPhoneService.getDefault4GSlotId(), 1, ability);
            } else {
                for (int i = 0; i < SIM_NUM; i++) {
                    if (hwPhoneService.getServiceAbilityForSlotId(i, 1) == 1) {
                        hwPhoneService.setServiceAbilityForSlotId(i, 1, ability);
                    }
                }
            }
        }
    ...
    public boolean getSwitchState(int type) {
        HwPhoneService hwPhoneService;
        if (type != 0 || (hwPhoneService = HwPhoneService.getInstance()) == null) {
            return false;
        }
        boolean isSub1CardPresent = isCardPresent(0);
        boolean isSub2CardPresent = isCardPresent(1);
        boolean isSub1CardActive = isCardActive(0);
        boolean isSub2CardActive = isCardActive(1);
        if (!isSub1CardPresent || !isSub2CardPresent) {
            if (!isSub1CardPresent || isSub2CardPresent) {
                if (isSub1CardPresent || !isSub2CardPresent || hwPhoneService.getServiceAbilityForSlotId(1, 1) != 1) {
                    return false;
                }
                return true;
            } else if (hwPhoneService.getServiceAbilityForSlotId(0, 1) == 1) {
                return true;
            } else {
                return false;
            }
        } else if ((!isSub1CardActive || isSub2CardActive) && (isSub1CardActive || !isSub2CardActive)) {
            return hasNrSwitchOpened();
        } else {
            if (hwPhoneService.getServiceAbilityForSlotId(hwPhoneService.getDefault4GSlotId(), 1) == 1) {
                return true;
            }
            return false;
        }
    ...
    public boolean setNrOptionModeForSlotId(int slotId, int mode, Message msg) {
        ...
        hwPhoneService.setNrOptionModeForSlotId(slotId, mode, msg);
        return false;
    }
    ...
    public int getNrOptionModeForSlotId(int slotId) {
        ...
        hwPhoneService.getNrOptionModeForSlotId(slotId);
        return 0;
    }

    private boolean isCardPresent(int slotId) {
        UiccControllerExt uiccController = UiccControllerExt.getInstance();
    ...
    private boolean isCardActive(int slotId) {
        SubscriptionControllerEx subscriptionControllerEx = SubscriptionControllerEx.getInstance();
        if (subscriptionControllerEx != null && subscriptionControllerEx.getSubState(slotId) == 1) {
            return true;
        }
        return false;
    }

    private boolean hasNrSwitchOpened() {
        HwPhoneService hwPhoneService = HwPhoneService.getInstance();
        if (hwPhoneService == null) {
            return false;
        }

...
public class HwQualcommRIL extends RIL {
    ...
    private static final String REAL_PROVIDER = "com.qualcomm.qcrilhook.QcRilHook";
    private static final String REAL_PROVIDER_PATH = "system/product/framework/qcrilhook.jar";
    private static final String REAL_QTI_TELEPHONY_UTILS_PATH = "system/product/framework/qti-telephony-utils.jar";
    ...
    private synchronized void getQcRilHook(Context context) {
        ...
        if (qcRilHook == null) {
            try {
                PathClassLoader classLoader = new PathClassLoader(REAL_PROVIDER_PATH, null, ClassLoader.getSystemClassLoader());
                classLoader.addDexPath(REAL_QTI_TELEPHONY_UTILS_PATH);
                setQcRilHook(classLoader.loadClass(REAL_PROVIDER).getConstructor(Context.class).newInstance(context));
    ...
    public void setImsSwitch(boolean isOpen) {
        setImsSwitch(isOpen, true);
    }

    private void setImsSwitch(boolean isOpen, boolean isSaveDB) {
        String dbName;
        if (this.mContext == null) {
            qcomRiljLog("setImsSwitch, context is null");
            return;
        }
        qcomRiljLog("setImsSwitch: " + isOpen + ", isSaveDB: " + isSaveDB);
        if (isSaveDB) {
            if (HwImsManagerInner.isDualImsAvailable()) {
                dbName = HW_VOLTE_USER_SWITCH_DUALIMS[this.mRilInstanceId.intValue()];
            } else {
                dbName = "hw_volte_user_switch";
            }
            Settings.System.putInt(this.mContext.getContentResolver(), dbName, isOpen ? 1 : 0);
        }
        HwImsManagerInner.setVolteSwitch(this.mContext, this.mRilInstanceId.intValue(), isOpen);
    }

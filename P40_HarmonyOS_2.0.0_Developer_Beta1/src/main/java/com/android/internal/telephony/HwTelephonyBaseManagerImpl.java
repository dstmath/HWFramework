...
public class HwTelephonyBaseManagerImpl implements HwTelephonyBaseManager {
    private static final String LOG_TAG = "HwTelephonyBaseManagerImpl";
    ...
    public CommandException fromRilErrnoEx(int ril_errno) {
        ...
        if (ril_errno == 406) {
            return new CommandException(CommandException.Error.SERVICE_NOT_SUBSCRIBED);
        }
    ...
    public <T> T createHwRil(Context context, int networkMode, int cdmaSubscription, Integer instanceId) {
        try {
            String rilClassName = SystemPropertiesEx.get("ro.telephony.ril_class", "HwHisiRIL").trim();
            RlogEx.i(LOG_TAG, "RILClassname is " + rilClassName);
            Class<?> clazz = Class.forName("com.android.internal.telephony." + rilClassName);

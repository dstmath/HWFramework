...
public class HwUiccSmsManager {
    ...
    private static IHwUiccSmsManager getService() {
        try {
    ...
    public static void processSmsAntiAttack(int serviceType, int smsType, int slotId, Message msg) {
        Rlog.i(TAG, "processSmsAntiAttack enter fwk.");
        IHwUiccSmsManager service = getService();
        if (service != null) {
            try {
                service.processSmsAntiAttack(serviceType, smsType, slotId, msg);
            } catch (RemoteException e) {
                Rlog.e(TAG, "processSmsAntiAttack failed: catch RemoteException!");
            }
        }
    }
}

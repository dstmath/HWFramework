...
public final class NetworkScanRequestTracker {
    ...
    public class NetworkScanRequestScheduler {
        ...
        private void receiveResult(AsyncResult ar) {
            ...
            LocationAccessPolicy.LocationPermissionQuery locationQuery = new LocationAccessPolicy.LocationPermissionQuery.Builder().setCallingPackage(nsri.mCallingPackage).setCallingPid(nsri.mPid).setCallingUid(nsri.mUid).setMinSdkVersionForFine(29).setMinSdkVersionForCoarse(1).setMethod("NetworkScanTracker#onResult").build();
            if (ar.exception != null || ar.result == null) {
                NetworkScanRequestTracker.this.logEmptyResultOrException(ar);

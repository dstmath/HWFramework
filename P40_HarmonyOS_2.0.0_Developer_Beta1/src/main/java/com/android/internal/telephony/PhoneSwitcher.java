...
public class PhoneSwitcher extends Handler implements IHwPhoneSwitcherInner {
        ...
        switch (msg.what) {
            ...
            case 112:
                ...
                if (this.mEmergencyOverride != null) {
                    ...
                    return;
                } else if (ar != null && !commandSuccess) {
            ...
            default:
                ...
                this.mHwPhoneSwitcherEx.handleMessage(null, null, msg);
                return;
        }
    ...
    private boolean onEvaluate(boolean requestsChanged, String reason) {
        ...
        if (isEmergency()) {
            ...
            return false;
        } else if ((VSimUtilsInner.isVSimEnabled() || VSimUtilsInner.isVSimCauseCardReload()) && HuaweiTelephonyConfigs.isHisiPlatform()) {
            ...
            if (diffDetected) {
                ...
                if (this.mHalCommandToUse == 2) {
                    sendRilCommandsWhenUsePreferredData();
                } else {
            ...
            if ((PhoneFactory.IS_QCOM_DUAL_LTE_STACK || (SystemProperties.getBoolean("ro.hwpp.qcril_cross_mapping", false) && this.requestPhoneCount == 0 && "defaultChanged".equals(reason))) && !HwModemCapability.isCapabilitySupport(28)) {
                for (int phoneId2 = 0; phoneId2 < this.mNumPhones; phoneId2++) {
                    log("defaultChanged onRadioCapChanged + phoneid =  " + phoneId2);
                    onRadioCapChanged(phoneId2);
                }
            }
            this.mManualDdsSwitch = false;
            return diffDetected;
    ...
    private void updateHalCommandToUse() {
        int oldHalCommandToUse = this.mHalCommandToUse;
        this.mHalCommandToUse = this.mRadioConfig.isSetPreferredDataCommandSupported() ? 2 : 1;
        if (oldHalCommandToUse != 2 && this.mHalCommandToUse == 2) {
            log("updateHalCommandToUse from " + oldHalCommandToUse + " to " + this.mHalCommandToUse);
            sendRilCommandsWhenUsePreferredData();
            this.mActivePhoneRegistrants.notifyRegistrants();
        }
    }
    ...
    public int getTopPrioritySubscriptionId() {
        List<DcRequestExt> dcRequests = new ArrayList<>();
        for (DcRequest dcRequest : this.mPrioritizedDcRequests) {
            dcRequests.add(DcRequestExt.getDcRequestExt(dcRequest));
        }
        return this.mHwPhoneSwitcherEx.getTopPrioritySubscriptionId(dcRequests, this.mPhoneSubscriptions);
    }
    ...
    private void sendRilCommandsWhenUsePreferredData() {
        log("sendRilCommandsWhenUsePreferredData");
        for (int phoneId = 0; phoneId < this.mNumPhones; phoneId++) {

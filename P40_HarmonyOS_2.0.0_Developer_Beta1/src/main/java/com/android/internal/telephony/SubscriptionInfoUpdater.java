...
public class SubscriptionInfoUpdater extends Handler implements ISubscriptionInfoUpdaterInner {
    ...
    public SubscriptionInfoUpdater(Looper looper, Context context, Phone[] phone, CommandsInterface[] ci, IPackageManager packageMgr) {
        ...
        this.mHwSubscriptionInfoUpdaterEx = HwPartTelephonyFactory.loadFactory(HwPartTelephonyFactory.TELEPHONY_FACTORY_IMPL_NAME).createHwSubscriptionInfoUpdaterEx(this, context, CommandsInterfaceEx.getCommandsInterfaceExs(ci));
    ...
    /* JADX INFO: Multiple debug info for r12v9 android.telephony.UiccAccessRule[]: [D('carrierConfigAccessRules' android.telephony.UiccAccessRule[]), D('i' int)] */
    ...
    public void updateSubscriptionByCarrierConfig(int phoneId, String configPackageName, PersistableBundle config) {
        ...
        ParcelUuid groupUuid3 = null;
        String[] certs = config.getStringArray("carrier_certificate_string_array");
        if (certs != null) {
            logd("updateSubscriptionByCarrierConfig sub id = " + currentSubId);
            int realLength = 0;
            String[] certsTmp = new String[certs.length];
            for (int i = 0; i < certs.length; i++) {
                if (certs[i] == null || !certs[i].matches("[A-Fa-f0-9]+$")) {
                    logd("updateSubscriptionByCarrierConfig skip index: " + i);
                } else {
                    certsTmp[realLength] = certs[i];
                    realLength++;
                }
            }
            UiccAccessRule[] carrierConfigAccessRules = new UiccAccessRule[realLength];
            int i2 = 0;
            while (i2 < realLength) {
                carrierConfigAccessRules[i2] = new UiccAccessRule(IccUtils.hexStringToBytes(certsTmp[i2]), (String) null, 0);
                i2++;
                groupUuid3 = groupUuid3;
                certs = certs;
            }
            groupUuid = groupUuid3;
            cv.put("access_rules_from_carrier_configs", UiccAccessRule.encodeRules(carrierConfigAccessRules));
        } else {
            groupUuid = null;
        }
        if (!isCarrierServicePackage(phoneId, configPackageName)) {
            ...
            if (!TextUtils.isEmpty(groupUuidString)) {
                try {
                    groupUuid2 = ParcelUuid.fromString(groupUuidString);
                    try {
                        if (!groupUuid2.equals(REMOVE_GROUP_UUID) || currentSubInfo.getGroupUuid() == null) {
                            if (SubscriptionController.getInstance().canPackageManageGroup(groupUuid2, configPackageName)) {
                                cv.put("group_uuid", groupUuid2.toString());
                                cv.put("group_owner", configPackageName);
                                logd("Group Added for" + currentSubId);
                            } else {
                                loge("configPackageName " + configPackageName + " doesn't own grouUuid " + groupUuid2);
                            }
                            if (cv.size() <= 0 && mContext.getContentResolver().update(SubscriptionManager.getUriForSubscriptionId(currentSubId), cv, null, null) > 0) {
                                ...
                                MultiSimSettingController.getInstance().notifySubscriptionGroupChanged(groupUuid2);
                                return;
                            }
                            return;
                        }
    ...
    private void broadcastSimCardStateChanged(int phoneId, int state) {
        ...
        if (state != iArr[phoneId]) {
            ...
            i.addFlags(67108864);
            i.addFlags(ApnSettingHelper.TYPE_WIFI_MMS);

...
public class ComprehensionTlv {
    ...
    /* JADX INFO: Multiple debug info for r3v4 int: [D('tag' int), D('curIndex' int)] */
            ...
            if (!(temp == 0 || temp == 255)) {
                ...
                try {
                    ...
                    if (temp2 < 128) {
                        ...
                        int length2 = 255 & data[curIndex3];
                        if (length2 >= 128) {
                            curIndex = curIndex4;
                            length = length2;
                        } else {
                            throw new ResultException(ResultCode.CMD_DATA_NOT_UNDERSTOOD, "length < 0x80 length=" + Integer.toHexString(length2) + " startIndex=" + startIndex + " curIndex=" + curIndex4 + " endIndex=" + endIndex);
                        }
                    } else if (temp2 == 130) {
                        int length3 = ((data[curIndex3] & 255) << 8) | (255 & data[curIndex3 + 1]);
                        int curIndex5 = curIndex3 + 2;
                        if (length3 >= 256) {
                            length = length3;
                            curIndex = curIndex5;
                        } else {
                            try {
                                throw new ResultException(ResultCode.CMD_DATA_NOT_UNDERSTOOD, "two byte length < 0x100 length=" + Integer.toHexString(length3) + " startIndex=" + startIndex + " curIndex=" + curIndex5 + " endIndex=" + endIndex);
                        ...
                        int length4 = ((data[curIndex3] & 255) << 16) | ((data[curIndex3 + 1] & 255) << 8) | (255 & data[curIndex3 + 2]);
                        int curIndex6 = curIndex3 + 3;
                        if (length4 >= 65536) {
                            length = length4;
                            curIndex = curIndex6;
                        } else {
                            throw new ResultException(ResultCode.CMD_DATA_NOT_UNDERSTOOD, "three byte length < 0x10000 length=0x" + Integer.toHexString(length4) + " startIndex=" + startIndex + " curIndex=" + curIndex6 + " endIndex=" + endIndex);
                        }
                    } else {

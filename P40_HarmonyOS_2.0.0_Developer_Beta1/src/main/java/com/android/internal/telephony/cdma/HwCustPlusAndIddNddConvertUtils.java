...
public class HwCustPlusAndIddNddConvertUtils {
    ...
    private static final String LOG_TAG = "HwCustPlusAndIddNddConvertUtils";
    ...
    private static HwCustMccIddNddSid getNetworkMccIddList() {
        ...
        HwCustMccIddNddSid mccIddNddSid = HwCustPlusCountryListTable.getItemFromCountryListByMcc(findMcc);
    ...
    public static String getCurMccBySidLtmoff(String sSid) {
        ...
        if (itemList == null || itemList.size() == 0) {
            ...
            findMcc = HwCustPlusCountryListTable.getCcFromConflictTableByLTM(itemList, sLtmoff);
        }
        if (findMcc == null) {
    ...
    public static String replaceIddNddWithPlusForSms(String number) {
        if (number == null || number.length() == 0) {
            Rlog.d(LOG_TAG, "please check the param");
            return number;
            ...
            HwCustMccIddNddSid mccIddNddSid = getMccIddListForSms();
            if (mccIddNddSid == null) {
                Rlog.d(LOG_TAG, "find no operator that match the MCC");
                return number;
            }
            String idd = mccIddNddSid.Idd;
            Rlog.d(LOG_TAG, "find match the cc, Idd = " + idd);

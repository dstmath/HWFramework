...
public abstract class HwFullNetworkCheckStateHisiBase extends HwFullNetworkCheckStateBase {
    ...
    public void handleMessage(Message msg) {
        ...
        if (i == 302) {
            ...
            if (this.mChipHisi.needSetDataAllowCount == this.mChipHisi.curSetDataAllowCount) {
                HwFullNetworkChipHisi hwFullNetworkChipHisi2 = this.mChipHisi;
                hwFullNetworkChipHisi2.needSetDataAllowCount = 0;
                hwFullNetworkChipHisi2.curSetDataAllowCount = 0;
    ...
    public boolean switchDualCardsSlotIfNeeded(boolean isNeedSwitchMainSlotForEuicc) {
        if (!(!SystemPropertiesEx.getBoolean("persist.sys.dualcards", false) || this.mChipHisi.mAutoSwitchDualCardsSlotDone) || this.mChipHisi.needFixMainSlotPosition || isNeedSwitchMainSlotForEuicc) {
            ...
            if (isCard1Present || isCard2Present) {
                ...
                if (HwFullNetworkConfigInner.IS_HISI_DSDX && !isVSimOn) {
                    if (this.mChipHisi.needSetDataAllowCount == 0) {
                        ...
                        if (this.mChipHisi.needSetDataAllowCount > 0) {
                            ...
                            HwFullNetworkChipHisi hwFullNetworkChipHisi = this.mChipHisi;
                            hwFullNetworkChipHisi.curSetDataAllowCount = 0;
                            hwFullNetworkChipHisi.setWaitingSwitchBalongSlot(true);
                            this.mChipHisi.mAutoSwitchDualCardsSlotDone = true;
                            return false;
                        }
                ...
                HwFullNetworkChipHisi hwFullNetworkChipHisi2 = this.mChipHisi;
                hwFullNetworkChipHisi2.mAutoSwitchDualCardsSlotDone = true;
                if (isVSimOn) {
                    logd("switchDualCardsSlotIfNeeded, vsim is on, not set mark to true.");
                } else {
                    hwFullNetworkChipHisi2.setWaitingSwitchBalongSlot(true);
                }
                return true;
            }

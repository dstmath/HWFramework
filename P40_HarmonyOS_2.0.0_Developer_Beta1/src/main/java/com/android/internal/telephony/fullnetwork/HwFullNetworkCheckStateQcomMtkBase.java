...
public abstract class HwFullNetworkCheckStateQcomMtkBase extends HwFullNetworkCheckStateBase {
    ...
    public boolean checkIfAllCardsReady(Message msg) {
        ...
        if (!HwTelephonyManager.getDefault().isPlatformSupportVsim() || !HwVSimUtils.isPlatformTwoModems() || !HwVSimUtils.isVSimEnabled()) {
            if (HwFullNetworkConfigInner.USE_USER_PREFERENCE_DEFAULT_SLOT_IN_QC_AND_MTK) {
                    ...
                    if (i < HwFullNetworkConstantsInner.SIM_NUM) {
                        if (this.mChipOther.mGetUiccCardsStatusDone[i]) {
                            if (this.mChipOther.mUiccCardsStatus != null) {
                                boolean isSubActivated = HwTelephonyManager.getDefault().getSubState((long) i) == 1;
                                ...
                                if (this.mChipOther.mUiccCardsStatus[i] != IccCardStatusExt.CardStateEx.CARDSTATE_PRESENT || this.mChipCommon.isCardPresent(i)) {
                                    if (this.mChipOther.mSimHotPlugIn[i] && !isSubActivated) {
                ...
                if (!ready) {
                    checkIfNeedSetDdsAfterReboot();
                    return false;
                } else if (!this.mChipOther.mHasSetDdsAfterReboot) {
                    logd("ready is true, set mHasSetDdsAfterReboot to true");
                    this.mChipOther.mHasSetDdsAfterReboot = true;
                }
            }
    ...
    public boolean processSimStateChanged() {
        ...
        if (this.mChipOther.is4GSlotReviewNeeded == 1) {
            ...
            isSetDefault4GNeed = hwFullNetworkChipOther.isSetDefault4GSlotNeeded(hwFullNetworkChipOther.mUserPref4GSlot);
    ...
    private boolean processExtraValueNoChange() {
        ...
        if (!this.mChipCommon.judgeSubCarrierTypeByMccMnc(userPref4GSlot)) {
            ...
            hwFullNetworkChipOther.mUserPref4GSlot = userPref4GSlot;
            ...
            return false;
        } else if (this.mChipOther.isSetDefault4GSlotNeeded(userPref4GSlot)) {
    ...
    private void checkIfNeedSetDdsAfterReboot() {
        SubscriptionControllerEx subscriptionControllerEx;
        if (!this.mChipOther.mHasSetDdsAfterReboot) {
            int insertSimCount = 0;
            for (int slotId = 0; slotId < HwFullNetworkConstantsInner.SIM_NUM; slotId++) {
                if (this.mChipOther.mUiccCardsStatus[slotId] == IccCardStatusExt.CardStateEx.CARDSTATE_PRESENT) {
                    insertSimCount++;
                }
            }
            logd("checkIfNeedSetDdsAfterReboot, insert " + insertSimCount + " cards.");
            if (!(insertSimCount == 0 || (subscriptionControllerEx = SubscriptionControllerEx.getInstance()) == null)) {
                int currentDds = subscriptionControllerEx.getDefaultDataSubId();
                if (SubscriptionManagerEx.isValidSubscriptionId(currentDds)) {
                    boolean isMatchAnyCard = false;
                    int slotId2 = 0;
                    while (true) {
                        if (slotId2 >= HwFullNetworkConstantsInner.SIM_NUM) {
                            break;
                        }
                        int subId = SubscriptionManagerEx.getSubIdUsingSlotId(slotId2);
                        logd("checkIfNeedSetDdsAfterReboot, slotId: " + slotId2 + ", subId: " + subId + ", dds: " + currentDds);
                        if (currentDds == subId) {
                            isMatchAnyCard = true;
                            break;
                        }
                    ...
                    if (!isMatchAnyCard) {
                        SubscriptionControllerEx.getInstance().setDefaultDataSubIdBySlotId(this.mChipCommon.getUserSwitchDualCardSlots());
                    }
                    this.mChipOther.mHasSetDdsAfterReboot = true;
                }
            }

...
public class HwFullNetworkChipHisi implements HwFullNetworkChipCommon.HwFullNetworkChipInterface {
    ...
    private boolean isValidSwitchType() {
        int[] iArr = this.mSwitchTypes;
        if (iArr[0] == 3 && iArr[1] == 3) {
            return true;
        }
        int[] iArr2 = this.mSwitchTypes;
        if (iArr2[0] == 3 && iArr2[1] == 2) {
            return true;
        }
        int[] iArr3 = this.mSwitchTypes;
        if (iArr3[0] == 2 && iArr3[1] == 3) {
            return true;
        }
        int[] iArr4 = this.mSwitchTypes;
        if (iArr4[0] == 2 && iArr4[1] == 2) {
            return true;
        }
        int[] iArr5 = this.mSwitchTypes;
        return iArr5[0] == 1 && iArr5[1] == 1;
    }
    ...
    public boolean isSimPresentBySubState(int slotId) {
        ...
        boolean isSubActive = HwTelephonyManager.getDefault().getSubState((long) slotId) == 1;
        boolean haveSimApplication = haveSimApplications(slotId);
        logd("isSimPresentBySubState, slotId = " + slotId + ", isSimPresent = " + isSimPresent + ", subState = " + isSubActive + ",haveSimApplication = " + haveSimApplication);
        if (isSimPresent) {
            return isSubActive || haveSimApplication;
        }
        return false;
    }

    private boolean haveSimApplications(int slotId) {
        UiccCardExt[] uiccCards = mChipCommon.getUiccController().getUiccCards();
        if (uiccCards != null && slotId >= 0 && slotId < uiccCards.length && uiccCards[slotId].getNumApplications() > 0) {
            return true;
        }
        return false;
    }

...
public class HwFullNetworkDefaultStateQcomMtkBase extends HwFullNetworkDefaultStateBase {
    ...
    private NetworkModeSettingsChangeObserver mSettingsChangeObserverForSlot0;
    private NetworkModeSettingsChangeObserver mSettingsChangeObserverForSlot1;

    public HwFullNetworkDefaultStateQcomMtkBase(Context context, CommandsInterfaceEx[] ci, Handler handler) {
        super(context, ci, handler);
        ...
        if (DualNrSwitchHelper.getInstance().isDualNrSwitchFeatureEnabled()) {
            DualNrSwitchHelper.getInstance().init(context);
        }
        initSettingsChangeObserver(handler);
    }

    private void initSettingsChangeObserver(Handler handler) {
        if (HwFullNetworkConfigInner.isSupportFastSetNetworkMode()) {
            this.mSettingsChangeObserverForSlot0 = new NetworkModeSettingsChangeObserver(handler, 0);
            this.mSettingsChangeObserverForSlot1 = new NetworkModeSettingsChangeObserver(handler, 1);
            this.mContext.getContentResolver().registerContentObserver(Settings.Global.getUriFor("preferred_network_mode0"), true, this.mSettingsChangeObserverForSlot0);
            this.mContext.getContentResolver().registerContentObserver(Settings.Global.getUriFor("preferred_network_mode1"), true, this.mSettingsChangeObserverForSlot1);
        }
    }
    ...
    private void syncNetworkTypeFromDB(int subId) {
        if (this.mChipCommon.isValidIndex(subId)) {
            ...
            if (pefMode == -1 || firstStart) {
                ...
                logd("syncNetworkTypeFromDB, subId = " + subId + ", ability = " + ability);
    ...
    private int getPrimaryAndSecondaryStackAbility(int subId) {
        return HwTelephonyManager.getDefault().isNrSupported() ? (!HwNetworkTypeUtils.isDualNrSwitchOpened(this.mContext) && subId != this.mChipCommon.getUserSwitchDualCardSlots()) ? 0 : 1 : (!this.mChipCommon.isDualImsSwitchOpened() && subId != this.mChipCommon.getUserSwitchDualCardSlots()) ? 0 : 1;
    }
    ...
    public void setMainSlot(int slotId, Message responseMsg) {
        if (!SystemPropertiesEx.getBoolean("persist.sys.dualcards", false)) {
            ...
            hwFullNetworkChipOther.is4GSlotReviewNeeded = 2;
            hwFullNetworkChipOther.mUserPref4GSlot = slotId;
    ...
    public void setServiceAbilityForQCOM(int slotId, int type, int ability, int serviceOnMappingMode) {
        getStackPhoneId();
        if (!this.mChipOther.isNeedExchangeDualCardNetworkDb(false) || slotId != this.mChipCommon.getUserSwitchDualCardSlots()) {
            logd("setServiceAbilityForQCOM, dual ims or dual nr, slotId = " + slotId + ", type = " + type + ", ability =" + ability + ", onMode = " + serviceOnMappingMode);
            if (this.mChipOther.mPrimaryStackPhoneId == slotId) {
                this.mChipOther.mPrimaryStackNetworkType = getPrimaryStackNetworkType(type, ability, serviceOnMappingMode);
            } else {
                this.mChipOther.mSecondaryStackNetworkType = getSecondaryStackNetworkType(ability, serviceOnMappingMode);
            }
            recordPrimaryAndSecondaryStackNetworkTypeForCmcc(ability);
            this.mCis[slotId].getPreferredNetworkType(obtainMessage(HwFullNetworkConstantsInner.EVENT_SET_LTE_SERVICE_ABILITY, slotId, 0));
        } else {
            logd("in setServiceAbilityForQCOM, single ims or single nr");
            recordPrimaryAndSecondaryStackNetworkType(type, ability, serviceOnMappingMode);
            this.mCis[this.mChipOther.mPrimaryStackPhoneId].getPreferredNetworkType(obtainMessage(HwFullNetworkConstantsInner.EVENT_SET_LTE_SERVICE_ABILITY, this.mChipOther.mPrimaryStackPhoneId, 0));
        }
        if (DualNrSwitchHelper.getInstance().isDualNrSwitchFeatureEnabled()) {
            recordNrSwrichStatusIfNeed(slotId, type, ability);
        }
    }

    private void recordNrSwrichStatusIfNeed(int slotId, int type, int ability) {
        if (this.mChipCommon.isRetainDualCardNetworkTypeForNr()) {
            boolean z = true;
            if (type == 1) {
                logd("recordNrSwrichStatusIfNeed, record slotId = " + slotId + " ability = " + ability);
                DualNrSwitchHelper instance = DualNrSwitchHelper.getInstance();
                String str = this.mChipCommon.mIccIds[slotId];
                if (ability != 1) {
                    z = false;
                }
                instance.putNrSwitchRecord(str, z);
            }
        }
    }
    ...
    private void getStackPhoneId() {
        ...
        HwFullNetworkChipOther hwFullNetworkChipOther = this.mChipOther;
        if (hwFullNetworkChipOther.mPrimaryStackPhoneId == 0) {
            i = 1;
        }
    ...
    private int getPrimaryStackNetworkType(int type, int ability, int serviceOnMappingMode) {
        ...
        int primaryNetworkType = ability == 1 ? primaryServiceOnNetworkType : primaryServiceOffNetworkType;
        logd("getPrimaryStackNetworkType primaryNetworkType:" + primaryNetworkType);
    ...
    private void recordPrimaryAndSecondaryStackNetworkTypeForCmcc(int ability) {
        if (HwFullNetworkConfigInner.isCMCCDsdxDisable() && this.mChipCommon.isDualImsSwitchOpened() && this.mChipOther.mSetSecondStackCount.get() > 0) {
            ...
            if (this.mChipOther.mPrimaryStackPhoneId == mainSlot) {
                HwFullNetworkChipOther hwFullNetworkChipOther = this.mChipOther;
                hwFullNetworkChipOther.mPrimaryStackNetworkType = mainNetworkMode;
                hwFullNetworkChipOther.mSecondaryStackNetworkType = otherNetworkMode;
            } else if (this.mChipOther.mSecondaryStackPhoneId == mainSlot) {
                HwFullNetworkChipOther hwFullNetworkChipOther2 = this.mChipOther;
                hwFullNetworkChipOther2.mSecondaryStackNetworkType = mainNetworkMode;
                hwFullNetworkChipOther2.mPrimaryStackNetworkType = otherNetworkMode;
            } else {
    ...
    private void setNetworkType(AsyncResultEx ar, int subId) {
        ...
        if (this.mChipOther.mPrimaryStackPhoneId == subId) {
            ...
            if (this.mChipOther.mSecondaryStackNetworkType == -1) {
                HwFullNetworkChipOther hwFullNetworkChipOther = this.mChipOther;
                hwFullNetworkChipOther.mSecondaryStackNetworkType = hwFullNetworkChipOther.getNetworkTypeFromDB(hwFullNetworkChipOther.mSecondaryStackPhoneId);
            }
            Message response2 = obtainMessage(HwFullNetworkConstantsInner.EVENT_SET_SECONDARY_STACK_LTE_SWITCH_DONE, this.mChipOther.mSecondaryStackNetworkType, 0, Integer.valueOf(this.mChipOther.mSecondaryStackPhoneId));
            if (modemNetworkMode == -1 || modemNetworkMode != this.mChipOther.mSecondaryStackNetworkType) {
                this.mCis[this.mChipOther.mSecondaryStackPhoneId].setPreferredNetworkType(this.mChipOther.mSecondaryStackNetworkType, response2);
    ...
    private void handleSetPrimaryStackLteSwitchDone(AsyncResultEx ar, int networkType) {
        ...
        if (ar == null || ar.getException() != null) {
            ...
            this.mChipOther.decrementCountIfNeccessary();
            return;
        }
        ...
        boolean isNeedSetSecondStack = this.mChipOther.isNeedExchangeDualCardNetworkDb(HwFullNetworkConfigInner.isCMCCDsdxDisable());
        if (!HwFullNetworkConfigInner.IS_DUAL_4G_SUPPORTED || isNeedSetSecondStack) {
    ...
    private void handleSetSecondaryStackLteSwitchDone(AsyncResultEx ar, int networkType) {
        ...
        boolean isChipDualAndNeedSave = this.mChipOther.isNeedExchangeDualCardNetworkDb(this.mChipCommon.isCmccHybirdAndNoRoaming());
        if (!HwFullNetworkConfigInner.IS_DUAL_4G_SUPPORTED || isChipDualAndNeedSave) {
            saveNetworkTypeToDB();
    ...
    private void saveNetworkTypeToDB(int subId) {
        ...
        if (subId == this.mChipOther.mPrimaryStackPhoneId) {
            ...
            if (curPrefMode != this.mChipOther.mSetPrimaryStackPrefMode && this.mChipOther.mSetPrimaryStackPrefMode != -1) {
                HwFullNetworkChipOther hwFullNetworkChipOther = this.mChipOther;
                hwFullNetworkChipOther.setNetworkTypeToDB(subId, hwFullNetworkChipOther.mSetPrimaryStackPrefMode);
                return;
            }
            return;
        }
        ...
        if (curPrefMode != this.mChipOther.mSetSecondaryStackPrefMode && this.mChipOther.mSetSecondaryStackPrefMode != -1) {
            HwFullNetworkChipOther hwFullNetworkChipOther2 = this.mChipOther;
            hwFullNetworkChipOther2.setNetworkTypeToDB(subId, hwFullNetworkChipOther2.mSetSecondaryStackPrefMode);
        }
    }
    ...
    private void rollbackPrimaryStackPrefNetworkType() {
        HwFullNetworkChipOther hwFullNetworkChipOther = this.mChipOther;
        int curPrefMode = hwFullNetworkChipOther.getNetworkTypeFromDB(hwFullNetworkChipOther.mPrimaryStackPhoneId);
        ...
        if (HwFullNetworkConfigInner.isCMCCDsdxDisable() && cmccSlotId != -1 && intent != null) {
            ...
            if (slotId == cmccSlotId && this.mChipCommon.getCombinedRegState(serviceState) == 0) {
                ...
                if (oldRoamingState != newRoamingState) {
                    ...
                    hwFullNetworkChipOther.setServiceAbility();
                }
            }
            if (slotId == cmccSlotId) {
                if (this.mChipCommon.mCmccSubIdOldState != 0 && this.mChipCommon.getCombinedRegState(serviceState) == 0) {
                    ...
                    HwFullNetworkChipOther hwFullNetworkChipOther2 = this.mChipOther;
                    hwFullNetworkChipOther2.mNeedSetLteServiceAbility = true;
                    hwFullNetworkChipOther2.setServiceAbility();
                }
    ...
    public class NetworkModeSettingsChangeObserver extends ContentObserver {
        private int mSlotId;

        NetworkModeSettingsChangeObserver(Handler handler, int slotId) {
            super(handler);
            this.mSlotId = slotId;
            HwFullNetworkDefaultStateQcomMtkBase.this.logd("NetworkModeSettingsChangeObserver, init slotId = " + slotId);
        }

        @Override // android.database.ContentObserver
        public void onChange(boolean selfChange) {
            int value = HwFullNetworkDefaultStateQcomMtkBase.this.mChipOther.getNetworkTypeFromDB(this.mSlotId);
            SystemPropertiesEx.set(HwFullNetworkConstantsInner.PERSIST_RADIO_NETWORK_MODE + this.mSlotId, String.valueOf(value));
            HwFullNetworkDefaultStateQcomMtkBase hwFullNetworkDefaultStateQcomMtkBase = HwFullNetworkDefaultStateQcomMtkBase.this;
            hwFullNetworkDefaultStateQcomMtkBase.logd("NetworkModeSettingsChangeObserver, set slotId " + this.mSlotId + " to value " + value);
        }
    }
}

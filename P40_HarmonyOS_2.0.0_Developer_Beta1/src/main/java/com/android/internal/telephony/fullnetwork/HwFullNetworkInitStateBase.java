...
public abstract class HwFullNetworkInitStateBase extends Handler {
    ...
    private boolean isNeedToGetIccId() {
        if (!DualNrSwitchHelper.getInstance().isDualNrSwitchFeatureEnabled() && !HwFullNetworkConfigInner.isCMCCDsdxEnable() && !HwFullNetworkConfigInner.IS_CT_4GSWITCH_DISABLE && HwTelephonyManagerInner.getDefault().getDefaultMainSlotCarrier() == 0) {
            return false;
        }
        return true;
    }

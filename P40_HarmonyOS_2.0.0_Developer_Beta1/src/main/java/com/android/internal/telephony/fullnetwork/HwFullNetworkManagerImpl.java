...
import com.huawei.internal.telephony.smartnet.HwSmartNetStateMachine;
...
public class HwFullNetworkManagerImpl extends DefaultHwFullNetworkManager {
    ...
    public void makeHwFullNetworkManager(Context context, CommandsInterfaceEx[] ci) {
        synchronized (LOCK) {
            sInstance = new HwFullNetworkManagerImpl(context, ci);
            if (HwFullNetworkConstantsInner.SIM_NUM > 1) {
                mDualCardSwitcher = HwDualCardSwitcher.make(context);
                if (SystemPropertiesEx.getBoolean("persist.smartnet.on", false)) {
                    logd("make HwSmartNetStateMachine");
                    HwSmartNetStateMachine.make(context);
                }
            }
        }
    }
    ...
    public void onGetIccCardStatusDone(Object ar, Integer index) {
        HwFullNetworkInitStateBase hwFullNetworkInitStateBase = mInitStateBase;
        if (hwFullNetworkInitStateBase != null) {
            hwFullNetworkInitStateBase.onGetIccCardStatusDone(ar, index);
        }
    }
    ...
    public void setDefault4GSlotForMDM() {
        ...
        this.mStateHandler.sendMessage(handler.obtainMessage(204, 0));
        HwDualCardSwitcher hwDualCardSwitcher = mDualCardSwitcher;
        if (hwDualCardSwitcher != null) {
            hwDualCardSwitcher.stop();
        }
    }
    ...
    public void setLteServiceAbilityForQCOM(int subId, int ability, int lteOnMappingMode) {
        HwFullNetworkDefaultStateBase hwFullNetworkDefaultStateBase = mDefaultStateBase;
        if (hwFullNetworkDefaultStateBase != null) {
            hwFullNetworkDefaultStateBase.setLteServiceAbilityForQCOM(subId, ability, lteOnMappingMode);
        }
    }
    ...
    public void setServiceAbilityForQCOM(int slotId, int type, int ability, int serviceOnMappingMode) {
        HwFullNetworkDefaultStateBase hwFullNetworkDefaultStateBase = mDefaultStateBase;
        if (hwFullNetworkDefaultStateBase != null) {
            hwFullNetworkDefaultStateBase.setServiceAbilityForQCOM(slotId, type, ability, serviceOnMappingMode);
        }
    }
    ...
    public void setMainSlot(int slotId, Message responseMsg) {
        ...
        HwFullNetworkDefaultStateBase hwFullNetworkDefaultStateBase = mDefaultStateBase;
        if (hwFullNetworkDefaultStateBase != null) {
            hwFullNetworkDefaultStateBase.setMainSlot(slotId, responseMsg);
    ...
    public void setSimContactLoaded(int slotId, boolean loaded) {
        mChipCommon.setSimContactLoaded(slotId, loaded);
    }

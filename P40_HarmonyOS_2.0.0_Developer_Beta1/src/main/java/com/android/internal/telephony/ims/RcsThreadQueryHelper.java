...
public class RcsThreadQueryHelper {
    ...
    public RcsThreadQueryResultParcelable performThreadQuery(Bundle bundle) throws RemoteException {
        ...
        if (cursor != null) {
            while (cursor.moveToNext()) {
                if (cursor.getInt(cursor.getColumnIndex("thread_type")) == THREAD_ID_INDEX_IN_INSERTION_URI ? THREAD_ID_INDEX_IN_INSERTION_URI : false) {
                    rcsThreadIdList.add(new RcsTypeIdPair((int) THREAD_ID_INDEX_IN_INSERTION_URI, cursor.getInt(cursor.getColumnIndex("rcs_thread_id"))));
                } else {
    ...
    public int create1To1Thread(int participantId) throws RemoteException {
        ContentValues contentValues = new ContentValues((int) THREAD_ID_INDEX_IN_INSERTION_URI);
    ...
    public int createGroupThread(String groupName, Uri groupIcon) throws RemoteException {
        ...
        if (groupUri != null) {
            return Integer.parseInt(groupUri.getPathSegments().get(THREAD_ID_INDEX_IN_INSERTION_URI));
        }

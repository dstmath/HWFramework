...
public class HwIccUtils {
        ...
        if (length < 1 || data[offset] != Byte.MIN_VALUE) {
            ...
            if (isucs2) {
                ...
                while (len > 0 && (offset + len) - 1 < data.length) {
                    if (data[offset] < 0) {
                        ret.append((char) ((data[offset] & Byte.MAX_VALUE) + base));
                        offset++;
                        len--;
                    }
                    int count = 0;
            ...
            String defaultCharset = BuildConfig.FLAVOR;
            try {
                defaultCharset = resource.getString(HwPartResourceUtils.getResourceId("gsm_alphabet_default_charset"));
            } catch (Resources.NotFoundException e) {
            }
            return HwGsmAlphabet.gsm8BitUnpackedToString(data, offset, length, defaultCharset.trim(), true);
        }
        String ret2 = new String(data, offset + 1, ((length - 1) / 2) * 2, StandardCharsets.UTF_16BE);
        int ucslen = ret2.length();
        while (ucslen > 0 && ret2.charAt(ucslen - 1) == 65535) {
            ucslen--;
        }
        return ret2.substring(0, ucslen);
    }
    ...
    public static String bcdIccidToString(byte[] data, int offset, int length) {
        ...
        for (int i = offset; i < offset + length; i++) {
            int v = data[i] & 15;
            if (v > MDN_NUMBER_TYPE_INDEX) {
                ret.append(cnum[v - 10]);
            } else {
                ret.append((char) (v + 48));
            }
            int v2 = (data[i] >> 4) & 15;
            if (v2 > MDN_NUMBER_TYPE_INDEX) {
    ...
    public static byte[] buildAdnStringHw(int recordSize, String mAlphaTag, String mNumber) {
        ...
        if (mNumber3 == null) {
            ...
            if (bestConverts < ucs81Converts) {
                ...
                bestMode = HwPhoneNumberUtils.TOA_Unknown;
                bestLen = ucs81Len;
            }
            ...
            if (bestConverts < ucs80Converts2) {
                bestConverts = ucs80Converts2;
                bestMode = HwSmsMessage.SMS_TOA_UNKNOWN;
                bestLen = ucs80Len;
            }
            String mAlphaTag2 = mAlphaTag.substring(0, bestConverts);
            if (bestLen > footerOffset) {
                mAlphaTag2 = mAlphaTag2.substring(0, bestConverts - 1);
                bestLen -= 2;
            }
            ...
            if (bestMode != 0) {
                switch (bestMode) {
                    case HwSmsMessage.SMS_TOA_UNKNOWN /* 128 */:
                        byte[] byteTag = mAlphaTag2.getBytes(StandardCharsets.UTF_16BE);
                        adnString[0] = Byte.MIN_VALUE;
                        System.arraycopy(byteTag, 0, adnString, 1, byteTag.length);
                        break;
                    case HwPhoneNumberUtils.TOA_Unknown /* 129 */:
    ...
    public static boolean arrayCompareNullEqualsEmpty(String[] s1, String[] s2) {
        ...
        if (s1 == null) {
            s1 = new String[]{BuildConfig.FLAVOR};
        }
        if (s2 == null) {
            s2 = new String[]{BuildConfig.FLAVOR};
        }
        for (String str : s1) {
    ...
    public static String cdmaDTMFToString(byte[] data, int offset, int length) {
        ...
        StringBuilder ret = new StringBuilder();
        if (11 == data.length && 1 == (data[MDN_NUMBER_TYPE_INDEX] & 1)) {
            ret.append('+');
        }
        int count = 0;
        int i = offset;
    ...
    public static char intToCdmaDTMFChar(int c) {
        if (c >= 0 && c <= MDN_NUMBER_TYPE_INDEX) {
            return (char) (c + 48);
        }
        if (c == 10) {
            return '0';
    ...
    public static String prependPlusInLongAdnNumber(String Number) {
        ...
        if (!m.matches()) {
            ...
            } else if (BuildConfig.FLAVOR.equals(m2.group(2))) {
                ret = new StringBuilder();
        ...
        } else if (BuildConfig.FLAVOR.equals(m.group(2))) {
            ret = new StringBuilder();
    ...
    public static String cdmaBcdToStringHw(byte[] data, int offset, int length) {
        ...
        if (length > 0 && data.length > MDN_NUMBER_TYPE_INDEX) {
            prependPlus = data[MDN_NUMBER_TYPE_INDEX] == MDN_NUMBER_TYPE_INDEX;
        }
        if (prependPlus) {
            ret.append('+');
        }

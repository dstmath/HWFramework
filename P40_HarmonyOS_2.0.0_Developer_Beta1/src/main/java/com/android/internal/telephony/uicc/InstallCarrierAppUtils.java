...
public class InstallCarrierAppUtils {
    ...
    static void showNotification(Context context, String pkgName) {
        ...
        String title = res.getString(17040304);
        String appName = getAppNameFromPackageName(context, pkgName);
        boolean persistent = true;
        if (TextUtils.isEmpty(appName)) {
            message = res.getString(17040302);
        } else {
            message = res.getString(17040303, appName);
        }
        String downloadButtonText = res.getString(17040301);
        if (Settings.Global.getInt(context.getContentResolver(), "install_carrier_app_notification_persistent", 1) != 1) {
            persistent = false;
        }
        getNotificationManager(context).notify(pkgName, 12, new Notification.Builder(context, NotificationChannelController.CHANNEL_ID_SIM).setContentTitle(title).setContentText(message).setSmallIcon(17302825).addAction(new Notification.Action.Builder((Icon) null, downloadButtonText, PendingIntent.getActivity(context, 0, getPlayStoreIntent(pkgName), 201326592)).build()).setOngoing(persistent).setVisibility(-1).build());
    }

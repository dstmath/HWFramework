...
public class UiccProfile extends IccCard implements IUiccProfileInner {
    ...
    static void broadcastIccStateChangedIntent(String value, String reason, int phoneId) {
        ...
        putEsimSlot1ExtraForHotPlugStateChanged(intent);
        ...
        synchronized (this.mLock) {
            if (!SubscriptionManager.isValidSlotIndex(this.mPhoneId)) {
                ...
                if (this.mExternalState == IccCardConstants.State.LOADED && this.mIccRecords != null) {
                    ...
                    sb.append("setExternalState: operator=");
                    ...
                    if (!TextUtils.isEmpty(operator)) {
                        ...
                        if (countryCode != null) {
                            try {
                                ...
                                sb2.append("countryCodeForMcc error for countryCode = ");
                                sb2.append(HW_DBG ? countryCode : "***");
                                loge(sb2.toString());
                            }
                        } else {
    ...
    private static void putEsimSlot1ExtraForHotPlugStateChanged(Intent intent) {
        HwTelephonyFactory.getHwUiccManager().putEsimSlot1ExtraForHotPlugStateChanged(intent);
    }
}

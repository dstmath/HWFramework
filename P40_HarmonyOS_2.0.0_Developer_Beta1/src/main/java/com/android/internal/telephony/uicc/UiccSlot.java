...
public class UiccSlot extends Handler {
    ...
    private void onIccSwap(boolean isAdded) {
        if (this.mContext.getResources().getBoolean(17891469)) {
    ...
    private void promptForRestart(boolean isAdded) {
        if (!VSimUtilsInner.isPlatformTwoModems() || (!VSimUtilsInner.isVSimSlot(this.mPhoneId) && !VSimUtilsInner.isVSimInProcess() && !VSimUtilsInner.isVSimOn() && !VSimUtilsInner.isVSimCauseCardReload())) {
            synchronized (this.mLock) {
                String dialogComponent = this.mContext.getResources().getString(17039849);

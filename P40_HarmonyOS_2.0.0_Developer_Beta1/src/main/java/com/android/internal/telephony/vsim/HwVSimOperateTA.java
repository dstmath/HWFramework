...
public class HwVSimOperateTA {
    ...
    private int writeToTa(String imsi, int cardType, int apnType, String challenge, String taPath, int vsimLoc, int modemId) {
        int taResult = this.mHwVSimOperateTANative.operTANative(43, 1, cardType, apnType, challenge, imsi, taPath, vsimLoc, modemId);
        if (taResult == 0) {
            return 0;
        }
        if (taResult == 1 || taResult != 2) {
            return 1;
        }
        return 6;
    }
    ...
    private int writeApnToTa(String imsi, int cardType, int apnType, String challenge, String taPath) {
        int taResult = this.mHwVSimOperateTANative.operTANative(43, 2, cardType, apnType, challenge, imsi, taPath, 0, 0);
        if (taResult == 0) {
            return 0;
        }
        if (taResult == 1 || taResult != 2) {
    ...
    private int clearToTa() {
        int taResult = this.mHwVSimOperateTANative.operTANative(43, 3, 0, 0, "000", (String) null, (String) null, 0, 0);
        if (taResult == 0) {
            return 0;
        }
        if (taResult != 1) {
    ...
    public int operateTA(int operation, String imsi, int cardType, int apnType, String challenge, boolean isForHash, String taPath, int vsimLoc, int modemId) {
        ...
        if (operation == 1) {
            ...
            result = clearToTa();
        } else if (operation == 4 || operation == 51 || operation == 52 || operation == 100) {

...
public class HwVSimSlotSwitchController extends StateMachineEx {
    ...
    private static final int SUB_COUNT;
    private static final int SUB_VSIM;
    ...
    public void restartRildBySubState() {
        ...
        int slotId = 0;
        while (true) {
            boolean needRadioPowerOn = false;
            if (slotId >= PHONE_COUNT) {
                break;
            }
            if (mVSimController.getSubState(slotId) != 0) {
                needRadioPowerOn = true;
            }
            if (needRadioPowerOn) {
    ...
    private class InitialState extends StateEx {
        ...
        public boolean processMessage(Message msg) {
            ...
            } else if (HwVSimSlotSwitchController.this.mSlotSwitchDone) {
                onCheckSimMode(msg);
                return true;
            } else {
        ...
        private void checkSwitchSlotDone() {
            ...
            if (HwVSimSlotSwitchController.this.mIsVSimEnabled) {
                ...
                HwVSimSlotSwitchController hwVSimSlotSwitchController = HwVSimSlotSwitchController.this;
                hwVSimSlotSwitchController.transitionTo(hwVSimSlotSwitchController.mDefaultState);
                return;
            }
            checkSimMode();
        }

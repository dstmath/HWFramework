...
public class HwVSimDefaultProcessor extends HwVSimHisiProcessor {
    ...
    public int networksScan(int subId, int type) {
        this.mModemAdapter.networksScan(this, subId);
        return 0;
    }
    ...
    private void onSendOpenSessionConfigDone(Message msg) {
        ...
        if (!(argument instanceof HwVSimConstants.EnableParam)) {
            closeChipSessionWhenOpenFailOrTimeout(request, 1, false);
            return;
        }
        HwVSimConstants.EnableParam param = (HwVSimConstants.EnableParam) argument;

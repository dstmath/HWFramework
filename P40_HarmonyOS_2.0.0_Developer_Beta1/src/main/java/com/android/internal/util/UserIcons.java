...
public class UserIcons {
    ...
    private static Drawable createRoundPhotoForHonor(Resources resources, int userId) {
        ...
        SweepGradient sweepGradient = new SweepGradient(((float) width) / 2.0f, ((float) height) / 2.0f, sweepCircleColorValues, SWEEP_CICLE_POSITIONS);
        Matrix matrix = new Matrix();
        matrix.setRotate(0.0f, ((float) width) / 2.0f, ((float) height) / 2.0f);
        sweepGradient.setLocalMatrix(matrix);
        paint.setShader(sweepGradient);
        canvas.drawArc(new RectF(sweepStrokeWidth / 2.0f, sweepStrokeWidth / 2.0f, ((float) width) - (sweepStrokeWidth / 2.0f), ((float) height) - (sweepStrokeWidth / 2.0f)), 0.0f, 360.0f, false, paint);

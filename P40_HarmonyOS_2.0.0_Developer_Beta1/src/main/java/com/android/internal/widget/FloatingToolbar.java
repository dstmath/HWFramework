...
public final class FloatingToolbar {
    ...
    private void doShow() {
        ...
        if (IS_CHINA_AREA) {
            removeGoogleTranslation(menuItems);
        }
    ...
    private void removeGoogleTranslation(List<MenuItem> menuItems) {
        ComponentName component;
        int googleTranslationFirstIndex = -1;
        int googleTranslationCounter = 0;
        for (int i = 0; i < menuItems.size(); i++) {
            Intent intent = menuItems.get(i).getIntent();
            if (!(intent == null || (component = intent.getComponent()) == null || !"com.google.android.apps.translate".equals(component.getPackageName()))) {
                if (googleTranslationFirstIndex == -1) {
                    googleTranslationFirstIndex = i;
                }
                googleTranslationCounter++;
            }
        }
        if (googleTranslationCounter > 1) {
            menuItems.remove(googleTranslationFirstIndex);
        }
    }
    ...
    private void processTextMenu(MenuItem menuItem) {
        ...
        if (intent != null) {
            ...
            if (Intent.ACTION_PROCESS_TEXT.equals(action) && info != null) {
                ...
                if (IS_CHINA_AREA && "com.huawei.hitouch".equals(info.activityInfo.packageName)) {

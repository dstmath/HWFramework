...
public class MessagingPropertyAnimator implements View.OnLayoutChangeListener {
    ...
    private static final int TAG_ALPHA_ANIMATOR = 16909469;
    private static final int TAG_FIRST_LAYOUT = 16909470;
    private static final int TAG_LAYOUT_TOP = 16909471;
    private static final int TAG_TOP = 16909473;
    private static final int TAG_TOP_ANIMATOR = 16909472;
    ...
    private static boolean isFirstLayout(View view) {
        Boolean tag = (Boolean) view.getTag(16909470);
        if (tag == null) {
            return true;
        }
    ...
    private static void setFirstLayout(View view, boolean first) {
        view.setTagInternal(16909470, Boolean.valueOf(first));
    }

    private static void setLayoutTop(View view, int top) {
        view.setTagInternal(16909471, Integer.valueOf(top));
    }

    public static int getLayoutTop(View view) {
        Integer tag = (Integer) view.getTag(16909471);
        if (tag == null) {
            return getTop(view);
        }
        return tag.intValue();
    ...
    public static int getTop(View v) {
        Integer tag = (Integer) v.getTag(16909473);
        if (tag == null) {
            return v.getTop();
        }
        return tag.intValue();
    ...
    public static void setTop(View v, int value) {
        v.setTagInternal(16909473, Integer.valueOf(value));
    ...
    private static void startTopAnimation(final View v, int start, int end, Interpolator interpolator) {
        ObjectAnimator existing = (ObjectAnimator) v.getTag(16909472);
        if (existing != null) {
            existing.cancel();
        }
        ...
        animator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                View.this.setTagInternal(16909472, null);
        ...
        v.setTagInternal(16909472, animator);
        animator.start();
    }
    ...
    public static void fadeIn(final View v) {
        ObjectAnimator existing = (ObjectAnimator) v.getTag(16909469);
        if (existing != null) {
            existing.cancel();
        }
    ...
    public static void fadeOut(final View view, final Runnable endAction) {
        ObjectAnimator existing = (ObjectAnimator) view.getTag(16909469);
        if (existing != null) {
            existing.cancel();
        }
        ...
        view.setTagInternal(16909469, animator);
        animator.start();
    }
    ...
    public static boolean isAnimatingTranslation(View v) {
        return v.getTag(16909472) != null;
    }

    public static boolean isAnimatingAlpha(View v) {
        return v.getTag(16909469) != null;
    }

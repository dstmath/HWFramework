...
public class SwipeDismissLayout extends FrameLayout {
    ...
    private void updateDismiss(MotionEvent ev) {
        ...
        if (!this.mDismissed && ((deltaX > ((float) getWidth()) * Math.max(Math.min(((-0.23000002f * xVelocity) / ((float) this.mMinFlingVelocity)) + MAX_DIST_THRESHOLD, (float) MAX_DIST_THRESHOLD), (float) MIN_DIST_THRESHOLD) && ev.getRawX() >= this.mLastX) || xVelocity >= ((float) this.mMinFlingVelocity))) {

...
public class HwConnectivityExService extends IConnectivityExManager.Stub {
    ...
    private static boolean sIsUseCtrlSocket = SystemProperties.getBoolean("ro.config.hw_useCtrlSocket", false);
    private static String sSmartKeyguardLevel = "normal_level";
    ...
    private static void setStaticSmartKeyguardLevel(String level) {
        sSmartKeyguardLevel = level;
        Slog.d(TAG, "set smartKeyguardLevel = " + sSmartKeyguardLevel);
    }
    ...
    private static void setUseCtrlSocketStatic(boolean isFlag) {
        sIsUseCtrlSocket = isFlag;
    }

    public void setUseCtrlSocket(boolean isFlag) {
        this.mContext.enforceCallingOrSelfPermission("android.permission.CONNECTIVITY_INTERNAL", TAG);
        setUseCtrlSocketStatic(isFlag);

...
public class HwConnectivityManagerImpl implements HwConnectivityManager {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent != null && (action = intent.getAction()) != null) {
                if (SmartDualCardConsts.ACTION_CONNECTIVITY_CHANGE.equals(action)) {
                    ...
                    Log.d(HwConnectivityManagerImpl.LOG_TAG, "other action");
                }
            }
        }
    };
    ...
    public void setTetheringProp(Tethering tetheringService, boolean tethering, boolean usb, String ifaceName) {
        ...
        try {
            ...
            if (usb) {
                ...
                String str3 = TAG;
                Log.d(str3, "usb is not exist" + prop);
            } else {
                SystemProperties.set(PROPERTY_BTHOTSPOT_ON, prop);
                String str4 = TAG;
                Log.d(str4, "set isbthotspoton = " + prop);
            }
        } catch (RuntimeException e) {
            String str5 = TAG;
            Log.e(str5, "when setTetheringProp ,error  ifaceNmae =" + ifaceName);
        }
    }
        ...
        public void handleMessage(Message msg) {
            ...
            if (i != 0) {
                ...
                try {
                    ...
                    Log.e(HwConnectivityManagerImpl.LOG_TAG, "DnsEventHandler ON_DNS_EVENT_MSG call SystemProperties.set fail.");
                }
    ...
    private void onDnsReportFailProcess(Context context, DnsQueryStat dnsStat, int returnCode, int latencyMs, int netId) {
        if (this.mDataServiceQoeDnsMonitor != null) {
            ...
            try {
                ...
                Log.e(LOG_TAG, "onDnsReportFailProcess call SystemProperties.set fail.");
            }
        }
    }
    ...
    private void sendIntentPsSlowDnsFail(Context context, int returnCode, int latencyMs, int netId) {
        if (netId == cellNetId) {
            ...
            if (!this.mSendDnsFailFlag) {
                ...
                sendChrBroadcastWhenPsSlowDnsFail(context, returnCode, latencyMs, now);
    ...
    private void sendChrBroadcastWhenPsSlowDnsFail(Context context, int returnCode, int latencyMs, Date now) {
    ...
    private void sendIntentPsSlowDnsOver2000(Context context, int delay, int netId) {
        if (netId == cellNetId) {
            ...
            if (!this.mSendDnsOver2000Flag) {
                ...
                sendChrBroadcastWhenPsSlowDnsOver(context, delay, now);
    ...
    private void sendChrBroadcastWhenPsSlowDnsOver(Context context, int delay, Date now) {
    ...
    private boolean isHostnameMatches(String hostName, String hostNamePolicy) {
        int index;
        if (TextUtils.isEmpty(hostName) || TextUtils.isEmpty(hostNamePolicy) || hostNamePolicy.length() > hostName.length() || (index = hostName.lastIndexOf(hostNamePolicy)) != hostName.length() - hostNamePolicy.length() || (index != 0 && hostName.charAt(index - 1) != '.')) {
            return false;
        }
        return true;
    }

...
public class HwNetworkManagementService extends NetworkManagementService {
    ...
    private static final int CODE_SET_IP_FWD_INTERFACE_CMD = 1125;
    ...
    private static final String[] DEFAULT_NETWORK_APP_TRUST_LIST = {HwCustPkgNameConstant.HW_HWOUC_PACKAGE, "com.huawei.hwid"};
    ...
    private static boolean sResetNetworkAppTrustList = false;
    ...
    public boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
        if (code == 201) {
            ...
            if (code == CODE_SET_AD_STRATEGY_RULE) {
                ...
                reply.writeInt(pid);
                return true;
            } else if (code == CODE_SET_CHR_REPORT_APP_LIST) {
                ...
                reply.writeNoException();
                return true;
            } else if (code == CODE_SET_IP_FWD_INTERFACE_CMD) {
                Slog.d(TAG, "code == CODE_SET_IP_FWD_INTERFACE_CMD");
                data.enforceInterface(DESCRIPTOR_NETWORKMANAGEMENT_SERVICE);
                this.mContext.enforceCallingOrSelfPermission("android.permission.CONNECTIVITY_INTERNAL", TAG);
                if (data.readInt() != 0) {
                    needReset = true;
                }
                setIpForwardingInterface(needReset, data.readString(), data.readString());
                reply.writeNoException();
                return true;
            } else if (code != CODE_SET_APP_NETWORK_ACCESS_LIST) {
                return HwNetworkManagementService.super.onTransact(code, data, reply, flags);
            } else {
                setAppNetworkAccessListForTransact(data, reply);
                return true;
            }
        }
    }
    ...
    public String getIgnorebroadcastssid() {
        if (SettingsEx.Systemex.getInt(this.mContext.getContentResolver(), "show_broadcast_ssid_config", 0) != 1) {
            return "broadcast";
        }
    ...
    private void notifyApLinkedStaListChange(String event, String macStr) {
        ...
        if (STA_JOIN_EVENT.equals(event)) {
            this.mApLinkedStaHandler.sendMessageDelayed(msg, HwSwingMotionGestureConstant.HOVER_SCREEN_OFF_THRESHOLD);
                ...
                while (n < userCount3) {
                    try {
                        ...
                        if (code == i4) {
                            ...
                            userCount2 = userCount3;
                            uid = uid2;
                            if (code == CODE_SET_APK_CONTROL_STRATEGY) {
                                try {
                                    String str2 = TAG;
                                    StringBuilder sb2 = new StringBuilder();
                                    pm2 = pm3;
    ...
    private void initNetworkAccessList() {
        ...
        final int networkAppPolicyFlag = getNetworkAppPolicyFlag();
        if (networkPolicyFlag != -1 || networkAppPolicyFlag != 1) {
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (networkAppPolicyFlag != 1) {
                        HwNetworkManagementService.this.initAppNetworkAccessList();
                    }
                }
            }, "initIptables").start();
        }
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void initAppNetworkAccessList() {
        setAppNetworkAccessList(HwDeviceManager.getList(74), 2);
    }
    ...
    private int getNetworkAppPolicyFlag() {
        ...
        if (TextUtils.isEmpty(flagStr)) {
            Slog.d(TAG, "getNetworkAppPolicyFlag flagStr null");
            return 1;
        }
        try {
            ...
            String str = TAG;
            Slog.e(str, "getNetworkAppPolicyFlag parseInt error flagStr = " + flagStr);
            return 1;
        }
    }

    private void setAppNetworkAccessListForTransact(Parcel data, Parcel reply) {
        Slog.d(TAG, "code == CODE_SET_APP_NETWORK_ACCESS_LIST");
        data.enforceInterface(DESCRIPTOR_NETWORKMANAGEMENT_SERVICE);
        this.mContext.enforceCallingOrSelfPermission("com.huawei.permission.sec.MDM_NETWORK_MANAGER", TAG);
        List<String> appNetworkAccessList = new ArrayList<>();
        data.readStringList(appNetworkAccessList);
        int trustOrBlock = data.readInt();
        int setOrRemove = data.readInt();
        if (appNetworkAccessList.size() < 1 && setOrRemove == 1) {
            clearAppNetworkAccessList(trustOrBlock);
            String str = TAG;
            Slog.i(str, "setAppNetworkAccessListForTransact clear uid rules trustOrBlock " + trustOrBlock);
        } else if (setOrRemove == 0) {
            setAppNetworkAccessList(appNetworkAccessList, trustOrBlock);
        } else if (setOrRemove == 1) {
            removeAppNetworkAccessList(appNetworkAccessList, trustOrBlock);
        } else {
            Slog.d(TAG, "addOrRemove uid error");
        }
        reply.writeNoException();
    }

    private void initDefaultTrustList(List<String> setUidList, int trustOrBlock) {
        if (trustOrBlock == 2 && !sResetNetworkAppTrustList) {
            setUidList.addAll(Arrays.asList(DEFAULT_NETWORK_APP_TRUST_LIST));
            Slog.i(TAG, "setAppNetworkAccessList add default trust app to uid trust rules");
            sResetNetworkAppTrustList = true;
        }
    }

    private ArrayList<String> getUidList(List<String> appTrustList) {
        ArrayList<String> appUidList = new ArrayList<>();
        for (int i = 0; i < appTrustList.size(); i++) {
            String uid = getUidByPkgName(appTrustList.get(i));
            if (!TextUtils.isEmpty(uid)) {
                appUidList.add(uid);
            } else {
                String str = TAG;
                Slog.i(str, appTrustList.get(i) + " is not install, ignore uid rule");
            }
        }
        return appUidList;
    }

    private void setAppNetworkAccessList(List<String> appTrustList, int trustOrBlock) {
        if (appTrustList == null || appTrustList.size() <= 0) {
            Slog.d(TAG, "setAppNetworkAccessList fail");
            return;
        }
        initDefaultTrustList(appTrustList, trustOrBlock);
        try {
            this.mOemNetdService.HwNetFilterAddUidRules((String[]) getUidList(appTrustList).toArray(new String[0]), trustOrBlock);
            Slog.i(TAG, "setAppNetworkAccessList sucess");
        } catch (RemoteException | ServiceSpecificException e) {
            Slog.e(TAG, "setAppNetworkAccessList occur Exception");
        }
    }

    private void removeAppNetworkAccessList(List<String> appTrustList, int trustOrBlock) {
        if (appTrustList == null || appTrustList.size() <= 0) {
            Slog.d(TAG, "removeAppNetworkAccessList fail");
            return;
        }
        try {
            this.mOemNetdService.HwNetFilterRemoveUidRules((String[]) getUidList(appTrustList).toArray(new String[0]), trustOrBlock);
            Slog.i(TAG, "removeAppNetworkAccessList sucess");
        } catch (RemoteException | ServiceSpecificException e) {
            Slog.e(TAG, "removeAppNetworkAccessList occur Exception");
        }
    }

    private void clearAppNetworkAccessList(int trustOrBlock) {
        try {
            this.mOemNetdService.HwNetFilterClearUidRules(trustOrBlock);
            Slog.i(TAG, "clearAppNetworkAccessList sucess");
        } catch (RemoteException | ServiceSpecificException e) {
            Slog.e(TAG, "clearAppNetworkAccessList occur Exception");
        }
        if (trustOrBlock == 2 && sResetNetworkAppTrustList) {
            sResetNetworkAppTrustList = false;
            Slog.i(TAG, "clear uid trust rules, init defalut app network uid trustlist");
        }
    }

    private String getUidByPkgName(String pkgName) {
        if (TextUtils.isEmpty(pkgName)) {
            return "";
        }
        try {
            return String.valueOf(this.mContext.getPackageManager().getApplicationInfo(pkgName, 0).uid);
        } catch (PackageManager.NameNotFoundException e) {
            Slog.e(TAG, "getUidByPkgName occur Exception");
            return "";
        }
    }
    ...
    private void setIpForwardingInterface(boolean isEnable, String srcAddress, String ifaceName) {
        if (!TextUtils.isEmpty(srcAddress) && !TextUtils.isEmpty(ifaceName) && this.mOemNetdService != null) {
            try {
                String str = TAG;
                Slog.d(str, "set interface ip forwarding, enable?" + isEnable + ",iface name:" + ifaceName);
                this.mOemNetdService.SetIpForwardingInterface(isEnable, srcAddress, ifaceName);
            } catch (RemoteException e) {
                Slog.d(TAG, "set ip forward exception.");
            }
        }
    }
}

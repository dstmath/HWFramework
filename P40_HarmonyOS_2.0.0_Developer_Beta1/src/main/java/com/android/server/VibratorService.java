...
import com.android.server.HwHapticPlayerService;
...
public class VibratorService extends IVibratorService.Stub implements InputManager.InputDeviceListener {
    ...
    private static final long SEND_AUDIO_DELAY_TIME = 10000;
    ...
    private HwHapticPlayerService mHwHapticPlayerService;
    ...
    public static native int playHapticSlice(int i, int i2, int i3, float f, float f2);

    public static native int setCurvePoint(int i, int i2, float f);
    ...
    private void sendAudioVibratorStateOn() {
        ...
        if (handler != null) {
            ...
            this.mH.postDelayed(this.mSendVibratorStateOffRunnable, 10000);
        }
    }
    ...
    VibratorService(Context context) {
        ...
        this.mAllowPriorityVibrationsInLowPowerMode = this.mContext.getResources().getBoolean(17891346);
        ...
        this.mHwHapticPlayerService = new HwHapticPlayerService(context, this, this.mLock);
    }
                ...
                synchronized (obj2) {
                    try {
                        ...
                        if (this.mCurrentExternalVibration != null) {
                            ...
                            obj = obj2;
                            try {
                                vib = new Vibration(token, effect, usageHint, uid, opPkg, reason);
                                if (effect.getType() == HwHapticPlayerService.HAPTIC_TYPE_PLAYER) {
                                    vib.setType(HwHapticPlayerService.HAPTIC_TYPE_PLAYER);
                                }
    ...
    private void startVibrationLocked(Vibration vib) {
        ...
        try {
            ...
            if (intensity == 0) {
                ...
                if (vib.getType() != null) {
                    VibrationEffect.Waveform wf = vib.effect;
                    if (vib.getType() == HwHapticPlayerService.HAPTIC_TYPE_PLAYER) {
                        this.mCurrentVibration = vib;
                        this.mHwHapticPlayerService.onStateChange(1, 0);
                        sendAudioVibratorStateOn();
                        Trace.traceEnd(8388608);
                        return;
                    } else if (wf.getRepeatIndex() < 0) {
                        if (isCurrentMaskToken(vib)) {
                            Trace.traceEnd(8388608);
                            return;
    ...
    public void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        if (DumpUtils.checkDumpPermission(this.mContext, TAG, pw)) {
            ...
            synchronized (this.mLock) {
                ...
                pw.println("  Haptic Slice:");
                Iterator<HwHapticPlayerService.SliceInfo> it6 = this.mHwHapticPlayerService.getHapticSliceInfo().iterator();
                while (it6.hasNext()) {
                    pw.print("    ");
                    pw.println(it6.next().toString());
                }
                pw.println("  Static Adjust point:");
                Iterator<HwHapticPlayerService.AdjustPointInfo> it7 = this.mHwHapticPlayerService.getStaticAdjustPointInfo().iterator();
                while (it7.hasNext()) {
                    pw.print("    ");
                    pw.println(it7.next().toString());
                }
                pw.println("  Dynamic Adjust point:");
                Iterator<HwHapticPlayerService.AdjustPointInfo> it8 = this.mHwHapticPlayerService.getDynamicAdjustPointInfo().iterator();
                while (it8.hasNext()) {
                    pw.print("    ");
                    pw.println(it8.next().toString());
                }
                pw.println("  Haptic Curve:");
                Iterator<HwHapticPlayerService.CurveInfo> it9 = this.mHwHapticPlayerService.getHapticCurveInfo().iterator();
                while (it9.hasNext()) {
                    pw.print("    ");
                    pw.println(it9.next().toString());
                }
            }
        }
    }
    ...
    private boolean isCancelHwVibrate() {
        ...
        if (!(vibration == null || vibration.getType() == null)) {
            if (this.mCurrentVibration.getType() == HwHapticPlayerService.HAPTIC_TYPE_PLAYER) {
                this.mHwHapticPlayerService.cancel();
                return true;
            }
            isCanceled = true;
    ...
    public void startHapticPlay(int uid, String opPkg, VibrationEffect effect, int usageHint, IBinder token) {
        vibrate(uid, opPkg, effect, usageHint, "hapticplay", token);
    }

    /* access modifiers changed from: package-private */
    public void stopHapticPlay(IBinder token) {
        cancelVibrate(token);
    }
    ...
    public class HwInnerVibratorService extends IHwVibrator.Stub {
        ...
        public void setHwParameter(String command) {
            VibratorService.this.mContext.enforceCallingOrSelfPermission("android.permission.VIBRATE", "setHwParameter");
            VibratorService vibratorService = this.service;
            VibratorService.setParameter(command);
        }

        public String getHwParameter(String command) {
            VibratorService.this.mContext.enforceCallingOrSelfPermission("android.permission.VIBRATE", "getHwParameter");
            VibratorService vibratorService = this.service;
        ...
        public IHwHapticPlayer getHwHapticPlayer() {
            return VibratorService.this.mHwHapticPlayerService;
        }
    }
}

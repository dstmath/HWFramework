...
public class HwAdbService extends AdbServiceEx {
    ...
    public void handleUserSwtiched(int newUserId) {
        boolean isRepairMode = newUserId == REPAIR_MODE_USER_ID;
        HdbSettingsObserver hdbSettingsObserver = new HdbSettingsObserver(getAdbHandler());
        if (isRepairMode) {
            ContentResolverExt.registerContentObserver(getContentResolver(), Settings.System.getUriFor("hdb_enabled"), false, hdbSettingsObserver.getContentObserver(), (int) REPAIR_MODE_USER_ID);
            if (SystemPropertiesEx.get("ro.product.locale.region", "null").equals("CN")) {
                try {
                    SettingsEx.System.putIntForUser(getContentResolver(), "hdb_enabled", 1, (int) REPAIR_MODE_USER_ID);
                } catch (SecurityException e) {

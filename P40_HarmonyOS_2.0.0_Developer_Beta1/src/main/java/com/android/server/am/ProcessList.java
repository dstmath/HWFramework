...
public final class ProcessList {
    ...
    public final boolean killPackageProcessesLocked(String packageName, int appId, int userId, int minOomAdj, boolean callerWillRestart, boolean allowRestart, boolean doit, boolean evenPersistent, boolean setRemoved, String reason) {
        int NP;
        boolean isNeedLogForRelaunchIme;
        ...
        while (ip < NP2) {
            ...
            while (ia < NA2) {
                ...
                if (!"relaunchIME".equals(reason) || packageName == null) {
                    NP = NP2;
                } else {
                    NP = NP2;
                    if ((app.info != null && packageName.equals(app.info.packageName)) || ((app.pkgDeps != null && app.pkgDeps.contains(packageName)) || (app.pkgList != null && app.pkgList.containsKey(packageName)))) {
                        isNeedLogForRelaunchIme = true;
                        if (superWhiteListApp2 == null) {
                            NA = NA2;
                            if (superWhiteListApp2.contains(app.info.packageName)) {
                                StringBuilder sb = new StringBuilder();
                        ...
                        if (app.isPersistent() || evenPersistent) {
                            if (app.removed) {
                                if (app.setAdj < i) {
                                    if (HwPCUtils.isPcCastModeInServer() && "relaunchIME".equals(reason) && packageName != null && packageName.equals(app.info.packageName)) {
                                        Slog.i("ActivityManager", "relaunchIME:" + app.info.packageName + "," + app.setAdj + "-" + i);
                                    } else if (isNeedLogForRelaunchIme) {
                                        Slog.w("ActivityManager", "killPackageProcessesLocked: [Relaunch IME] app.setAdj < minOomAdj, continue. App: " + app.toShortString());
                                    }
                                }
                                if (packageName != null) {
                                    ...
                                    if (isDep || UserHandle.getAppId(app.uid) == appId) {
                                        if (userId == -1 || app.userId == userId) {
                                            ...
                                            Slog.w("ActivityManager", "killPackageProcessesLocked: [Relaunch IME] userId is invalid, continue. App: " + app.toShortString());
                                        }
                                    } else if (isNeedLogForRelaunchIme) {
                                        Slog.w("ActivityManager", "killPackageProcessesLocked: [Relaunch IME] appId is invalid, continue. App: " + app.toShortString());
                                    }
                            ...
                            if (isNeedLogForRelaunchIme) {
                                Slog.w("ActivityManager", "killPackageProcessesLocked: [Relaunch IME] App is persistent, continue. App: " + app.toShortString());
                            }
                            ia++;
                            i = minOomAdj;
                            NP2 = NP;
        ...
        if ("relaunchIME".equals(reason)) {
            Slog.i("ActivityManager", "killPackageProcessesLocked: [Relaunch IME] the count of the removed process is " + N);
        }
    ...
    public boolean removeProcessLocked(ProcessRecord app, boolean callerWillRestart, boolean allowRestart, String reason) {
        ...
        if ((app.pid <= 0 || app.pid == ActivityManagerService.MY_PID) && (app.pid != 0 || !app.pendingStart)) {
            ...
            if ("relaunchIME".equals(reason)) {
                Slog.w("ActivityManager", "removeProcessLocked: [Relaunch IME] not kill. processName: " + app.toShortString());
            }
        } else {
            if (app.pid > 0) {
            ...
            if ("relaunchIME".equals(reason)) {
                Slog.i("ActivityManager", "removeProcessLocked: [Relaunch IME] kill process. processName: " + app.toShortString());
            }
        }
        return needRestart;
    }

...
public class HwApsManagerService extends IHwApsManager.Stub {
    ...
    private static final String HICAR_BROADCAST = "com.huawei.action.hwmultidisplay.MULTIDISPLAY_STATE_CHANGED";
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent != null && intent.getAction() != null) {
                if (intent.getAction().equals(UiModeManager.ACTION_ENTER_CAR_MODE) || intent.getAction().equals(UiModeManager.ACTION_EXIT_CAR_MODE)) {
                    HwApsManagerService.this.processForAuto();
                } else if (intent.getAction().equals(HwApsManagerService.HICAR_BROADCAST)) {
                    HwApsManagerService.this.processForHiCar(intent);
                } else {
                    Slog.i(HwApsManagerService.TAG, "mCarModeReceiver, unknow action!");
                }
            }
        }
    };
    ...
    boolean mIsInCarMode = false;
    private int mLongSize = 0;
    private int mShortSize = 0;

    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void processForAuto() {
        try {
            changeCarMode(IUiModeManager.Stub.asInterface(ServiceManager.getService("uimode")).getCurrentModeType() == 3);
            Slog.i(TAG, "APS: processForAuto, now is in auto car mode? = " + this.mIsInCarMode);
        } catch (RemoteException e) {
            Slog.e(TAG, "APS: HwApsManagerService.BroadcastReceiver.onRecive, remote exception is thrown. e = " + e.getMessage());
        }
    }

    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void processForHiCar(Intent intent) {
        boolean isEnter = false;
        boolean isHiCarMode = intent.getIntExtra("mode", 0) == 1;
        if (intent.getIntExtra("state", 0) == 1) {
            isEnter = true;
        }
        if (isHiCarMode) {
            changeCarMode(isEnter);
        }
        Slog.i(TAG, "APS: processForHiCar, isHiCarMode = " + isHiCarMode + " isEnter = " + isEnter);
    }

    private void changeCarMode(boolean isInCarMode) {
        this.mIsInCarMode = isInCarMode;
        this.mHwApsManagerServiceConfig.stopAllAppsInLowResolution();
    }
    ...
    public HwApsManagerService(Context context) {
        ...
        if (displayMetrics.widthPixels < displayMetrics.heightPixels) {
            this.mLongSize = displayMetrics.heightPixels;
            this.mShortSize = displayMetrics.widthPixels;
        } else {
            this.mLongSize = displayMetrics.widthPixels;
            this.mShortSize = displayMetrics.heightPixels;
        }
        Slog.e(TAG, "APS ManagerService.screen mLongSize " + this.mLongSize + ", mShortSize = " + this.mShortSize);
        IntentFilter filter = new IntentFilter();
        filter.addAction(UiModeManager.ACTION_ENTER_CAR_MODE);
        filter.addAction(UiModeManager.ACTION_EXIT_CAR_MODE);
        filter.addAction(HICAR_BROADCAST);
    ...
    final class ApsMainHandler extends Handler {
        ApsMainHandler(Looper looper) {
            super(looper, null, true);
        }
    }

    private boolean checkCallingPermission(String permission, String name) {
        if (Binder.getCallingPid() == Process.myPid() || this.mContext.checkCallingPermission(permission) == 0) {
            return true;
        }
        Slog.w(TAG, "Permission Denial: func=" + name + " from uid=" + Binder.getCallingUid() + ", pid=" + Binder.getCallingPid() + " requires " + permission);
        return false;
    }
    ...
    public int setResolution(String pkgName, float ratio, boolean isSwitchable) {
        int resolutionLocked;
        if (!checkCallingPermission("com.huawei.aps.permission.UPDATE_APS_INFO", "setResolution")) {
            return -3;
        }
        synchronized (this) {
            resolutionLocked = this.mHwApsManagerServiceConfig.setResolutionLocked(pkgName, ratio, isSwitchable);
        }
    ...
    public int setBrightness(String pkgName, int ratioPercent) {
        ...
        synchronized (this) {
            ...
            if (result == 0) {
                result = this.mHwApsManagerServiceConfig.notifyApsManagerServiceCallback(pkgName, 3, ratioPercent);
            }
        }
        return result;
    }
    ...
    public int setTexture(String pkgName, int ratioPercent) {
        ...
        synchronized (this) {
            ...
            if (result == 0) {
                result = this.mHwApsManagerServiceConfig.notifyApsManagerServiceCallback(pkgName, 2, ratioPercent);
            }
        }
        return result;
    }
    ...
    public int setDynamicResolutionRatio(String pkgName, float ratio) {
        ...
        synchronized (this) {
            ...
            if (result == 0) {
                result = this.mHwApsManagerServiceConfig.notifyApsManagerServiceCallback(pkgName, 1, (int) (100000.0f * ratio));
            }
        }
        return result;
    }
    ...
    public int setDynamicFps(String pkgName, int fps) {
        ...
        synchronized (this) {
            ...
            if (result == 0) {
                result = this.mHwApsManagerServiceConfig.notifyApsManagerServiceCallback(pkgName, 0, fps);
            }
        }
        return result;
    }
    ...
    public boolean stopPackages(List<String> list) {
        return !checkCallingPermission("com.huawei.aps.permission.UPDATE_APS_INFO", "stopPackages") ? false : false;
    }

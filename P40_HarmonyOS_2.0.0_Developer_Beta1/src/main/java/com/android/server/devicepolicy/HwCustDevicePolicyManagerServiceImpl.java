...
public class HwCustDevicePolicyManagerServiceImpl extends HwCustDevicePolicyManagerService {
    ...
    private static final boolean IS_FORBIDDEN_SIMPLE_PWD = SystemProperties.getBoolean("ro.config.not_allow_simple_pwd", false);
    ...
    private static final String MASTER_CLEAR_LOW_LEVEL = "masterClearWipeDataFactoryLowlevel";
    private static final double MAX_RETRY_TIMES = 3.0d;
    private static final int[] MODIFY_CATEGARYS = {1, 1, 1, 1};
    private static final int[] POLICY_CATEGARYS = {0, 1, 1, 1};
    private static final int[] SHOW_CATAGARYS = {0, 0, 0, 0};
    ...
    public boolean wipeDataAndReset(Context context) {
        ...
        intent.putExtra(MASTER_CLEAR_LOW_LEVEL, true);
        context.sendBroadcast(intent);
        return true;
    }
            ...
            if (failedAttemps >= maxTimes) {
                new AsyncTask<Void, Void, Void>() {
                    ...
                    public void onPostExecute(Void result) {
                        Intent intent = new Intent("android.intent.action.MASTER_CLEAR");
                        intent.addFlags(285212672);
                        intent.putExtra(HwCustDevicePolicyManagerServiceImpl.MASTER_CLEAR_LOW_LEVEL, true);
                        context.sendBroadcast(intent);
                    }
                }.execute(new Void[0]);
            }
        ...
        if (UserHandle.getCallingUserId() != 0) {
            ...
            try {
                ...
                if (devicePolices != null && devicePolices.canRead()) {
                    boolean isSet = FileUtils.copyFile(devicePolices, new File(policyPath));
                    Slog.d(LOG_TAG, "active preset device admins result : " + isSet);
                }
    ...
    public boolean isForbiddenSimplePwdFeatureEnable() {
        return IS_FORBIDDEN_SIMPLE_PWD;
    }
    ...
    public void clearWipeDataFactoryLowlevel(Context context, String reason, boolean isWipeEuicc) {
        Slog.d(LOG_TAG, "wipeData, reason=" + reason + ", wipeEuicc=" + isWipeEuicc);
        Intent intent = new Intent("android.intent.action.MASTER_CLEAR");
        intent.addFlags(285212672);
        intent.putExtra(MASTER_CLEAR_LOW_LEVEL, true);
        intent.putExtra("com.android.internal.intent.extra.WIPE_ESIMS", isWipeEuicc);
    ...
    private void setPackageLaunchableAndBackgroundRunable(String packageName) {
        ...
        aePackageConfigs.add(new HwAppStartupSettingEx(packageName, POLICY_CATEGARYS, MODIFY_CATEGARYS, SHOW_CATAGARYS));

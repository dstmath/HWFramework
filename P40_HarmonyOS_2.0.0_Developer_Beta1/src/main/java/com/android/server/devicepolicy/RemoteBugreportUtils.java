...
public class RemoteBugreportUtils {
    ...
    static Notification buildNotification(Context context, int type) {
        ...
        if (type == 2) {
            builder.setContentTitle(context.getString(17041220)).setProgress(0, 0, true);
        } else if (type == 1) {
            builder.setContentTitle(context.getString(17041359)).setProgress(0, 0, true);
        } else if (type == 3) {
            ...
            builder.addAction(new Notification.Action.Builder((Icon) null, context.getString(17039954), PendingIntent.getBroadcast(context, NOTIFICATION_ID, new Intent("com.android.server.action.REMOTE_BUGREPORT_SHARING_DECLINED"), 268435456)).build()).addAction(new Notification.Action.Builder((Icon) null, context.getString(17041215), pendingIntentAccept).build()).setContentTitle(context.getString(17041217)).setContentText(context.getString(17041216)).setStyle(new Notification.BigTextStyle().bigText(context.getString(17041216)));
        }

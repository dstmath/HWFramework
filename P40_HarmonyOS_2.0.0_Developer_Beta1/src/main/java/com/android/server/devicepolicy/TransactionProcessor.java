...
public class TransactionProcessor {
    ...
    private static final boolean IS_HWFLOW = (Log.HWINFO || (Log.HWModuleLog && Log.isLoggable(TAG, 4)));
    ...
    private void processSetPolicy(int code, Parcel data, Parcel reply) {
        ...
        try {
            ...
            setFunctionDisabled(code, who, isDisable, userHandle);
    ...
    private void processInstallCertificateWithType(int code, Parcel data, Parcel reply) {
        ...
        if (len < 0) {
            ...
            boolean isRequestAccess = data.readInt() == 1;
            int userHandle = data.readInt();
            try {
                bdReportMdmPolicy(who, LegacyPolicy.getPolicyName(code));
                str2 = TAG;
                try {
                    boolean isInstallSuccess = installCertificateWithType(who, type, certBuffer, name, password, flag, isRequestAccess, userHandle);
                    reply.writeNoException();
                    if (isInstallSuccess) {
                        i = 1;
                    }
    ...
    private void processAccount(int code, Parcel data, Parcel reply) {
        ...
        try {
            bdReportMdmPolicy(who, LegacyPolicy.getPolicyName(code));
            if (code == 5018) {
                setAccountDisabled(who, accountType, isDisabled, userHandle);
                reply.writeNoException();
            } else if (code == 5019) {
                boolean isDisabled2 = isAccountDisabled(who, accountType, userHandle);
                reply.writeNoException();
                if (isDisabled2) {
                    i = 1;
                }
    ...
    private void processSdCard(int code, Parcel data, Parcel reply) {
        ...
        try {
            ...
            reply.writeNoException();
            reply.writeInt(isFormatSuccess ? 1 : 0);
        } catch (RuntimeException e) {
    ...
    private void processDefaultDataCard(int code, Parcel data, Parcel reply) {
        ...
        try {
            bdReportMdmPolicy(who, LegacyPolicy.getPolicyName(code));
            boolean isSuccess = setDefaultDataCard(who, slotId, response, userHandle);
            reply.writeNoException();
            reply.writeInt(isSuccess ? 1 : 0);
        } catch (RuntimeException e) {
    ...
    public Bundle getPolicy(ComponentName who, String policyName, Bundle keyWords, int userHandle, int type) {
        ...
        if (type == 1) {
            ...
            if (c == 0) {
                ...
                result.putStringArrayList(DeviceSettingsPlugin.STATE_VALUE, this.mService.queryBrowsingHistory(who, userHandle));
                return result;
            } else if (c != 1) {
                if (c == 2) {
    ...
    private void bdReportMdmPolicy(ComponentName who, String policyName) {
        if (!TextUtils.isEmpty(policyName)) {
            try {
                ...
                obj.put(BDREPORT_MDM_KEY_PACKAGE, who == null ? DeviceSettingsPlugin.EMPTY_STRING : who.getPackageName());
                obj.put(BDREPORT_MDM_KEY_APINAME, policyName);
                Flog.bdReport(991310126, obj.toString());

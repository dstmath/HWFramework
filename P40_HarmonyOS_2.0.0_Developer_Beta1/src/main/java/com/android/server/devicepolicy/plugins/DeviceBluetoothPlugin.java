...
public class DeviceBluetoothPlugin extends DevicePolicyPlugin {
    ...
    private static final String TAG = DeviceBluetoothPlugin.class.getSimpleName();
        ...
        switch (c) {
            ...
            case 1:
                if (policyData != null) {
                    ...
                    try {
                        devices = policyData.getStringArrayList("value");
                    } catch (ArrayIndexOutOfBoundsException e) {
                        HwLog.e(TAG, "onSetPolicy exception.");
                    }
                    notifyBluetoohListPolicyChange(0, devices, policyName);
                    break;
                }
                break;
            case 2:
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:31:0x007a, code lost:
        if (r7.equals(com.android.server.devicepolicy.plugins.DeviceBluetoothPlugin.BLUETOOTH_BLACK_LIST) != false) goto L_0x007e;
     */
            ...
            switch (c) {
                ...
                case 1:
                    ...
                    try {
                        ...
                        HwLog.e(TAG, "BLUETOOTH_WHITE_LIST exception.");
                    }
                    notifyBluetoohListPolicyChange(1, devices, policyName);
                    break;
                case 2:
                case 3:
                case HwDevicePolicyManagerService.SD_CRYPT_STATE_DECRYPTING /* 4 */:
    ...
    private void procBlackAndWhiteList(PolicyStruct.PolicyItem item, String policyName) {
        Iterator<PolicyStruct.PolicyItem> it = item.getChildItem().iterator();
        while (it.hasNext()) {
            ArrayList<String> oldPolicyList = null;
            try {
                oldPolicyList = it.next().getAttributes().getStringArrayList("value");
            } catch (ArrayIndexOutOfBoundsException e) {
                HwLog.e(TAG, "procBlackAndWhiteList exception.");
            }
            notifyBluetoohListPolicyChange(1, oldPolicyList, policyName);
        }
    }
        ...
        while (it.hasNext()) {
            ...
            if (item != null) {
                ...
                switch (c) {
                    case 0:
                    case 1:
                        procBlackAndWhiteList(item, policyName);
                        continue;
                    case 2:
                    case 3:
                    case HwDevicePolicyManagerService.SD_CRYPT_STATE_DECRYPTING /* 4 */:
    ...
    private void notifyBluetoohListPolicyChange(int type, ArrayList<String> policyData, String policyName) {
        ...
        Intent intent = new Intent(ACTION_DEVICE_POLICY_CHANGED);
        intent.setPackage(BLUETOOTH_PACKAGE_NAME);
        intent.addFlags(1073741824);
        intent.putExtra(BLUETOOTH_POLICY_EXTRA_NAME, policyName);
        intent.putExtra(BLUETOOTH_POLICY_EXTRA_TYPE, type);

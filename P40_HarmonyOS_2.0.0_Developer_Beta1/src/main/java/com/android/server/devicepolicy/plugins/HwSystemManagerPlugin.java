...
public class HwSystemManagerPlugin extends DevicePolicyPlugin {
    ...
    private static final String TAG = HwSystemManagerPlugin.class.getSimpleName();
    ...
    public PolicyStruct getPolicyStruct() {
        ...
        PolicyStruct.PolicyType enterpriseListType = PolicyStruct.PolicyType.LIST;
        ...
        struct.addStruct("enterprise-whitelist-hwsystemmanager/enterprise-whitelist-hwsystemmanager-item", enterpriseListType, new String[]{"value"});
    ...
    public void onSetPolicyCompleted(ComponentName who, String policyName, boolean isChanged) {
        String str = TAG;
        HwLog.i(str, "onSetPolicyCompleted policy: " + policyName);
        if (DATA_SAVE_MODE.equals(policyName) && isChanged) {
            this.mContext.sendBroadcast(new Intent(UPDATE_DATA_SAVER_MODE), SYSTEM_MANAGER_PERMISSION);
        } else if (POLICY_SUPER_WHITELIST.equals(policyName)) {
        ...
        } else if (POLICY_ENTERPRISE_WHITE_LIST.equals(policyName) && isChanged) {
    ...
    public boolean onRemovePolicy(ComponentName who, String policyName, Bundle policyData, boolean isGlobalPolicyChanged) {
        String str = TAG;
        HwLog.i(str, "onRemovePolicy policy: " + policyName);
        if (DATA_SAVE_MODE.equals(policyName)) {
            ...
            this.mContext.sendBroadcastAsUser(intent, UserHandle.getUserHandleForUid(Binder.getCallingUid()), SYSTEM_MANAGER_PERMISSION);
            return true;
        } else if (POLICY_ENTERPRISE_WHITE_LIST.equals(policyName)) {
    ...
    public boolean onActiveAdminRemoved(ComponentName who, ArrayList<PolicyStruct.PolicyItem> removedPolicies) {
        ...
        while (it.hasNext()) {
            ...
            if (!(policy == null || (policyName = policy.getPolicyName()) == null)) {
                char c = 65535;

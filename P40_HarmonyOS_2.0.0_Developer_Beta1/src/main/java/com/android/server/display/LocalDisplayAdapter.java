...
public final class LocalDisplayAdapter extends DisplayAdapter {
    ...
    private void tryConnectDisplayLocked(long physicalDisplayId) {
        ...
        if (displayToken != null) {
            ...
            int activeColorMode3 = SurfaceControl.getActiveColorMode(displayToken);
            if (activeColorMode3 < 0) {
                Slog.w(TAG, "Unable to get active color mode for display device " + physicalDisplayId);
                activeColorMode = -1;
            } else {
                activeColorMode = activeColorMode3;
            }
            int[] colorModes = SurfaceControl.getDisplayColorModes(displayToken);
            int[] allowedConfigs2 = SurfaceControl.getAllowedDisplayConfigs(displayToken);
            if (DEBUG_HWLOCALDISPLAY) {
                ...
                Slog.i(TAG, "getAllowedDisplayConfigs, allowedConfigs =" + Arrays.toString(allowedConfigs2));
            }
            if (allowedConfigs2 == null) {
                Slog.e(TAG, "Unable to get allowedConfigs when connected is false");
            } else if (allowedConfigs2.length > configs.length) {
                Slog.e(TAG, "allowedConfigs array is in EXCEPTION state: allowedConfigs length = " + allowedConfigs2.length + ", configs length = " + configs.length);
            } else {
                ...
                if (device2 == null) {
                    ...
                    if (physicalDisplayId == BUILT_IN_DISPLAY_ID_MAIN.longValue()) {
                        allowedConfigs = allowedConfigs2;
                        activeColorMode2 = activeColorMode;
                    } else if (physicalDisplayId == 100000) {
                        allowedConfigs = allowedConfigs2;
                        activeColorMode2 = activeColorMode;
                    } else {
                        device = new LocalDisplayDevice(displayToken, physicalDisplayId, configs, activeConfig, allowedConfigs2, colorModes, activeColorMode, isInternal);
                        j = physicalDisplayId;
                    ...
                    j = physicalDisplayId;
                    localDisplayAdapter = this;
                    device = new LocalDisplayDevice(displayToken, physicalDisplayId, configs, activeConfig, allowedConfigs, colorModes, activeColorMode2, isInternal);
                    localDisplayAdapter.mDevices.put(j, device);
                    localDisplayAdapter.sendDisplayDeviceEventLocked(device, 1);
                } else if (device2.updatePhysicalDisplayInfoLocked(configs, activeConfig, allowedConfigs2, colorModes, activeColorMode)) {
    ...
    public final class LocalDisplayDevice extends DisplayDevice {
        ...
        public DisplayDeviceInfo getDisplayDeviceInfoLocked() {
            if (this.mInfo == null) {
                ...
                if (this.mIsInternal) {
                    this.mInfo.name = res.getString(17039983);
                    ...
                    this.mInfo.name = LocalDisplayAdapter.this.getContext().getResources().getString(17039984);
                    this.mInfo.touch = 2;

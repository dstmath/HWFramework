...
public class HwGeocoderProxy extends GeocoderProxy implements HwLocationProxy {
    ...
    private static final int COLLECTION_DEFAULT_SIZE = 10;
    ...
    private static final String REFUSE_REQUEST = "refuse request";
    private static final int REQUEST_GEOCODER_LIMIT_EACH_APP = 10;
    private static final int REQUEST_GEOCODER_LIMIT_TOTAL = 20;
    ...
    private final Map<String, Integer> mPkgCount = new HashMap(10);
    ...
    private boolean isRequestUnderLimit(String pkgName) {
        if (pkgName != null) {
            synchronized (MAP_LOCK) {
                if (this.mRequestCount > 20) {
                    LBSLog.w(TAG, false, "request geocoder total size over limit", new Object[0]);
                    return false;
                }
                int curTimes = this.mPkgCount.getOrDefault(pkgName, 0).intValue();
                if (curTimes > 10) {
                    LBSLog.w(TAG, false, "request geocoder over limit %{public}s", pkgName);
                    return false;
                }
                this.mPkgCount.put(pkgName, Integer.valueOf(curTimes + 1));
                this.mRequestCount++;
            }
        }
        return true;
    ...
    private void updateRequestCount(String pkgName) {
        if (pkgName != null) {
            synchronized (MAP_LOCK) {
                int curTimes = this.mPkgCount.get(pkgName).intValue();
                if (curTimes >= 1) {
                    this.mPkgCount.put(pkgName, Integer.valueOf(curTimes - 1));
                    this.mRequestCount--;
                }
    ...
    public String getFromLocation(double latitude, double longitude, int maxResults, GeocoderParams params, List<Address> addrs) {
        if (!isRequestUnderLimit(params.getClientPackage())) {
            return REFUSE_REQUEST;
        }
        this.mNlpUsed = 0;
        String result = getFromLocationImpl(latitude, longitude, maxResults, params, addrs);
        LBSLog.i(TAG, false, "getFromLocation %{public}s %{public}s", result, params.getClientPackage());
        if (!this.mIsDualNlpAlive || result == null) {
            if (needRequestGlobalAb(params, result)) {
                result = getFromLocationGlobalAb(latitude, longitude, maxResults, params, addrs);
            }
        } else if (this.mNlpUsed == 1) {
            this.mNlpUsed = 2;
            result = getFromLocationImpl(latitude, longitude, maxResults, params, addrs);
        } else {
            this.mNlpUsed = 1;
            result = getFromLocationImpl(latitude, longitude, maxResults, params, addrs);
        }
        updateRequestCount(params.getClientPackage());
        return result;
    }

...
public class LockSettingsStrongAuth {
    ...
    public long getStrongAuthTrigerTime(int userId) {
        return SystemClock.elapsedRealtime() + ((DevicePolicyManager) this.mContext.getSystemService("device_policy")).getRequiredStrongAuthTimeout(null, userId);
    }
    ...
    private void handleScheduleStrongAuthTimeout(int userId) {
        ...
        long when = getStrongAuthTrigerTime(userId);
        Slog.i(TAG, "will alarm in " + when + " user " + userId);
        StrongAuthTimeoutAlarmListener alarm2 = this.mStrongAuthTimeoutAlarmListenerForUser.get(Integer.valueOf(userId));
        if (alarm2 != null) {
            this.mAlarmManager.cancel(alarm2);
            alarm2.setTrigerTime(true, when);
            alarm = alarm2;
        } else {
            StrongAuthTimeoutAlarmListener alarm3 = createAlarmListener(userId);
            alarm3.setTrigerTime(true, when);
            this.mStrongAuthTimeoutAlarmListenerForUser.put(Integer.valueOf(userId), alarm3);
            alarm = alarm3;
        }

...
public class AwareAppMngSort {
    ...
    private static final String CACHED_CLEAN_APPS = "cached_clean_apps";
    ...
    private ArrayList<String> mCachedCleanApps = null;
    ...
    private ArrayMap<Integer, List<AwareProcessBlockInfo>> getAppMngSortGroupForMemCleanChina(int groupId, int memLevel) {
        ...
        List<AwareProcessBlockInfo> needClean = CleanSource.mergeBlockForLowMemory(rawInfo, DecisionMaker.getInstance().getProcessList(AppMngConstant.AppMngFeature.APP_CLEAN, AppMngConstant.AppCleanSource.MEMORY), getCachedCleanApps());
        ...
        setFloatBallAppImport(needClean);
        return getAppGroupById(groupId, rawInfo, needClean);
    }

    private void setFloatBallAppImport(List<AwareProcessBlockInfo> needClean) {
        if (FloatBallAssociate.getInstance().isFloatBallProtectEnable()) {
            int improtance = 1;
            Iterator<FloatBallAssociate.AppInfo> it = FloatBallAssociate.getInstance().getTopFloatBall().iterator();
            while (it.hasNext()) {
                FloatBallAssociate.AppInfo floatBallApp = it.next();
                if (floatBallApp != null) {
                    boolean isInNeedClean = false;
                    for (AwareProcessBlockInfo blockInfo : needClean) {
                        if (blockInfo != null && blockInfo.procPackageName != null && blockInfo.procPackageName.equals(floatBallApp.pkgName) && floatBallApp.userId == UserHandleEx.getUserId(blockInfo.procUid)) {
                            blockInfo.floatBallAppImport = improtance;
                            isInNeedClean = true;
                        }
                    }
                    if (isInNeedClean) {
        ...
        if (pw != null || !toPrint) {
            for (AwareProcessBlockInfo pinfo : list) {
                if (pinfo != null) {
                    ...
                    print(pw, "AppProc:uid:" + pinfo.procUid + ",import:" + pinfo.procImportance + ",classRates:" + pinfo.procClassRate + ",classStr:" + getClassRateStr(pinfo.procClassRate) + ",subStr:" + getClassStr(pinfo.procClassRate, pinfo.procSubClassRate) + ",subTypeStr:" + pinfo.procSubTypeStr + ",appType:" + pinfo.procAppType + ",policy:" + pinfo.procDetailedReason.get("policy") + ",reason:" + pinfo.procReason + ",weight:" + pinfo.procWeight + ",floatBallAppImport:" + pinfo.floatBallAppImport);
    ...
    private void dumpPolicy(AwareAppMngSortPolicy policy, PrintWriter pw, boolean toPrint, int level) {
        ...
        if (pw != null || !toPrint) {
            ...
            sortByImportRecentIfNeed(list, level);
    ...
    public void sortByImportRecentIfNeed(List<AwareProcessBlockInfo> procs, int level) {
        if (procs != null && !isLowLevel(level)) {
            sortByRecent(procs);
            sortByFloatBallApp(procs);
            sortByImportSysApp(procs);
        }
    }

    private void sortByRecent(List<AwareProcessBlockInfo> procs) {
        ArrayList<String> recentApps = AwareAppUseDataManager.getInstance().getRecentApp();
    ...
    private void sortByFloatBallApp(List<AwareProcessBlockInfo> procs) {
        ArrayList<FloatBallAssociate.AppInfo> floatBallApps = FloatBallAssociate.getInstance().getTopFloatBall();
        if (floatBallApps != null && !floatBallApps.isEmpty()) {
            List<AwareProcessBlockInfo> tmpFloatBallApp = new ArrayList<>();
            Iterator<FloatBallAssociate.AppInfo> it = floatBallApps.iterator();
            while (it.hasNext()) {
                FloatBallAssociate.AppInfo app = it.next();
                if (!(app == null || app.pkgName == null)) {
                    Iterator<AwareProcessBlockInfo> iter = procs.iterator();
                    while (iter.hasNext()) {
                        AwareProcessBlockInfo info = iter.next();
                        if (info != null && app.pkgName.equals(info.procPackageName) && app.userId == UserHandleEx.getUserId(info.procUid)) {
                            tmpFloatBallApp.add(info);
                            iter.remove();
                        }
                    }
                }
            }
            procs.addAll(0, tmpFloatBallApp);
        }
    }

    private void sortByImportSysApp(List<AwareProcessBlockInfo> procs) {
        ArrayList<String> importSysApps = AwareAppUseDataManager.getInstance().getImportSysApps();
        List<AwareProcessBlockInfo> tmpImportSysApp = new ArrayList<>();
        Iterator<String> it = importSysApps.iterator();
        while (it.hasNext()) {
            String app = it.next();
            if (app != null) {
                Iterator<AwareProcessBlockInfo> iter = procs.iterator();
                while (iter.hasNext()) {
                    AwareProcessBlockInfo info = iter.next();
                    if (info != null && isCurUser(info.procUid) && app.equals(info.procPackageName)) {
                        tmpImportSysApp.add(info);
                        iter.remove();
                    }
                }
            }
        }
        procs.addAll(0, tmpImportSysApp);
    }
    ...
    private void updateCachedCleanApps() {
        synchronized (LOCK) {
            this.mCachedCleanApps = DecisionMaker.getInstance().getRawConfig(AppMngConstant.AppMngFeature.APP_CLEAN.getDesc(), CACHED_CLEAN_APPS);
            if (this.mCachedCleanApps == null) {
                this.mCachedCleanApps = new ArrayList<>();
            }
        }
    }

    private ArrayList<String> getCachedCleanApps() {
        ...
        synchronized (LOCK) {
            ...
            tmp = new ArrayList<>();
            tmp.addAll(this.mCachedCleanApps);
        }
        return tmp;
    }

    public boolean isCachedCleanApps(String pkgName) {
        synchronized (LOCK) {
            if (pkgName == null) {
                return false;
            }
            ...
            return this.mCachedCleanApps.contains(pkgName);
        }
    }
}

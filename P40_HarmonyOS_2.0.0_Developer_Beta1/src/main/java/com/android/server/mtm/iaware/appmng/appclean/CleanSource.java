...
public abstract class CleanSource {
    ...
    public static List<AwareProcessBlockInfo> mergeBlockForLowMemory(List<AwareProcessBlockInfo> rawBlock, ArrayMap<String, ListItem> processList, ArrayList<String> cachedCleanApps) {
        if (rawBlock == null || rawBlock.isEmpty()) {
            return rawBlock;
        }
        return mergeBlockInternal(rawBlock, true, null, processList, cachedCleanApps);
    }
    ...
    public static List<AwareProcessBlockInfo> mergeBlockForMemory(List<AwareProcessBlockInfo> rawBlock, ArrayMap<String, ListItem> processList) {
        ...
        return mergeBlockInternal(rawBlock, true, null, processList, null);
    }
    ...
    public static List<AwareProcessBlockInfo> mergeBlock(List<AwareProcessBlockInfo> rawBlock) {
        ...
        return mergeBlockInternal(rawBlock, false, null, null, null);
    }
    ...
    public static List<AwareProcessBlockInfo> mergeBlock(List<AwareProcessBlockInfo> rawBlock, List<ProcessCleaner.CleanType> priority) {
        ...
        return mergeBlockInternal(rawBlock, false, priority, null, null);
    }
    ...
    private static List<AwareProcessBlockInfo> mergeBlockInternal(List<AwareProcessBlockInfo> rawBlock, boolean forMemory, List<ProcessCleaner.CleanType> priority, ArrayMap<String, ListItem> processList, ArrayList<String> cachedCleanApps) {
        ...
        while (iterator.hasNext()) {
            ...
            if (lastBlock == null) {
                if (!ProcessCleaner.CleanType.NONE.equals(curBlock.procCleanType)) {
                    checkForProc(curBlock, processList, groupBy.get(BLOCK_INFO_LIST), cachedCleanApps);
                }
                lastBlock = curBlock;
    ...
    private static boolean needCheckForProc(AwareProcessBlockInfo block, ArrayMap<String, ListItem> processList, List<AwareProcessBlockInfo> blockInfoList) {
    ...
    private static boolean needCheckCachedCleanApps(ArrayList<String> cachedCleanApps, String pkg) {
        if (cachedCleanApps == null || cachedCleanApps.isEmpty() || pkg == null) {
            return false;
        }
        return cachedCleanApps.contains(pkg);
    }

    private static void checkForProc(AwareProcessBlockInfo block, ArrayMap<String, ListItem> processList, List<AwareProcessBlockInfo> blockInfoList, ArrayList<String> cachedCleanApps) {
        ListItem item;
        if (needCheckForProc(block, processList, blockInfoList)) {
            boolean checkCachedCleanApps = needCheckCachedCleanApps(cachedCleanApps, block.procPackageName);
            ...
            while (iterator.hasNext()) {
                ...
                if (procInfo == null || procInfo.procProcInfo == null || procInfo.procProcInfo.mProcessName == null) {
                    iterator.remove();
                } else if (checkCachedCleanApps && checkForProcInCachedCleanApps(cachedCleanApps, block.procPackageName, procInfo, processList)) {
                    AwareProcessBlockInfo tempBlock = new AwareProcessBlockInfo(block.procReason, block.procUid, procInfo, block.procCleanType.ordinal(), block.procDetailedReason);
                    tempBlock.isCachedCleanApp = true;
                    blockInfoList.add(tempBlock);
                    iterator.remove();
                } else if (procInfo.procListPolicy != 1 && (((item = processList.get(procInfo.procProcInfo.mProcessName)) == null || !AppSceneMngFeature.isEnable() || isProcessListSupportedCleanType(item.getPolicy())) && ((procInfo.procListPolicy != -1 || item == null || item.getPolicy() == ProcessCleaner.CleanType.NONE.ordinal()) && item != null && item.getPolicy() < block.procCleanType.ordinal()))) {
            ...
            updateBlockProcList(block);
        }
    }

    private static void updateBlockProcList(AwareProcessBlockInfo block) {
        if (block.procProcessList.isEmpty()) {
            block.procProcessList = null;
        }
    }

    private static boolean checkForProcInCachedCleanApps(ArrayList<String> arrayList, String pkg, AwareProcessInfo procInfo, ArrayMap<String, ListItem> processList) {
        if (procInfo.procProcInfo.mCurAdj < HwActivityManagerService.CACHED_APP_MIN_ADJ) {
            return false;
        }
        ListItem item = processList.get(procInfo.procProcInfo.mProcessName);
        if (item == null || procInfo.procListPolicy == 1 || item.getPolicy() != ProcessCleaner.CleanType.NONE.ordinal()) {
            return true;
        }
        return false;
    }

    private static void mergeBlockInfoIfNeed(AwareProcessBlockInfo lastBlock, AwareProcessBlockInfo block, List<ProcessCleaner.CleanType> priority, Map<String, List<AwareProcessBlockInfo>> groupBy) {
        if (block.isCachedCleanApp) {
            mergeCachedCleanAppToList(block, groupBy);
        } else {
            mergeBlockInfo(lastBlock, block, priority);
        }
    }

    private static void mergeCachedCleanAppToList(AwareProcessBlockInfo block, Map<String, List<AwareProcessBlockInfo>> groupBy) {
        AwareProcessBlockInfo tempBlock = new AwareProcessBlockInfo(AppMngConstant.CleanReason.CACHED_CLEAN_APP.getCode(), block.procUid, block.procProcessList.get(0), ProcessCleaner.CleanType.KILL_ALLOW_START.ordinal(), block.procDetailedReason);
        tempBlock.procDetailedReason.put(RuleParserUtil.AppMngTag.POLICY.getDesc(), Integer.valueOf(ProcessCleaner.CleanType.KILL_ALLOW_START.ordinal()));
        setProperFlag(tempBlock, true);
        mergeBlockToList(tempBlock, groupBy.get(CLEAN_BLOCK_LIST), groupBy.get(PROTECTED_BLOCK_LIST), true);
    }
    ...
    private static void handleBlockWithProcessList(AwareProcessBlockInfo lastBlock, Map<String, List<AwareProcessBlockInfo>> groupBy, ArrayMap<String, ListItem> processList, List<ProcessCleaner.CleanType> priority, boolean forMemory) {
        ...
        if (!(lastBlock == null || blockInfoList == null || cleanBlock == null || processList == null || protectedBlock == null)) {
            ...
            for (AwareProcessBlockInfo block : blockInfoList) {
                mergeBlockInfoIfNeed(lastBlock, block, priority, groupBy);
            }
        }
    }
    ...
    private static void mergeBlockInfo(AwareProcessBlockInfo lastBlock, AwareProcessBlockInfo curBlock, List<ProcessCleaner.CleanType> priority) {
        if (curBlock.procProcessList != null) {
            ...
            if (lastBlock.procSwapIndex > curBlock.procSwapIndex) {
                lastBlock.procSwapIndex = curBlock.procSwapIndex;
            }

...
public class HwMultiWinClipImageView extends ImageView {
    ...
    private void init(Context context) {
        ...
        int borderWidth = context.getResources().getDimensionPixelSize(34472613);
        if (borderWidth > 0) {
            this.mBorderPaint = new Paint();
            this.mBorderPaint.setColor(context.getResources().getColor(33883091));
    ...
    private float getPushScaleFactorWithSplitBar(float scaleFactorWithNoSplitBar) {
        ...
        if (((float) totalLength) > 0.0f) {
            return ((((float) totalLength) * scaleFactorWithNoSplitBar) - (((float) getContext().getResources().getDimensionPixelSize(34472627)) / 2.0f)) / ((float) totalLength);
        }

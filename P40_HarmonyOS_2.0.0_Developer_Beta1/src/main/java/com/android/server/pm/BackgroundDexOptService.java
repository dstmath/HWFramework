...
public class BackgroundDexOptService extends JobService implements IHwBackgroundDexOptInner {
    ...
    public static void schedule(Context context) {
        if ("factory".equals(SystemProperties.get("ro.runmode", "normal"))) {
            Log.i(TAG, "factory version, not run background dexopt.");
        } else if (!isBackgroundDexoptDisabled()) {
            initializePeriod();
    ...
    private int optimizePackages(PackageManagerService pm, ArraySet<String> pkgs, long lowStorageThreshold, boolean isForPrimaryDex) {
        ...
        while (it.hasNext()) {
            ...
            if (unusedPackages.contains(pkg) && shouldDowngrade) {
                dex_opt_performed = downgradePackage(packageManagerService, pkg, isForPrimaryDex);
            } else if (abort_code != 3) {
                dex_opt_performed = optimizePackage(packageManagerService, pkg, isForPrimaryDex);
                if (isForPrimaryDex && dex_opt_performed) {
                    HwServiceExFactory.getHwPackageDynamicCodeLoading().record(pkg, System.currentTimeMillis(), 2);
                }
            }
        ...
        notifyPinService(updatedPackages);
        if (supportSecondaryDex() && isForPrimaryDex) {
            return 0;
        }
        HwServiceExFactory.getHwPackageDynamicCodeLoading().makeDexOptReport();
        return 0;
    }

...
public class GunstallUtil {
    ...
    public boolean forbidGMSUpgrade(PackageParser.Package pkg, PackageParser.Package oldPackage, int callingSessionUid, HwGunstallSwitchState hwGSwitchState) {
        ...
        String[] callerUidPacakges = this.mIPmsInner.getPackagesForUid(callingSessionUid);
        if (!(callerUidPacakges == null || callerUidPacakges[0] == null)) {
            Slog.i(TAG, "forbidGMSUpgrade: callerUidPacakge=" + callerUidPacakges[0] + ", updatingPackageName=" + updatingPackageName);
        }
        if (updatingPackageName.equals("com.google.android.gms") && callerUidPacakges != null && callerUidPacakges[0] != null && (callerUidPacakges[0].equals(PLAY_STORE_NAME) || ArrayUtils.contains(callerUidPacakges, PLAY_STORE_NAME))) {

...
public final class HwBackgroundDexOptServiceEx implements IHwBackgroundDexOptServiceEx {
                ...
                if (params.getJobId() == JOB_POST_COMPENSATE_OPTIMIZE) {
                    if (this.mJobParamsMap.get(JOB_POST_BOOT_UPDATE_DELAYOPT) == null) {
                        ...
                        if (this.mDexoptHandler != null && !this.mDexoptHandler.hasMessages(1) && !this.mDexoptHandler.hasMessages(2)) {
                            ...
                            msg.what = 2;
                            msg.obj = params;
                            this.mDexoptHandler.sendMessageDelayed(msg, HwSwingMotionGestureConstant.HOVER_SCREEN_OFF_THRESHOLD);
                            if (IS_DEBUG) {
    ...
    private void handlerCompensateOptimize(JobParameters params) {
        ...
        while (true) {
            ...
            if (excuteCounter >= 5 || this.isExitCompensateDexopt.get() || !isCharging() || isScreenOn()) {
                break;
            } else if (excutePackageDexOpt(packageName, compensateDexOptManager, packageManagerService, true)) {
                excuteCounter++;
            }
        }
        ...
        while (true) {
            ...
            if (excuteCounter >= 5 || this.isExitCompensateDexopt.get() || !isCharging() || isScreenOn()) {
                break;
            } else if (excutePackageDexOpt(packageName2, compensateDexOptManager, packageManagerService, false)) {
                excuteCounter++;
            }
        }
        Log.i(TAG, "excutePackageDexOpt interrupt");
    ...
    private boolean excutePackageDexOpt(String packageName, HwCompensateDexOptManager compensateDexOptManager, PackageManagerService packageManagerService, boolean isInstallSkipDexOpt) {
        Log.i(TAG, "runCompensateDexOpt begin:" + packageName);
        int dexOptFlags = 4;
        if (isInstallSkipDexOpt) {
            dexOptFlags = 4 | 1024;
        }
        int result = packageManagerService.performDexOptWithStatus(new DexoptOptions(packageName, 3, dexOptFlags));
        Log.i(TAG, "runCompensateDexOpt finish:" + packageName + ",flags:" + dexOptFlags + ",result:" + result);
                ...
                if (i != 2) {
                    ...
                    new Thread("HwBackgroundDexOptService_CompensateOptimize") {
                        ...
                        public void run() {
                            ...
                            HwBackgroundDexOptServiceEx.this.mService.jobFinished(params, false);
                            Log.i(HwBackgroundDexOptServiceEx.TAG, "jobFinished " + params.getJobId());
                        }
                    }.start();
                }
            } else if (msg.obj instanceof JobParameters) {

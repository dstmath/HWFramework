...
public class HwMdmDFTUtilImpl {
    ...
    private static String getSha256FingerData(byte[] cert) {
        try {
            ...
            String hexString = byte2HexFormatted(MessageDigest.getInstance("SHA256").digest(((X509Certificate) CertificateFactory.getInstance("X509").generateCertificate(input)).getEncoded()));
            input.close();
            return hexString;
    ...
    public static void getMdmInstallInfoDft(Context context, PackageParser.Package pkg) {
        Signature[] signatures;
        String sign;
        if (pkg != null && context != null) {
            ...
            if (context.getPackageManager() != null && pkg.mSigningDetails != null && (signatures = pkg.mSigningDetails.signatures) != null && (sign = getSha256FingerData(signatures[0].toByteArray())) != null) {

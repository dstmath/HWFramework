...
public class HwMdmFixPermission {
    ...
    public void fixMdmRuntimePermission(String packageName, String permName, IHwPackageManagerServiceExInner pmsEx, int flag) {
        ...
        if (fixApps != null && !fixApps.isEmpty()) {
            if (fixApps.contains(packageName)) {
                PackageManagerServiceEx pmsInner = pmsEx.getIPmsInner();
                if (pmsInner != null) {
                    if (isSysComponentOrPersistentPlatformSignedPrivApp(packageName, pmsInner.getPackageManagerSerivce())) {
                        Slog.i(TAG, "package is isSysComponentOrPersistentPlatformSignedPrivApp : " + packageName);
                        return;
                    }
                    PermissionManagerServiceInternalEx permsInner = pmsInner.getPermissionManager();
                    if (permsInner == null) {
                    ...
                    PermissionInfoEx permInfo = permsInner.getPermissionInfo(permName, packageName, 0, 1000);
                    if (permInfo != null) {
                        if (permInfo.getProtection() == 1) {
                            if (permsInner.checkPermission(permName, packageName, 1000, 0) == -1) {
                                ...
                                permsInner.updatePermissionFlags(permName, packageName, 16, 16, 1000, 0, true, null);
                                Slog.i(TAG, "premInfo.name is " + permInfo.getName() + " is fix to packge " + packageName);
                                return;
                            }
                        }
                    }
    ...
    private boolean isSysComponentOrPersistentPlatformSignedPrivApp(String pkgName, IHwPackageManagerInner pmsInner) {
        ...
        if (appInfo == null) {
            ...
            if (appInfo.isPrivilegedApp() && (appInfo.flags & 8) != 0 && pmsInner.checkSignaturesInner(PackageManagerServiceEx.PLATFORM_PACKAGE_NAME, pkgName) == 0) {
                return true;
            }
            return false;
        }
    ...
    public boolean isMdmFix(String packageName) {
        List<String> fixApps = HwDeviceManager.getList(56);
        if (fixApps != null && !fixApps.isEmpty()) {
            return fixApps.contains(packageName);
        }
        Slog.i(TAG, "fixApps is null");
        return false;
    }
}

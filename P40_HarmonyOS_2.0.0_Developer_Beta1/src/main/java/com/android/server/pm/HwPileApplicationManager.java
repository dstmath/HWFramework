...
public class HwPileApplicationManager {
    ...
    private DefaultHwPackageManagerUtils managerUtils;
    ...
    public HwPileApplicationManager(Context context) {
        ...
        this.hwPmsEx = null;
        this.managerUtils = HwBasicPlatformFactory.loadFactory(HwBasicPlatformFactory.HW_PART_BASIC_PLATFORM_SERVICES_FACTORY_IMPL).getHwPackageManagerUtils();
    ...
    private void sendResultBroadcast() {
        synchronized (this) {
            if (this.mIsInstallFinish) {
                if (!this.managerUtils.isUserUnlockedEx(this.mContext)) {
                }
            }
            if (IS_DEBUG) {
    ...
    private void installApk(final String apkFile) {
        final String packageName = this.managerUtils.getPackageNameFromApkEx(apkFile);
    ...
    private void scanApkFileInDir(File dirFile, int dep) {
        if (dirFile != null && dirFile.exists() && dirFile.isDirectory() && dep <= 2) {
            ...
            for (File getFile : files) {
                if (getFile.isFile() && this.managerUtils.isPackageFilenameEx(getFile.getName())) {

...
public class HwThemeInstaller {
    ...
    public static final String HWT_NEW_RECORDER = "com.huawei.phone.recorder";
    public static final String HWT_OLD_CONTACT = "com.android.contacts";
    public static final String HWT_OLD_RECORDER = "com.android.phone.recorder";
    ...
    private final String frameworkResHwext = "framework-res-hwext";
        ...
        if (this.mContext.checkCallingOrSelfPermission("android.permission.WRITE_MEDIA_STORAGE") == 0) {
            ...
            try {
                ...
                if (userId != 0 || !isDataSkinExists()) {
                    ...
                    if (HwDeviceManagerEx.disallowOp(35)) {
                    ...
                    renameRecorderFile(userId);
                    changeFilePermission(userId, "framework-res-hwext", "0775");
                } else {
    ...
    private void changeFilePermission(int userId, String file, String rule) {
        CommandLineUtil.chmod(rule, (String) null, getHwThemePathAsUser(userId) + FLAG_SPLIT + file);
    }
    ...
    private File getCustThemePath(String path) {
        ...
        String diffThemePath = SystemPropertiesEx.get("ro.config.diff_themes");
    ...
    public void createMagazineFolder() {
        ...
        if (new File(dir).exists()) {
            SlogEx.i(TAG, " Magazine Folder already exist return");
            return;
        }
        SlogEx.i(TAG, "createMagazineFolder Magazine Folder ing");
        CommandLineUtil.mkdir("system", dir);
        CommandLineUtil.chown("system", "system", "media_rw", dir);
        FileUtilsEx.setPermissions(dir, FileUtilsEx.getSIRWXU() | FileUtilsEx.getSIRWXG(), -1, -1);
        boolean isRestoreconResult = SELinuxEx.restorecon(dir);
        SlogEx.i(TAG, "createMagazineFolder Magazine Folder isRestoreconResult " + isRestoreconResult);
    }

    private void createThemeTempFolder() {
        CommandLineUtil.rm("system", "/data/skin.tmp");
        CommandLineUtil.mkdir("system", "/data/skin.tmp");
        CommandLineUtil.chmod("system", "0771", "/data/skin.tmp");
    }
    ...
    private void renameRecorderFile(int userId) {
        PackageInfo pi = null;
        try {
            pi = this.mContext.getPackageManager().getPackageInfo(HWT_NEW_RECORDER, 0);
        } catch (PackageManager.NameNotFoundException e) {
            Log.w(TAG, "renameContactFile NameNotFoundException.");
        }
        if (pi != null) {
            if (new File(getHwThemePathAsUser(userId), HWT_OLD_RECORDER).exists()) {
                CommandLineUtil.mv("system", getHwThemePathAsUser(userId) + FLAG_SPLIT + HWT_OLD_RECORDER, getHwThemePathAsUser(userId) + FLAG_SPLIT + HWT_NEW_RECORDER);
            }
        } else if (new File(getHwThemePathAsUser(userId), HWT_NEW_RECORDER).exists()) {
            CommandLineUtil.mv("system", getHwThemePathAsUser(userId) + FLAG_SPLIT + HWT_NEW_RECORDER, getHwThemePathAsUser(userId) + FLAG_SPLIT + HWT_OLD_RECORDER);
        }
    }
    ...
    private void moveOldHomeWallpaper() {
        ...
        if (newWallpaperFile.exists() || newWallpaperFile.mkdirs()) {
            ...
            CommandLineUtil.chmod("system", "0771", "/data/skin.tmp/wallpaper/");
            return;
        }

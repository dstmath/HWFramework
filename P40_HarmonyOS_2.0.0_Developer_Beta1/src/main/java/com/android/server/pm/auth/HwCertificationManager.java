...
public class HwCertificationManager {
    ...
    public int verifyHwAuthCertification(String packagePath) {
        ...
        try {
            ...
            File scanPath = new File(packagePath);
            pp.setCallback(new PackageParser.CallbackImpl(sContext.getPackageManager()));
            PackageParser.Package pkg = pp.parsePackage(scanPath, 0);
            if (!isSupportHwCertification(pkg)) {
                return 1;
            }
            PackageParser.collectCertificates(pkg, true);
            if (parseAndVerify(pkg) != null) {
                HwAuthLogger.info("HwCertificationManager", "current package contains valid cert.");
                return 0;
            }
            return 1;
        } catch (PackageParser.PackageParserException e) {
            HwAuthLogger.error("HwCertificationManager", "get package parser exception.");
        } catch (OutOfMemoryError e2) {
            HwAuthLogger.error("HwCertificationManager", "can not parse package.");
        } catch (Exception e3) {
            HwAuthLogger.error("HwCertificationManager", "check HwCertification with raw path error!");
        }
    }
}

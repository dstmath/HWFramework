...
public final class DexoptUtils {
    ...
    public static String[] getHarmonyClassLoaderContexts(String[] codePaths, ApplicationInfo info, List<SharedLibraryInfo> sharedLibraries, boolean[] pathsWithCode) {
        if (info == null || codePaths == null) {
            return new String[0];
        }
        String sharedLibrariesContext = "";
        if (sharedLibraries != null) {
        ...
        String baseApkContextClassLoader = encodeClassLoader("", info.classLoaderName, sharedLibrariesContext);
        String baseApkName = new File(info.getBaseCodePath()).getName();
        String[] classLoaderContexts = new String[codePaths.length];
        classLoaderContexts[0] = pathsWithCode[0] ? baseApkContextClassLoader : null;
        String classpath = baseApkName;
        for (int i = 1; i < classLoaderContexts.length; i++) {
            if (pathsWithCode[i]) {
            ...
            classpath = encodeClasspath(classpath, codePaths[i]);
        }
        return classLoaderContexts;
    }

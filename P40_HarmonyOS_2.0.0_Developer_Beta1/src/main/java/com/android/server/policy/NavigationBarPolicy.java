...
public class NavigationBarPolicy implements GestureDetector.OnGestureListener {
    ...
    private boolean touchDownIsValid(float pointX, float pointY) {
        ...
        HIT_REGION_TO_MAX = (int) (((double) this.mContext.getResources().getDimensionPixelSize(17105309)) / 3.5d);
        ...
        if (navigationBarPosition == 4) {
            Log.i(TAG, "pointY:" + pointY + "realSize.y:" + this.realSize.y + "HIT_REGION_TO_MAX:" + HIT_REGION_TO_MAX);
        ...
        Log.i(TAG, "pointX:" + pointX + "realSize.x:" + this.realSize.x + "HIT_REGION_TO_MAX:" + HIT_REGION_TO_MAX + "mHoleHeight:" + this.mHoleHeight);
    ...
    private void sendBroadcast(boolean minNaviBar) {
        Log.i(TAG, "sendBroadcast minNaviBar = " + minNaviBar);
    ...
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
        if (isFlingOnFrontNaviMode() && !isInLockTaskMode()) {
            Log.i(TAG, "onFling::FRONT_FINGERPRINT_NAVIGATION, return! ");
            return false;
        } else if (isGestureNavigationEnable()) {
            Log.i(TAG, "onFling::Gest_Navigation_Enable, return! ");
            return false;
            ...
            if (this.IS_SUPPORT_PRESSURE && IS_CHINA_AREA) {
                ...
                if (!ret) {
                    Log.i(TAG, "onFling::move distance is not enough, return! ");
                    return false;
                }
            }
            if (!this.mIsValidGesture) {
                Log.i(TAG, "onFling::not valid gesture , " + this.mIsValidGesture + ", return!");
                return false;
            }
            ...
            if (navigationBarPosition2 == 4) {
                ...
                Log.i(TAG, "sendBroadcast getGameControlReslut" + getGameControlReslut(2) + "1#getRawY" + e1.getRawY() + "2#getRawY" + e2.getRawY());
                ...
                Log.i(TAG, "sendBroadcast getGameControlReslut" + getGameControlReslut(3) + "1#getRawX" + e1.getRawX() + "2#getRawX" + e2.getRawX());
    ...
    private boolean isNaviBarEnabled() {
        ...
        Log.i(TAG, "isNaviBarEnable is: " + isNaviBarEnable);
    ...
    private void parseHole() {
        ...
        if (props != null && props.length == 4) {
            ...
            Log.i(TAG, "mHoleHeight = " + this.mHoleHeight);
        }
    }
    ...
    private void showGameControlNavToast(boolean isEnableSwipe, long timeElapse) {
        if (isEnableSwipe || !isNaviBarEnabled() || isGestureNavigationEnable()) {
            ...
            this.mToast = Toast.makeText(this.mContext, 33686250, 1);
            this.mToast.show();
        }
    }
}

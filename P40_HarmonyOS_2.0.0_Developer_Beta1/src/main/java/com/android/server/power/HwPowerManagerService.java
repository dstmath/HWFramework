...
public class HwPowerManagerService extends PowerManagerService {
    ...
    private native int nativDisplaySetAodBrightness(int i);
    ...
    public int setAodBrightnessInternal(int brightness) {
        Slog.i(TAG, "displaySetAodBrightness:" + brightness);
        try {
            if (!isLoadLibraryFailed) {
                return nativDisplaySetAodBrightness(brightness);
            }
            Slog.i(TAG, "nativDisplaySetAodBrightness not valid!");
            return -1;
        } catch (UnsatisfiedLinkError e) {
            Slog.e(TAG, "nativDisplaySetAodBrightness not found!");
            return -1;
        }
    }
    ...
    public void sendBootCompletedToMonitor() {
        if (this.mDisplayEffectMonitor != null) {
            ArrayMap<String, Object> params = new ArrayMap<>();
            params.put("paramType", "bootCompleted");
            this.mDisplayEffectMonitor.sendMonitorParam(params);
        }
    }
    ...
    private void setWakeUpReasonByPackageName(String opPackageName) {
        ...
        if (opPackageName != null) {
            ...
            if (hashCode != -861391249) {
                if (hashCode == 1698344559 && opPackageName.equals(HwCustPkgNameConstant.HW_SYSTEMUI_PACKAGE)) {
                    c = 0;
                }
            } else if (opPackageName.equals(PackageManagerServiceEx.PLATFORM_PACKAGE_NAME)) {
                c = 1;
            }
            if (c == 0) {
    ...
    public void configTpWhenScreenOnIfNeed() {
        if (IS_SUPPORT_AP) {
            Log.i(TAG, "Notify aod screen on");
    ...
    public void configTpWhenScreenOffIfNeed() {
        if (IS_SUPPORT_AP) {
            Log.i(TAG, "Notify aod screen off");
            HwAodManager.getInstance().configTpInApMode(101);
        }
    }
    ...
    public boolean wakeUpNoUpdateLocked(long eventTime, int reason, String details, int reasonUid, String opPackageName, int opUid) {
        if (!HwPowerManagerService.super.wakeUpNoUpdateLocked(eventTime, reason, details, reasonUid, opPackageName, opUid)) {
            return false;
        }
        configTpWhenScreenOnIfNeed();
        return true;
    ...
    public boolean goToSleepNoUpdateLocked(long eventTime, int reason, int flags, int uid) {
        if (!HwPowerManagerService.super.goToSleepNoUpdateLocked(eventTime, reason, flags, uid)) {
            return false;
        }
        configTpWhenScreenOffIfNeed();
        return true;
    }
}

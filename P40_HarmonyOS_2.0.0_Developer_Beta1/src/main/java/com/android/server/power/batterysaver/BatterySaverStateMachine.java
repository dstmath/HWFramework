...
public class BatterySaverStateMachine {
    ...
    public void triggerDynamicModeNotification() {
        ...
        ensureNotificationChannelExists(manager, DYNAMIC_MODE_NOTIF_CHANNEL_ID, 17040019);
        manager.notifyAsUser(TAG, DYNAMIC_MODE_NOTIFICATION_ID, buildNotification(DYNAMIC_MODE_NOTIF_CHANNEL_ID, this.mContext.getResources().getString(17040021), 17040020, "android.intent.action.POWER_USAGE_SUMMARY"), UserHandle.ALL);
    }
    ...
    public void triggerStickyDisabledNotification() {
        NotificationManager manager = (NotificationManager) this.mContext.getSystemService(NotificationManager.class);
        ensureNotificationChannelExists(manager, BATTERY_SAVER_NOTIF_CHANNEL_ID, 17039704);
        manager.notifyAsUser(TAG, STICKY_AUTO_DISABLED_NOTIFICATION_ID, buildNotification(BATTERY_SAVER_NOTIF_CHANNEL_ID, this.mContext.getResources().getString(17039702, NumberFormat.getPercentInstance().format(((double) this.mBatteryLevel) / 100.0d)), 17039706, "android.settings.BATTERY_SAVER_SETTINGS"), UserHandle.ALL);
    }
    ...
    private Notification buildNotification(String channelId, String title, int summaryId, String intentAction) {
        ...
        return new Notification.Builder(this.mContext, channelId).setSmallIcon(17302314).setContentTitle(title).setContentText(summary).setContentIntent(batterySaverIntent).setStyle(new Notification.BigTextStyle().bigText(summary)).setOnlyAlertOnce(true).setAutoCancel(true).build();
    }

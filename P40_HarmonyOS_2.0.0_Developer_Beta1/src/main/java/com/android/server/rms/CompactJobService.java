...
public class CompactJobService extends JobService {
    private static final int ARRAYLIST_INIT_SIZE = 16;
    private static final int COMPACT_PERIOD_INTERVAL;
    private static final List<IDefraggler> DEFRAGGLER_LISTS = new ArrayList(16);
    ...
    private static ComponentName sCompactServiceName = new ComponentName(PackageManagerServiceEx.PLATFORM_PACKAGE_NAME, CompactJobService.class.getName());
    ...
    public /* synthetic */ void lambda$onStartJob$0$CompactJobService(JobParameters jobParams) {
        ...
        Iterator<IDefraggler> it = DEFRAGGLER_LISTS.iterator();
        while (true) {
            if (!it.hasNext()) {
                break;
            }
    ...
    public boolean onStopJob(JobParameters params) {
        ...
        for (IDefraggler defraggler : DEFRAGGLER_LISTS) {
    ...
    public static void addDefragglers(IDefraggler defraggler) {
        if (defraggler != null) {
            DEFRAGGLER_LISTS.add(defraggler);
        }
    }
}

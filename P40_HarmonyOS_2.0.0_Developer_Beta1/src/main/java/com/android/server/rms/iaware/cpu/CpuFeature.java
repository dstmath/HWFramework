...
public class CpuFeature extends RFeature {
    ...
    private static final int MAX_HARMONY_THREADS = 3;
    ...
    public static final int MSG_RESET_INPUT_RTG = 189;
    ...
    public static final int MSG_SET_CPUMASK = 187;
    ...
    private static final int MSG_SET_HM_THREAD_RTG = 191;
    public static final int MSG_SET_INPUT_RTG = 188;
    public static final int MSG_SET_LIMIT_CGROUP = 137;
    private static final int MSG_SET_RTG_PROCESS = 149;
    ...
    private static final int RESET_PREAD_INPUT_RTG_DELAY_MS = 250;
    ...
    private boolean mInputMethodUseRtg = false;
    private int mRtgFocusPid = 0;
    ...
    public boolean reportData(CollectData data) {
        ...
        if (resId == AwareConstant.ResourceType.RESOURCE_SCREEN_ON.ordinal()) {
            ...
            Message msg = this.mCpuFeatureHandler.obtainMessage(MSG_SET_RTG_PROCESS);
            msg.obj = data;
            this.mCpuFeatureHandler.sendMessage(msg);
        } else if (resId == AwareConstant.ResourceType.RESOURCE_SET_HM_THREAD_RTG.ordinal()) {
            Message msg2 = this.mCpuFeatureHandler.obtainMessage(MSG_SET_HM_THREAD_RTG);
            msg2.obj = data;
            ...
            Message msg3 = this.mCpuFeatureHandler.obtainMessage(MSG_INPUT_EVENT);
            msg3.obj = data;
            this.mCpuFeatureHandler.sendMessage(msg3);
            CpuHighLoadManager.getInstance().reportData(data);
        } else if (resId == AwareConstant.ResourceType.RESOURCE_SHOW_INPUTMETHOD.ordinal()) {
            Bundle bundle = data.getBundle();
            if (bundle != null) {
                int eventId = bundle.getInt("eventid", -1);
                int pid = bundle.getInt("callPid");
                if (eventId == 34 && pid > 0) {
                    Message msg4 = this.mCpuFeatureHandler.obtainMessage(MSG_SET_INPUT_RTG);
                    msg4.arg1 = pid;
                    this.mCpuFeatureHandler.sendMessage(msg4);
                }
            }
        } else {
    ...
    private void subscribeResourceTypes() {
        ...
        this.mDataRegister.subscribeData(AwareConstant.ResourceType.RESOURCE_SET_HM_THREAD_RTG, this.mFeatureType);
    ...
    private void unsubscribeResourceTypes() {
        ...
        this.mDataRegister.unSubscribeData(AwareConstant.ResourceType.RESOURCE_SET_HM_THREAD_RTG, this.mFeatureType);
    ...
    private void enableRtgSchedPlugin() {
        if (isFeatureEnable(SWITCH_RTG_SCHED)) {
            AwareRmsRtgSchedPlugin.getInstance().enable(this.mCpuFeatureHandler, this);
        }
    }
    ...
    public boolean enableFeatureEx(int realVersion) {
        ...
        enableMultiDisplay();
        ResourceCollector.setThreadVipPriority(Process.myTid(), 8);
        return true;
    }
    ...
    public boolean disable() {
        ...
        disableGameScene();
        ResourceCollector.setThreadVipPriority(Process.myTid(), 0);
        return true;
    }
    ...
    public class CpuFeatureHandler extends Handler {
        ...
        private void handleMessageDefault(Message msg) {
            int i = msg.what;
            if (i == CpuFeature.MSG_SET_RTG_PROCESS) {
                CpuFeature.this.handleMessageRtgProcess(msg);
            } else if (i == 153) {
                CpuFeature.this.doVipGroupSet(msg);
            } else if (i == CpuFeature.MSG_SET_HM_THREAD_RTG) {
                CpuFeature.this.handleMessageSetHmThreadToRtg(msg);
            } else if (i == 188) {
                CpuFeature.this.handleMessageSetInputRtg(msg);
            } else if (i != 189) {
                switch (i) {
                ...
                CpuFeature.this.handleMessageResetInputRtg(msg);
            }
        }
    }

    public void removeInputRtgMessage() {
        this.mInputMethodUseRtg = false;
        this.mCpuFeatureHandler.removeMessages(MSG_RESET_INPUT_RTG);
    }

    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void handleMessageResetInputRtg(Message msg) {
        if (this.mInputMethodUseRtg) {
            this.mInputMethodUseRtg = false;
            AwareRmsRtgSchedPlugin.getInstance().setRtgProcess(this.mRtgFocusPid, -1, 5);
        }
    }

    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void handleMessageSetInputRtg(Message msg) {
        if (!this.mInputMethodUseRtg) {
            int inputMethodPid = msg.arg1;
            this.mRtgFocusPid = AwareRmsRtgSchedPlugin.getInstance().getFocusAppPid();
            this.mInputMethodUseRtg = true;
            AwareRmsRtgSchedPlugin.getInstance().setRtgProcess(inputMethodPid, -1, 5);
            Message resetMsg = this.mCpuFeatureHandler.obtainMessage(MSG_RESET_INPUT_RTG);
            resetMsg.arg1 = inputMethodPid;
            this.mCpuFeatureHandler.removeMessages(MSG_RESET_INPUT_RTG);
            this.mCpuFeatureHandler.sendMessageDelayed(resetMsg, 250);
        }
    }

    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void handleMessageRtgProcess(Message msg) {
        Bundle bundle;
        if ((msg.obj instanceof CollectData) && (bundle = ((CollectData) msg.obj).getBundle()) != null) {
            ...
            if (CpuMultiDisplay.getInstance().isPhoneDisplay(pid)) {
                removeInputRtgMessage();
                AwareRmsRtgSchedPlugin.getInstance().setRtgProcess(pid, renderTid, type);
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void handleMessageSetHmThreadToRtg(Message msg) {
        Bundle bundle;
        if ((msg.obj instanceof CollectData) && (bundle = ((CollectData) msg.obj).getBundle()) != null) {
            int pid = bundle.getInt(SceneRecogFeature.DATA_PID);
            try {
                String[] params = bundle.getString("param").split(CpuCustBaseConfig.CPUCONFIG_GAP_IDENTIFIER);
                int type = "set".equals(params[0].split(":")[1]) ? 1 : 0;
                List<Integer> tids = new ArrayList<>();
                for (String tid : params[1].split(":")[1].split(",")) {
                    tids.add(Integer.valueOf(CpuCommonUtil.parseInt(tid)));
                }
                if (tids.size() > 3) {
                    AwareLog.e(TAG, "failed to set Harmony threads to RTG, too many threads");
                } else {
                    AwareRmsRtgSchedPlugin.getInstance().setHmThreadToRtg(pid, tids, type);
                }
            } catch (ArrayIndexOutOfBoundsException | PatternSyntaxException e) {
                AwareLog.e(TAG, "failed to set Harmony threads to RTG, wrong format");
            }
        }
    }

...
public class TrustManagerService extends SystemService {
    ...
    private final Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case 14:
                    if (msg.arg2 == 1) {
                        TrustManagerService.this.updateTrust(msg.arg1, 0, true, false, false, false);
                    }
    ...
    private boolean mTrustedThingsState;
    ...
    public void updateTrust(int userId, int flags) {
        updateTrust(userId, flags, false, false, false, false);
    }

    public void updateTrustFromRevoke(int userId, int flags, boolean isFeatureAndScreenOff) {
        updateTrust(userId, flags, false, isFeatureAndScreenOff, true, false);
    }

    public void updateTrustFromGrand(int userId, int flags, boolean isNewAuth) {
        updateTrust(userId, flags, false, false, false, isNewAuth);
    }
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:63:0x00ce, code lost:
        r0 = th;
     */
    private void updateTrust(int userId, int flags, boolean isFromUnlock, boolean isFeatureAndScreenOff, boolean needUpdate, boolean isNewAuth) {
        boolean changed;
        if (needUpdate) {
            this.mTrustedThingsState &= isFeatureAndScreenOff;
        }
        ...
        synchronized (this.mUserIsTrusted) {
            ...
            if (this.mSettingsObserver.getTrustAgentsExtendUnlock()) {
                trusted = trusted && (!showingKeyguard || isFromUnlock || !(this.mUserIsTrusted.get(userId) != trusted) || (this.mTrustedThingsState && isNewAuth)) && userId == this.mCurrentUser;
                if (DEBUG) {
    ...
    private static ComponentName getDefaultFactoryTrustAgent(Context context) {
        String defaultTrustAgent = context.getResources().getString(17039824);
    ...
    private void dispatchUnlockAttempt(boolean successful, int userId) {
        if (successful) {
            ...
            updateTrust(userId, 0, true, false, false, false);
        }
        for (int i = 0; i < this.mActiveAgents.size(); i++) {
            AgentInfo info = this.mActiveAgents.valueAt(i);

...
class MtpNotificationManager {
    ...
    public void showNotification(UsbDevice device) {
        ...
        Notification.Builder builder = new Notification.Builder(this.mContext, SystemNotificationChannels.USB).setContentTitle(resources.getString(17041417, device.getProductName())).setContentText(resources.getString(17041416)).setSmallIcon(17303576).setCategory("sys");
        Intent intent = new Intent(ACTION_OPEN_IN_APPS);
        intent.putExtra("device", device);
        intent.addFlags(1342177280);
        builder.setContentIntent(PendingIntent.getBroadcastAsUser(this.mContext, device.getDeviceId(), intent, DumpState.DUMP_HWFEATURES, UserHandle.SYSTEM));

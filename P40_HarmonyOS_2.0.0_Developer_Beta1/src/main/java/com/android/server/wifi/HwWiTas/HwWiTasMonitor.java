...
public class HwWiTasMonitor implements IGameReportCallback {
    ...
    public void onReportGameDelay(int delay) {
        ...
        if (this.mWiTasArbitra.isTestMode()) {
            gameDelay = this.mWiTasTest.getGameDelay(delay);
        }
        this.mWiTasArbitra.setGameDelay(gameDelay);
        if (this.mWiTasArbitra.isGameLag(gameDelay)) {
            this.mHandler.sendEmptyMessage(9);
        }
    }

    public void onReportGameState(boolean isEnable, boolean isInWhiteList, int id) {
        HwHiLog.d(HwWiTasUtils.TAG, false, "reportGameState, enable: %{public}s, inWhiteList: %{public}s, id: %{public}d", new Object[]{String.valueOf(isEnable), String.valueOf(isInWhiteList), Integer.valueOf(id)});
        ...
        this.mWiTasArbitra.updateGameWhiteListInfo(isInWhiteList);
    }
    ...
    public void reportAntRssi(int index, int antRssi) {
        ...
        int tmpAntRssi = antRssi;
        if (this.mWiTasArbitra.isTestMode()) {
            if (measureState == 1) {
                tmpAntRssi = this.mWiTasTest.getSrcRssi(antRssi);
            } else if (measureState == 2) {
                tmpAntRssi = this.mWiTasTest.getDstRssi(antRssi);
            } else {
                return;
            }
        }
        if (HwWiTasUtils.mCoreAntIndex == 0 && tmpAntRssi == -130) {
        ...
        if (HwWiTasUtils.mCoreAntIndex == 0 && tmpAntRssi == -128) {
            this.mHandler.sendEmptyMessage(20);
            return;
        }
        Handler handler = this.mHandler;
        handler.sendMessage(handler.obtainMessage(msgId, tmpAntRssi, 0));
    }
            ...
            if (intent != null) {
                ...
                if ("android.net.wifi.WIFI_STATE_CHANGED".equals(action)) {
                    ...
                    if (netInfo.getState() == NetworkInfo.State.CONNECTED) {
                        ...
                        HwHiLog.d(HwWiTasUtils.TAG, false, "no match netInfo state.", new Object[0]);
                    }
                } else if ("android.intent.action.BOOT_COMPLETED".equals(action)) {
                    HwHiLog.d(HwWiTasUtils.TAG, false, "BOOT_COMPLETED", new Object[0]);
                    HwArbitrationCallbackImpl.getInstance(context).registerGameReport(HwWiTasMonitor.mWiTasMonitor);

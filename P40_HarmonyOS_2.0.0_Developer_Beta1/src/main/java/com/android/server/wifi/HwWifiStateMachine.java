...
public class HwWifiStateMachine extends ClientModeImpl {
    ...
    private static final String[] AP_INFO_WHITE_LIST = {"com.android.nfc", "com.android.mediacenter", "com.android.settings", "com.android.imedia.syncplay", "com.huawei.smarthome", "com.huaweioverseas.smarthome", "com.huawei.smartspeaker", "com.huawei.health:PhoneService"};
    ...
    private static final int FREQUENCY_BOUNDARY = 5180;
    private static final int GET_WIFI_CATEGORY_TIMEOUT = 100;
    private static final int HAL_SET_WIFI_SLEEP_MODE_COMMAND = 155;
    private static final int HAL_WIFI_SLEEP_MODE_OFF = 0;
    private static final int HAL_WIFI_SLEEP_MODE_ON = 1;
    ...
    private static final long WIFI_LOW_LATENCY_AGING_TIMEOUT = 300000;
    private static final long WIFI_LOW_LATENCY_SCAN_INTERVAL = 60000;
    ...
    private HashMap<String, Long> lowLatencyMap = new HashMap<>();
            ...
            if (intent != null) {
                ...
                if (HwWifiStateMachine.BRCM_CHIP_4359.equals(chipName)) {
                    if ("android.intent.action.BATTERY_CHANGED".equals(action)) {
                        ...
                        HwWifiStateMachine hwWifiStateMachine7 = HwWifiStateMachine.this;
                        hwWifiStateMachine7.logd("setpmlock: do nothing action = " + action);
                    }
                }
            }
        }
    ...
    private final Object setLowLarencyLock = new Object();
    ...
    static /* synthetic */ int access$2408(HwWifiStateMachine x0) {
        int i = x0.mPwrBoostOncnt;
        x0.mPwrBoostOncnt = i + 1;
        return i;
    }

    static /* synthetic */ int access$2508(HwWifiStateMachine x0) {
    ...
    private void disableAllNetworksExceptLastConnected() {
        ...
        WifiConfigStoreUtils.getConfiguredNetworks(wifiStateMachineUtils.getWifiConfigManager(this));
    }
    ...
    public boolean hasMeteredHintForWi(Inet4Address ip) {
        ...
        if ("mshome.net".equals(SystemProperties.get("dhcp.wlan0.domain", ""))) {
    ...
    private void clearHwPortalHistory(WifiConfiguration config) {
        String currentBssid = getCurrentBSSID();
        if (!TextUtils.isEmpty(currentBssid) && ScanResultRecords.getDefault().getHiLinkAp(currentBssid) == 1) {
            HwHiLog.d(TAG, false, "failed if HwAp, clear the portal history", new Object[0]);
            if (config != null && config.internetHistory != null && config.internetHistory.contains(String.valueOf(2))) {
    ...
    private void closeWifi2WhenWifi1Disconnected(NetworkInfo networkInfo) {
        if (networkInfo.getDetailedState() == NetworkInfo.DetailedState.DISCONNECTED) {
            ...
            hwWifi2Manager.setWifi2Enable(false, HwWifi2Manager.CLOSE_WIFI2_WIFI1_DISCONNECTED_BY_WLAN_PRO);
        }
    }
    ...
    public void sendStaFrequency(int frequency) {
        if (mFrequency != frequency && frequency >= FREQUENCY_BOUNDARY) {
    ...
    public void handleSimAbsent(WifiConfiguration config) {
        ...
        if (PreconfiguredNetworkManager.IS_R1 && config.enterpriseConfig != null && TelephonyUtil.isSimEapMethod(config.enterpriseConfig.getEapMethod()) && PreconfiguredNetworkManager.getInstance().isPreconfiguredNetwork(config.SSID)) {
            ...
            this.wifiEapUIManager.showDialog(Resources.getSystem().getString(33686278), Resources.getSystem().getString(33686274));
        }
    }
    ...
    private class pwrBoostHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (i != 0) {
                ...
                handleChargePluggedinNotFinishLinkDetect();
        ...
        private void handleChargePluggedinNotFinishLinkDetect() {
            if (HwWifiStateMachine.this.mIsChargePluggedin && !HwWifiStateMachine.this.mIsFinishLinkDetect) {
                ...
                if (!hwWifiStateMachine.mCurrentPwrBoostStat) {
                    ...
                    HwWifiStateMachine.access$2408(HwWifiStateMachine.this);
                    ...
                    HwWifiStateMachine.access$2508(HwWifiStateMachine.this);
                }
            }
        }
    }
    ...
    public int setWifiLowLatencyMode(String packageName, boolean enabled) {
        if (TextUtils.isEmpty(packageName)) {
            HwHiLog.e(TAG, false, "setWifiLowLatencyMode: invalid package name", new Object[0]);
            return -1;
        } else if (enabled) {
            return enableWifiLowLatency(packageName);
        } else {
            return disableWifiLowLatency(packageName);
        }
    }

    private int enableWifiLowLatency(String packageName) {
        synchronized (this.setLowLarencyLock) {
            if (this.lowLatencyMap.size() == 0) {
                if (setWifiSleepMode(0) != 0) {
                    HwHiLog.e(TAG, false, "enableWifiLowLatency: setWifiSleepMode failed", new Object[0]);
                    return -1;
                } else if (!WifiInjector.getInstance().getClientModeImplHandler().postDelayed(new Runnable() {
                    /* class com.android.server.wifi.$$Lambda$HwWifiStateMachine$Mv5Kc9X99twQXRFqg9eak2fh6oo */

                    @Override // java.lang.Runnable
                    public final void run() {
                        HwWifiStateMachine.this.lambda$enableWifiLowLatency$3$HwWifiStateMachine();
                    }
                }, WIFI_LOW_LATENCY_SCAN_INTERVAL)) {
                    setWifiSleepMode(1);
                    HwHiLog.e(TAG, false, "enableWifiLowLatency: failed to post runnable", new Object[0]);
                    return -1;
                }
            }
            this.lowLatencyMap.put(packageName, Long.valueOf(SystemClock.elapsedRealtime()));
            return 0;
        }
    }

    private int disableWifiLowLatency(String packageName) {
        synchronized (this.setLowLarencyLock) {
            if (!this.lowLatencyMap.containsKey(packageName)) {
                HwHiLog.e(TAG, false, "disableWifiLowLatency: no node for " + packageName, new Object[0]);
                return -1;
            }
            this.lowLatencyMap.remove(packageName);
            if (this.lowLatencyMap.size() != 0 || setWifiSleepMode(1) == 0) {
                return 0;
            }
            HwHiLog.e(TAG, false, "disableWifiLowLatency: setWifiSleepMode failed", new Object[0]);
            return -1;
        }
    }

    /* access modifiers changed from: private */
    /* renamed from: wifiLowLatencyAgingProc */
    public void lambda$wifiLowLatencyAgingProc$4$HwWifiStateMachine() {
        synchronized (this.setLowLarencyLock) {
            Iterator<Map.Entry<String, Long>> it = this.lowLatencyMap.entrySet().iterator();
            while (it.hasNext()) {
                if (SystemClock.elapsedRealtime() - it.next().getValue().longValue() > WIFI_LOW_LATENCY_AGING_TIMEOUT) {
                    it.remove();
                }
            }
            if (this.lowLatencyMap.size() == 0) {
                setWifiSleepMode(1);
                HwHiLog.i(TAG, false, "wifiLowLatencyAgingProc: disable low latency mode at timeout", new Object[0]);
            } else if (!WifiInjector.getInstance().getClientModeImplHandler().postDelayed(new Runnable() {
                /* class com.android.server.wifi.$$Lambda$HwWifiStateMachine$JBirSXAL8Cg3y934GzVn8aCQCRY */

                @Override // java.lang.Runnable
                public final void run() {
                    HwWifiStateMachine.this.lambda$wifiLowLatencyAgingProc$4$HwWifiStateMachine();
                }
            }, WIFI_LOW_LATENCY_SCAN_INTERVAL)) {
                setWifiSleepMode(1);
                HwHiLog.e(TAG, false, "wifiLowLatencyAgingProc: failed to post runnable", new Object[0]);
            }
        }
    }

    private int setWifiSleepMode(int cmd) {
        IHwWifiNativeEx wifiNativeEx = null;
        if (!(WifiInjector.getInstance() == null || WifiInjector.getInstance().getWifiNative() == null)) {
            wifiNativeEx = WifiInjector.getInstance().getWifiNative().mHwWifiNativeEx;
        }
        if (wifiNativeEx != null) {
            return wifiNativeEx.sendCmdToDriver("wlan0", (int) HAL_SET_WIFI_SLEEP_MODE_COMMAND, new byte[]{(byte) cmd});
        }
        HwHiLog.e(TAG, false, "setWifiSleepMode: IHwWifiNativeEx is null", new Object[0]);
        return -1;
    }
}

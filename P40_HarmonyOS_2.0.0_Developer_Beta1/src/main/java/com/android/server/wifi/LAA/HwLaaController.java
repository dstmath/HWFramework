...
public class HwLaaController {
    ...
    private static HwLaaController sHwLaaController;
    ...
    public static HwLaaController createHwLaaController(Context context) {
        if (sHwLaaController == null) {
            sHwLaaController = new HwLaaController(context);
            HwLaaUtils.logD(TAG, false, "createHwLaaController", new Object[0]);
        }
        return sHwLaaController;
    }
        ...
        this.mHwLaaControllerHandler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                ...
                if (i == 1) {
                    ...
                    if (i != 3) {
                        if (i != 4) {
                            HwLaaUtils.logD(HwLaaController.TAG, false, "unknown msg %{public}d", Integer.valueOf(msg.what));
                            return;
                        }
    ...
    private boolean sendLaaCmdToTelephony(int cmd) {
        ...
        try {
            ...
            HwLaaUtils.logD(TAG, false, "NoSuchMethodException", new Object[0]);
        }
        if (method != null) {
            try {
                result = method.invoke(this.hwTelephonyManagerProxy, Integer.valueOf(cmd), null, null);
            } catch (IllegalAccessException e2) {
                HwLaaUtils.logD(TAG, false, "IllegalAccessException", new Object[0]);
            } catch (IllegalArgumentException e3) {
                HwLaaUtils.logD(TAG, false, "IllegalArgumentException", new Object[0]);
            } catch (InvocationTargetException e4) {
                HwLaaUtils.logD(TAG, false, "InvocationTargetException", new Object[0]);
            }
            if (result == null) {
                ...
                HwLaaUtils.logD(TAG, false, "sendLaaCmdToTelephony unknown result type", new Object[0]);
                return false;
            }
        } else {

...
public class HwMssBluetoothManager {
    ...
    private static HwMssBluetoothManager sHwMssBluetoothManager = null;
    ...
    private BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if ("android.net.wifi.p2p.CONNECTION_STATE_CHANGE".equals(action) || "android.net.wifi.p2p.CONNECT_STATE_CHANGE".equals(action)) {
                ...
                HwHiLog.d(HwMssBluetoothManager.TAG, false, "No processing type", new Object[0]);
            }
    ...
    public static synchronized HwMssBluetoothManager getInstance(Context context) {
        ...
        synchronized (HwMssBluetoothManager.class) {
            if (sHwMssBluetoothManager == null) {
                sHwMssBluetoothManager = new HwMssBluetoothManager(context);
            }
            hwMssBluetoothManager = sHwMssBluetoothManager;
        }
    ...
    private void wifiP2pStateChange(Intent intent) {
        if (intent != null) {
            ...
            if ("android.net.wifi.p2p.CONNECTION_STATE_CHANGE".equals(action)) {
                ...
                HwHiLog.d(TAG, false, "p2p receiver not handled", new Object[0]);
    ...
    private boolean isAppointedA2dpDeviceConnected() {
        BluetoothCodecConfig config = getBtCodecConfig();
        if (config == null) {
            HwHiLog.e(TAG, false, "isAppointedA2dpDeviceConnected, config is null", new Object[0]);
            return false;
        }
        ...
        if (bluetoothA2dp == null) {
            HwHiLog.e(TAG, false, "isAppointedA2dpDeviceConnected, mA2dpService is null", new Object[0]);
            return false;
        }
    ...
    public boolean isAntennaSwitchSuccess(int coextMode) {
        if (this.mWifiNative == null) {
            ...
            return false;
        } else if (SystemProperties.getInt("runtime.hwmss.errtest", 0) == 1) {
    ...
    private synchronized void checkAndSwitchCoextMode() {
        int coextMode = 0;
        if (this.mIsWiFi2GConnected && isAppointedA2dpDeviceConnected()) {
            coextMode = 1;
        }

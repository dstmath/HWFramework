    ...
    public void onPostNotification() {
        ...
        if (!this.mPosted) {
            ...
            if (aInfo == null || (aInfo.privateFlags & 8) == 0 || (!PKG_NOTE_PAD.equals(this.mPackageName) && !PKG_NOTE_PAD_NEW.equals(this.mPackageName))) {
                ...
                Notification.Builder builder = new Notification.Builder(context, this.mNotificationTag).setOngoing(true).setContentTitle(context.getString(17039586, appName)).setContentText(message).setSmallIcon(17301716).setColor(context.getColor(17170460)).setStyle(new Notification.BigTextStyle().bigText(message)).setLocalOnly(true).addExtras(extras).setContentIntent(getContentIntent(context, this.mPackageName));

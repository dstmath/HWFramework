...
public class AppTransition implements DumpUtils.Dump {
    ...
    static final int DEFAULT_APP_TRANSITION_DURATION = (IS_TV ? 350 : THUMBNAIL_APP_TRANSITION_DURATION);
    ...
    private long mLastClipRevealTransitionDuration = ((long) DEFAULT_APP_TRANSITION_DURATION);
    ...
    /* access modifiers changed from: package-private */
    public boolean isTvTransitionSet() {
    ...
    private boolean prepare() {
        ...
        this.mLastClipRevealTransitionDuration = (long) DEFAULT_APP_TRANSITION_DURATION;
        return true;
    }
    ...
    /* JADX DEBUG: Multi-variable search result rejected for r6v13, resolved type: android.view.animation.AnimationSet */
    ...
    private Animation createScaleUpAnimationLocked(int transit, boolean enter, Rect containingFrame) {
        ...
        if (transit == NEXT_TRANSIT_TYPE_THUMBNAIL_ASPECT_SCALE_DOWN || transit == NEXT_TRANSIT_TYPE_CUSTOM_IN_PLACE) {
            ...
            duration = (long) DEFAULT_APP_TRANSITION_DURATION;
        }
    ...
    private long calculateClipRevealTransitionDuration(boolean cutOff, float translationX, float translationY, Rect displayFrame) {
        if (!cutOff) {
            return (long) DEFAULT_APP_TRANSITION_DURATION;
        }
        float fraction = Math.max(Math.abs(translationX) / ((float) displayFrame.width()), Math.abs(translationY) / ((float) displayFrame.height()));
        int i = DEFAULT_APP_TRANSITION_DURATION;
        return (long) (((float) i) + (((float) (420 - i)) * fraction));
    }
    ...
    public GraphicBuffer createCrossProfileAppsThumbnail(int thumbnailDrawableRes, Rect frame) {
        ...
        int thumbnailSize = this.mService.mContext.getResources().getDimensionPixelSize(17105087);
    ...
    private Animation createRelaunchAnimation(Rect containingFrame, Rect contentInsets) {
        ...
        set.setDuration((long) DEFAULT_APP_TRANSITION_DURATION);
        set.setZAdjustment(1);
        return set;
    }
    ...
    /* JADX WARNING: Removed duplicated region for block: B:158:0x03a3  */
                        ...
                        if (i14 == 3) {
                            ...
                            if (i14 == 5) {
                                ...
                                if (i14 == NEXT_TRANSIT_TYPE_OPEN_CROSS_PROFILE_APPS && enter) {
                                    ...
                                    a = new AlphaAnimation(1.0f, 1.0f);
                                    a.setDuration((long) WindowChangeAnimationSpec.ANIMATION_DURATION);
                                    if (WindowManagerDebugConfig.DEBUG_APP_TRANSITIONS || WindowManagerDebugConfig.DEBUG_ANIM) {
    ...
    static boolean isChangeTransit(int transit) {
        return transit == 27 || transit == 50 || transit == 51 || transit == 52;
    }

...
public class Dimmer {
    ...
    private int mLastUpdateWindowingMode;
    ...
    Dimmer(WindowContainer host, SurfaceAnimatorStarter surfaceAnimatorStarter) {
        ...
        if (windowContainer != null) {
            this.mLastUpdateWindowingMode = windowContainer.getWindowingMode();
        }
    }
    ...
    public boolean updateDims(SurfaceControl.Transaction t, Rect bounds) {
        ...
        int currentWindowingMode = windowContainer2 != null ? windowContainer2.getWindowingMode() : this.mLastUpdateWindowingMode;
        int i = this.mLastUpdateWindowingMode;
        boolean isWindowingModeChanged = currentWindowingMode != i && i == 102;
        WindowContainer windowContainer3 = this.mHost;
        if (windowContainer3 != null && ((windowContainer3.inHwFreeFormWindowingMode() || isWindowingModeChanged) && this.mHost.mWmService != null)) {
            if (this.mHost.isAppAnimating() && this.mHost.inHwFreeFormWindowingMode()) {
                return false;
            }
            float hwFreeFormScale = 1.0f;
            WindowContainer windowContainer4 = this.mHost;
            if (windowContainer4 instanceof Task) {
                TaskStack stack = ((Task) windowContainer4).mStack;
                hwFreeFormScale = stack == null ? 1.0f : stack.mHwStackScale;
            } else if (windowContainer4 instanceof TaskStack) {
                hwFreeFormScale = ((TaskStack) windowContainer4).mHwStackScale;
            }
        ...
        this.mLastUpdateWindowingMode = currentWindowingMode;
        return true;
    }

...
public final class HwActivityStackSupervisor extends ActivityStackSupervisorBridgeEx {
    ...
    private boolean isUninstallableApk(String pkgName, Intent homeIntent, int userId) {
        ...
        ComponentInfo ci;
        if (pkgName == null || "android".equals(pkgName) || (homeInfo = resolveIntent(homeIntent, homeIntent.resolveTypeIfNeeded(this.mAtmsEx.getContext().getContentResolver()), userId, 786432, Binder.getCallingUid())) == null || (ci = ResolveInfoEx.getComponentInfo(homeInfo)) == null) {
            return false;
        }
    ...
    public void recognitionMaliciousApp(IApplicationThreadEx caller, Intent intent, int userId) {
        Intent homeIntent;
        if ((caller == null || caller.isApplicationThreadNull()) && intent != null && (homeIntent = this.mAtmsEx.getHomeIntent()) != null && homeIntent.getCategories() != null) {
            ...
            if (isActionEqualsHome && isCategoryContainsHome) {
                ...
                if (strPkg != null && !HwPartCommInterfaceWraper.disallowOp(26, strPkg) && isUninstallableApk(strPkg, homeIntent, userId) && (IntentExEx.getHwFlags(intent) & 32768) == 0) {
    ...
    private void updateCrashInfo(String strPkg) {
        if (strPkg.equals(this.mLastHomePkg)) {
            ...
            if (now - this.mFirstLaunchTime < 60000) {
                try {
                    ActivityThreadEx.clearPackagePreferredActivities(strPkg);
                } catch (RemoteException e) {
                    SlogEx.e(TAG, " Update crash info fail.");
                }
    ...
    public boolean shouldNotKillProcWhenRemoveTask(String pkg) {
        if (!"com.tencent.mm".equals(pkg) || SystemPropertiesEx.getBoolean("hw.app.smart_cleaning", false)) {
            return false;
        }
        SlogEx.d(TAG, "cleanUpRemovedTaskLocked, do not kill process : " + pkg);
        return true;
    }
    ...
    public boolean hasActivityInStackLocked(ActivityInfo activityInfo) {
        ...
        int userId = UserHandleEx.getUserId(activityInfo.applicationInfo.uid);

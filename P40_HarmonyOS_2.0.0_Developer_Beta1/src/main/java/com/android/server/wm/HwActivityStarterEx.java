...
public class HwActivityStarterEx implements IHwActivityStarterEx {
    ...
    private void effectiveIawareToLaunchAppWhenNotCurrent(ActivityInfo targetActivityInfo, String strPkg, int canBoost, String strMsg) {
        ...
        if (IS_WATCH) {
            isProcessRunning = isWatchAppWarmStart(strPkg, targetActivityInfo);
        }
        ...
        if (hasActivityInStack) {
            ...
            UniPerf.getInstance().uniPerfEvent(4399, strMsg, new int[0]);
        } else if (HwCustPkgNameConstant.HW_SYSTEMUI_PACKAGE.equals(strPkg)) {
    ...
    private boolean isWatchAppWarmStart(String strPkg, ActivityInfo targetActivityInfo) {
        if (this.mService.getProcessController(strPkg, (targetActivityInfo == null || targetActivityInfo.applicationInfo == null) ? 0 : targetActivityInfo.applicationInfo.uid) != null) {
            return true;
        }
        return false;
    }
    ...
    public boolean isAbleToLaunchInPCCastMode(String shortComponentName, int displayId, ActivityRecord reusedActivity) {
        ...
        String[] tmp = shortComponentName.split(AodThemeConst.SPLASH);
        if (tmp.length != 2) {
            return true;
        }
    ...
    private boolean packageShouldNotHandle(String pkgName) {
        return HwPCUtils.enabledInPad() ? "com.huawei.desktop.explorer".equals(pkgName) || "com.huawei.desktop.systemui".equals(pkgName) || HwCustPkgNameConstant.HW_SYSTEMUI_PACKAGE.equals(pkgName) || "com.android.incallui".equals(pkgName) || "com.huawei.android.wfdft".equals(pkgName) || PKG_PARENT_CONTROL.equals(pkgName) || PKG_ICONNECT.equals(pkgName) : HwPCUtils.isHiCarCastMode() ? "com.huawei.desktop.explorer".equals(pkgName) || "com.huawei.desktop.systemui".equals(pkgName) || "com.huawei.hicar".equals(pkgName) : "com.huawei.desktop.explorer".equals(pkgName) || "com.huawei.desktop.systemui".equals(pkgName);
    }
                    ...
                    public void run() {
                        ...
                        if (HwPCUtils.isValidExtDisplayId(displayId)) {
                            if (isAppLock) {
                                ...
                                hwActivityStarterEx.mToast = Toast.makeText(context, context.getResources().getString(33686113), 0);
    ...
    private boolean isKillProcessOnOtherDisplay(ActivityRecord startActivity, int sourceDisplayId) {
        ...
        while (true) {
            ...
            if (HwPCUtils.enabledInPad() && WifiProCommonUtils.WIFI_SETTINGS_PHONE.equals(startActivity.packageName) && "com.android.phone".equals(list.get(i).mName)) {
                HwPCUtils.log(TAG, "settings in phone process");
            } else if (TextUtils.isEmpty(startActivity.packageName) || !startActivity.packageName.contains("com.tencent.mm") || startActivity.app == null || startActivity.app.mPid == list.get(i).mPid) {
                break;
            } else {
        ...
        String processName = list.get(i).mName;
        int targetDisplayId = list.get(i).mDisplayId;
    ...
    public Bundle checkActivityStartedOnDisplay(ActivityRecord startActivity, int preferredDisplayId, ActivityOptions options, ActivityRecord reusedActivity) {
        if (startActivity == null) {
            ...
            Slog.i(TAG, "App support multiDisplay, do not check for " + startActivity.packageName);
            if (this.mService.mHwATMSEx.getPkgDisplayMaps().containsKey(startActivity.packageName)) {
                this.mService.mHwATMSEx.getPkgDisplayMaps().remove(startActivity.packageName);
            }
            return null;
    ...
    private boolean isSupportMultiDisplayMetaData(String packageName) {
        ...
        try {
            ...
            Slog.i(TAG, "isSupportMultiDisplay:" + isSupportMultiDisplay + ", pkg:" + packageName);
            ...
            public void run() {
                ...
                try {
                    if (record == null || record.appInfo == null) {
                        ...
                        HwPCUtils.log(HwActivityStarterEx.TAG, "AMS forceStopPackage, start. name:" + record.packageName);
    ...
    /* JADX WARNING: Removed duplicated region for block: B:31:0x0063 A[Catch:{ RemoteException -> 0x0073 }] */
    /* JADX WARNING: Removed duplicated region for block: B:32:0x006d A[Catch:{ RemoteException -> 0x0073 }] */

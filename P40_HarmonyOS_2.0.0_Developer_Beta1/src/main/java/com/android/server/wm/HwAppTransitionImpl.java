...
public class HwAppTransitionImpl implements IHwAppTransition {
    ...
    private static final long APLHA_DELAY_TIME = 100;
    ...
    private static final PathInterpolator CUBIC_BEZIER_INTERPOLATOR_20_80 = new PathInterpolator(0.2f, 0.0f, 0.2f, 1.0f);
    ...
    private static final int G1_CORNER_CURVE_POSITION = 3;
    private static final int G1_CORNER_CURVE_TYPE = 1;
    ...
    private static final long SCREEN_ROTATION_ALPHA_DURATION = 150;
    private static final long SCREEN_ROTATION_CLIP_DURATION = 200;
    private static final long SCREEN_ROTATION_OVERALL_DURATION = 350;
    ...
    private static final long SUPER_LITE_SCALE_DURATION = 250;
    ...
    public AttributeCache.Entry overrideAnimation(WindowManager.LayoutParams lp, int animAttr, Context ctx, AttributeCache.Entry entry, AppTransition appTransition) {
        int anim;
        int hwAnimResId;
        int hwAnimResId2;
        if (lp != null) {
        ...
        if (entry == null || entry.array.getResourceId(animAttr, 0) == 0) {
            return null;
        }
        ...
        if (this.mHwextContext == null) {
            try {
                this.mHwextContext = ctx.createPackageContext("androidhwext", 0);
        ...
        if (this.mHwextContext != null) {
            ...
            if (title != null && !"".equals(title)) {
                if ((IS_EMUI_LITE || IS_NOVA_PERF) && !IS_FORCE_FULL_ANIM_ENABLE) {
                    ...
                    hwAnimResId2 = resources.getIdentifier("HwAnimation_lite." + title, AodThemeConst.THEME_STYLE_KEY, "androidhwext");
                } else {
                    Resources resources2 = this.mHwextContext.getResources();
                    hwAnimResId2 = resources2.getIdentifier("HwAnimation." + title, AodThemeConst.THEME_STYLE_KEY, "androidhwext");
                }
            ...
            if ((IS_EMUI_LITE || IS_NOVA_PERF) && !IS_FORCE_FULL_ANIM_ENABLE && anim2 == 0 && (hwAnimResId = this.mHwextContext.getResources().getIdentifier("HwAnimation_lite", AodThemeConst.THEME_STYLE_KEY, "androidhwext")) != 0 && (ent = appTransition.getCachedAnimations("androidhwext", hwAnimResId)) != null) {
                Context context3 = ent.context;
                anim2 = ent.array.getResourceId(animAttr, 0);
            }
            if (anim2 == 0) {
                int hwAnimResId3 = this.mHwextContext.getResources().getIdentifier("HwAnimation", AodThemeConst.THEME_STYLE_KEY, "androidhwext");
                if (hwAnimResId3 != 0) {
                    AttributeCache.Entry ent2 = appTransition.getCachedAnimations("androidhwext", hwAnimResId3);
                    if (ent2 != null) {
                        Context context4 = ent2.context;
                        anim = ent2.array.getResourceId(animAttr, 0);
                        ent = ent2;
                    } else {
                        anim = anim2;
                        ent = ent2;
    ...
    static Animation loadScreenRotationAnim(int originRotation, int curRotation, boolean isEnter, Rect screenRect) {
        float f;
        float f2;
        ...
        float pivotX = 0.5f;
        float pivotY = 0.5f;
        int delta = DisplayContent.deltaRotation(curRotation, originRotation);
        boolean isFromLand = originRotation == 0 || originRotation == 2;
        float f3 = -90.0f;
        float f4 = 90.0f;
        if (delta == 1) {
            if (!isEnter) {
            ...
            pivotX = 0.5f;
            if (isFromLand) {
                f = (((float) screenRect.height()) / 2.0f) / ((float) screenRect.width());
            } else {
                f = (((float) screenRect.width()) / 2.0f) / ((float) screenRect.height());
            }
            pivotY = f;
        } else if (delta == 2) {
            fromDegree = isEnter ? 180.0f : 0.0f;
            toDegree = isEnter ? 0.0f : -180.0f;
            pivotX = 0.5f;
            pivotY = 0.5f;
        } else if (delta == 3) {
            if (!isEnter) {
            ...
            toDegree = f4;
            if (isFromLand) {
                f2 = (((float) screenRect.width()) / 2.0f) / ((float) screenRect.height());
            } else {
                f2 = (((float) screenRect.height()) / 2.0f) / ((float) screenRect.width());
            }
            pivotX = f2;
            pivotY = 0.5f;
        }
        ...
        if (isEnter) {
            if (delta == 1 || delta == 3) {
                animSet.addAnimation(getScreenRotationScaleAnim(originRotation, screenRect.width(), screenRect.height()));
            }
            animSet.addAnimation(getScreenRotationAnim(fromDegree, toDegree, pivotX, pivotY));
        } else {
            animSet.addAnimation(getScreenRotationAnim(fromDegree, toDegree, pivotX, pivotY));
            animSet.addAnimation(getScreenRotationClipAnim(originRotation, curRotation, screenRect.width(), screenRect.height()));
            animSet.addAnimation(getScreenRotationAlphaAnim(1.0f, 0.0f));
        }
        return animSet;
    }

    private static Animation getScreenRotationScaleAnim(int originRotation, int screenWidth, int screenHeight) {
        float fromY;
        float fromX;
        if (originRotation == 0 || originRotation == 2) {
            fromX = 1.0f;
            fromY = ((float) screenHeight) / (((float) screenWidth) * 1.0f);
        } else {
            fromY = 1.0f;
            fromX = ((float) screenHeight) / (((float) screenWidth) * 1.0f);
        }
        ScaleAnimation scaleAnimation = new ScaleAnimation(fromX, 1.0f, fromY, 1.0f, 0.0f, 0.0f);
        scaleAnimation.setDuration(350);
        scaleAnimation.setInterpolator(CUBIC_BEZIER_INTERPOLATOR_20_80);
    ...
    private static Animation getScreenRotationAnim(float fromDegree, float toDegree, float pivotX, float pivotY) {
        RotateAnimation rotateAnimation = new RotateAnimation(fromDegree, toDegree, 1, pivotX, 1, pivotY);
        rotateAnimation.setDuration(350);
        rotateAnimation.setInterpolator(CUBIC_BEZIER_INTERPOLATOR_20_80);
    ...
    private static Animation getScreenRotationAlphaAnim(float startAlpha, float endAlpha) {
        AlphaAnimation alphaAnimation = new AlphaAnimation(startAlpha, endAlpha);
        alphaAnimation.setDuration(SCREEN_ROTATION_ALPHA_DURATION);
        alphaAnimation.setInterpolator(SHARP_CURVE_INTERPOLATOR);
    ...
    private static Animation getScreenRotationClipAnim(int originRotation, int curRotation, int screenWidth, int screenHeight) {
        IHwExtDisplaySizeUtil displaySizeUtil;
        int left = 0;
        int top = 0;
        int right = screenWidth;
        int bottom = screenHeight;
        if (PhoneWindow.IS_SIDE_PROP && (displaySizeUtil = HwFrameworkFactory.getHwExtDisplaySizeUtil()) != null) {
            Rect rect = displaySizeUtil.getDisplaySideSafeInsets();
            if (originRotation == 0 || originRotation == 3) {
                right -= rect.right;
            } else {
                left = rect.left;
            }
        }
        Rect fromRect = new Rect(left, 0, right, bottom);
        if ((originRotation == 0 && curRotation == 1) || (originRotation == 1 && curRotation == 0)) {
            bottom = screenWidth;
        }
        if ((originRotation == 2 && curRotation == 3) || (originRotation == 3 && curRotation == 2)) {
            top = screenWidth;
        }
        ClipRectAnimation clipRectAnimation = new ClipRectAnimation(fromRect, new Rect(left, top, right, bottom));
        clipRectAnimation.setDuration(200);
        clipRectAnimation.setInterpolator(CUBIC_BEZIER_INTERPOLATOR_20_80);
    ...
    private static Animation createScaleDownAnimationForSuperLite(Rect winAnimFrame, float iconLeft, float iconRight, float iconTop, float iconBottom) {
        ...
        scale.setDuration(SUPER_LITE_SCALE_DURATION);
        Animation alpha = new AlphaAnimation(1.0f, 0.0f);
        alpha.setStartOffset(100);
        alpha.setInterpolator(SHARP_CURVE_INTERPOLATOR);
        alpha.setDuration(SCREEN_ROTATION_ALPHA_DURATION);
    ...
    /* JADX INFO: Multiple debug info for r7v3 float[]: [D('alphaAnim' android.view.animation.Animation), D('scaleInValues' float[])] */
    /* JADX INFO: Multiple debug info for r9v1 float[]: [D('winWidth' int), D('scaleOutValuesX' float[])] */
    static Animation createAppExitToIconAnimation(AppWindowToken atoken, int containingHeight, int iconWidth, int iconHeight, float originPivotX, float originPivotY, Bitmap icon, int exitFlag, int lazyMode) {
        ...
        if (winWidth > 0) {
            if (winHeight > 0) {
                ...
                float pivotY = ((((float) winAnimFrame.bottom) * iconTop) - (((float) winAnimFrame.top) * iconBottom)) / (((((float) winAnimFrame.bottom) - iconBottom) + iconTop) - ((float) winAnimFrame.top));
                ...
                if (HwMwUtils.ENABLED && window.inHwMagicWindowingMode()) {
                    ...
                    pivotY2 = bundle.getFloat("BUNDLE_EXITANIM_PIVOTY", 0.0f);
                }
                boolean isHorizontal = winWidth > winHeight;
                float[] scaleInValues = {0.0f, 0.16f, 1.0f};
                PhaseInterpolator interpolatorX = new PhaseInterpolator(scaleInValues, new float[]{1.0f, 1.0f - ((((float) (winWidth - iconWidth)) * (isHorizontal ? MID_SCALE_X_RATIO_HORIZANTAL : 0.45999998f)) / ((float) winWidth)), toX}, isHorizontal ? SIZE_BIG_INTERPOLATORS : SIZE_SMALL_INTERPOLATORS);
                ScaleAnimation scaleXAnim = new ScaleAnimation(0.0f, 1.0f, 1.0f, 1.0f, pivotX, pivotY2);
                ...
                PhaseInterpolator interpolatorY = new PhaseInterpolator(scaleInValues, new float[]{1.0f, 1.0f - ((((float) (winHeight - iconHeight)) * (isHorizontal ? 0.45999998f : MID_SCALE_X_RATIO_HORIZANTAL)) / ((float) winHeight)), toY}, isHorizontal ? SIZE_SMALL_INTERPOLATORS : SIZE_BIG_INTERPOLATORS);
                ScaleAnimation scaleYAnim = new ScaleAnimation(1.0f, 1.0f, 0.0f, 1.0f, pivotX, pivotY2);
    ...
    /* JADX INFO: Multiple debug info for r8v1 long: [D('window' com.android.server.wm.WindowState), D('durationOpenAnim' long)] */
    /* JADX INFO: Multiple debug info for r14v1 long: [D('radius' float), D('durationDrawBackExcludeTrans' long)] */
    static WindowAnimationSpec setFloatSceneCornerRadiusInfo(AppWindowToken atoken, WindowAnimationSpec windowAnimationSpec, float originPivotX, float originPivotY, boolean isPositive) {
        ...
        long durationDrawBackExcludeTrans = (duration - j) - 100;
        if (duration != 0) {
            if (durationOpenAnim != 0) {
                if (isPositive) {
                    interpolator = new PhaseInterpolator(new float[]{0.0f, (((float) durationDrawBackExcludeTrans) * 1.0f) / (((float) duration) * 1.0f), 1.0f}, new float[]{0.0f, 1.0f, 1.0f}, new Interpolator[]{new FastOutSlowInInterpolator(), new LinearInterpolator()});
                } else {
                    interpolator = new PhaseInterpolator(new float[]{0.0f, (((float) j) * 1.0f) / (((float) durationOpenAnim) * 1.0f), 1.0f}, new float[]{0.0f, 0.0f, 1.0f}, new Interpolator[]{new LinearInterpolator(), new FastOutSlowInInterpolator()});
                }
                windowAnimationSpec.setDynamicCornerRadiusInfo(interpolator, isPositive ? finalRadius : iconCornerRadius, isPositive ? iconCornerRadius : finalRadius, durationScale, durationOpenAnim, 0);
                windowAnimationSpec.setDynamicCornerCurveParams(new SurfaceControl.CornerCurveParams(1, 0, 3, 0.0f, 0.0f));
                return windowAnimationSpec;
            }
        }
    ...
    private static void addfloatDrawBackScaleAnimation(AnimationSet animationSet, long duration, int winWidth, int winHeight) {
        ...
        scaleXAnim.setFillBefore(true);
        scaleXAnim.setFillAfter(true);
        scaleXAnim.setDuration((duration - FLOAT_SCENCE_ANIMATION_OFFSET_TIME) - 100);
    ...
    private static void addFloatDrawBackClipAnimation(AnimationSet animationSet, long duration, int winWidth, int winHeight, float iconRadius) {
        ...
        clipRectAnimation.setFillBefore(true);
        clipRectAnimation.setFillAfter(true);
        clipRectAnimation.setDuration((duration - FLOAT_SCENCE_ANIMATION_OFFSET_TIME) - 100);

...
public class HwFreeFormAssistant {
    ...
    private final Map<String, HwFreeFormBoundsRecord> mAppBoundsRecords = new ConcurrentHashMap();
    ...
    private final Map<Integer, HwFreeFormBoundsRecord> mTaskBoundsRecords = new ConcurrentHashMap();
    ...
    private <T> void onUserRemoved(Map<T, HwFreeFormBoundsRecord> records, int userId) {
    ...
    private <T> void addRecord(Map<T, HwFreeFormBoundsRecord> recordsMap, T key, HwFreeFormBoundsRecord record) {
        recordsMap.put(key, record);
        trimBoundsRecords(recordsMap);
    }

    private <T> void trimBoundsRecords(Map<T, HwFreeFormBoundsRecord> records) {
        if (records.size() > this.mRecordsLimits) {
            Long middleRecordTime = getMiddleRecordTime(records);
            Iterator<Map.Entry<T, HwFreeFormBoundsRecord>> it = records.entrySet().iterator();
            while (it.hasNext()) {
                Map.Entry<T, HwFreeFormBoundsRecord> entry = it.next();
                if (!(entry == null || entry.getValue() == null || entry.getValue().recordTime >= middleRecordTime.longValue())) {
                    it.remove();
                }
            }
        }
    ...
    private <T> Long getMiddleRecordTime(Map<T, HwFreeFormBoundsRecord> records) {
        List<Long> addTimeList = new ArrayList<>(records.size());
        for (Map.Entry<T, HwFreeFormBoundsRecord> entry : records.entrySet()) {
            if (!(entry == null || entry.getValue() == null)) {
                addTimeList.add(Long.valueOf(entry.getValue().recordTime));
            }
        }
        Collections.sort(addTimeList);
        return addTimeList.get(addTimeList.size() / 2);
    }
    ...
    private void resetHwFreeFormBoundsRecords(Map<?, HwFreeFormBoundsRecord> records, int displayId, int displayWidth, int displayHeight) {
    ...
    public ActivityOptions updateToHwFreeFormIfNeeded(Intent intent, ActivityInfo aInfo, TaskRecord inTask, int launchFlags, ActivityRecord resultTo, ActivityOptions options) {
        if (isInScaleMode()) {
            ...
            return options;
        } else if (options != null && isNeedClearHwMultiWindowMode(intent, options)) {
    ...
    private boolean isInScaleMode() {
        if (this.mService.mWindowManager.getLazyMode() != 0) {
            return true;
        }
        if (!HwFoldScreenState.isFoldScreenDevice()) {
            return false;
        }
        if (((HwFoldScreenManagerInternal) LocalServices.getService(HwFoldScreenManagerInternal.class)).getDisplayMode() == 3) {
            return true;
        }
        return false;
    }
    ...
    private void dumpBoundsRecords(PrintWriter pw, Map<?, HwFreeFormBoundsRecord> records, String header) {
        ...
        for (Map.Entry<?, HwFreeFormBoundsRecord> recordEntry : records.entrySet()) {
            HwFreeFormBoundsRecord record = recordEntry.getValue();
            pw.println("  * Records #" + index + ": userId=" + record.userId + " package=" + record.packageName + " taskId=" + record.taskId + " bounds=" + record.bounds + " scale=" + record.stackScale + " recordTime=" + record.recordTime);
            index++;
        }
        pw.println();
    }

...
public class HwMagicModeBase {
    ...
    private int getTargetPositionInner(ActivityRecordEx focus, ActivityRecordEx next, HwMagicContainer container) {
        ...
        if (!this.mPolicy.isSpecTransActivity(container, focus) && isEnterDoubleWindow && !isLaunchFromMain(next)) {
            return 2;
        }
        return 3;
    }

    public boolean isLaunchFromMain(ActivityRecordEx next) {
        HwMagicContainer container = this.mMwManager.getContainer(next);
        return (container == null || next == null || next.getTaskRecordEx() == null || !this.mMwManager.getAmsPolicy().isSupportMainRelatedMode(container, next) || !next.getTaskRecordEx().getMagicWindowExtras().getBoolean(Utils.KEY_LAUNCH_FROM_MAIN)) ? false : true;
    }
        ...
        if (container != null) {
            if (!this.mMwManager.isMaster(activity)) {
                ...
                if (slaveNext == null || isLeftTopMove) {
                    ...
                    this.mPolicy.updateStackVisibility(slaveNext, true);
                    return;
                }

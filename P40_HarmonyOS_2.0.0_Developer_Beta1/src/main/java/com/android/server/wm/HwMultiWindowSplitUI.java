...
public class HwMultiWindowSplitUI {
    ...
    private HwMultiWindowSplitUI(Context context, ActivityTaskManagerService service, int displayId) {
        ...
        if (displayManager != null) {
            ...
            this.mPanelType = HwPowerManager.getDisplayPanelType();
            return;
        }
        ...
        if (bundle != null) {
            ...
            if (packageName != null && userIds != null) {
                ...
                if (isMagic) {
                    ...
                    if (appTypes != null && appTypes.size() >= 2 && packageName.size() >= 2 && userIds.size() >= 2) {
    ...
    private void initDragView(View backgroundLayout) {
        this.mDragView = backgroundLayout.findViewById(34603281);
        if (isPadOrFoldAmoled() && !HwMultiWinUtils.isInNightMode(this.mContext)) {
            this.mDragView.setBackgroundColor(Color.parseColor("#202020"));
        }
        this.mDragLine = (ImageView) backgroundLayout.findViewById(34603278);
        this.mDragPoint = (ImageView) backgroundLayout.findViewById(34603280);
        this.mDragRegion = (ImageView) backgroundLayout.findViewById(34603275);
        this.mDragFullZone = (RelativeLayout) backgroundLayout.findViewById(34603277);
        this.mFarLine = (ImageView) backgroundLayout.findViewById(34603276);
        this.mNearLine = (ImageView) backgroundLayout.findViewById(34603279);
                ...
                if (this.mDown) {
                    ...
                    if (!this.mIsSetBackground && this.mLeftCover.getBackground() != null && (layoutParams = this.mCoverLp) != null && layoutParams.width > this.mWindowWH && this.mCoverLp.height > this.mWindowWH) {
                        if (!isPadOrFoldAmoled() || HwMultiWinUtils.isInNightMode(this.mContext)) {
                            ...
                            this.mCoverLayout.setBackgroundColor(Color.parseColor("#202020"));
                        }
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private boolean isPadOrFoldAmoled() {
        if ((isRealTablet(this.mDisplayId) || HwFoldScreenState.isFoldScreenDevice()) && this.mPanelType == 1) {
            return true;
        }
        return false;
    }
    ...
    private void setCoverBackground() {
        this.mLeftCover.post(new Runnable() {
            ...
            @Override // java.lang.Runnable
            public void run() {
                HwMultiWindowSplitUI.this.mLeftCover.setBackgroundResource(33752034);
                HwMultiWindowSplitUI.this.mRightCover.setBackgroundResource(33752034);
            }
        });
    }
            ...
            if (!this.mIsDragFullZoneDragable || !isInDragFullMode()) {
                ...
                if (activityStack != null) {
                    ...
                    if (activityStack.inHwMagicWindowingMode()) {
                        ...
                        if (!HwFoldScreenManager.isFoldable() || (!isInDragFullMode() && !this.mHwMultiWindowManager.isDragFullModeByType(type))) {
                            z = false;
                        }
    ...
    private void resetRotationLocked() {
        if (this.mDisplayId == 0) {
            int i = this.userRotationLocked;
            if (!(i == -1 || i == 0)) {
                RotationPolicy.setRotationLock(this.mContext, i == 0);
            }
    ...
    private void addSplitBarIfNeeded() {
        ...
        if (i == -1 || i == this.mWindowMode || this.mWindowMode != 103) {
            this.mOldWindowMode = -1;
            this.mWindowMode = -1;
            return;
        }
        Slog.i(TAG, "add split bar after remove if needed. mOldWindowMode:" + this.mOldWindowMode + " mWindowMode:" + this.mWindowMode);
        this.mIsAddedSplitBar = true;
        createSplitCover();
        this.mOldWindowMode = -1;
    }
    ...
    private void loadDimens() {
        ...
        if (bundle != null) {
            ...
            if (bundle.getInt(HwMultiWindowManager.HW_SPLIT_SCREEN_PRIMARY_POSITION) == 1) {
                ...
                this.mCoverLayout = (FrameLayout) LayoutInflater.from(this.mContext).inflate(34013364, (ViewGroup) null);
            } else {
                Slog.i(TAG, "up and down split screen");
                this.mIsLeftAndRightPos = false;
                this.mCoverLayout = (FrameLayout) LayoutInflater.from(this.mContext).inflate(34013365, (ViewGroup) null);
            }
            this.mLeftCover = (RelativeLayout) this.mCoverLayout.findViewById(34603292);
            this.mRightCover = (RelativeLayout) this.mCoverLayout.findViewById(34603310);
            this.mLeftCover.setVisibility(8);
            this.mRightCover.setVisibility(8);
    ...
    private void loadDimensFormRes() {
        this.mWindowWH = this.mContext.getResources().getDimensionPixelSize(34472629);
        this.mDividerBarHeight = this.mContext.getResources().getDimensionPixelSize(34472627);
        this.mFarLineMargin = this.mContext.getResources().getDimensionPixelSize(34472634);
        this.mNearLineMargin = this.mContext.getResources().getDimensionPixelSize(34472635);
        this.mDragFullZoneWidth = this.mContext.getResources().getDimensionPixelSize(34472631);
        this.mDragFullZoneHeight = this.mContext.getResources().getDimensionPixelSize(34472630);
        this.mDividerBarRadio = this.mContext.getResources().getDimensionPixelSize(34472610);
        this.mHotRegionDefaultWidth = this.mContext.getResources().getDimensionPixelSize(34472611);
        this.mWindowOffset = this.mContext.getResources().getDimensionPixelSize(34472628);
        this.mDragLineColor = this.mContext.getResources().getColor(33883078);
        this.mDragFullLineColor = this.mContext.getResources().getColor(33883077);
        sNavBarWidth = this.mContext.getResources().getDimensionPixelSize(17105314);
        sNavBarHeight = this.mContext.getResources().getDimensionPixelSize(17105309);
    ...
    private View getBoarderRect() {
        float f;
        View rectFrameLayout = LayoutInflater.from(this.mContext).inflate(34013362, (ViewGroup) null);
        View rectFrameView = rectFrameLayout.findViewById(34603300);
        ...
        if (HwActivityManager.IS_PHONE) {
            ...
            f = this.mContext.getResources().getDimension(34472617);
        }
    ...
    private void setSplitBarDarkDelay() {
        this.mHandler.removeMessages(8);
        this.mHandler.sendMessageDelayed(this.mHandler.obtainMessage(8), HwSwingMotionGestureConstant.HOVER_SCREEN_OFF_THRESHOLD);
    }
        ...
        dragAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                if (!HwMultiWindowSplitUI.this.mIsSetBackground) {
                    if (!HwMultiWindowSplitUI.this.isPadOrFoldAmoled() || HwMultiWinUtils.isInNightMode(HwMultiWindowSplitUI.this.mContext)) {
                        HwMultiWindowSplitUI.this.mCoverLayout.setBackgroundColor(-16777216);
                    } else {
                        HwMultiWindowSplitUI.this.mCoverLayout.setBackgroundColor(Color.parseColor("#202020"));
                    }

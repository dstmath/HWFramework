...
public class HwRogEx implements IHwRogEx {
    ...
    private void initRogMode(WindowManagerService wm, Context context) {
        ...
        if (wm != null) {
            if (SystemProperties.get("ro.runmode", "normal").equals("factory")) {
                str2 = "persist.sys.rog.height";
                str = "persist.sys.realdpi";
            } else if (SystemProperties.getInt("sys.bopd", 0) == 1) {
                str2 = "persist.sys.rog.height";
                str = "persist.sys.realdpi";
            } else if (SystemProperties.getInt("persist.sys.aps.firstboot", 1) > 0 || SystemProperties.getInt("persist.sys.rog.width", 0) == 0 || isDemoVersionForSpecialPhone(context)) {
                ...
                if (initWidth == 0 || initHeight == 0 || initDensity == 0) {
                    ...
                    str2 = "persist.sys.rog.height";
                    Settings.Global.putString(contentResolver, FingerprintSupportEx.DISPLAY_SIZE_FORCED, initWidth + "," + initHeight);
                    str = "persist.sys.realdpi";
            ...
            if (isDemoVersionForSpecialPhone(context)) {
                Settings.Global.putInt(context.getContentResolver(), "aps_display_resolution", 3);
                Settings.Global.putInt(context.getContentResolver(), "low_resolution_switch", 0);
            }
            ...
            if ("TAH".equals(productName) && isFoldableScreen) {
                ...
                if (rogWidth != 0 && rogWidth != fullRect.height()) {
                    ...
                    Settings.Global.putString(contentResolver2, FingerprintSupportEx.DISPLAY_SIZE_FORCED, targetWidth + "," + targetHeight);
                    ...
                    SystemProperties.set(str, Integer.toString(targetDensity));
                    SystemProperties.set("persist.sys.rog.width", Integer.toString(targetHeight));
                    SystemProperties.set(str2, Integer.toString(targetWidth));
                }
            }
        }
    }
    ...
    private boolean isDemoVersionForSpecialPhone(Context context) {
        String productName = SystemProperties.get("ro.build.product", "");
        return ("NOH".equals(productName) || "NOP".equals(productName)) && (SystemProperties.get("ro.hw.vendor", "").contains("demo") || SystemProperties.get("ro.hw.country", "").contains("demo")) && (SystemProperties.getInt("persist.sys.rog.demoChanged", 0) == 0);
    }
}

...
public final class HwTaskPositionerEx extends HwTaskPositionerBridgeEx {
    ...
    public void updateFreeFormOutLine(final int color) {
        if (HwFreeFormUtils.isFreeFormEnable()) {
            this.mService.postEx(new Runnable() {
                /* class com.android.server.wm.HwTaskPositionerEx.AnonymousClass1 */

                @Override // java.lang.Runnable
                public void run() {
                    HwActivityTaskManager.updateFreeFormOutLine(color);
                }
            });
        }
    }

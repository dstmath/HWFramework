...
public final class HwWindowStateEx extends WindowStateBridgeEx {
    ...
    public boolean updateMagicWindowDimmer() {
        ...
        if ((this.mWinState.getAttrs().flags & 2) != 0 && this.mWinState.isVisibleNow() && !this.mWinState.isHidden()) {
            ...
            dimmer.dimBelow(windowStateEx, windowStateEx.getAttrs().dimAmount);
        }
        ...
        if (dimmer.updateDims(this.mWinState, this.mDimBoundsRect)) {
    ...
    public void stopMagicWindowDimmer() {
        ...
        if (HwMwUtils.ENABLED && (windowStateEx = this.mWinState) != null && windowStateEx.getMwDimmer() != null) {
            this.mWinState.getMwDimmer().stopDim(this.mWinState);
        }
    }
    ...
    public void initializeHwAnim(Animation anim, int appWidth, int appHeight, int frameWidth) {
        if (anim == null) {
            SlogEx.e(TAG, "Fail to initialize animation because the parameter is null");
        } else {
    ...
    public boolean isPopUpIme(int inputMethodWindowHeight, boolean isImeWithHwFlag, WindowManager.LayoutParams attrs) {
        if (attrs == null) {
            SlogEx.e(TAG, "Fail to get the type of the pop up because the parameter is null");
            return false;
        } else if ((!isImeWithHwFlag || (WindowManagerEx.LayoutParamsEx.getHwFlags(attrs) & 1048576) == 0) && (isImeWithHwFlag || inputMethodWindowHeight > INPUT_METHOD_WINDOW_THRESHOLD_HEIGHT)) {
            return false;
        } else {
            return true;
        }

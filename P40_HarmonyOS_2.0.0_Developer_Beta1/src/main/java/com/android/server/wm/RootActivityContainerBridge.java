...
public class RootActivityContainerBridge implements IHwRootActivityContainerEx {
    ...
    public boolean resumeAppLockActivityIfNeeded(ActivityStack stack, ActivityOptions targetOptions, boolean ignoreResumed) {
        ActivityStackEx activityStackEx = null;
        if (stack != null) {
            activityStackEx = new ActivityStackEx();
            activityStackEx.setActivityStack(stack);
        }
        return this.mRootActivityContainerBridgeEx.resumeAppLockActivityIfNeeded(activityStackEx, targetOptions, ignoreResumed);
    }

    public void removeStartingAppLockTaskId(int stackId) {
        this.mRootActivityContainerBridgeEx.removeStartingAppLockTaskId(stackId);
    }

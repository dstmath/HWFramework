...
public class WindowProcessController extends ConfigurationContainer<ConfigurationContainer> implements ConfigurationContainerListener {
    ...
    public boolean areBackgroundActivityStartsAllowed(boolean shouldAbortSelfLaunchWhenReturnHome) {
        if (this.mAllowBackgroundActivityStarts) {
            Slog.i("ActivityTaskManager", "Background activity start: allow if the whitelisting flag was explicitly set");
            return true;
        }
        long now = SystemClock.uptimeMillis();
        if ((now - this.mLastActivityLaunchTime < 10000 || now - this.mLastActivityFinishTime < 10000) && ((this.mLastActivityLaunchTime > this.mAtm.getLastStopAppSwitchesTime() || this.mLastActivityFinishTime > this.mAtm.getLastStopAppSwitchesTime()) && !shouldAbortSelfLaunchWhenReturnHome)) {
            Slog.i("ActivityTaskManager", "Background activity start: allow if any activity in the caller has either started or finished very recently");
            return true;
        } else if (this.mInstrumentingWithBackgroundActivityStartPrivileges) {
            Slog.i("ActivityTaskManager", "Background activity start: allow if the proc is instrumenting with background activity starts privs");
            return true;
        } else if (hasActivityInVisibleTask() && !shouldAbortSelfLaunchWhenReturnHome) {
            Slog.i("ActivityTaskManager", "Background activity start: allow if the caller has an activity in any foreground task");
            return true;
        } else if (!isBoundByForegroundUid() || shouldAbortSelfLaunchWhenReturnHome) {
            return false;
        } else {
            Slog.i("ActivityTaskManager", "Background activity start: allow if the caller is bound by a UID that's currently foreground");
            return true;
        }
    }

...
public class WindowState extends WindowContainer<WindowState> implements WindowManagerPolicy.WindowState {
    ...
    static final int HW_FREEFORM_CAPTION_BAR_HEIGHT_IN_DP_FOR_PAD = 45;
    ...
    /* JADX WARNING: Removed duplicated region for block: B:107:0x0336  */
    /* JADX WARNING: Removed duplicated region for block: B:138:0x0402  */
    /* JADX WARNING: Removed duplicated region for block: B:139:0x0449  */
    /* JADX WARNING: Removed duplicated region for block: B:166:0x04fd  */
    /* JADX WARNING: Removed duplicated region for block: B:170:0x0534  */
    /* JADX WARNING: Removed duplicated region for block: B:196:0x069f  */
    /* JADX WARNING: Removed duplicated region for block: B:199:0x06c0  */
    /* JADX WARNING: Removed duplicated region for block: B:200:0x06da  */
    /* JADX WARNING: Removed duplicated region for block: B:215:0x0724  */
    /* JADX WARNING: Removed duplicated region for block: B:244:0x0831  */
    /* JADX WARNING: Removed duplicated region for block: B:247:0x0847  */
    /* JADX WARNING: Removed duplicated region for block: B:250:0x0852  */
    /* JADX WARNING: Removed duplicated region for block: B:253:0x086a  */
    /* JADX WARNING: Removed duplicated region for block: B:268:0x08b8  */
    /* JADX WARNING: Removed duplicated region for block: B:269:0x08bd  */
    public void computeFrameLw() {
        ...
        if (!this.mWillReplaceWindow || (!this.mAnimatingExit && this.mReplacingRemoveRequested)) {
            ...
            if (WindowManagerDebugConfig.DEBUG_FOCUS) {
                Slog.v(TAG, "start compute frame " + this + " isFullscreenAndFillsDisplay:" + isFullscreenAndFillsDisplay + " windowsAreFloating:" + windowsAreFloating + " isLayoutInParentFrame:" + isLayoutInParentFrame + " parentFrame:" + this.mWindowFrames.mParentFrame + " isImeTarget:" + isImeTarget + " mContainingFrame:" + this.mWindowFrames.mContainingFrame);
            }
            if (isFullscreenAndFillsDisplay || isLayoutInParentFrame) {
                str2 = TAG;
                str = " mContainingFrame:";
                ...
                if (HwActivityTaskManager.isPCMultiCastMode()) {
                    if (!this.mAttrs.mOverrideDisplayFrame.isEmpty()) {
                        layoutContainingFrame.set(this.mAttrs.mOverrideDisplayFrame);
                        layoutDisplayFrame.set(this.mAttrs.mOverrideDisplayFrame);
                    } else if (getAttrs().type >= 1000 && getAttrs().type <= 1999) {
                        WindowContainer wc = getParent();
                        if (wc != null && (wc instanceof WindowState)) {
                            WindowState holder = this.mWmService.mImeHolderOfPcFreeForm;
                            if (((WindowState) wc).mAttrs.type == 2011 && holder != null) {
                                if (holder.getWindowingMode() == 105) {
                                    WindowState statusBar = this.mWmService.getDefaultDisplayContentLocked().getDisplayPolicy().getStatusBar();
                                    Rect adjust = holder.getDisplayFrameLw();
                                    if (statusBar != null) {
                                        adjust.top += statusBar.getContentFrameLw().bottom;
                                    }
                                    layoutContainingFrame.set(adjust);
                                    layoutDisplayFrame.set(adjust);
                                }
                            }
                        }
                    }
                ...
                if (!WindowManagerDebugConfig.DEBUG_FOCUS) {
                    str3 = str2;
                    Slog.v(str3, "second compute frame mFrame:" + this.mWindowFrames.mFrame + str + this.mWindowFrames.mContainingFrame + " mDisplayFrame:" + this.mWindowFrames.mDisplayFrame + " containingFrame:" + layoutContainingFrame + " displayFrame:" + layoutDisplayFrame);
                } else {
                    str3 = str2;
                }
                ...
                if (WindowManagerDebugConfig.DEBUG_FOCUS) {
                    Slog.v(str3, "sixth compute frame mContentFrame:" + this.mWindowFrames.mContentFrame);
                }
                if (this.mAttrs.type != 2034) {
                ...
                if (!WindowManagerDebugConfig.DEBUG_FOCUS) {
                    Slog.v(str3, "complete compute frame " + this + "(mRequestedWidth=" + this.mRequestedWidth + ", mRequestedheight=" + this.mRequestedHeight + ") to (pw=" + pw + ", ph=" + ph2 + "): frame=" + this.mWindowFrames.mFrame.toShortString() + " " + this.mWindowFrames.getInsetsInfo() + " " + ((Object) this.mAttrs.getTitle()));
                    return;
                }
                return;
            }
    ...
    private void recalculateFrameOnSplitSceen(int height) {
        WindowState imeWin = this.mWmService.mRoot.getCurrentInputMethodWindow();
        if (imeWin != null && imeWin.isVisibleNow() && this.mWindowFrames.mFrame.height() < 0 && getInputMethodTop(imeWin) < this.mWindowFrames.mFrame.top) {
            this.mWindowFrames.mFrame.top = this.mWindowFrames.mFrame.bottom - height;
        }
    }
    ...
    private void applyGravityAndUpdateFrame(Rect containingFrame, Rect displayFrame) {
        ...
        int originalHeight = this.mWindowFrames.mFrame.height();
        ...
        if (inSplitScreenSecondaryWindowingMode() || inHwSplitScreenSecondaryWindowingMode()) {
            recalculateFrameOnSplitSceen(originalHeight);
        }
        if (WindowManagerDebugConfig.DEBUG_FOCUS) {
    ...
    public boolean performShowLocked() {
        ...
        if (this.mWinAnimator.mDrawState != 3 || !isReadyForDisplay()) {
            if (this.mWinAnimator.mDrawState == 3) {
                ...
                if (windowToken == null || !windowToken.isHidden()) {
                    z = false;
                }
                sb.append(z);
    ...
    public WindowState getWindowByWindowMode(int windowMode) {
        TaskStack taskStack;
        ActivityStack activityStack;
        ActivityRecord activityRecord;
        AppWindowToken appWindowToken;
        DisplayContent displayContent = getDisplayContent();
        if (displayContent == null || (taskStack = displayContent.getStack(windowMode, 1)) == null || (activityStack = taskStack.mActivityStack) == null || (activityRecord = activityStack.topRunningActivityLocked()) == null || (appWindowToken = activityRecord.mAppWindowToken) == null) {
            return null;
        }

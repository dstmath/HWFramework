...
public class ShortcutPlugin implements ModuleGroup.ModuleGroupHandler, ErrorCode {
    ...
    public void onFunctionCall(Function function, Result result) {
        if (function != null && function.name != null) {
            ...
            if ("create".equals(function.name)) {
                ...
                shortcutInstallParam.setId(getStringArg(function, 8));
                ...
                int hasInstalled = hasInstalled(getStringArg(function, 0), getStringArg(function, 1), getStringArg(function, 2));
                if (hasInstalled == 2) {
    ...
    private boolean createShortcut(ShortcutInstallParam shortcutInstallParam) {
        ...
        if (shortcutInstallParam.getId() == null) {
            str = composeShortcutId(fullClassName, shortcutInstallParam.getPage());
        } else {
            str = shortcutInstallParam.getId();
        }
        shortcutInfo.setId(str);
        ...
        try {
            ...
            if (shortcutInstallParam.isUpdateIfExists() && hasInstalled(null, null, shortcutInfo.getId()) == 0) {
    ...
    private Resource createResourceFromRawFile(ResourceManager resourceManager, String str) {
        ...
        if (rawFileEntry == null || rawFileEntry.getType() != Entry.Type.FILE) {
            LogUtil.info(LOG_TAG, "Cannot find icon file " + str);
            return null;
        }
        try {
    ...
    private int hasInstalled(String str, String str2, String str3) {
        Context context = this.applicationContext;
        if (context == null) {
            return 2;
        }
        ...
        if (str3 == null) {
            ...
            str3 = composeShortcutId(getFullClassName(bundleName, str), str2);
        }
        return bundleManager.isShortcutExist(str3, 0);
    }
    ...
    private String composeShortcutId(String str, String str2) {
        ...
        return str + DMSDPConfig.SPLIT + str2;
    }

...
public class PlayerClient implements IPlayerManager, IEventListener {
    ...
    private ServiceConnection mPlayerServiceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName name, IBinder authService) {
            ...
            try {
                PlayerClient.sLog.d(PlayerClient.TAG, "set kit listener");
                if (PlayerClient.this.mKitEventListener == null) {
                    PlayerClient.this.mKitEventListener = new KitServiceStatusListener();
                }
                PlayerClient.this.mAidlHwPlayerManager.setKitServiceStatusListener(new KitServiceStatusAgent(PlayerClient.this.mKitEventListener));
            } catch (RemoteException e2) {
                PlayerClient.sLog.e(PlayerClient.TAG, "onServiceConnected throw RemoteException");
            }
            PlayerClient.this.linkToDeath(authService);
        }
    };

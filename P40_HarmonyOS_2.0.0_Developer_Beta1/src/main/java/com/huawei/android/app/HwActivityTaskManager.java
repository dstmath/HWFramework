...
public class HwActivityTaskManager {
    ...
    public static final String HW_MULTIWINDOW_COMPRESS = "is_compress";
    ...
    public static final String HW_MULTIWINDOW_STACKSCALE = "stackScale";
    ...
    private static Bundle hookStartActivityOptionsInPadCastMode(Context context, Bundle options) {
        if (!SystemProperties.getBoolean(PAD_CAST_MODE_KEY, false)) {
            return options;
        }
    ...
    public static void hwResizeTaskForMultiDisplay(int taskId, Rect bounds) {
        try {
            ...
            Log.e(TAG, "hwResizeTaskForMultiDisplay failed");
        }
    }
    ...
    public static void adjustOverlayWindowWhileActivityConfigChanged(Configuration newConfig, Configuration oldConfig) {
        if (newConfig != null && oldConfig != null) {
            if ((newConfig.windowConfiguration.inHwPCFreeFormWindowingMode() || oldConfig.windowConfiguration.inHwPCFreeFormWindowingMode()) && newConfig.windowConfiguration.getWindowingMode() != oldConfig.windowConfiguration.getWindowingMode() && isPCMultiCastMode()) {
                Slog.v(TAG, "adjustOverlayWindowWhileActivityConfigChanged newConfig " + newConfig + " oldConfig " + oldConfig);
                WindowManagerGlobal.getInstance().adjustOverlayWindowWhileActivityConfigChanged(newConfig);
            }
        }
    }

    public static boolean adjustParamsForPCCast(Configuration newConfig, WindowManager.LayoutParams params, boolean isModeChange) {
        if (params == null || newConfig == null || params.type < 1000 || params.type > 2999) {
            return false;
        }
        if ((!newConfig.windowConfiguration.inHwPCFreeFormWindowingMode() && !isModeChange) || newConfig.equals(Configuration.EMPTY)) {
            return false;
        }
        params.mOverrideDisplayFrame.set(newConfig.windowConfiguration.getAppBounds());
        return true;
    }
    ...
    public static boolean minimizeHwFreeForm(IBinder token, String packageName, boolean isNonRoot) {
        try {
            return getService().minimizeHwFreeForm(token, packageName, isNonRoot);
        } catch (RemoteException e) {
    ...
    public static boolean isAdjustConfig(Configuration config) {
        ...
        if (!(config == null || config.windowConfiguration.getAppBounds() == null)) {
            if ((config.windowConfiguration.inHwFreeFormWindowingMode() && !config.windowConfiguration.inHwTvMultiWindowingMode()) || config.windowConfiguration.inHwMagicWindowingMode()) {
                return true;
            }
    ...
    public static boolean isAdjustPCConfig(Configuration config) {
        if (IS_HW_MULTIWINDOW_APPCOMPACT_SUPPORTED && config != null && config.windowConfiguration.getAppBounds() != null && config.windowConfiguration.inHwPCFreeFormWindowingMode()) {
            return true;
        }
        return false;
    }
    ...
    public static void takeTaskSnapshot(IBinder binder) {
        try {
            getService().takeTaskSnapshot(binder);
        } catch (RemoteException e) {
            Log.e(TAG, "takeTaskSnapshot RemoteException");
        }
    }

    public static void setTaskWindowingMode(IBinder token, int taskId, int windowingMode, Rect bounds, float scale) {
        try {
            getService().setTaskWindowingMode(token, taskId, windowingMode, bounds, scale);
        } catch (RemoteException e) {
            Log.e(TAG, "setTaskWindowingMode failed: " + e.getMessage());
        }
    }

    public static boolean resizeStack(IBinder token, int taskId, Rect bounds, float scale) {
        try {
            return getService().resizeStack(token, taskId, bounds, scale);
        } catch (RemoteException e) {
            Log.e(TAG, "resizeStack failed: " + e.getMessage());
            return false;
        }
    }
    ...
    public static void setTaskCombinedWindowingMode(int taskId1, int windowingMode1, Rect bounds1, float scale1, int taskId2, int windowingMode2, Rect bounds2, float scale2) {
        try {
            getService().setTaskCombinedWindowingMode(taskId1, windowingMode1, bounds1, scale1, taskId2, windowingMode2, bounds2, scale2);
        } catch (RemoteException e) {
            Log.e(TAG, "setTaskCombinedWindowingMode failed: " + e.getMessage());
        }
    }

    public static void resizeCombinedStack(int taskId1, Rect bounds1, float scale1, int taskId2, Rect bounds2, float scale2) {
        try {
            getService().resizeCombinedStack(taskId1, bounds1, scale1, taskId2, bounds2, scale2);
        } catch (RemoteException e) {
            Log.e(TAG, "resizeCombinedStack failed: " + e.getMessage());
        }
    }

    public static void minimizeTvFreeForm(int taskId) {
        try {
            getService().minimizeTvFreeForm(taskId);
        } catch (RemoteException e) {
            Log.e(TAG, "minimizeTvFreeForm failed: " + e.getMessage());
        }
    }
    ...
    public static void startActivityTvSplit(Intent intent, int tvSplitWindowingMode, Rect startBounds, float startScale, Rect otherBounds, float otherScale) {
        try {
            getService().startActivityTvSplit(intent, tvSplitWindowingMode, startBounds, startScale, otherBounds, otherScale);
        } catch (RemoteException e) {
            Log.e(TAG, "startActivityTvSplit failed: " + e.getMessage());
        }
    }

    public static boolean isInBopdModeAndKoBackUpApp(String startingPackage) {
        try {
            return getService().isInBopdModeAndKoBackUpApp(startingPackage);
        } catch (RemoteException e) {
            Log.e(TAG, "isInBopdModeAndKoBackUpApp failed ");
            return false;
        }
    }
    ...
    public static void setFocusableStack(int taskId, boolean isSetFocusableStack) {
        try {
            getService().setFocusableStack(taskId, isSetFocusableStack);
        } catch (RemoteException e) {
            Log.e(TAG, "setFocusableStack failed: " + e.getMessage());
        }
    }

    public static boolean isPendingShowStack(IBinder token) {
        try {
            return getService().isPendingShowStack(token);
        } catch (RemoteException e) {
            Log.e(TAG, "isPendingShowStack failed: " + e.getMessage());
            return false;
        }
    }
}

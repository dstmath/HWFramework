...
public class DeviceBluetoothManager {
    ...
    private static Pattern sCheckPattern = Pattern.compile(BLUETOOTH_MAC_PATTERN);
    ...
    private ArrayList<String> getDeviceListsWithType(ComponentName admin, String checkType) {
        ...
        try {
            ...
            Log.e(TAG, "getDeviceListsWithType exception.");
            return null;
        }
    }
    ...
    private boolean checkBluetoothMacFormat(ArrayList<String> devices) {
        ...
        while (it.hasNext()) {
            String dev = it.next();
            if (dev.length() != BLUETOOTH_MAC_LENGTH || !sCheckPattern.matcher(dev).find()) {
                return false;
            }
            while (it.hasNext()) {
            }
    ...
    private ArrayList<String> filterDuplicatedDevices(ComponentName admin, String checkType, ArrayList<String> devices) {
        List<String> currentDevices = getDeviceListsWithType(admin, checkType);
        ...
        if (currentDevices == null || currentDevices.size() <= 0) {
            ...
            while (it2.hasNext()) {
                String dev2 = it2.next();
                if (!currentDevices.contains(dev2)) {
                    filteredDevices.add(dev2);
                }
            }
        }
    ...
    private ArrayList<String> checkBluetoothListConflict(String checkType, ArrayList<String> filteredDevices) {
        ...
        ArrayList<String> oppositeLists = null;
        try {
            oppositeLists = bundle.getStringArrayList("value");
        } catch (ArrayIndexOutOfBoundsException e) {
            Log.e(TAG, "checkBluetoothListConflict exception.");
        }
        if (oppositeLists == null || oppositeLists.isEmpty()) {
            return devices;
        }
    ...
    public boolean setBluetoothDataTransferDisable(ComponentName admin, boolean isDisabled) {
        Bundle bundle = new Bundle();
        bundle.putBoolean(BLUETOOTH_DISABLE_PROFILE_STRUCT_NAME, isDisabled);
    ...
    public boolean setBluetoothPairingDisable(ComponentName admin, boolean isDisabled) {
        Bundle bundle = new Bundle();
        bundle.putBoolean(BLUETOOTH_DISABLE_PROFILE_STRUCT_NAME, isDisabled);
    ...
    public boolean setBluetoothOutGoingCallDisable(ComponentName admin, boolean isDisabled) {
        Bundle bundle = new Bundle();
        bundle.putBoolean(BLUETOOTH_DISABLE_PROFILE_STRUCT_NAME, isDisabled);
    ...
    public boolean setBluetoothDisabledProfiles(ComponentName admin, ArrayList<Integer> disabledProfiles) {
        ...
        Bundle bundle = new Bundle();
        bundle.putStringArrayList(BLUETOOTH_DISABLE_PROFILE_STRUCT_NAME, profiles);
    ...
    public ArrayList<Integer> getBluetoothDisabledProfiles(ComponentName admin) {
        ...
        try {
            profiles = bundle.getStringArrayList(BLUETOOTH_DISABLE_PROFILE_STRUCT_NAME);
        } catch (ArrayIndexOutOfBoundsException e) {
            Log.e(TAG, "getBluetoothDevicesFromBlackLists exception.");
        }
        if (profiles == null) {
            return profilesList;
        }
    ...
    public boolean setDiscoverableDisabled(ComponentName admin, boolean isDisabled) {
        Bundle bundle = new Bundle();
        bundle.putBoolean(BLUETOOTH_DISCOVER_EXTRA_DATA, isDisabled);
    ...
    public boolean setLimitedDiscoverableDisabled(ComponentName admin, boolean isDisabled) {
        Bundle bundle = new Bundle();
        bundle.putBoolean(BLUETOOTH_LIMITED_EXTRA_DATA, isDisabled);

...
public class DeviceEmailManager {
    ...
    private boolean dealAccountList(Account[] accounts, ArrayList<String> lists) {
        for (Account account : accounts) {
            if (account != null && !isInRuleList(account.name, lists)) {
                return true;
            }
        }
        return false;
    ...
    private boolean isAccountDataDeletionDisabled(Context context) {
        ...
        boolean isDisabled = isAccountDeletionDisabled(null);
        ...
        if (isDisabled) {
            ...
            if (dealAccountList(emailAccounts, trustList) || dealAccountList(exchangeAccounts, trustList)) {
                return true;
            }
            return false;
        }
        ArrayList<String> blockList = getAccountsBlockListAllowingDeletion(null);
        if (dealAccountList(emailAccounts, blockList) || dealAccountList(exchangeAccounts, blockList)) {
            return true;
        }
        return false;
    }

    public boolean setAppDataClearanceDisabled(ComponentName admin, boolean isDisabled) {
        Bundle bundle = new Bundle();
        bundle.putBoolean("value", isDisabled);
    ...
    public boolean setEmailForwardingDisabled(ComponentName admin, String emailAddress, boolean isDisabled) {
        if (isAddressValid(emailAddress)) {
            ...
            bundle.putString("email_forward", String.valueOf(isDisabled));
            setAppDataClearanceDisabled(admin, true);
    ...
    private boolean wildCardMatch(String text, String pattern) {
        ...
        String pattern2 = pattern.toLowerCase(Locale.ROOT);
    ...
    public boolean isInRuleList(String emailAddress, ArrayList<String> rules) {
        ...
        while (it.hasNext()) {
            if (wildCardMatch(emailAddress, it.next())) {
                return true;
            }
        }
        return false;
    ...
    public void setAccountSync(Context context, String emailAddress, int syncType, int syncValue, int userId) {
        ...
        } else if (syncType < 0 || syncType > 1 || syncValue < 0 || syncValue > 2) {
            Log.w(TAG, "MDM6->setAccountSync->invalid param! syncType: " + syncType + " syncValue: " + syncValue);
        } else {
        ...
        if (context != null) {
            if (!TextUtils.isEmpty(emailAddress)) {
                ...
                try {
                    ...
                    if (cursor != null && cursor.moveToNext()) {
                        isAdded = Boolean.valueOf(cursor.getString(0)).booleanValue();
                    }
                    return isAdded;
                } finally {
                    if (cursor != null) {
    ...
    public boolean isForceSMIMECertificateAlias(Context context, String emailAddress, String alias, int type) {
        if (context == null || TextUtils.isEmpty(emailAddress) || TextUtils.isEmpty(alias)) {
            return false;
        }
        if (type != 1 && type != 2) {
            return false;
    ...
    public boolean setForceSMIMEMessages(ComponentName admin, String emailAddress, boolean isForced, int type) {
        return setForceSmimePolicy(admin, emailAddress, "", type, isForced ? 1 : 0);
    }
    ...
    public boolean setForceRequiredSMIMEAndCertificateAlias(ComponentName admin, String emailAddress, String alias, boolean isForced, int type) {
        return setForceSmimePolicy(admin, emailAddress, alias, type, isForced ? 1 : 0);
    }

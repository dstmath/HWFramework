...
public class DevicePhoneManager {
    ...
    private static final int IP_HEADER_LENGTH = 5;
    ...
    private Country mCountryDetector;
    ...
    public boolean setDataRoamingDisabled(ComponentName admin, boolean isDisabled) {
    ...
    public boolean setAccessPointNameDisabled(ComponentName admin, boolean isDisabled) {
        Bundle bundle = new Bundle();
        bundle.putBoolean("value", isDisabled);
    ...
    public boolean setNonEmergencyCallDisabled(ComponentName admin, boolean isDisabled) {
        Bundle bundle = new Bundle();
        bundle.putBoolean("value", isDisabled);
    ...
    public boolean setRoamingCallDisabled(ComponentName admin, boolean isDisabled) {
        Bundle bundle = new Bundle();
        bundle.putBoolean("value", isDisabled);
    ...
    public boolean setPhoneNumberShowDisabled(ComponentName admin, boolean isDisabled) {
        Bundle bundle = new Bundle();
        bundle.putBoolean("value", isDisabled);
    ...
    public boolean addMdmNumberList(ComponentName admin, ArrayList<String> numbers, int blockMode, boolean isOutgoing, boolean isBlockList) {
        boolean isBlockAllMode = blockMode == 2;
        boolean isAllowAllMode = blockMode == 3;
        boolean isBlackListType = isBlockList;
        if (isBlockAllMode) {
            isBlackListType = true;
        ...
        if (!setListTypePolicy(admin, isBlackListType, isOutgoing)) {
            Log.e(TAG, "Set policy failed, return false.");
            return false;
        } else if (isBlockAllMode || isAllowAllMode) {
            String policyName = isOutgoing ? APPLY_TO_ALL_OUTGOING_CALLS : APPLY_TO_ALL_INCOMING_CALLS;
            Bundle bundle = new Bundle();
            bundle.putBoolean("value", true);
            return this.mDpm.setPolicy(admin, policyName, bundle);
        } else {
            Bundle bundle2 = new Bundle();
            bundle2.putStringArrayList("value", numbers);
            return this.mDpm.setPolicy(admin, isOutgoing ? PHONE_NUMBER_OUTGOING_LIST : PHONE_NUMBER_INCOMING_LIST, bundle2);
        }
    }
    ...
    private boolean isAddNumbersOutOfLimit(int size, boolean isOutgoing) {
        ...
        try {
            numbers = listBundle.getStringArrayList("value");
        } catch (ArrayIndexOutOfBoundsException e) {
            Log.e(TAG, "isAddNumbersOutOfLimit exception.");
        }
    ...
    public boolean removeMdmNumberList(ComponentName admin, ArrayList<String> numbers, int blockMode, boolean isOutgoing, boolean isRemoveAll) {
        if (isRemoveAll) {
    ...
    private boolean updateListTypePolicy(ComponentName admin, boolean isOutgoing) {
        ...
        if (!this.mDpm.removePolicy(admin, listTypePolicy, null) || !this.mDpm.removePolicy(admin, listPolicy, null)) {
            return false;
        }
        return true;
    }
    ...
    public boolean isBlockNumber(ComponentName admin, String number, boolean isOutgoing) {
        ...
        try {
            ...
            Log.e(TAG, "isBlockNumber exception.");
            numbers = null;
        }
    ...
    private boolean setListBefore(boolean isOutgoing) {
        ...
        if (listBundle != null) {
            ...
            try {
                ...
                Log.e(TAG, "setListBefore exception.");
            }
            if (numbers != null && !numbers.isEmpty()) {
                return true;
            }
    ...
    private boolean isListTypeValid(boolean isBlackList, String policyName) {
        ...
        if (bundle != null) {
            isBlack = bundle.getBoolean("value", isBlackList);
        }
        return isBlack == isBlackList;
    }
    ...
    private String getCountryIso(Context context) {
        ...
        if (!(this.mCountryDetector != null || context == null || (detector = (CountryDetector) context.getSystemService("country_detector")) == null)) {
            this.mCountryDetector = detector.detectCountry();
        }
        CountryDetector detector2 = this.mCountryDetector;
        if (detector2 == null) {
            return null;
        }
    ...
    private String ipHeadBarber(String oriNumber) {
        String[] ipHeads = {"17900", "17901", "17908", "17909", "11808", "17950", "17951", "12593", "17931", "17910", "17911", "17960", "17968", "17969", "10193"};
        int numberLen = oriNumber.length();
        if (numberLen < 5) {
            return oriNumber;
        }
        String ipHead = oriNumber.substring(0, 5);
        for (String item : ipHeads) {
            if (ipHead.equals(item)) {
    ...
    public boolean setCallStatusNotificationApp(ComponentName admin, String packageName) {
        Bundle bundle = new Bundle();
        if (TextUtils.isEmpty(packageName)) {
            return this.mDpm.removePolicy(admin, CALL_STATUS_NOTIFICATION_APP, null);
        }
        bundle.putString("value", packageName);
        return this.mDpm.setPolicy(admin, CALL_STATUS_NOTIFICATION_APP, bundle);
    }

    public String getCallStatusNotificationApp() {
        Bundle bundle = this.mDpm.getPolicy(null, CALL_STATUS_NOTIFICATION_APP);
        if (bundle == null) {
            return "";
        }
    ...
    public boolean setMultiCallingDisabled(ComponentName admin, boolean isDisabled) {
        Bundle bundle = new Bundle();
        bundle.putBoolean("value", isDisabled);
        return this.mDpm.setPolicy(admin, DISALLOW_MULTI_CALLING, bundle);
    }

    public boolean isMultiCallingDisabled(ComponentName admin) {
        Bundle bundle = this.mDpm.getPolicy(admin, DISALLOW_MULTI_CALLING);
        if (bundle == null) {
            return false;
        }
        return bundle.getBoolean("value", false);

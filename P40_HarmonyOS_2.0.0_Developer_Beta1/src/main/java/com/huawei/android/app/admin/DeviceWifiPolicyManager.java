...
public class DeviceWifiPolicyManager {
    ...
    public boolean setUserProfilesDisabled(ComponentName admin, boolean isDisabled) {
        Bundle bundle = new Bundle();
        bundle.putBoolean(VALUE_DISABLE, isDisabled);
    ...
    public boolean setWifiProfile(ComponentName admin, WifiConfiguration config) {
        ...
        List<String> currentLists = getWifiProfileList(admin);
        if (currentLists != null && currentLists.contains(config.SSID)) {
            return true;
        }
        ArrayList<String> addConfigs = new ArrayList<>();
        addConfigs.add(config.SSID);
        Bundle bundle = new Bundle();
        bundle.putStringArrayList(VALUE_STRING_LIST, addConfigs);
    ...
    public ArrayList<String> getWifiProfileList(ComponentName admin) {
        ...
        try {
            ...
            Log.e(TAG, "getWifiProfileList exception.");
            return null;
        }
    }
    ...
    public boolean isWifiProfileSet(ComponentName admin, WifiConfiguration config) {
        ...
        List<String> currentList = getWifiProfileList(admin);
    ...
    public boolean removeWifiProfile(ComponentName admin, WifiConfiguration config) {
        ...
        ArrayList<String> removeLists = new ArrayList<>();
        Bundle bundle = new Bundle();
        removeLists.add(config.SSID);
        bundle.putStringArrayList(VALUE_STRING_LIST, removeLists);
        return this.mDpm.removePolicy(admin, PROFILE_ENTERPRISE_CONFIG, bundle);
    }

    public boolean setWifiAutoConnectionDisabled(ComponentName admin, boolean isDisabled) {
        Bundle bundle = new Bundle();
        bundle.putBoolean(VALUE_DISABLE, isDisabled);
    ...
    public boolean setUnsecureSoftApDisabled(ComponentName admin, boolean isDisabled) {
        Bundle bundle = new Bundle();
        bundle.putBoolean(VALUE_DISABLE, isDisabled);
    ...
    public boolean addUnavailableSsidToList(ComponentName admin, ArrayList<String> ssids) {
        ...
        List<String> lists = getUnavailableSsidList(admin);
        int len = 0;
        if (lists != null) {
            len = lists.size();
        }
    ...
    public boolean removeUnavailableSsidFromList(ComponentName admin, ArrayList<String> ssids) {
        if (ssids == null || (ssids.size() != 0 && ssids.size() <= 200)) {
            Bundle bundle = new Bundle();
            if (ssids == null) {
                ArrayList<String> lists = getUnavailableSsidList(admin);
                if (lists == null) {
                    return false;
                }
                bundle.putStringArrayList("value", lists);
    ...
    public ArrayList<String> getUnavailableSsidList(ComponentName admin) {
        ...
        try {
            ...
            Log.e(TAG, "getUnavailableSsidList exception.");
            return lists;
        }
    }
    ...
    public boolean setWifiProDisabled(ComponentName admin, boolean isDisabled) {
        Bundle bundle = new Bundle();
        bundle.putBoolean(VALUE_DISABLE, isDisabled);

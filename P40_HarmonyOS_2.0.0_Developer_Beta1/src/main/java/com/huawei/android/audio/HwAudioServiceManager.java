...
public class HwAudioServiceManager {
        ...
        if (cb != null) {
            synchronized (this.mAudioModeCallbackLock) {
                ...
                if (!hasAudioModeCallback_sync(cb)) {
                    this.mAudioModeCallbackList.add(new AudioModeCallbackInfo(cb, new ServiceEventHandlerDelegate(handler).getServiceEventHandler()));
    ...
    private class ServiceEventHandlerDelegate {
        ...
        public Handler getServiceEventHandler() {
    ...
    public boolean registerVolumeChangeCallback(IVolumeChangeDispatcher cb, String callback, String pkgName) throws IllegalArgumentException {
        if (cb != null) {
            try {
                return getService().registerVolumeChangeCallback(cb, callback, pkgName);
            } catch (RemoteException e) {
                Log.e(TAG, "registerVolumeChangeCallback failed");
                return false;
            }
        } else {
    ...
    public boolean unregisterVolumeChangeCallback(IVolumeChangeDispatcher cb, String callback, String pkgName) throws IllegalArgumentException {
        if (cb != null) {
            try {
                return getService().unregisterVolumeChangeCallback(cb, callback, pkgName);
            } catch (RemoteException e) {
                Log.e(TAG, "unregisterVolumeChangeCallback failed");
                return false;
            }
        } else {
            throw new IllegalArgumentException("Illegal null VolumeChangeCallback argument");
    ...
    public void setHistenNaturalMode(boolean on, IBinder cb) {
        try {
            getService().setHistenNaturalMode(on, cb);
        } catch (RemoteException e) {
            Log.e(TAG, "setHistenNaturalMode failed");
        }
    }

    public void setMultiAudioRecordEnable(boolean enable) {
        try {
            getService().setMultiAudioRecordEnable(enable);
        } catch (RemoteException e) {
            Log.e(TAG, "setMultiAudioRecordEnable failed");
        }
    }

    public boolean isMultiAudioRecordEnable() {
        try {
            return getService().isMultiAudioRecordEnable();
        } catch (RemoteException e) {
            Log.e(TAG, "isMultiAudioRecordEnable failed");
            return false;
        }
    }

    public void setVoiceRecordingEnable(boolean enable) {
        if (!SystemProperties.getBoolean("hw_mc.audio.voice_recording_enable", true) || !"CN".equalsIgnoreCase(SystemProperties.get("ro.product.locale.region", ""))) {
            Log.e(TAG, "do not support VoiceRecording");
        } else {
            AudioSystem.setVoiceRecordingEnable(enable, Binder.getCallingUid());
        }
    }

    public boolean isVoiceRecordingEnable() {
        try {
            return getService().isVoiceRecordingEnable();
        } catch (RemoteException e) {
            Log.e(TAG, "isVoiceRecordingEnable failed");
            return false;
        }
    }
    ...
    public boolean setVolumeByPidStream(int pid, int streamType, float volume, IBinder cb) {
        try {
            return getService().setVolumeByPidStream(pid, streamType, volume, cb);
        } catch (RemoteException e) {
            Log.e(TAG, "setVolumeByPidStream failed");
            return false;
        }
    }
}

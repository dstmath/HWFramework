...
public class HiPkgSignManager implements SignVerifyStrategy {
    ...
    public static boolean doCheckArchiveApk(Context context, String str, String str2, String str3) {
        ...
        if (TextUtils.isEmpty(packageName) || TextUtils.isEmpty(unInstalledAppHash)) {
            return false;
        }
        return str.equals(unInstalledAppHash.toUpperCase(Locale.getDefault()));
    }
    ...
    public boolean verifyFingerPrint(Context context, DynamicModuleInfo dynamicModuleInfo) {
        if (context == null || dynamicModuleInfo == null) {
            Log.e(TAG, "The context or dynamic module info is null.");
            return false;
        }
        Log.i(TAG, "Verify strategy: sign SHA256 verify.");
        return doCheckArchiveApk(context, dynamicModuleInfo.mExpectSignInfo, dynamicModuleInfo.mTempPath, dynamicModuleInfo.mModuleName);
    }
}

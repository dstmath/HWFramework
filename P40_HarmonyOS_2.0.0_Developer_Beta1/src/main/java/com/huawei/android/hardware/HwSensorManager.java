...
public class HwSensorManager implements SensorEventListener {
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if (type == 1) {
            ...
            if (Math.abs(gyroY) >= SENSOR_CORRENT_THRESHOLD || Math.abs(gyroX) >= SENSOR_CORRENT_THRESHOLD || Math.abs(gyroZ) >= SENSOR_CORRENT_THRESHOLD || (sensorEventDetector3 = this.mSensorEventDetector) == null) {

...
public class HwInputManager {
    ...
    public static int[] runSideTouchCommandByType(int type, Bundle bundle) {
        try {
            IHwInputManager service = getService();
            if (service != null) {
                return service.runSideTouchCommandByType(type, bundle);
            }
            Log.w(TAG, "input service is not ready");
            return null;
        } catch (RemoteException e) {
            Log.e(TAG, "runSideTouchCommandByType failed: catch RemoteException!");
            return null;
        }
    }

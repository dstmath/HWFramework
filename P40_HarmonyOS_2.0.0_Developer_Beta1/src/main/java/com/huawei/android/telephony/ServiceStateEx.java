...
public class ServiceStateEx {
    ...
    public static String getDataOperatorNumeric(ServiceState serviceState) {
        if (serviceState != null) {
            return serviceState.getDataOperatorNumeric();
        }
        return null;
    }
    ...
    public static boolean bitmaskHasTech(int bearerBitmask, int radioTech) {
        return ServiceState.bitmaskHasTech(bearerBitmask, radioTech);
    }

    @HwSystemApi
    public static int convertNetworkTypeBitmaskToBearerBitmask(int networkTypeBitmask) {
        return ServiceState.convertNetworkTypeBitmaskToBearerBitmask(networkTypeBitmask);
    }

    @HwSystemApi
    public static int convertBearerBitmaskToNetworkTypeBitmask(int bearerBitmask) {
        return ServiceState.convertBearerBitmaskToNetworkTypeBitmask(bearerBitmask);
    }

    @HwSystemApi
    public static int rilRadioTechnologyToNetworkType(int rat) {
        return ServiceState.rilRadioTechnologyToNetworkType(rat);
    }

    @HwSystemApi
    public static List<NetworkRegistrationInfoEx> getNetworkRegistrationInfoExListForTransportType(int transportType, ServiceState serviceState) {
        List<NetworkRegistrationInfo> list = serviceState.getNetworkRegistrationInfoListForTransportType(transportType);
        List<NetworkRegistrationInfoEx> resultList = new ArrayList<>(list.size());
        for (NetworkRegistrationInfo networkRegistrationInfo : list) {
            resultList.add(NetworkRegistrationInfoEx.getNetworkRegistrationInfoEx(networkRegistrationInfo));
        }
        return resultList;
    }
}

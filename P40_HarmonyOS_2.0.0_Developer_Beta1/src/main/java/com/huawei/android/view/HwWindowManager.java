...
public class HwWindowManager {
    ...
    private static final Singleton<IHwWindowManager> IWindowManagerSingleton = new Singleton<IHwWindowManager>() {
        /* class com.huawei.android.view.HwWindowManager.AnonymousClass2 */
    ...
    public static Map<IRotateCallback, IHwRotateObserver> rotateObserverMap = new HashMap();
    ...
    public static boolean dragStartForBasicMode(ClipData data, View.DragShadowBuilder builder, Point shadowSize) {
        Bitmap bitmap = Bitmap.createBitmap(shadowSize.x, shadowSize.y, Bitmap.Config.ARGB_8888);
        builder.onDrawShadow(new Canvas(bitmap));
        try {
            boolean dragStartForBasicMode = getService().dragStartForBasicMode(data, bitmap);
            if (bitmap != null && !bitmap.isRecycled()) {
                bitmap.recycle();
            }
            return dragStartForBasicMode;
        } catch (RemoteException e) {
            Log.e(TAG, "dragStartForBasicMode failed.");
            if (bitmap != null && !bitmap.isRecycled()) {
                bitmap.recycle();
            }
            return false;
    ...
    public static void registerRotateCallback(final IRotateCallback callback) {
        if (getService() == null || callback == null) {
            Log.e(TAG, "getService or callback is null.");
            return;
        }
        try {
            IHwRotateObserver observer = new IHwRotateObserver.Stub() {
                /* class com.huawei.android.view.HwWindowManager.AnonymousClass3 */

                @Override // android.view.IHwRotateObserver
                public void onRotate(int oldRotation, int newRotation) {
                    IRotateCallback.this.onRotate(oldRotation, newRotation);
                }
            };
            getService().registerRotateObserver(observer);
            rotateObserverMap.put(callback, observer);
        } catch (RemoteException e) {
            Log.e(TAG, "registerRotateObserver failed.");
        }
    }
    ...
    public static void unregisterRotateCallback(IRotateCallback callback) {
        ...
        try {
            if (rotateObserverMap.get(callback) != null) {
                getService().unregisterRotateObserver(rotateObserverMap.get(callback));
                rotateObserverMap.remove(callback);
            }
        } catch (RemoteException e) {
            Log.e(TAG, "registerRotateObserver failed.");
        }
    ...
    public static void notifyFocusViewLocationChange(List<Rect> focusAreas, ComponentName componentName) {
        if (getService() == null) {
            Log.e(TAG, "notifyFocusViewLocationChange getService return null.");
            return;
        }
        try {
            getService().notifyFocusViewLocationChange(focusAreas, componentName);
        } catch (RemoteException e) {
            Log.e(TAG, "notifyFocusViewLocationChange failed.");
        }
    }
    ...
    public static boolean isNeedForbidDialogAct(String packageName, ComponentName componentName) {
        try {
            return getService().isNeedForbidDialogAct(packageName, componentName);
        } catch (RemoteException e) {
            Log.e(TAG, "isNeedForbidDialogAct RemoteException");
            return false;
        }
    }

    public static boolean isTaskPositionDebugOpen() {
        try {
            return getService().isTaskPositionDebugOpen();
        } catch (RemoteException e) {
            Log.e(TAG, "isTaskPositionDebugOpen RemoteException");
            return false;
        }
    }

    public static boolean isAppControlPolicyExists() {
        try {
            return getService().isAppControlPolicyExists();
        } catch (RemoteException e) {
            Log.e(TAG, "isAppControlPolicyExists RemoteException");
            return false;
        }
    }
}

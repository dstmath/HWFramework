...
public final class HwCameraUtil {
    ...
    public static synchronized int getRequestQueueSize() {
        synchronized (HwCameraUtil.class) {
            ICameraService cameraService = CameraManagerGlobal.get().getCameraService();
            if (cameraService == null) {
                Log.e(TAG, "getCameraService is null");
            ...
            try {
                return cameraService.getRequestQueueSize();
            } catch (RemoteException e) {
                Log.e(TAG, "getRequestQueueSize RemoteException " + e.getMessage());
                return -1;
            }
        }
    }

    public static synchronized boolean isLightStrapCaseOn() {
        synchronized (HwCameraUtil.class) {
            ICameraService cameraService = CameraManagerGlobal.get().getCameraService();
            if (cameraService == null) {
                Log.e(TAG, "getCameraService is null");
            ...
            try {
                return cameraService.isLightStrapCaseOn();
            } catch (RemoteException e) {
                Log.e(TAG, "isLightStrapCaseOn RemoteException " + e.getMessage());
                return false;
            }
        }
    }

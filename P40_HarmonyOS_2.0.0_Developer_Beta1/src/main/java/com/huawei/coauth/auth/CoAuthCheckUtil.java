...
public class CoAuthCheckUtil {
    ...
    public static boolean destroyCoAuthPairGroupCheck(CoAuthGroup coAuthGroup) {
        if (coAuthGroup != null && groupIdCheck(coAuthGroup.getGroupId())) {
            return true;
        }
    ...
    public static boolean groupIdCheck(String groupId) {
        if (groupId != null && groupId.length() == CoAuthUtil.INVALID_GID.length() && !groupId.equalsIgnoreCase(CoAuthUtil.INVALID_GID)) {
            return true;
        }
        Log.e(TAG, "groupIdCheck request failed, invalid groupId");
        return false;
    }
    ...
    private static boolean coAuthGroupCheck(CoAuthContext coAuthContext) {
        if (coAuthContext.getCoAuthGroup() != null && groupIdCheck(coAuthContext.getCoAuthGroup().getGroupId())) {
            return true;
        }
    ...
    private static boolean coAuthDeviceCheck(CoAuthDevice peerDevice) {
        if (peerDevice == null) {
            ...
            return false;
        } else if (peerDevice.getPeerLinkType() != 1 || (peerDevice.getIp() != null && !peerDevice.getIp().isEmpty())) {
            return true;
        } else {

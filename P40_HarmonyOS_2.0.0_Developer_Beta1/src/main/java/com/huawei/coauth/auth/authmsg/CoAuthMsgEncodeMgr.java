...
public class CoAuthMsgEncodeMgr implements IcoAuthMsgEncodeMgr {
    ...
    @Override // com.huawei.coauth.auth.authmsg.IcoAuthMsgEncodeMgr
    public byte[] destroyCoAuthPairGroupMsg(long sessionId, byte[] groupId) {
        ...
        try {
            ...
            TlvWrapper containerHeader = new TlvWrapper();
            encodeHeader(containerHeader, CoAuthUtil.bytesToHexString(groupId).get());
    ...
    @Override // com.huawei.coauth.auth.authmsg.IcoAuthMsgEncodeMgr
    public byte[] cancelCoAuth(long sessionId, CoAuthContext coAuthContext) {
        ...
        try {
            ...
            TlvWrapper containerHeader = new TlvWrapper();
            encodeHeader(containerHeader, coAuthContext);
    ...
    private TlvWrapper encodeHeader(TlvWrapper containerHeader, CoAuthContext coAuthContext) throws UnsupportedEncodingException {
        TlvWrapper containerHeaderPayload = new TlvWrapper();
        containerHeaderPayload.appendString(CoAuthHeaderTagType.GROUP_ID.getValue(), coAuthContext.getCoAuthGroup().getGroupId());
        containerHeader.appendTlvWrapper(CoAuthOperationType.HEADER.getValue(), containerHeaderPayload);
        return containerHeader;
    }
    ...
    private TlvWrapper encodeCoAuthDevice(TlvWrapper container, CoAuthDevice coAuthDevice) throws UnsupportedEncodingException {
        ...
        if (coAuthDevice.getIp() != null && !coAuthDevice.getIp().isEmpty()) {

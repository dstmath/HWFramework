...
public class HwMinLuminanceUptoXnit {
    ...
    private String getLcdPanelName() {
        ...
        try {
            ...
            if (ret == 0) {
                return new String(name, StandardCharsets.UTF_8).trim().replace(' ', '_').replace("'", StorageManagerExt.INVALID_KEY_DESC);
            }
    ...
    private void setXccCoef(float xccCoefRed, float xccCoefGreen, float xccCoefBlue) {
        if (xccCoefRed <= 1.0f && xccCoefRed >= XCC_MIN && xccCoefGreen <= 1.0f && xccCoefGreen >= XCC_MIN && xccCoefBlue <= 1.0f && xccCoefBlue >= XCC_MIN) {

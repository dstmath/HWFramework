    ...
    protected static List<HwXmlAmPoint> parseAmPointList(XmlPullParser parser, List<HwXmlAmPoint> list) throws XmlPullParserException, IOException {
        ...
        try {
            ...
            if (pointSplited.length == 3) {
                float valueX = string2Float(pointSplited[0]);
                float valueY = string2Float(pointSplited[1]);
                float valueZ = string2Float(pointSplited[2]);
                if (result == null) {
                    result = new ArrayList<>();
                }
                result.add(new HwXmlAmPoint(valueX, valueY, valueZ));
                return result;
            }
            throw new XmlPullParserException("parseAmPointList() split failed, text=" + str);
        } catch (NumberFormatException e) {
            throw new XmlPullParserException("parseAmPointList() " + e);
        }
    }
}

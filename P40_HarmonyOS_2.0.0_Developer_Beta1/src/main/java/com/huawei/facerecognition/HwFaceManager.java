...
public class HwFaceManager extends FaceManager {
    private static final int FACE_HARDWARE_3D_DUAL_CAMERA = 3;
    ...
    @Override // com.huawei.facerecognition.FaceManager
    public FaceManager.FaceRecognitionCapability getFaceRecognitionCapability() {
        FaceManager.FaceRecognitionCapability capability = new FaceManager.FaceRecognitionCapability();
        FaceAuthenticationManager faceAuthenticationManager = this.mFaceAuthenticationManager;
        if (faceAuthenticationManager == null) {
            SlogEx.e(TAG, "face authentication manager is null.");
            return capability;
        }
        FaceAuthenticationManager.FaceRecognitionAbility ability = faceAuthenticationManager.getFaceRecognitionAbility();
        if (ability == null) {
            SlogEx.e(TAG, "ability is null.");
            return capability;
        }
        capability.isSupport = ability.isFaceRecognitionSupport;
        capability.isEnrolled = hasEnrolledTemplates();
        capability.is3D = ability.faceMode == 3 || ability.faceMode == 4 || ability.faceMode == 5;
        return capability;
    }

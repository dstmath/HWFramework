...
public class GoRotateTransition extends BaseTransition {
    ...
    public boolean transform(int part, boolean isOverScrollFirst, boolean isOverScrollLast, float scrollProgress, View child) {
        ...
        if (scrollProgress < 0.0f) {
            ...
            this.mTransformationInfo.mPivotX = AlgorithmUtil.transformPivotX(child, (float) childWidth);
        }
        BaseTransition.TransformationInfo transformationInfo = this.mTransformationInfo;
        transformationInfo.mPivotY = AlgorithmUtil.transformPivotY(child, ((float) childHeight) / 2.0f);

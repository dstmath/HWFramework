...
public class HuaweiWifiWatchdogStateMachine extends StateMachine {
    ...
    private static final int BSSID_STAT_EMPTY_DATA_POINT = 3;
    ...
    private void sendResultMsgToQM(int mQosLevel) {
        ...
        if (bssidStatistics != null) {
            strBssid = bssidStatistics.macBssid;
        }
    ...
    public class WifiProVerifyingLinkState extends State {
        ...
        private void processMessageOfCmdRssiFetch() {
            if (!HuaweiWifiWatchdogStateMachine.this.mCurrentBssid.mIsNotFirstChk) {
                HuaweiWifiWatchdogStateMachine huaweiWifiWatchdogStateMachine = HuaweiWifiWatchdogStateMachine.this;
                huaweiWifiWatchdogStateMachine.logI("VL first check bssid:" + WifiProCommonUtils.safeDisplayBssid(HuaweiWifiWatchdogStateMachine.this.mCurrentBssid.macBssid) + ",call newLinkDetected");
    ...
    public class WifiProLinkMonitoringState extends State {
        ...
        private void handleRssiFetched(Message msg) {
            if (msg.arg1 == HuaweiWifiWatchdogStateMachine.this.mRssiFetchToken) {
                if (HuaweiWifiWatchdogStateMachine.this.mCurrentBssid == null) {
                    ...
                    if (!HuaweiWifiWatchdogStateMachine.this.mCurrentBssid.mIsNotFirstChk) {
                        HuaweiWifiWatchdogStateMachine huaweiWifiWatchdogStateMachine = HuaweiWifiWatchdogStateMachine.this;
                        huaweiWifiWatchdogStateMachine.logI("WP Link Monitor State first check bssid:" + WifiProCommonUtils.safeDisplayBssid(HuaweiWifiWatchdogStateMachine.this.mCurrentBssid.macBssid) + ", call newLinkDetected");
                        HuaweiWifiWatchdogStateMachine.this.mCurrentBssid.newLinkDetected();
                        HuaweiWifiWatchdogStateMachine.this.mCurrentBssid.mIsNotFirstChk = true;
                    }
        ...
        private boolean handleTcpCondition() {
            if (isNetSpeedOk(HuaweiWifiWatchdogStateMachine.this.mProcessedTcpResult.mAvgRtt)) {
                ...
                this.mNetworkDisableCount = 0;
                return true;
            } else if (HuaweiWifiWatchdogStateMachine.this.lastPeriodTcpResultValid && !detectNetworkAvailable(HuaweiWifiWatchdogStateMachine.this.tcpTxPkts, HuaweiWifiWatchdogStateMachine.this.tcpRxPkts, HuaweiWifiWatchdogStateMachine.this.tcpRetransPkts, HuaweiWifiWatchdogStateMachine.this.mCurrentBssid.macBssid)) {
        ...
        private void handleBadCondition() {
            if (HuaweiWifiWatchdogStateMachine.this.isOTABad || HuaweiWifiWatchdogStateMachine.this.isTCPBad || HuaweiWifiWatchdogStateMachine.this.isVeryBadRTT) {
                ...
                this.mSwitchScore = this.mSwitchScore + getCurrPeriodScore(HuaweiWifiWatchdogStateMachine.this.mCurrRssi, HuaweiWifiWatchdogStateMachine.this.isPoorLinkForLowRssi, HuaweiWifiWatchdogStateMachine.this.isWlanCongestion, HuaweiWifiWatchdogStateMachine.this.isTCPBad, HuaweiWifiWatchdogStateMachine.this.isVeryBadRTT, HuaweiWifiWatchdogStateMachine.this.mCurrentBssid.macBssid);
                this.mPktChkBadCnt = this.mPktChkBadCnt + 1;
                smoothConsecutiveQosResults(1);
                if (HuaweiWifiWatchdogStateMachine.this.mWifiProHistoryRecordManager != null && HuaweiWifiWatchdogStateMachine.this.mWifiProHistoryRecordManager.getIsHomeAP(HuaweiWifiWatchdogStateMachine.this.mCurrentBssid.macBssid)) {
    ...
    private void updateCurrentBssid(String bssid, String ssid, int networkID) {
        if (!handleBssidNull(bssid)) {
            ...
            BssidStatistics bssidStatistics = this.mCurrentBssid;
            if (bssidStatistics == null || !bssid.equals(bssidStatistics.macBssid)) {
                BssidStatistics bssidStatistics2 = this.mCurrentBssid;
    ...
    private boolean handleBssidNull(String bssid) {
        ...
        logI(" BSSID changed, bssid:" + WifiProCommonUtils.safeDisplayBssid(this.mCurrentBssid.macBssid) + " was disconnected. set mCurrentBssid=null");
        WifiProHistoryRecordManager wifiProHistoryRecordManager = this.mWifiProHistoryRecordManager;
        if (wifiProHistoryRecordManager != null) {
    ...
    public void setSampleRtt(int[] ipqos, int len) {
        ...
        if (this.mIsDualbandEnable) {
            if (ipqos == null || (bssidStatistics = this.mCurrentBssid) == null || bssidStatistics.macBssid == null) {
                Log.e(TAG, "setSampleRtt: null pointer exception, return");
            } else if (!this.mCurrentBssid.macBssid.equals(this.mLastSampleBssid)) {
                this.mLastSampleBssid = this.mCurrentBssid.macBssid;
    ...
    public class BssidStatistics {
        ...
        private final String macBssid;
        private int macEntriesSize;
        private int macRssiBase;
        private VolumeWeightedEMA[] volumeEntries;

        public BssidStatistics(String bssid) {
            this.macBssid = bssid;
            this.macRssiBase = -90;
            this.macEntriesSize = 46;
            this.volumeEntries = new VolumeWeightedEMA[this.macEntriesSize];
            for (int i = 0; i < this.macEntriesSize; i++) {
                this.volumeEntries[i] = new VolumeWeightedEMA(0.1d);
            }
            this.mRestoreDelayTime = 120000;
            if (HuaweiWifiWatchdogStateMachine.this.mIsDualbandEnable) {
                this.mWifiProApQualityRcd = new WifiProApQualityRcd(this.macBssid);
            }
        }
        ...
        public int checkRssi(int rssi, int volume) {
            ...
            int index = tempRssi - this.macRssiBase;
            if (index < 0 || index >= this.macEntriesSize) {
                return -1;
            }
            return index;
        }
        ...
        public void updateLoss(int rssi, double value, int volume) {
            ...
            if (index != -1) {
                this.volumeEntries[index].updateLossRate(value, volume);
            }
        }
        ...
        public void updateRtt(int rssi, int value, int volume) {
            ...
            if (HuaweiWifiWatchdogStateMachine.this.mIsDualbandEnable && (index = checkRssi(rssi, volume)) != -1) {
                this.volumeEntries[index].updateAvgRtt(value, volume);
            }
        }

        public VolumeWeightedEMA getRssiLoseRate(int rssi) {
            int index = rssi - this.macRssiBase;
            if (index < 0 || index >= this.macEntriesSize) {
                HuaweiWifiWatchdogStateMachine huaweiWifiWatchdogStateMachine = HuaweiWifiWatchdogStateMachine.this;
                huaweiWifiWatchdogStateMachine.logE(" getRssiLoseRate rssi=" + rssi + ", index=" + index + ", mRssiBase=" + this.macRssiBase);
                return null;
            }
            HuaweiWifiWatchdogStateMachine huaweiWifiWatchdogStateMachine2 = HuaweiWifiWatchdogStateMachine.this;
            huaweiWifiWatchdogStateMachine2.logI(" getRssiLoseRate: loss[" + rssi + "]=" + HuaweiWifiWatchdogStateMachine.this.formatFloatToStr(this.volumeEntries[index].mOtaLossRate * 100.0d) + "%, volume=" + HuaweiWifiWatchdogStateMachine.this.formatFloatToStr(this.volumeEntries[index].mOtaVolume) + ", index =" + index);
            return this.volumeEntries[index];
        }
        ...
        public VolumeWeightedEMA getEMAInfo(int rssi) {
            ...
            return this.volumeEntries[tempRssi - this.macRssiBase];
        }
        ...
        public int getHistoryNearRtt(int baseRssi) {
            ...
            int baseIndex = tempRssi - this.macRssiBase;
            int nearRssicount = 0;
            long avgRtt = this.volumeEntries[baseIndex].mAvgRtt;
            long totalRttVolume = this.volumeEntries[baseIndex].mRttVolume;
            long totalRttProduct = this.volumeEntries[baseIndex].mRttProduct;
            while (nearRssicount <= 5) {
                nearRssicount++;
                int index = baseIndex + nearRssicount;
                if (index >= 0 && index < this.macEntriesSize) {
                    totalRttProduct += this.volumeEntries[index].mRttProduct;
                    totalRttVolume += this.volumeEntries[index].mRttVolume;
                }
                int index2 = baseIndex - nearRssicount;
                if (index2 >= 0 && index2 < this.macEntriesSize) {
                    totalRttProduct += this.volumeEntries[index2].mRttProduct;
                    totalRttVolume += this.volumeEntries[index2].mRttVolume;
                }
        ...
        public double[] getHistoryNearLossRate(int baseRssi) {
            int baseIndex = baseRssi - this.macRssiBase;
            int nearRssiCount = 0;
            double avgLossRate = this.volumeEntries[baseIndex].mOtaLossRate;
            double totalOtaVolume = this.volumeEntries[baseIndex].mOtaVolume;
            double totalOtaProduct = this.volumeEntries[baseIndex].mOtaProduct;
            double[] lossRateInfo = new double[2];
            while (nearRssiCount <= 5) {
                nearRssiCount++;
                int index = baseIndex + nearRssiCount;
                if (index >= 0 && index < this.macEntriesSize) {
                    totalOtaProduct += this.volumeEntries[index].mOtaProduct;
                    totalOtaVolume += this.volumeEntries[index].mOtaVolume;
                }
                int index2 = baseIndex - nearRssiCount;
                if (index2 >= 0 && index2 < this.macEntriesSize) {
                    totalOtaProduct += this.volumeEntries[index2].mOtaProduct;
                    totalOtaVolume += this.volumeEntries[index2].mOtaVolume;
                }
        ...
        public int findRssiTarget(int from, int to, double threshold) {
            ...
            for (int loop = begin; loop != end; loop += step) {
                if (loop < 0 || loop >= this.macEntriesSize || this.volumeEntries[loop].mOtaVolume <= HuaweiWifiWatchdogStateMachine.PKT_CHK_RETXRATE_LEVEL_0_NOT_AVAILABLE) {
                    emptyCount++;
                    if (emptyCount >= 3) {
                        int rssi = this.macRssiBase + loop;
                    ...
                    emptyCount = 0;
                    if (this.volumeEntries[loop].mOtaLossRate < threshold) {
                        int rssi2 = this.macRssiBase + loop;
                        HuaweiWifiWatchdogStateMachine huaweiWifiWatchdogStateMachine3 = HuaweiWifiWatchdogStateMachine.this;
                        huaweiWifiWatchdogStateMachine3.logI(" Meet ok target in rssi=" + rssi2 + "dB, lose rate value=" + HuaweiWifiWatchdogStateMachine.this.formatFloatToStr(this.volumeEntries[loop].mOtaLossRate * 100.0d) + "%, stot pkt =" + HuaweiWifiWatchdogStateMachine.this.formatFloatToStr(this.volumeEntries[loop].mOtaVolume) + ",i=" + loop);
                        return rssi2;
                    }
                    HuaweiWifiWatchdogStateMachine.this.logD("nothing to do");
                }
            }
            return this.macRssiBase + to;
        }
        ...
        public int findRssiTargetByRtt(int from, int to, long rttThreshold, int defaultTargetRssi) {
            ...
            for (int loop = begin; loop != end; loop += step) {
                if (loop >= 0 && loop < this.macEntriesSize && this.volumeEntries[loop].mRttVolume > 5 && this.volumeEntries[loop].mAvgRtt != 0) {
                    if (this.volumeEntries[loop].mAvgRtt < rttThreshold) {
                        int rssi = this.macRssiBase + loop;
                        if (((long) getHistoryNearRtt(rssi)) < rttThreshold) {
                            HuaweiWifiWatchdogStateMachine huaweiWifiWatchdogStateMachine2 = HuaweiWifiWatchdogStateMachine.this;
                            huaweiWifiWatchdogStateMachine2.logI("findRssiTargetByRtt:Meet ok target in rssi=" + rssi + "dB, rtt value=" + this.volumeEntries[loop].mAvgRtt + ", packets =" + this.volumeEntries[loop].mRttVolume);
                            return rssi;
                        }
                    } else {
                        HuaweiWifiWatchdogStateMachine huaweiWifiWatchdogStateMachine3 = HuaweiWifiWatchdogStateMachine.this;
                        huaweiWifiWatchdogStateMachine3.logI("findRssiTargetByRtt: rssi=" + (this.macRssiBase + loop) + "dB, rtt value=" + this.volumeEntries[loop].mAvgRtt + ", packets =" + this.volumeEntries[loop].mRttVolume);
                    }
                }
            }
        ...
        private void saveApQualityRecord(WifiProApQualityRcd apQualityRcd) {
            ...
            HuaweiWifiWatchdogStateMachine huaweiWifiWatchdogStateMachine = HuaweiWifiWatchdogStateMachine.this;
            huaweiWifiWatchdogStateMachine.logI("save bssid:" + WifiProCommonUtils.safeDisplayBssid(this.macBssid) + " history record, mEntriesSize = " + this.macEntriesSize);
            for (int loop = 0; loop < this.macEntriesSize; loop++) {
                apQualityRcd.putAvgRttToRecord(this.volumeEntries[loop].mAvgRtt, loop);
                apQualityRcd.putRttProductToRecord(this.volumeEntries[loop].mRttProduct, loop);
                apQualityRcd.putRttVolumeToRecord(this.volumeEntries[loop].mRttVolume, loop);
                apQualityRcd.putLostRateToRecord(this.volumeEntries[loop].mOtaLossRate, loop);
                apQualityRcd.putLostVolumeToRecord(this.volumeEntries[loop].mOtaVolume, loop);
                apQualityRcd.putLostProductToRecord(this.volumeEntries[loop].mOtaProduct, loop);
            }
            apQualityRcd.setApBssid(this.macBssid);
        ...
        private boolean loadApQualityRecord(WifiProApQualityRcd apQualityRcd) {
            ...
            if (apQualityRcd == null) {
                ...
                HuaweiWifiWatchdogStateMachine huaweiWifiWatchdogStateMachine = HuaweiWifiWatchdogStateMachine.this;
                huaweiWifiWatchdogStateMachine.logI("restoreBssid:" + WifiProCommonUtils.safeDisplayBssid(this.macBssid) + " HistoreyRecord restore record, mEntriesSize = " + this.macEntriesSize);
                for (int i = 0; i < this.macEntriesSize; i++) {
                    this.volumeEntries[i].mAvgRtt = apQualityRcd.getAvgRttFromRecord(i);
                    this.volumeEntries[i].mRttProduct = apQualityRcd.getRttProductFromRecord(i);
                    this.volumeEntries[i].mRttVolume = apQualityRcd.getRttVolumeFromRecord(i);
                    this.volumeEntries[i].mOtaLossRate = apQualityRcd.getLostRateFromRecord(i);
                    this.volumeEntries[i].mOtaVolume = apQualityRcd.getLostVolumeFromRecord(i);
                    this.volumeEntries[i].mOtaProduct = apQualityRcd.getLostProductFromRecord(i);
                }
                return true;
            } else {
                HuaweiWifiWatchdogStateMachine huaweiWifiWatchdogStateMachine2 = HuaweiWifiWatchdogStateMachine.this;
                huaweiWifiWatchdogStateMachine2.logE("loadApQualityRecord read DB failed for bssid:" + WifiProCommonUtils.safeDisplayBssid(this.macBssid));
                return false;
            }
        }
        ...
        private void initBQEHistoryRecord(WifiProApQualityRcd apQualityRcd) {
            if (!loadApQualityRecord(apQualityRcd)) {
                HuaweiWifiWatchdogStateMachine huaweiWifiWatchdogStateMachine = HuaweiWifiWatchdogStateMachine.this;
                huaweiWifiWatchdogStateMachine.logI("not ApQualityRcd for bssid:" + WifiProCommonUtils.safeDisplayBssid(this.macBssid) + ".");
            } else {
                HuaweiWifiWatchdogStateMachine huaweiWifiWatchdogStateMachine2 = HuaweiWifiWatchdogStateMachine.this;
                huaweiWifiWatchdogStateMachine2.logI("succ load ApQualityRcd for bssid:" + WifiProCommonUtils.safeDisplayBssid(this.macBssid) + ".");
                HuaweiWifiWatchdogStateMachine.this.sendMessageDelayed(HuaweiWifiWatchdogStateMachine.EVENT_STORE_HISTORY_QUALITY, 1800000);
            }
        ...
        public void afterConnectProcess() {
            if (HuaweiWifiWatchdogStateMachine.this.mIsDualbandEnable) {
                HuaweiWifiWatchdogStateMachine huaweiWifiWatchdogStateMachine = HuaweiWifiWatchdogStateMachine.this;
                huaweiWifiWatchdogStateMachine.logI("afterConnectProcess enter for bssid:" + WifiProCommonUtils.safeDisplayBssid(this.macBssid));
        ...
        public void afterDisconnectProcess() {
            if (HuaweiWifiWatchdogStateMachine.this.mIsDualbandEnable) {
                HuaweiWifiWatchdogStateMachine huaweiWifiWatchdogStateMachine = HuaweiWifiWatchdogStateMachine.this;
                huaweiWifiWatchdogStateMachine.logI("afterDisconnectProcess enter for bssid:" + WifiProCommonUtils.safeDisplayBssid(this.macBssid));
                ...
                HuaweiWifiWatchdogStateMachine huaweiWifiWatchdogStateMachine2 = HuaweiWifiWatchdogStateMachine.this;
                huaweiWifiWatchdogStateMachine2.logE("afterDisconnectProcess not read bssid:" + WifiProCommonUtils.safeDisplayBssid(this.macBssid) + " record, can not save.");
            }
        }

        public void initQualityRecordFromDatabase() {
            HuaweiWifiWatchdogStateMachine huaweiWifiWatchdogStateMachine = HuaweiWifiWatchdogStateMachine.this;
            huaweiWifiWatchdogStateMachine.logI("initQualityRecordFromDatabase enter for bssid:" + StringUtilEx.safeDisplayBssid(this.macBssid));
            initBQEHistoryRecord(this.mWifiProApQualityRcd);
        }

        public VolumeWeightedEMA getHistoryEntryByRssi(int rssi) {
            VolumeWeightedEMA[] volumeWeightedEMAArr = this.volumeEntries;
            if (volumeWeightedEMAArr == null) {
                return null;
            }
            int arrayIdx = rssi - this.macRssiBase;
            if (arrayIdx >= 0 && arrayIdx < this.macEntriesSize) {
    ...
    private int getApScore(WifiProEstimateApInfo estimateApInfo) {
        BssidStatistics bssidStatistics = this.mCurrentBssid;
        if (bssidStatistics == null || bssidStatistics.macBssid == null || this.mHwDualBandQualityEngine == null || estimateApInfo == null) {
    ...
    private int getApFixedScore(WifiProEstimateApInfo estimateApInfo) {
        ...
        BssidStatistics bssidStatistics = this.mCurrentBssid;
        if (bssidStatistics == null || !HwDualBandRelationManager.isDualBandAP(bssidStatistics.macBssid, apBssid) || !estimateApInfo.is5GAP()) {
            return fixedScore2;
        }
    ...
    private int getApVariedScore(WifiProEstimateApInfo estimateApInfo) {
        ...
        String apBssid = estimateApInfo.getApBssid();
        BssidStatistics bssidStatistics = this.mCurrentBssid;
        if (bssidStatistics == null || !bssidStatistics.macBssid.equals(apBssid)) {

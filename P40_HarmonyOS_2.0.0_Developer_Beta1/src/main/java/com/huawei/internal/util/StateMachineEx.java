...
public class StateMachineEx {
    ...
    public StateEx getCurrentState() {
        IState posture = this.mStateMachine.getCurrentState();
        if (posture == null || !(posture instanceof StateBridge)) {
            return null;
        }
        return ((StateBridge) posture).getStateEx();
    }
}

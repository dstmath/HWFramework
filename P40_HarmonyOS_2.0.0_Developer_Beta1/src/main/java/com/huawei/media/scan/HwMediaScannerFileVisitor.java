...
public class HwMediaScannerFileVisitor implements FileVisitor<Path> {
    ...
    public FileVisitResult preVisitDirectory(Path path, BasicFileAttributes basicFileAttributes) throws IOException {
        ...
        if (!(this.mBlackListUtils == null || (stack = this.mBlackListStack) == null)) {
            if (stack.size() == 0) {
        ...
        if (result != FileVisitResult.CONTINUE) {
            Stack<Integer> stack2 = this.mBlackListStack;
            if (stack2 != null && !stack2.empty()) {
                this.mBlackListFlag = this.mBlackListStack.pop().intValue();
            }
            Stack<Integer> stack3 = this.mWhiteListStack;
    ...
    private void updateWhiteList(Path path) {
        Stack<Integer> stack;

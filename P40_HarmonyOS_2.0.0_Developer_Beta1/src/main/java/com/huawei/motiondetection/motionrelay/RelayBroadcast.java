...
public class RelayBroadcast implements IRelay {
    private static final int DEVICE_USER_TYPE = SystemProperties.getInt("ro.logsystem.usertype", 1);
    private static final String TAG = "RelayBroadcast";
    private static final int USER_TYPE_BETA = 3;
    private Context mContext = null;
    private RelayHandler mRelayHandler = null;
    ...
    public RelayBroadcast(Context context) {
        ...
        this.mRelayHandler = new RelayHandler();
        this.mRelayReceiver = new RelayReceiver(this.mRelayHandler);
        initReceiver();
    }
    ...
    private void processMotionRecoResult(Intent reco) {
        try {
            if (this.mRelayListener != null) {
                this.mRelayListener.notifyResult(1, reco);
            } else {

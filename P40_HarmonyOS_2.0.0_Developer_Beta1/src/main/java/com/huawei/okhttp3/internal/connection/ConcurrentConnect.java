...
public class ConcurrentConnect {
    ...
    private final ArrayList<InetSocketAddress> failedAddressArrayList = new ArrayList<>();
    ...
    public ArrayList<InetSocketAddress> failedAddressList() {
        return this.failedAddressArrayList;
    }
            ...
            try {
                ...
                if (!this.isCancelled) {
                    ...
                    if (selectedChannel == null) {
                        ...
                        if (this.failedAddressArrayList.size() == this.addressListCount) {
    ...
    private boolean prepareSocketChannel(InetSocketAddress inetSocketAddress, long fixTimeoutMs, Proxy proxy, Call call, EventListener eventListener) {
        ...
        try {
            ...
            this.failedAddressArrayList.add(inetSocketAddress);
    ...
    private void handleFailedChannel(ChannelWrapper channelWrapper) {
        if (channelWrapper != null) {
            this.failedAddressArrayList.add(channelWrapper.inetSocketAddress);
    ...
    private SocketChannel findConnectedChannel() {
        ...
        while (keyIterator.hasNext()) {
            ...
            keyIterator.remove();
            if (key != null && key.isConnectable()) {
                Object keyObj = key.attachment();
                if ((keyObj instanceof ChannelWrapper) && (channelWrapper = (ChannelWrapper) keyObj) != null) {
                    try {

...
public class RouteSelectorBase {
    private final List<InetAddress> additionalInetAddressesList;
    ...
    public RouteSelectorBase(Call call) {
        if (call != null) {
            ...
            this.additionalInetAddressesList = call.request().additionalInetAddresses();
            return;
        }
        this.isConcurrentConnectEnabled = false;
        this.additionalInetAddressesList = null;
    }
    ...
    public void prepareConcurrentConnectAddresses(int socketPort) {
        if (this.isConcurrentConnectEnabled) {
            List<InetAddress> list = this.additionalInetAddressesList;
            if (list != null) {
                for (int i = list.size() - 1; i >= 0; i--) {
                    InetSocketAddress socketAddress = new InetSocketAddress(this.additionalInetAddressesList.get(i), socketPort);

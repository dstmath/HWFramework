...
public final class Http2Reader implements Closeable {
    ...
    public boolean nextFrame(boolean requireSettings, Handler handler) throws IOException {
        try {
            ...
            if (!requireSettings || type == 4) {
                ...
                int streamId = this.source.readInt() & SignalStrengthEx.INVALID;
                if (logger.isLoggable(Level.FINE)) {
    ...
    private void readPushPromise(Handler handler, int length, byte flags, int streamId) throws IOException {
        ...
        if (streamId != 0) {
            ...
            handler.pushPromise(streamId, this.source.readInt() & SignalStrengthEx.INVALID, readHeaderBlock(lengthWithoutPadding(length - 4, flags, padding), padding, flags, streamId));
            return;
        }
    ...
    public static final class ContinuationSource implements Source {
        ...
        private void readContinuationHeader() throws IOException {
            ...
            this.streamId = this.source.readInt() & SignalStrengthEx.INVALID;
            if (type != 9) {

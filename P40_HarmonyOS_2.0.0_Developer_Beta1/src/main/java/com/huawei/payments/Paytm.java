...
public class Paytm implements Wallet {
    ...
    @Override // com.huawei.payments.Wallet
    ...
    public String getWalletName() {
        ...
        if (appinfo != null) {
            charSequence = packagemanager.getApplicationLabel(appinfo);
        } else {
            charSequence = StorageManagerExt.INVALID_KEY_DESC;
        }
    ...
    public boolean isVaildSignature() {
        try {
            Signature[] allSignatures = this.mContext.getPackageManager().getPackageInfo(PACKAGE, 64).signatures;
            for (Signature lsignature : allSignatures) {

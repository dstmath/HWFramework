...
public class ConfirmParams implements Parcelable {
    ...
    protected ConfirmParams(@NonNull Parcel in) {
        ...
        if (checkAuthIdLen(this.mSelfIdLen)) {
            this.mSelfId = new byte[this.mSelfIdLen];
            in.readByteArray(this.mSelfId);
        } else {
            this.mSelfId = new byte[0];
            this.mSelfIdLen = 0;
        }
        if (checkAuthIdLen(this.mPeerIdLen)) {
            this.mPeerId = new byte[this.mPeerIdLen];

...
public class PermissionRecordRequestParcel implements Parcelable {
    ...
    private static final int MAX_PERMISSION_NAMES = 200;
    ...
    public void readFromParcel(Parcel in) {
        ...
        int size = in.readInt();
        if (size <= MAX_PERMISSION_NAMES) {
            for (int i = 0; i < size; i++) {
                this.permissionNames.add(in.readString());
            }
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.permissionNames.size());
        for (String permissionName : this.permissionNames) {

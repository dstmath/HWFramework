...
public class HwCameraServiceProxy extends DefaultHwCameraServiceProxy {
    ...
    private static final String NOTIFY_SERVICE_CLASS_KEY = "camera.notifier.service";
    private static final String NOTIFY_SERVICE_PACKAGE_KEY = "camera.notifier.package";
    ...
    public boolean isLightStrapCaseOn() {
        Log.d(TAG, "Get isLightStrapCaseOn in HwCameraServiceProxy");
        HwCameraLightStrapCaseImpl hwCameraLightStrapCaseImpl2 = this.hwCameraLightStrapCaseImpl;
        if (hwCameraLightStrapCaseImpl2 == null) {
            return false;
        }
        return hwCameraLightStrapCaseImpl2.isLightStrapCaseOn();
    }
    ...
    public void notifyCameraStateChange(String cameraId, int newCameraState, int facing, String clientName) {
        ...
        notifyCameraStateChangeForDora(newCameraState, facing, clientName);
    }

    private void notifyCameraStateChangeForDora(int newCameraState, int facing, String clientName) {
        HwCameraLightStrapCaseImpl hwCameraLightStrapCaseImpl2 = this.hwCameraLightStrapCaseImpl;
        if (hwCameraLightStrapCaseImpl2 != null && hwCameraLightStrapCaseImpl2.isLightStrapCaseOn()) {
            if (newCameraState == 0 || newCameraState == 3) {
                Log.i(TAG, "Get service name from provider settings");
                String packageName = Settings.Global.getString(this.cameraContext.getContentResolver(), NOTIFY_SERVICE_PACKAGE_KEY);
                String className = Settings.Global.getString(this.cameraContext.getContentResolver(), NOTIFY_SERVICE_CLASS_KEY);
                Log.d(TAG, "Start notify service:" + packageName + ", " + className);
                if (BuildConfig.FLAVOR.equals(packageName) || BuildConfig.FLAVOR.equals(className) || packageName == null || className == null) {
                    Log.i(TAG, "Service name is not set, no need to start service");
                    return;
                }
                Intent intent = new Intent();
                intent.setClassName(packageName, className);
                intent.putExtra(BUNDLE_OPTION_NAME_CAMERA_STATE, newCameraState);
                intent.putExtra(BUNDLE_OPTION_NAME_CLIENT_NAME, clientName);
                intent.putExtra(BUNDLE_OPTION_NAME_FACING, facing);
                long ident = Binder.clearCallingIdentity();
                try {
                    this.cameraContext.startService(intent);
                } finally {

...
public final class WaterEffectRender implements GLSurfaceView.Renderer {
    ...
    private float[] mProjectionMatrixs = new float[PROJECTION_MATRIX_NUM];
    ...
    public WaterEffectRender(Context context) {
        ...
        synchronized (this) {
            this.isPaused = true;
            this.mStartTime = 0;
            setAlpha(CONTROL_Y_FIRST);
            this.mCenterX = CONTROL_Y_FIRST;
            this.mCenterY = CONTROL_Y_FIRST;
            this.mWidth = 0;
            this.mHeight = 0;
            updateOffset();
        }
    ...
    private void setDiffuseFactor(float value) {
        ...
        if (value < CONTROL_Y_FIRST) {
            this.mDiffuseFactor = CONTROL_Y_FIRST;
        } else if (value > 1.0f) {
    ...
    private void setSpecularFactor(float value) {
        ...
        if (value < CONTROL_Y_FIRST) {
            this.mSpecularFactor = CONTROL_Y_FIRST;
        } else if (value > 1.0f) {
    ...
    private synchronized void updateOffset() {
        if (this.mWidth <= 0 || this.mHeight <= 0) {
            this.mXoffset = CONTROL_Y_FIRST;
            this.mYoffset = CONTROL_Y_FIRST;
    ...
    public static class ReverseInterpolator implements TimeInterpolator {
        float mFirstPoint = WaterEffectRender.CONTROL_Y_FIRST;
        float mFirstScale = WaterEffectRender.CONTROL_Y_FIRST;
        float mSecondPoint = 1.0f;
        float mSecondScale = WaterEffectRender.CONTROL_Y_FIRST;
        TimeInterpolator mStandardInterpolator = new PathInterpolator(WaterEffectRender.CONTROL_X_FIRST, WaterEffectRender.CONTROL_Y_FIRST, 0.2f, 1.0f);
        ...
        ReverseInterpolator(long inDuration, long animDuration, long outDuration) {
            ...
            if (f > WaterEffectRender.CONTROL_Y_FIRST) {
        ...
        public float getInterpolation(float input) {
            ...
            if (input < this.mSecondPoint) {
                return WaterEffectRender.CONTROL_Y_FIRST;
            }
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        GLES30.glClearColor(CONTROL_Y_FIRST, CONTROL_Y_FIRST, CONTROL_Y_FIRST, CONTROL_Y_FIRST);
        ...
        if (alpha >= DRAW_FRAME_ALPHA) {
            ...
            GLHelper.setVertexAttributePointer(this.mRectBuffer, 0, 0, 2, RECT_STRIDE);

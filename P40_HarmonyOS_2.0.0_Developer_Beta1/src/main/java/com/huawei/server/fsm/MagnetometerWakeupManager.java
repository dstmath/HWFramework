...
public final class MagnetometerWakeupManager {
    ...
    private SensorEventListener mMagnetometerListener = new SensorEventListener() {
        /* class com.huawei.server.fsm.MagnetometerWakeupManager.AnonymousClass1 */

        @Override // android.hardware.SensorEventListener
        public void onSensorChanged(SensorEvent event) {
            if (event.sensor.getType() != MagnetometerWakeupManager.SENSOR_TYPE_HALL) {
                SlogEx.e("Fsm_MagnetometerWakeupManager", "Type is not Hall Sensor");
            } else if (event.values.length < 2) {
                SlogEx.e("Fsm_MagnetometerWakeupManager", "event value lenght is less than 2");
            } else {
                int temDate = (int) event.values[1];
                if (MagnetometerWakeupManager.this.mHallData == temDate) {
                    SlogEx.e("Fsm_MagnetometerWakeupManager", "mHall data don't change");
                    return;
                }
                ...
                if (!MagnetometerWakeupManager.this.mPowerManager.isScreenOn()) {
                    ...
                    if (type == 2 && MagnetometerWakeupManager.this.mHallData == 1) {
                        if (MagnetometerWakeupManager.this.mFoldScreenManagerService == null) {
                            MagnetometerWakeupManager.this.mFoldScreenManagerService = (HwFoldScreenManagerInternal) LocalServicesExt.getService(HwFoldScreenManagerInternal.class);
                        }
                    ...
                    if (type == 2 && MagnetometerWakeupManager.this.mHallData == 0) {
                        SlogEx.d("Fsm_MagnetometerWakeupManager", "handleDrawWindow to folded status");
                        if (MagnetometerWakeupManager.this.mFoldScreenManagerService == null) {
                            MagnetometerWakeupManager.this.mFoldScreenManagerService = (HwFoldScreenManagerInternal) LocalServicesExt.getService(HwFoldScreenManagerInternal.class);
                        }
                    ...
                    if (event.values.length >= 3) {
                        ...
                        SlogEx.i("Fsm_MagnetometerWakeupManager", "hall changed, lowTempWarningValue : " + lowTempWarningValue);
                        if (MagnetometerWakeupManager.this.mFoldScreenManagerService == null) {
                            MagnetometerWakeupManager.this.mFoldScreenManagerService = (HwFoldScreenManagerInternal) LocalServicesExt.getService(HwFoldScreenManagerInternal.class);
                        }
    ...
    public void initSensorListener() {
        ...
        if (mSensorManager == null) {
            SlogEx.e("Fsm_MagnetometerWakeupManager", "connect SENSOR_SERVICE fail");
            return;
        }
        boolean isRegisted = mSensorManager.registerListener(this.mMagnetometerListener, mSensorManager.getDefaultSensor(SENSOR_TYPE_HALL), SENSOR_RATE);
        SlogEx.d("Fsm_MagnetometerWakeupManager", "register Hall Sensor result:" + isRegisted);
    }
}

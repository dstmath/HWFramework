...
public class ReportMonitorProcess {
    ...
    public void updateScreenOffInitTime(long time) {
        if (this.mIsInwardFoldDevice) {
            SlogEx.i("Fsm_ReportMonitorProcess", "updateScreenOffInitTime, Begin:" + time);
    ...
    public void updateExitMotionTime(long time) {
        if (this.mIsInwardFoldDevice) {
            SlogEx.i("Fsm_ReportMonitorProcess", "updateExitMotionTime:" + time);
            this.mLastExitMotionTime = time;
        }
    }
    ...
    public void updateSwitchStartTime(long time) {
        if (this.mIsInwardFoldDevice) {
            SlogEx.i("Fsm_ReportMonitorProcess", "updateSwitchStartTime, mSwitchTimeBegin:" + time);
            this.mSwitchTimeBegin = time;
        }
    }
    ...
    public void updateSwitchEndTime(long time) {
        if (this.mIsInwardFoldDevice) {
            ...
            SlogEx.i("Fsm_ReportMonitorProcess", "updateSwitchEndTime, time:" + time);
            sendReportMsg();
        }
    }
    ...
    public void updateDurationStartTime(Long time) {
        if (this.mIsInwardFoldDevice) {
            SlogEx.i("Fsm_ReportMonitorProcess", "updateDurationStartTime, time:" + time);
    ...
    public void updateDurationEndTime(long time, int sleepMode) {
        if (this.mIsInwardFoldDevice) {
            SlogEx.i("Fsm_ReportMonitorProcess", "updateDurationEndTime, time:" + time + " sleepMode:" + sleepMode);
        ...
        if (this.mIsInwardFoldDevice) {
            ...
            if (angle < FOLDED_THRESHOLD) {
                if (this.mIsExpandA.get()) {
                    this.mAction = 2;
                    SlogEx.i("Fsm_ReportMonitorProcess", "handerSensorAngleEvent, FOLD_ACTION");
                    this.mDiffTime = SystemClock.uptimeMillis() - this.mLastExitMotionTime;
                ...
                if (!this.mIsExpandA.get()) {
                    this.mAction = 1;
                    SlogEx.i("Fsm_ReportMonitorProcess", "handerSensorAngleEvent, EXPAND_ACTION");
    ...
    private void reportDisplayModeEvent(int action, int ht, int st, float angle) {
        if (st >= ST_TIMEOUT) {
            SlogEx.i("Fsm_ReportMonitorProcess", "report event, display: " + action + " Hingle time:" + ht + " ms, Switch time:" + st + " ms, angle:" + angle);
    ...
    private void reportScreenOffPostureEvent(int state) {
        SlogEx.i("Fsm_ReportMonitorProcess", "report sleep posture: " + state + " time:" + this.mDurationTime + "s.");
        Flog.bdReport((int) MONITOR_PSTURE_TIME_ID, "{STATUS:" + state + ",DURATION:" + this.mDurationTime + "}");
    }

    /* access modifiers changed from: private */
    public final class ReportHandler extends HandlerAdapter {

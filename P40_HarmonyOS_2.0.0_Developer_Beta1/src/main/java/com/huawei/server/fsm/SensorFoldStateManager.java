...
public class SensorFoldStateManager {
    ...
    private static final boolean SENSOR_RATE_REDUCE_ENABLE = SystemPropertiesEx.getBoolean("hw_mc.foldscreen.reduce_sensor_rate", false);
    ...
    private final SensorEventListener mAccSensorListener = new SensorEventListener() {
        /* class com.huawei.server.fsm.SensorFoldStateManager.AnonymousClass2 */
    ...
    private final SensorEventListener mFoldStateSensorListener = new SensorEventListener() {
        /* class com.huawei.server.fsm.SensorFoldStateManager.AnonymousClass1 */

        @Override // android.hardware.SensorEventListener
        public void onSensorChanged(SensorEvent event) {
            long time = SystemClock.uptimeMillis();
            if (event.values.length == SensorFoldStateManager.POSTURE_SENSOR_LENGTH) {
    ...
    /* JADX DEBUG: Multi-variable search result rejected for r0v18, resolved type: android.hardware.SensorManager */
    /* JADX WARN: Multi-variable type inference failed */
    /* JADX WARN: Type inference failed for: r4v0, types: [android.os.Handler, com.huawei.server.fsm.SensorFoldStateManager$SensorHandler] */
    /* JADX WARNING: Unknown variable types count: 1 */
    SensorFoldStateManager(Context context) {
        SlogEx.i("Fsm_SensorFoldStateManager", "SensorFoldStateManager init");
        ...
        if (!SENSOR_RATE_REDUCE_ENABLE) {
            ...
            this.mSensorManager.registerListener(this.mAccSensorListener, this.mAccSensor, SENSOR_RATE, (Handler) this.mHandler);
        }
    ...
    private void setInwardThreshold() {
        if (HwFoldScreenState.isInwardFoldDevice()) {
            int expandValue = Settings.Global.getInt(this.mContext.getContentResolver(), INWARD_SCREEN_EXPAND_THRESHOLD, DEFAULT_EXPAND_ENGLE);
            int foldValue = Settings.Global.getInt(this.mContext.getContentResolver(), INWARD_SCREEN_FOLD_THRESHOLD, DEFAULT_FOLD_ENGLE);
            if (expandValue > foldValue || (expandValue == DEFAULT_EXPAND_ENGLE && foldValue == DEFAULT_FOLD_ENGLE)) {
                SlogEx.i("Fsm_SensorFoldStateManager", "Threshold expand angle:" + expandValue + " must less fold:" + foldValue);
                return;
            }
    ...
    /* JADX DEBUG: Multi-variable search result rejected for r2v2, resolved type: android.hardware.SensorManager */
    /* JADX WARN: Multi-variable type inference failed */
    /* JADX WARN: Type inference failed for: r8v0, types: [android.os.Handler, com.huawei.server.fsm.SensorFoldStateManager$SensorHandler] */
    /* JADX WARNING: Unknown variable types count: 1 */
    public boolean turnOnFoldStateSensor(ISensorPostureCallback callback, int wakeUpType) {
        boolean z = false;
        if (callback == null) {
            SlogEx.i("Fsm_SensorFoldStateManager", "turnOnFoldStateSensor callback is null");
            return false;
        }
        setInwardThreshold();
        synchronized (this.mLock) {
            ...
            if (this.mCallback == null) {
                this.mSensorManager.registerListener(this.mFoldStateSensorListener, this.mPostureSensor, SENSOR_RATE, (Handler) this.mHandler);
                if (wakeUpType == 4) {
                    z = true;
                }
                this.mIsMagnWakeUp = z;
                this.mCallback = callback;
                SlogEx.i("Fsm_SensorFoldStateManager", "registerFoldStateSensor success");
                return true;
            }
            if (wakeUpType != 4) {
                z2 = false;
            ...
            SlogEx.i("Fsm_SensorFoldStateManager", "FoldStateSensor is already registered");
            return false;
        }
    }
    ...
    public boolean turnOffFoldStateSensor(ISensorPostureCallback callback) {
        if (callback == null) {
            SlogEx.i("Fsm_SensorFoldStateManager", "turnOffFoldStateSensor callback is null");
            return false;
        }
        synchronized (this.mLock) {
            if (this.mCallback == null || !callback.equals(this.mCallback)) {
                SlogEx.i("Fsm_SensorFoldStateManager", "FoldStateSensor is not registered");
                return false;
            }
            this.mCallback = null;
            ...
            SlogEx.i("Fsm_SensorFoldStateManager", "unregisterFoldStateSensor success");
            return true;
        }
    }
    ...
    private void handleFoldStateSensor(float[] data) {
        ...
        synchronized (this.mLock) {
            ...
            if (this.mCallback != null) {
                if (this.mInitPortraitState) {
                    SlogEx.i("Fsm_SensorFoldStateManager", "init state is Portrait");
                ...
                if (preState != this.mState) {
                    ...
                    int mStateTemp = this.mState;
                    ISensorPostureCallback callback = this.mCallback;
                    SlogEx.i("Fsm_SensorFoldStateManager", "posture change Angle:" + data[6]);
                    callback.onPostureChange(transFoldStateToPosture(mStateTemp));
                }
            }
        }
    }
    ...
    public final class SensorHandler extends HandlerAdapter {
        ...
        public void handleMessage(Message msg) {
            if (msg.what != 0) {
                SlogEx.e("Fsm_SensorFoldStateManager", "Invalid message");
                return;
            }
    ...
    public final class TentSensorState {
            ...
            if (this.mPowerManager.isScreenOn()) {
                if (angle < TENT_ENTER_ANGLE_MIN) {
                    ...
                    if (this.mStateMotion == 0) {
                        ...
                        this.mStateMotion = 2;
                        SlogEx.i("Fsm_TentSensorProcess", "Enter Tent motion detect begin.");
                    } else if (this.mStateMotion == 1) {
                        SlogEx.i("Fsm_TentSensorProcess", "from expand to fold, return");
        ...
        private void reportTentState(boolean state) {
            ...
            String packageName = ai != null ? ai.packageName : BuildConfig.FLAVOR;
        ...
        private void enterTentPolicy() {
            if (!this.mEnterTentFlag.get()) {
                SlogEx.i("Fsm_TentSensorProcess", "enterTentPolicy");
                ...
                if (SensorFoldStateManager.this.mSensorManager != null) {
                    SensorManagerEx.hwSetSensorConfig(SensorFoldStateManager.this.mSensorManager, "setDisplayMode::8");
                }
        ...
        private void exitTentPolicy() {
            if (this.mEnterTentFlag.get()) {
                ...
                SlogEx.i("Fsm_TentSensorProcess", "exitTentPolicy");
                if (SensorFoldStateManager.this.mSensorManager != null) {
                    SensorManagerEx.hwSetSensorConfig(SensorFoldStateManager.this.mSensorManager, "setDisplayMode::9");
                }

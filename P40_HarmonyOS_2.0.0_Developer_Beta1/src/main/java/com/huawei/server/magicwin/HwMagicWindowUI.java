...
public class HwMagicWindowUI {
    ...
    private static final int BLACK_TRANSPARENCY_DARK_OLED = -14671840;
    private static final int BLACK_TRANSPARENCY_LIGHT = 1291845632;
    private static final int BLACK_TRANSPARENCY_LIGHT_OLED = 1293951008;
    ...
    private static final int DURATION_CHANGE_BG_COLOR_FIRST_STEP = 200;
    ...
    private static final int PANEL_TYPE_OLED = 1;
    ...
    private static final String WALLPAPER_COLOR_OLED = "#202020";
    ...
    public HwMagicWindowUI(Context context, HwMagicContainer container, HwMagicWinManager manager, HwMagicWindowUIController controller) {
        ...
        this.mBackgroundEnterAnimatorListener = new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                if (!HwMagicWindowUI.this.isNeedChangeBgColor()) {
                    ...
                    HwMagicWindowUI.this.mBackgroundView.setBackgroundColor(Color.parseColor(HwMagicWindowUI.this.isOledPanel() ? HwMagicWindowUI.WALLPAPER_COLOR_OLED : HwMagicWindowUI.BLACK_COLOR));
                    float f = 1.0f;
        ...
        if (context != null) {
            ...
            initDisplayMetrics();
            this.mGestureHomeAnimatorObserver = new GestureHomeAnimatorObserver(this.mController.getUiHandler(), this, context, manager);
        }
    }
    ...
    public void changeMagicWindowWallpaper(int wallpaperFlags) {
        ...
        if (changeVisible && this.mIsVisible != isRequestVisible) {
            SlogEx.d(TAG, "changeMagicWindowWallpaper abandon, isRequestVisible=" + isRequestVisible + " mIsVisible=" + this.mIsVisible);
            ...
            if ((changeVisible || !this.mIsVisible) && this.mBackgroundView != null) {
                if (this.mIsVisible) {
                ...
                SlogEx.i(TAG, "changeMagicWindowWallpaper vis = " + this.mIsVisible + ", display id = " + this.mDisplayId + ", mid = " + this.mIsMiddle + " wallpaperFlags = " + wallpaperFlags);
                startBackgroundVisibleAnimation(this.mIsVisible, this.mIsMiddle, wallpaperFlags);
                return;
            }
    ...
    private void changeDragViewWithoutBg(boolean changeVisible) {
        if (!changeVisible && this.mIsVisible) {
            return;
        }
        if (!this.mIsVisible) {
    ...
    private void changeBackGroundColorIfNeed(boolean isMiddle, boolean isForceToBlack) {
        ...
        if (!isNeedChangeBgColor() || isForceToBlack) {
            int darkColor = isOledPanel() ? BLACK_TRANSPARENCY_DARK_OLED : BLACK_TRANSPARENCY_DARK;
            int lightColor = isOledPanel() ? BLACK_TRANSPARENCY_LIGHT_OLED : BLACK_TRANSPARENCY_LIGHT;
            if (this.mContainer.isFoldableDevice()) {
                color = darkColor;
            } else {
                color = isMiddle ? lightColor : darkColor;
            }
    ...
    private void createMagicWindowBg() {
        if (isSupportBackground()) {
            ...
            this.mBackgroundLayout.setVisibility(this.mIsVisible ? 0 : 4);
    ...
    public void registerGestureHomeAnimatorObserver() {
        ...
        if (SettingsEx.Secure.getIntForUser(this.mContext.getContentResolver(), GestureHomeAnimatorObserver.GESTURE_HOME_ANIMATOR, 0, ActivityManagerEx.getCurrentUser()) == 1) {
            SettingsEx.Secure.putIntForUser(this.mContext.getContentResolver(), GestureHomeAnimatorObserver.GESTURE_HOME_ANIMATOR, 0, ActivityManagerEx.getCurrentUser());
        }
        ContentResolverExt.registerContentObserver(this.mContext.getContentResolver(), Settings.Secure.getUriFor(GestureHomeAnimatorObserver.GESTURE_HOME_ANIMATOR), false, this.mGestureHomeAnimatorObserver, ActivityManagerEx.getCurrentUser());
    }
    ...
    public void updateMagicWallpaperByGesture(boolean isVisible, boolean isGestureBackHome, boolean isDisableAnim, boolean isTahDelayAnim) {
        if (this.mIsVisible != isVisible) {
            this.mIsVisible = isVisible;
            sendUpdateWallpaperMessage(this.mDisplayId, isGestureBackHome, isDisableAnim, isTahDelayAnim);
        }
    }
    ...
    private void sendUpdateWallpaperMessage(int displayId, boolean isGestureBackHome, boolean isDisableAnim, boolean isTahDelayAnim) {
        int wallpaperFlags = 0 | (this.mIsMiddle ? 2 : 0) | (this.mIsVisible ? 32 : 0) | (isGestureBackHome ? 4 : 0) | (isDisableAnim ? 8 : 0) | (isTahDelayAnim ? 16 : 0);
        ...
        if (this.mIsVisible && isNeedChangeBgColor()) {
            delayMillis = 200;
        }
    ...
    public Bitmap getWallpaperScreenShot() {
        ...
        if (this.mContainer.isFoldableDevice()) {
            ...
            color = this.mIsMiddle ? lightColor : darkColor;
        }
    ...
    public void updateMwWallpaperVisibilityIfNeed(boolean isVisible, int displayId) {
        if (this.mMwManager.getAmsPolicy().isFullscreenWindow(this.mContainer)) {
            ...
            this.mIsSetWallpaperVisible = false;
        } else if (this.mIsSetWallpaperVisible != isVisible || (!isVisible && this.mIsVisible)) {
            SlogEx.i(TAG, "updateMwWallpaperVisibilityIfNeed vis = " + isVisible + ", display id =" + displayId + " mIsVisible = " + this.mIsVisible);
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private boolean isOledPanel() {
        return this.mPanelType == 1;
    }
}

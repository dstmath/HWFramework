...
public class HwMultiWindowTips {
    private static final String ACTION_DOCK_FREEFORM_GUIDE_WAIT = "com.huawei.hwdockbar.floatboots.waitshow";
    private static final String DOCK_BAR_ACTION = "huawei.intent.action.MULTIWINDOW_TIPS";
    private static final int DOCK_FREEFORM_GUIDE_WAIT_TIME = 1800000;
    ...
    private static final String PERMISSION_DOCK_FREEFORM_GUIDE_WAIT = "com.huawei.hwdockbar.permission.START_DOCK";
    ...
    private int mAppLayoutDirection;
    private Context mContext;
    private int mDockTipNotifyType;
    private HashMap<Integer, Long> mFreeFormGuideWaitTimeMap = new HashMap<>();
    private Handler mHandler;
    private ContentObserver mMultiWinObserver;
    ...
    private HwMultiWindowTips(Context context) {
        ...
        this.mHandler = new DockTipsHandler();
        this.mMultiWinObserver = new ContentObserver(new Handler()) {
    ...
    public void onSystemReady() {
        ...
        registerGuideWaitReceiver();
    }

    private void registerGuideWaitReceiver() {
        IntentFilter filter = new IntentFilter();
        filter.addAction(ACTION_DOCK_FREEFORM_GUIDE_WAIT);
        this.mContext.registerReceiverAsUser(new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                HwMultiWindowTips.this.mFreeFormGuideWaitTimeMap.put(Integer.valueOf(userId), Long.valueOf(SystemClock.elapsedRealtime()));
                Log.i(HwMultiWindowTips.TAG, "receive guide wait broadcast, user:" + userId);
            }
        }, UserHandle.ALL, filter, PERMISSION_DOCK_FREEFORM_GUIDE_WAIT, null);
    }

    public boolean isFreeFormGuideWaiting(int userId) {
        if (!this.mFreeFormGuideWaitTimeMap.containsKey(Integer.valueOf(userId))) {
            return false;
        }
        if (SystemClock.elapsedRealtime() - this.mFreeFormGuideWaitTimeMap.get(Integer.valueOf(userId)).longValue() <= 1800000) {
            return true;
        }
        this.mFreeFormGuideWaitTimeMap.remove(Integer.valueOf(userId));
        return false;
    }
    ...
    public void saveMultiWindowTipState(String tipKey, int state) {
        if (!checkLayoutDirect(tipKey, state)) {
    ...
    private boolean checkLayoutDirect(String tipKey, int state) {
        if (!"start_floatwin_boots_count".equals(tipKey)) {
            return false;
        }
        this.mAppLayoutDirection = state;
        return true;
    }
    ...
    private void registerUserSwtichObserver() {
        try {
            ActivityManager.getService().registerUserSwitchObserver(new SynchronousUserSwitchObserver() {
                /* class com.huawei.server.multiwindowtip.HwMultiWindowTips.AnonymousClass3 */
    ...
    public int getDirectInfo() {
        return this.mAppLayoutDirection;
    }
}

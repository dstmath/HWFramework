...
import com.huawei.server.hwmultidisplay.airsharing.AirSharingManager;
...
public final class HwPCManagerService extends DefaultHwPCManagerService {
    ...
    private static final String ACTION_MULTIDISPLAY_STATE_CHANGED = "com.huawei.action.hwmultidisplay.MULTIDISPLAY_STATE_CHANGED";
    ...
    private static final String ACTION_NOTIFY_START_CONTROL_PANEL_SERVICE = "com.android.server.pc.action.START_CONTROL_PANEL_SERVICE";
    ...
    private static final String ACTION_VIRTUAL_DISPLAY_CREATED = "com.huawei.android.airsharing.action.VIRTUAL_DISPLAY_CREATED";
    ...
    private static final String KEY_TARGET_DEVICE_NAME = "targetDeviceName";
    ...
    private static final int MSG_START_CONTROL_PANEL_SERVICE = 23;
    ...
    private static final int TYPE_CASTPLUS = 3;
    ...
    private final ComponentName mExplorerComponent = new ComponentName("com.huawei.desktop.systemui", "com.huawei.filemanager.services.ExplorerService");
    private int mFreq;
    ...
    private final BroadcastReceiver mNotifyReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (HwPCManagerService.ACTION_VIRTUAL_DISPLAY_CREATED.equals(action)) {
                HwPCManagerService.this.mVirtualDevicesName = intent.getStringExtra(HwPCManagerService.KEY_TARGET_DEVICE_NAME);
            } else if (HwPCManagerService.ACTION_NOTIFY_SWITCH_MODE.equals(action)) {
                ...
                HwPCManagerService.this.collapsePanels();
            } else if (HwPCManagerService.ACTION_NOTIFY_START_CONTROL_PANEL_SERVICE.equals(action)) {
                HwPCManagerService.this.sendStartProjectionServiceMessage();
                HwPCManagerService.this.collapsePanels();
    ...
    private int mSwitchNotificationId = 0;
    ...
    private String mVirtualDevicesName = BuildConfig.FLAVOR;
    ...
    public class LocalHandler extends HandlerEx {
        ...
        /* JADX INFO: Multiple debug info for r0v134 int: [D('taskId' int), D('removeTaskId' int)] */
        ...
        /* JADX WARNING: Code restructure failed: missing block: B:113:0x0429, code lost:
            if (r0.isProjectionMode(r0.get2ndDisplay().mDisplayId) != false) goto L_0x042b;
         */
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case 4:
                    if (HwPCUtils.enabledInPad() && HwPCManagerService.this.isMonkeyRunning()) {
                        ...
                        if (!HwPCManagerService.isDesktopMode(HwPCManagerService.this.mProjMode)) {
                            ...
                            if (HwPCUtils.enabledInPad()) {
                            ...
                            } else if (!(HwPCManagerService.this.get1stDisplay() == null || HwPCManagerService.this.get2ndDisplay() == null)) {
                                HwPCManagerService hwPCManagerService = HwPCManagerService.this;
                                ...
                                HwPCManagerService.this.startProjectionService(false);
                            }
                ...
                case 22:
                    ...
                    return;
                case 23:
                    HwPCManagerService.this.startProjectionService(true);
                    HwPCManagerService.this.sendProjectionCapsuleMsg(0, -1);
                    return;
    ...
    private void startProjectionService(boolean isShow) {
        if (this.mContext != null) {
            HwPCUtils.log(TAG, "startProjectionService start.");
            Intent controlPanelIntent = new Intent();
            controlPanelIntent.putExtra(SHOW_CONTROL_PANEL, isShow);
    ...
    /* JADX WARN: Type inference failed for: r12v0, types: [android.os.Handler, com.huawei.server.pc.HwPCManagerService$LocalHandler] */
    /* JADX WARN: Type inference failed for: r13v2, types: [android.os.Handler, com.huawei.server.pc.HwPCManagerService$LocalHandler] */
    ...
    public HwPCManagerService(Context context, ActivityManagerServiceEx amsEx) {
        ...
        Looper pcLooper = this.mHandlerThread.getLooper();
        this.mHandler = new LocalHandler(pcLooper);
        HwMultiDisplayPowerManager.getDefault().init(context, pcLooper);
        ...
        filter.addAction(ACTION_NOTIFY_START_CONTROL_PANEL_SERVICE);
        filter.addAction(ACTION_VIRTUAL_DISPLAY_CREATED);
    ...
    private void onSwitchUser(int userId) {
        ...
        if (get1stDisplay().mDisplayId != -1) {
            ...
            cancelSwitchNotification("user switched");
        }
    }
    ...
    private void sendNotificationForSwitch(HwPCUtils.ProjectionMode projMode) {
        ...
        if (!HwPCUtils.enabledInPad() && this.mNm != null && !HwPCUtils.isHiCarCastMode() && isDesktopEnabled()) {
            ...
            int iconId = HwPartResourceUtils.getResourceId("ic_notify_cast_control");
            ...
            boolean isConnectFromCastPlus = isConnectFromThirdApp(get1stDisplay().mDisplayId) == 3;
            boolean isSupportPcVirtualDisplay = isConnectFromWelink || isConnectFromCastPlus;
            if (HwPCUtils.getIsWifiMode() || isSupportPcVirtualDisplay) {
                NotificationEx.Builder.setAppName(builder, this.mContext.getString(HwPartResourceUtils.getResourceId("notification_wireless_projection")));
            } else {
            ...
            if (HwPCUtils.getIsWifiMode()) {
            ...
            } else if (isConnectFromCastPlus) {
                this.mWireLessDeviceName = String.format(Locale.ROOT, this.mContext.getResources().getString(HwPartResourceUtils.getResourceId("notification_pc_connected")), this.mVirtualDevicesName);
                ...
                if (!(displayManager2 == null || displayManager2.getDisplay(get1stDisplay().mDisplayId) == null || DisplayEx.getType(this.mDisplayManager.getDisplay(get1stDisplay().mDisplayId)) != 2)) {
                    this.mWireLessDeviceName = String.format(this.mContext.getResources().getString(HwPartResourceUtils.getResourceId("notification_pc_connected")), this.mWireLessDeviceName);
                }
            }
            PendingIntent clickPendingIntent = PendingIntentEx.getBroadcastAsUser(this.mContext, 0, new Intent(ACTION_NOTIFY_START_CONTROL_PANEL_SERVICE), 134217728, UserHandleEx.OWNER);
            ...
            RemoteViews contentView = createContentView(mode, switchModePendingIntent, showMKPendingIntent, disconnectPendingIntent, isSupportPcVirtualDisplay, false);
            RemoteViews bigContentView = createContentView(mode, switchModePendingIntent, showMKPendingIntent, disconnectPendingIntent, isSupportPcVirtualDisplay, true);
            if (isConnectFromThirdApp(get1stDisplay().mDisplayId) == 1) {
                builder.setCustomContentView(contentView);
                builder.setCustomBigContentView(bigContentView);
            } else {
                builder.setContentTitle(this.mWireLessDeviceName);
                builder.setContentText(this.mContext.getString(HwPartResourceUtils.getResourceId("pc_notification_click_more_action")));
            }
            if (HwPCUtils.getIsWifiMode() || isSupportPcVirtualDisplay) {
                builder.addAction(new Notification.Action.Builder(HwPartResourceUtils.getResourceId("ic_notify_cast_control"), this.mContext.getString(HwPartResourceUtils.getResourceId("notification_btn_disconnect")), disconnectPendingIntent).build());
            }
            builder.setContentIntent(clickPendingIntent);
            Notification notification = builder.getNotification();
            notification.flags |= 2;
            notification.flags |= 32;
            if (this.mSwitchNotificationId == 0) {
                this.mSwitchNotificationId = iconId;
            }
            HwPCUtils.log(TAG, "send switch notification id:" + this.mSwitchNotificationId);
            NotificationManagerExt.notifyAsUser(this.mNm, (String) null, this.mSwitchNotificationId, notification, UserHandleEx.ALL);
            this.mHasSwitchNtf = true;
        }
    }

    private void cancelSwitchNotification(String reason) {
        if (this.mNm != null && this.mHasSwitchNtf) {
            HwPCUtils.log(TAG, "cancel switch notification id:" + this.mSwitchNotificationId + " as " + reason);
            this.mHasSwitchNtf = false;
            NotificationManagerExt.cancelAsUser(this.mNm, (String) null, this.mSwitchNotificationId, UserHandleEx.ALL);
        }
    }
            ...
            if (!isInWindowsCastMode() || displayId != HwPCUtils.getWindowsCastDisplayId()) {
                ...
                if (HwPCUtils.getPCDisplayID() != displayId && !isInPhoneMode) {
                    ...
                    try {
                        ...
                        cancelSwitchNotification("display removed");
                        ...
                        if (HwPCUtils.isHiCarCastMode()) {
                            ...
                            broadcastMultiDisplayStateChanged(1, false);
                        }
                ...
                if (HwMultiDisplayUtils.isConnectedToWindows(this.mContext, displayId)) {
                    ...
                    if (isAodDisplay(displayId)) {
                        ...
                        if (HiCarManager.isConnToHiCar(this.mContext, displayId)) {
                            ...
                            this.mHiCarManager = new HiCarManager(this.mHandler, this.mContext, this.mAMS);
                            HwPCUtils.bdReport(this.mContext, 10062, BuildConfig.FLAVOR);
                            removeTaskFromRecent();
                            broadcastMultiDisplayStateChanged(1, true);
                        } else {
                            boolean oldState = HwPCUtils.isHiCarCastMode();
                            HwPCUtils.setIsHiCarMode(false);
                            if (oldState) {
                                broadcastMultiDisplayStateChanged(1, false);
                            }
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void sendStartProjectionServiceMessage() {
    ...
    private boolean checkCallingPermission(int taskId) {
        ...
        HwRecentTaskInfo targetTask = this.mAMS.getHwRecentTaskInfo(taskId);
    ...
    public void exitFullScreen(int taskId) {
        ...
        this.mAMS.hwRestoreTask(taskId, -1.0f, -1.0f);
    }
    ...
    public boolean isPackageRunningOnPCMode(String packageName, int uid) {
        if (checkCallingPermission(PERMISSION_PC_MANAGER_API)) {
            return this.mAMS.isPackageRunningOnPCMode(packageName, uid);
        }
    ...
    public boolean isScreenOnAccurately() {
        return HwMultiDisplayPowerManager.getDefault().isScreenOnAccurately();
    }

    public boolean isScreenPowerOn() {
        if (checkCallingPermission(PERMISSION_PC_MANAGER_API)) {
            return HwMultiDisplayPowerManager.getDefault().isScreenOnFully();
        }
    ...
    public void setScreenPower(boolean powerOn) {
        ...
        HwMultiDisplayPowerManager.getDefault().setScreenPower(powerOn);
    }

    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void lightPhoneScreen() {
        HwMultiDisplayPowerManager.getDefault().setScreenPower(true);
    }
    ...
    public void onTaskMovedToBack(int taskId) {
        ...
        HwPCUtils.log(TAG, "onTaskMovedToBack for taskId=" + taskId);
    ...
    public void onTaskMovedToFront(int taskId) {
        ...
        HwPCUtils.log(TAG, "onTaskMovedToFront for taskId=" + taskId);
    ...
    public void showDialogForSwitchDisplay(int displayId, String pkgName) {
        if (!checkCallingPermission(PERMISSION_PC_MANAGER_API)) {
            HwPCUtils.log(TAG, "showDialogForSwitchDisplay checkCallingPermission failed. " + Binder.getCallingPid());
            return;
        }
        Bundle bundle = new Bundle();
        bundle.putString(PKG_NAME_DATA, pkgName);
    ...
    public void setDockBarInfo(int mode, Bundle info) {
        if (!checkCallingPermission(PERMISSION_PC_MANAGER_API)) {
            HwPCUtils.log(TAG, "setDockBarInfo failed as permission denied.");
        } else if (mode != 1) {
        } else {
            if (HwPCUtils.isHiCarCastMode()) {
                HwHiCarMultiWindowManager.getInstance().setDockBarInfo(info);
            } else {
                HwPCUtils.log(TAG, "setDockBarInfo failed as not in hicar mode currently.");
            }
        }
    }

    private void broadcastMultiDisplayStateChanged(int mode, boolean isEnter) {
        this.mHandler.post(new Runnable(mode, isEnter) {
            /* class com.huawei.server.pc.$$Lambda$HwPCManagerService$W8HXfI7R0em9fVybMMm21IWn2Y */
            ...
            public final void run() {
                HwPCManagerService.this.lambda$broadcastMultiDisplayStateChanged$12$HwPCManagerService(this.f$1, this.f$2);
            }
        });
    }

    public /* synthetic */ void lambda$broadcastMultiDisplayStateChanged$12$HwPCManagerService(int mode, boolean isEnter) {
        HwPCUtils.log(TAG, "multi display state changed, mode:" + mode + ", enter:" + isEnter);
        Intent intent = new Intent(ACTION_MULTIDISPLAY_STATE_CHANGED);
        intent.putExtra("mode", mode);
        intent.putExtra("state", isEnter ? 1 : 0);

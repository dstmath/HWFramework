...
public final class KernelEventReceiver {
    ...
    private boolean callKillControl() {
        ...
        Slog.i(TAG, "handleZswapdUnmet:try to kill too frequently" + lastCallKillTime + AwarenessInnerConstants.DASH_KEY + curTime);
        return true;
    }

    public void handleZswapdUnmet(boolean isFull) {
        KillDecision killDecision2;
        BufferProc.getInstance().notifyCurrentEvent("zswapdFail");
        if (callKillControl() || (killDecision2 = this.killDecision) == null) {
            return;
        }
        if (isFull) {

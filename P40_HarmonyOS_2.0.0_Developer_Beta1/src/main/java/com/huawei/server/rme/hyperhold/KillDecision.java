...
public final class KillDecision {
    ...
    private static final int LEVEL_NORMAL_LOWEND = 3;
    private static final int LEVEL_QUICK_LOWEND = 4;
    ...
    private AdvancedKiller advancedKiller = null;
    ...
    private int lowEndNormalKill = 0;
    private int lowEndQuickKillEnable = 0;
    ...
    public void init(Context context2) {
        ...
        this.advancedKiller = new AdvancedKiller(context2);
        ...
        this.lowEndNormalKill = ParaConfig.getInstance().getKillParamOpt().getLowEndNormalKill();
        this.lowEndQuickKillEnable = ParaConfig.getInstance().getKillParamOpt().getLowEndQuickKillEnable();
        this.ubSortWeightLimitNormal = ParaConfig.getInstance().getKillParamOpt().getUbSortWeightLimitNormal();
        this.ubSortWeightLimitQuick = ParaConfig.getInstance().getKillParamOpt().getUbSortWeightLimitQuick();
        this.killWeightLimitNormal = ParaConfig.getInstance().getKillParamOpt().getKillWeightLimitNormal();
        this.killWeightLimitQuick = ParaConfig.getInstance().getKillParamOpt().getKillWeightLimitQuick();
    ...
    public void updateModel(String appName) {
        AdvancedKiller advancedKiller2 = this.advancedKiller;
        if (advancedKiller2 != null) {
            advancedKiller2.updateModel(appName);
        } else {
    ...
    public void serializeModel() {
        AdvancedKiller advancedKiller2 = this.advancedKiller;
        if (advancedKiller2 != null) {
            advancedKiller2.serializeModel();
        } else {
            Slog.e(TAG, "Try to serialize model before init of advancedKiller");
        }
    }

    public void notifyAkBg(String appName) {
        AdvancedKiller advancedKiller2 = this.advancedKiller;
        if (advancedKiller2 != null) {
            advancedKiller2.notifyAkBg(appName);
        }
    }
    ...
    public void notifyScreenOn() {
        AdvancedKiller advancedKiller2 = this.advancedKiller;
        if (advancedKiller2 != null) {
            advancedKiller2.notifyScreenOn();
        }
    }
    ...
    private void doQuickKill(int tempQuickKillThreshold) {
        ...
        if (this.lowEndQuickKillEnable != 0) {
            Slog.i(TAG, "BigMemory kill lowEnd");
            killByAk((long) (tempKillThreshold - freeBuffer), true, 4);
            return;
        }
    ...
    private int killByAk(long reqMem, boolean isQuick) {
        return killByAk(reqMem, isQuick, 0);
    }

    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private int killByAk(long reqMem, boolean isQuick, int level) {
        Bundle bundle = new Bundle();
        bundle.putLong("reqMem", reqMem);
        if (level == 3) {
            bundle.putSerializable("listSettings", new AdvancedKiller.AdvancedKillerListSettings(Arrays.asList(AdvancedKillerAwareUtils.MemoryLevelListType.LEVEL_4, AdvancedKillerAwareUtils.MemoryLevelListType.LEVEL_CACHED_PROCESS), this.ubSortWeightLimitNormal, this.killWeightLimitNormal));
            Slog.i(TAG, "LEVEL_NORMAL_LOWEND: reqMem:" + reqMem + " u:" + this.ubSortWeightLimitNormal + " k:" + this.killWeightLimitNormal);
        }
        if (level == 4) {
            bundle.putSerializable("listSettings", new AdvancedKiller.AdvancedKillerListSettings(Arrays.asList(AdvancedKillerAwareUtils.MemoryLevelListType.LEVEL_5, AdvancedKillerAwareUtils.MemoryLevelListType.LEVEL_CACHED_PROCESS), this.ubSortWeightLimitQuick, this.killWeightLimitQuick));
            Slog.i(TAG, "LEVEL_QUICK_LOWEND: reqMem:" + reqMem + " u:" + this.ubSortWeightLimitQuick + " k:" + this.killWeightLimitQuick);
        }
        if (isQuick) {
    ...
    public class KillExecuteThread implements Runnable {
        ...
        public void run() {
            ...
            int reqMem2 = 0;
            if (KillDecision.this.lowEndNormalKill != 0) {
                reqMem2 = KillDecision.this.needKillByBuffer(3);
            }
            if (reqMem <= 0 && reqMem2 <= 0) {
                ...
                KillDecision.this.killByAk((long) reqMem2, false, 3);
                Process.setThreadPriority(tid, beforePriority);
            } else {
                if (reqMem > 0) {
    ...
    private int needKillByBuffer(int level) {
        ...
        if (level == 1) {
            ...
            tempKillThreshold = this.lowEndNormalKill;
        } else {

...
public class HwAddViewManager {
    ...
    public boolean addViewPermissionCheck(String packageName, int type, int uid) {
        if (packageName == null) {
            SlogEx.e(TAG, "packageName is null, deny.");
            return false;
        } else if (uid == 0) {
            SlogEx.i(TAG, "uid is root, allow.");
            return true;
        } else {
            synchronized (serviceLock) {
                ...
                if (this.addviewMapWithUser.size() != 0) {
                    if (this.addviewMapWithUser.get(Integer.valueOf(userId)) != null) {
                        if (((AppOpsManager) this.mContext.getSystemService("appops")).checkOpNoThrow("android:system_alert_window", uid, packageName) == 0) {
                            SlogEx.d(TAG, packageName + " allow due to the explicit setting in AppOps");
                            return true;
                        }
                        ...
                        if (currentUserPermissionMap == null || !currentUserPermissionMap.containsKey(packageName)) {
                            SlogEx.d(TAG, packageName + " not appear in deny list, allow.");
                            return true;
                        }
                        boolean isAllow = getPermissionResult(type, currentUserPermissionMap.get(packageName).intValue());
                ...
                SlogEx.i(TAG, packageName + " allow for uid " + uid + " due to deny list not ready");
                return true;
            }
        }
    }
    ...
    private boolean getPermissionResult(int compareValue, int sourceValue) {
        return sourceValue == 0 || sourceValue == 1 || (compareValue & sourceValue) == 0;
    }

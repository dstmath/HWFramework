...
public class HwPermDbHelper extends SQLiteOpenHelper {
    ...
    private void createPermissionCfgTable(@NonNull SQLiteDatabase db, int userId) {
        ...
        db.execSQL("create table if not exists " + tableName + " ( " + PermConst.PACKAGE_NAME + " text primary key, " + PermConst.APP_UID + " int DEFAULT (-1), " + PermConst.PERMISSION_CODE + " bigint DEFAULT (0), " + PermConst.PERMISSION_CFG + " bigint DEFAULT (0));");
    ...
    /* access modifiers changed from: package-private */
    public void removeAndUpdateAll(final String tableName, final String infoType, final List<ContentValues> valueList) {
        if (TextUtils.isEmpty(tableName) || TextUtils.isEmpty(infoType) || valueList == null) {
            ...
            this.mPermissionSingleExecutor.submit(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        if (tableName.equals(PermConst.ACTIVE_ONE_TIME_PERM_TABLE_NAME) || tableName.equals(PermConst.ONE_TIME_PERM_TABLE_NAME)) {
                            if (!HwPermDbHelper.this.dropAndCreateOneTimePermDb(HwPermDbHelper.this.mDatabase, tableName, infoType)) {
                                if (HwPermDbHelper.this.mDatabase != null) {
                                    HwPermDbHelper.this.mDatabase.endTransaction();
                                    return;
                                }
                        ...
                        } else if (tableName.equals(PermConst.RECOMMEND_PERMISSION_TABLE_NAME)) {
                            HwPermDbHelper.this.dropAndCreateRecommendPermDb(HwPermDbHelper.this.mDatabase);
                        } else {
                            SlogEx.e(HwPermDbHelper.LOG_TAG, "create table with invalid table name.");
                            if (HwPermDbHelper.this.mDatabase != null) {
                                HwPermDbHelper.this.mDatabase.endTransaction();
                                return;
                            }
                        ...
                        for (ContentValues values : valueList) {
                            HwPermDbHelper.this.mDatabase.insert(tableName, null, values);
                        }
                        ...
                        SlogEx.e(HwPermDbHelper.LOG_TAG, "removeAndUpdateAll error.");
                        if (HwPermDbHelper.this.mDatabase == null) {
                        }
                    } catch (Exception e2) {
                        SlogEx.e(HwPermDbHelper.LOG_TAG, "removeAndUpdateAll get unexpected exception.");
                        if (HwPermDbHelper.this.mDatabase == null) {
                        }
                    } catch (Throwable th) {
                        if (HwPermDbHelper.this.mDatabase != null) {
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private boolean dropAndCreateOneTimePermDb(@NonNull SQLiteDatabase db, @NonNull String tableName, @NonNull String infoType) {
        String columnName;
        if (PermConst.ONE_TIME_PERM_TYPE_SUPPORT.equals(infoType)) {
            ...
            SlogEx.e(LOG_TAG, "get invalid type, update table fail.");
            return false;
        }
        db.execSQL("DROP TABLE IF EXISTS " + tableName);
        db.execSQL("create table if not exists " + tableName + " ( " + columnName + " text primary key);");
        return true;
    }

    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void dropAndCreateRecommendPermDb(@NonNull SQLiteDatabase db) {
        db.execSQL("DROP TABLE IF EXISTS permissionOuterTable");
        db.execSQL("create table if not exists permissionOuterTable ( packageName text primary key, permissionCode bigint  DEFAULT (0), permissionCfg bigint  DEFAULT (0) );");
    }
}

...
public abstract class ResultParser {
    ...
    protected static boolean isSubstringOfDigits(CharSequence value, int offset, int length) {
        int max;
        if (value != null && value.length() >= (max = offset + length) && DIGITS.matcher(value.subSequence(offset, max)).matches()) {
            return true;
        }
        return false;
    }
    ...
    protected static boolean isSubstringOfAlphaNumeric(CharSequence value, int offset, int length) {
        int max;
        if (value != null && value.length() >= (max = offset + length) && ALPHANUM.matcher(value.subSequence(offset, max)).matches()) {
            return true;
        }
        return false;
    }

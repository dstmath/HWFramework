    ...
    private static int byteCompaction(int mode, int[] codewords, int codeIndex, StringBuilder result) {
        ...
        if (mode == 901) {
            ...
            while (count3 < codewords[0] && !end2) {
                ...
                if (nextCode == 900 || nextCode == 901 || nextCode == 902 || nextCode == BYTE_COMPACTION_MODE_LATCH_6 || nextCode == i2 || nextCode == i || nextCode == MACRO_PDF417_TERMINATOR) {
                    count3 = codeIndex2 - 1;
                    end2 = true;
                    count2 = count4;

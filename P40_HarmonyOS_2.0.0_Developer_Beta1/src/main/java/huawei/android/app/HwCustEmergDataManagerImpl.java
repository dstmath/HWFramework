...
public class HwCustEmergDataManagerImpl extends HwCustEmergDataManager {
    ...
    private static final boolean IS_HWFLOW;
    ...
    private static boolean isLocalLogV;

    static {
        boolean z = Log.HWINFO;
        boolean z2 = IS_HWFLOW;
        IS_HWFLOW = z || (Log.HWModuleLog && Log.isLoggable(TAG, 4));
    ...
    private static class FileBackup {
        ...
        static {
            boolean z = Log.HWINFO;
            boolean z2 = IS_HWFLOW;
            IS_HWFLOW = z || (Log.HWModuleLog && Log.isLoggable(TAG, ONE_INT_LENGTH));
            if (Log.HWLog || (Log.HWModuleLog && Log.isLoggable(TAG, 3))) {
                z2 = true;
            }
            isLocalLogV = z2;
        ...
        private int writeCommand(String command, byte[] buf) {
            try {
                byte[] cmds = command.getBytes("UTF-8");
                int len = cmds.length;
        ...
        private boolean copy(String srcPath, String dstPath) {
            byte[] copyBufs = new byte[MAX_BUF_LENGTH];
            writeCommand(COPY + srcPath + " " + dstPath, copyBufs);
            byte[] returnValues = new byte[ONE_INT_LENGTH];
            System.arraycopy(copyBufs, 0, returnValues, 0, ONE_INT_LENGTH);
            if (bytesToInt(returnValues) != 0) {

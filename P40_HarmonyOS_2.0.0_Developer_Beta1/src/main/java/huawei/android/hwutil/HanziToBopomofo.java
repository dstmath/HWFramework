...
public class HanziToBopomofo {
    ...
    private TransliteratorEx mAsciiTransliterator;
    private TransliteratorEx mZhuyinTransliterator;
    ...
    private HanziToBopomofo() {
        try {
            this.mZhuyinTransliterator = TransliteratorEx.getInstance("Han-Latin/Names; Latin-Ascii; Latin-Bopomofo");
            this.mAsciiTransliterator = TransliteratorEx.getInstance("Latin-Ascii");
    ...
    private void tokenize(char character, Token token) {
        ...
        if (character < 128) {
            ...
            token.type = 1;
            TransliteratorEx transliteratorEx = this.mAsciiTransliterator;
            if (transliteratorEx == null) {
                str = token.source;
            } else {
                str = transliteratorEx.transliterate(token.source);
            }
            token.target = str;
        } else {
            token.type = 2;

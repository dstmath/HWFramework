...
public class HwSearchAnimation {
    ...
    @TargetApi(DynamicAnimation.ANDROID_LOLLIPOP)
    ...
    private ViewGroup bringToRootFront(View view, ViewGroup hostView) {
        ...
        int[] hostViewLocations = new int[2];
        parentView.getLocationOnScreen(parentLocations);
        hostView.getLocationOnScreen(hostViewLocations);
        parentLocations[1] = hostViewLocations[1] + this.mAppbar.getHeight() + this.mAppbar.getTop();
        view.offsetLeftAndRight(parentLocations[0] - hostViewLocations[0]);
        view.offsetTopAndBottom(parentLocations[1] - hostViewLocations[1]);
    ...
    private void backToOldViewParent(View view, ViewGroup hostView) {
        ...
        if (hostView == null || parent == null) {
            Log.e(TAG, "backToOldViewParent: mSearchHostBack is null.");
        } else if (parent != hostView) {
            int[] parentLocations = new int[2];
            int[] hostViewLocations = new int[2];
            parent.getLocationOnScreen(parentLocations);
            hostView.getLocationOnScreen(hostViewLocations);
            hostViewLocations[1] = parentLocations[1] + this.mAppbar.getHeight() + this.mAppbar.getTop();
            view.offsetLeftAndRight(parentLocations[0] - hostViewLocations[0]);
            view.offsetTopAndBottom(parentLocations[1] - hostViewLocations[1]);
    ...
    public void scrollListWithSearchView(float oldTranslationY, float newTranslationY, float fraction, boolean isOpen) {
        ...
        if (view != null && !isActivityInactive(view)) {
            ...
            if (Float.compare(fraction, 0.0f) == 0) {

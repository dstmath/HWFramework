...
public class CounterTextLayout extends LinearLayout {
    ...
    public CounterTextLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (theme != null) {
            TypedArray typedArray = theme.obtainStyledAttributes(attrs, resLoader.getIdentifierArray(context, ResLoaderUtil.STAYLEABLE, "CounterTextLayout"), resLoader.getIdentifier(context, "attr", "counterTextLayoutStyle"), resLoader.getIdentifier(context, "style", "Widget.Emui.CounterTextLayout"));
            this.mCounterTextAppearance = typedArray.getResourceId(ResLoaderUtil.getStyleableId(context, "CounterTextLayout_counterTextAppearance"), 0);
            this.mIsSpaceOccupied = typedArray.getBoolean(ResLoaderUtil.getStyleableId(context, "CounterTextLayout_spaceOccupied"), false);
            this.mShapeMode = ShapeMode.getShapeMode(typedArray.getInt(ResLoaderUtil.getStyleableId(context, "CounterTextLayout_shape_mode"), ShapeMode.Bubble.getModeValue()));
            this.mLinearEditBgResId = typedArray.getResourceId(ResLoaderUtil.getStyleableId(context, "CounterTextLayout_linearEditBg"), 0);
            this.mSpaceEditBgResId = typedArray.getResourceId(ResLoaderUtil.getStyleableId(context, "CounterTextLayout_spaceEditBg"), 0);
            this.mCounterResBgId = typedArray.getResourceId(ResLoaderUtil.getStyleableId(context, "CounterTextLayout_counterResBg"), 0);
            this.mErrorResBgId = typedArray.getResourceId(ResLoaderUtil.getStyleableId(context, "CounterTextLayout_errorResBg"), 0);
            this.mErrorResBgWhiteId = typedArray.getResourceId(ResLoaderUtil.getStyleableId(context, "CounterTextLayout_errorResBgWhite"), 0);
            this.mEditTextBgResId = typedArray.getResourceId(ResLoaderUtil.getStyleableId(context, "CounterTextLayout_editTextBg"), 0);
            this.mEditTextBgWhiteResId = typedArray.getResourceId(ResLoaderUtil.getStyleableId(context, "CounterTextLayout_editTextBgWhite"), 0);
            this.mErrorLinearEditBgResId = typedArray.getResourceId(ResLoaderUtil.getStyleableId(context, "CounterTextLayout_errorLiearEditBg"), 0);
            typedArray.recycle();
        }
        setupCounterView();
    ...
    public void addView(View child, int index, ViewGroup.LayoutParams params) {
        if (child instanceof EditText) {
            ...
            if (this.mShapeMode == ShapeMode.Linear || this.mShapeMode == ShapeMode.Space) {
    ...
    private void setEditText(EditText editText) {
        if (this.mEditText == null) {
            ...
            } else if (this.mShapeMode == ShapeMode.White) {
                this.mEditText.setBackgroundResource(this.mEditTextBgWhiteResId);
            } else if (this.mShapeMode == ShapeMode.Space) {
    ...
    public void onLayout(boolean isChanged, int left, int top, int right, int bottom) {
        ...
        if ((this.mShapeMode == ShapeMode.Bubble || this.mShapeMode == ShapeMode.White) && (editText = this.mEditText) != null) {
            ViewGroup.LayoutParams layoutParams = editText.getLayoutParams();
            if (layoutParams instanceof LinearLayout.LayoutParams) {
                LinearLayout.LayoutParams params = (LinearLayout.LayoutParams) layoutParams;
                int newBottom = ((((this.mEditText.getTop() + getHeight()) - getPaddingBottom()) - getPaddingTop()) - params.bottomMargin) - params.topMargin;
                EditText editText2 = this.mEditText;
                editText2.layout(editText2.getLeft(), this.mEditText.getTop(), this.mEditText.getRight(), newBottom);
            }
        ...
        if ((this.mShapeMode == ShapeMode.Linear || this.mShapeMode == ShapeMode.Space) && this.mEditText != null) {
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void updateTextLayoutBackground(int bubbleBackgroundId, int bubbleBackgroundWhiteId, int linearBackgroundId, int linearSpaceBackgroundId) {
        if (this.mEditText != null) {
            if (this.mShapeMode == ShapeMode.Bubble) {
                this.mEditText.setBackgroundResource(bubbleBackgroundId);
            } else if (this.mShapeMode == ShapeMode.Linear) {
                this.mEditText.setBackgroundResource(linearBackgroundId);
            } else if (this.mShapeMode == ShapeMode.White) {
                this.mEditText.setBackgroundResource(bubbleBackgroundWhiteId);
            } else if (this.mShapeMode == ShapeMode.Space) {
                this.mEditText.setBackgroundResource(linearSpaceBackgroundId);
            }
        }
    }
        ...
        private ChangeWatcher() {
            ...
            this.mShake.setAnimationListener(new Animation.AnimationListener(CounterTextLayout.this) {
                ...
                public void onAnimationStart(Animation animation) {
                    ...
                    CounterTextLayout.this.updateTextLayoutBackground(CounterTextLayout.this.mErrorResBgId, CounterTextLayout.this.mErrorResBgWhiteId, CounterTextLayout.this.mErrorLinearEditBgResId, CounterTextLayout.this.mErrorLinearEditBgResId);
                }
                ...
                public void onAnimationEnd(Animation animation) {
                    CounterTextLayout.this.updateTextLayoutBackground(CounterTextLayout.this.mEditTextBgResId, CounterTextLayout.this.mEditTextBgWhiteResId, CounterTextLayout.this.mLinearEditBgResId, CounterTextLayout.this.mSpaceEditBgResId);
    ...
    private class TextInputAccessibilityDelegate extends View.AccessibilityDelegate {
        ...
        public void onInitializeAccessibilityNodeInfo(View host, AccessibilityNodeInfo info) {
            if (info != null) {
                ...
                CharSequence error = CounterTextLayout.this.mCounterView.getText();
                if (!TextUtils.isEmpty(error)) {

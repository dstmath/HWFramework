...
public class HwAlphaIndexResourceManager {
    ...
    private HwAlphaIndexResourceManager(Locale locale) {
        ...
        if ("zh_Hant".equals(validLocale)) {
            ...
            String lang = locale.getDisplayLanguage(Locale.ENGLISH).split(" ")[0].toUpperCase(Locale.ENGLISH);
    ...
    public static List<String> populateBulletAlphaIndex(int maxSize, List<String> alphaList) {
        ...
        while (bulletSize > 0) {
            ...
            int index = 0;
            while (index < bulletCount + 1 && pos < size) {
        ...
        if (pos < size) {
            return combineBullet(minBulletSize, bulletCount, pos, alphaList, bulletAlphaIndexList);
        }
        Log.w(TAG, "populateBulletAlphaIndex: pos(3) is invalid");
        return bulletAlphaIndexList;
    }

    private static List<String> combineBullet(int minSize, int bulletCount, int position, List<String> alphaList, List<String> bulletAlphaIndexList) {
        int size = alphaList.size();
        int minBulletSize = minSize;
        int pos = position;
        while (minBulletSize > 0) {
            minBulletSize--;
            StringBuilder bulletContent = new StringBuilder((int) INITIAL_CAPACITY);
            int index = 0;
            while (index < bulletCount && pos < size) {
                bulletContent.append(alphaList.get(pos));
                bulletContent.append(' ');
                index++;
                pos++;
        ...
        for (int index2 = pos; index2 < size; index2++) {
            bulletAlphaIndexList.add(alphaList.get(index2));
        }
        return bulletAlphaIndexList;
    }

...
public class HwQuickIndexController {
    ...
    private AlphaIndexerListView.OnItemClickListener mOnItemClickListener = new AlphaIndexerListView.OnItemClickListener() {
        ...
        public void onItemClick(String str, int pos) {
            if (str != null) {
                Object[] objects = HwQuickIndexController.this.mDataAdapter.getSections();
                if (objects instanceof String[]) {
                    String[] sections = (String[]) objects;
                    int sectionPos = pos;
                    ...
                    while (true) {
                        if (index >= sections.length) {
                            break;
                        } else if (HwQuickIndexController.this.mAlphaView.equalsChar(str, sections[index], pos)) {
                    ...
                    if (sectionText != null) {
                        ...
                        if (position + countOfScreen > HwQuickIndexController.this.mListView.getCount()) {
                            Object object = HwQuickIndexController.this.mDataAdapter.getSectionNameForPosition(HwQuickIndexController.this.mListView.getCount() - countOfScreen);
                            sectionText = object instanceof String ? (String) object : sectionText;
                        }
    ...
    public HwQuickIndexController(ListView listview, AlphaIndexerListView alphaView) {
        ...
        if (adapter instanceof HwSortedTextListAdapter) {
            this.mDataAdapter = (HwSortedTextListAdapter) adapter;
    ...
    private String getCurrentSection(int sectionPos) {
        ...
        Object object = this.mDataAdapter.getSections()[sectionPos];

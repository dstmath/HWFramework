...
public class HwToolBarMenuContainer extends ActionBarContainer implements HwCornerInsetsInterface {
    ...
    public HwToolBarMenuContainer(Context context, AttributeSet attrs) {
        ...
        if (theme != null) {
            TypedArray typedArray = theme.obtainStyledAttributes(this.mAttrs, resLoader.getIdentifierArray(getContext(), "styleable", "HwToolbar"), 16843946, 0);
            HwBlurEngine.BlurType blurType = HwBlurEngine.BlurType.fromTypeValue(typedArray.getInteger(resLoader.getIdentifier(this.mContext, "styleable", "HwToolbar_hwToolbarSplitBlurType"), DEFAULT_BLUR_TYPE));
            if (blurType != null) {
                this.mBlurType = blurType;
            }
            this.mBlurOverlayColor = typedArray.getColor(resLoader.getIdentifier(this.mContext, "styleable", "HwToolbar_hwToolbarSplitBlurOverlayColor"), INVALID_BLUR_OVERLAY_COLOR);
            typedArray.recycle();
        }
    }
    ...
    private void initBackgroundResource() {
        ...
        if (theme != null) {
            TypedArray typedArray = theme.obtainStyledAttributes(this.mAttrs, resLoader.getIdentifierArray(getContext(), "styleable", "HwToolbar"), 16843946, 0);
            int backgroundId = typedArray.getResourceId(resLoader.getIdentifier(getContext(), "styleable", "HwToolbar_hwToolbarSplitBarBackground"), -1);
            if (backgroundId != -1) {
                setBackgroundResource(backgroundId);
            }
            int menuDividerId = typedArray.getResourceId(resLoader.getIdentifier(getContext(), "styleable", "HwToolbar_hwToolbarMenuDivider"), -1);
            if (menuDividerId != -1) {

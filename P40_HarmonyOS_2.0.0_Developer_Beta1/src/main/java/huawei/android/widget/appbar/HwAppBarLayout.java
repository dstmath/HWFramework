...
public class HwAppBarLayout extends LinearLayout {
    ...
    public HwAppBarLayout(Context context, AttributeSet attrs) {
        ...
        if (theme != null) {
            TypedArray array = theme.obtainStyledAttributes(attrs, R.styleable.HwAppBarLayout, 34668579, 33947776);
    ...
    public void setDefaultHeight(int defaultHeight) {
        if (defaultHeight >= 0) {
            this.mExpandedAppBarDefaultHeight = defaultHeight;
        }
        ViewGroup.LayoutParams params = getLayoutParams();
        if (params != null) {
            params.height = this.mExpandedAppBarDefaultHeight;
            setLayoutParams(params);
        }
    }
    ...
    private int dip2px(float dpValue) {
        return (int) ((dpValue * getResources().getDisplayMetrics().density) + ROUND_UP_DOWN_PLUS);
    }

...
public class HwBlurEngine {
    ...
    private static BlurAlgorithm sAlgorithm = new BlurAlgorithm((Context) null, 2, true);
    private static HwBlurEngine sBlurEngine = new HwBlurEngine();
    ...
    public static HwBlurEngine getInstance(View targetView, BlurType blurType) {
        if (DEBUG) {
            Log.d(TAG, "getInstance: this function deprecated!!!");
        }
        sBlurEngine.addBlurTargetView(targetView, blurType);
        return sBlurEngine;
    }
    ...
    public void setTargetViewBlurEnable(View targetView, boolean isBlurEnable) {
        if (targetView == null) {
            ...
            String str2 = TAG;
            Log.d(str2, "setBlurEnable: target view is not contained in blur engine. view: " + targetView + ", enable: " + isBlurEnable);
        }
    }
    ...
    public void setTargetViewCornerRadius(View targetView, int cornerRadius) {
        if (targetView == null) {
            ...
            Log.d(TAG, "setCornerRadius: target view is not contained in blur engine.");
        }
    }

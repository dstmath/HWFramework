...
public class VerticalImageSpan extends ImageSpan {
    ...
    @Override // android.text.style.DynamicDrawableSpan, android.text.style.ReplacementSpan
    public void draw(Canvas canvas, CharSequence text, int start, int end, float dx, int top, int dy, int bottom, Paint paint) {
        Drawable drawable = getDrawable();
        if (drawable != null) {
            canvas.save();
            canvas.translate(dx, (float) ((((bottom - top) - drawable.getBounds().bottom) / 2) + top));

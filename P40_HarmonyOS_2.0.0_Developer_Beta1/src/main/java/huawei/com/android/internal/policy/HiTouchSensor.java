...
public class HiTouchSensor {
    ...
    public boolean getStatus() {
        ...
        if (this.mIsLandscapeOrient) {
            Log.i(TAG, "mIsLandscapeOrient: " + this.mIsLandscapeOrient);
            return false;
        }
        updateSwitchStatus();
        if (!this.mIsHiTouchEnabled) {
            Log.i(TAG, "mIsHiTouchEnabled: " + this.mIsHiTouchEnabled);
            return false;
        }
        boolean isGameMode = ActivityManagerEx.isInGameSpace(this.mContext.getPackageName());
        if (isGameMode) {
            Log.i(TAG, "mIsGameMode: " + isGameMode);
            return false;
        }
        boolean isSecurityImePopup = isSecurityImePopup();
        if (isSecurityImePopup) {
            Log.i(TAG, "isSecurityImePopup: " + isSecurityImePopup);
            return false;
        }
        updateAccessibilityStatus();
        if (this.mIsAccessibilityEnabled) {
            Log.i(TAG, "mIsAccessibilityEnabled: " + this.mIsAccessibilityEnabled);
            return false;
        }
        Context context = this.mContextActivity;
        if (context instanceof Activity) {
            this.mIsInMultiWindowMode = ((Activity) context).isInMultiWindowMode();
            if (this.mIsInMultiWindowMode) {
                Log.i(TAG, "10.X mIsInMultiWindowMode: " + this.mIsInMultiWindowMode);
                return false;
            }
        }
        this.mIsTouchEffectiveHiAction = checkEffectiveStatusFromHiAction(this.mContextActivity.getPackageName());
        if (!this.mIsTouchEffectiveHiAction) {
            Log.i(TAG, "mIsTouchEffectiveHiAction: " + this.mIsTouchEffectiveHiAction);
            return false;
        }
        Log.i(TAG, "mIsHiTouchEnabled: " + this.mIsHiTouchEnabled + " isSecurityImePopup: " + isSecurityImePopup + " mIsLandscapeOrient: " + this.mIsLandscapeOrient + " mIsInMultiWindowMode: " + this.mIsInMultiWindowMode + " mIsTouchEffectiveHiAction: " + this.mIsTouchEffectiveHiAction + " isGameMode: " + isGameMode);
        return true;
    }

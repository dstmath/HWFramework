...
public class HwEditor extends Editor {
    ...
    private static Class<?> sClazzViewEx;
    private static int sCursorMoveVibrateInt;
    private static String sCursorMoveVibrateType;
    private static Class<?> sHapticFeedbackConstantsEx;
    private static boolean sIsSupportCursorMoveVibrator;
    private static boolean sIsSupportLongClickVibrator;
    private static int sLongClickVibrateInt;
    private static String sLongClickVibrateType;
    private static Method sPerformHwHapticFeedback;
    ...
    static {
        try {
            sClazzViewEx = Class.forName("com.huawei.android.view.ViewEx");
            sPerformHwHapticFeedback = sClazzViewEx.getMethod("performHwHapticFeedback", View.class, Integer.TYPE, Integer.TYPE);
            sHapticFeedbackConstantsEx = Class.forName("com.huawei.android.view.HapticFeedbackConstantsEx");
            Class<?> clazz = Class.forName("com.huawei.android.os.VibratorEx");
            sVibratorEx = clazz.newInstance();
            Object editString = clazz.getDeclaredField("HW_VIBRATOR_TYPE_COMMON_LONG_PRESS1").get(null);
            if (editString instanceof String) {
                sLongClickVibrateType = (String) editString;
            }
            Object moveString = clazz.getDeclaredField("HW_VIBRATOR_TYPE_SLIDE_TYPE6").get(null);
            if (moveString instanceof String) {
                sCursorMoveVibrateType = (String) moveString;
            }
            Method isSupportHwVibrator = clazz.getDeclaredMethod("isSupportHwVibrator", String.class);
            Object longClickFlag = isSupportHwVibrator.invoke(sVibratorEx, sLongClickVibrateType);
            if (longClickFlag instanceof Boolean) {
                sIsSupportLongClickVibrator = ((Boolean) longClickFlag).booleanValue();
            }
            Object cursorMoveFlag = isSupportHwVibrator.invoke(sVibratorEx, sCursorMoveVibrateType);
            if (cursorMoveFlag instanceof Boolean) {
                sIsSupportCursorMoveVibrator = ((Boolean) cursorMoveFlag).booleanValue();
            }
            Object editNums = sHapticFeedbackConstantsEx.getDeclaredField("HW_LONG_PRESS1").get(null);
            if (editNums instanceof Integer) {
                sLongClickVibrateInt = ((Integer) editNums).intValue();
            }
            Object moveNums = sHapticFeedbackConstantsEx.getDeclaredField("HW_SLIDE_6").get(null);
            if (moveNums instanceof Integer) {
                sCursorMoveVibrateInt = ((Integer) moveNums).intValue();
            }
        } catch (ClassNotFoundException e) {
            Log.e(TAG, "class init failed.");
            ...
            Log.e(TAG, "Invoke method isSupport error");
        } catch (NoSuchFieldException e6) {
            Log.e(TAG, "Not found field");
        }
    }
    ...
    public void recogniseLineEnd() {
        ...
        if (this.mTouchPosX != -1.0f && layout != null) {
            ...
            if (offset == layout.getLineStart(currentLine) && currentLine > 0) {
                ...
                float end = (float) (((int) layout.getLineMax(currentLine - 1)) + this.mTextView.getCompoundPaddingLeft());
                if (Float.compare(end, start) >= 0) {
                    this.mIsPosLineEnd = this.mTouchPosX >= ((end - start) * WORD_END_SCALE) + start;
                }
            }
    ...
    private void vibrate(boolean isMoveType) {
        int type = isMoveType ? sCursorMoveVibrateInt : sLongClickVibrateInt;
        if (isVibratorSwitchOn()) {
            try {
                if (sPerformHwHapticFeedback != null) {
                    sPerformHwHapticFeedback.invoke(null, this.mTextView, Integer.valueOf(type), 0);
                }
    ...
    private boolean isCursorHandleVisible() {
        try {
            ...
            Object isActiveTemp = isActive.invoke(insertionCursorController, new Object[0]);
            Object isCursorBeingModifiedTemp = isCursorBeingModified.invoke(insertionCursorController, new Object[0]);
            if (!(isActiveTemp instanceof Boolean) || !(isCursorBeingModifiedTemp instanceof Boolean) || !((Boolean) isActiveTemp).booleanValue() || !((Boolean) isCursorBeingModifiedTemp).booleanValue()) {
                return false;
            }
            return true;

...
public class HwFragmentMenuItemView extends TextView {
    ...
    public void setIcon(Drawable icon) {
        if (icon != null) {
            int rawWidth = icon.getIntrinsicWidth();
            int rawHeight = icon.getIntrinsicHeight();
            float widthscale = 1.0f;
            float heightscale = 1.0f;
            int i = this.mMaxIconSize;
            if (rawWidth > i) {
                widthscale = ((float) i) / ((float) rawWidth);
            }
            int i2 = this.mMaxIconSize;
            if (rawHeight > i2) {
                heightscale = ((float) i2) / ((float) rawHeight);
            }
            float scale = widthscale < heightscale ? widthscale : heightscale;
            icon.setBounds(0, 0, (int) (((float) rawWidth) * scale), (int) (((float) rawHeight) * scale));

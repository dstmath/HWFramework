...
public class HwGlobalActionsData {
    ...
    private HwGlobalActionsData() {
        if (IS_DEBUG) {
            Log.d(TAG, "creating HwGlobalActionsData");
        }
    }
    ...
    public void setSilentMode(int state) {
        ...
        if (IS_DEBUG) {
            Log.d(TAG, "setSilentMode-mState=0x" + Integer.toHexString(this.mState));
        }
        this.mState &= MASK_FLAG_SILENT;
        this.mState |= state;
        if (IS_DEBUG) {
            Log.d(TAG, "setSilentMode_after mask_mState=0x" + Integer.toHexString(this.mState));
        }
        if ((prevState & state) == 0 && this.mObserver != null) {
            if (IS_DEBUG) {
                Log.d(TAG, "setSilentMode--onSilentModeActionStateChanged");
            }
    ...
    public void setKeyCombinationMode(int state) {
        ...
        if (this.mObserver != null) {
            if (IS_DEBUG) {
                Log.d(TAG, "setKeyCombinationMode--onKeyCombinationActionStateChanged");
            }
            this.mObserver.onKeyCombinationActionStateChanged();
        }
    }
    ...
    public void init(boolean isKeyguardShowing, boolean isKeyguardSecure, boolean isDeviceProvisioned) {

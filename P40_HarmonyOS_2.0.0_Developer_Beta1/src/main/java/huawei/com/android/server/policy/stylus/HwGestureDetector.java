...
public class HwGestureDetector {
    ...
    private float mMinNonEnclosedRegionDiag = NON_ENCLOSED_REGION_DIAGONAL_THRESHOLD;
    private float mMinPredictionScore = MIN_PREDICTION_SCORE;
    ...
    public class GestureState {
        ...
        private final ArrayList<GesturePoint> mStrokeBufferList = new ArrayList<>((int) INITIAL_CAPACITY);
        ...
        private void touchMove(MotionEvent event) {
            if (this.mIsListeningGestures) {
                ...
                if (dx >= HwGestureDetector.MOVE_TOLERANCE || dy >= HwGestureDetector.MOVE_TOLERANCE) {
                    this.mLastX = eventX;
                    this.mLastY = eventY;
                    this.mStrokeBufferList.add(new GesturePoint(eventX, eventY, event.getEventTime()));
    ...
    private float calcMoveDistance(Gesture gesture) {
        ...
        if (points.length < MIN_POINT_LENGTH) {
            return 0.0f;
        }
    ...
    private boolean notifyIfLetterGesture(GestureState gestureState) {
        ...
        Gesture gesture = gestureState.getGesture();
        ArrayList<Prediction> multiOrientPredictions = null;
        ArrayList<Prediction> singleOrientPredictions = null;
        try {
            multiOrientPredictions = this.mMultiOrientGestureLib.recognize(gesture);
            singleOrientPredictions = this.mSigleOrientGestureLib.recognize(gesture);
        } catch (IndexOutOfBoundsException e) {
            Log.e(TAG, "gesture recognize throw IndexOutOfBoundsException");
        } catch (Exception e2) {
            Log.e(TAG, "gesture recognize throw Exception");
        }
    ...
    private boolean bdReportIfNeed(GestureState gestureState) {
        ...
        Flog.bdReport(991310952, "issuccess", "false");
        return true;
    }

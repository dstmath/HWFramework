...
public class HwPointPositionView extends View {
    ...
    private PointF[] mTraceControlPoints = new PointF[POINT_TOTAL_SIZE];
    ...
    public HwPointPositionView(Context context, StylusGestureListener stylusGestureListener) {
        ...
        this.mGlowTraceSystem = new HwGlowTraceSystem(POINT_SYSTEM_SIZE, BitmapFactory.decodeResource(getResources(), 33751446), MAX_DIVERGENCE_NUM);
        updateTrackColors();
        ...
        if (canvas != null) {
            ...
            if (!this.mPointerPath.isEmpty()) {
                ...
                if (!this.mIsDrawing) {
                    ...
                    Reporter.c(getContext(), (int) TYPE_STYLUS_DRAW_LINE);
    ...
    private void processMoveEvent(MotionEvent event) {
        ...
        if (this.mPointCounter == 4) {
            ...
            pointFArr[3].set((pointFArr[2].x + this.mTraceControlPoints[4].x) / DOUBLE, (this.mTraceControlPoints[2].y + this.mTraceControlPoints[4].y) / DOUBLE);

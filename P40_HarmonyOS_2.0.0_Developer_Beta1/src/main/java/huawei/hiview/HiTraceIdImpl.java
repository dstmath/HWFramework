...
public final class HiTraceIdImpl implements HiTraceId {
    ...
    private long getValueFromByte(int fromBit, int toBit) {
        ...
        return (value << count) | ((long) (((this.idArray[toIndex] & getHighBitMask(count)) & ConstantValues.MAX_CHANNEL_VALUE) >>> (8 - count)));
    }
    ...
    /* JADX INFO: Multiple debug info for r8v3 int: [D('fromBitIndex' int), D('index' int)] */
    private void setValueToByte(long value, int fromBit, int toBit) {
        ...
        if (fromBit == toBit) {
            ...
            int index2 = fromBit % 8;
            if (index2 == 0) {
                this.idArray[fromIndex] = (byte) ((int) setValue);
                return;
            }
            int lowMask = getLowBitMask(8 - index2);
    ...
    private void setVersion(byte version) {
        if (isValid()) {
            setValueToByte((long) version, 60, 62);
        }
    }
    ...
    public boolean isFlagEnabled(int flag) {
        if (isValid() && flag >= 0 && flag < 127 && (getFlags() & flag) != 0) {
            return true;
        }
        return false;
    }
    ...
    public String toString() {
        ...
        for (int i = 0; i < length; i++) {
            String tmp = String.format(Locale.ENGLISH, "%s", Integer.toHexString(bArr[i] & 255));
            if (tmp.length() == 1) {

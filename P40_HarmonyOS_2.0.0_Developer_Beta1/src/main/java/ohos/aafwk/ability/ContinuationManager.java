...
public final class ContinuationManager {
    ...
    public void continueAbility(boolean z, String str) throws IllegalStateException {
        ...
        if (checkContinuationIllegal()) {
            ...
            debugLog(this.ability.getAbilityInfo(), nanoTime);
        } else {
    ...
    private boolean checkContinuationIllegal() {
        if (this.ability.getCurrentState() < Ability.STATE_INITIAL) {
    ...
    private static void debugLog(AbilityInfo abilityInfo, long j) {
        if (abilityInfo != null) {
            long nanoTime = System.nanoTime();
            JLog.debug(JLogConstants.JLID_LOCAL_CONTINUE_ABILITY_BEGIN, abilityInfo.getBundleName() + PsuedoNames.PSEUDONAME_ROOT + abilityInfo.getClassName() + " cost: " + ((nanoTime - j) / 1000000) + "ms");
        }
    }
}

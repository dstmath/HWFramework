...
public class DataAbilityHelper {
    ...
    public boolean reload(Uri uri2, PacMap pacMap) throws DataAbilityRemoteException {
        checkUriParam(uri2);
        Bytrace.startTrace(2147483648L, "providerReload");
        if (this.uri == null) {
            this.dataAbility = this.context.getDataAbility(uri2, false);
            IDataAbility iDataAbility = this.dataAbility;
            if (iDataAbility != null) {
                try {
                    return iDataAbility.reload(uri2, pacMap);
                } catch (DataAbilityDeadException unused) {
                    this.context.releaseDataAbility(this.dataAbility);
                    this.dataAbility = this.context.getDataAbility(uri2, true);
                    return this.dataAbility.reload(uri2, pacMap);
                } finally {
                    Bytrace.finishTrace(2147483648L, "providerReload");
                }
            } else {
                Bytrace.finishTrace(2147483648L, "providerReload");
                throw new IllegalStateException("No corresponding dataAbility, reload failed");
            }
        } else {
            try {
                return this.dataAbility.reload(uri2, pacMap);

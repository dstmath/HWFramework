...
public class TimeEvent {
    ...
    public void dump(String str, PrintWriter printWriter) {
        if (str != null && printWriter != null) {
            String format = this.startMs != -1 ? new SimpleDateFormat("yyyy-MM-dd HH:mm:ss:SSS").format(new Date(this.startMs)) : INVALID_TIME_STRING;
            ...
            printWriter.println(str + "Start: " + format);
            ...
            if (j >= 0) {
                printWriter.format("%.3f%n", Float.valueOf(((float) j) / 1000000.0f));
            } else {

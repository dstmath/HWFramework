...
public class AbilityShellActivityDelegate extends AbilityShellDelegate {
    private static final String CALLER_URI_SCHEME = "app://";
    ...
    public void onCreate(Bundle bundle) {
        ...
        if (this.abilityInfo == null) {
            AppLog.w(SHELL_LABEL, "AbilityShellPageComponentDelegate::onCreate can't find AbilityInfo and ShellInfo relationship from bms", new Object[0]);
            return;
        }
    ...
    private void handleCreateAsContinuation(Intent intent) {
        ...
        boolean isFlagExists = isFlagExists(1024, intent.getFlags());
    ...
    public void onStart() {
        ...
        int currentState = this.ability.getCurrentState();
        scheduleAbilityLifecycle(this.zidaneIntent, LifecycleState.AbilityState.INACTIVE_STATE.getValue());
        if (currentState != LifecycleState.AbilityState.INACTIVE_STATE.getValue()) {
    ...
    public Object onRetainNonConfigurationInstance() {
        return this.ability.onStoreDataWhenConfigChange();
    }

    public Uri onProvideReferrer() {
        ohos.utils.net.Uri onSetCaller = this.ability.onSetCaller();
        if (onSetCaller != null) {
            return UriConverter.convertToAndroidUri(onSetCaller);
        }
        return Uri.parse(CALLER_URI_SCHEME + this.abilityInfo.getBundleName());
    }
    ...
    public void onTopResumedActivityChanged(boolean z) {
        this.ability.onTopActiveAbilityChanged(z);
    }
}

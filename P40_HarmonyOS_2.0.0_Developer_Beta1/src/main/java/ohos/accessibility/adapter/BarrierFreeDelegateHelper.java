...
public abstract class BarrierFreeDelegateHelper extends View.AccessibilityDelegate {
    ...
    public int getViewIdByCoordinates(float f, float f2) {
        AccessibilityViewInfo queryRootAccessibilityViewInfo = queryRootAccessibilityViewInfo();
    ...
    private AccessibilityViewInfo getClickedAccessibilityView(AccessibilityViewInfo accessibilityViewInfo, int i, int i2) {
        android.graphics.Rect rectOnScreen = getRectOnScreen(accessibilityViewInfo.getRect());
        LogUtil.debug(TAG, "getVirtualViewAt clickId:" + accessibilityViewInfo.getId() + " rawX:" + i + " rawY:" + i2 + " groupRegion:" + rectOnScreen.toString());
        ...
        if (childIdList.length > 0) {
            ...
            while (true) {
                ...
                if (i3 <= 0) {
                    ...
                    AccessibilityViewInfo queryAccessibilityViewInfoById = queryAccessibilityViewInfoById(i3);
                    if (queryAccessibilityViewInfoById == null) {
                        LogUtil.info(TAG, "getVirtualViewAt can not find view:" + i3);
                    } else {

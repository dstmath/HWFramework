...
public class AccessibleControlAdapter {
    private static final float DEFAULT_DISPLAY_RESIZE_SCALE = 1.0f;
    private static final float DEFAULT_RESIZE_CENTER_X = 0.0f;
    private static final int SAMPLE_TIME_MS = 100;
    ...
    public static float getDisplayResizeCenterX(int i, int i2) {
        IAccessibilityServiceConnection connection = AccessibilityInteractionClient.getConnection(i);
        if (connection == null) {
            return DEFAULT_RESIZE_CENTER_X;
        }
        try {
    ...
    public static boolean dispatchGesture(int i, List<GesturePathDefine> list, int i2) {
        ...
        if (connection == null) {
            ...
            LogUtil.info(TAG, "dispatchGesture failed,gesture paths is empty");
            return false;
        } else {
            try {
    ...
    private static GestureDescription.StrokeDescription getGestureStrokes(List<GesturePathDefine> list) {
        ...
        int size = list.size();
        Path path = new Path();
        path.moveTo(list.get(0).getStartPosition().getPositionX(), list.get(0).getStartPosition().getPositionY());
        path.lineTo(list.get(0).getEndPosition().getPositionX(), list.get(0).getEndPosition().getPositionY());
        if (size == 1) {
            strokeDescription = new GestureDescription.StrokeDescription(path, 0, (long) list.get(0).getDurationTime(), false);
        } else {
            strokeDescription = new GestureDescription.StrokeDescription(path, 0, (long) list.get(0).getDurationTime(), true);
        }
        for (int i = 1; i < size; i++) {
            Path path2 = new Path();
            path2.moveTo(list.get(i).getStartPosition().getPositionX(), list.get(i).getStartPosition().getPositionY());

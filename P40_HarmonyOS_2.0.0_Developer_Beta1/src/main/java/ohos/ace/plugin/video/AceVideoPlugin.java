        ...
        try {
            ...
            if (object != null) {
                if (object instanceof AceTexture) {
                    Surface harmonySurface = new AceSurfaceAdapter(((AceTexture) object).getSurface()).getHarmonySurface();
                    if (harmonySurface == null) {
                        ALog.e(LOG_TAG, "surface is null");
                        return -1;
                    }
                    long atomicId = getAtomicId();
                    addResource(atomicId, new AceVideo(atomicId, instanceName, harmonySurface, this.context, getEventCallback()));
                    return atomicId;
                }
            }

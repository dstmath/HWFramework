...
public class ShapeElement extends Element {
    ...
    private static final HiLogLabel TAG = new HiLogLabel(3, (int) LogDomain.END, "ShapeElement");
                        ...
                        if (typedAttribute != null) {
                            try {
                                if ("shape".equals(typedAttribute.getName())) {
                                    ...
                                    HiLog.error(TAG, "do not support this tag: %s", new Object[]{typedAttribute.getName()});
                                }
    ...
    private void parseNodeInternal(Node node) {
        ...
        HiLog.debug(TAG, "parsing shapeElement attr: %{public}s", new Object[]{name});
        ...
        for (TypedAttribute typedAttribute : list) {
            if (typedAttribute != null) {
                ...
                if (c == 0) {
                    ...
                    HiLog.error(TAG, "do not support this attr : %s", new Object[]{typedAttribute.getName()});
                } else {
        ...
        for (TypedAttribute typedAttribute : list) {
            if (typedAttribute != null) {
                if ("color".equals(typedAttribute.getName())) {
                    ...
                    HiLog.error(TAG, "do not parse this solid tag: %{public}s", new Object[]{typedAttribute.getName()});
                }
            }
        }
    }

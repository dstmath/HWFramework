...
public enum CoreFormat {
    ...
    static CoreFormat swigToEnum(int i) {
        CoreFormat[] coreFormatArr = (CoreFormat[]) CoreFormat.class.getEnumConstants();
        if (i < coreFormatArr.length && i >= 0 && coreFormatArr[i].swigValue == i) {
            return coreFormatArr[i];
        }
        for (CoreFormat coreFormat : coreFormatArr) {
            if (coreFormat.swigValue == i) {
                return coreFormat;
            }
        }
        throw new IllegalArgumentException("No enum " + CoreFormat.class + " with value " + i);
    }
    ...
    private CoreFormat(int i) {
        this.swigValue = i;
        int unused = SwigNext.next = i + 1;
    }

    private CoreFormat(CoreFormat coreFormat) {
        this(coreFormat.swigValue);
    }

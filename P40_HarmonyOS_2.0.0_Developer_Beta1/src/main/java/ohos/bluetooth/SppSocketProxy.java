...
public class SppSocketProxy implements ISppSocket {
    ...
    private static final HiLogLabel TAG = new HiLogLabel(3, (int) LogHelper.BT_DOMAIN_ID, "SppSocketProxy");
    ...
    private Optional<FileDescriptor> createFileDescriptor(MessageParcel messageParcel) {
        ...
        HiLog.info(TAG, "createFileDescriptor read ec %{public}d.", new Object[]{Integer.valueOf(readInt)});
        if (readInt != 0) {
            return Optional.ofNullable(null);
        }
        FileDescriptor readFileDescriptor = messageParcel.readFileDescriptor();
        HiLog.info(TAG, "createFileDescriptor is %{public}d.", new Object[]{readFileDescriptor});

...
public class GattService implements Sequenceable {
    ...
    public boolean unmarshallingSpecially(Parcel parcel) {
        ...
        for (int i2 = 0; i2 < readInt2; i2++) {
            ...
            SequenceUuid sequenceUuid = new SequenceUuid();
            parcel.readSequenceable(sequenceUuid);
            this.mIncludedServices.add(new GattService(sequenceUuid.getUuid(), parcel.readInt(), parcel.readInt() == 0));
        }
        return true;
    }
    ...
    public boolean marshallingSpecially(Parcel parcel) {
        ...
        for (GattService gattService : this.mIncludedServices) {
            arrayList.add(new GattIncludedService(gattService.getUuid(), gattService.getHandle(), gattService.isPrimary()));
        }
        parcel.writeInt(arrayList.size());
        Iterator it = arrayList.iterator();
        while (it.hasNext()) {
            GattIncludedService gattIncludedService = (GattIncludedService) it.next();
            if (gattIncludedService != null) {
                parcel.writeInt(1);
                gattIncludedService.marshallingSpecially(parcel);
            } else {
                parcel.writeInt(0);
            }
        }

...
public class SqliteExceptionUtils {
    ...
    private static final int NEW_PROXY_CANT_PROCESS_EXCEPTION_ERROR_CODE = 22;
    private static final int NEW_PROXY_MAIN_VERSION_EXCEPTION_ERROR_CODE = 20;
    private static final int OLD_PROXY_MAIN_VERSION_EXCEPTION_ERROR_CODE = 21;
    private static final int OLD_PROXY_NOT_SUPPORT_EXCEPTION_ERROR_CODE = 23;
    private static final int OLD_PROXY_WITH_COMPATIBLE_RESPONSE_EXCEPTION_ERROR_CODE = 24;
    private static final int OLD_PROXY_WITH_DEFAULT_REPLY_EXCEPTION_ERROR_CODE = 25;
    ...
    private static int getCode(Exception exc) {
        ...
        if (exc instanceof RdbReadOnlyException) {
            return 18;
        }
        if (exc instanceof NewProxyMainVersionException) {
            return 20;
        }
        if (exc instanceof OldProxyMainVersionException) {
            return 21;
        }
        if (exc instanceof NewProxyCantProcessException) {
            return 22;
        }
        if (exc instanceof OldProxyNotSupportException) {
            return 23;
        }
        if (exc instanceof OldProxyWithCompatibleResponseException) {
            return 24;
        }
        return exc instanceof OldProxyWithDefaultReplyException ? 25 : 19;
    }
    ...
    private static void throwIfSqliteException(int i, String str) {
        switch (i) {
            ...
            case 20:
                throw new NewProxyMainVersionException(str);
            case 21:
                throw new OldProxyMainVersionException(str);
            case 22:
                throw new NewProxyCantProcessException(str);
            case 23:
                throw new OldProxyNotSupportException(str);
            case 24:
                throw new OldProxyWithCompatibleResponseException(str);
            case 25:
                throw new OldProxyWithDefaultReplyException(str);
        }
    }
}

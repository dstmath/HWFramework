...
import ohos.data.rdb.impl.SqliteSqlBuilder;
...
public class RdbStoreAdapter {
    ...
    public static <T extends OrmObject> ResultSet query(RdbStore rdbStore, RdbPredicates rdbPredicates) {
        ...
        return rdbStore.querySqlByStep(SqliteSqlBuilder.buildQueryStringWithExpr(rdbPredicates, new String[]{StringUtils.surroundWithQuote(tableName, "`") + "." + StringUtils.surroundWithQuote(getRowidColumnName(), "`") + " AS " + getRowidColumnName(), StringUtils.surroundWithQuote(tableName, "`") + ".*"}), (String[]) rdbPredicates.getWhereArgs().toArray(new String[0]));
    }

...
public class SqliteSharedResultSet extends AbsSharedResultSet {
    ...
    public void setFillBlockForwardOnly(boolean z) {
    ...
    public void finalize() {
        try {
            if (!isClosed()) {
                ...
                if (rdbStore.isOpen()) {
                    HiLog.info(LABEL, "Finalizing a shared block that has not been closed, database name is %{public}s", new Object[]{rdbStore.isMemoryRdb() ? SqliteDatabaseConfig.MEMORY_DB_PATH : new File(rdbStore.getPath()).getName()});
                }
                close();
            }
        } finally {

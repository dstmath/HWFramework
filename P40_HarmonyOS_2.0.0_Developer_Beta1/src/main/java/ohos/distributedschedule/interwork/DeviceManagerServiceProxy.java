...
public class DeviceManagerServiceProxy {
            ...
            if (!obtain.writeInterfaceToken(DEVICEMANAGER_SERVICE_INTERFACE_TOKEN)) {
                ...
                synchronized (PROXY_LOCK) {
                    if (!initDMSProxyLocked()) {
                        ...
                        boolean sendRequest = this.mDeviceManagerProxy.sendRequest(212, obtain, obtain2, new MessageOption());
                        if (!sendRequest) {
                            obtain.reclaim();
                            obtain2.reclaim();
            ...
            if (obtain.writeInterfaceToken(DEVICEMANAGER_SERVICE_INTERFACE_TOKEN) && obtain.writeString(str)) {
                synchronized (PROXY_LOCK) {
                    if (!initDMSProxyLocked()) {
                        ...
                        boolean sendRequest = this.mDeviceManagerProxy.sendRequest(213, obtain, obtain2, new MessageOption());
                    }
                }
            }
            return null;
            ...
            if (obtain.writeInterfaceToken(DEVICEMANAGER_SERVICE_INTERFACE_TOKEN) && obtain.writeRemoteObject(iRemoteObject)) {
                synchronized (PROXY_LOCK) {
                    if (!initDMSProxyLocked()) {
                        ...
                        if (this.mDeviceManagerProxy.sendRequest(i, obtain, obtain2, new MessageOption())) {
                            int readInt = obtain2.readInt();
    ...
    private boolean initDMSProxyLocked() {
        ...
        if (iRemoteObject == null) {
            ...
            return false;
        } else if (iRemoteObject.addDeathRecipient(new IRemoteObject.DeathRecipient() {
            ...
            HiLog.error(SHELL_LABEL, "initDMSProxyLocked add death recipient failed", new Object[0]);
            return true;
        }
    }

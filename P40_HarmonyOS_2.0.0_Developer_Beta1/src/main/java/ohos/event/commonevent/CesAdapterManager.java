...
public class CesAdapterManager {
    ...
    private boolean handlePublishCommonEvent(long j, long j2, long j3) {
        ...
        if (!createAndroidIntent.isPresent()) {
            HiLog.error(LOG_LABEL, "handlePublishCommonEvent, createESystemIntent failed!", new Object[0]);
            return false;
        }
    ...
    private int handleSubscribeCommonEvent(long j) {
        ...
        Parcel create = Parcel.create(j);
        CommonEventSubscribeInfo commonEventSubscribeInfo = new CommonEventSubscribeInfo(new MatchingSkills());
        ...
        create.reclaim();
        String str = null;
        Skills orElse = MatchingSkillsTransformation.convertMatchingSkillsToZSkills(commonEventSubscribeInfo.getMatchingSkills()).orElse(null);
        if (orElse == null) {
            HiLog.error(LOG_LABEL, "subscribeCommonEvent, intent filter is null!", new Object[0]);
            return 0;
        }
        Optional<IntentFilter> createAndroidIntentFilter = DataConverter.createAndroidIntentFilter(true, commonEventSubscribeInfo.getPriority(), orElse);
        ...
        if (!registerReceiver(commonEventSubscribeInfo, cesProxyReceiver, createAndroidIntentFilter.get(), str)) {
    ...
    private boolean registerReceiver(CommonEventSubscribeInfo commonEventSubscribeInfo, CesProxyReceiver cesProxyReceiver, IntentFilter intentFilter, String str) {
        try {

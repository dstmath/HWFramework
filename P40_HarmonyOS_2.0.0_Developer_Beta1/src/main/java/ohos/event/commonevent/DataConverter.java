...
public class DataConverter {
    ...
    public static Optional<IntentFilter> createAndroidIntentFilter(boolean z, int i, Skills skills) {
        if (skills == null) {
            return Optional.empty();
        }
        Optional<IntentFilter> of = Optional.of(new IntentFilter());
        baseConverter.convertSkillsToAospIntentFilter(skills, of.get());
        int countActions = skills.countActions();
        for (int i2 = 0; i2 < countActions; i2++) {
            String action = skills.getAction(i2);
            if (!isEmptyStr(action)) {
                Object obj = actionObjectMap.get(action);
                if (obj == null && ActionMapper.queryZAction(action)) {
                    obj = getActionClassName(z, action);
                ...
                if (obj instanceof CommonEventBaseConverter) {
                    ((CommonEventBaseConverter) obj).convertSkillsToAospIntentFilter(skills, of.get());
                }
            }
        }

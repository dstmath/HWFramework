...
public final class NotificationSlot implements Sequenceable {
    ...
    public void setVibrationStyle(long[] jArr) {
        this.vibrationEnabled = jArr != null && jArr.length > 0;
        if (jArr != null) {
            this.vibrationValues = (long[]) jArr.clone();
        } else {
    ...
    public long[] getVibrationStyle() {
        long[] jArr = this.vibrationValues;
        if (jArr != null) {
    ...
    private boolean writeToParcel(Parcel parcel) {
        ...
        if (parcel.writeBoolean(this.lightEnabled) && parcel.writeInt(this.lightColor) && parcel.writeString(this.slotGroupId)) {
            return writeToParcelSecond(parcel);
        }
        return false;
    }

    private boolean writeToParcelSecond(Parcel parcel) {
        if (this.vibrationValues != null) {
            if (!parcel.writeByte((byte) 1) || !parcel.writeLongArray(this.vibrationValues)) {
                return false;
            }
        } else if (!parcel.writeByte((byte) 0)) {
            return false;
    ...
    private boolean readFromParcel(Parcel parcel) {
        ...
        if (readByte2 == 1) {
            this.vibrationValues = parcel.readLongArray();
        } else if (readByte2 == 0) {
            this.vibrationValues = null;
        } else {
            HiLog.warn(LABEL, "NotificationSlot: readFromParcel read vibrationValues parcel fail.", new Object[0]);
            return false;
        }
        return true;
    }
    ...
    public String toString() {
        return "NotificationSlot[ slotId = " + this.slotId + " slotName = " + this.slotName + " slotGroupId = " + this.slotGroupId + " desc = " + this.desc + " level = " + this.level + " showBadgeFlag = " + this.showBadgeFlag + " bypassDnd = " + this.bypassDnd + " lockscreenVisibility = " + this.lockscreenVisibility + " vibrationEnabled = " + this.vibrationEnabled + " vibrationValues = " + Arrays.toString(this.vibrationValues) + " sound = " + this.sound + " lightEnabled = " + this.lightEnabled + " lightColor = " + this.lightColor + "]";
    }
}

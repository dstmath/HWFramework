...
public class ScientificNotation extends Notation implements Cloneable {
    ...
    private static class ScientificHandler implements MicroPropsGenerator, MultiplierProducer, Modifier {
        ...
        private int doApply(int i, FormattedStringBuilder formattedStringBuilder, int i2) {
            ...
            int i3;
            int insert;
            int insert2 = formattedStringBuilder.insert(i2, (CharSequence) this.symbols.getExponentSeparator(), (Format.Field) NumberFormat.Field.EXPONENT_SYMBOL) + i2;
            if (i >= 0 || this.notation.exponentSignDisplay == NumberFormatter.SignDisplay.NEVER) {
                if (i >= 0 && this.notation.exponentSignDisplay == NumberFormatter.SignDisplay.ALWAYS) {
                    insert = formattedStringBuilder.insert(insert2, (CharSequence) this.symbols.getPlusSignString(), (Format.Field) NumberFormat.Field.EXPONENT_SIGN);
                }
                abs = Math.abs(i);
                i3 = 0;
                while (true) {
                    ...
                    insert2 += formattedStringBuilder.insert(insert2 - i3, (CharSequence) this.symbols.getDigitStringsLocal()[abs % 10], (Format.Field) NumberFormat.Field.EXPONENT);
                    i3++;
                    abs /= 10;
                }
            } else {
                insert = formattedStringBuilder.insert(insert2, (CharSequence) this.symbols.getMinusSignString(), (Format.Field) NumberFormat.Field.EXPONENT_SIGN);
            }
            insert2 += insert;
            abs = Math.abs(i);
            i3 = 0;

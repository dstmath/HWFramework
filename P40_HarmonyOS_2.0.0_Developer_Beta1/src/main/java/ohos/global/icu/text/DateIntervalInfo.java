...
public class DateIntervalInfo implements Cloneable, Freezable<DateIntervalInfo>, Serializable {
    ...
    private void initializeData(ULocale uLocale) {
        ...
        DateIntervalInfo dateIntervalInfo = (DateIntervalInfo) DIICACHE.get(uLocale2);
    ...
    private void setup(ULocale uLocale) {
        ...
        try {
            ...
            setFallbackIntervalPattern(bundleInstance.getStringWithFallback(CALENDAR_KEY + "/" + keywordValue + "/" + INTERVAL_FORMATS_KEY + "/" + FALLBACK_STRING));
            HashSet hashSet = new HashSet();
            while (keywordValue != null) {
                if (!hashSet.contains(keywordValue)) {
                    hashSet.add(keywordValue);
                    bundleInstance.getAllItemsWithFallback(CALENDAR_KEY + "/" + keywordValue, dateIntervalSink);

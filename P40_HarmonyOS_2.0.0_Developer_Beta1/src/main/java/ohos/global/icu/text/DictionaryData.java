...
final class DictionaryData {
    ...
    public static DictionaryMatcher loadDictionaryFor(String str) throws IOException {
        ByteBuffer requiredData = ICUBinary.getRequiredData("brkitr/" + UResourceBundle.getBundleInstance("ohos/global/icu/impl/data/icudt66b/brkitr").getStringWithFallback("dictionaries/" + str));
        ICUBinary.readHeader(requiredData, (int) DATA_FORMAT_ID, (ICUBinary.Authenticate) null);
        int[] iArr = new int[8];
        boolean z = false;
        for (int i = 0; i < 8; i++) {

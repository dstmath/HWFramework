...
public class ElementImpl extends Element {
    ...
    public int getColor() throws NotExistException, IOException, WrongTypeException {
        Matcher matcher = Pattern.compile("^#(?<AA>([A-F]|\\d){2})?(?<RR>([A-F]|\\d){2})(?<GG>([A-F]|\\d){2})(?<BB>([A-F]|\\d){2})$").matcher(getString().toUpperCase());
        if (matcher.find()) {
            String group = matcher.group("AA") == null ? "FF" : matcher.group("AA");
            return Integer.parseInt(matcher.group("BB"), 16) | (Integer.parseInt(matcher.group("GG"), 16) << 8) | (Integer.parseInt(matcher.group("RR"), 16) << 16) | (Integer.parseInt(group, 16) << 24);
        }

...
public class ResourceManagerImpl extends ResourceManager {
    ...
    private native long nativeSetup(Locale locale, int i, int i2, int i3, int i4);

    private native void nativeUpdateConfig(long j, Locale locale, int i, int i2, int i3, int i4);
    ...
    private boolean addResourceInternal(ResourcePath resourcePath) throws IOException {
        ...
        for (String str : list) {
            ...
            if (str.endsWith(str2)) {
                if (!aaName.isEmpty()) {
                    str2 = resourcePath.getAaName() + File.separator + str;
                }
                try {
                    ...
                    if (open == null) {
                        ...
                        if (available != 0) {
                            ...
                            } else if (nativeAddIndex(this.handle, bArr)) {
                                i++;
                            }
                        }
                    }
    ...
    private void modifyColorMode() {
        if (this.resConfig.colorMode == 0) {
            this.resConfig.sysColorMode = 0;
        } else if (this.resConfig.colorMode == 1) {
            this.resConfig.sysColorMode = 1;
        }
    }

    private long setupManager(Configuration configuration, DeviceCapability deviceCapability) {
        initConfigInner(configuration, deviceCapability);
        return nativeSetup(this.resConfig.getFirstLocale(), this.resConfig.direction, this.resCapability.screenDensity, this.resCapability.deviceType, this.resConfig.sysColorMode);
    }
        ...
        if (!sysInit) {
            synchronized (LOCK) {
                if (!sysInit) {
                    ...
                    String[] strArr = {"ohos:resources.index", "ohos:entry" + File.separator + RESOURCE_INDEX_NAME};
                    int length = strArr.length;
                    for (int i = 0; i < length; i++) {
                        try {
                            InputStream open = this.resPackage.open(strArr[i]);
                            if (open == null) {
    ...
    private void computePreferedLocale(long j, boolean z) {
        ...
        int nativeGetBestLocale = nativeGetBestLocale(j, strArr, new int[]{this.resConfig.direction, this.resCapability.screenDensity, this.resCapability.deviceType, this.resConfig.sysColorMode}, getAceResources());
    ...
    private String[] getAceResources() {
        ArrayList arrayList = new ArrayList();
        try {
            String[] list = this.resPackage.list("js");
            for (String str : list) {
                String[] list2 = this.resPackage.list("js/" + str + "/i18n");
                int length = list2.length;
                for (int i = 0; i < length; i++) {
                    String str2 = list2[i];
                    if (!arrayList.contains(str2) && str2.endsWith("\\.json")) {
                        arrayList.add(str2.split("\\.json")[0]);
                    }
                }
            }
        } catch (IOException unused) {
            HiLog.error(LABEL, "Get ace resource error.", new Object[0]);
        }
        if (arrayList.size() > 0) {
            Locale.setDefault(this.resConfig.getLocaleProfile().getLocales()[0]);
        }

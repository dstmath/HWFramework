...
public final class HiTraceIdImpl {
    ...
    private long getValueFromByte(int i, int i2) {
        ...
        if (i3 >= i4) {
            j = (j << 8) | ((long) (this.idArray[i3] & 255));
            i3++;
            if (i3 >= i4) {
            }
        }
        if (i6 == 7) {
            return (j << 8) | ((long) (this.idArray[i4] & 255));
        }
        int i7 = i6 + 1;
    ...
    private void setVersion(byte b) {
        if (isValid()) {
            setValueToByte((long) b, TRACEID_VERSION_FROM_BIT, TRACEID_VERSION_TO_BIT);
        }
    }
    ...
    public long getSpanId() {
        ...
        return getValueFromByte(90, TRACEID_SPANID_TO_BIT);
    }

    public void setSpanId(long j) {
        if (isValid()) {
            setValueToByte(j, 90, TRACEID_SPANID_TO_BIT);
        }
    }
    ...
    public String toString() {
        ...
        for (int i = 0; i < length; i++) {
            String format = String.format(Locale.ENGLISH, "%s", Integer.toHexString(bArr[i] & 255));
            if (format.length() == 1) {

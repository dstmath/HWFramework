...
public class AudioRenderer {
    ...
    public boolean setSpeed(float f) {
        if (Math.abs(f - 0.0f) <= THRESHOLD || f < 0.0f) {
    ...
    public boolean setPlaybackSpeed(SpeedPara speedPara) {
        if (speedPara == null) {
            ...
            return false;
        } else if (Math.abs(speedPara.pitch - 0.0f) <= THRESHOLD || speedPara.pitch < 0.0f || Math.abs(speedPara.speed - 0.0f) <= THRESHOLD || speedPara.speed < 0.0f) {

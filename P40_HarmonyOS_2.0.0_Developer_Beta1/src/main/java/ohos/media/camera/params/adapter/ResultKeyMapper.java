...
public class ResultKeyMapper {
    ...
    static {
        ...
        hashMap.put(ResultKey.AUTO_ZOOM_STATE, CaptureResultEx.HUAWEI_AUTO_ZOOM_STATE);
        hashMap.put(ResultKey.FACE_AE_STATE, CaptureResultEx.HUAWEI_FACE_AE_STATE);
        hashMap.put(ResultKey.VENDOR_CUSTOM_RESULT, CaptureResultEx.HUAWEI_VENDOR_CUSTOM_RESULT);
        ...
        hashMap2.put(ResultKey.EXPOSURE_FPS_RANGE_RESULT, new SpecialMappedKeyMethod() {
            /* class ohos.media.camera.params.adapter.ResultKeyMapper.AnonymousClass6 */
            ...
            public <T> T getMappedResultKeyValue(CameraMetadataNative cameraMetadataNative, StaticCameraCharacteristics staticCameraCharacteristics) {
                ...
                if (range == null) {
                    ResultKeyMapper.LOGGER.debug("There is no CONTROL_AE_TARGET_FPS_RANGE in CaptureResult", new Object[0]);
                    return null;
                }
                T t = (T) Scope.create((Integer) range.getLower(), (Integer) range.getUpper());
                ResultKeyMapper.LOGGER.debug("EXPOSURE_FPS_RANGE_RESULT is %{public}s", t);
                return t;
            }
        });

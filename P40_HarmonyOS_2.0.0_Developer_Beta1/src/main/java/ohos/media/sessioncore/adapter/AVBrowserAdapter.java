                    ...
                    AVBrowserAdapter.this.service.getMediaItem(str, new GeneralReceiver() {
                        ...
                        public void sendResult(int i, PacMap pacMap) {
                            if (AVBrowserAdapter.this.isConnected()) {
                                if (i != 0 || pacMap == null || !pacMap.hasKey(AVBrowserAdapter.MEDIA_ITEM)) {
                                    aVElementCallback.onError(str);
                                } else if (pacMap.getSequenceable(AVBrowserAdapter.MEDIA_ITEM).isPresent()) {
                                    Sequenceable sequenceable = pacMap.getSequenceable(AVBrowserAdapter.MEDIA_ITEM).get();

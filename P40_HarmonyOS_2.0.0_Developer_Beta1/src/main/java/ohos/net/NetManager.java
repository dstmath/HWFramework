...
public final class NetManager {
    ...
    public boolean setupSpecificNet(NetSpecifier netSpecifier, NetStatusCallback netStatusCallback) {
        ...
        try {
            NetRemoteEvent netRemoteEvent = new NetRemoteEvent(ICourierEx.MESSENGER_DESCRIPTOR);
    ...
    public boolean sendNetConnectState(NetHandle netHandle, boolean z) {
        if (netHandle == null) {
            return false;
        }
        try {
            return this.mNetManagerProxy.sendNetConnectState(netHandle, z);
        } catch (RemoteException unused) {
            HiLog.warn(LABEL, "Failed to sendNetConnectState", new Object[0]);
            return false;
        }
    }
}

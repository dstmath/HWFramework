...
public abstract class ProxyGenHandler implements InvocationHandler {
        ...
        if (!(method == null || (methodArr = this.methods) == null)) {
            for (Method method2 : methodArr) {
                if (compareMethod(method, method2)) {
                    HiLog.debug(LABEL_LOG, " findMethod found.", new Object[0]);
                    return method2;
                }
            }
            HiLog.debug(LABEL_LOG, " findMethod not found.", new Object[0]);
        }
        return null;
    }
    ...
    private boolean compareMethod(Method method, Method method2) {
        ...
        if (parameterTypes.length != parameterTypes2.length) {
            HiLog.debug(LABEL_LOG, "compareMethod  param type len not equal", new Object[0]);
            return false;
        }
        int i = 0;
        while (true) {

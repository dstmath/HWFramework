...
public class MessageParcel extends Parcel {
    ...
    private static final int EXC_PARCEL_CAPACITY_ERROR = -14;
    ...
    private native boolean nativeContainFileDescriptors();
    ...
    private native long nativeReadAshmem();
    ...
    private native void nativeReleaseAshmem(long j);

    private native boolean nativeWriteAshmem(long j);
    ...
    public boolean containFileDescriptors() {
        return nativeContainFileDescriptors();
    }
    ...
    private Exception createException(int i, String str) {
        ...
        switch (i) {
            case EXC_PARCEL_CAPACITY_ERROR /* -14 */:
                return new ParcelException(str);
            case -13:
    ...
    public void writeException(Exception exc) {
        ...
        if (exc instanceof SecurityException) {
            ...
            i = EXC_REMOTE_TRANSACTION_FAILED;
        } else {
            i = exc instanceof ParcelException ? EXC_PARCEL_CAPACITY_ERROR : 0;
        }
        writeInt(i);
        if (i != 0) {
    ...
    public boolean writeAshmem(Ashmem ashmem) {
        return nativeWriteAshmem(ashmem.getAshmemIdentity());
    }

    public Ashmem readAshmem() {
        return Ashmem.createAshmemFromExisting(nativeReadAshmem());
    }

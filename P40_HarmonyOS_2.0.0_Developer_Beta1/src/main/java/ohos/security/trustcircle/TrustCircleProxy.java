...
public class TrustCircleProxy implements ITrustCircleProxy {
    ...
    private Object switchAck(AuthPara.Type type, Object obj) {
        ...
        if (i != 1) {
            ...
            return new AuthPara.RecAuthAckInfo(recAuthAckInfo.getTcisIdSecondary(), recAuthAckInfo.getPkVersionSecondary(), recAuthAckInfo.getNonceSecondary(), recAuthAckInfo.getMacSecondary(), recAuthAckInfo.getAuthKeyAlgoTypeSecondary(), recAuthAckInfo.getAuthKeyInfoSecondary(), recAuthAckInfo.getAuthKeyInfoSignSecondary());
        }
    }
    ...
    private static class AuthSyncAckInfoBase {
        ...
        AuthSyncAckInfoBase(AuthPara.OnAuthSyncAckInfo onAuthSyncAckInfo) {
            if (onAuthSyncAckInfo == null) {
                ...
                this.mAckInfo = new AuthParaGroup.OnAuthSyncAckInfo.Builder().setTcisIdSecondary(onAuthSyncAckInfo.mTcisIdSlave).setPkVersionSecondary(onAuthSyncAckInfo.mPkVersionSlave).setNonceSecondary(onAuthSyncAckInfo.mNonceSlave).setMac(onAuthSyncAckInfo.mMAC).setAuthKeyAlgoType(onAuthSyncAckInfo.mAuthKeyAlgoType).setAuthKeyInfo(onAuthSyncAckInfo.mAuthKeyInfo).setAuthKeyInfoSign(onAuthSyncAckInfo.mAuthKeyInfoSign).create();
            }
        }

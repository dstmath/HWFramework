...
public class UpdateMachineAdapter {
    ...
    public boolean setCurrentSlotAdapter() throws UpdateMachineException {
        getUpdateMachineService();
        try {
            HiLog.debug(TAG, "UpdateMachineAdapter setCurrentSlotAdapter begin.", new Object[0]);
            boolean currentSlot = this.updateMachineService.setCurrentSlot();
            HiLog.debug(TAG, "UpdateMachineAdapter setCurrentSlotAdapter end. setSlot=%{public}s", Boolean.valueOf(currentSlot));
            return currentSlot;
        } catch (RemoteException e) {
            StartUpStringUtil.printException(TAG, e, "UpdateMachineAdapter setCurrentSlotAdapter:");
            throw new UpdateMachineException(UpdateMachineException.UpdateMachineExceptionType.UPDATE_MACHINE_SERVICE_REMOTE_EXCEPTION);
        } catch (ServiceSpecificException e2) {
            StartUpStringUtil.printException(TAG, e2, "UpdateMachineAdapter setCurrentSlotAdapter ServiceSpecificException:");
            throw new UpdateMachineException(UpdateMachineException.UpdateMachineExceptionType.UPDATE_MACHINE_SERVICE_NOT_EXIST);
        }
    }

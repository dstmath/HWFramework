...
public abstract class WorkScheduler extends Ability {
    ...
    /* access modifiers changed from: protected */
    public final void onDisconnect(Intent intent) {
    ...
    private static boolean prepareServiceConnect() {
        proxyConnect = SysAbilityManager.getSysAbility(SystemAbilityDefinition.WORK_SCHEDULE_SERVICE_ID);
        if (proxyConnect == null) {
            HiLog.error(LOG_LABEL, "Get WorkScheduler Ability failed!!", new Object[0]);
            return false;
    ...
    public static final WorkInfo getWorkStatus(int i) throws RemoteException {
        if (i <= 0) {
            ...
            return null;
        } else if (!prepareServiceConnect()) {
            HiLog.error(LOG_LABEL, "get work status failed", new Object[0]);
            return null;
        } else {
    ...
    public static final List<WorkInfo> obtainAllWorks() throws RemoteException {
        if (prepareServiceConnect()) {
            return workSchedulerService.obtainAllWorks();
        }
        HiLog.error(LOG_LABEL, "Stop And Cancel All works failed!!", new Object[0]);
        return Collections.emptyList();
    }

    public static final boolean stopAndClearWorks() throws RemoteException {
        if (prepareServiceConnect()) {
            return workSchedulerService.stopAndClearWorks();
        }
        HiLog.error(LOG_LABEL, "Stop And Cancel All works failed!!", new Object[0]);
        return false;
    }

    public static final boolean isLastWorkTimeOut(int i) throws RemoteException {
        if (i < 0) {
            HiLog.error(LOG_LABEL, "Work id check failed", new Object[0]);
            return false;
        } else if (prepareServiceConnect()) {
            return workSchedulerService.isLastWorkTimeOut(i);
        } else {
            HiLog.error(LOG_LABEL, "Stop And Cancel All works failed!!", new Object[0]);
            return false;
        }

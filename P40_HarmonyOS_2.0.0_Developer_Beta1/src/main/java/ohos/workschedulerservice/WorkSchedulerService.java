...
public class WorkSchedulerService {
    ...
    public final class StubInterface extends WorkSchedulerServiceSkeleton {
        ...
        public List<WorkInfo> obtainAllWorks() throws RemoteException {
            return WorkSchedulerService.this.workQueueMgr.tryGetAllWorkStatus(IPCSkeleton.getCallingUid());
        }
        ...
        public boolean stopAndClearWorks() throws RemoteException {
            ...
            for (WorkInfo workInfo : obtainAllWorks()) {
                if (workInfo != null) {
                    HiLog.info(WorkSchedulerService.LOG_LABEL, "stopAndCancelAllWork: %{public}d", Integer.valueOf(workInfo.getCurrentWorkID()));
                    z &= stopWork(workInfo, true, false);
                }
            }
            return z;
        }
        ...
        public boolean isLastWorkTimeOut(int i) throws RemoteException {
            WorkStatus workStatus = WorkSchedulerService.this.workQueueMgr.getWorkStatus(IPCSkeleton.getCallingUid(), i);
            if (workStatus != null) {
                return workStatus.isTimeout();
            }
            return false;
        }

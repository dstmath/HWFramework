...
public class IFAAManagerV2Impl extends IFAAManagerV2 {
    ...
    private static final int PARAM_LENGTH = 4096;
    ...
    public int getSupportBIOTypes(Context context) {
        ...
        int type = 0;
        if (context == null) {
            Log.e(LOG_TAG, "getSupportBIOTypes failed.");
            return 0;
        }
        FingerprintManager fingerprintManager = null;
        if (context.getSystemService("fingerprint") instanceof FingerprintManager) {
    ...
    public byte[] processCmdV2(Context context, byte[] param) {
        ...
        if (param == null || param.length > PARAM_LENGTH) {
            Log.e(LOG_TAG, "Bad parameter!");
            return retByte;
        } else if (!isOkProcessCmd()) {

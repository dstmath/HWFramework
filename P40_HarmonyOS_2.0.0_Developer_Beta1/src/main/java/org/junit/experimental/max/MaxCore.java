...
public class MaxCore {
    ...
    private Request constructLeafRequest(List<Description> leaves) {
        ...
        return new Request() {
            /* class org.junit.experimental.max.MaxCore.AnonymousClass1 */

            @Override // org.junit.runner.Request
            public Runner getRunner() {
                try {
                    return new Suite(null, runners) {
                        /* class org.junit.experimental.max.MaxCore.AnonymousClass1.AnonymousClass1 */
                    };
    ...
    private void findLeaves(Description parent, Description description, List<Description> results) {
        if (!description.getChildren().isEmpty()) {
            ...
            while (it.hasNext()) {
                findLeaves(description, it.next(), results);
            }

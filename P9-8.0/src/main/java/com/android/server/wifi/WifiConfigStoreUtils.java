...
public class WifiConfigStoreUtils {
    ...
    public static boolean ignoreEnableNetwork(Context context, WifiConfiguration config, WifiNative wifiNative) {
        if (!WifiProCommonUtils.isWifiProSwitchOn(context) || config == null || ((!config.noInternetAccess || (WifiProCommonUtils.allowWifiConfigRecovery(config.internetHistory) ^ 1) == 0) && (!config.portalNetwork || !isOpenType(config) || config.internetHistory == null || !config.internetHistory.contains("2")))) {
            return false;
        }
    ...
    private static boolean isAutoConnect(Context context) {
        boolean z = true;
        if (context == null || (isAutoConnectPropEnabled ^ 1) != 0) {
            return true;
        }
    ...
    public static boolean isSkipAutoConnect(Context context, WifiConfiguration config) {
        if (!existWifiApInDatabase(context, config) || (isAutoConnect(context) ^ 1) == 0) {
            return false;
        }
    ...
    public static boolean skipEnableNetwork(Context context, int networkId, WifiNative wifiNative) {
        WifiConfiguration savedConfig = getSavedWifiConfig(context, networkId);
        if (savedConfig == null || !existWifiApInDatabase(context, savedConfig) || (isAutoConnect(context) ^ 1) == 0) {
            return false;
        }

...
public final class CertificatePinner {
    ...
    static final class Pin {
        ...
        Pin(String pattern, String pin) {
            ...
            if (pattern.startsWith(WILDCARD)) {
                url = HttpUrl.parse("http://" + pattern.substring(WILDCARD.length()));
            } else {
                url = HttpUrl.parse("http://" + pattern);
            }
            if (url == null) {
        ...
        public boolean equals(Object other) {
            if ((other instanceof Pin) && this.pattern.equals(((Pin) other).pattern) && this.hashAlgorithm.equals(((Pin) other).hashAlgorithm)) {
                return this.hash.equals(((Pin) other).hash);
            }
            return false;
        }
    ...
    public boolean equals(Object other) {
        ...
        if (!(other instanceof CertificatePinner)) {
            z = false;
        } else if (Util.equal(this.certificateChainCleaner, ((CertificatePinner) other).certificateChainCleaner)) {
            z = this.pins.equals(((CertificatePinner) other).pins);
        } else {
            z = false;
        }
        return z;
    ...
    public void check(String hostname, List<Certificate> peerCertificates) throws SSLPeerUnverifiedException {
        List<okhttp3.CertificatePinner.Pin> pins = findMatchingPins(hostname);
        if (!pins.isEmpty()) {
            int c;
    ...
    List<Pin> findMatchingPins(String hostname) {
        List<okhttp3.CertificatePinner.Pin> result = Collections.emptyList();

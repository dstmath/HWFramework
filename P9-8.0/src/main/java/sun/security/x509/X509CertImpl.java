...
public class X509CertImpl extends X509Certificate implements DerEncoder {
    ...
    private static Collection<List<?>> makeAltNames(GeneralNames names) {
        ...
        for (GeneralName gname : names.names()) {
            ...
            nameEntry.-java_util_stream_Collectors-mthref-2(Integer.valueOf(name.getType()));
            switch (name.getType()) {
                case 1:
                    nameEntry.-java_util_stream_Collectors-mthref-2(((RFC822Name) name).getName());
                    break;
                case 2:
                    nameEntry.-java_util_stream_Collectors-mthref-2(((DNSName) name).getName());
                    break;
                case 4:
                    nameEntry.-java_util_stream_Collectors-mthref-2(((X500Name) name).getRFC2253Name());
                    break;
                case 6:
                    nameEntry.-java_util_stream_Collectors-mthref-2(((URIName) name).getName());
                    break;
                case 7:
                    try {
                        nameEntry.-java_util_stream_Collectors-mthref-2(((IPAddressName) name).getName());
                        break;
                    } catch (IOException ioe) {
                        throw new RuntimeException("IPAddress cannot be parsed", ioe);
                    }
                case 8:
                    nameEntry.-java_util_stream_Collectors-mthref-2(((OIDName) name).getOID().toString());
                    break;
                default:
                    DerOutputStream derOut = new DerOutputStream();
                    try {
                        name.encode(derOut);
                        nameEntry.-java_util_stream_Collectors-mthref-2(derOut.toByteArray());
                        break;
                    } catch (IOException ioe2) {
                        throw new RuntimeException("name cannot be encoded", ioe2);
                    }
            }
            newNames.-java_util_stream_Collectors-mthref-2(Collections.unmodifiableList(nameEntry));
        }
    ...
    private static Collection<List<?>> cloneAltNames(Collection<List<?>> altNames) {
        ...
        for (Collection nameEntry2 : altNames) {
            ...
            if (nameObject instanceof byte[]) {
                ...
                namesCopy.-java_util_stream_Collectors-mthref-2(Collections.unmodifiableList(nameEntryCopy));
            } else {
                namesCopy.-java_util_stream_Collectors-mthref-2(nameEntry2);
            }
        }
    ...
    /* JADX WARNING: Removed duplicated region for block: B:7:0x0025 A:{Splitter: B:1:0x0003, ExcHandler: java.security.NoSuchAlgorithmException (e java.security.NoSuchAlgorithmException)} */

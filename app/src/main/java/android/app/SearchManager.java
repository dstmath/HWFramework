...
public class SearchManager implements android.content.DialogInterface.OnDismissListener, android.content.DialogInterface.OnCancelListener {
    ...
    private final String FULL_TEXT_SEARCH_CONFIG;
    private boolean isSupportFullTextSearch;
    ...
    SearchManager(Context context, Handler handler) {
        this.FULL_TEXT_SEARCH_CONFIG = "ro.config.hw_globalSearch";
        this.isSupportFullTextSearch = true;
        this.mDismissListener = null;
        this.mCancelListener = null;
        this.mContext = context;
        this.mHandler = handler;
        setSupportFullTextSearchState();
        mService = Stub.asInterface(ServiceManager.getService(Searches.SEARCH));
    }
    ...
    public void startSearch(String initialQuery, boolean selectInitialQuery, ComponentName launchActivity, Bundle appSearchData, boolean globalSearch, Rect sourceBounds) {
        ...
        if (new UiModeManager().getCurrentModeType() != 4) {
            ensureSearchDialog();
    ...
    public void triggerSearch(String query, ComponentName launchActivity, Bundle appSearchData) {
        ...
        startSearch(query, DBG, launchActivity, appSearchData, DBG);
    ...
    public boolean isVisible() {
        return this.mSearchDialog == null ? DBG : this.mSearchDialog.isShowing();
    }
    ...
    public void trigerGlobalSearchService(boolean enabled) {
        if (enabled) {
            ...
            this.mContext.bindService(intent, this.mCurrentConn, INDEX_SERVICE_BIND_FLAGS);
    ...
    private void setSupportFullTextSearchState() {
        this.isSupportFullTextSearch = SystemProperties.get("ro.config.hw_globalSearch", "true").equals("true") ? true : DBG;
    }
    ...
    public Cursor getSuggestions(SearchableInfo searchable, String query, int limit) {
        ...
        Builder uriBuilder = new Builder().scheme(VoiceInteractionSession.KEY_CONTENT).authority(authority).query(ProxyInfo.LOCAL_EXCL_LIST).fragment(ProxyInfo.LOCAL_EXCL_LIST);
        ...
        String[] strArr = null;
        if (selection != null) {
            strArr = new String[INDEX_SERVICE_BIND_FLAGS];
            strArr[0] = query;
        } else {
    ...
    public List<SearchableInfo> getOnlineSearchablesInGlobalSearch() {
        try {
            return mService.getOnlineSearchablesInGlobalSearch();
        } catch (RemoteException e) {

...
public class MediaEncoderFilter extends Filter {
    ...
    public MediaEncoderFilter(String name) {
        ...
        this.mSourceRegion = new Quad(new Point(0.0f, 0.0f), new Point(Engine.DEFAULT_VOLUME, 0.0f), new Point(0.0f, Engine.DEFAULT_VOLUME), new Point(Engine.DEFAULT_VOLUME, Engine.DEFAULT_VOLUME));
    ...
    public boolean skipFrameAndModifyTimestamp(long timestampNs) {
        if (this.mNumFramesEncoded == 0) {
            ...
            this.mLastTimeLapseFrameRealTimestampNs = timestampNs;
            this.mTimestampNs += Directory.ENTERPRISE_DIRECTORY_ID_BASE / ((long) this.mFps);
            if (this.mLogVerbose) {
                Log.v(TAG, "timelapse: encoding frame, setting t = " + this.mTimestampNs + ", delta t = " + (Directory.ENTERPRISE_DIRECTORY_ID_BASE / ((long) this.mFps)) + ", fps = " + this.mFps);
            }
            return false;
        } else {
            if (this.mLogVerbose) {

...
public class LegacyResultMapper {
    ...
    private static CameraMetadataNative convertResultMetadata(LegacyRequest legacyRequest) {
        ...
        result.set(r16, (Integer) request.get(CaptureRequest.COLOR_CORRECTION_ABERRATION_MODE));
        ...
        key = CaptureResult.REQUEST_PIPELINE_DEPTH;
        result.set(r16, (Byte) characteristics.get(CameraCharacteristics.REQUEST_PIPELINE_MAX_DEPTH));
        mapScaler(result, zoomData, params);
        result.set(CaptureResult.SENSOR_TEST_PATTERN_MODE, Integer.valueOf(0));
        key = CaptureResult.JPEG_GPS_LOCATION;
        result.set(r16, (Location) request.get(CaptureRequest.JPEG_GPS_LOCATION));
        key = CaptureResult.JPEG_ORIENTATION;
        result.set(r16, (Integer) request.get(CaptureRequest.JPEG_ORIENTATION));
        ...
        key = CaptureResult.NOISE_REDUCTION_MODE;
        result.set(r16, (Integer) request.get(CaptureRequest.NOISE_REDUCTION_MODE));
        return result;
    }
    ...
    private static void mapAe(CameraMetadataNative m, CameraCharacteristics characteristics, CaptureRequest request, Rect activeArray, ZoomData zoomData, Parameters p) {
        ...
        boolean autoExposureLock = p.isAutoExposureLockSupported() ? p.getAutoExposureLock() : DEBUG;
    ...
    private static void mapAwb(CameraMetadataNative m, Parameters p) {
        m.set(CaptureResult.CONTROL_AWB_LOCK, Boolean.valueOf(p.isAutoWhiteBalanceLockSupported() ? p.getAutoWhiteBalanceLock() : DEBUG));
    ...
    private static void mapAeAndFlashMode(CameraMetadataNative m, CameraCharacteristics characteristics, Parameters p) {
        ...
        if (!(flashModeSetting == null || flashModeSetting.equals(Parameters.FLASH_MODE_OFF))) {
            if (flashModeSetting.equals(CalendarCache.TIMEZONE_TYPE_AUTO)) {
                aeMode = 2;
    ...
    private static int convertLegacyAfMode(String mode) {
        if (mode == null) {
            ...
            return 0;
        } else if (mode.equals(CalendarCache.TIMEZONE_TYPE_AUTO)) {
            return 1;
        } else {
    ...
    private static int convertLegacyAwbMode(String mode) {
        if (mode == null || mode.equals(CalendarCache.TIMEZONE_TYPE_AUTO)) {
            return 1;
        }

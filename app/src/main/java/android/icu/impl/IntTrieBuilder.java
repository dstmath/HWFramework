...
public class IntTrieBuilder extends TrieBuilder {
    ...
    public IntTrieBuilder(int[] aliasdata, int maxdatalength, int initialvalue, int leadunitvalue, boolean latin1linear) {
        if (maxdatalength < 32 || (latin1linear && maxdatalength < NodeFilter.SHOW_DOCUMENT_FRAGMENT)) {
    ...
    public int getValue(int ch) {
        if (this.m_isCompacted_ || ch > UnicodeSet.MAX_VALUE || ch < 0) {
            return 0;
        }
    ...
    public IntTrie serialize(DataManipulate datamanipulate, Trie.DataManipulate triedatamanipulate) {
        ...
        if (this.m_dataLength_ >= DateUtilsBridge.FORMAT_ABBREV_RELATIVE) {
        ...
        if (this.m_isLatin1Linear_) {
            options = 293 | NodeFilter.SHOW_DOCUMENT_TYPE;
        }
    ...
    public int serialize(OutputStream os, boolean reduceTo16Bits, DataManipulate datamanipulate) throws IOException {
        ...
        if (length >= DateUtilsBridge.FORMAT_ABBREV_RELATIVE) {
            throw new ArrayIndexOutOfBoundsException("Data length too small");
        }
        ...
        if (this.m_isLatin1Linear_) {
            options |= NodeFilter.SHOW_DOCUMENT_TYPE;
        }
        ...
        if (reduceTo16Bits) {
            ...
            for (i = 0; i < this.m_dataLength_; i++) {
                dos.writeChar(this.m_data_[i] & DexFormat.MAX_TYPE_IDX);
            }
        } else {
            for (i = 0; i < this.m_indexLength_; i++) {
    ...
    public boolean setRange(int start, int limit, int value, boolean overwrite) {
        if (this.m_isCompacted_ || start < 0 || start > UnicodeSet.MAX_VALUE || limit < 0 || limit > 1114112 || start > limit) {
            return false;
        }
        if (start == limit) {
            return true;
    ...
    private int getDataBlock(int ch) {
        ...
        System.arraycopy(this.m_data_, Math.abs(indexValue), this.m_data_, newBlock, NodeFilter.SHOW_COMMENT);
        return newBlock;
    }
    ...
    private final void fold(DataManipulate manipulate) {
        ...
        int indexLength = NodeFilter.SHOW_NOTATION;
        c = DateUtilsBridge.FORMAT_ABBREV_MONTH;
        while (c < PropsVectors.INITIAL_VALUE_CP) {
            if (index[c >> 5] != 0) {
                c &= -1024;
        ...
        System.arraycopy(index, NodeFilter.SHOW_NOTATION, index, 2080, indexLength - 2048);
        System.arraycopy(leadIndexes, 0, index, NodeFilter.SHOW_NOTATION, 32);

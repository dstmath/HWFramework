...
public class TrieIterator implements RangeValueIterator {
    ...
    public final boolean next(Element element) {
        if (this.m_nextCodepoint_ > UnicodeSet.MAX_VALUE) {
            return false;
        }
        if (this.m_nextCodepoint_ < DateUtilsBridge.FORMAT_ABBREV_MONTH && calculateNextBMPElement(element)) {
            return true;
        }
    ...
    public final void reset() {
        ...
        this.m_nextTrailIndexOffset_ = TRAIL_SURROGATE_INDEX_BLOCK_LENGTH_;
    }
    ...
    private final boolean calculateNextBMPElement(Element element) {
        ...
        if (checkBlockDetail(currentValue)) {
            while (this.m_nextCodepoint_ < DateUtilsBridge.FORMAT_ABBREV_MONTH) {
                if (this.m_nextCodepoint_ == LEAD_SURROGATE_MIN_VALUE_) {
                    this.m_nextIndex_ = BMP_INDEX_LENGTH_;
                } else if (this.m_nextCodepoint_ == TRAIL_SURROGATE_MIN_VALUE_) {
    ...
    private final void calculateNextSupplementaryElement(Element element) {
        ...
        while (nextLead < TRAIL_SURROGATE_MIN_VALUE_) {
            ...
            if (leadBlock == this.m_trie_.m_dataOffset_) {
                ...
                nextLead += TRAIL_SURROGATE_INDEX_BLOCK_LENGTH_;
                ...
                if (this.m_nextIndex_ > 0) {
                    ...
                    this.m_nextCodepoint_ += TRAIL_SURROGATE_COUNT_;
                }
                nextLead++;
            }
        }
        setResult(element, this.m_currentCodepoint_, PropsVectors.INITIAL_VALUE_CP, currentValue);
    }

    private final boolean checkBlockDetail(int currentValue) {
        while (this.m_nextBlockIndex_ < TRAIL_SURROGATE_INDEX_BLOCK_LENGTH_) {
    ...
    private final boolean checkBlock(int currentValue) {
        ...
        if (this.m_nextBlock_ == currentBlock && this.m_nextCodepoint_ - this.m_currentCodepoint_ >= TRAIL_SURROGATE_INDEX_BLOCK_LENGTH_) {
            this.m_nextCodepoint_ += TRAIL_SURROGATE_INDEX_BLOCK_LENGTH_;
    ...
    private final boolean checkTrailBlock(int currentValue) {
        while (this.m_nextTrailIndexOffset_ < TRAIL_SURROGATE_INDEX_BLOCK_LENGTH_) {
            this.m_nextBlockIndex_ = 0;
            if (!checkBlock(currentValue)) {
                return false;
            }
    ...
    private final boolean checkNullNextTrailIndex() {
        ...
        this.m_nextIndex_ = this.m_trie_.m_dataManipulate_.getFoldingOffset(this.m_trie_.getValue((nextLead & 31) + leadBlock));
        this.m_nextIndex_--;
        this.m_nextBlockIndex_ = TRAIL_SURROGATE_INDEX_BLOCK_LENGTH_;
        return true;
    }
}

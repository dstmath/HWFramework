...
final class UCharacterNameReader implements Authenticate {
    ...
    protected void read(UCharacterName data) throws IOException {
        ...
        char[] group = ICUBinary.getChars(this.m_byteBuffer_, count * GROUP_INFO_SIZE_, 0);
    ...
    private AlgorithmName readAlg() throws IOException {
        ...
        char c = (char) (this.m_byteBuffer_.get() & Opcodes.OP_CONST_CLASS_JUMBO);
        while (c != '\u0000') {
            prefix.append(c);
            c = (char) (this.m_byteBuffer_.get() & Opcodes.OP_CONST_CLASS_JUMBO);
        }
        result.setPrefix(prefix.toString());
        size -= (prefix.length() + ALG_INFO_SIZE_) + 1;
        if (size > 0) {

    ...
    private StringBuilder normalizeSecondAndAppend(StringBuilder first, CharSequence second, boolean doNormalize) {
        if (first == second) {
            ...
            if (prefixLimit != 0) {
                ...
                int suffixStart = this.set.spanBack(first, AnnualTimeZoneRule.MAX_YEAR, SpanCondition.SIMPLE);
                if (suffixStart != 0) {
                    ...
                    first.delete(suffixStart, AnnualTimeZoneRule.MAX_YEAR).append(middle);
                } else if (doNormalize) {

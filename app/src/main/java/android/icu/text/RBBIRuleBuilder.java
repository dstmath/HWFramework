...
class RBBIRuleBuilder {
    ...
    void flattenData(OutputStream os) throws IOException {
        ...
        int totalSize = ((((((forwardTableSize + 96) + reverseTableSize) + safeFwdTableSize) + safeRevTableSize) + statusTableSize) + trieSize) + align8(strippedRules.length() * fSafeFwdTree);
        int outputPos = fForwardTree;
        ICUBinary.writeHeader(1114794784, 50397184, fForwardTree, dos);
        int[] header = new int[24];
        header[fForwardTree] = 45472;
        header[fReverseTree] = 50397184;
        header[fSafeFwdTree] = totalSize;
        header[fSafeRevTree] = this.fSetBuilder.getNumCharCategories();
        header[4] = 96;
        ...
        header[15] = strippedRules.length() * fSafeFwdTree;
        int i = fForwardTree;
        while (true) {
        ...
        outputPos += header[13];
        while (outputPos % 8 != 0) {
            dos.write(fForwardTree);
            outputPos += fReverseTree;
        }
        ...
        for (outputPos += strippedRules.length() * fSafeFwdTree; outputPos % 8 != 0; outputPos += fReverseTree) {
            dos.write(fForwardTree);
        }
    }

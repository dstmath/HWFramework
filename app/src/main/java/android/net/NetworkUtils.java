...
public class NetworkUtils {
    ...
    public static InetAddress intToInetAddress(int hostAddress) {
        try {
            return InetAddress.getByAddress(new byte[]{(byte) (hostAddress & Process.PROC_TERM_MASK), (byte) ((hostAddress >> 8) & Process.PROC_TERM_MASK), (byte) ((hostAddress >> 16) & Process.PROC_TERM_MASK), (byte) ((hostAddress >> 24) & Process.PROC_TERM_MASK)});
    ...
    public static int inetAddressToInt(Inet4Address inetAddr) throws IllegalArgumentException {
        ...
        return ((((addr[3] & Process.PROC_TERM_MASK) << 24) | ((addr[2] & Process.PROC_TERM_MASK) << 16)) | ((addr[1] & Process.PROC_TERM_MASK) << 8)) | (addr[0] & Process.PROC_TERM_MASK);
    }
    ...
    public static void maskRawAddress(byte[] array, int prefixLength) {
        ...
        byte mask = (byte) (Process.PROC_TERM_MASK << (8 - (prefixLength % 8)));
    ...
    public static int getImplicitNetmask(Inet4Address address) {
        int firstByte = address.getAddress()[0] & Process.PROC_TERM_MASK;
        if (firstByte < KeymasterDefs.KM_ALGORITHM_HMAC) {
            return 8;
        }
        if (firstByte < Impl.STATUS_RUNNING) {
            return 16;
        }
        if (firstByte < UsbConstants.USB_CLASS_WIRELESS_CONTROLLER) {
            return 24;
        }
        return 32;
    }

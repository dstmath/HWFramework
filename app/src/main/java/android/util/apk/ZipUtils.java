...
abstract class ZipUtils {
    ...
    private static Pair<ByteBuffer, Long> findZipEndOfCentralDirectoryRecord(RandomAccessFile zip, int maxCommentSize) throws IOException {
        ...
        ByteBuffer buf = ByteBuffer.allocate(((int) Math.min((long) maxCommentSize, fileSize - 22)) + ZIP_EOCD_REC_MIN_SIZE);
    ...
    private static int findZipEndOfCentralDirectoryRecord(ByteBuffer zipContents) {
        ...
        int maxCommentLength = Math.min(archiveSize - 22, UINT16_MAX_VALUE);
        ...
        while (expectedCommentLength < maxCommentLength) {
            ...
            if (zipContents.getInt(eocdStartPos) == ZIP_EOCD_REC_SIG && getUnsignedInt16(zipContents, eocdStartPos + ZIP_EOCD_COMMENT_LENGTH_FIELD_OFFSET) == expectedCommentLength) {
                return eocdStartPos;
            }

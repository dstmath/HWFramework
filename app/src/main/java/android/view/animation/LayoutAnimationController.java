...
public class LayoutAnimationController {
    ...
    public LayoutAnimationController(Context context, AttributeSet attrs) {
        ...
        this.mDelay = Description.parseValue(a.peekValue(ORDER_REVERSE)).value;
        this.mOrder = a.getInt(3, ORDER_NORMAL);
        int resource = a.getResourceId(ORDER_RANDOM, ORDER_NORMAL);
        if (resource > 0) {
            setAnimation(context, resource);
        }
        resource = a.getResourceId(ORDER_NORMAL, ORDER_NORMAL);
        if (resource > 0) {
    ...
    public boolean willOverlap() {
        return this.mDelay < LayoutParams.BRIGHTNESS_OVERRIDE_FULL;
    }

...
public class AlphabetIndexer extends DataSetObserver implements SectionIndexer {
    ...
    public int getPositionForSection(int sectionIndex) {
        ...
        char key = letter;
        int pos = alphaMap.get(key, RtlSpacingHelper.UNDEFINED);
        if (RtlSpacingHelper.UNDEFINED != pos) {
            if (pos >= 0) {
                return pos;
            }
            end = -pos;
        ...
        if (sectionIndex > 0) {
            int prevLetterPos = alphaMap.get(this.mAlphabet.charAt(sectionIndex - 1), RtlSpacingHelper.UNDEFINED);
            if (prevLetterPos != RtlSpacingHelper.UNDEFINED) {

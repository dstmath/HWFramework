...
public class HsOpenSSLSocketFactory extends OpenSSLSocketFactoryImpl {
    ...
    public HsOpenSSLSocketFactory(SSLSocketFactory ssl) {
    ...
    private void setEnableSafeCipherSuites(Socket socket) {
        if (HwLog.HWFLOW) {
            HwLog.d(LOG_TAG, "enter setEnableSafeCipherSuites");
        }
        if (socket instanceof SSLSocket) {
            SSLSocket sslsock = (SSLSocket) socket;
            String[] cipherSuites = sslsock.getEnabledCipherSuites();
            if (cipherSuites == null || cipherSuites.length == 0) {
                HwLog.w(LOG_TAG, "Current enabled cipherSuites is invalid!");
                return;
            }
        ...
        HwLog.e(LOG_TAG, "socket is not instanceof SSLSocket");
    }
}

...
public final class CodedInputByteBufferNano {
    ...
    public int readRawVarint32() throws IOException {
        ...
        int result = tmp & LogPower.MIME_TYPE;
        tmp = readRawByte();
        if (tmp >= null) {
            result |= tmp << 7;
        } else {
            result |= (tmp & LogPower.MIME_TYPE) << 7;
            tmp = readRawByte();
            if (tmp >= null) {
                result |= tmp << 14;
            } else {
                result |= (tmp & LogPower.MIME_TYPE) << 14;
                tmp = readRawByte();
                if (tmp >= null) {
                    result |= tmp << 21;
                } else {
                    result |= (tmp & LogPower.MIME_TYPE) << 21;
                    tmp = readRawByte();
                    result |= tmp << 28;
                    if (tmp < null) {
                        for (int i = 0; i < 5; i++) {
    ...
    public long readRawVarint64() throws IOException {
        ...
        for (int shift = 0; shift < DEFAULT_RECURSION_LIMIT; shift += 7) {
            byte b = readRawByte();
            result |= ((long) (b & LogPower.MIME_TYPE)) << shift;
            if ((b & LogPower.START_CHG_ROTATION) == 0) {
                return result;
            }
        }
        throw InvalidProtocolBufferNanoException.malformedVarint();
    ...
    public int readRawLittleEndian32() throws IOException {
        return (((readRawByte() & MetricsEvent.ACTION_DOUBLE_TAP_POWER_CAMERA_GESTURE) | ((readRawByte() & MetricsEvent.ACTION_DOUBLE_TAP_POWER_CAMERA_GESTURE) << 8)) | ((readRawByte() & MetricsEvent.ACTION_DOUBLE_TAP_POWER_CAMERA_GESTURE) << 16)) | ((readRawByte() & MetricsEvent.ACTION_DOUBLE_TAP_POWER_CAMERA_GESTURE) << 24);
    }
    ...
    private CodedInputByteBufferNano(byte[] buffer, int off, int len) {
        this.currentLimit = HwBootFail.STAGE_BOOT_SUCCESS;
    ...
    public int getBytesUntilLimit() {
        if (this.currentLimit == HwBootFail.STAGE_BOOT_SUCCESS) {
            return -1;
        }

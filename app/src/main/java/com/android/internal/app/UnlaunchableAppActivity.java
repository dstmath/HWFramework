...
public class UnlaunchableAppActivity extends Activity implements OnDismissListener, OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (this.mUserId == -10000) {
            ...
            finish();
        } else if (this.mReason == UNLAUNCHABLE_REASON_QUIET_MODE) {
            ...
            if (this.mReason == UNLAUNCHABLE_REASON_QUIET_MODE) {
    ...
    public void onClick(DialogInterface dialog, int which) {
        if (this.mReason == UNLAUNCHABLE_REASON_QUIET_MODE && which == -1 && UserManager.get(this).trySetQuietModeDisabled(this.mUserId, this.mTarget) && this.mTarget != null) {
            try {

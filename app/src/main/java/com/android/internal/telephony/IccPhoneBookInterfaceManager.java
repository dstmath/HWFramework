...
public abstract class IccPhoneBookInterfaceManager extends AbstractIccPhoneBookInterfaceManager {
    ...
    public int[] getAdnRecordsSize(int efid) {
        ...
        synchronized (this.mLock) {
            checkThread();
            this.mRecordSize = new int[EVENT_UPDATE_DONE];
            AtomicBoolean status = new AtomicBoolean(ALLOW_SIM_OP_IN_UI_THREAD);
            Message response = this.mBaseHandler.obtainMessage(EVENT_GET_SIZE_DONE, status);
    ...
    private int updateEfForIccType(int efid) {
        if (efid == IccConstants.EF_CSIM_LI && (this.mPhone.getCurrentUiccAppType() == AppType.APPTYPE_USIM || this.mPhone.getCurrentUiccAppType() == AppType.APPTYPE_CSIM || this.mPhone.getCurrentUiccAppType() == AppType.APPTYPE_ISIM)) {

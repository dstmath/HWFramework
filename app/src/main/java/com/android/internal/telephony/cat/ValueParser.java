...
abstract class ValueParser {
    ...
    static Item retrieveItem(ComprehensionTlv ctlv) throws ResultException {
        ...
        try {
            return new Item(rawValue[valueIndex] & PduHeaders.STORE_STATUS_ERROR_END, HwTelephonyFactory.getHwUiccManager().adnStringFieldToStringForSTK(rawValue, valueIndex + 1, length - 1));
        } catch (IndexOutOfBoundsException e) {
            throw new ResultException(ResultCode.CMD_DATA_NOT_UNDERSTOOD);
        }
    }
    ...
    static IconId retrieveIconId(ComprehensionTlv ctlv) throws ResultException {
        ...
        try {
            if ((rawValue[valueIndex] & PduHeaders.STORE_STATUS_ERROR_END) == 0) {
                z = true;
            }
    ...
    static String retrieveAlphaId(ComprehensionTlv ctlv) throws ResultException {
        ...
        try {
            noAlphaUsrCnf = Resources.getSystem().getBoolean(17956997);
    ...
    static String retrieveTextString(ComprehensionTlv ctlv) throws ResultException {
        ...
        try {
            ...
            if (codingScheme == null || codingScheme == 12) {
    ...
    static int retrieveBufferSize(ComprehensionTlv ctlv) throws ResultException {
        ...
        int valueIndex = ctlv.getValueIndex();
        try {
            return ((rawValue[valueIndex] & PduHeaders.STORE_STATUS_ERROR_END) << 8) | (rawValue[valueIndex + 1] & PduHeaders.STORE_STATUS_ERROR_END);
        } catch (IndexOutOfBoundsException e) {
            throw new ResultException(ResultCode.CMD_DATA_NOT_UNDERSTOOD);
        }
    }
    ...
    static InterfaceTransportLevel retrieveInterfaceTransportLevel(ComprehensionTlv ctlv) throws ResultException {
        ...
        int valueIndex = ctlv.getValueIndex();
        try {
            return new InterfaceTransportLevel(((rawValue[valueIndex + 1] & PduHeaders.STORE_STATUS_ERROR_END) << 8) | (rawValue[valueIndex + 2] & PduHeaders.STORE_STATUS_ERROR_END), TransportProtocol.values()[rawValue[valueIndex] & PduHeaders.STORE_STATUS_ERROR_END]);
        } catch (IndexOutOfBoundsException e) {
            throw new ResultException(ResultCode.CMD_DATA_NOT_UNDERSTOOD);
        }
    }
    ...
    static byte[] retrieveOtherAddress(ComprehensionTlv ctlv) throws ResultException {
        ...
        try {
            int addrType = rawValue[valueIndex] & PduHeaders.STORE_STATUS_ERROR_END;

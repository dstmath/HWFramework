...
public class DcController extends StateMachine {
    ...
    /* renamed from: com.android.internal.telephony.dataconnection.DcController.1 */
    ...
    private class DccDefaultState extends State {
        ...
        private void onDataStateChanged(ArrayList<DataCallResponse> dcsList) {
                        ...
                        if (newState.active == 0) {
                            ...
                            if (result.oldLp.equals(result.newLp)) {
                                ...
                                if (result.oldLp.isIdenticalInterfaceName(result.newLp)) {
                                    ...
                                    if (!(DcController.this.mPhone.getPhoneType() == DcController.DATA_CONNECTION_ACTIVE_PH_LINK_UP && isSupportRcnInd)) {
                                        ...
                                        if (needToClean) {
                                            ...
                                            HashMap hashMap = dc2.mApnContexts;
                                            r0.log("onDataStateChanged: addr change, cleanup apns=" + r0 + " oldLp=" + result.oldLp + " newLp=" + result.newLp);
                                            apnsToCleanup.addAll(dc2.mApnContexts.keySet());
                                        } else {
                                    ...
                                    DcController.this.log("onDataStateChanged: no changes, is CDMA Phone:" + (DcController.this.mPhone.getPhoneType() == DcController.DATA_CONNECTION_ACTIVE_PH_LINK_UP ? DcController.DBG : false) + " is Supprot Reconnect ind:" + isSupportRcnInd);
                                } else {
                                    apnsToCleanup.addAll(dc2.mApnContexts.keySet());
            ...
            for (ApnContext apnContext22 : apnsToCleanup) {
                DcController.this.mDct.sendCleanUpConnection(DcController.DBG, apnContext22);
            }
    ...
    private DcController(String name, Phone phone, DcTracker dct, Handler handler) {
        ...
        log("X ctor");
        this.mPhoneStateListener = new AnonymousClass1(handler.getLooper());
    ...
    void dispose() {
        ...
        if (this.mTelephonyManager != null) {
            this.mTelephonyManager.listen(this.mPhoneStateListener, DATA_CONNECTION_ACTIVE_PH_LINK_INACTIVE);
        }
        quitNow();
    }

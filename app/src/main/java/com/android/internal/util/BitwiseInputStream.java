...
public class BitwiseInputStream {
    ...
    public int read(int bits) throws AccessException {
        ...
        int data = (this.mBuf[index] & MetricsEvent.ACTION_DOUBLE_TAP_POWER_CAMERA_GESTURE) << 8;
        if (offset < 8) {
            data |= this.mBuf[index + 1] & MetricsEvent.ACTION_DOUBLE_TAP_POWER_CAMERA_GESTURE;
        }

...
public class ButtonBarLayout extends LinearLayout {
    ...
    public ButtonBarLayout(Context context, AttributeSet attrs) {
        ...
        boolean allowStackingDefault = context.getResources().getConfiguration().screenHeightDp >= ALLOW_STACKING_MIN_HEIGHT_DP;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (isStacked() || MeasureSpec.getMode(widthMeasureSpec) != EditorInfo.IME_FLAG_NO_ENTER_ACTION) {
            initialWidthMeasureSpec = widthMeasureSpec;
        } else {
            initialWidthMeasureSpec = MeasureSpec.makeMeasureSpec(widthSize, RtlSpacingHelper.UNDEFINED);
            needsRemeasure = true;
        }
        super.onMeasure(initialWidthMeasureSpec, heightMeasureSpec);
        if (this.mAllowStacking && !isStacked() && (getMeasuredWidthAndState() & View.MEASURED_STATE_MASK) == AsyncService.CMD_ASYNC_SERVICE_DESTROY) {
            setStacked(true);
            needsRemeasure = true;
        }
        if (needsRemeasure) {

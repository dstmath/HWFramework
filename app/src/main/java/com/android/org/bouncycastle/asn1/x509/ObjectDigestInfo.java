...
public class ObjectDigestInfo extends ASN1Object {
    ...
    private ObjectDigestInfo(ASN1Sequence seq) {
        ...
        this.digestedObjectType = ASN1Enumerated.getInstance(seq.getObjectAt(publicKey));
        int offset = publicKey;
        if (seq.size() == 4) {
            this.otherObjectTypeID = ASN1ObjectIdentifier.getInstance(seq.getObjectAt(publicKeyCert));
            offset = publicKeyCert;
        }
        this.digestAlgorithm = AlgorithmIdentifier.getInstance(seq.getObjectAt(offset + publicKeyCert));
        this.objectDigest = DERBitString.getInstance(seq.getObjectAt(offset + otherObjectDigest));
    }

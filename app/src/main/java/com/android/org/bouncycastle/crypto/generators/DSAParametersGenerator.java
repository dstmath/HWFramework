...
/*
        Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: com.android.org.bouncycastle.crypto.generators.DSAParametersGenerator.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: com.android.org.bouncycastle.crypto.generators.DSAParametersGenerator.<clinit>():void
                ...
                if (isProbablePrime(q)) {
                    ...
                    for (int counter = 0; counter < 4096; counter++) {
                        ...
                        if (p.bitLength() == this.L && isProbablePrime(p)) {
                            ...
                            return new DSAParameters(p, q, calculateGenerator_FIPS186_2(p, q, secureRandom), new DSAValidationParameters(seed, counter));
                        }
                    }
                    continue;
                }
        ...
        while (true) {
            ...
            if (isProbablePrime(q)) {
                ...
                while (counter < counterLimit) {
                    ...
                    BigInteger shiftLeft = q.shiftLeft(1);
                    p = X.subtract(X.mod(r24).subtract(ONE));
                    if (p.bitLength() == this.L && isProbablePrime(p)) {
                        break loop0;
                    }
                    counter++;

...
public final class HwSdLockService implements INativeDaemonConnectorCallbacks, Monitor {
    ...
    private static final String[] COLUMNS_FOR_QUERY_IV;
    private static final String[] COLUMNS_FOR_QUERY_VALUE;
    ...
    private static volatile HwSdLockService mInstance;
    private final Object lock;
    private final BroadcastReceiver mBroadcastReceiver;
    private NativeDaemonConnector mConnector;
    private Context mContext;
    private Runnable mEraseStateRunable;
    private boolean mErasingSDLock;
    private String mExternalStorageVolumeID;
    private boolean mForceEraseCompleted;
    ...
    /* renamed from: com.android.server.HwSdLockService.5 */
    class AnonymousClass5 implements Runnable {
        final /* synthetic */ String val$key;

        AnonymousClass5(String val$key) {
            this.val$key = val$key;
        }

        public void run() {
            HwSdLockService.this.handleEraseSDLock(this.val$key);
        }
    }
    ...
    static {
        mInstance = null;
        DEFAULT = new Object();
        COLUMNS_FOR_QUERY_VALUE = new String[]{COLUMN_VALUE};
        COLUMNS_FOR_QUERY_IV = new String[]{COLUMN_IV};
    }
    ...
    public HwSdLockService(Context context) {
        this.mForceEraseCompleted = DEBUG;
        this.lock = new Object();
        this.mSystemReady = DEBUG;
        this.mBroadcastReceiver = new BroadcastReceiver() {
            public void onReceive(Context context, Intent intent) {
                if (intent == null) {
                    Log.i(HwSdLockService.TAG, "mBroadcastReceiver onReceive, the intent is null!");
                    return;
                }
                if ("android.intent.action.BOOT_COMPLETED".equals(intent.getAction())) {
                    HwSdLockService.this.autoUnlockSD();
                    HwSdLockService.this.mSystemReady = true;
                }
            }
        };
        ...
        this.mErasingSDLock = DEBUG;
        this.mExternalStorageVolumeID = null;
        this.mProgressDialog = null;
        this.mEraseStateRunable = new Runnable() {
            public void run() {
                HwSdLockService.this.mErasingSDLock = HwSdLockService.DEBUG;
            }
        };
        ...
        bootCompleteFilter.setPriority(IOTController.TYPE_MASTER);
    ...
    public void eraseSDLock() {
        ...
        if (this.mErasingSDLock) {
            ...
            this.mHandler.post(new Runnable() {
                public void run() {
                    if (HwSdLockService.this.mProgressDialog == null) {
                        ...
                        HwSdLockService.this.mProgressDialog.setMessage(HwSdLockService.this.mContext.getText(17040503));
                    }
            ...
            new Thread(new AnonymousClass5(key)).start();
        }
    }
    ...
    private void showToast() {
        this.mHandler.post(new Runnable() {
            public void run() {
                Toast.makeText(HwSdLockService.this.mContext, HwSdLockService.this.mContext.getString(33685822), 0).show();
            }
        });
    }
    ...
    private int formatSD() {
        int ret = -1;
        IMountService service = Stub.asInterface(ServiceManager.getService("mount"));
        if (service != null) {
    ...
    private boolean autoUnlockSD() {
        ...
        if (key != null) {
            ...
            if (encryptPwd == null || encryptPwdIv == null) {
                ...
                if (unlockSDCard(decryptPwd) == RESPONSE_CODE_OK) {
    ...
    private byte[] genAESKey() throws Exception {
        ...
        return PBKDF2WithHmacSHA1.generateStorngPasswordHash(new String(genMasterKey(), Charset.defaultCharset()), new String(Calculator.xor(s1, imei.getBytes(Charset.defaultCharset())), Charset.defaultCharset()), IOTController.TYPE_MASTER);
    }

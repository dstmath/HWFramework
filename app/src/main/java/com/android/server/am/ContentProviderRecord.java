...
public final class ContentProviderRecord {
    ...
    public ContentProviderRecord(ActivityManagerService _service, ProviderInfo _info, ApplicationInfo ai, ComponentName _name, boolean _singleton) {
        ...
        boolean hsmProvider = false;
        if (_info.processName != null && _info.processName.contains("com.huawei.systemmanager")) {
            Slog.d(TAG, "Install provider in hsm ui process, name:" + this.name);
            hsmProvider = true;
        }
        if ((this.uid == 0 || this.uid == ProcessList.PSS_SAFE_TIME_FROM_STATE_CHANGE) && !hsmProvider) {
            z = true;
        }
    ...
    public String toString() {
        ...
        sb.append(" euid:");
        sb.append(this.info.applicationInfo != null ? this.info.applicationInfo.euid : 0);
        sb.append(' ');

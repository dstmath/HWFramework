...
public class WechatGrabController extends BaseGrabController {
    ...
    private Map<String, String> mConfigParams;
    ...
    protected void handleWinStateChangeEvent(AccessibilityEvent event) {
        ...
        if (className.endsWith(launcherUi) && this.mState == TOUCH_DISABLE_ENABLE) {
            Log.d(TAG, "enter the wechat launcher UI");
            if (this.mReceiveGrabCmd) {
                searchPacket();
            }
        } else if (className.endsWith(receiveUi)) {
            if (this.mReceiveGrabCmd && this.mState == GRAB_TIMEOUT) {
                openPacket();
            } else if (!this.mReceiveGrabCmd && this.mState == TOUCH_DISABLE_ENABLE) {
                Log.d(TAG, "enter receuve ui,send cancle message");
                afterGrabPacket(false);
                sendMessage((byte) 3);
            }
        } else if (className.endsWith(detailUi)) {
            if (this.mState == FLAG_STATE_PKG_OPEND_SUCCESS) {
                this.mIsWaittingResult = false;
                afterGrabPacket(true);
            } else if (this.mState == GRAB_TIMEOUT) {
                afterGrabPacket(false);
            }
    ...
    private void onNewPacketReceive(WechatMessage localWechatMessage) {
        ...
        sendMessage((byte) 1);
        if (this.mState == 0) {
            this.mState = TOUCH_DISABLE_ENABLE;
        }
        this.mHandler.sendMessageDelayed(Message.obtain(this.mHandler, FLAG_STATE_PKG_OPEND_SUCCESS, TOUCH_DISABLE_ENABLE, this.mNotifyId), 30000);
    }

    private void sendMessage(byte eventType) {
        this.mHandler.obtainMessage(GRAB_TIMEOUT, new BluetoothMessage((byte) 1, eventType, (byte) 1, AutoGrabTools.intToByteArray(this.mNotifyId))).sendToTarget();
    }

    private void afterGrabPacket(boolean result) {
        Log.d(TAG, "grab success:" + result);
        if (this.mIsDeviceLockedBefore) {
            this.mService.performGlobalAction(TOUCH_DISABLE_ENABLE);
            backToHome();
            AutoGrabTools.lockDevice(this.mService);
        }
        if (this.mIsTouchDisableBefore) {
            System.putInt(this.mService.getContentResolver(), KEY_TOUCH_DISABLE_MODE, TOUCH_DISABLE_ENABLE);
        }
        resetParams();
    ...
    private int getDelay() {
        int baseDelay = TOUCH_DISABLE_ENABLE;
    ...
    private void openPacket() {
        ...
        if (openButton != null) {
            ...
            this.mState = FLAG_STATE_PKG_OPEND_SUCCESS;
    ...
    private AccessibilityNodeInfo getOpenPacketBtn() {
        ...
        while (!stack.isEmpty()) {
            ...
            if (k > 0) {
                for (int i = TOUCH_DISABLE_DISABLE; i < k; i += TOUCH_DISABLE_ENABLE) {
    ...
    private AccessibilityNodeInfo getOpenPacketFailNode() {
        ...
        return (AccessibilityNodeInfo) nodes.get(TOUCH_DISABLE_DISABLE);
    }
    ...
    private void sendPendingIntent() {
        ...
        try {
            ...
            this.mHandler.sendMessageDelayed(Message.obtain(this.mHandler, FLAG_STATE_PKG_OPEND_SUCCESS, GRAB_TIMEOUT, this.mGrabingId), 25000);
    ...
    public void executeCommand(int cmdId, int notifyId) {
        ...
        switch (cmdId) {
            ...
            case LifeCycleStateMachine.DELETE_ACCOUNT /*6*/:
                if (this.mState == TOUCH_DISABLE_ENABLE) {
                    this.mReceiveGrabCmd = true;
                    if (System.getInt(this.mService.getContentResolver(), KEY_TOUCH_DISABLE_MODE, TOUCH_DISABLE_ENABLE) != TOUCH_DISABLE_ENABLE) {
                        z = false;
                    }
                    this.mIsTouchDisableBefore = z;
                    System.putInt(this.mService.getContentResolver(), KEY_TOUCH_DISABLE_MODE, TOUCH_DISABLE_DISABLE);
    ...
    private void resetParams() {
        this.mState = TOUCH_DISABLE_DISABLE;
        this.mNotifyId = TOUCH_DISABLE_DISABLE;
        this.mGrabingId = TOUCH_DISABLE_DISABLE;
    ...
    public void parseGrabParams(String params) {
        ...
        String[] items = params.split(CONFIG_ITEM_SPLITER);
        int length = items.length;
        for (int i = TOUCH_DISABLE_DISABLE; i < length; i += TOUCH_DISABLE_ENABLE) {
            String[] entry = items[i].split(CONFIG_ENTRY_SPLITER);
            if (entry.length == GRAB_TIMEOUT) {
                this.mConfigParams.put(entry[TOUCH_DISABLE_DISABLE].trim(), entry[TOUCH_DISABLE_ENABLE].trim());
            } else {

...
public class HwGnssLocationProvider extends GnssLocationProvider {
    ...
    private boolean AUTO_ACC_Enable;
    ...
    /* renamed from: com.android.server.location.HwGnssLocationProvider.1 */
    ...
    private class BroadcastHelper {
        ...
        public BroadcastHelper() {
            this.innerBroadcastReciever = new BroadcastReceiver() {
        ...
        private void checkAGpsServer(Intent intent) {
            ...
            if (supl_host != null) {
                ...
                HwGnssLocationProvider.this.utils.native_set_agps_server(HwGnssLocationProvider.this, HwGnssLocationProvider.GNSS_REQ_CHANGE_START, supl_host, HwGnssLocationProvider.this.utils.getSuplServerPort(HwGnssLocationProvider.this));
            }
        }
    }
    ...
    public HwGnssLocationProvider(Context context, ILocationManager ilocationManager, Looper looper) {
        ...
        this.AUTO_ACC_Enable = SystemProperties.getBoolean("ro.config.hw_auto_acc_enable", false);
        this.mNetworkChecker = new HwNetworkPropertyChecker(context, (WifiManager) context.getSystemService(GnssConnectivityLogManager.SUBSYS_WIFI), null, true, null, true);
    ...
    public void initDefaultApnObserver(Handler handler) {
        this.utils.setDefaultApnObserver(this, new AnonymousClass1(handler));
    }
    ...
    public int getPreferred_accuracy() {
        ...
        int accuracy_set = 0;
        if (existLocationMap()) {
            accuracy_set = WifiProCommonUtils.HTTP_REACHALBE_HOME;
        }
    ...
    public boolean shouldReStartNavi() {
        ...
        if ((this.mPreferAccuracy == WifiProCommonUtils.HTTP_REACHALBE_HOME || !existLocationMap()) && (this.mPreferAccuracy != WifiProCommonUtils.HTTP_REACHALBE_HOME || existLocationMap())) {
            return false;
        }
        return true;
    }
    ...
    public void handleGnssRequirementsChange(int reson) {
        if (reson == GNSS_REQ_CHANGE_START) {
            handleGnssRequirementsChange(existLocationMap());
        } else if (reson == GNSS_REQ_CHANGE_STOP) {
    ...
    private void handleGnssRequirementsChange(boolean mapNavigating) {
        ...
        if (this.isLastExistMapLocation != mapNavigating) {
            ...
            Secure.putInt(this.mContext.getContentResolver(), MAPS_LOCATION_FLAG, mapNavigating ? GNSS_REQ_CHANGE_START : 0);
    ...
    public void handleGnssNavigatingStateChange(boolean start) {
        ...
        Secure.putInt(this.mContext.getContentResolver(), GNSS_NAVIGATING_FLAG, start ? GNSS_REQ_CHANGE_START : 0);
    }
}

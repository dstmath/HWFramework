...
public class HwCustPowerManagerServiceImpl extends HwCustPowerManagerService {
    ...
    private boolean mCharingBase;
    private UEventObserver mCharingObserver;
    private Context mContext;
    private DreamManagerInternal mDreamManager;
    private boolean mEnableC2DMDelay;
    private ContentObserver mSettingsObserver;
    private boolean startDreamFromUser;
    ...
    public class C2DMHelperHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case HwCustPowerManagerServiceImpl.CHARING_MODE_USB /*1*/:
    ...
    static {
        boolean z = HWLOGW_E;
        boolean isLoggable = !Log.HWLog ? Log.HWModuleLog ? Log.isLoggable(TAG, 3) : HWFLOW : HWLOGW_E;
        HWDBG = isLoggable;
        if (!Log.HWINFO) {
    ...
    public HwCustPowerManagerServiceImpl(Context context) {
        super(context);
        this.mEnableC2DMDelay = SystemProperties.getBoolean("ro.config.enable_c2dm_delay", HWFLOW);
        this.startDreamFromUser = HWFLOW;
        this.mCharingBase = HWFLOW;
        this.mCharingObserver = new UEventObserver() {
            public void onUEvent(UEvent event) {
                ...
                if (HwCustPowerManagerServiceImpl.CHARING_UEVENT_CONNECTED.equals(state)) {
                    ...
                    HwCustPowerManagerServiceImpl.this.mCharingBase = HwCustPowerManagerServiceImpl.HWFLOW;
                }
            }
        };
    ...
    public void systemReady(BatteryManagerInternal batterymanager, DreamManagerInternal dreammanager, ContentObserver observer) {
        if (isChargingAlbumSupported()) {
            ...
            resolver.registerContentObserver(System.getUriFor(CHARGING_ALBUM_ENABLED), HWFLOW, this.mSettingsObserver, -1);
            resolver.registerContentObserver(System.getUriFor(CHARGING_ALBUM_MODE), HWFLOW, this.mSettingsObserver, -1);
        }
    }
    ...
    public void updateSettingsLocked() {
        ...
        if (isChargingAlbumSupported()) {
            ContentResolver resolver = this.mContext.getContentResolver();
            if (System.getInt(resolver, CHARGING_ALBUM_ENABLED, CHARING_MODE_USB) == 0) {
                z = HWFLOW;
            }
            this.mChargingAlbumEnabled = z;
            this.mChargingMode = System.getInt(resolver, CHARGING_ALBUM_MODE, CHARING_MODE_BASE);
        }
    }

    public boolean isStartDreamFromUser() {
        return isChargingAlbumSupported() ? this.startDreamFromUser : HWFLOW;
    }
    ...
    public void init(Context context) {
        if (context != null) {
            ...
            if (pm != null) {
                this.mC2DMWakeLock = pm.newWakeLock(CHARING_MODE_USB, "C2DMHandlerThread");
            }
    ...
    public void checkDelay(String tagName) {
        if ("google_c2dm".compareToIgnoreCase(tagName) == 0) {
            if (this.mC2DMWakeLock == null) {
                ...
                this.mC2DMHelperHandler.sendMessage(this.mC2DMHelperHandler.obtainMessage(CHARING_MODE_USB));
            }
        }
    }
}

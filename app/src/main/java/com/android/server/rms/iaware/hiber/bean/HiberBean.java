...
public class HiberBean implements Parcelable {
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        if (flags == 1) {
            if (this.pkgName == null || this.pkgName.trim().isEmpty()) {
                AwareLog.w(TAG, " pkgName is Empty ");
                dest.recycle();
                return;
            } else if (this.pidArray == null || this.pidArray.length == 0) {
                AwareLog.w(TAG, " pidArray is Empty ");
                dest.recycle();
                return;
            } else {

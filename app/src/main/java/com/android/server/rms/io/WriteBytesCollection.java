...
public class WriteBytesCollection implements Reader {
    private static final String TAG = "IO.LifeTimeCollection";
    private List<WriteBytesData> mDataList;

    static class TotalWrittenRewriter implements Rewriter {
        private WriteBytesCollection mWriteBytesCollection;

        public TotalWrittenRewriter(WriteBytesCollection collection) {
            this.mWriteBytesCollection = null;
    ...
    public void write(OutputStream out) throws IOException {
        ...
        for (WriteBytesData data : this.mDataList) {
            if (Utils.getDifferencesByDay(currentTime, data.mTime) <= 7) {

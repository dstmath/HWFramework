...
public class AuthUtils {
    ...
    public static OnAuthSyncInfo processAuthSync(InitAuthInfo info) {
        ...
        if (resultCode != 0) {
            return new OnAuthSyncInfo(info.mAuthID, new byte[0], FATS_AUTH, (short) 1, 1, (short) 1, new byte[0], new byte[0], resultCode);
            ...
            return new OnAuthSyncInfo(info.mAuthID, new byte[0], FATS_AUTH, (short) 1, 1, (short) 1, new byte[0], new byte[0], 2046820353);
        }
    }
    ...
    public static OnAuthSyncAckInfo processAuthSyncRec(RecAuthInfo info) {
        ...
        if (isNeedRequestPk(result)) {
            resultCode = FATS_AUTH;
        }
        LogHelper.d(TAG, "processAuthSyncRec result, resultCode: " + Integer.toHexString(resultCode));
        TLVTree ret = tlvResult.getResultTLV();
        if (resultCode != 0) {
            return new OnAuthSyncAckInfo(info.mAuthID, new byte[0], FATS_AUTH, 1, new byte[0], (short) 1, new byte[0], new byte[0], resultCode);
            ...
            return new OnAuthSyncAckInfo(info.mAuthID, new byte[0], FATS_AUTH, 1, new byte[0], (short) 1, new byte[0], new byte[0], 2046820353);
        }
    }
    ...
    public static OnAuthAckInfo processAckRec(RecAckInfo info) {
        ...
        if (resultCode != 0) {
            return new OnAuthAckInfo(info.mAuthID, new byte[0], new byte[0], new byte[0], resultCode);
        } else if (ret instanceof RET_AUTH_ACK_RECV) {
    ...
    public static RespPkInfo processGetPk(ReqPkInfo info) {
        ...
        } else if (ret instanceof RET_GET_PK) {
    ...
    private static boolean isNeedRequestPk(byte[] result) {
        return result != null && result.length >= 8 && result[4] == null && result[5] == null && result[6] == null && result[7] == (byte) 1;
    }
}

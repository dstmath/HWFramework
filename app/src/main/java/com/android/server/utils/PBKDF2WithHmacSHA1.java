...
public class PBKDF2WithHmacSHA1 {
    ...
    public static byte[] generateStorngPasswordHash(String mainKey, String salt, int iterations) throws NoSuchAlgorithmException, InvalidKeySpecException {
        return SecretKeyFactory.getInstance("PBKDF2WithHmacSHA1").generateSecret(new PBEKeySpec(mainKey.toCharArray(), salt.getBytes(StandardCharsets.UTF_8), iterations, HwGlobalActionsData.FLAG_SILENTMODE_SILENT)).getEncoded();
    }
}

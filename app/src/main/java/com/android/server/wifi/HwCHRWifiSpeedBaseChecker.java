...
class HwCHRWifiSpeedBaseChecker {
    ...
    public HwCHRWifiSpeedBaseChecker() {
        ...
        this.recovage_times = ACCESS_OK;
        this.suckTimes = ACCESS_OK;
        this.age = RESEND_COUNT;
        this.counters = null;
        this.mFailReason = ACCESS_OK;
    ...
    public long getCounterDetaByTab(String tag) {
        for (int i = ACCESS_OK; i < this.counters.size(); i += ACCESS_SLOWLY_RTT) {
    ...
    protected void checkSuckStatus() {
        if (this.mNeedChecked) {
            ...
            if (rtt_packets < 3 || rtt < RTT_THRESHOLD_400) {
                ...
                if (rtt_packets < 3 && rtt > RTT_THRESHOLD_400 && send_packet >= 5 && ((double) resend_packets) / ((double) send_packet) > RESEND_RATE) {
                    this.suckTimes += ACCESS_SLOWLY_RTT;
                    this.recovage_times = ACCESS_OK;
                    this.mFailReason = SUCT_TIME_COUNT;
            ...
            this.recovage_times = ACCESS_OK;
            this.suckTimes += ACCESS_SLOWLY_RTT;
            this.mFailReason = ACCESS_SLOWLY_RTT;

...
public class FaceMetaSmoothFilter extends Filter {
    ...
    private Vector<FacePos> getCurrentPositions(FaceMeta face) {
        ...
        for (int i = 0; i < num_face; i++) {
            ...
            if (this.mLastPositions.containsKey(Integer.valueOf(p.id))) {
                ...
                float dx = ((p.left_eye_x - v.left_eye_x) + (p.right_eye_x - v.right_eye_x)) / 2.0f;
                float dy = ((p.left_eye_y - v.left_eye_y) + (p.right_eye_y - v.right_eye_y)) / 2.0f;
                float dt = ((float) (t - v.last_seen)) / 1000.0f;
                p.speed_x = v.speed_x + (((dx / dt) - v.speed_x) * 0.5f);
            ...
            if (f.last_seen < t - 300) {
                ...
                if (f.last_seen != t) {
                    if (f.last_seen > t - 300) {
                        p = f.clone();
                        float f2 = f.speed_x;
                        dx = (r0 / 1000.0f) * ((float) (t - f.last_seen));
                        f2 = f.speed_x;
                        dy = (r0 / 1000.0f) * ((float) (t - f.last_seen));
                        p.left_eye_x += dx;
                        p.left_eye_y += dy;
                        p.right_eye_x += dx;
                        p.right_eye_y += dy;
    ...
    public FaceMetaSmoothFilter(String name) {
        ...
        this.mMaximumGap = 300;
        this.mMaximumPredictionGap = 300;

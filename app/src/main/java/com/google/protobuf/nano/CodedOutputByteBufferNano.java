...
public final class CodedOutputByteBufferNano {
    ...
    private static void encodeDirect(CharSequence sequence, ByteBuffer byteBuffer) {
        ...
        while (i < utf16Length) {
            ...
            if (c < '\u0080') {
                ...
                byteBuffer.put((byte) ((c & 63) | CivicLocationElement.SCRIPT));
            } else if (c < '\ud800' || '\udfff' < c) {
                byteBuffer.put((byte) ((c >>> 12) | WifiQualifiedNetworkSelector.LAST_SELECTION_AWARD));
                byteBuffer.put((byte) (((c >>> 6) & 63) | CivicLocationElement.SCRIPT));
                byteBuffer.put((byte) ((c & 63) | CivicLocationElement.SCRIPT));
            } else {
                if (i + 1 != sequence.length()) {
                    ...
                    if (Character.isSurrogatePair(c, low)) {
                        ...
                        byteBuffer.put((byte) (((codePoint >>> 12) & 63) | CivicLocationElement.SCRIPT));
                        byteBuffer.put((byte) (((codePoint >>> 6) & 63) | CivicLocationElement.SCRIPT));
                        byteBuffer.put((byte) ((codePoint & 63) | CivicLocationElement.SCRIPT));
                    }
                }
    ...
    private static int encode(CharSequence sequence, byte[] bytes, int offset, int length) {
        ...
        while (i < utf16Length) {
            ...
            if (c < '\u0080' && j2 < limit) {
                ...
                j2 = j + 1;
                bytes[j] = (byte) ((c & 63) | CivicLocationElement.SCRIPT);
                j = j2;
            } else if ((c < '\ud800' || '\udfff' < c) && j2 <= limit - 3) {
                j = j2 + 1;
                bytes[j2] = (byte) ((c >>> 12) | WifiQualifiedNetworkSelector.LAST_SELECTION_AWARD);
                j2 = j + 1;
                bytes[j] = (byte) (((c >>> 6) & 63) | CivicLocationElement.SCRIPT);
                j = j2 + 1;
                bytes[j2] = (byte) ((c & 63) | CivicLocationElement.SCRIPT);
            } else if (j2 <= limit - 4) {
                if (i + 1 != sequence.length()) {
                    i++;
                    char low = sequence.charAt(i);
                    if (Character.isSurrogatePair(c, low)) {
                        ...
                        j2 = j + 1;
                        bytes[j] = (byte) (((codePoint >>> 12) & 63) | CivicLocationElement.SCRIPT);
                        j = j2 + 1;
                        bytes[j2] = (byte) (((codePoint >>> 6) & 63) | CivicLocationElement.SCRIPT);
                        j2 = j + 1;
                        bytes[j] = (byte) ((codePoint & 63) | CivicLocationElement.SCRIPT);
                        j = j2;
                    }
                }
                throw new IllegalArgumentException("Unpaired surrogate at index " + (i - 1));
    ...
    public void writeRawVarint32(int value) throws IOException {
        while ((value & WifiNetworkScoreCache.INVALID_NETWORK_SCORE) != 0) {
            writeRawByte((value & SupportedRates.MASK) | CivicLocationElement.SCRIPT);
            value >>>= 7;
        }
        writeRawByte(value);
    }
    ...
    public void writeRawVarint64(long value) throws IOException {
        while ((-128 & value) != 0) {
            writeRawByte((((int) value) & SupportedRates.MASK) | CivicLocationElement.SCRIPT);
            value >>>= 7;
        }

...
public class OffHostService {
    ...
    public void setBanner(int bannerResId) throws NotFoundException {
        ...
        if (this.mBannerResId > 0) {
            try {
                ...
                Log.e(TAG, "NotFoundException : " + ne.getMessage());
                throw ne;
            } catch (Exception e) {
                Log.e(TAG, "Exception : " + e.getMessage());
            }
    ...
    public void commit() throws InsufficientResourcesException {
        ...
        Log.d(TAG, "do not commit, no aid");
    }

    private boolean hasAids() {
        int i = 0;
        boolean hasAids = false;
        if (getAidGroups() == null) {
            return false;
        }
        AidGroup[] groups = getAidGroups();
        int length = groups.length;
        while (i < length) {
            if (groups[i].getAidList().size() > 0) {
                hasAids = true;
            }
            i++;
        }
        Log.d(TAG, "hasAids:" + hasAids);
        return hasAids;
    }
    ...
    private ArrayList<AidGroup> convertToOffHostAidGroupList(List<AidGroup> mAidGroups) {
        ...
        for (AidGroup mCeAidGroup : mAidGroups) {
            ...
            if (mCeAidGroup.getDescription() == null) {
                ...
                description = mCeAidGroup.getDescription();
            }
            AidGroup mAidGroup = defineAidGroup(description, mCeAidGroup.getCategory());

...
/*
        Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: com.huawei.android.hardware.fmradio.FmReceiver.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: com.huawei.android.hardware.fmradio.FmReceiver.<clinit>():void
...
*/
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: com.huawei.android.hardware.fmradio.FmReceiver.<clinit>():void");
    }
    ...
    public boolean setSignalThreshold(int threshold) {
        ...
        if (!(TAVARUA_BUF_EVENTS == null || FmReceiverWrapper.setControlNative(sFd, V4L2_CID_PRIVATE_TAVARUA_SIGNAL_TH, rssiLev) == 0)) {
            bStatus = false;
        }
        return bStatus;
    }
    ...
    public FmRxRdsData getPSInfo() {
        ...
        FmReceiverWrapper.getBufferNative(sFd, buff, TAVARUA_BUF_PS_RDS);
        this.mRdsData.setPrgmId(((buff[TAVARUA_BUF_RT_RDS] & IccConstantsEx.RUIM_SMS_BEARER_DATA_LEN) << FM_RX_SRCHLIST_MODE_STRONGEST) | (buff[TAVARUA_BUF_PS_RDS] & IccConstantsEx.RUIM_SMS_BEARER_DATA_LEN));
    ...
    public FmRxRdsData getRTInfo() {
        byte[] buff = new byte[STD_BUF_SIZE];
        FmReceiverWrapper.getBufferNative(sFd, buff, TAVARUA_BUF_RT_RDS);
    ...
    public int[] getAFInfo() {
        ...
        FmReceiverWrapper.getBufferNative(sFd, buff, TAVARUA_BUF_AF_LIST);
        ...
        Log.d(TAG, "AF_buff 0: " + (buff[TAVARUA_BUF_SRCH_LIST] & IccConstantsEx.RUIM_SMS_BEARER_DATA_LEN));
        Log.d(TAG, "AF_buff 1: " + (buff[TAVARUA_BUF_EVENTS] & IccConstantsEx.RUIM_SMS_BEARER_DATA_LEN));
        Log.d(TAG, "AF_buff 2: " + (buff[TAVARUA_BUF_RT_RDS] & IccConstantsEx.RUIM_SMS_BEARER_DATA_LEN));
        Log.d(TAG, "AF_buff 3: " + (buff[TAVARUA_BUF_PS_RDS] & IccConstantsEx.RUIM_SMS_BEARER_DATA_LEN));
        Log.d(TAG, "AF_buff 4: " + (buff[TAVARUA_BUF_RAW_RDS] & IccConstantsEx.RUIM_SMS_BEARER_DATA_LEN));
        for (byte i = (byte) 0; i < buff[TAVARUA_BUF_RAW_RDS]; i += TAVARUA_BUF_EVENTS) {
            AfList[i] = ((buff[i + TAVARUA_BUF_RAW_RDS] & IccConstantsEx.RUIM_SMS_BEARER_DATA_LEN) * MotionTypeApps.TYPE_TAKE_OFF) + lowerBand;
    ...
    public boolean getInternalAntenna() {
        if (FmReceiverWrapper.getControlNative(sFd, V4L2_CID_PRIVATE_TAVARUA_ANTENNA) == TAVARUA_BUF_EVENTS) {
            return true;
        }
        return false;
    }
    ...
    public byte[] getRawRDS(int numBlocks) {
        ...
        int re = FmReceiverWrapper.getRawRdsNative(sFd, rawRds, numBlocks * TAVARUA_BUF_PS_RDS);

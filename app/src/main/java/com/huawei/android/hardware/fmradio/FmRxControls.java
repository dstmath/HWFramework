...
class FmRxControls {
    ...
    public void fmOff(int fd) {
        FmReceiverWrapper.setControlNative(fd, V4L2_CID_PRIVATE_TAVARUA_STATE, SEEK_FORWARD);
    }

    public void muteControl(int fd, boolean on) {
        if (on) {
            FmReceiverWrapper.setControlNative(fd, V4L2_CID_AUDIO_MUTE, SCAN_BACKWARD);
        } else {
            FmReceiverWrapper.setControlNative(fd, V4L2_CID_AUDIO_MUTE, SEEK_FORWARD);
        }
    }
    ...
    public int[] stationList(int fd) {
        ...
        FmReceiverWrapper.getBufferNative(fd, sList, SEEK_FORWARD);
        ...
        for (int i = SEEK_FORWARD; i < station_num; i += SEEK_BACKWARD) {
            ...
            int tmpFreqByte1 = sList[(i * SCAN_FORWARD) + SEEK_BACKWARD] & IccConstantsEx.RUIM_SMS_BEARER_DATA_LEN;
            int tmpFreqByte2 = sList[(i * SCAN_FORWARD) + SCAN_FORWARD] & IccConstantsEx.RUIM_SMS_BEARER_DATA_LEN;
    ...
    public int stereoControl(int fd, boolean stereo) {
        if (stereo) {
            return FmReceiverWrapper.setMonoStereoNative(fd, SEEK_BACKWARD);
        }
        return FmReceiverWrapper.setMonoStereoNative(fd, SEEK_FORWARD);
    }
    ...
    public int setLowPwrMode(int fd, boolean lpmOn) {
        if (lpmOn) {
            return FmReceiverWrapper.setControlNative(fd, V4L2_CID_PRIVATE_TAVARUA_LP_MODE, SEEK_BACKWARD);
        }
        return FmReceiverWrapper.setControlNative(fd, V4L2_CID_PRIVATE_TAVARUA_LP_MODE, SEEK_FORWARD);
    }

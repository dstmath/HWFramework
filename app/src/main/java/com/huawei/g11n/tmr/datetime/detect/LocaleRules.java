...
public class LocaleRules {
    private HashMap<String, HashMap<Integer, String>> locale_rules;
    private HashMap<Integer, String> rules;

    public LocaleRules(Rules rules) {
        this.rules = null;
        this.locale_rules = null;
        this.rules = rules.getRules();
        init();
    }

    public HashMap<Integer, String> getLocaleRules(String str) {
        HashMap<Integer, String> hashMap = new HashMap();
        if (this.locale_rules.containsKey(str)) {
            hashMap.putAll((Map) this.locale_rules.get(str));
        }
        hashMap.putAll(getOthers(str));
        return hashMap;
    }
    ...
    private HashMap<Integer, String> getOthers(String str) {
        HashMap<Integer, String> hashMap = new HashMap();
        List asList = Arrays.asList(new String[]{"zh_hans", "ja", "ko"});
        Integer valueOf = Integer.valueOf(31002);
        if (!asList.contains(str)) {
            hashMap.put(valueOf, (String) this.rules.get(valueOf));
        }
        return hashMap;
    }
}

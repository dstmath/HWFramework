        ...
        this.rules = new HashMap<Integer, String>() {
            {
                ...
                put(Integer.valueOf(907), "(\\bam|\\bpm)?\\s*(\\d{1,2})[\\s.:]+(\\d{2})([\\s.:]+(\\d{2}))?\\s*(\\bam|\\bpm)?[\\s(]*([regex_zzzz])?[\\s)]*");
                put(Integer.valueOf(908), "(\\bam|\\bpm)?\\s*(\\d{1,2}).\\s*(\\d{2})?(.\\s*(\\d{2}))?.?\\s*\\(?([regex_zzzz])?");
                put(Integer.valueOf(909), "(\\bam|\\bpm)?\\s*(\\d{1,2})\\s*[^0-9+-]+\\s*(\\d{2})(\\s*[^0-9+-]+\\s*(\\d{2}))?[^0-9+-]+([regex_zzzz])?");
                put(Integer.valueOf(910), "(([param_EEEE]|[param_E])\\s*)?(([regex_zzzz])\\s*)?(\\bam|\\bpm)?\\s*(\\d{1,2}[param_digit])(\u6642|\u65f6|\u70b9|\u9ede|\u0433\u0430\u0434\u0437\u0456\u043d|\\s)+(\\d{1,2}[param_timesuf][param_digit])?[\\s\u5206]*((\\d{1,2}[param_digit])[\\s\u79d2]+)?");
                put(Integer.valueOf(911), "(2[0-4]|[0-1]?[0-9])[:.]([0-5][0-9]|60)([:.]([0-5][0-9]|60))?");
                put(Integer.valueOf(912), "(\\b(am|pm)\\b)?\\s*(\\d{1,2})");
                put(Integer.valueOf(913), "(([regex_zzzz])\\s*)?(\\bam|\\bpm)\\s*(\\d{1,2})[\\s.:]+(\\d{2})([\\s.:]+(\\d{2}))?");

...
class PduHeaders {
    ...
    protected void setOctet(int value, int field) throws InvalidHeaderValueException {
        switch (field) {
            ...
            case PRIORITY /*143*/:
                if (value < VALUE_YES || value > PRIORITY_HIGH) {
                    throw new InvalidHeaderValueException("Invalid Octet value!");
                }
            case RESPONSE_STATUS /*146*/:
                ...
                if ((value <= RESPONSE_STATUS_ERROR_PERMANENT_LACK_OF_PREPAID || value > STORE_STATUS_ERROR_END) && value >= VALUE_YES && (value <= RESPONSE_STATUS_ERROR_UNSUPPORTED_MESSAGE || value >= STORE_STATUS_ERROR_TRANSIENT_FAILURE)) {
                    if (value > STORE_STATUS_ERROR_END) {
                    }
                }
            ...
            case RETRIEVE_STATUS /*153*/:
                if (value <= RETRIEVE_STATUS_ERROR_TRANSIENT_NETWORK_PROBLEM || value >= STORE_STATUS_ERROR_PERMANENT_FAILURE) {
                    if (value > RETRIEVE_STATUS_ERROR_PERMANENT_CONTENT_UNSUPPORTED && value <= STORE_STATUS_ERROR_END) {
                        value = STORE_STATUS_ERROR_PERMANENT_FAILURE;
                        break;
                    }
                    if (value >= VALUE_YES && (value <= VALUE_YES || value >= STORE_STATUS_ERROR_TRANSIENT_FAILURE)) {
                        if (value > STORE_STATUS_ERROR_END) {
                        }
                    }
                    value = STORE_STATUS_ERROR_PERMANENT_FAILURE;
            ...
            case REPLY_CHARGING /*156*/:
                if (value < VALUE_YES || value > REPLY_CHARGING_ACCEPTED_TEXT_ONLY) {
                    throw new InvalidHeaderValueException("Invalid Octet value!");
                }
            case MM_STATE /*163*/:
                if (value < VALUE_YES || value > MM_STATE_FORWARDED) {
                    throw new InvalidHeaderValueException("Invalid Octet value!");
                }
    ...
    protected void setTextString(byte[] value, int field) {
        ...
        switch (field) {
            case REPLY_CHARGING_ACCEPTED_TEXT_ONLY /*131*/:
            case MM_STATE_FORWARDED /*132*/:

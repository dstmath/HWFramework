...
public class CylinderTransition extends BaseTransition {
    ...
    public boolean transform(int part, boolean isOverScrollFirst, boolean isOverScrollLast, float scrollProgress, View child) {
        ...
        this.mRadius = (((float) rect.width()) * RADIUS_SCALE) / 2.0f;
        ...
        float tz = this.mRadius * (1.0f - FloatMath.cos(DEGREE_TO_RADIAN * angle));
        float tx = (((this.mRadius * FloatMath.sin(DEGREE_TO_RADIAN * angle)) + ((float) rect.left)) + (((float) rect.width()) / 2.0f)) - clipCenter;
        ...
        if (Math.abs(scrollProgress) > RADIUS_SCALE) {

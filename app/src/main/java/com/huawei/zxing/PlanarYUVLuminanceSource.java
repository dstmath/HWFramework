...
public final class PlanarYUVLuminanceSource extends LuminanceSource {
    ...
    public int[] renderThumbnail() {
        ...
        for (int y = 0; y < height; y++) {
            ...
            for (int x = 0; x < width; x++) {
                pixels[outputOffset + x] = (65793 * (yuv[(x * THUMBNAIL_SCALE_FACTOR) + inputOffset] & IccConstantsEx.RUIM_SMS_BEARER_DATA_LEN)) | CoverManager.DEFAULT_COLOR;
            }
    ...
    public LuminanceSource rotateCounterClockwise() {
        ...
        for (i = 0; i < this.dataWidth; i += THUMBNAIL_SCALE_FACTOR) {
            for (j = 0; j < this.dataHeight / THUMBNAIL_SCALE_FACTOR; j++) {
                yuvRData[k] = this.yuvData[((this.dataWidth * j) + wh) + i];
                yuvRData[k + 1] = this.yuvData[(((this.dataWidth * j) + wh) + i) + 1];
                k += THUMBNAIL_SCALE_FACTOR;
            }
        }

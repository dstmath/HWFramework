...
/*
        Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: com.huawei.zxing.oned.Code128Reader.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: com.huawei.zxing.oned.Code128Reader.<clinit>():void
...
*/
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: com.huawei.zxing.oned.Code128Reader.<clinit>():void");
    }
    ...
    public Result decodeRow(int rowNumber, BitArray row, Map<DecodeHintType, ?> hints) throws NotFoundException, FormatException, ChecksumException {
        ...
        int codeSet;
        if (hints != null) {
            containsKey = hints.containsKey(DecodeHintType.ASSUME_GS1);
        } else {
            containsKey = false;
        }
        ...
        if (!row.isRange(nextStart, Math.min(row.getSize(), ((nextStart - lastStart) / 2) + nextStart), false)) {
            ...
            r32 = new ResultPoint[2];
            r32[0] = new ResultPoint(left, (float) rowNumber);
            r32[1] = new ResultPoint(right, (float) rowNumber);
            return new Result(stringBuilder2, rawBytes, r32, BarcodeFormat.CODE_128);
        }
    }
}

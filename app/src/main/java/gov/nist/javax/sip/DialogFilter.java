...
class DialogFilter implements ServerRequestInterface, ServerResponseInterface {
    ...
    private void sendServerInternalErrorResponse(SIPRequest sipRequest, SIPServerTransaction transaction) {
        ...
        Response sipResponse = sipRequest.createResponse(Response.SERVER_INTERNAL_ERROR);
    ...
    public void processRequest(SIPRequest sipRequest, MessageChannel incomingMessageChannel) {
        ...
        if (!(dialog == null || sipProvider == dialog.getSipProvider())) {
            ...
            if (contact != null) {
                ...
                if (contactPort == -1) {
                    if (!contactTransport.equals(ParameterNames.UDP)) {
                        if (!contactTransport.equals(ParameterNames.TCP)) {
                            contactPort = SIPConstants.DEFAULT_TLS_PORT;
                        }
                    }
                    contactPort = SIPConstants.DEFAULT_PORT;
                }
                if (ipAddress != null) {
                    if (ipAddress.equals(this.listeningPoint.getIPAddress())) {
                    }
                    if (sipStack.isLoggingEnabled()) {
        ...
        if (!(sipRequest.getHeader(RouteHeader.NAME) == null || transaction.getDialog() == null)) {
            ...
            if (uri.getHostPort().hasPort()) {
                ...
                if (this.listeningPoint.getTransport().equalsIgnoreCase(ListeningPoint.TLS)) {
                    port = SIPConstants.DEFAULT_TLS_PORT;
                } else {
                    port = SIPConstants.DEFAULT_PORT;
                }
            }
            String host = uri.getHost();
            if (!host.equals(this.listeningPoint.getIPAddress())) {
            }
            if (port == this.listeningPoint.getPort()) {
                if (routes.size() == 1) {
        ...
        if (sipRequest.getMethod().equals(Request.REFER) && dialog != null && sipProvider.isDialogErrorsAutomaticallyHandled()) {
            ...
            if (lastTransaction != null && sipProvider.isDialogErrorsAutomaticallyHandled()) {
                ...
                if (lastTransaction instanceof SIPServerTransaction) {
                    if (!dialog.isAckSeen() && lastRequest.getMethod().equals(TokenNames.INVITE)) {
                    ...
                    if (lastRequest.getMethod().equals(TokenNames.INVITE) && !dialog.isAckSent(cseqno)) {
                        sendRequestPendingResponse(sipRequest, transaction);
                        return;
                    }
                }
            ...
            if (transaction == null || !transaction.isInviteTransaction()) {
                ...
                if (dialog == null) {
                    ...
                    if (sipRequest.getMethod().equals(TokenNames.INVITE) && sipProvider.isDialogErrorsAutomaticallyHandled()) {
        ...
        } else if (sipRequest.getMethod().equals(TokenNames.BYE)) {
                ...
                if (st != null && st.getState() == SIPTransaction.TERMINATED_STATE) {
                    ...
                    try {
                        ...
                        if (ex222.getCause() != null) {
                            if (ex222.getCause() instanceof IOException) {
        ...
        } else if (sipRequest.getMethod().equals(TokenNames.INVITE)) {
            if (dialog == null) {
            ...
            if (dialog == null || transaction == null || r24 == null || sipRequest.getCSeq().getSeqNumber() <= dialog.getRemoteSeqNumber() || !(r24 instanceof SIPServerTransaction) || !sipProvider.isDialogErrorsAutomaticallyHandled() || !dialog.isSequnceNumberValidation() || !r24.isInviteTransaction() || r24.getState() == TransactionState.COMPLETED || r24.getState() == TransactionState.TERMINATED || r24.getState() == TransactionState.CONFIRMED) {
                ...
                if (dialog != null && sipProvider.isDialogErrorsAutomaticallyHandled() && lastTransaction != null && lastTransaction.isInviteTransaction() && (lastTransaction instanceof ClientTransaction) && lastTransaction.getLastResponse() != null && lastTransaction.getLastResponse().getStatusCode() == 200) {
                    if (!dialog.isAckSent(lastTransaction.getLastResponse().getCSeq().getSeqNumber())) {
                        if (sipStack.isLoggingEnabled()) {
        ...
        if (!(dialog == null || transaction == null || sipRequest.getMethod().equals(TokenNames.BYE) || sipRequest.getMethod().equals(Request.CANCEL) || sipRequest.getMethod().equals(TokenNames.ACK) || sipRequest.getMethod().equals(Request.PRACK))) {
        ...
        if (dialog == null && sipRequest.getMethod().equals(TokenNames.NOTIFY)) {
            ...
            if (sipProvider.isAutomaticDialogSupportEnabled() && pendingSubscribeClientTx == null && !sipStack.deliverUnsolicitedNotify) {
                ...
                if (!(subscriptionDialog == null || subscriptionDialog.getDialogId() == null)) {
                    if (subscriptionDialog.getDialogId().equals(dialogId)) {
                ...
                if (subscriptionDialog == null && (sipProvider.isAutomaticDialogSupportEnabled() || pendingSubscribeClientTx.getDefaultDialog() != null)) {
                    if (sipStack.isEventForked(((Event) sipRequest.getHeader(EventHeader.NAME)).getEventType())) {
        ...
        if (this.listeningPoint == null) {
            ...
            if (sipProvider == null) {
                ...
                if (transaction == null) {
                    ...
                    if (response.getCSeqHeader().getMethod().equals(TokenNames.INVITE)) {

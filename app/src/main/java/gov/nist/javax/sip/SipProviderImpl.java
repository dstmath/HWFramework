...
public class SipProviderImpl implements SipProvider, SipProviderExt, SIPTransactionEventListener, SIPDialogEventListener {
            ...
            if (sipRequest.getTransaction() != null) {
                ...
                try {
                    ...
                    if (sipRequest.getTopmostVia().getBranch() != null && sipRequest.getTopmostVia().getBranch().startsWith(SIPConstants.BRANCH_MAGIC_COOKIE)) {
                        if (this.sipStack.findTransaction((SIPRequest) request, false) != null) {
                        ...
                        try {
                            ...
                            if (SIPTransactionStack.isDialogCreated(request.getMethod())) {
                                if (dialog != null) {
                                    ...
                                    ct.setDialog(sipStackImpl.createDialog(ct), sipRequest.getDialogId(false));
                                }
                            } else if (dialog != null) {
                                ct.setDialog(dialog, sipRequest.getDialogId(false));
                            }
    ...
    public ServerTransaction getNewServerTransaction(Request request) throws TransactionAlreadyExistsException, TransactionUnavailableException {
        if (this.sipStack.isAlive()) {
            ...
            try {
                sipRequest.checkHeaders();
                if (request.getMethod().equals(TokenNames.ACK)) {
                    if (this.sipStack.isLoggingEnabled()) {
                ...
                } else if (sipRequest.getMethod().equals(TokenNames.NOTIFY) && sipRequest.getFromTag() != null && sipRequest.getToTag() == null && this.sipStack.findSubscribeTransaction(sipRequest, (ListeningPointImpl) getListeningPoint()) == null && !this.sipStack.deliverUnsolicitedNotify) {
    ...
    public void sendRequest(Request request) throws SipException {
        if (this.sipStack.isAlive()) {
            if (((SIPRequest) request).getRequestLine() != null && request.getMethod().equals(TokenNames.ACK)) {

...
/*
        Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: gov.nist.javax.sip.message.SIPMessage.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: gov.nist.javax.sip.message.SIPMessage.<clinit>():void
...
*/
    ...
    public byte[] encodeAsBytes(String transport) {
        ...
        try {
            ((ViaHeader) getHeader(ViaHeader.NAME)).setTransport(transport);
        } catch (Exception e) {
    ...
    public SIPMessage() {
        this.contentEncodingCharset = MessageFactoryImpl.getDefaultContentEncodingCharset();
    ...
    public void setHeader(Header sipHeader) {
        ...
        try {
            if (header instanceof SIPHeaderList) {
                if (((SIPHeaderList) header).isEmpty()) {
                    return;
                }
            }
        ...
        if (via == null || via.getBranch() == null || !via.getBranch().toUpperCase().startsWith(SIPConstants.BRANCH_MAGIC_COOKIE_UPPER_CASE)) {
            ...
            if (via != null) {
                ...
                if (!via.getSentBy().hasPort()) {
                    retval.append("-").append(SIPConstants.DEFAULT_PORT);
                }
            }

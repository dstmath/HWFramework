...
/*
        Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: gov.nist.javax.sip.message.SIPRequest.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: gov.nist.javax.sip.message.SIPRequest.<clinit>():void
...
*/
    ...
    public void checkHeaders() throws ParseException {
        ...
        if (getCSeq() == null) {
            ...
            if (getMethod().equals(TokenNames.NOTIFY)) {
                ...
                } else if (getHeader(EventHeader.NAME) == null) {
                    throw new ParseException(prefix + EventHeader.NAME, 0);
                }
            } else if (getMethod().equals(TokenNames.PUBLISH) && getHeader(EventHeader.NAME) == null) {
                throw new ParseException(prefix + EventHeader.NAME, 0);
            }
            if (this.requestLine.getMethod().equals(TokenNames.INVITE) || this.requestLine.getMethod().equals(TokenNames.SUBSCRIBE) || this.requestLine.getMethod().equals(Request.REFER)) {
                if (getContactHeader() == null && getToTag() == null) {
                    ...
                    if (TokenNames.SIPS.equalsIgnoreCase(((SipUri) this.requestLine.getUri()).getScheme())) {
                        SipUri sipUri = (SipUri) getContactHeader().getAddress().getURI();
                        if (!sipUri.getScheme().equals(TokenNames.SIPS)) {
                            throw new ParseException("Scheme for contact should be sips:" + sipUri, 0);
                        }
                    }
                }
            }
            if (getContactHeader() == null && (getMethod().equals(TokenNames.INVITE) || getMethod().equals(Request.REFER) || getMethod().equals(TokenNames.SUBSCRIBE))) {
    ...
    protected void setDefaults() {
        if (this.requestLine != null) {
            ...
            if (method != null) {
                ...
                if (u != null) {
                    if ((method.compareTo(TokenNames.REGISTER) == 0 || method.compareTo(TokenNames.INVITE) == 0) && (u instanceof SipUri)) {
    ...
    public SIPRequest createAckRequest(To responseToHeader) {
        ...
        newRequest.setMethod(TokenNames.ACK);
        Iterator headerIterator = getHeaders();
        while (headerIterator.hasNext()) {
            SIPHeader nextHeader = (SIPHeader) headerIterator.next();
    ...
    public final SIPRequest createErrorAck(To responseToHeader) throws SipException, ParseException {
        ...
        newRequest.getCSeq().setMethod(TokenNames.ACK);
        if (getRouteHeaders() != null) {
    ...
    public SIPRequest createSIPRequest(RequestLine requestLine, boolean switchHeaders) {
        ...
        while (headerIterator.hasNext()) {
            ...
            if (nextHeader instanceof CSeq) {
                ...
                if (nextHeader instanceof MaxForwards) {
                }
            }
            try {
                newRequest.attachHeader(nextHeader, false);

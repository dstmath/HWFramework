...
public abstract class MessageChannel {
    ...
    protected final String createBadReqRes(String badReq, ParseException pe) {
        ...
        if (!copyViaHeaders(badReq, buf) || !copyHeader(CSeqHeader.NAME, badReq, buf) || !copyHeader(CallIdHeader.NAME, badReq, buf) || !copyHeader(FromHeader.NAME, badReq, buf) || !copyHeader(ToHeader.NAME, badReq, buf)) {
            return null;
        }
        ...
        if (!(this instanceof UDPMessageChannel) || (((buf.length() + clength) + ContentTypeHeader.NAME.length()) + ": message/sipfrag\r\n".length()) + ContentLengthHeader.NAME.length() < 1300) {

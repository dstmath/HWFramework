...
public class SIPClientTransaction extends SIPTransaction implements ServerResponseInterface, ClientTransaction, ClientTransactionExt {
    ...
    public void sendMessage(SIPMessage messageToSend) throws IOException {
        try {
            ...
            if ((TransactionState.PROCEEDING == getState() || TransactionState.CALLING == getState()) && transactionRequest.getMethod().equals(TokenNames.ACK)) {
                if (isReliable()) {
            ...
            if (getState() == null) {
                ...
                if (transactionRequest.getMethod().equals(TokenNames.INVITE)) {
    ...
    public void sendRequest() throws SipException {
        ...
        try {
            ...
            if (getMethod().equals(TokenNames.SUBSCRIBE) && sipRequest.getHeader(ExpiresHeader.NAME) == null && this.sipStack.isLoggingEnabled()) {
            ...
            try {
                ...
                if (getOriginalRequest().getMethod().equals(Request.CANCEL) && this.sipStack.isCancelClientTransactionChecked()) {
                    ...
                    } else if (!ct.getMethod().equals(TokenNames.INVITE)) {
                        throw new SipException("Cannot cancel non-invite requests RFC 3261 9.1");
                    }
                } else if (getOriginalRequest().getMethod().equals(TokenNames.BYE) || getOriginalRequest().getMethod().equals(TokenNames.NOTIFY)) {
        ...
        try {
            if (getState() != null && this.isMapped) {
                ...
                if (this.lastRequest != null) {
                    if (this.sipStack.generateTimeStampHeader && this.lastRequest.getHeader(TimeStampHeader.NAME) != null) {
    ...
    protected void fireTimeoutTimer() {
        ...
        if (dialog == null || !(dialog.getState() == null || dialog.getState() == DialogState.EARLY)) {
            if (dialog != null && getOriginalRequest().getMethod().equalsIgnoreCase(TokenNames.BYE) && dialog.isTerminatedOnBye()) {
                dialog.delete();
            }
            if (TransactionState.COMPLETED != getState()) {
                setState(TransactionState.TERMINATED);
    ...
    public Request createCancel() throws SipException {
        ...
        } else if (!originalRequest.getMethod().equals(TokenNames.INVITE)) {
            throw new SipException("Only INIVTE may be cancelled");
        } else if (originalRequest.getMethod().equalsIgnoreCase(TokenNames.ACK)) {
    ...
    public Request createAck() throws SipException {
        ...
        if (originalRequest == null) {
            throw new SipException("bad state " + getState());
        } else if (getMethod().equalsIgnoreCase(TokenNames.ACK)) {
                    ...
                    synchronized (this) {
                        if (this.defaultDialog != null) {
                            if (sipResponse.getFromTag() != null) {
                                ...
                                if (dialogResponse == null || (method.equals(TokenNames.SUBSCRIBE) && dialogResponse.getCSeq().getMethod().equals(TokenNames.NOTIFY) && defaultDialogId.equals(dialogId))) {
    ...
    public void setDialog(SIPDialog sipDialog, String dialogId) {
        ...
        if (this.defaultDialog == null) {
            ...
            if (getMethod().equals(TokenNames.INVITE) && getSIPStack().maxForkTime != 0) {

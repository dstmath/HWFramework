...
public class IconBitmapUtils {
    ...
    private static String TAG_ICON = null;
    private static int VALID_TRANSPARENCY = 0;
    private static final float VFACTOR = 1.2f;

    static {
        /* JADX: method processing error */
/*
        Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: huawei.android.hwutil.IconBitmapUtils.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: huawei.android.hwutil.IconBitmapUtils.<clinit>():void
...
*/
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: huawei.android.hwutil.IconBitmapUtils.<clinit>():void");
    }
    ...
    public static void mask(int[] pixels, int w, int h, Bitmap mask) {
        if (mask != null) {
            ...
            for (int i = (w * h) - 1; i >= 0; i--) {
                pixels[i] = ((((pixels[i] >>> 24) * (maskPixels[i] >>> 24)) / PduHeaders.STORE_STATUS_ERROR_END) << 24) | (pixels[i] & 16777215);
            }
        }
    }
    ...
    public static Bitmap mask(Bitmap bmpSrc, Bitmap bmpMask) {
        ...
        try {
            ...
            for (int i = (w * h) - 1; i >= 0; i--) {
                maskPixels[i] = ((((maskPixels[i] >>> 24) * (srcPixels[i] >>> 24)) / PduHeaders.STORE_STATUS_ERROR_END) << 24) | (srcPixels[i] & 16777215);
            }
    ...
    private static void drawBackground(Canvas canvas, Bitmap background, int w, int h, int[] pixels) {
        ...
        for (int i = (bw * bh) - 1; i >= 0; i--) {
            ...
            backPixels[i] = (((CoverManager.DEFAULT_COLOR & backPixel) | ((((16711680 & backPixel) * r) >>> 8) & 16711680)) | ((((65280 & backPixel) * g) >>> 8) & 65280)) | ((((backPixel & PduHeaders.STORE_STATUS_ERROR_END) * b) >>> 8) & PduHeaders.STORE_STATUS_ERROR_END);
        }
    ...
    private static float getSaturationFactor(int hue, float factor) {
        if (((float) hue) <= SMALLER_HUE) {
            return HwFragmentMenuItemView.ALPHA_PRESSED * factor;
        }
        return factor;
    }

...
public class HwSplineOverScrollerImpl implements IHwSplineOverScroller {
    ...
    public HwSplineOverScrollerImpl(SplineOverScroller sos, Context context) {
        ...
        this.mLastDistanceActual = 0.0d;
        this.mAdjustBallisticDuration = 0;
        this.mSos = sos;
        this.mSpringbackInterpolator = AnimationUtils.loadInterpolator(context, 34209802);
        this.mOvershootInterpolator = AnimationUtils.loadInterpolator(context, 34209801);
    ...
    public int adjustBallisticVelocity(int originalVelocity, float acceleration, int maxOver) {
        float ratio = (((float) Math.abs(originalVelocity)) * HwFragmentMenuItemView.ALPHA_NORMAL) / ((float) this.mMaximumVelocity);
    ...
    public int getSplineFlingDuration(int orignDurtion, int velocity, double decelerationRate, float flingFriction, float physicalCoeff) {
        ...
        return (int) ((((double) getSplineFlingDurationModify(velocity)) * 1000.0d) * Math.exp(Math.log((double) ((((float) Math.abs(velocity)) * INFLEXION_DURTION) / (flingFriction * physicalCoeff))) / (decelerationRate - 1.0d)));
    }
}

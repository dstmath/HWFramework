...
public class NumberPicker extends android.widget.NumberPicker {
    ...
    public NumberPicker(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        this.mEdgeOffset = context.getResources().getDimensionPixelSize(34472059);
    ...
    protected void setSelectorColor(int i, int currentOffset, int initOffset, int index, int height, Paint paint) {
        ...
        if (i == 0 || this.mSelectorIndices.length - 1 == i) {
            paint.setAlpha(MotionTypeApps.TYPE_PICKUP_REDUCE_CLOCK);
        }
    }
    ...
    private void initialNumberPicker(Context context, AttributeSet attrs) {
        ...
        this.mSelectorOffset = res.getDimensionPixelSize(34472056);
        this.mSelectorTextSize = (float) res.getDimensionPixelSize(34472057);
        this.mNormalTextSize = (float) res.getDimensionPixelSize(34472058);
        this.mSelectorTextColor = res.getColor(33882270);
        this.mIsDarkHwTheme = HwWidgetFactory.isHwDarkTheme(context);
        if (this.mIsDarkHwTheme) {
            this.mSmallTextColor = res.getColor(33882196);
            this.mNormalTextColor = res.getColor(33882197);
            return;
        }
        this.mSmallTextColor = res.getColor(33882191);
        this.mNormalTextColor = res.getColor(33882192);
    }

    private void handleFireList() {
        getInputText().setTextSize(0, this.mSelectorTextSize);
        for (int i = 0; i < this.mFireList.size(); i++) {

...
public class HwActionMenuItemView extends ActionMenuItemView {
    ...
    public void updateTextButtonVisibility() {
        ...
        if (getParent() != null) {
            CharSequence title;
            boolean visible = !TextUtils.isEmpty(getTitle());
            if (getIcon() != null) {
    ...
    private boolean showHwTextWithAction() {
        ...
        if (menuView != null) {
            ...
            if (container != null && ((View) container).getId() == 16909293) {
                return true;
            }
        }
        return false;
    ...
    private void updateTextAndIcon() {
        if (getParent() != null) {
            ...
            if (getItemData().isChecked()) {
                HwWidgetFactory.setImmersionStyle(getContext(), this, 33882394, 33882393, 0, false);
            } else {
    ...
    protected int getMaxIconSize() {
        int maxIconSize = getContext().getResources().getDimensionPixelSize(34472095);
        if (((int) ((this.mDensity * -1.0f) + HwFragmentMenuItemView.ALPHA_PRESSED)) == maxIconSize) {

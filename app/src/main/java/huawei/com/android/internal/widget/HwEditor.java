...
public class HwEditor extends Editor {
    ...
    private boolean mCursorShouldMoveEnd;
    private int mCursorTop;
    private float mCursorX;
    private float mInsertHanlePosX;
    private boolean mPosIsLineEnd;
    ...
    public HwEditor(TextView textView) {
        ...
        this.mInsertHanlePosX = -1.0f;
        this.mCursorTop = -1;
        this.mCursorBottom = -1;
        this.mCursorX = -1.0f;
    }
    ...
    private int adjustOffsetAtLineEnd(int offset, boolean isTouchPos) {
        ...
        if (!(-1.0f == getPos(isTouchPos) || layout == null)) {
            boolean offsetNearEnd;
            int currLine = layout.getLineForOffset(offset);
            if (offset != layout.getLineEnd(currLine) - 1 || currLine == layout.getLineCount() - 1) {
                offsetNearEnd = false;
            } else {
                offsetNearEnd = true;
            }
    ...
    protected boolean adjustHandlePos(int[] coordinate, HandleView handleView, Layout layout, int offset, int line) {
        ...
        if (selectionEndFlag) {
            selectionEndFlag = !((SelectionHandleView) handleView).isStartHandle();
        }
        ...
        if (line > 0 && this.mPosIsLineEnd && insertionFlag) {
            ...
            this.mCursorShouldMoveEnd = true;
            return true;
        } else if (line <= 0 || !r3 || offset != layout.getLineStart(line)) {
            return false;
        } else {

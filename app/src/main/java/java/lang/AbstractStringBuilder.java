...
abstract class AbstractStringBuilder implements CharSequence {
    ...
    public AbstractStringBuilder append(boolean b) {
        if (b) {
            ...
            this.value[this.count + 2] = UnicodeLocaleExtension.SINGLETON;
    ...
    public AbstractStringBuilder reverse() {
        ...
        for (int j = (n - 1) >> 1; j >= 0; j--) {
            ...
            if (!hasSurrogate) {
                hasSurrogate = (temp < Character.MIN_SURROGATE || temp > Character.MAX_SURROGATE) ? temp2 >= Character.MIN_SURROGATE && temp2 <= Character.MAX_SURROGATE : true;
            }
            this.value[j] = temp2;
            this.value[n - j] = temp;
        }

        ...
        public void run() {
            ...
            while (true) {
                ...
                synchronized (AbstractPreferences.eventQueue) {
                    while (true) {
                        try {
                            if (AbstractPreferences.eventQueue.isEmpty()) {
...
/*
        Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: java.util.prefs.AbstractPreferences.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: java.util.prefs.AbstractPreferences.<clinit>():void
...
*/
    ...
    public void put(String key, String value) {
        ...
        } else if (value.length() > Preferences.MAX_VALUE_LENGTH) {
    ...
    public boolean getBoolean(String key, boolean def) {
        ...
        if (value.equalsIgnoreCase(Unpacker.TRUE)) {
            return true;
        }
        if (value.equalsIgnoreCase(Unpacker.FALSE)) {
            return false;
        }
        return result;
    }
    ...
    private void flush2() throws BackingStoreException {
        synchronized (this.lock) {
            ...
            AbstractPreferences[] cachedKids = cachedChildren();
            for (AbstractPreferences flush2 : cachedKids) {
                flush2.flush2();
            }
        }
    }

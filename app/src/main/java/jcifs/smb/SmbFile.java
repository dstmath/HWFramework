                ...
                while (true) {
                    try {
                        ...
                        if (this.n == -1) {
                            ...
                            this.reqx.setParam(this.dest.fid, this.off, this.n, this.b, SmbFile.HASH_DOT_DOT, this.n);
                            this.dest.send(this.reqx, this.resp);
                        } else {
                            this.req.setParam(this.dest.fid, this.off, this.n, this.b, SmbFile.HASH_DOT_DOT, this.n);
...
/*
        Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: jcifs.smb.SmbFile.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: jcifs.smb.SmbFile.<clinit>():void
...
*/
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: jcifs.smb.SmbFile.<clinit>():void");
    }
    ...
    void doFindFirstNext(ArrayList list, boolean files, String wildcard, int searchAttributes, SmbFilenameFilter fnf, SmbFileFilter ff) throws SmbException, UnknownHostException, MalformedURLException {
        ...
        while (true) {
            for (int i = HASH_DOT_DOT; i < resp.numEntries; i += TYPE_FILESYSTEM) {
                FileEntry e = resp.results[i];
                String name = e.getName();
                if (name.length() < O_RDWR) {
                    int h = name.hashCode();
                    if (h != HASH_DOT) {
                    }
                    if (!name.equals(".")) {
    ...
    void copyTo0(SmbFile dest, byte[][] b, int bsize, WriterThread w, SmbComReadAndX req, SmbComReadAndXResponse resp) throws SmbException {
        ...
        int i;
        if (isDirectory()) {
            if (dest.getUncPath0().length() > TYPE_FILESYSTEM) {
                try {
                    dest.mkdir();
        ...
        open(TYPE_FILESYSTEM, HASH_DOT_DOT, ATTR_NORMAL, HASH_DOT_DOT);
        try {
            dest.open(82, 258, this.attributes, HASH_DOT_DOT);
        } catch (Throwable ie) {
            throw new SmbException(dest.url.toString(), ie);
        } catch (SmbAuthException sae) {
            if ((dest.attributes & TYPE_FILESYSTEM) != 0) {
        ...
        while (true) {
            ...
            resp.setParam(b[i], HASH_DOT_DOT);
            send(req, resp);
            synchronized (w) {
                if (w.e != null) {
                    break;
                ...
                if (w.e != null) {
                    ...
                    i = i == TYPE_FILESYSTEM ? HASH_DOT_DOT : TYPE_FILESYSTEM;

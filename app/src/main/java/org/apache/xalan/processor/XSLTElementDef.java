...
public class XSLTElementDef {
    ...
    XSLTElementDef(XSLTSchema schema, String namespace, String name, String nameAlias, XSLTElementDef[] elements, XSLTAttributeDef[] attributes, XSLTElementProcessor contentHandler, Class classObject) {
        ...
        if (namespace.equals(Constants.S_XSLNAMESPACEURL) || namespace.equals(Constants.S_EXTENSIONS_XALANLIB_URL) || namespace.equals(Constants.S_BUILTIN_OLD_EXTENSIONS_URL)) {
    ...
    XSLTElementProcessor getProcessorFor(String uri, String localName) {
        ...
        for (int i = 0; i < n; i += T_ELEMENT) {
            XSLTElementDef def = this.m_elements[i];
            if (def.m_name.equals(PsuedoNames.PSEUDONAME_OTHER)) {
    ...
    XSLTAttributeDef getAttributeDef(String uri, String localName) {
        ...
        for (int k = 0; k < nAttrDefs; k += T_ELEMENT) {
            XSLTAttributeDef attrDef = attrDefs[k];
            String uriDef = attrDef.getNamespace();
            String nameDef = attrDef.getName();
            if (nameDef.equals(PsuedoNames.PSEUDONAME_OTHER) && (equalsMayBeNullOrZeroLen(uri, uriDef) || (uriDef != null && uriDef.equals(PsuedoNames.PSEUDONAME_OTHER) && uri != null && uri.length() > 0))) {
                return attrDef;
            }
            if (nameDef.equals(PsuedoNames.PSEUDONAME_OTHER) && uriDef == null) {
                defaultDef = attrDef;
    ...
    String getRequiredElem() {
        ...
        String s = SerializerConstants.EMPTYSTRING;
        boolean first = true;

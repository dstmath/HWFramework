...
/*
        Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: org.apache.xalan.templates.ElemNumber.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: org.apache.xalan.templates.ElemNumber.<clinit>():void
...
*/
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: org.apache.xalan.templates.ElemNumber.<clinit>():void");
    }
    ...
    int findAncestor(XPathContext xctxt, XPath fromMatchPattern, XPath countMatchPattern, int context, ElemNumber namespaceContext) throws TransformerException {
        ...
        while (-1 != context && ((fromMatchPattern == null || fromMatchPattern.getMatchScore(xctxt, context) == XPath.MATCH_SCORE_NONE) && (countMatchPattern == null || countMatchPattern.getMatchScore(xctxt, context) == XPath.MATCH_SCORE_NONE))) {
    ...
    private int findPrecedingOrAncestorOrSelf(XPathContext xctxt, XPath fromMatchPattern, XPath countMatchPattern, int context, ElemNumber namespaceContext) throws TransformerException {
        DTM dtm = xctxt.getDTM(context);
        while (-1 != context) {
            if (fromMatchPattern != null && fromMatchPattern.getMatchScore(xctxt, context) != XPath.MATCH_SCORE_NONE) {
                return -1;
            }
            if (countMatchPattern != null && countMatchPattern.getMatchScore(xctxt, context) != XPath.MATCH_SCORE_NONE) {
                return context;
            }
    ...
    String getCountString(TransformerImpl transformer, int sourceNode) throws TransformerException {
        ...
        if (this.m_valueExpr != null) {
            double d_count = Math.floor(this.m_valueExpr.execute(xctxt, sourceNode, (PrefixResolver) this).num() + XPath.MATCH_SCORE_OTHER);
            if (Double.isNaN(d_count)) {
                return Constants.ATTRVAL_NAN;
            }
            ...
            if (3 == this.m_level) {
                list = new long[1];
                list[0] = (long) ctable.countNode(xctxt, this, sourceNode);
            } else {
        ...
        return list != null ? formatNumberList(transformer, list, sourceNode) : SerializerConstants.EMPTYSTRING;
    }
        ...
        if (3 == this.m_level) {
            ...
            while (-1 != pos) {
                ...
                if (-1 == next) {
                    ...
                    if (-1 != next && (!(fromMatchPattern == null || fromMatchPattern.getMatchScore(xctxt, next) == XPath.MATCH_SCORE_NONE) || dtm.getNodeType(next) == (short) 9)) {
                        return -1;
                    }
                }
                int child = next;
                ...
                if (-1 != next) {
                    ...
                    if (countMatchPattern.getMatchScore(xctxt, pos) != XPath.MATCH_SCORE_NONE) {
                        return pos;
                    }
                }
            }
    ...
    NodeVector getMatchingAncestors(XPathContext xctxt, int node, boolean stopAtFirstFound) throws TransformerException {
        ...
        while (-1 != node && (this.m_fromMatchPattern == null || this.m_fromMatchPattern.getMatchScore(xctxt, node) == XPath.MATCH_SCORE_NONE || stopAtFirstFound)) {
            if (countMatchPattern == null) {
    ...
    Locale getLocale(TransformerImpl transformer, int contextNode) throws TransformerException {
        ...
        Locale locale = new Locale(langValue.toUpperCase(), SerializerConstants.EMPTYSTRING);
        if (locale != null) {
            return locale;
        }
    ...
    private DecimalFormat getNumberFormatter(TransformerImpl transformer, int contextNode) throws TransformerException {
        String evaluate;
        String digitGroupSepValue = null;
        ...
        if (!(digitGroupSepValue == null || this.m_groupingSeparator_avt.isSimple() || digitGroupSepValue.length() == 1)) {
            transformer.getMsgMgr().warn(this, XSLTErrorResources.WG_ILLEGAL_ATTRIBUTE_VALUE, new Object[]{Keywords.FUNC_NAME_STRING, this.m_groupingSeparator_avt.getName()});
        }
        if (this.m_groupingSize_avt != null) {
            evaluate = this.m_groupingSize_avt.evaluate(transformer.getXPathContext(), contextNode, this);
        } else {
            evaluate = null;
        }
        if (digitGroupSepValue == null || evaluate == null || digitGroupSepValue.length() <= 0) {
            return null;
        }
        try {
            formatter = (DecimalFormat) NumberFormat.getNumberInstance(locale);
            formatter.setGroupingSize(Integer.valueOf(evaluate).intValue());
    ...
    private void getFormattedNumber(TransformerImpl transformer, int contextNode, char numberType, int numberWidth, long listElement, FastStringBuffer formattedNumber) throws TransformerException {
        ...
        switch (numberType) {
            case Constants.TATTRNAME_VALUE /*65*/:
            ...
            case '\u03b1':
                thisBundle = XResourceBundle.loadResourceBundle(XResourceBundle.LANG_BUNDLE_NAME, new Locale("el", SerializerConstants.EMPTYSTRING));
            ...
            case '\u0430':
                thisBundle = XResourceBundle.loadResourceBundle(XResourceBundle.LANG_BUNDLE_NAME, new Locale("cy", SerializerConstants.EMPTYSTRING));
                if (evaluate == null || !evaluate.equals(Constants.ATTRVAL_TRADITIONAL)) {
                    int2alphaCount(listElement, (CharArrayWrapper) thisBundle.getObject(XResourceBundle.LANG_ALPHABET), formattedNumber);
                } else {
                    formattedNumber.append(tradAlphaCount(listElement, thisBundle));
            ...
            case '\u05d0':
                thisBundle = XResourceBundle.loadResourceBundle(XResourceBundle.LANG_BUNDLE_NAME, new Locale("he", SerializerConstants.EMPTYSTRING));
                if (evaluate == null || !evaluate.equals(Constants.ATTRVAL_TRADITIONAL)) {
                    int2alphaCount(listElement, (CharArrayWrapper) thisBundle.getObject(XResourceBundle.LANG_ALPHABET), formattedNumber);
                } else {
                    formattedNumber.append(tradAlphaCount(listElement, thisBundle));
            ...
            case '\u0e51':
                thisBundle = XResourceBundle.loadResourceBundle(XResourceBundle.LANG_BUNDLE_NAME, new Locale("th", SerializerConstants.EMPTYSTRING));
                if (evaluate == null || !evaluate.equals(Constants.ATTRVAL_TRADITIONAL)) {
                    int2alphaCount(listElement, (CharArrayWrapper) thisBundle.getObject(XResourceBundle.LANG_ALPHABET), formattedNumber);
                } else {
                    formattedNumber.append(tradAlphaCount(listElement, thisBundle));
            ...
            case '\u10d0':
                thisBundle = XResourceBundle.loadResourceBundle(XResourceBundle.LANG_BUNDLE_NAME, new Locale("ka", SerializerConstants.EMPTYSTRING));
                if (evaluate == null || !evaluate.equals(Constants.ATTRVAL_TRADITIONAL)) {
                    int2alphaCount(listElement, (CharArrayWrapper) thisBundle.getObject(XResourceBundle.LANG_ALPHABET), formattedNumber);
                } else {
                    formattedNumber.append(tradAlphaCount(listElement, thisBundle));
    ...
    protected String tradAlphaCount(long val, XResourceBundle thisBundle) {
        if (val > Long.MAX_VALUE) {
            ...
            return XPATHErrorResources.ERROR_STRING;
        }
        int charPos;
        char[] table;
        int j;
    ...
    protected String long2roman(long val, boolean prefixesAreOK) {
        ...
        String roman = SerializerConstants.EMPTYSTRING;
        int place = 0;
        if (val <= 3999) {
            while (true) {
            ...
            roman = XPATHErrorResources.ERROR_STRING;
        }
        return roman;
    }

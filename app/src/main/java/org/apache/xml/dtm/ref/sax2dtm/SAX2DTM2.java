...
public class SAX2DTM2 extends SAX2DTM {
    ...
    public class AncestorIterator extends InternalAxisIteratorBase {
        ...
        public AncestorIterator() {
            super();
            this.m_ancestors = new int[m_blocksize];
            this.m_size = 0;
        }
...
/*
        Error: jadx.core.utils.exceptions.DecodeException: Load method exception in method: org.apache.xml.dtm.ref.sax2dtm.SAX2DTM2.<clinit>():void
...
Caused by: jadx.core.utils.exceptions.DecodeException:  in method: org.apache.xml.dtm.ref.sax2dtm.SAX2DTM2.<clinit>():void
...
*/
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: org.apache.xml.dtm.ref.sax2dtm.SAX2DTM2.<clinit>():void");
    }
    ...
    public SAX2DTM2(DTMManager mgr, Source source, int dtmIdentity, DTMWSFilter whiteSpaceFilter, XMLStringFactory xstringfactory, boolean doIndexing, int blocksize, boolean usePrevsib, boolean buildIdIndex, boolean newNameTable) {
        ...
        while (true) {
            ...
            if (blocksize != 0) {
                ...
                this.m_values = new Vector(32, DTMDefaultBase.DEFAULT_BLOCKSIZE);
                this.m_maxNodeIndex = WalkerFactory.BIT_CHILD;
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
        ...
        if (!this.m_pastFirstElement) {
            String prefix = SerializerConstants.XML_PREFIX;
            String declURL = JAXPPrefixResolver.S_XMLNAMESPACEURI;
        ...
        if (this.m_textPendingStart >= 0) {
            ...
            if (doStrip) {
                ...
                if (length > TEXT_LENGTH_MAX || this.m_textPendingStart > TEXT_OFFSET_MAX) {
                    ...
                    this.m_previous = addNode(this.m_coalescedTextType, 3, this.m_parents.peek(), this.m_previous, length + (this.m_textPendingStart << TEXT_LENGTH_BITS), false);
                }
            }
    ...
    public XMLString getStringValue(int nodeHandle) {
        ...
        if (type == 1 || type == 9) {
            ...
            if (dataIndex >= 0) {
                if (this.m_xstrf != null) {
                    return this.m_xstrf.newstr(this.m_chars, dataIndex >>> TEXT_LENGTH_BITS, dataIndex & TEXT_LENGTH_MAX);
                }
                return new XMLStringDefault(this.m_chars.getString(dataIndex >>> TEXT_LENGTH_BITS, dataIndex & TEXT_LENGTH_MAX));
    ...
    public final String getStringValueX(int nodeHandle) {
        ...
        if (type == 1 || type == 9) {
            ...
            dataIndex = this.m_dataOrQName.elementAt(identity);
            if (dataIndex >= 0) {
                return this.m_chars.getString(dataIndex >>> TEXT_LENGTH_BITS, dataIndex & TEXT_LENGTH_MAX);
            }
    ...
    public final void dispatchCharactersEvents(int nodeHandle, ContentHandler ch, boolean normalize) throws SAXException {
        ...
        if (identity != -1) {
            ...
            if (type == 1 || type == 9) {
                ...
                if (dataIndex >= 0) {
                    if (normalize) {
                        this.m_chars.sendNormalizedSAXcharacters(ch, dataIndex >>> TEXT_LENGTH_BITS, dataIndex & TEXT_LENGTH_MAX);
                    } else {
                        this.m_chars.sendSAXcharacters(ch, dataIndex >>> TEXT_LENGTH_BITS, dataIndex & TEXT_LENGTH_MAX);
                    }
                } else if (normalize) {
    ...
    protected final void copyTextNode(int nodeID, SerializationHandler handler) throws SAXException {
        if (nodeID != -1) {
            ...
            if (dataIndex >= 0) {
                this.m_chars.sendSAXcharacters(handler, dataIndex >>> TEXT_LENGTH_BITS, dataIndex & TEXT_LENGTH_MAX);
            } else {

...
class Lexer {
        ...
        while (i < nChars) {
            ...
            switch (c) {
                ...
                case FunctionTable.FUNC_LANG /*32*/:
                    ...
                    continue;
                case FunctionTable.FUNC_EXT_FUNCTION_AVAILABLE /*33*/:
                ...
                case Constants.TATTRNAME_TOSTRING /*61*/:
                ...
                case OpCodes.FROM_PARENT /*45*/:
                    break;
                case Constants.TATTRNAME_SEQUENCESRC /*58*/:
                    if (i > 0) {
                        if (posOfNSSep == i - 1) {
    ...
    private boolean mapPatternElemPos(int nesting, boolean isStart, boolean isAttrName) {
        ...
        if (!isStart) {
            ...
            iArr[i2] = iArr[i2] + StringToIntTable.INVALID_KEY;
        }
        iArr = this.m_patternMap;
    ...
    private int getTokenQueuePosFromMap(int i) {
        ...
        return pos >= TARGETEXTRA ? pos + StringToIntTable.INVALID_KEY : pos;
    }
        ...
        if (this.m_processor.lookahead('(', 1)) {
            switch (getKeywordToken(this.m_processor.m_token)) {
                case OpCodes.NODETYPE_ROOT /*35*/:
                    ...
                    return;
                case OpCodes.NODETYPE_ANYELEMENT /*36*/:
                    targetStrings.addElement(PsuedoNames.PSEUDONAME_OTHER);
                    return;
    ...
    private int mapNSTokens(String pat, int startSubstring, int posOfNSSep, int posOfScan) throws TransformerException {
        ...
        if (this.m_namespaceContext == null || prefix.equals(PsuedoNames.PSEUDONAME_OTHER) || prefix.equals(SerializerConstants.XMLNS_PREFIX)) {
            uName = prefix;
        } else {
            try {
                if (prefix.length() > 0) {
        ...
        if (uName == null || uName.length() <= 0) {
            this.m_processor.errorForDOM3(XPATHErrorResources.ER_PREFIX_MUST_RESOLVE, new String[]{prefix});
        } else {

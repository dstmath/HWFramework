...
public class FuncSystemProperty extends FunctionOneArg {
    ...
    public XObject execute(XPathContext xctxt) throws TransformerException {
        ...
        String str = null;
        String propName = SerializerConstants.EMPTYSTRING;
        Properties xsltInfo = new Properties();
        loadPropertyFile(XSLT_PROPERTIES, xsltInfo);
        if (indexOfNSSep > 0) {
            String namespace = xctxt.getNamespaceContext().getNamespaceForPrefix(indexOfNSSep >= 0 ? fullName.substring(0, indexOfNSSep) : SerializerConstants.EMPTYSTRING);
            if (indexOfNSSep < 0) {
                propName = fullName;
            } else {
        ...
        if (!propName.equals(org.apache.xalan.templates.Constants.ATTRNAME_VERSION) || str.length() <= 0) {

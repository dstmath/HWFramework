...
public class IPAddressName implements GeneralNameInterface {
    ...
    public IPAddressName(byte[] address) throws IOException {
        if (address.length == 4 || address.length == 8) {
            this.isIPv4 = true;
        } else if (address.length == MASKSIZE || address.length == 32) {
            this.isIPv4 = false;
        } else {
    ...
    private void parseIPv6(String name) throws IOException {
        ...
        System.arraycopy(InetAddress.getByName(name.substring(0, slashNdx)).getAddress(), 0, this.address, 0, (int) MASKSIZE);
        int prefixLen = Integer.parseInt(name.substring(slashNdx + 1));
        if (prefixLen > Pattern.CANON_EQ) {
            throw new IOException("IPv6Address prefix is longer than 128");
        }
        int i;
        BitArray bitArray = new BitArray((int) Pattern.CANON_EQ);
        for (i = 0; i < prefixLen; i++) {
            bitArray.set(i, true);
        }
        byte[] maskArray = bitArray.toByteArray();
        for (i = 0; i < MASKSIZE; i++) {
            this.address[i + MASKSIZE] = maskArray[i];
        }
    }
    ...
    public String getName() throws IOException {
        ...
        if (this.isIPv4) {
            ...
            host = new byte[MASKSIZE];
            System.arraycopy(this.address, 0, host, 0, (int) MASKSIZE);
            this.name = InetAddress.getByAddress(host).getHostAddress();
            if (this.address.length == 32) {
                int i;
                byte[] maskBytes = new byte[MASKSIZE];
                for (i = MASKSIZE; i < 32; i++) {
                    maskBytes[i - 16] = this.address[i];
                }
                BitArray ba = new BitArray(Pattern.CANON_EQ, maskBytes);
                i = 0;
                while (i < Pattern.CANON_EQ && ba.get(i)) {
                    i++;
                }
                this.name += "/" + i;
                while (i < Pattern.CANON_EQ) {
                    if (ba.get(i)) {
